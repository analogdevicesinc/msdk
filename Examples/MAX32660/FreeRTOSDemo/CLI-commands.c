    ...
    char* pcWriteBuffer, size_t xWriteBufferLen, const char* pcCommandString)
{
    ...
    pcWriteBuffer += snprintf(pcWriteBuffer, xWriteBufferLen,
        ...
        ticks / portTICK_PERIOD_MS, MXC_RTC->sec);
#else
    ...
    char* pcWriteBuffer, size_t xWriteBufferLen, const char* pcCommandString)
{
    ...
    if (pcParameter != NULL) {
        if (pcParameter[0] == '0') {
            ...
            pcWriteBuffer += snprintf(
                pcWriteBuffer, xWriteBufferLen, "Must supply 0 (Disable) or 1 (Enable)\r\n");
        }
    } else {
        pcWriteBuffer += snprintf(
            pcWriteBuffer, xWriteBufferLen, "Must supply 0 (Disable) or 1 (Enable)\r\n");
