 ...
 *          connected between P1.0 (UART 2 RX) -> P2.7 (UART 3 TX).
...
#warning "This example has been written for the MAX78000 FTHR board."
#endif
...
int main(void)
{
    ...
    // Initialize the UART
    error = MXC_UART_Init(TX_UART, UART_BAUD, MXC_UART_APB_CLK);
    if (error < E_NO_ERROR) {
    ...
    error = MXC_UART_Init(RX_UART, UART_BAUD, MXC_UART_APB_CLK);
    if (error < E_NO_ERROR) {
        printf("-->Error initializing UART: %d\n", error);
        printf("-->Example Failed\n");
        return error;
