...
static BaseType_t prvEraseCommand(
...
static BaseType_t prvWriteCommand(
    char* pcWriteBuffer, size_t xWriteBufferLen, const char* pcCommandString);
...
static const CLI_Command_Definition_t xWriteCommand = {
    "write",
    "\r\nwrite <word offset> <text>: \r\n Writes text string to flash starting at the 32-bit word "
...
static BaseType_t prvReadCommand(
    char* pcWriteBuffer, size_t xWriteBufferLen, const char* pcCommandString);
...
static const CLI_Command_Definition_t xReadCommand = {
    "read",
    "\r\nread <word offset> <number of letters>: \r\n Reads text from flash starting at the 32-bit "
...
static BaseType_t prvCRCCommand(
    char* pcWriteBuffer, size_t xWriteBufferLen, const char* pcCommandString);
    ...
    char* pcWriteBuffer, size_t xWriteBufferLen, const char* pcCommandString)
{
  ...
  write buffer length is adequate, so does not check for buffer overflows. */
    ...
    uint32_t crc = calculate_crc(
        (uint32_t*)FLASH_STORAGE_START_ADDR, MXC_FLASH_PAGE_SIZE / sizeof(uint32_t));
    portEXIT_CRITICAL();
    memset(pcWriteBuffer, 0x00, xWriteBufferLen);
