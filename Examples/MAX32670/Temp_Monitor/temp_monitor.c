...
#define TR_STORAGE_PAGE \
    1 //NOTE: Be sure this value is less than the total number of flash pages in the device.  \
...
int temp_monitor_init(void)
{
    ...
    if ((err = init_flash()) != E_NO_ERROR) { // Initialize flash space used to store temp readings
        ...
        return err;
               ...
               E_NO_ERROR) { // Initialize and start RTC, and set up TOD alarm
        return err;
    }
...
void temp_monitor_check_temp(void)
{
    ...
    if (temp_warning &&
        !(MXC_RTC->ctrl &
          MXC_RTC_INT_EN_SHORT)) { //Temp warning active and warning light inactive --> enable
        enable_warning_light();
    } else if (!temp_warning &&
               (MXC_RTC->ctrl &
                MXC_RTC_INT_EN_SHORT)) { //Temp warning inactive and warning light active --> disable
