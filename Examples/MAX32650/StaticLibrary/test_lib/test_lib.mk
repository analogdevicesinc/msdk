...
$(CUSTOM_LIB_OUT_DIR)/$(CUSTOM_LIB_OUT): $(CUSTOM_LIB_DIR)/*.c
	make -f $(abspath $(CUSTOM_LIB_DIR)/$(CUSTOM_LIB_NAME).make) lib \
		BUILD_DIR=$(CUSTOM_LIB_OUT_DIR) PROJECT_NAME=$(CUSTOM_LIB_NAME) \
		PROJ_CFLAGS="$(PROJ_CFLAGS)" PROJ_LDFLAGS="$(PROJ_LDFLAGS)" \
		MXC_OPTIMIZE_CFLAGS=$(MXC_OPTIMIZE_CFLAGS) \
		MFLOAT_ABI=$(MFLOAT_ABI) DUAL_CORE=$(DUAL_CORE) \
