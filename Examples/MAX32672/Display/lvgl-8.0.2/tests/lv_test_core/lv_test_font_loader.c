...
static int compare_fonts(lv_font_t* f1, lv_font_t* f2)
{
    ...
    for (int i = 0; i < dsc1->cmap_num; ++i) {
        ...
        if (cmaps1->unicode_list != NULL && cmaps2->unicode_list != NULL) {
            ...
            lv_test_assert_array_eq((uint8_t*)cmaps1->unicode_list, (uint8_t*)cmaps2->unicode_list,
        ...
        if (cmaps1->glyph_id_ofs_list != NULL && cmaps2->glyph_id_ofs_list != NULL) {
            ...
            lv_test_assert_ptr_eq(cmaps1->glyph_id_ofs_list, cmaps2->glyph_id_ofs_list,
                                  "glyph_id_ofs_list");
        }
    }
    ...
    if (dsc1->kern_classes == 1 && dsc2->kern_classes == 1) {
        ...
        if (kern1 != NULL && kern2 != NULL) {
            lv_test_assert_int_eq(kern1->right_class_cnt, kern2->right_class_cnt,
                                  "right_class_cnt");
            lv_test_assert_int_eq(kern1->left_class_cnt, kern2->left_class_cnt, "left_class_cnt");

            lv_test_assert_array_eq((uint8_t*)kern1->left_class_mapping,
                                    (uint8_t*)kern2->left_class_mapping, kern1->left_class_cnt,
                                    "left_class_mapping");

            lv_test_assert_array_eq((uint8_t*)kern1->right_class_mapping,
                                    (uint8_t*)kern2->right_class_mapping, kern1->right_class_cnt,
                                    "right_class_mapping");

            lv_test_assert_array_eq(
                (uint8_t*)kern1->class_pair_values, (uint8_t*)kern2->class_pair_values,
        ...
        if (kern1 != NULL && kern2 != NULL) {
            ...
            lv_test_assert_array_eq((uint8_t*)kern1->values, (uint8_t*)kern2->values,
    ...
    for (int i = 0; i < total_glyphs; ++i) {
        if (i < total_glyphs - 1) {
            ...
            if (size1 > 0) {
                lv_test_assert_array_eq(dsc1->glyph_bitmap + glyph_dsc1[i].bitmap_index,
