...
static void event_cb(lv_event_t* e)
{
    ...
    if (code == LV_EVENT_VALUE_CHANGED) {
        ...
        if (dsc->part == LV_PART_CURSOR && dsc->p1 && dsc->p2 && dsc->p1->y == dsc->p2->y &&
            last_id >= 0) {
...
void lv_example_chart_6(void)
{
    ...
    cursor =
        lv_chart_add_cursor(chart, lv_palette_main(LV_PALETTE_BLUE), LV_DIR_LEFT | LV_DIR_BOTTOM);

    ser = lv_chart_add_series(chart, lv_palette_main(LV_PALETTE_RED), LV_CHART_AXIS_PRIMARY_Y);
    uint32_t i;
    for (i = 0; i < 10; i++) { lv_chart_set_next_value(chart, ser, lv_rand(10, 90)); }
