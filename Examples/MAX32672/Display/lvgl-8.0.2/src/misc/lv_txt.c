                     ...
                     lv_text_flag_t flag)
{
    ...
    while (text[line_start] != '\0') {
        new_line_start +=
            _lv_txt_get_next_line(&text[line_start], font, letter_space, max_width, flag);
        ...
        lv_coord_t act_line_length = lv_txt_get_width(
            &text[line_start], new_line_start - line_start, font, letter_space, flag);
                               ...
                               lv_coord_t max_width, lv_text_flag_t flag)
{
    ...
    while (txt[i] != '\0' && max_width > 0) {
        uint32_t word_w  = 0;
        uint32_t advance = lv_txt_get_next_word(&txt[i], font, letter_space, max_width, flag,
...
void _lv_txt_ins(char* txt_buf, uint32_t pos, const char* ins_txt)
{
    ...
    pos =
        _lv_txt_encoded_get_byte_id(txt_buf, pos); /*Convert to byte index instead of letter index*/
