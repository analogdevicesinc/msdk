...
static lv_base_dir_t get_next_run(const char* txt, lv_base_dir_t base_dir, uint32_t max_len,
                                  uint32_t* len, uint16_t* pos_conv_len);
static void rtl_reverse(char* dest, const char* src, uint32_t len, uint16_t* pos_conv_out,
                        uint16_t pos_conv_rd_base, uint16_t pos_conv_len);
static uint32_t char_change_to_pair(uint32_t letter);
static lv_base_dir_t bracket_process(const char* txt, uint32_t next_pos, uint32_t len,
...
void _lv_bidi_process(const char* str_in, char* str_out, lv_base_dir_t base_dir)
{
    ...
    while (str_in[par_start] != '\0') {
        ...
        _lv_bidi_process_paragraph(&str_in[par_start], &str_out[par_start], par_len, base_dir, NULL,
                                   0);
        par_start += par_len;
                                  ...
                                  lv_base_dir_t base_dir, uint32_t visual_pos, bool* is_rtl)
{
    ...
    _lv_bidi_process_paragraph(str_in, bidi_txt ? *bidi_txt : NULL, len, base_dir, pos_conv_buf,
                               pos_conv_len);
    ...
    if (rd) {
        if (base_dir == LV_BASE_DIR_LTR) {
            ...
            rtl_reverse(str_out ? &str_out[wr] : NULL, str_in, rd,
                        pos_conv_out ? &pos_conv_out[pos_conv_wr] : NULL, 0, pos_conv_rd);
        }
    }
        ...
        if (base_dir == LV_BASE_DIR_LTR) {
            if (run_dir == LV_BASE_DIR_LTR) {
                ...
                rtl_reverse(str_out ? &str_out[wr] : NULL, &str_in[rd], run_len,
                            pos_conv_out ? &pos_conv_out[pos_conv_wr] : NULL, pos_conv_rd,
                            pos_conv_run_len);
            wr += run_len;
    ...
    while (i) {
        ...
        if (lv_bidi_letter_is_weak(letter)) {
            ...
            while (i) {
                ...
                if (lv_bidi_letter_is_weak(letter) == false && letter != '.' && letter != ',' &&
                    letter != '$' && letter != '%') {
