...
void lv_arc_set_start_angle(lv_obj_t* obj, uint16_t start)
{
    ...
    /*Only a smaller incremental move*/
        ...
        inv_arc_area(obj, LV_MIN(arc->indic_angle_start, start),
                     LV_MAX(arc->indic_angle_start, start), LV_PART_INDICATOR);
    }
    /*Only a smaller incremental move*/
    else if (arc->indic_angle_start < arc->indic_angle_end && start < arc->indic_angle_end) {
        inv_arc_area(obj, LV_MIN(arc->indic_angle_start, start),
...
void lv_arc_set_end_angle(lv_obj_t* obj, uint16_t end)
{
    ...
    if (end > (arc->indic_angle_start + 360))
        end = arc->indic_angle_start + 360;
    ...
    /*Only a smaller incremental move*/
        ...
        inv_arc_area(obj, LV_MIN(arc->indic_angle_end, end), LV_MAX(arc->indic_angle_end, end),
                     LV_PART_INDICATOR);
    }
    /*Only a smaller incremental move*/
    else if (arc->indic_angle_end < arc->indic_angle_start && end < arc->indic_angle_start) {
...
void lv_arc_set_bg_start_angle(lv_obj_t* obj, uint16_t start)
{
    ...
    /*Only a smaller incremental move*/
        ...
        inv_arc_area(obj, LV_MIN(arc->bg_angle_start, start), LV_MAX(arc->bg_angle_start, start),
                     LV_PART_MAIN);
    }
    /*Only a smaller incremental move*/
    else if (arc->bg_angle_start < arc->bg_angle_end && start < arc->bg_angle_end) {
...
void lv_arc_set_bg_end_angle(lv_obj_t* obj, uint16_t end)
{
    ...
    if (end > (arc->bg_angle_start + 360))
        end = arc->bg_angle_start + 360;
    ...
    /*Only a smaller incremental move*/
        ...
        inv_arc_area(obj, LV_MIN(arc->bg_angle_end, end), LV_MAX(arc->bg_angle_end, end),
                     LV_PART_MAIN);
    }
    /*Only a smaller incremental move*/
    else if (arc->bg_angle_end < arc->bg_angle_start && end < arc->bg_angle_start) {
...
static void lv_arc_draw(lv_event_t* e)
{
    ...
    if (arc_r > 0) {
        ...
        lv_draw_arc(center.x, center.y, arc_r, arc->bg_angle_start + arc->rotation,
                    arc->bg_angle_end + arc->rotation, clip_area, &arc_dsc);
    ...
    if (indic_r > 0) {
        ...
        lv_draw_arc(center.x, center.y, indic_r, arc->indic_angle_start + arc->rotation,
                    arc->indic_angle_end + arc->rotation, clip_area, &arc_dsc);
...
static void inv_arc_area(lv_obj_t* obj, uint16_t start_angle, uint16_t end_angle, lv_part_t part)
{
    ...
    lv_coord_t rout =
        (LV_MIN(lv_obj_get_width(obj) - left - right, lv_obj_get_height(obj) - top - bottom)) / 2;
    ...
    if (start_quarter == end_quarter && start_angle <= end_angle) {
        if (start_quarter == 0) {
            ...
            inv_area.x2 =
                x + ((lv_trigo_sin(start_angle + 90) * rout) >> LV_TRIGO_SHIFT) + extra_area;
            ...
            inv_area.x2 =
                x + ((lv_trigo_sin(start_angle + 90) * rin) >> LV_TRIGO_SHIFT) + extra_area;

            inv_area.y1 = y + ((lv_trigo_sin(end_angle) * rin) >> LV_TRIGO_SHIFT) - extra_area;
            inv_area.x1 =
                x + ((lv_trigo_sin(end_angle + 90) * rout) >> LV_TRIGO_SHIFT) - extra_area;
        ...
        inv_area.y1 =
            y +
            ((LV_MIN(lv_trigo_sin(end_angle), lv_trigo_sin(start_angle)) * rin) >> LV_TRIGO_SHIFT) -
            extra_area;
        ...
        inv_area.x2 =
            x +
            ((LV_MAX(lv_trigo_sin(start_angle + 90), lv_trigo_sin(end_angle + 90)) * rin) >>
             LV_TRIGO_SHIFT) +
            extra_area;
        inv_area.y2 = y + ((lv_trigo_sin(start_angle) * rout) >> LV_TRIGO_SHIFT) + extra_area;
        lv_obj_invalidate_area(obj, &inv_area);
        ...
        inv_area.y2 = y +
                      (LV_MAX(lv_trigo_sin(end_angle) * rin, lv_trigo_sin(start_angle) * rin) >>
                       LV_TRIGO_SHIFT) +
                      extra_area;
        lv_obj_invalidate_area(obj, &inv_area);
        ...
        inv_area.x1 =
            x +
            ((LV_MIN(lv_trigo_sin(end_angle + 90), lv_trigo_sin(start_angle + 90)) * rin) >>
             LV_TRIGO_SHIFT) -
            extra_area;
        inv_area.y1 = y + ((lv_trigo_sin(start_angle) * rout) >> LV_TRIGO_SHIFT) - extra_area;
...
static void value_update(lv_obj_t* obj)
{
    ...
    switch (arc->type) {
        case LV_ARC_MODE_SYMMETRICAL:
            ...
            if (arc->value < range_midpoint) {
                angle = lv_map(arc->value, arc->min_value, range_midpoint, arc->bg_angle_start,
                               bg_midpoint);
