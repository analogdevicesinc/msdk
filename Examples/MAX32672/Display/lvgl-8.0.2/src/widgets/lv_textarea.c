 ...
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t lv_textarea_class = {.constructor_cb = lv_textarea_constructor,
...
void lv_textarea_add_char(lv_obj_t* obj, uint32_t c)
{
    ...
    if (char_is_accepted(obj, c_uni) == false) {
        LV_LOG_INFO("Character is not accepted by the text area (too long text or not in the "
                    "accepted list)");
        return;
    }
    ...
    if (ta->pwd_mode != 0) {
        ta->pwd_tmp = lv_mem_realloc(ta->pwd_tmp, strlen(ta->pwd_tmp) + strlen(letter_buf) +
                                                      1); /*+2: the new char + \0*/
