...
void lv_img_set_zoom(lv_obj_t* obj, uint16_t zoom)
{
    ...
    lv_coord_t h = lv_obj_get_height(obj);
    lv_area_t a;
    _lv_img_buf_get_transformed_area(&a, w, h, transf_angle, (transf_zoom * img->zoom) >> 8,
                                     &img->pivot);
...
static void draw_img(lv_event_t* e)
{
    ...
    if (code == LV_EVENT_COVER_CHECK) {
        ...
        if (zoom_final == LV_IMG_ZOOM_NONE) {
            ...
            lv_area_t a;
            _lv_img_buf_get_transformed_area(&a, lv_obj_get_width(obj), lv_obj_get_height(obj), 0,
        ...
        lv_area_t bg_coords;
        _lv_img_buf_get_transformed_area(&bg_coords, obj_w, obj_h, angle_final, zoom_final,
                                         &bg_pivot);
        ...
        if (code == LV_EVENT_DRAW_MAIN) {
            ...
            if (img->src_type == LV_IMG_SRC_FILE || img->src_type == LV_IMG_SRC_VARIABLE) {
                ...
                for (; coords_tmp.y1 < img_max_area.y2;
                     coords_tmp.y1 += img->h, coords_tmp.y2 += img->h) {
                    ...
                    for (; coords_tmp.x1 < img_max_area.x2;
                         coords_tmp.x1 += img->w, coords_tmp.x2 += img->w) {
