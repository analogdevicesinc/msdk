...
void lv_roller_set_visible_row_count(lv_obj_t* obj, uint8_t row_cnt)
{
    ...
    lv_obj_set_height(obj,
                      (lv_font_get_line_height(font) + line_space) * row_cnt + 2 * border_width);
}
    ...
    if (code == LV_EVENT_GET_SELF_SIZE) {
        ...
        if (indev_type == LV_INDEV_TYPE_ENCODER) {
            ...
            roller->sel_opt_id_ori =
                roller->sel_opt_id; /*Save the current value. Used to revert this state if
        ...
        if (c == LV_KEY_RIGHT || c == LV_KEY_DOWN) {
            if (roller->sel_opt_id + 1 < roller->option_cnt) {
                uint16_t ori_id =
                    roller->sel_opt_id_ori; /*lv_roller_set_selected will overwrite this*/
    ...
    /*Post draw when the children are drawn*/
        ...
        if (area_ok) {
            ...
            lv_txt_get_size(&res_p, lv_label_get_text(label), label_dsc.font,
                            label_dsc.letter_space, label_dsc.line_space, lv_obj_get_width(obj),
                            LV_TEXT_FLAG_EXPAND);
...
static void draw_label(lv_event_t* e)
{
    ...
    if (_lv_area_intersect(&clip2, clip_area, &clip2)) {
        lv_draw_label(&label_obj->coords, &clip2, &label_draw_dsc, lv_label_get_text(label_obj),
                      NULL);
    }
