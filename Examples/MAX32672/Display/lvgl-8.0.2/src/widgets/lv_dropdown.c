...
static void draw_box(lv_obj_t* dropdown_obj, const lv_area_t* clip_area, uint16_t id,
                     lv_state_t state);
static void draw_box_label(lv_obj_t* dropdown_obj, const lv_area_t* clip_area, uint16_t id,
                           lv_state_t state);
 ...
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t lv_dropdown_class = {.constructor_cb = lv_dropdown_constructor,
...
const lv_obj_class_t lv_dropdownlist_class = {.constructor_cb = lv_dropdownlist_constructor,
...
void lv_dropdown_open(lv_obj_t* dropdown_obj)
{
    ...
    if (align == LV_TEXT_ALIGN_AUTO) {
        if (lv_obj_get_style_base_dir(label, LV_PART_MAIN) == LV_BASE_DIR_RTL)
    ...
    switch (align) {
        ...
        case LV_TEXT_ALIGN_CENTER:
            lv_obj_set_x(label, lv_obj_get_content_width(dropdown->list) / 2 -
...
static void draw_main(lv_event_t* e)
{
    ...
    if (dropdown->symbol) {
        ...
        if (symbol_type == LV_IMG_SRC_SYMBOL) {
            lv_point_t size;
            lv_txt_get_size(&size, dropdown->symbol, symbol_dsc.font, symbol_dsc.letter_space,
                            symbol_dsc.line_space, LV_COORD_MAX, symbol_dsc.flag);
            symbol_w = size.x;
            symbol_h = size.y;
        } else {
    ...
    lv_point_t size;
    lv_txt_get_size(&size, opt_txt, label_dsc.font, label_dsc.letter_space, label_dsc.line_space,
        ...
        if (has_common) {
            if (dropdown->selected_highlight) {
                if (dropdown->pr_opt_id == dropdown->sel_opt_id) {
                    draw_box(dropdown_obj, &clip_area_core, dropdown->pr_opt_id,
                             LV_STATE_CHECKED | LV_STATE_PRESSED);
                    draw_box_label(dropdown_obj, &clip_area_core, dropdown->pr_opt_id,
                                   LV_STATE_CHECKED | LV_STATE_PRESSED);
                } else {
                    ...
                    draw_box_label(dropdown_obj, &clip_area_core, dropdown->sel_opt_id,
                                   LV_STATE_CHECKED);
                }
            } else {
                draw_box(dropdown_obj, &clip_area_core, dropdown->pr_opt_id, LV_STATE_PRESSED);
