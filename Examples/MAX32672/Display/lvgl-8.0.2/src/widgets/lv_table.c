...
void lv_table_set_cell_value_fmt(lv_obj_t* obj, uint16_t row, uint16_t col, const char* fmt, ...)
{
    ...
    table->cell_data[cell] =
        lv_mem_realloc(table->cell_data[cell], len + 2); /*+1: trailing '\0; +1: format byte*/
    LV_ASSERT_MALLOC(table->cell_data[cell]);
    if (table->cell_data[cell] == NULL) {
        va_end(ap2);
        return;
    ...
    lv_coord_t h = get_row_height(obj, row, font, letter_space, line_space, cell_left, cell_right,
...
void lv_table_set_row_cnt(lv_obj_t* obj, uint16_t row_cnt)
{
    ...
    table->cell_data =
        lv_mem_realloc(table->cell_data, table->row_cnt * table->col_cnt * sizeof(char*));
...
static void lv_table_event(const lv_obj_class_t* class_p, lv_event_t* e)
{
    ...
    if (code == LV_EVENT_STYLE_CHANGED) {
        ...
        if (table->col_act != LV_TABLE_CELL_NONE && table->row_act != LV_TABLE_CELL_NONE &&
            scroll_obj == NULL) {
    ...
    for (row = 0; row < table->row_cnt; row++) {
        ...
        for (col = 0; col < table->col_cnt; col++) {
            ...
            if (table->cell_data[cell]) {
                ...
                if (label_mask_ok) {
                    lv_draw_label(&txt_area, &label_mask, &label_dsc_act,
...
static void refr_size(lv_obj_t* obj, uint32_t strat_row)
{
    ...
    for (i = strat_row; i < table->row_cnt; i++) {
        table->row_h[i] = get_row_height(obj, i, font, letter_space, line_space, cell_left,
