    ...
    if (code == LV_EVENT_HIT_TEST) {
        ...
        if (type == LV_SLIDER_MODE_NORMAL || type == LV_SLIDER_MODE_SYMMETRICAL) {
            ...
            if (hor) {
                if ((base_dir != LV_BASE_DIR_RTL && p.x > slider->right_knob_area.x2) ||
                    ...
                    dist_left =
                        LV_ABS((slider->left_knob_area.x1 +
                                (slider->left_knob_area.x2 - slider->left_knob_area.x1) / 2) -
                               p.x);
                    dist_right =
                        LV_ABS((slider->right_knob_area.x1 +
                ...
                if (p.y < slider->right_knob_area.y1) {
                    ...
                    /*Calculate the distance from each knob*/
                    dist_left =
                        LV_ABS((slider->left_knob_area.y1 +
                                (slider->left_knob_area.y2 - slider->left_knob_area.y1) / 2) -
                               p.y);
                    dist_right =
                        LV_ABS((slider->right_knob_area.y1 +
        ...
        if (w >= h) {
            ...
            if (base_dir == LV_BASE_DIR_RTL) {
                new_value =
                    (obj->coords.x2 - bg_right) - p.x; /*Make the point relative to the indicator*/
            } else {
                new_value =
                    p.x - (obj->coords.x1 + bg_left); /*Make the point relative to the indicator*/
            }
            ...
            new_value =
                p.y - (obj->coords.y2 + bg_bottom); /*Make the point relative to the indicator*/
        ...
        lv_coord_t knob_size =
            LV_MIN(lv_obj_get_width(obj) + 2 * trans_w, lv_obj_get_height(obj) + 2 * trans_h) >> 1;
...
static void draw_knob(lv_event_t* e)
{
    ...
    if (hor) {
        if (!sym) {
            knob_area.x1 =
                LV_SLIDER_KNOB_COORD(hor, base_dir == LV_BASE_DIR_RTL, slider->bar.indic_area);
        } else {
