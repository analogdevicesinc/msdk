...
uint32_t lv_label_get_letter_on(const lv_obj_t* obj, lv_point_t* pos_in)
{
    ...
    if (txt[line_start + cid] == '\0') {
        ...
        bool is_rtl;
        logical_pos =
            _lv_bidi_get_logical_pos(&txt[line_start], NULL, txt_len,
...
bool lv_label_is_char_under_pos(const lv_obj_t* obj, lv_point_t* pos)
{
    ...
    if (align == LV_TEXT_ALIGN_CENTER) {
        lv_coord_t line_w;
        line_w = lv_txt_get_width(&txt[line_start], new_line_start - line_start, font, letter_space,
                                  flag);
        x += lv_area_get_width(&txt_coords) / 2 - line_w / 2;
    } else if (align == LV_TEXT_ALIGN_RIGHT) {
        lv_coord_t line_w;
...
static void draw_main(lv_event_t* e)
{
    ...
    if ((label->long_mode == LV_LABEL_LONG_SCROLL ||
        ...
        lv_point_t size;
        lv_txt_get_size(&size, label->text, label_draw_dsc.font, label_draw_dsc.letter_space,
    ...
    if (label->long_mode == LV_LABEL_LONG_SCROLL_CIRCULAR) {
        ...
        if (size.y > lv_area_get_height(&txt_coords)) {
            ...
            label_draw_dsc.ofs_y =
                label->offset.y + size.y + lv_font_get_line_height(label_draw_dsc.font);
...
static void lv_label_refr_text(lv_obj_t* obj)
{
    ...
    if (label->long_mode == LV_LABEL_LONG_SCROLL) {
        ...
        if (size.y > lv_area_get_height(&txt_coords) && hor_anim == false) {
            lv_anim_set_values(
                &a, 0, lv_area_get_height(&txt_coords) - size.y - (lv_font_get_line_height(font)));
    ...
    /*In roll inf. mode keep the size but start offset animations*/
        ...
        if (size.y <=
            lv_area_get_height(&txt_coords)) { /*No dots are required, the text is short enough*/
            label->dot_end = LV_LABEL_DOT_END_INV;
            ...
            y_overed =
                p.y % (lv_font_get_line_height(font) + line_space); /*Round down to the last line*/
