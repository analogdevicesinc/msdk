...
#define LV_LABEL_HINT_UPDATE_TH \
    1024 /*Update the "hint" if the label's y coordinates have changed more then this*/
 ...
 *  STATIC PROTOTYPES
 **********************/
LV_ATTRIBUTE_FAST_MEM static void draw_letter_normal(lv_coord_t pos_x, lv_coord_t pos_y,
                                                     lv_font_glyph_dsc_t* g,
                                                     const lv_area_t* clip_area,
                                                     const uint8_t* map_p, lv_color_t color,
                                                     lv_opa_t opa, lv_blend_mode_t blend_mode);
#if LV_DRAW_COMPLEX && LV_USE_FONT_SUBPX
static void draw_letter_subpx(lv_coord_t pos_x, lv_coord_t pos_y, lv_font_glyph_dsc_t* g,
                              const lv_area_t* clip_area, const uint8_t* map_p, lv_color_t color,
                              lv_opa_t opa, lv_blend_mode_t blend_mode);
#endif
                                         ...
                                         lv_draw_label_hint_t* hint)
{
    ...
    uint32_t line_end =
        line_start + _lv_txt_get_next_line(&txt[line_start], font, dsc->letter_space, w, dsc->flag);
    ...
    while (txt[line_start] != '\0') {
        ...
        _lv_bidi_process_paragraph(txt + line_start, bidi_txt, line_end - line_start, dsc->bidi_dir,
                                   NULL, 0);
#else
        ...
        while (i < line_end - line_start) {
            ...
            if (sel_start != 0xFFFF && sel_end != 0xFFFF) {
                ...
                logical_char_pos += _lv_bidi_get_logical_pos(bidi_txt, NULL, line_end - line_start,
                                                             dsc->bidi_dir, t, NULL);
#else
                                          ...
                                          lv_blend_mode_t blend_mode)
{
    ...
    if (pos_x + g.box_w < clip_area->x1 || pos_x > clip_area->x2 ||
        pos_y + g.box_h < clip_area->y1 || pos_y > clip_area->y2) {
        return;
    }
                                                     ...
                                                     lv_opa_t opa, lv_blend_mode_t blend_mode)
{
    ...
    if (opa < LV_OPA_MAX) {
        if (prev_opa != opa || prev_bpp != bpp) {
            ...
            for (i = 0; i < shades; i++) {
                opa_table[i] =
                    bpp_opa_table_p[i] == LV_OPA_COVER ? opa : ((bpp_opa_table_p[i] * opa) >> 8);
            }
        }
    ...
    for (row = row_start; row < row_end; row++) {
        ...
        /*Apply masks if any*/
        if (other_mask_cnt) {
            lv_draw_mask_res_t mask_res = lv_draw_mask_apply(
                mask_buf + mask_p_start, fill_area.x1, fill_area.y2, lv_area_get_width(&fill_area));
        ...
        if ((uint32_t)mask_p + (col_end - col_start) < mask_buf_size) {
            ...
            _lv_blend_fill(clip_area, &fill_area, color, mask_buf, LV_DRAW_MASK_RES_CHANGED,
                           LV_OPA_COVER, blend_mode);
                              ...
                              lv_opa_t opa, lv_blend_mode_t blend_mode)
{
    ...
    int32_t mask_buf_size =
        box_w * box_h > _LV_MASK_BUF_MAX_SIZE ? _LV_MASK_BUF_MAX_SIZE : g->box_w * g->box_h;
    lv_opa_t* mask_buf = lv_mem_buf_get(mask_buf_size);
    int32_t mask_p     = 0;
    ...
    for (row = row_start; row < row_end; row++) {
...
#if LV_COLOR_16_SWAP == 0
                uint8_t bg_rgb[3] = {disp_buf_buf_tmp->ch.red, disp_buf_buf_tmp->ch.green,
                                     disp_buf_buf_tmp->ch.blue};
#else
                uint8_t bg_rgb[3] = {disp_buf_buf_tmp->ch.red,
                                     (disp_buf_buf_tmp->ch.green_h << 3) +
                                         disp_buf_buf_tmp->ch.green_l,
                                     disp_buf_buf_tmp->ch.blue};
#endif
        ...
        if (other_mask_cnt) {
            lv_draw_mask_res_t mask_res = lv_draw_mask_apply(
                mask_buf + mask_p_start, map_area.x1, map_area.y2, lv_area_get_width(&map_area));
            if (mask_res == LV_DRAW_MASK_RES_TRANSP) {
        ...
        if ((int32_t)mask_p + (col_end - col_start) < mask_buf_size) {
            ...
            _lv_blend_map(clip_area, &map_area, color_buf, mask_buf, LV_DRAW_MASK_RES_CHANGED, opa,
                          blend_mode);
