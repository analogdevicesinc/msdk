 ...
 *  STATIC PROTOTYPES
 **********************/
LV_ATTRIBUTE_FAST_MEM static lv_res_t lv_img_draw_core(const lv_area_t* coords,
                                                       const lv_area_t* clip_area, const void* src,
                                                       const lv_draw_img_dsc_t* draw_dsc);

LV_ATTRIBUTE_FAST_MEM static void lv_draw_map(const lv_area_t* map_area, const lv_area_t* clip_area,
                                              const uint8_t* map_p,
                                              const lv_draw_img_dsc_t* draw_dsc, bool chroma_key,
                                              bool alpha_byte);
                                                       ...
                                                       const lv_draw_img_dsc_t* draw_dsc)
{
     ...
     *Just draw it!*/
        ...
        if (draw_dsc->angle || draw_dsc->zoom != LV_IMG_ZOOM_NONE) {
            ...
            _lv_img_buf_get_transformed_area(&map_area_rot, w, h, draw_dsc->angle, draw_dsc->zoom,
                                             &draw_dsc->pivot);
    ...
    /*The whole uncompressed image is not available. Try to read it line-by-line*/
        ...
        uint8_t* buf =
            lv_mem_buf_get(lv_area_get_width(&mask_com) *
                                              ...
                                              bool alpha_byte)
{
    ...
    if (other_mask_cnt == 0 && draw_dsc->angle == 0 && draw_dsc->zoom == LV_IMG_ZOOM_NONE &&
        ...
        _lv_blend_map(clip_area, map_area, (lv_color_t*)map_p, NULL, LV_DRAW_MASK_RES_FULL_COVER,
                      draw_dsc->opa, draw_dsc->blend_mode);
    }
    ...
    /*Simple case without masking and transformations*/
             ...
             alpha_byte == false && chroma_key == true &&
             draw_dsc->recolor_opa == LV_OPA_TRANSP) { /*copy with color keying (+ alpha)*/
        lv_gpu_nxp_pxp_enable_color_key();
        _lv_blend_map(clip_area, map_area, (lv_color_t*)map_p, NULL, LV_DRAW_MASK_RES_FULL_COVER,
                      draw_dsc->opa, draw_dsc->blend_mode);
        lv_gpu_nxp_pxp_disable_color_key();
    } else if (other_mask_cnt == 0 && draw_dsc->angle == 0 && draw_dsc->zoom == LV_IMG_ZOOM_NONE &&
               alpha_byte == false && chroma_key == false &&
               draw_dsc->recolor_opa != LV_OPA_TRANSP) { /*copy with recolor (+ alpha)*/
    ...
    /*In the other cases every pixel need to be checked one-by-one*/
        ...
        if (other_mask_cnt == 0 && !transform && !chroma_key &&
            draw_dsc->recolor_opa == LV_OPA_TRANSP && alpha_byte) {
            ...
            if (lv_area_get_size(&draw_area) > 240) {
                ...
                lv_gpu_stm32_dma2d_blend(disp_buf_first, disp_w, (const lv_color_t*)map_buf_tmp,
                                         draw_dsc->opa, map_w, draw_area_w, draw_area_h);
                return;
            }
#endif
            ...
            for (y = 0; y < draw_area_h; y++) {
                ...
                if (px_i + lv_area_get_width(&draw_area) < mask_buf_size) {
                    ...
                    _lv_blend_map(clip_area, &blend_area, map2, mask_buf, LV_DRAW_MASK_RES_CHANGED,
                                  draw_dsc->opa, draw_dsc->blend_mode);
        ...
        /*Most complicated case: transform or other mask or chroma keyed*/
            ...
            uint32_t mask_buf_size =
                lv_area_get_size(&draw_area) > hor_res ? hor_res : lv_area_get_size(&draw_area);
            lv_color_t* map2   = lv_mem_buf_get(mask_buf_size * sizeof(lv_color_t));
            lv_opa_t* mask_buf = lv_mem_buf_get(mask_buf_size);
            ...
            for (y = 0; y < draw_area_h; y++) {
                ...
                if (other_mask_cnt) {
                    ...
                    mask_res_sub = lv_draw_mask_apply(
                        mask_buf + px_i_start, draw_area.x1 + draw_buf->area.x1,
                        y + draw_area.y1 + draw_buf->area.y1, lv_area_get_width(&draw_area));
