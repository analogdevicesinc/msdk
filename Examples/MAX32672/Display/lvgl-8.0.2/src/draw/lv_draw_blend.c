...
static void fill_set_px(const lv_area_t* disp_area, lv_color_t* disp_buf,
...
#if LV_DRAW_COMPLEX
static void fill_blended(const lv_area_t* disp_area, lv_color_t* disp_buf,
                         const lv_area_t* draw_area, lv_color_t color, lv_opa_t opa,
...
LV_ATTRIBUTE_FAST_MEM static void map_normal(const lv_area_t* disp_area, lv_color_t* disp_buf,
                                             const lv_area_t* draw_area, const lv_area_t* map_area,
                                             const lv_color_t* map_buf, lv_opa_t opa,
                                             const lv_opa_t* mask, lv_draw_mask_res_t mask_res);

#if LV_DRAW_COMPLEX
static void map_blended(const lv_area_t* disp_area, lv_color_t* disp_buf,
                        const lv_area_t* draw_area, const lv_area_t* map_area,
                        const lv_color_t* map_buf, lv_opa_t opa, const lv_opa_t* mask,
                        lv_draw_mask_res_t mask_res, lv_blend_mode_t mode);
...
#define FILL_NORMAL_MASK_PX_SCR_TRANSP(out_x, color)                                              \
    if (*mask_tmp_x) {                                                                            \
        if (*mask_tmp_x == LV_OPA_COVER)                                                          \
            ...
            lv_color_mix_with_alpha(disp_buf_first[out_x], disp_buf_first[out_x].ch.alpha, color, \
                                    *mask_tmp_x, &disp_buf_first[out_x],                          \
    ...
    if (*mask_tmp_x) {                                                                          \
        if (*mask_tmp_x == LV_OPA_COVER)                                                        \
            ...
            lv_color_mix_with_alpha(disp_buf_first[x], disp_buf_first[x].ch.alpha,              \
                                    map_buf_first[x], *mask_tmp_x, &disp_buf_first[x],          \
    ...
    if (mask_res == LV_DRAW_MASK_RES_FULL_COVER) {
        if (opa > LV_OPA_MAX) {
            ...
            if (lv_area_get_size(draw_area) >= LV_GPU_NXP_VG_LITE_FILL_SIZE_LIMIT) {
                if (lv_gpu_nxp_vglite_fill(disp_buf, disp_w, lv_area_get_height(disp_area),
                                           draw_area, color, opa) == LV_RES_OK) {
                    return;
                }
                ...
                for (x = 0; x < draw_area_w; x++) {
                    if (last_dest_color.full != disp_buf_first[x].full) {
                        ...
                        if (disp->driver->screen_transp) {
                            lv_color_mix_with_alpha(disp_buf_first[x], disp_buf_first[x].ch.alpha,
                                                    color, opa, &last_res_color,
                                                    &last_res_color.ch.alpha);
                        } else
#endif
                        {
                            last_res_color =
                                lv_color_mix_premult(color_premult, disp_buf_first[x], opa_inv);
                        }
                    }
                ...
                for (x = 0; x < draw_area_w; x++) {
                    if (*mask_tmp_x) {
                        if (*mask_tmp_x != last_mask)
                            opa_tmp = *mask_tmp_x == LV_OPA_COVER ?
                                          opa :
...
#if LV_COLOR_SCREEN_TRANSP
                            if (disp->driver->screen_transp) {
                                lv_color_mix_with_alpha(disp_buf_first[x],
                                                        disp_buf_first[x].ch.alpha, color, opa_tmp,
                                                        &last_res_color, &last_res_color.ch.alpha);
                            } else
#endif
                            {
                                if (opa_tmp == LV_OPA_COVER)
                                    ...
                                    last_res_color =
                                        lv_color_mix(color, disp_buf_first[x], opa_tmp);
                            }
        ...
        for (y = draw_area->y1; y <= draw_area->y2; y++) {
            for (x = draw_area->x1; x <= draw_area->x2; x++) {
                ...
                if (mask_tmp[x] != last_mask || last_dest_color.full != disp_buf_tmp[x].full) {
                    lv_opa_t opa_tmp = mask_tmp[x] >= LV_OPA_MAX ?
                                           opa :
                                           (uint32_t)((uint32_t)mask_tmp[x] * opa) >> 8;
...

{
    ...
    const lv_color_t* map_buf_tmp =
        map_buf + map_w * (draw_area->y1 - (map_area->y1 - disp_area->y1));
    ...
    if (mask_res == LV_DRAW_MASK_RES_FULL_COVER) {
        ...
        for (y = draw_area->y1; y <= draw_area->y2; y++) {
            for (x = draw_area->x1; x <= draw_area->x2; x++) {
                if (mask_tmp[x]) {
                    disp->driver->set_px_cb(disp->driver, (void*)disp_buf, disp_w, x, y,
                                            map_buf_tmp[x],
                                            (uint32_t)((uint32_t)opa * mask_tmp[x]) >> 8);
                }
            }
                                             ...
                                             const lv_opa_t* mask, lv_draw_mask_res_t mask_res)
{
    ...
    const lv_color_t* map_buf_first =
        map_buf + map_w * (draw_area->y1 - (map_area->y1 - disp_area->y1));
    ...
    if (mask_res == LV_DRAW_MASK_RES_FULL_COVER) {
        if (opa > LV_OPA_MAX) {
            ...
            if (lv_area_get_size(draw_area) >= LV_GPU_NXP_PXP_BLIT_SIZE_LIMIT) {
                lv_gpu_nxp_pxp_blit(disp_buf_first, disp_w, map_buf_first, map_w, draw_area_w,
                                    draw_area_h, opa);
                return;
            }
#elif (LV_USE_GPU_NXP_VG_LITE)
...
#elif LV_USE_GPU_STM32_DMA2D
            if (lv_area_get_size(draw_area) >= 240) {
                lv_gpu_stm32_dma2d_copy(disp_buf_first, disp_w, map_buf_first, map_w, draw_area_w,
                                        draw_area_h);
                return;
            }
#endif
...
#elif LV_USE_GPU_STM32_DMA2D
            if (lv_area_get_size(draw_area) >= 240) {
                lv_gpu_stm32_dma2d_blend(disp_buf_first, disp_w, map_buf_first, opa, map_w,
                                         draw_area_w, draw_area_h);
                return;
            }
#endif
            ...
            for (y = 0; y < draw_area_h; y++) {
                for (x = 0; x < draw_area_w; x++) {
                    ...
                    if (disp->driver->screen_transp) {
                        lv_color_mix_with_alpha(disp_buf_first[x], disp_buf_first[x].ch.alpha,
                                                map_buf_first[x], opa, &disp_buf_first[x],
                                                &disp_buf_first[x].ch.alpha);
                    } else
#endif
                    {
            ...
            for (y = 0; y < draw_area_h; y++) {
                for (x = 0; x < draw_area_w; x++) {
                    if (mask[x]) {
                        ...
                        if (disp->driver->screen_transp) {
                            lv_color_mix_with_alpha(disp_buf_first[x], disp_buf_first[x].ch.alpha,
                                                    map_buf_first[x], opa_tmp, &disp_buf_first[x],
                                                    &disp_buf_first[x].ch.alpha);
                        } else
#endif
                        {
                            disp_buf_first[x] =
                                lv_color_mix(map_buf_first[x], disp_buf_first[x], opa_tmp);
                        }
                    }
                }
                disp_buf_first += disp_w;
