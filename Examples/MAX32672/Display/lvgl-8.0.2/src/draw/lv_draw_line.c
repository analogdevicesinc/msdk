                                                ...
                                                const lv_draw_line_dsc_t* dsc)
{
    ...
    if (simple_mode) {
        _lv_blend_fill(clip, &draw_area, dsc->color, NULL, LV_DRAW_MASK_RES_FULL_COVER, opa,
                       dsc->blend_mode);
    }
#if LV_DRAW_COMPLEX
    /*If there other mask apply it*/
        ...
        for (h = draw_area.y1; h <= draw_area.y2; h++) {
            ...
            lv_draw_mask_res_t mask_res = lv_draw_mask_apply(
                mask_buf, draw_buf->area.x1 + draw_area.x1, draw_buf->area.y1 + h, draw_area_w);
            ...
            _lv_blend_fill(clip, &fill_area, dsc->color, mask_buf, mask_res, dsc->opa,
                           dsc->blend_mode);
                                                 ...
                                                 const lv_draw_line_dsc_t* dsc)
{
    ...
    if (flat) {
        if (xdiff > 0) {
            lv_draw_mask_line_points_init(&mask_left_param, p1.x, p1.y - w_half0, p2.x,
                                          p2.y - w_half0, LV_DRAW_MASK_LINE_SIDE_LEFT);
            lv_draw_mask_line_points_init(&mask_right_param, p1.x, p1.y + w_half1, p2.x,
                                          p2.y + w_half1, LV_DRAW_MASK_LINE_SIDE_RIGHT);
        } else {
            lv_draw_mask_line_points_init(&mask_left_param, p1.x, p1.y + w_half1, p2.x,
                                          p2.y + w_half1, LV_DRAW_MASK_LINE_SIDE_LEFT);
            lv_draw_mask_line_points_init(&mask_right_param, p1.x, p1.y - w_half0, p2.x,
                                          p2.y - w_half0, LV_DRAW_MASK_LINE_SIDE_RIGHT);
        }
    } else {
    ...
    for (h = draw_area.y1 + disp_area->y1; h <= draw_area.y2 + disp_area->y1; h++) {
        ...
        if ((uint32_t)mask_p + draw_area_w < mask_buf_size) {
            ...
            _lv_blend_fill(&fill_area, clip, dsc->color, mask_buf, LV_DRAW_MASK_RES_CHANGED,
                           dsc->opa, dsc->blend_mode);
    ...
    if (fill_area.y1 != fill_area.y2) {
        fill_area.y2--;
        _lv_blend_fill(&fill_area, clip, dsc->color, mask_buf, LV_DRAW_MASK_RES_CHANGED, dsc->opa,
                       dsc->blend_mode);
    }
