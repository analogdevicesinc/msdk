...
static void draw_outline(const lv_area_t* coords, const lv_area_t* clip,
                         const lv_draw_rect_dsc_t* dsc);

#if LV_DRAW_COMPLEX
LV_ATTRIBUTE_FAST_MEM static void draw_shadow(const lv_area_t* coords, const lv_area_t* clip,
                                              const lv_draw_rect_dsc_t* dsc);
LV_ATTRIBUTE_FAST_MEM static void shadow_draw_corner_buf(const lv_area_t* coords, uint16_t* sh_buf,
                                                         lv_coord_t s, lv_coord_t r);
LV_ATTRIBUTE_FAST_MEM static void shadow_blur_corner(lv_coord_t size, lv_coord_t sw,
                                                     uint16_t* sh_ups_buf);

static void draw_full_border(const lv_area_t* area_inner, const lv_area_t* area_outer,
                             const lv_area_t* clip, lv_coord_t radius, bool radius_is_in,
                             lv_color_t color, lv_opa_t opa, lv_blend_mode_t blend_mode);
#else
static void draw_simple_border(const lv_area_t* clip, const lv_area_t* area_inner,
                               const lv_area_t* area_outer, lv_color_t color, lv_opa_t opa,
                               lv_border_side_t side);
#endif

#if LV_DRAW_COMPLEX
LV_ATTRIBUTE_FAST_MEM static inline lv_color_t grad_get(const lv_draw_rect_dsc_t* dsc, lv_coord_t s,
                                                        lv_coord_t i);
#endif
                                          ...
                                          const lv_draw_rect_dsc_t* dsc)
{
    ...
    if (simple_mode && rout == 0 && (grad_dir == LV_GRAD_DIR_NONE)) {
        _lv_blend_fill(clip, &coords_bg, dsc->bg_color, NULL, LV_DRAW_MASK_RES_FULL_COVER, opa,
                       dsc->blend_mode);
    }
#if LV_DRAW_COMPLEX
    /*More complex case: there is a radius, gradient or other mask.*/
        ...
        for (h = draw_area.y1; h <= draw_area.y2; h++) {
            ...
            if (y > coords_bg.y1 + rout + 1 && y < coords_bg.y2 - rout - 1) {
                ...
                if (simple_mode == false) {
                    ...
                    mask_res = lv_draw_mask_apply(mask_buf, draw_buf->area.x1 + draw_area.x1,
                                                  draw_buf->area.y1 + h, draw_area_w);
                }
            }
            ...
            if (simple_mode && split &&
                (y < coords_bg.y1 + rout + 1 || y > coords_bg.y2 - rout - 1)) {
                /*Left part*/
                ...
                _lv_blend_fill(clip, &fill_area2, grad_color, mask_buf, mask_res, opa2,
                               dsc->blend_mode);
                ...
                if (grad_dir == LV_GRAD_DIR_VER) {
                    ...
                    _lv_blend_fill(clip, &fill_area2, grad_color, NULL, LV_DRAW_MASK_RES_FULL_COVER,
                                   opa, dsc->blend_mode);
                }
                ...
                if (mask_ofs < 0)
                    mask_ofs = 0;
                _lv_blend_fill(clip, &fill_area2, grad_color, mask_buf + mask_ofs, mask_res, opa2,
                               dsc->blend_mode);

            } else {
                if (grad_dir == LV_GRAD_DIR_HOR) {
                    _lv_blend_map(clip, &fill_area, grad_map, mask_buf, mask_res, opa2,
                                  dsc->blend_mode);
                } else if (grad_dir == LV_GRAD_DIR_VER) {
                    _lv_blend_fill(clip, &fill_area, grad_color, mask_buf, mask_res, opa2,
                                   dsc->blend_mode);
        ...
        if (grad_dir == LV_GRAD_DIR_NONE && other_mask_cnt == 0 && split) {
            ...
            _lv_blend_fill(clip, &fill_area, dsc->bg_color, NULL, LV_DRAW_MASK_RES_FULL_COVER, opa,
                           dsc->blend_mode);
                                              ...
                                              const lv_draw_rect_dsc_t* dsc)
{
    ...
    if (src_type == LV_IMG_SRC_SYMBOL) {
        lv_point_t size;
        lv_txt_get_size(&size, dsc->bg_img_src, dsc->bg_img_symbol_font, 0, 0, LV_COORD_MAX,
                        LV_TEXT_FLAG_NONE);
        lv_area_t a;
                                              ...
                                              const lv_draw_rect_dsc_t* dsc)
{
    ...
    if (dsc->border_side == LV_BORDER_SIDE_FULL) {
        draw_full_border(&area_inner, coords, clip, dsc->radius, false, dsc->border_color,
                         dsc->border_opa, dsc->blend_mode);
    } else {
    ...
    draw_simple_border(clip, &area_inner, coords, dsc->border_color, dsc->border_opa,
                       dsc->border_side);
#endif
}
                                              ...
                                              const lv_draw_rect_dsc_t* dsc)
{
    ...
    if (dsc->shadow_width == 1 && dsc->shadow_ofs_x == 0 && dsc->shadow_ofs_y == 0 &&
        dsc->shadow_spread <= 0) {
        return;
    }
    ...
    if (has_com && _lv_area_is_in(&a, &bg_coords, r_bg) == false) {
        ...
        if (w > 0) {
            ...
            for (y = 0; y < h; y++) {
                ...
                _lv_blend_fill(clip, &fa, dsc->shadow_color, mask_buf, mask_res, opa,
                               dsc->blend_mode);
                fa.y1++;
                fa.y2++;
    ...
    if (has_com && _lv_area_is_in(&a, &bg_coords, r_bg) == false) {
        ...
        if (w > 0) {
            ...
            for (y = 0; y < h; y++) {
                ...
                _lv_blend_fill(clip, &fa, dsc->shadow_color, mask_buf, mask_res, dsc->shadow_opa,
                               dsc->blend_mode);
                fa.y1++;
                fa.y2++;
            }
    ...
    if (has_com && _lv_area_is_in(&a, &bg_coords, r_bg) == false) {
        ...
        for (y = 0; y < h; y++) {
            ...
            _lv_blend_fill(clip, &fa, dsc->shadow_color, mask_buf, mask_res, LV_OPA_COVER,
                           dsc->blend_mode);
            fa.y1++;
            fa.y2++;
            sh_buf_tmp += corner_size;
...
#if LV_DRAW_COMPLEX
    draw_full_border(&area_inner, &area_outer, clip, dsc->radius, true, dsc->outline_color,
                     dsc->outline_opa, dsc->blend_mode);
#else
    draw_simple_border(clip, &area_inner, &area_outer, dsc->outline_color, dsc->outline_opa,
                       LV_BORDER_SIDE_FULL);
#endif
}
    ...
    if (simple_mode) {
        ...
        for (h = draw_area.y1; h <= upper_corner_end; h++) {
            ...
            if (fill_area2.y2 < area_outer->y1 + border_width) {
                ...
                _lv_blend_fill(clip, &fill_area2, color, NULL, LV_DRAW_MASK_RES_FULL_COVER, opa,
                               blend_mode);
            }
            ...
            if (mask_ofs < 0)
                mask_ofs = 0;
            _lv_blend_fill(clip, &fill_area2, color, mask_buf + mask_ofs, mask_res, opa,
                           blend_mode);
                               ...
                               lv_border_side_t side)
{
    ...
    if (side & LV_BORDER_SIDE_TOP) {
        _lv_blend_fill(clip, &a, color, NULL, LV_DRAW_MASK_RES_FULL_COVER, opa,
