                                  ...
                                  lv_point_t* p_out)
{
    ...
    if (chart->type == LV_CHART_TYPE_LINE) {
        ...
        int32_t ser_gap  = (lv_obj_get_style_pad_column(obj, LV_PART_ITEMS) * chart->zoom_x) >>
                          8; /*Gap between the column on the ~same X*/
        int32_t block_gap = (lv_obj_get_style_pad_column(obj, LV_PART_MAIN) * chart->zoom_x) >>
...
static void lv_chart_event(const lv_obj_class_t* class_p, lv_event_t* e)
{
    ...
    if (code == LV_EVENT_PRESSED) {
        ...
        lv_event_set_ext_draw_size(e, LV_MAX4(chart->tick[0].draw_size, chart->tick[1].draw_size,
...
static void draw_series_line(lv_obj_t* obj, const lv_area_t* clip_area)
{
    ...
    _LV_LL_READ_BACK(&chart->series_ll, ser)
    {
        ...
        int32_t y_tmp =
            (int32_t)((int32_t)ser->y_points[p_prev] - chart->ymin[ser->y_axis_sec]) * h;
...
static void draw_series_bar(lv_obj_t* obj, const lv_area_t* clip_area)
{
    ...
    for (i = 0; i < chart->point_cnt; i++) {
        lv_coord_t x_act =
            (int32_t)((int32_t)(w + block_gap) * i) / (chart->point_cnt) + obj->coords.x1 + x_ofs;
...
static void draw_y_ticks(lv_obj_t* obj, const lv_area_t* clip_area, lv_chart_axis_t axis)
{
    ...
    for (i = 0; i <= total_tick_num; i++) {
        ...
        if (major && t->label_en) {
            ...
            lv_point_t size;
            lv_txt_get_size(&size, dsc.text, label_dsc.font, label_dsc.letter_space,
