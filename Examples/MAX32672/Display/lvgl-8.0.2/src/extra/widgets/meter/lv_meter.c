...
static void draw_ticks_and_labels(lv_obj_t* obj, const lv_area_t* clip_area,
                                  const lv_area_t* scale_area);
static void draw_needles(lv_obj_t* obj, const lv_area_t* clip_area, const lv_area_t* scale_area);
static void inv_arc(lv_obj_t* obj, lv_meter_indicator_t* indic, int32_t old_value,
                    int32_t new_value);
...
static void draw_arcs(lv_obj_t* obj, const lv_area_t* clip_area, const lv_area_t* scale_area)
{
    ...
    _LV_LL_READ_BACK(&meter->indicator_ll, indic)
    {
        ...
        int32_t end_angle   = lv_map(indic->end_value, scale->min, scale->max, scale->rotation,
                                   scale->rotation + scale->angle_range);
        lv_draw_arc(scale_center.x, scale_center.y, r_out + indic->type_data.arc.r_mod, start_angle,
                    end_angle, clip_area, &arc_dsc);
    }
}
                                  ...
                                  const lv_area_t* scale_area)
{
    ...
    lv_point_t p_center;
    lv_coord_t r_edge =
        LV_MIN(lv_area_get_width(scale_area) / 2, lv_area_get_height(scale_area) / 2);
    ...
    _LV_LL_READ_BACK(&meter->scale_ll, scale)
    {
        ...
        for (i = 0; i < scale->tick_cnt; i++) {
                ...
                if (value_of_line >= indic->start_value && value_of_line <= indic->end_value) {
                    ...
                    if (indic->type_data.scale_lines.color_start.full ==
                        ...
                        if (indic->type_data.scale_lines.local_grad) {
                            ratio = lv_map(value_of_line, indic->start_value, indic->end_value,
                                           LV_OPA_TRANSP, LV_OPA_COVER);
                        } else {
                            ratio = lv_map(value_of_line, scale->min, scale->max, LV_OPA_TRANSP,
                                           LV_OPA_COVER);
                        }
            ...
            lv_point_t p_outer;
            p_outer.x =
                (int32_t)(((int32_t)cos_mid * (r_out + line_width) + 127) >> (LV_TRIGO_SHIFT)) +
                p_center.x;
            p_outer.y =
                (int32_t)(((int32_t)sin_mid * (r_out + line_width) + 127) >> (LV_TRIGO_SHIFT)) +
                p_center.y;
            ...
            lv_point_t p_outer;
            p_outer.x =
                (int32_t)(((int32_t)cos_mid * (r_out) + 127) >> (LV_TRIGO_SHIFT)) + p_center.x;
            p_outer.y =
                (int32_t)(((int32_t)sin_mid * (r_out) + 127) >> (LV_TRIGO_SHIFT)) + p_center.y;
            lv_point_t p_inner;
            lv_coord_t r_in = major ? r_in_major : r_in_minor;
            p_inner.x =
                (int32_t)(((int32_t)cos_mid * (r_in) + 127) >> (LV_TRIGO_SHIFT)) + p_center.x;
            p_inner.y =
                (int32_t)(((int32_t)sin_mid * (r_in) + 127) >> (LV_TRIGO_SHIFT)) + p_center.y;
            ...
            if (major) {
                ...
                lv_point_t p;
                p.x = (int32_t)((int32_t)((int32_t)cos_mid * r_text + 127) >> LV_TRIGO_SHIFT) +
                      p_center.x;
                p.y = (int32_t)((int32_t)((int32_t)sin_mid * r_text + 127) >> LV_TRIGO_SHIFT) +
                      p_center.y;
                ...
                lv_txt_get_size(&label_size, dsc.text, label_dsc.font, label_dsc.letter_space,
                    ...
                    int32_t new_value)
{
    ...
    int32_t end_angle   = lv_map(new_value, scale->min, scale->max, scale->rotation,
                               scale->angle_range + scale->rotation);
    lv_area_t a;
    lv_draw_arc_get_area(scale_center.x, scale_center.y, r_out, LV_MIN(start_angle, end_angle),
