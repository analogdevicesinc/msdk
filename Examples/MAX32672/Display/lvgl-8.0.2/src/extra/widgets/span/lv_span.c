...
lv_coord_t lv_spangroup_get_expand_height(lv_obj_t* obj, lv_coord_t width)
{
    ...
    while (cur_span) {
        ...
        while (1) {
            ...
            bool isfill =
                lv_txt_get_snippet(&cur_txt[cur_txt_ofs], snippet.font, snippet.letter_space, max_w,
...
static void lv_draw_span(lv_obj_t* obj, const lv_area_t* coords, const lv_area_t* mask)
{
    ...
    while (cur_span) {
        ...
        if (spans->overflow == LV_SPAN_OVERFLOW_ELLIPSIS) {
            ...
            if (cur_txt[cur_txt_ofs] ==
                '\0') { /* current span deal with ok, need get next line first line height */
                next_line_h = 0;
                if (cur_span) {
                    lv_span_t* next_span = _lv_ll_get_next(&spans->child_ll, cur_span);
                    if (next_span) { /* have the next line */
                        next_line_h =
                            lv_font_get_line_height(lv_span_get_style_text_font(obj, next_span)) +
                            line_space;
                    }
                }
            }
        ...
        for (int i = 0; i < item_cnt; i++) {
            ...
            while (j < txt_bytes) {
                ...
                if (ellipsis_valid && pos.x + letter_w + pinfo->letter_space > ellipsis_width) {
                    for (int ell = 0; ell < 3; ell++) {
                        lv_draw_letter(&pos, &clipped_area, pinfo->font, '.', letter_color,
                                       letter_opa, blend_mode);
                        pos.x = pos.x + dot_letter_w + pinfo->letter_space;
                    }
                    break;
                } else {
                    lv_draw_letter(&pos, &clipped_area, pinfo->font, letter, letter_color,
                                   letter_opa, blend_mode);
                    if (letter_w > 0) {
                        pos.x = pos.x + letter_w + pinfo->letter_space;
                    }
            ...
            if (ellipsis_valid && i == item_cnt - 1 && pos.x <= ellipsis_width) {
                for (int ell = 0; ell < 3; ell++) {
                    lv_draw_letter(&pos, &clipped_area, pinfo->font, '.', letter_color, letter_opa,
                                   blend_mode);
                    pos.x = pos.x + dot_letter_w + pinfo->letter_space;
                }
            }
            ...
            if (decor != LV_TEXT_DECOR_NONE) {
                ...
                line_dsc.width =
                    pinfo->font->underline_thickness ? pinfo->font->underline_thickness : 1;
