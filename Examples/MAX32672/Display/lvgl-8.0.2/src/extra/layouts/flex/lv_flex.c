...
static void children_repos(lv_obj_t* cont, flex_t* f, int32_t item_first_id, int32_t item_last_id,
                           lv_coord_t abs_x, lv_coord_t abs_y, lv_coord_t max_main_size,
                           lv_coord_t item_gap, track_t* t);
static void place_content(lv_flex_align_t place, lv_coord_t max_size, lv_coord_t content_size,
                          lv_coord_t item_cnt, lv_coord_t* start_pos, lv_coord_t* gap);
...
static void flex_update(lv_obj_t* cont, void* user_data)
{
    ...
    lv_coord_t track_gap = !f.row ? lv_obj_get_style_pad_column(cont, LV_PART_MAIN) :
    ...
    lv_coord_t max_main_size =
        (f.row ? lv_obj_get_content_width(cont) : lv_obj_get_content_height(cont));
    lv_coord_t border_widt = lv_obj_get_style_border_width(cont, LV_PART_MAIN);
    lv_coord_t abs_y       = cont->coords.y1 + lv_obj_get_style_pad_top(cont, LV_PART_MAIN) +
                       border_widt - lv_obj_get_scroll_y(cont);
    lv_coord_t abs_x = cont->coords.x1 + lv_obj_get_style_pad_left(cont, LV_PART_MAIN) +
                       border_widt - lv_obj_get_scroll_x(cont);
    ...
    if (track_cross_place != LV_FLEX_ALIGN_START) {
        ...
        lv_coord_t max_cross_size =
            (f.row ? lv_obj_get_content_height(cont) : lv_obj_get_content_width(cont));
        place_content(track_cross_place, max_cross_size, total_track_cross_size, track_cnt,
                      cross_pos, &gap);
    }
    ...
    while (track_first_item < (int32_t)cont->spec_attr->child_cnt && track_first_item >= 0) {
        ...
        children_repos(cont, &f, track_first_item, next_track_first_item, abs_x, abs_y,
                              ...
                              lv_coord_t max_main_size, lv_coord_t item_gap, track_t* t)
{
    ...
    if (f->wrap &&
        ((f->row && w_set == LV_SIZE_CONTENT) || (!f->row && h_set == LV_SIZE_CONTENT))) {
        f->wrap = false;
    }
                           ...
                           lv_coord_t item_gap, track_t* t)
{
    ...
    place_content(f->main_place, max_main_size, t->track_main_size, t->item_cnt, &main_pos,
                  &place_gap);
    if (f->row && rtl)
    ...
    while (item && item_first_id != item_last_id) {
        ...
        switch (f->cross_place) {
             ...
             *The issue comes up e,g, with column direction with center cross direction if an element's width changes*/
                cross_pos =
                    (((t->track_cross_size + 1) & (~1)) - area_get_cross_size(&item->coords)) / 2;
                break;
