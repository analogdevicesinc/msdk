...
#define PAD_DEF                        \
    (disp_size == DISP_LARGE ?         \
         ...
         disp_size == DISP_MEDIUM ? lv_disp_dpx(theme.disp, 20) : lv_disp_dpx(theme.disp, 16))
#define PAD_SMALL                      \
    (disp_size == DISP_LARGE ?         \
         lv_disp_dpx(theme.disp, 14) : \
         disp_size == DISP_MEDIUM ? lv_disp_dpx(theme.disp, 12) : lv_disp_dpx(theme.disp, 10))
#define PAD_TINY                      \
    (disp_size == DISP_LARGE ?        \
         lv_disp_dpx(theme.disp, 8) : \
         disp_size == DISP_MEDIUM ? lv_disp_dpx(theme.disp, 6) : lv_disp_dpx(theme.disp, 2))
...
static void style_init(void)
{
    ...
    lv_style_transition_dsc_init(&trans_delayed, trans_props, lv_anim_path_linear, TRANSITION_TIME,
                                 70, NULL);

    static lv_style_transition_dsc_t trans_normal;
    lv_style_transition_dsc_init(&trans_normal, trans_props, lv_anim_path_linear, TRANSITION_TIME,
                                 0, NULL);
    ...
    lv_style_set_bg_color(&styles->scrollbar, (theme.flags & MODE_DARK) ?
    ...
    lv_style_set_radius(&styles->btn, (disp_size == DISP_LARGE ?
    ...
    lv_style_set_text_color(&styles->ta_placeholder, (theme.flags & MODE_DARK) ?
                                                         lv_palette_darken(LV_PALETTE_GREY, 2) :
