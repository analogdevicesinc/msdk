...
{}
    ...
    LV_LOG_WARN("Memory integrity checks are enabled via LV_USE_ASSERT_MEM_INTEGRITY which makes "
                "LVGL much slower")
#endif
...
static void lv_obj_set_state(lv_obj_t* obj, lv_state_t new_state)
{
    ...
    _lv_obj_style_transition_dsc_t* ts =
        lv_mem_buf_get(sizeof(_lv_obj_style_transition_dsc_t) * STYLE_TRANSITION_MAX);
