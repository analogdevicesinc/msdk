...
static void indev_pointer_proc(lv_indev_t* i, lv_indev_data_t* data)
{
    ...
    if (data->point.y >= lv_disp_get_ver_res(i->driver->disp))
        LV_LOG_WARN("Y is %d which is greater than hor. res", data->point.y);
            ...
            if (data->key == LV_KEY_ENTER) {
                ...
                if (editable) {
                    ...
                    if (lv_group_get_obj_count(g) > 1) {
                        ...
                        lv_obj_clear_state(indev_obj_act,
...
static void indev_proc_press(_lv_indev_proc_t* proc)
{
    ...
    if (indev_obj_act == NULL) {
        indev_obj_act =
            lv_indev_search_obj(lv_disp_get_layer_sys(disp), &proc->types.pointer.act_point);
        if (indev_obj_act == NULL)
            indev_obj_act =
                lv_indev_search_obj(lv_disp_get_layer_top(disp), &proc->types.pointer.act_point);
        if (indev_obj_act == NULL)
            indev_obj_act =
                lv_indev_search_obj(lv_disp_get_scr_act(disp), &proc->types.pointer.act_point);
