...
This application combines the BLE_FreeRTOS and BLE_otas applications. It uses the FreeRTOS 
operating system and integrates over the air firmware updates.

Bluetooth data server that advertises as "FreeRTOS_OTA" and accepts connection requests.
...
The red LED will indicate that the device is out of standby mode.

The green LED indicates CPU activity. When the LED is on, the CPU is active, when the LED
is off, the CPU is in sleep mode or standby mode when the red LED is off.
