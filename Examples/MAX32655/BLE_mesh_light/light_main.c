...
#define LIGHT_HT_SR_COMPANY_ID 0xffff // Analog Devices
...
#define LIGHT_PRINT4(msg, var1, var2, var3, var4) APP_TRACE_INFO4(msg, var1, var2, var3, var4)
...
/* Button identifiers */
enum { LIGHT_BUTTON_1, LIGHT_BUTTON_MAX };
...
static void lightBearerCback(uint8_t slot)
{
    ...
    // APP_TRACE_INFO0("Bearer CBACK");

    /* Switch ADV data on Proxy and Node Identity*/
    if ((slot == BR_GATT_SLOT) && MeshIsProvisioned()) {
        idType = (lightCb.nodeIdentityRunning) ? MESH_PROXY_NODE_IDENTITY_TYPE :
                                                 MESH_PROXY_NWK_ID_TYPE;
...
static void lightProcMeshPrvSrMsg(const meshPrvSrEvt_t *pMsg)
{
    ...
    case MESH_PRV_SR_PROVISIONING_FAILED_EVENT:
        LIGHT_PRINT1("prvsr_ind prv_failed reason=0x%x" LIGHT_NEWLINE, pMsg->prvFailed.reason);
...
static void lightProcMeshCfgMdlSrMsg(const meshCfgMdlSrEvt_t *pEvt)
{
    ...
    case MESH_CFG_MDL_NODE_IDENTITY_SET_EVENT:
        ...
        if (pEvt->nodeIdentity.state == MESH_NODE_IDENTITY_RUNNING) {
            ...
            MeshProxySrGetServiceData(pEvt->nodeIdentity.netKeyIndex,
                                      MESH_PROXY_NODE_IDENTITY_TYPE);
...
static void lightProcMeshCoreMsg(meshEvt_t *pMsg)
{
    ...
    case MESH_CORE_NODE_STARTED_EVENT:
        if (pMsg->nodeStarted.hdr.status == MESH_SUCCESS) {
            LIGHT_PRINT2(LIGHT_NEWLINE
                         "mesh_ind node_started elemaddr=0x%x elemcnt=%d" LIGHT_NEWLINE,
         ...
         */
            ...
            LIGHT_PRINT0(LIGHT_NEWLINE "mesh_ind node_started failed" LIGHT_NEWLINE);
        }
        break;
    ...
    case MESH_CORE_PROXY_SERVICE_DATA_EVENT:
        ...
        if (pMsg->serviceData.serviceDataLen != 0) {
            ...
            GattBearerSrSetPrxSvcData(pMsg->serviceData.serviceData,
                                      pMsg->serviceData.serviceDataLen);
        }
        break;
    ...
    case MESH_CORE_IV_UPDATED_EVENT:
        LIGHT_PRINT1(LIGHT_NEWLINE "mesh_ind ividx=0x%x" LIGHT_NEWLINE, pMsg->ivUpdt.ivIndex);
        break;
...
static void lightProcMsg(dmEvt_t *pMsg)
{
    ...
    case DM_ADV_START_IND:
        APP_TRACE_INFO0("ADV started!");
        break;
    case DM_ADV_STOP_IND:
        APP_TRACE_INFO0("ADV Stopped!");
        break;
    ...
    case DM_CONN_OPEN_IND:
        APP_TRACE_INFO1("%d", pMsg->hdr.event);
        APP_TRACE_INFO0("Connection open!");
...
void LightHandler(wsfEventMask_t event, wsfMsgHdr_t *pMsg)
{
    ...
    if (pMsg != NULL) {
        ...
        LlGetBdAddr(bdAddr);
        APP_TRACE_INFO6("LIGHT: App got evt  bdAddr %02x:%02x:%02x:%02x:%02x:%02x", bdAddr[5],
        ...
        if (pMsg->event <= ATT_CBACK_END) {
            ...
            if (pMsg->status == HCI_SUCCESS) {
                if (pMsg->event == DM_CONN_OPEN_IND) {
                    ...
                    if (lightCb.prvSrStarted || lightCb.proxyFeatEnabled ||
                        lightCb.nodeIdentityRunning) {
            ...
            if (pMsg->event == APP_MESH_NODE_IDENTITY_TIMEOUT_EVT) {
                ...
                if (!lightCb.proxyFeatEnabled) {
                    /* Disable bearer slot. */
                    APP_TRACE_INFO0("Disabling APP Bearer!");
                    AppBearerDisableSlot(BR_GATT_SLOT);
                }
            }
