...
/*! local IRK */
static uint8_t localIrk[] = { 0xA6, 0xD9, 0xFF, 0x70, 0xD6, 0x1E, 0xF0, 0xA4, 0x46, 0x5F, 0x8D,
    0x68, 0x19, 0xF3, 0xB4, 0x96 };
...
static void datcDmCback(dmEvt_t* pDmEvt)
{
    ...
    if (pDmEvt->hdr.event == DM_SEC_ECC_KEY_IND) {
        ...
        if (datcOobCfg) {
            ...
            PalUartReadData(
                PAL_UART_ID_CHCI, datcOobCfg->peerRandom, (SMP_RAND_LEN + SMP_CONFIRM_LEN));
        } else {
...
static void datcPrintScanReport(dmEvt_t* pMsg)
{
    ...
    if ((pData = DmFindAdType(DM_ADV_TYPE_LOCAL_NAME, pMsg->scanReport.len, pMsg->scanReport.pData))
        != NULL) {
        datcPrintName(pData);
    } else if ((pData = DmFindAdType(
                    DM_ADV_TYPE_SHORT_NAME, pMsg->scanReport.len, pMsg->scanReport.pData))
        != NULL) {
        datcPrintName(pData);
    }
#endif
...
static void datcScanReport(dmEvt_t* pMsg)
{
    ...
    if (!connect
        && ((pData = DmFindAdType(
                 DM_ADV_TYPE_LOCAL_NAME, pMsg->scanReport.len, pMsg->scanReport.pData))
            != NULL)) {
