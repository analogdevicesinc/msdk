 ...
 */
/*************************************************************************************************/
static void AppWsfBufDiagnostics(WsfBufDiag_t *pInfo)
{
    if (pInfo->type == WSF_BUF_ALLOC_FAILED) {
        APP_TRACE_INFO2("Got WSF Buffer Allocation Failure - Task: %d Len: %d",
...
void AppStart(void)
{
    ...
    WsfBufDiagRegister(AppWsfBufDiagnostics);
