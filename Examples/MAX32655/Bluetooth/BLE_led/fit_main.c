...
#define LED_TMR_EVT         (0x99)
#define LED_TMR_PERIOD_MS   (1000)
...
/*! BLE Link Layer State machine states */
enum TBleLlSt {
    BLE_LL_ST_UNKNOWN = 0,
    BLE_LL_ST_SB,   /*! Standby */
    BLE_LL_ST_ADV,  /*! Advertising */
    BLE_LL_ST_INIT, /*! Initiating */
    BLE_LL_ST_CONN, /*! Connection */
    BLE_LL_ST_SYNC, /*! Synchronization */
    BLE_LL_ST_SCAN, /*! Scanning*/
    BLE_LL_ST_ISO   /*! Isochronous Broacasting*/
};
typedef uint8_t TBleLlSt;
...
/**************************************************************************************************
  Data Types
**************************************************************************************************/
TBleLlSt bleLlSt = BLE_LL_ST_UNKNOWN;
...
static void fitProcMsg(fitMsg_t *pMsg)
{
    ...
    case DM_RESET_CMPL_IND:
        ...
        bleLlSt = BLE_LL_ST_UNKNOWN;
        
        WsfTimerStartMs(&ledTimer, LED_TMR_PERIOD_MS);
    ...
    case LED_TMR_EVT:
        ...
        else if (bleLlSt == BLE_LL_ST_CONN)
        {
            SetLEDs(1);
        }
        else
...
void FitHandlerInit(wsfHandlerId_t handlerId)
{
    ...
    ledTimer.handlerId = handlerId;
    ledTimer.msg.event = LED_TMR_EVT;
}
...
{
    if (pMsg != NULL) {
        if (pMsg->event != LED_TMR_EVT)
        {
            APP_TRACE_INFO2("Fit got evt %d, %d", pMsg->event, bleLlSt);
        }
