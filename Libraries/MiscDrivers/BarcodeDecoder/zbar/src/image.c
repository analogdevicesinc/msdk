...
void zbar_image_get_crop(const zbar_image_t *img, unsigned *x, unsigned *y, unsigned *w,
                         unsigned *h)
{
    if (x)
        *x = img->crop_x;
...
int zbar_image_write_png(const zbar_image_t *img, const char *filename)
{
    ...
    png_set_IHDR(png, info, img->width, img->height, 8, PNG_COLOR_TYPE_GRAY, PNG_INTERLACE_NONE,
