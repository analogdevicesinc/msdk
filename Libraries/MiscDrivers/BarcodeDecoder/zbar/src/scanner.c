...
#endif
#define THRESH_INIT \
    ((unsigned)((ZBAR_SCANNER_THRESH_INIT_WEIGHT * (1 << (ZBAR_FIXED + 1)) + 1) / 2))
...
static inline unsigned calc_thresh(zbar_scanner_t *scn)
{
    ...
    dbprintf(1, " thr=%d t=%ld x=%d last=%d.%d (%d)", thresh, t, scn->x,
             scn->last_edge >> ZBAR_FIXED, scn->last_edge & ((1 << ZBAR_FIXED) - 1), dx);
    if (thresh > t) {
        thresh -= t;
...
void zbar_scanner_get_state(const zbar_scanner_t *scn, unsigned *x, unsigned *cur_edge,
                            unsigned *last_edge, int *y0, int *y1, int *y2, int *y1_thresh)
{
