...
static int rs_quadratic_solve(const rs_gf256 *_gf, unsigned _b, unsigned _c, unsigned char _x[2])
{
    ...
    c8 = _gf->exp[logc4 << 1];
    g3 = rs_hgmul(_gf,
                  _gf->exp[logb14 + logc] ^ _gf->exp[logb12 + logc2] ^ _gf->exp[logb8 + logc4] ^ c8,
                  logb);
    ...
    g2 = rs_hgmul(_gf, rs_hgmul(_gf, c0, 255 - 2 * (255 / 15)) ^ rs_gmul(_gf, c0, c0),
                  255 - 255 / 15);
    z2 = rs_gdiv(_gf, g2, _gf->exp[255 - (255 / 15) * 4] ^ _gf->exp[255 - (255 / 15)]);
    l2 = rs_hgmul(_gf, rs_gmul(_gf, z2, z2) ^ rs_hgmul(_gf, z2, 255 - (255 / 15)) ^ c0,
                  2 * (255 / 15));
    /*Back substitute to the solution in the original field.*/
    _x[0] =
        _gf->exp[_gf->log[z3 ^ rs_hgmul(_gf, rs_hgmul(_gf, l2, 255 / 3) ^ rs_hgmul(_gf, z2, 255 / 15),
                                        logb)] +
                 inc];
    _x[1] = _x[0] ^ _b;
    return 2;
...
/*Solve a cubic equation x**3 + _a*x**2 + _b*x + _c in GF(2**8).
  Returns the number of distinct roots.*/
static int rs_cubic_solve(const rs_gf256 *_gf, unsigned _a, unsigned _b, unsigned _c,
                          unsigned char _x[3])
{
    unsigned k;
    unsigned logd;
...
static int rs_quartic_solve(const rs_gf256 *_gf, unsigned _a, unsigned _b, unsigned _c, unsigned _d,
                            unsigned char _x[3])
{
    unsigned r;
    unsigned s;
...
/*Compute the first (d+1) coefficients of the product of a degree e and a
   degree f polynomial.*/
static void rs_poly_mult(const rs_gf256 *_gf, unsigned char *_p, int _dp1, const unsigned char *_q,
                         int _ep1, const unsigned char *_r, int _fp1)
{
    int m;
    int i;
...
static void rs_calc_syndrome(const rs_gf256 *_gf, int _m0, unsigned char *_s, int _npar,
...
static int rs_modified_berlekamp_massey(const rs_gf256 *_gf, unsigned char *_lambda,
                                        const unsigned char *_s, unsigned char *_omega, int _npar,
                                        const unsigned char *_erasures, int _nerasures, int _ndata)
{
    ...
    for (n = _nerasures + 1; n <= _npar; n++) {
        ...
        if (d != 0) {
            ...
            if (l < n - k) {
                ...
                for (i = 0; i <= l; i++) _lambda[i] = _lambda[i] ^ rs_hgmul(_gf, tt[i], logd);
        }
    }
...
static int rs_find_roots(const rs_gf256 *_gf, unsigned char *_epos, const unsigned char *_lambda,
                         int _nerrors, int _ndata)
{
    unsigned alpha;
    int nroots;
    int i;
...
int rs_correct(const rs_gf256 *_gf, int _m0, unsigned char *_data, int _ndata, int _npar,
               const unsigned char *_erasures, int _nerasures)
{
    ...
    for (i = 0; i < _npar; i++)
        if (s[i]) {
            ...
            nerrors = rs_modified_berlekamp_massey(_gf, lambda, s, omega, _npar, _erasures,
                                                   _nerasures, _ndata);
...
void rs_encode(const rs_gf256 *_gf, unsigned char *_data, int _ndata, const unsigned char *_genpoly,
               int _npar)
{
    unsigned char *lfsr;
    unsigned d;
...
int main(void)
{
    ...
    for (;;) {
        ...
        int nerrors;
        int i;
        if (scanf("%i", &ndata) < 1 || ndata < 0 || ndata > 255 || scanf("%i", &npar) < 1 ||
            npar < 0 || npar > ndata)
            break;
        for (i = 0; i < ndata; i++) {
