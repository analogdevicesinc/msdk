...
int qr_code_data_list_extract_text(const qr_code_data_list *_qrlist, zbar_image_scanner_t *iscn,
                                   zbar_image_t *img)
{
    iconv_t sjis_cd;
    iconv_t utf8_cd;
         ...
         when we decoded it.*/
            ...
            for (j = 0; j < sa_size && !err; j++, sym = &(*sym)->next) {
                *sym = _zbar_image_scanner_alloc_sym(iscn, ZBAR_QRCODE, 0);
                ...
                dir[0] = (qrdataj->bbox[0][0] - qrdataj->bbox[2][0] + qrdataj->bbox[1][0] -
                          qrdataj->bbox[3][0]);
                dir[1] = (qrdataj->bbox[2][1] - qrdataj->bbox[0][1] + qrdataj->bbox[3][1] -
                          qrdataj->bbox[1][1]);
                ...
                UTF-8 is rarely specified with ECI, and few decoders
                 ...
                 encoders to reliably indicate it.*/
                                    ...
                                    err = utf8_cd == (iconv_t)-1 ||
                                          iconv(utf8_cd, &in, &inleft, &out, &outleft) ==
                                              (size_t)-1;
                                    if (!err) {
                 ...
                 SJIS will corrupt the backslashes used for DoCoMo formats.*/
                  ...
                  So if we see any of those characters, move this
                   conversion to the end of the list.*/
                                        ...
                                        err = iconv(enc_list[ei], &in, &inleft, &out, &outleft) ==
                                              (size_t)-1;
                                        if (!err) {
                                            sa_ntext = out - sa_text;
