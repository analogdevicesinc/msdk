...
#include <zbar.h>
    ...
    defined(DEBUG_DATABAR) || defined(DEBUG_CODE128) || defined(DEBUG_SQ_FINDER) || \
...
static inline const unsigned int *decoder_get_configp(const zbar_decoder_t *dcode,
...
unsigned int zbar_decoder_get_configs(const zbar_decoder_t *dcode, zbar_symbol_type_t sym)
{
...
static inline int decoder_set_config_bool(zbar_decoder_t *dcode, zbar_symbol_type_t sym,
...
#if ENABLE_EAN == 1
    dcode->ean.enable =
        TEST_CFG(dcode->ean.ean13_config | dcode->ean.ean2_config | dcode->ean.ean5_config |
                     dcode->ean.ean8_config | dcode->ean.upca_config | dcode->ean.upce_config |
...
static inline int decoder_set_config_int(zbar_decoder_t *dcode, zbar_symbol_type_t sym,
                                         zbar_config_t cfg, int val)
{
    switch (sym) {
#if ENABLE_I25 == 1
...
int zbar_decoder_get_config(zbar_decoder_t *dcode, zbar_symbol_type_t sym, zbar_config_t cfg,
                            int *val)
{
    const unsigned *config = decoder_get_configp(dcode, sym);
...
int zbar_decoder_set_config(zbar_decoder_t *dcode, zbar_symbol_type_t sym, zbar_config_t cfg,
                            int val)
{
    if (sym == ZBAR_NONE) {
        ...
        for (symp = all; *symp; symp++) zbar_decoder_set_config(dcode, *symp, cfg, val);
        return (0);
    }
...
const char *_zbar_decoder_buf_dump(unsigned char *buf, unsigned int buflen)
{
    ...
    p = decoder_dump +
        snprintf(decoder_dump, 12, "buf[%04x]=", (buflen > 0xffff) ? 0xffff : buflen);
    for (i = 0; i < buflen; i++) p += snprintf(p, 4, "%s%02x", (i) ? " " : "", buf[i]);
    return (decoder_dump);
}
