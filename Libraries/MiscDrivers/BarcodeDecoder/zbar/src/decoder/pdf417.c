...
static inline signed short pdf417_decode8(zbar_decoder_t *dcode)
{
    ...
    if (clst < 0)
        clst += 9;
    dbprintf(2, " k=%d", clst);
    zassert(clst >= 0 && clst < 9, -1, "dir=%x sig=%lx k=%x %s\n", dcode->pdf417.direction, sig,
            clst, _zbar_decoder_buf_dump(dcode->buf, dcode->pdf417.character));
    ...
    zassert(g[0] >= 0 && g[1] >= 0 && g[2] >= 0, -1,
            ...
            g[0], g[1], g[2], _zbar_decoder_buf_dump(dcode->buf, dcode->pdf417.character));
