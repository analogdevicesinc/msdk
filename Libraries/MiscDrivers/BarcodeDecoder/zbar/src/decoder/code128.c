...
static const unsigned char lo_base[8] = { 0x00, 0x07, 0x0c, 0x19, 0x24, 0x32, 0x40, 0x47 };
...
static inline signed char decode_lo(int sig)
{
    ...
    c = characters[idx];
    dbprintf(2, " %02x(%x(%02x)/%x(%02x)) => %02x", idx, base, lo_base[base], offset,
             lo_offset[offset], (unsigned char)c);
    return (c);
}
...
static inline signed char decode_hi(int sig)
{
    ...
    unsigned char idx, c;
    if (rev)
        sig = (((sig >> 12) & 0x000f) | ((sig >> 4) & 0x00f0) | ((sig << 4) & 0x0f00) |
               ((sig << 12) & 0xf000));
...
static inline unsigned postprocess_c(zbar_decoder_t *dcode, unsigned start, unsigned end,
                                     unsigned dst)
{
    unsigned i, j;
...
static inline unsigned char postprocess(zbar_decoder_t *dcode)
{
    ...
    for (i = 1, j = 0; i < dcode128->character - 2; i++) {
        ...
        zassert(!(code & 0x80), 1, "i=%x j=%x code=%02x charset=%x cexp=%x %s\n", i, j, code,
                charset, cexp, _zbar_decoder_buf_dump(dcode->buf, dcode->code128.character));
        ...
        if ((charset & 0x2) && (code < 100))
            ...
            if (charset & 0x2) {
                ...
                zassert(cexp, 1, "i=%x j=%x code=%02x charset=%x cexp=%x %s\n", i, j, code, charset,
                        cexp, _zbar_decoder_buf_dump(dcode->buf, dcode->code128.character));
...
zbar_symbol_type_t _zbar_decode_code128(zbar_decoder_t *dcode)
{
    ...
    if ((dcode128->character < 0) ? get_color(dcode) != ZBAR_SPACE :
    ...
    dbprintf(2, "      code128[%c%02d+%x]:", (dcode128->direction) ? '<' : '>', dcode128->character,
             dcode128->element);
