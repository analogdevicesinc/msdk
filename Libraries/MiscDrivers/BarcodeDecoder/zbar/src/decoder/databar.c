...
static const unsigned char exp_checksums[] = { 1, 189, 62, 113, 46, 43, 109, 134, 6, 79, 161, 45 };
...
static inline int databar_postprocess_exp(zbar_decoder_t *dcode, int *data)
{
    ...
    i = buf - dcode->buf;
    zassert(i < dcode->buf_alloc, -1, "i=%02x %s\n", i, _zbar_decoder_buf_dump(dcode->buf, i));
...
static inline void merge_segment(databar_decoder_t *db, databar_segment_t *seg)
{
    ...
    for (i = 0; i < csegs; i++) {
        ...
        if (s != seg && s->finder == seg->finder && s->exp == seg->exp && s->color == seg->color &&
            s->side == seg->side && s->data == seg->data && s->check == seg->check &&
...
static inline zbar_symbol_type_t match_segment(zbar_decoder_t *dcode, databar_segment_t *seg)
{
    databar_decoder_t *db = &dcode->databar;
    ...
    for (i0 = 0; i0 < csegs; i0++) {
        ...
        if (s0 == seg || s0->finder != seg->finder || s0->exp || s0->color != seg->color ||
            s0->side == seg->side || (s0->partial && s0->count < 4) ||
        ...
        for (i1 = 0; i1 < csegs; i1++) {
            ...
            if (i1 == i0 || s1->finder < 0 || s1->exp || s1->color == seg->color ||
                ...
                continue;
            dbprintf(2, "\n\t[%d,%d] f=%d(0%xx)/%d(%x%x%x)", i0, i1, seg->finder, seg->color,
            ...
            for (i2 = i1 + 1; i2 < csegs; i2++) {
                ...
                if (i2 == i0 || s2->finder != s1->finder || s2->exp || s2->color != s1->color ||
                    s2->side == s1->side || s2->check != chk || (s2->partial && s2->count < 4) ||
...
static inline unsigned lookup_sequence(databar_segment_t *seg, int fixed, int seq[22])
{
...
static inline zbar_symbol_type_t match_segment_exp(zbar_decoder_t *dcode, databar_segment_t *seg,
                                                   int dir)
{
    databar_decoder_t *db = &dcode->databar;
    ...
    for (i = 0;; i--) {
        ...
        for (; i >= 0 && seq[i] >= 0; i--) {
            ...
            if (!i) {
                ...
                dbprintf(2, " %c%x%x@%d", 'A' + seg->finder, seg->color, seg->side, j);
            }
            segs[i++] = j;
            segs[i++] = -1;
        }
        ...
        maxcnt = cnt;
        maxage = age;
        for (i = 0; segs[i] >= 0; i++) bestsegs[i] = segs[i];
        bestsegs[i] = -1;
    }
    ...
    for (i = 0; bestsegs[i] >= 0; i++) segs[i] = db->segs[bestsegs[i]].data;
...
static inline zbar_symbol_type_t decode_char(zbar_decoder_t *dcode, databar_segment_t *seg, int off,
                                             int dir)
{
    databar_decoder_t *db = &dcode->databar;
    ...
    dbprintf(2, "\n        char[%c%d]: n=%d s=%d w=%d sig=", (dir < 0) ? '>' : '<', off, n, s,
             seg->width);
    ...
    i = ((n & 0x3) ^ 1) * 5 + (sum1 >> 1);
    zassert(i < sizeof(groups) / sizeof(*groups), -1, "n=%d sum=%d/%d sig=%04x/%04x g=%d", n, sum0,
            sum1, sig0, sig1, i);
    struct group_s *g = groups + i;
    dbprintf(2, "\n            g=%d(%d,%d,%d/%d)", i, g->sum, g->wmax, g->todd, g->teven);
    ...
    if (seg->exp) {
        ...
        if (seg->finder || seg->color || seg->side) {
            ...
            zassert(i >= 0 && i < 12, ZBAR_NONE, "f=%d(%x%x%x) side=%d i=%d\n", seg->finder,
                    seg->exp, seg->color, seg->side, side, i);
...
zbar_symbol_type_t _zbar_decode_databar(zbar_decoder_t *dcode)
{
    ...
    db->chars[i] = -1;
    seg = db->segs + iseg;
    dbprintf(2, "        databar: i=%d part=%d f=%d(%x%x%x)", iseg, seg->partial, seg->finder,
             seg->exp, seg->color, seg->side);
    zassert(seg->finder >= 0, ZBAR_NONE, "i=%d f=%d(%x%x%x) part=%x\n", iseg, seg->finder, seg->exp,
            seg->color, seg->side, seg->partial);
