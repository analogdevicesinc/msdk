...
static const unsigned char codabar_hi[8] = { 0x1, 0x4, 0x7, 0x6, 0x2, 0x3, 0x0, 0x5 };
...
static inline zbar_symbol_type_t codabar_postprocess(zbar_decoder_t *dcode)
{
    ...
    for (i = 0; i < NIBUF; i++) dcode->buf[i] = codabar->buf[i];
    if (dir)
        /* reverse buffer */
        for (i = 0; i < n / 2; i++) {
            unsigned j = n - 1 - i;
...
zbar_symbol_type_t _zbar_decode_codabar(zbar_decoder_t *dcode)
{
    ...
    dbprintf(1, "      codabar[%c%02d+%x]", (codabar->direction) ? '<' : '>', codabar->character,
             codabar->element);
    ...
    if (c & 0x10) {
        ...
        if (n < CFG(*codabar, ZBAR_CFG_MIN_LEN) ||
            (CFG(*codabar, ZBAR_CFG_MAX_LEN) > 0 && n > CFG(*codabar, ZBAR_CFG_MAX_LEN))) {
