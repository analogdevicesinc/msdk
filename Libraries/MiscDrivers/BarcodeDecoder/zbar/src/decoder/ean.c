...
static inline signed char decode4(zbar_decoder_t *dcode)
{
    ...
    unsigned e1 = ((get_color(dcode) == ZBAR_BAR) ? get_width(dcode, 0) + get_width(dcode, 1) :
    ...
    if ((1 << code) & 0x0660) {
        ...
        unsigned d2 = ((get_color(dcode) == ZBAR_BAR) ? get_width(dcode, 0) + get_width(dcode, 2) :
...
static inline zbar_symbol_type_t ean_part_end4(ean_pass_t *pass, unsigned char fwd)
{
    /* extract parity bits */
...
static inline char ean_part_end5(ean_decoder_t *ean, ean_pass_t *pass)
{
    ...
    /* extract parity bits */
    unsigned char par =
        ((pass->raw[1] & 0x10) | (pass->raw[2] & 0x10) >> 1 | (pass->raw[3] & 0x10) >> 2 |
         (pass->raw[4] & 0x10) >> 3 | (pass->raw[5] & 0x10) >> 4);
    /* calculate checksum */
    unsigned char chk =
        (((pass->raw[1] & 0x0f) + (pass->raw[2] & 0x0f) * 3 + (pass->raw[3] & 0x0f) +
          (pass->raw[4] & 0x0f) * 3 + (pass->raw[5] & 0x0f)) *
         3) %
        10;
...
static inline zbar_symbol_type_t ean_part_end7(ean_decoder_t *ean, ean_pass_t *pass,
                                               unsigned char fwd)
{
    /* calculate parity index */
    unsigned char par = ((fwd) ? ((pass->raw[1] & 0x10) << 1 | (pass->raw[2] & 0x10) |
    ...
    dbprintf(1, "decode=%x%x%x%x%x%x%x(%02x)\n", pass->raw[0] & 0xf, pass->raw[1] & 0xf,
             pass->raw[2] & 0xf, pass->raw[3] & 0xf, pass->raw[4] & 0xf, pass->raw[5] & 0xf,
             pass->raw[6] & 0xf, par);
...
static inline zbar_symbol_type_t decode_pass(zbar_decoder_t *dcode, ean_pass_t *pass)
{
        ...
        if (pass->state & STATE_ADDON) {
            ...
            if ((idx & 7) == 1) {
                ...
                idx += 2;
                   ...
                   TEST_CFG(dcode->ean.ean8_config, ZBAR_CFG_ENABLE) && !aux_end(dcode, fwd)) {
    ...
    if (!(idx & 0x03) && idx <= 0x14) {
        ...
        if ((code < 0 && idx != 0x10) || (idx > 0 && (pass->state & STATE_ADDON) && aux_mid(dcode)))
            pass->state = -1;
        else if (code < 0)
            pass->raw[5] = 0xff;
        else {
            dbprintf(2, "\n        raw[%x]=%02x =>", idx >> 2, digits[(unsigned char)code]);
...
static inline zbar_symbol_type_t integrate_partial(ean_decoder_t *ean, ean_pass_t *pass,
                                                   zbar_symbol_type_t part)
{
    ...
    if (((part == ZBAR_EAN13 || part == ZBAR_UPCE) && ean_verify_checksum(ean, 12)) ||
...
static inline void postprocess(zbar_decoder_t *dcode, zbar_symbol_type_t sym)
{
    ...
    if (base > ZBAR_PARTIAL) {
        ...
        for (; j < base && ean->buf[i] >= 0; i++, j++) dcode->buf[j] = ean->buf[i] + '0';
...
zbar_symbol_type_t _zbar_decode_ean(zbar_decoder_t *dcode)
{
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (pass->state >= 0 || i == pass_idx) {
            zbar_symbol_type_t part;
            dbprintf(2, "      ean[%x/%x]: idx=%x st=%d s=%d", i, pass_idx, dcode->idx, pass->state,
                     dcode->ean.s4);
