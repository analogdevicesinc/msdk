    ...
    if (wc >= 0x2018 && wc <= 0x201a) {
        ...
        ucs4_t substitute =
            (cd->oflags & HAVE_QUOTATION_MARKS ?
                 (wc == 0x201a ? 0x2018 : wc) :
                 (cd->oflags & HAVE_ACCENTS ? (wc == 0x2019 ? 0x00b4 : 0x0060) /* use accents */
                                              :
...
{
    struct uc_to_mb_fallback_locals *plocals = (struct uc_to_mb_fallback_locals *)callback_arg;
...
{
    struct mb_to_uc_fallback_locals *plocals = (struct mb_to_uc_fallback_locals *)callback_arg;
    /* Do nothing if already encountered an error in a previous call. */
    if (plocals->l_errno == 0) {
...
static size_t unicode_loop_convert(iconv_t icd, const char **inbuf, size_t *inbytesleft,
...
// These encoding may not needed in our case (barcode decoder)
#if 0
        if (cd->discard_ilseq) {
          switch (cd->iindex) {
          ...
          goto outcount_zero;
                ...
                else if (cd->fallbacks.mb_to_uc_fallback != NULL)
                {
                    ...
                    cd->fallbacks.mb_to_uc_fallback((const char *)inptr + incount, incount2,
                                                    mb_to_uc_write_replacement, &locals,
                                                    cd->fallbacks.data);
                    if (locals.l_errno != 0) {
                        inptr += incount;
                        inleft -= incount;
