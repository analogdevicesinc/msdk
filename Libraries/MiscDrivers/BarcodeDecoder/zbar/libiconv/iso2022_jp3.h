...
static int iso2022_jp3_mbtowc(conv_t conv, ucs4_t *pwc, const unsigned char *s, size_t n)
{
...
#define COMBINE_STATE_NO_LASTTWO /* assume lasttwo == 0, then prevstate is ignored */

static int iso2022_jp3_wctomb(conv_t conv, unsigned char *r, ucs4_t wc, size_t n)
{
    int count = 0;
    unsigned char buf[2];
    unsigned short jch;
    ...
    if (lasttwo) {
        ...
        if (wc == 0x02e5)
            idx = iso2022_jp3_comp_table02e5_idx, len = iso2022_jp3_comp_table02e5_len;
        else if (wc == 0x02e9)
            idx = iso2022_jp3_comp_table02e9_idx, len = iso2022_jp3_comp_table02e9_len;
        else if (wc == 0x0300)
            idx = iso2022_jp3_comp_table0300_idx, len = iso2022_jp3_comp_table0300_len;
        else if (wc == 0x0301)
            idx = iso2022_jp3_comp_table0301_idx, len = iso2022_jp3_comp_table0301_len;
        else if (wc == 0x309a)
            idx = iso2022_jp3_comp_table309a_idx, len = iso2022_jp3_comp_table309a_len;
        else
            goto not_combining;
...
static int iso2022_jp3_reset(conv_t conv, unsigned char *r, size_t n)
{
    ...
    {
        int count =
            (lasttwo ? (prevstate != state ? 3 : 0) + 2 : 0) + (state != STATE_ASCII ? 3 : 0);
        if (n < count)
            return RET_TOOSMALL;
        if (lasttwo) {
            if (prevstate != state) {
