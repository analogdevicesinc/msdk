...
enum {
    PROVISIONER_CCSTART_TIMER_EVENT =
        PROVISIONER_MSG_START, /*!< Configuration Start Timer event. */
};
...
#define PROVISIONER_PRINT4(msg, var1, var2, var3, var4) APP_TRACE_INFO4(msg, var1, var2, var3, var4)
...
typedef struct provisionerCb_tag {
    ...
    provisionerState_t
        currNodeState; /*!< Current Node provisioning state. See ::provisionerCfgStates. */
    uint8_t
        currNodeCCCommIdx; /*!< Current node configuration client index of common state machine. */
    ...
    provisionerActCCFn_t
        *currNodeStateMachine; /*!< Current node configuration client state machine. */
} provisionerCb_t;
...
static const attcDiscCfg_t discCfgList[] = {
    ...
    { provisionerDataOutCccNtfVal, sizeof(provisionerDataOutCccNtfVal),
      (MPRXC_MPRXS_DOUT_CCC_HDL_IDX) }
};
...
static provisionerActCCFn_t provisionerLightCCFuncs[] = { provisionerCCAppKeyBindGOOMmdlSr,
                                                          provisionerCCSubGOORoom,
                                                          provisionerCCSubGOOMaster,
                                                          provisionerCCEnd };

/*! Configuration Client Room Switch functions */
static provisionerActCCFn_t provisionerRoomSwCCFuncs[] = { provisionerCCAppKeyBindGOOMmdlCl,
                                                           provisionerCCPubGOORoom,
                                                           provisionerCCEnd };

/*! Configuration Client Master Switch functions */
static provisionerActCCFn_t provisionerMasterSwCCFunc[] = { provisionerCCAppKeyBindGOOMmdlCl,
...
static void provisionerCCSetNwkTrans(void)
{
    ...
    MeshCfgMdlClNwkTransmitSet(provCb.currNodePrimAddr, provCb.currNodeNetKeyIdx,
                               provCb.currNodeDevKey, &transState);
}
...
static void provisionerCCExecute(void)
{
    if (provCb.currNodeCCCommIdx <
        (sizeof(provisionerCommonCCFuncs) / sizeof(provisionerActCCFn_t))) {
...
static void provisionerDiscCback(dmConnId_t connId, uint8_t status)
{
    ...
    case APP_DISC_CMPL: {
        ...
        if (provCb.proxyClStarted) {
            PROVISIONER_PRINT3(
                "disc_ind mesh_prx data_in_hdl=0x%x data_out_hdl=0x%x data_out_cccd_hdl=0x%x" PROVISIONER_NEWLINE,
                provCb.hdlList[0], provCb.hdlList[1], provCb.hdlList[2]);
        } else if (provCb.prvGattClStarted) {
            PROVISIONER_PRINT3(
                "disc_ind mesh_prv data_in_hdl=0x%x data_out_hdl=0x%x data_out_cccd_hdl=0x%x" PROVISIONER_NEWLINE,
                provCb.hdlList[0], provCb.hdlList[1], provCb.hdlList[2]);
        }
...
static bool_t provisionerProcessGattBearerScanReport(dmEvt_t *pMsg)
{
    ...
    case DM_EXT_SCAN_REPORT_IND:
        ...
        pData = DmFindAdType(DM_ADV_TYPE_SERVICE_DATA, pMsg->extScanReport.len,
                             pMsg->extScanReport.pData);
    ...
    case DM_SCAN_REPORT_IND:
        /* Service has been found. Look for service data. */
        pData =
            DmFindAdType(DM_ADV_TYPE_SERVICE_DATA, pMsg->scanReport.len, pMsg->scanReport.pData);
...
static bool_t provisionerProcessAdvBearerScanReport(dmEvt_t *pMsg)
{
    ...
    case DM_EXT_SCAN_REPORT_IND:
        /* Service has been found. Look for service data. */
        pData =
            DmFindAdType(MESH_AD_TYPE_BEACON, pMsg->extScanReport.len, pMsg->extScanReport.pData);
        break;
...
static void provisionerProcMeshCoreMsg(meshEvt_t *pMsg)
{
    ...
    case MESH_CORE_GATT_CONN_ADD_EVENT:
        if (pMsg->hdr.status == MESH_SUCCESS) {
            ...
            PROVISIONER_PRINT1("gatt_ind added connid=%d" PROVISIONER_NEWLINE,
                               pMsg->gattConn.connId);

            if (provCb.prvGattClStarted) {
                /* Begin provisioning. */
                MeshPrvClStartPbGattProvisioning(pMsg->gattConn.connId,
                                                 &provisionerPrvClSessionInfo);
            }
        } else {
    ...
    case MESH_CORE_GATT_CONN_CLOSE_EVENT:
        if (pMsg->hdr.status == MESH_SUCCESS) {
            PROVISIONER_PRINT1("gatt_ind close connid=%d" PROVISIONER_NEWLINE,
                               pMsg->gattConn.connId);
    ...
    case MESH_CORE_GATT_CONN_REMOVE_EVENT:
        if (pMsg->hdr.status == MESH_SUCCESS) {
            PROVISIONER_PRINT1("gatt_ind removed connid=%d" PROVISIONER_NEWLINE,
                               pMsg->gattConn.connId);
        } else {
    ...
    case MESH_CORE_NODE_STARTED_EVENT:
        if (pMsg->nodeStarted.hdr.status == MESH_SUCCESS) {
            PROVISIONER_PRINT2(PROVISIONER_NEWLINE
                               "mesh_ind node_started elemaddr=0x%x elemcnt=%d" PROVISIONER_NEWLINE,
                               pMsg->nodeStarted.address, pMsg->nodeStarted.elemCnt);
        } else {
            PROVISIONER_PRINT0(PROVISIONER_NEWLINE
                               "mesh_ind node_started failed" PROVISIONER_NEWLINE);
        }
        break;
...
static void provisionerProcMeshPrvClMsg(const meshPrvClEvt_t *pMsg)
{
    ...
    case MESH_PRV_CL_RECV_CAPABILITIES_EVENT: {
        ...
        PROVISIONER_PRINT8(
            "prvcl_ind capabilities num_elem=%d algo=0x%x oobpk=0x%x static_oob=0x%x "
            "output_oob_size=0x%x output_oob_act=0x%x input_oob_size=0x%x "
            "input_oob_action=0x%x" PROVISIONER_NEWLINE,
            pMsg->recvCapab.capabilities.numOfElements, pMsg->recvCapab.capabilities.algorithms,
            pMsg->recvCapab.capabilities.publicKeyType, pMsg->recvCapab.capabilities.staticOobType,
            pMsg->recvCapab.capabilities.outputOobSize,
            pMsg->recvCapab.capabilities.outputOobAction, pMsg->recvCapab.capabilities.inputOobSize,
            pMsg->recvCapab.capabilities.inputOobAction);
    } break;

    case MESH_PRV_CL_ENTER_OUTPUT_OOB_EVENT:
        PROVISIONER_PRINT1(
            "prvcl_ind enter_output_oob type=%s" PROVISIONER_NEWLINE,
            ((pMsg->enterOutputOob.outputOobAction == MESH_PRV_OUTPUT_OOB_ACTION_OUTPUT_ALPHANUM) ?
                 "alpha" :
                 "num"));
        break;
    ...
    case MESH_PRV_CL_PROVISIONING_COMPLETE_EVENT:
        ...
        provisionerPrvClSessionInfo.pData->address =
            pMsg->prvComplete.address + pMsg->prvComplete.numOfElements;
        ...
        provCb.currNodeState = (provCb.currNodeState == PROVISIONER_ST_PRV_ADV_INPRG) ?
                                   PROVISIONER_ST_CC_ADV_INPRG :
                                   PROVISIONER_ST_CC_GATT_INPRG;
        ...
        PROVISIONER_PRINT3(
            "prvcl_ind prv_complete elemaddr=0x%x elemcnt=%d devkey=0x%s" PROVISIONER_NEWLINE,
...
static void provisionerProcessMmdlLighHslEventCback(const wsfMsgHdr_t *pEvt)
{
    ...
    case MMDL_LIGHT_HSL_CL_STATUS_EVENT:
        if (((mmdlLightHslClStatusEvent_t *)pEvt)->remainingTime > 0) {
            PROVISIONER_PRINT5(
                "lighthsl_ind status addr=0x%x lightness=0x%X hue=0x%X sat=0x%X remtime=0x%X" PROVISIONER_NEWLINE,
            ...
            PROVISIONER_PRINT4(
                "lighthsl_ind status addr=0x%x lightness=0x%X hue=0x%X sat=0x%X " PROVISIONER_NEWLINE,
                ((mmdlLightHslClStatusEvent_t *)pEvt)->serverAddr,
                ((mmdlLightHslClStatusEvent_t *)pEvt)->lightness,
                ((mmdlLightHslClStatusEvent_t *)pEvt)->hue,
                ((mmdlLightHslClStatusEvent_t *)pEvt)->saturation);
    ...
    case MMDL_LIGHT_HSL_CL_TARGET_STATUS_EVENT:
        if (((mmdlLightHslClStatusEvent_t *)pEvt)->remainingTime > 0) {
            PROVISIONER_PRINT5(
                "lighthsl_ind targetstatus addr=0x%x lightness=0x%X hue=0x%X sat=0x%X remtime=0x%X" PROVISIONER_NEWLINE,
                ((mmdlLightHslClStatusEvent_t *)pEvt)->serverAddr,
                ((mmdlLightHslClStatusEvent_t *)pEvt)->lightness,
                ((mmdlLightHslClStatusEvent_t *)pEvt)->hue,
                ((mmdlLightHslClStatusEvent_t *)pEvt)->saturation,
            ...
            PROVISIONER_PRINT4(
                "lighthsl_ind targetstatus addr=0x%x lightness=0x%X hue=0x%X sat=0x%X " PROVISIONER_NEWLINE,
                ((mmdlLightHslClStatusEvent_t *)pEvt)->serverAddr,
                ((mmdlLightHslClStatusEvent_t *)pEvt)->lightness,
                ((mmdlLightHslClStatusEvent_t *)pEvt)->hue,
                ((mmdlLightHslClStatusEvent_t *)pEvt)->saturation);
    ...
    case MMDL_LIGHT_HSL_CL_HUE_STATUS_EVENT:
        if (((mmdlLightHslClHueStatusEvent_t *)pEvt)->remainingTime > 0) {
            PROVISIONER_PRINT4(
                "lighth_ind status addr=0x%x present=0x%X target=0x%X remtime=0x%X" PROVISIONER_NEWLINE,
            ...
            PROVISIONER_PRINT2("lighth_ind status addr=0x%x present=0x%X " PROVISIONER_NEWLINE,
                               ((mmdlLightHslClHueStatusEvent_t *)pEvt)->serverAddr,
                               ((mmdlLightHslClHueStatusEvent_t *)pEvt)->presentHue);
        }
        break;
    ...
    case MMDL_LIGHT_HSL_CL_SAT_STATUS_EVENT:
        if (((mmdlLightHslClSatStatusEvent_t *)pEvt)->remainingTime > 0) {
            PROVISIONER_PRINT4(
                "lights_ind status addr=0x%x present=0x%X target=0x%X remtime=0x%X" PROVISIONER_NEWLINE,
            ...
            PROVISIONER_PRINT2("lights_ind status addr=0x%x present=0x%X " PROVISIONER_NEWLINE,
    ...
    case MMDL_LIGHT_HSL_CL_DEF_STATUS_EVENT:
        PROVISIONER_PRINT4(
            "lighthsl_ind default addr=0x%x lightness=0x%X hue=0x%X sat=0x%X" PROVISIONER_NEWLINE,
