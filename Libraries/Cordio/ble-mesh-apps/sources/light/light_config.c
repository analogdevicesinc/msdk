...
static mmdlGenOnOffState_t lightElem01GenOnOffStates[2]
                                                    [MMDL_GEN_ONOFF_STATE_CNT + MMDL_NUM_OF_SCENES];
static mmdlGenOnPowerUpState_t
    lightElem01GenPowOnOffStates[2][MMDL_GEN_POWER_ONOFF_STATE_CNT + MMDL_NUM_OF_SCENES];
static uint16_t lightElem0Scenes[MMDL_SCENE_STATE_CNT + MMDL_NUM_OF_SCENES];

static mmdlGenDefaultTransState_t
    lightElem1GenDefaultTransStates[MMDL_GEN_DEFAULT_TRANS_STATE_CNT + MMDL_NUM_OF_SCENES];
static mmdlGenLevelState_t lightElem1GenLevelStates[MMDL_GEN_LEVEL_STATE_CNT + MMDL_NUM_OF_SCENES];
static mmdlLightLightnessState_t
    lightElem1LightLightnessStates[MMDL_LIGHT_LIGHTNESS_STATE_CNT + MMDL_NUM_OF_SCENES] = {
        0, /* Actual */
        0, /* Linear */
        0, /* Target */
...
static mmdlLightHslSrStoredState_t lightElem1LightHslSrState = { .minHue = 0,
                                                                 .defaultHue = 1,
                                                                 .maxHue = 0xFFFF,
                                                                 .defaultSat = 1,
                                                                 .minSat = 0,
...
static mmdlLightHslHueStoredState_t lightElem2HueState = { .presentHue = 0 };

static mmdlGenLevelState_t lightElem3GenLevelStates[MMDL_GEN_LEVEL_STATE_CNT + MMDL_NUM_OF_SCENES];
static mmdlLightHslSatStoredState_t lightElem3SatState = { .presentSat = 0 };

/*! Descriptor for the lightElement 0 instance of the Generic OnOff Server */
static mmdlGenOnOffSrDesc_t lightElem0GenOnOffSr = { .pStoredStates =
                                                         lightElem01GenOnOffStates[0] };

/*! Descriptor for the lightElement 0 instance of the Scene Server */
static mmdlSceneSrDesc_t lightElem0SceneSr = { .pStoredScenes = lightElem0Scenes };

/*! Descriptor for the lightElement 0 instance of the Generic Power OnOff Server */
static mmdlGenPowOnOffSrDesc_t lightElem0GenPowOnOffSr = { .pStoredStates =
                                                               lightElem01GenPowOnOffStates[0] };

/*! Descriptor for the lightElement 1 instance of the Generic Power OnOff Server */
static mmdlGenPowOnOffSrDesc_t lightElem1GenPowOnOffSr = { .pStoredStates =
                                                               lightElem01GenPowOnOffStates[1] };

/*! Descriptor for the lightElement 1 instance of the Generic OnOff Server */
static mmdlGenOnOffSrDesc_t lightElem1GenOnOffSr = { .pStoredStates =
                                                         lightElem01GenOnOffStates[1] };
...
static mmdlGenLevelSrDesc_t lightElem1GenLevelSr = { .pStoredStates = lightElem1GenLevelStates };

/*! Descriptor for the lightElement 1 instance of the Scene Server */
static mmdlSceneSrDesc_t lightElem1SceneSr = { .pStoredScenes = lightElem1Scenes };
...
static mmdlLightHslSrDesc_t lightElem1LightHslSr = { .pStoredState = &lightElem1LightHslSrState };

/*! Descriptor for the lightElement 2 instance of Generic Level Server */
static mmdlGenLevelSrDesc_t lightElem2GenLevelSr = { .pStoredStates = lightElem2GenLevelStates };

/*! Descriptor for the lightElement 2 instance of Light HSL Hue Server */
static mmdlLightHslHueSrDesc_t lightElem2LightHslHueSr = { .pStoredState = &lightElem2HueState };

/*! Descriptor for the lightElement 3 instance of Generic Level Server */
static mmdlGenLevelSrDesc_t lightElem3GenLevelSr = { .pStoredStates = lightElem3GenLevelStates };

/*! Descriptor for the lightElement 3 instance of Light HSL Saturation Server */
static mmdlLightHslSatSrDesc_t lightElem3LightHslSatSr = { .pStoredState = &lightElem3SatState };
...
meshPrvSrUnprovisionedDeviceInfo_t lightPrvSrUpdInfo = { &lightPrvSrCapabilities,
...
static void genOnPowerUpNvmSave(meshElementId_t lightElementId)
{
    ...
    WsfNvmWriteData((uint64_t)MMDL_NVM_GEN_ONPOWERUP_STATE_DATASET_ID,
                    (uint8_t *)lightElem01GenPowOnOffStates, sizeof(lightElem01GenPowOnOffStates),
                    NULL);
}
...
{
    WsfNvmWriteData((uint64_t)MMDL_NVM_GEN_ONOFF_STATE_DATASET_ID,
                    (uint8_t *)lightElem01GenOnOffStates, sizeof(lightElem01GenOnOffStates), NULL);
}
...
{
    WsfNvmWriteData((uint64_t)MMDL_NVM_LIGHT_LIGHTNESS_STATE_DATASET_ID,
                    (uint8_t *)lightElem1LightLightnessStates,
...
static void lightHslNvmSave(meshElementId_t lightElementId)
{
    ...
    WsfNvmWriteData((uint64_t)MMDL_NVM_LIGHT_HSL_STATE_DATASET_ID,
                    (uint8_t *)&lightElem1LightHslSrState, sizeof(lightElem1LightHslSrState), NULL);
...
void LightConfig(void)
{
    ...
    retVal = WsfNvmReadData((uint64_t)MMDL_NVM_GEN_ONPOWERUP_STATE_DATASET_ID,
                            (uint8_t *)lightElem01GenPowOnOffStates,
                            sizeof(lightElem01GenPowOnOffStates), NULL);

    memset(lightElem01GenOnOffStates, 0, sizeof(lightElem01GenOnOffStates));

    retVal = WsfNvmReadData((uint64_t)MMDL_NVM_GEN_ONOFF_STATE_DATASET_ID,
                            (uint8_t *)lightElem01GenOnOffStates, sizeof(lightElem01GenOnOffStates),
                            NULL);

    retVal = WsfNvmReadData((uint64_t)MMDL_NVM_LIGHT_LIGHTNESS_STATE_DATASET_ID,
                            (uint8_t *)lightElem1LightLightnessStates,
                            sizeof(lightElem1LightLightnessStates), NULL);

    retVal = WsfNvmReadData((uint64_t)MMDL_NVM_LIGHT_HSL_STATE_DATASET_ID,
                            (uint8_t *)&lightElem1LightHslSrState,
