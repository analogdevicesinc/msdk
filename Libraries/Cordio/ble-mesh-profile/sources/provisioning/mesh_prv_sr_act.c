...
static uint8_t sampleRandom[MESH_PRV_PDU_RANDOM_RANDOM_SIZE] = { 0x55, 0xa2, 0xa2, 0xbc, 0xa0, 0x4c,
                                                                 0xd3, 0x2f, 0xf6, 0xf3, 0x46, 0xbd,
                                                                 0x0a, 0x0c, 0x1a, 0x3a };
...
static void meshPrvSrConfirmationCback(const uint8_t *pCmacResult, void *pParam)
{
    ...
    /* Peer confirmation has been computed */
    {
        ...
        bool_t confirmationVerified =
            (memcmp(pCmacResult, meshPrvSrCb.pSessionData->authParams.peerConfirmation,
...
static void meshPrvSrK1Cback(const uint8_t *pResult, uint8_t resultSize, void *pParam)
{
    ...
    if (pParam == K1_CBACK_ID_CONFIRMATION_KEY) {
        ...
        memcpy(meshPrvSrCb.pSessionData->authParams.confirmationKey, pResult,
               MESH_SEC_TOOL_AES_BLOCK_SIZE);

#if ((defined MESH_PRV_SAMPLE_BUILD) && (MESH_PRV_SAMPLE_BUILD == 1))
        /* Use device Random from sample data */
        memcpy(meshPrvSrCb.pSessionData->authParams.tempRandomAndAuthValue, sampleRandom,
               MESH_PRV_PDU_RANDOM_RANDOM_SIZE);
#else
        /* Generate own Random */
        SecRand(meshPrvSrCb.pSessionData->authParams.tempRandomAndAuthValue,
                MESH_PRV_PDU_RANDOM_RANDOM_SIZE);
#endif /* MESH_PRV_SAMPLE_BUILD */

        /* Save a copy of own Random for Session Key calculation */
        memcpy(&meshPrvSrCb.pSessionData->authParams
        ...
        (void)MeshSecToolK1Derive((uint8_t *)MESH_PRV_SESSION_NONCE_TEMP,
                                  ...
                                  meshPrvSrCb.pSessionData->ecdhSecret, MESH_SEC_TOOL_ECC_KEY_SIZE,
                                  meshPrvSrK1Cback, K1_CBACK_ID_SESSION_NONCE);
        ...
        (void)MeshSecToolK1Derive((uint8_t *)MESH_PRV_DEVICE_KEY_TEMP,
                                  ...
                                  meshPrvSrCb.pSessionData->authParams.provisioningSalt,
                                  meshPrvSrCb.pSessionData->ecdhSecret, MESH_SEC_TOOL_ECC_KEY_SIZE,
                                  meshPrvSrK1Cback, K1_CBACK_ID_DEVICE_KEY);
 ...
 */
/*************************************************************************************************/
static void meshPrvSrSecToolEccKeyGenCback(const uint8_t *pPubX, const uint8_t *pPubY,
                                           const uint8_t *pPriv)
{
...
static void meshPrvSrSaltCback(const uint8_t *pCmacResult, void *pParam)
{
    ...
    if (pParam == SALT_CBACK_ID_CONFIRMATION) {
        ...
        (void)MeshSecToolK1Derive(
            ...
            meshPrvSrCb.pSessionData->ecdhSecret, MESH_SEC_TOOL_ECC_KEY_SIZE, meshPrvSrK1Cback,
        ...
        memcpy(meshPrvSrCb.pSessionData->authParams.provisioningSalt, pCmacResult,
               MESH_SEC_TOOL_AES_BLOCK_SIZE);
        ...
        (void)MeshSecToolK1Derive((uint8_t *)MESH_PRV_SESSION_KEY_TEMP,
                                  ...
                                  meshPrvSrCb.pSessionData->authParams.provisioningSalt,
                                  meshPrvSrCb.pSessionData->ecdhSecret, MESH_SEC_TOOL_ECC_KEY_SIZE,
                                  meshPrvSrK1Cback, K1_CBACK_ID_SESSION_KEY);
    }
    /* Else ignore */
}
...
void meshPrvSrActSendTimeout(void *pCcb, void *pMsg)
{
    ...
    (void)pMsg;
    MESH_TRACE_INFO0(
        "MESH PRV SR: [ACT] Terminate provisioning when unable to send a Provisioning PDU.");
...
void meshPrvSrActWaitLink(void *pCcb, void *pMsg)
{
    ...
    MeshPrvBrEnablePbAdvServer(pEnterPbAdv->ifId, ((meshPrvSrEnterPbAdv_t *)pMsg)->beaconInterval,
                               pSr->pUpdInfo->pDeviceUuid, pSr->pUpdInfo->oobInfoSrc,
                               (uint8_t *)pSr->pUpdInfo->pUriData, pSr->pUpdInfo->uriLen);
}
...
void meshPrvSrActSendCapabilities(void *pCcb, void *pMsg)
{
    ...
    if (pBuf != NULL) {
        ...
        UINT16_TO_BE_BUF(&pBuf[MESH_PRV_PDU_CAPAB_ALGORITHMS_INDEX],
                         pSr->pUpdInfo->pCapabilities->algorithms);
        ...
        UINT16_TO_BE_BUF(&pBuf[MESH_PRV_PDU_CAPAB_OUT_OOB_ACT_INDEX],
                         pSr->pUpdInfo->pCapabilities->outputOobAction);
        pBuf[MESH_PRV_PDU_CAPAB_IN_OOB_SIZE_INDEX] = pSr->pUpdInfo->pCapabilities->inputOobSize;
        UINT16_TO_BE_BUF(&pBuf[MESH_PRV_PDU_CAPAB_IN_OOB_ACT_INDEX],
                         pSr->pUpdInfo->pCapabilities->inputOobAction);
#endif /* MESH_PRV_SAMPLE_BUILD */
        ...
        memcpy(&pSr->pSessionData->authParams.confirmationInputs[MESH_PRV_PDU_INVITE_PARAM_SIZE],
               &pBuf[MESH_PRV_PDU_PARAM_INDEX], MESH_PRV_PDU_CAPAB_PARAM_SIZE);
...
void meshPrvSrActWaitPublicKey(void *pCcb, void *pMsg)
{
    ...
    memcpy(&pSr->pSessionData->authParams
                .confirmationInputs[MESH_PRV_PDU_INVITE_PARAM_SIZE + MESH_PRV_PDU_CAPAB_PARAM_SIZE],
           pStart->packedPduParam, MESH_PRV_PDU_START_PARAM_SIZE);
...
void meshPrvSrActGeneratePublicKey(void *pCcb, void *pMsg)
{
    ...
    /* Application has provided ECC keys */
    {
        ...
        memcpy(&pSr->pSessionData->eccKeys, pSr->pUpdInfo->pAppOobEccKeys,
               sizeof(meshPrvEccKeys_t));

        /* Simulate that the Public Key has been generated */
        MESH_TRACE_INFO0(
            "MESH PRV SR: Public Key provided by the application. Simulating PublicKeyGenerated event...");
...
void meshPrvSrActValidatePublicKey(void *pCcb, void *pMsg)
{
    ...
    pPeerPubX =
        &meshPrvSrCb.pSessionData->authParams
             .confirmationInputs[MESH_PRV_PDU_INVITE_PARAM_SIZE + MESH_PRV_PDU_CAPAB_PARAM_SIZE +
                                 MESH_PRV_PDU_START_PARAM_SIZE];
    pPeerPubY =
        &meshPrvSrCb.pSessionData->authParams
             .confirmationInputs[MESH_PRV_PDU_INVITE_PARAM_SIZE + MESH_PRV_PDU_CAPAB_PARAM_SIZE +
                                 MESH_PRV_PDU_START_PARAM_SIZE + MESH_SEC_TOOL_ECC_KEY_SIZE];
...
void meshPrvSrActSendPublicKey(void *pCcb, void *pMsg)
{
    ...
    /* Public Key of the Server is not available OOB at the Client side */
    {
        ...
        if (pBuf != NULL) {
            ...
            memcpy(&pBuf[MESH_PRV_PDU_PUB_KEY_X_INDEX], pSr->pSessionData->eccKeys.pPubKeyX,
                   MESH_PRV_PDU_PUB_KEY_X_SIZE);
            memcpy(&pBuf[MESH_PRV_PDU_PUB_KEY_Y_INDEX], pSr->pSessionData->eccKeys.pPubKeyY,
                   MESH_PRV_PDU_PUB_KEY_Y_SIZE);

            /* Copy own public key to the ConfirmationInputs */
            memcpy(&pSr->pSessionData->authParams.confirmationInputs
                        [MESH_PRV_PDU_INVITE_PARAM_SIZE + MESH_PRV_PDU_CAPAB_PARAM_SIZE +
    ...
    /* Public Key of the Server is available OOB at the Client side */
    {
        ...
        MESH_TRACE_INFO0(
            "MESH PRV SR: Public Key available OOB at Client-side. Simulating SentPublicKey event...");
        ...
        memcpy(&pSr->pSessionData->authParams.confirmationInputs[MESH_PRV_PDU_INVITE_PARAM_SIZE +
               ...
               pPubKeyX, MESH_PRV_PDU_PUB_KEY_PARAM_SIZE / 2);
        memcpy(
            &pSr->pSessionData->authParams
                 .confirmationInputs[MESH_PRV_PDU_INVITE_PARAM_SIZE +
                                     MESH_PRV_PDU_CAPAB_PARAM_SIZE + MESH_PRV_PDU_START_PARAM_SIZE +
                                     MESH_PRV_PDU_PUB_KEY_PARAM_SIZE +
                                     MESH_PRV_PDU_PUB_KEY_PARAM_SIZE / 2],
            pPubKeyY, MESH_PRV_PDU_PUB_KEY_PARAM_SIZE / 2);
...
void meshPrvSrActPrepareOobAction(void *pCcb, void *pMsg)
{
    ...
    case MESH_PRV_START_AUTH_METHOD_INPUT_OOB:
        ...
        MESH_TRACE_INFO0(
            "MESH PRV SR: Authentication method is Input OOB. Changing to WAIT_INPUT...");
    ...
    case MESH_PRV_START_AUTH_METHOD_OUTPUT_OOB:
        ...
        if (pSr->pSessionData->startParams.authAction ==
            MESH_PRV_START_OUT_OOB_ACTION_ALPHANUMERIC) {
            /* Generate array of alphanumeric values, right-padded with zeros */
            pOutput = &pSr->pSessionData->authParams
            ...
            memset(&pSr->pSessionData->authParams
                        .tempRandomAndAuthValue[MESH_PRV_PDU_RANDOM_RANDOM_SIZE +
                                                pSr->pSessionData->startParams.authSize],
                   0x00, MESH_PRV_AUTH_VALUE_SIZE - pSr->pSessionData->startParams.authSize);

            /* Copy to upper layer event parameter */
            evtMsg.outputOobSize = pSr->pSessionData->startParams.authSize;
            memcpy(evtMsg.outputOobData.alphanumericOob, pOutput,
                   pSr->pSessionData->startParams.authSize);
            if (pSr->pSessionData->startParams.authSize < MESH_PRV_INOUT_OOB_MAX_SIZE) {
                memset(
                    &evtMsg.outputOobData.alphanumericOob[pSr->pSessionData->startParams.authSize],
                    0x00, MESH_PRV_INOUT_OOB_MAX_SIZE - pSr->pSessionData->startParams.authSize);
            }
        } else {
        ...
        evtMsg.outputOobAction =
            (meshPrvOutputOobAction_t)(1 << pSr->pSessionData->startParams.authAction);
        pSr->prvSrEvtNotifyCback((meshPrvSrEvt_t *)&evtMsg);
    ...
    case MESH_PRV_START_AUTH_METHOD_NO_OOB:
        ...
        MESH_TRACE_INFO0(
            "MESH PRV SR: Authentication method is No OOB. Changing to WAIT_CONFIRMATION...");
        pEvtMsg = WsfMsgAlloc(sizeof(wsfMsgHdr_t));
        if (pEvtMsg != NULL) {
    ...
    case MESH_PRV_START_AUTH_METHOD_STATIC_OOB:
        ...
        memcpy(
            &pSr->pSessionData->authParams.tempRandomAndAuthValue[MESH_PRV_PDU_RANDOM_RANDOM_SIZE],
            pSr->pUpdInfo->pStaticOobData, MESH_PRV_AUTH_VALUE_SIZE);

        /* Go to WAIT_CONFIRMATION state. */
        MESH_TRACE_INFO0(
            "MESH PRV SR: Authentication method is Static OOB. Changing to WAIT_CONFIRMATION...");
        pEvtMsg = WsfMsgAlloc(sizeof(wsfMsgHdr_t));
        if (pEvtMsg != NULL) {
            if (pSr->pSessionData->authParams.peerConfirmationReceived == TRUE) {
                pEvtMsg->event = PRV_SR_EVT_RECV_CONFIRMATION;
...
void meshPrvSrActWaitInput(void *pCcb, void *pMsg)
{
    ...
    evtMsg.hdr.status = MESH_SUCCESS;
    evtMsg.inputOobAction =
        (meshPrvInputOobAction_t)(1 << pSr->pSessionData->startParams.authAction);
    pSr->prvSrEvtNotifyCback((meshPrvSrEvt_t *)&evtMsg);
}
...
void meshPrvSrActSendInputComplete(void *pCcb, void *pMsg)
{
    ...
    meshPrvPackInOutOobToAuthArray(
        &pSr->pSessionData->authParams.tempRandomAndAuthValue[MESH_PRV_PDU_RANDOM_RANDOM_SIZE],
        pOob->inputOobData, pOob->inputOobSize);
...
void meshPrvSrActSaveConfirmation(void *pCcb, void *pMsg)
{
    ...
    memcpy(pSr->pSessionData->authParams.peerConfirmation, pConfirm->confirm,
...
void meshPrvSrActCalcConfirmation(void *pCcb, void *pMsg)
{
    ...
    (void)MeshSecToolGenerateSalt(pSr->pSessionData->authParams.confirmationInputs,
                                  MESH_PRV_CONFIRMATION_INPUTS_SIZE, meshPrvSrSaltCback,
...
void meshPrvSrActCheckConfirmation(void *pCcb, void *pMsg)
{
    ...
    memcpy(pSr->pSessionData->authParams.tempRandomAndAuthValue, pRandom->random,
           MESH_PRV_PDU_RANDOM_RANDOM_SIZE);
    ...
    MeshSecToolCmacCalculate(pSr->pSessionData->authParams.confirmationKey,
                             ...
                             MESH_PRV_PDU_RANDOM_RANDOM_SIZE + MESH_PRV_AUTH_VALUE_SIZE,
                             meshPrvSrConfirmationCback, CONFIRMATION_CBACK_ID_PEER);
}
...
void meshPrvSrActCalcSessionKey(void *pCcb, void *pMsg)
{
    ...
    (void)MeshSecToolGenerateSalt(pSr->pSessionData->authParams.confirmationSaltAndFinalRandoms,
                                  ...
                                  meshPrvSrSaltCback, SALT_CBACK_ID_PROVISIONING);
}
...
void meshPrvSrActDecryptData(void *pCcb, void *pMsg)
{
    ...
    memcpy(pSr->pSessionData->provisioningDataAndMic, pData->encryptedDataAndMic,
    ...
    (void)MeshSecToolCcmEncryptDecrypt(MESH_SEC_TOOL_CCM_DECRYPT, &params,
                                       meshPrvSrDataDecryptCback, NULL);
}
...
void meshPrvSrActSendComplete(void *pCcb, void *pMsg)
{
    ...
    if (pBuf != NULL) {
        ...
        evtMsg.flags =
            pSr->pSessionData->provisioningDataAndMic[MESH_PRV_DECRYPTED_DATA_FLAGS_INDEX];
        BYTES_BE_TO_UINT32(
            evtMsg.ivIndex,
