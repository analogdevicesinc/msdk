   ...
   * and when it has been indicated by the device in the provisioning capabilities */
    if (pParams->oobAuthMethod == MESH_PRV_CL_USE_STATIC_OOB &&
        (meshPrvClCb.pSessionInfo->pStaticOobData == NULL ||
         (!(meshPrvClCb.pSessionData->deviceCapab.staticOobType &
    ...
    if (pParams->oobAuthMethod == MESH_PRV_CL_USE_OUTPUT_OOB) {
        ...
        if (!(pParams->oobAction.outputOobAction &
              meshPrvClCb.pSessionData->deviceCapab.outputOobAction)) {
            return FALSE;
        }
    }
    ...
    if (pParams->oobAuthMethod == MESH_PRV_CL_USE_INPUT_OOB) {
        ...
        /* Check action is supported by the device */
        if (!(pParams->oobAction.inputOobAction &
              meshPrvClCb.pSessionData->deviceCapab.inputOobAction)) {
            return FALSE;
        }
    }
...
static void meshPrvClPduRecvCback(const uint8_t *pPrvPdu, uint8_t pduLen)
{
    ...
    case MESH_PRV_PDU_CAPABILITIES:
        if (pduLen != MESH_PRV_PDU_CAPAB_PDU_SIZE) {
            MESH_TRACE_WARN1(
                "MESH PRV CL: Received invalid Provisioning Capabilities PDU length: %d", pduLen);
            pMsg->hdr.event = PRV_CL_EVT_BAD_PDU;
        } else {
            /* Set event and parameters */
            pMsg->recvCapab.hdr.event = PRV_CL_EVT_RECV_CAPABILITIES;
            memcpy(pMsg->recvCapab.capabPdu, pPrvPdu, MESH_PRV_PDU_CAPAB_PDU_SIZE);
        }
        break;

    case MESH_PRV_PDU_PUB_KEY:
        if (pduLen != MESH_PRV_PDU_PUB_KEY_PDU_SIZE) {
            MESH_TRACE_WARN1("MESH PRV CL: Received invalid Provisioning Public Key PDU length: %d",
                             pduLen);
            pMsg->hdr.event = PRV_CL_EVT_BAD_PDU;
        } else {
    ...
    case MESH_PRV_PDU_INPUT_COMPLETE:
        if (pduLen != MESH_PRV_PDU_INPUT_COMPLETE_PDU_SIZE) {
            MESH_TRACE_WARN1(
                "MESH PRV CL: Received invalid Provisioning Input Complete PDU length: %d", pduLen);
            pMsg->hdr.event = PRV_CL_EVT_BAD_PDU;
        } else {
    ...
    case MESH_PRV_PDU_CONFIRMATION:
        if (pduLen != MESH_PRV_PDU_CONFIRM_PDU_SIZE) {
            MESH_TRACE_WARN1(
                "MESH PRV CL: Received invalid Provisioning Confirmation PDU length: %d", pduLen);
            pMsg->hdr.event = PRV_CL_EVT_BAD_PDU;
        } else {
    ...
    case MESH_PRV_PDU_RANDOM:
        if (pduLen != MESH_PRV_PDU_RANDOM_PDU_SIZE) {
            MESH_TRACE_WARN1("MESH PRV CL: Received invalid Provisioning Random PDU length: %d",
                             pduLen);
            pMsg->hdr.event = PRV_CL_EVT_BAD_PDU;
        } else {
    ...
    case MESH_PRV_PDU_COMPLETE:
        if (pduLen != MESH_PRV_PDU_COMPLETE_PDU_SIZE) {
            MESH_TRACE_WARN1("MESH PRV CL: Received invalid Provisioning Complete PDU length: %d",
                             pduLen);
            pMsg->hdr.event = PRV_CL_EVT_BAD_PDU;
        } else {
    ...
    case MESH_PRV_PDU_DATA:
        MESH_TRACE_WARN1("MESH PRV CL: Received unexpected Provisioning PDU type: 0x%02X",
                         pPrvPdu[0]);
        pMsg->hdr.event = PRV_CL_EVT_BAD_PDU;
        break;
 ...
 */
/*************************************************************************************************/
void MeshPrvClStartPbAdvProvisioning(uint8_t ifId, meshPrvClSessionInfo_t const *pSessionInfo)
{
    ...
    /* Validate parameters */
    if (pSessionInfo == NULL || pSessionInfo->pDeviceUuid == NULL || pSessionInfo->pData == NULL) {
 ...
 */
/*************************************************************************************************/
void MeshPrvClStartPbGattProvisioning(uint8_t connId, meshPrvClSessionInfo_t const *pSessionInfo)
{
