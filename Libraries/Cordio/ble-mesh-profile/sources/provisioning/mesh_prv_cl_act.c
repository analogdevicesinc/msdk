...
#define CONFIRMATION_CBACK_ID_PEER \
    ((void *)1) /*! Identifies result of computing peer Confirmation */

/*! Macros to identify the result of a Salt computation */
#define SALT_CBACK_ID_CONFIRMATION \
    ((void *)0) /*! Identifies result of computing ConfirmationSalt */
#define SALT_CBACK_ID_PROVISIONING \
    ((void *)1) /*! Identifies result of computing ProvisioningSalt */

/*! Macros to identify the result of a K1 computation */
#define K1_CBACK_ID_CONFIRMATION_KEY \
    ((void *)0) /*! Identifies result of computing ConfirmationKey */
...
static void meshPrvClConfirmationCback(const uint8_t *pCmacResult, void *pParam)
{
    ...
    /* Own confirmation has been computed */
    {
        ...
        if (pMsg != NULL) {
            ...
            memcpy(pMsg->confirmation, pCmacResult, MESH_PRV_PDU_CONFIRM_CONFIRM_SIZE);
    ...
    /* Peer confirmation has been computed */
    {
        ...
        bool_t confirmationVerified =
            (memcmp(pCmacResult, meshPrvClCb.pSessionData->authParams.peerConfirmation,
...
static void meshPrvClK1Cback(const uint8_t *pResult, uint8_t resultSize, void *pParam)
{
    ...
    if (pParam == K1_CBACK_ID_CONFIRMATION_KEY) {
        ...
        memcpy(meshPrvClCb.pSessionData->authParams.confirmationKey, pResult,
               MESH_SEC_TOOL_AES_BLOCK_SIZE);

        /* Generate own Random */
        SecRand(meshPrvClCb.pSessionData->authParams.tempRandomAndAuthValue,
                MESH_PRV_PDU_RANDOM_RANDOM_SIZE);

        /* Save a copy of own Random for Session Key Calculation */
        memcpy(&meshPrvClCb.pSessionData->authParams
                    .confirmationSaltAndFinalRandoms[MESH_PRV_CONFIRMATION_SALT_SIZE],
        ...
        (void)MeshSecToolK1Derive((uint8_t *)MESH_PRV_SESSION_NONCE_TEMP,
                                  ...
                                  meshPrvClCb.pSessionData->ecdhSecret, MESH_SEC_TOOL_ECC_KEY_SIZE,
                                  meshPrvClK1Cback, K1_CBACK_ID_SESSION_NONCE);
        ...
        (void)MeshSecToolK1Derive((uint8_t *)MESH_PRV_DEVICE_KEY_TEMP,
                                  ...
                                  meshPrvClCb.pSessionData->authParams.provisioningSalt,
                                  meshPrvClCb.pSessionData->ecdhSecret, MESH_SEC_TOOL_ECC_KEY_SIZE,
                                  meshPrvClK1Cback, K1_CBACK_ID_DEVICE_KEY);
 ...
 */
/*************************************************************************************************/
static void meshPrvClSecToolEccKeyGenCback(const uint8_t *pPubX, const uint8_t *pPubY,
                                           const uint8_t *pPriv)
{
...
static void meshPrvClSaltCback(const uint8_t *pCmacResult, void *pParam)
{
    ...
    if (pParam == SALT_CBACK_ID_CONFIRMATION) {
        ...
        (void)MeshSecToolK1Derive(
            (uint8_t *)MESH_PRV_CONFIRMATION_KEY_TEMP, sizeof(MESH_PRV_CONFIRMATION_KEY_TEMP) - 1,
            meshPrvClCb.pSessionData->authParams.confirmationSaltAndFinalRandoms,
            meshPrvClCb.pSessionData->ecdhSecret, MESH_SEC_TOOL_ECC_KEY_SIZE, meshPrvClK1Cback,
        ...
        memcpy(meshPrvClCb.pSessionData->authParams.provisioningSalt, pCmacResult,
               MESH_SEC_TOOL_AES_BLOCK_SIZE);
        ...
        (void)MeshSecToolK1Derive((uint8_t *)MESH_PRV_SESSION_KEY_TEMP,
                                  ...
                                  meshPrvClCb.pSessionData->authParams.provisioningSalt,
                                  meshPrvClCb.pSessionData->ecdhSecret, MESH_SEC_TOOL_ECC_KEY_SIZE,
                                  meshPrvClK1Cback, K1_CBACK_ID_SESSION_KEY);
    }
    /* Else ignore */
}
...
void meshPrvClActSendTimeout(void *pCcb, void *pMsg)
{
    ...
    (void)pMsg;
    MESH_TRACE_INFO0(
        "MESH PRV CL: [ACT] Terminate provisioning when unable to send a Provisioning PDU.");
...
void meshPrvClActSendInvite(void *pCcb, void *pMsg)
{
    ...
    if (pBuf != NULL) {
        ...
        memcpy(pCl->pSessionData->authParams.confirmationInputs, &pBuf[MESH_PRV_PDU_PARAM_INDEX],
...
void meshPrvClActWaitSelectAuth(void *pCcb, void *pMsg)
{
    ...
    MESH_TRACE_INFO0(
        "MESH PRV CL: [ACT] Send capabilities to the application and wait for selection.");
    ...
    memcpy(&pCl->pSessionData->authParams.confirmationInputs[MESH_PRV_PDU_INVITE_PARAM_SIZE],
           &pRecvCapab->capabPdu[MESH_PRV_PDU_PARAM_INDEX], MESH_PRV_PDU_CAPAB_PARAM_SIZE);

    /* Unpack capabilities for upper layer event notification */
    recvCapabEvt.capabilities.numOfElements =
        pRecvCapab->capabPdu[MESH_PRV_PDU_CAPAB_NUM_ELEM_INDEX];
    BYTES_BE_TO_UINT16(recvCapabEvt.capabilities.algorithms,
                       &pRecvCapab->capabPdu[MESH_PRV_PDU_CAPAB_ALGORITHMS_INDEX]);
    recvCapabEvt.capabilities.publicKeyType =
        pRecvCapab->capabPdu[MESH_PRV_PDU_CAPAB_PUB_KEY_TYPE_INDEX];
    recvCapabEvt.capabilities.staticOobType =
        pRecvCapab->capabPdu[MESH_PRV_PDU_CAPAB_STATIC_OOB_INDEX];
    recvCapabEvt.capabilities.outputOobSize =
        pRecvCapab->capabPdu[MESH_PRV_PDU_CAPAB_OUT_OOB_SIZE_INDEX];
    BYTES_BE_TO_UINT16(recvCapabEvt.capabilities.outputOobAction,
                       &pRecvCapab->capabPdu[MESH_PRV_PDU_CAPAB_OUT_OOB_ACT_INDEX]);
    recvCapabEvt.capabilities.inputOobSize =
        pRecvCapab->capabPdu[MESH_PRV_PDU_CAPAB_IN_OOB_SIZE_INDEX];
    BYTES_BE_TO_UINT16(recvCapabEvt.capabilities.inputOobAction,
                       &pRecvCapab->capabPdu[MESH_PRV_PDU_CAPAB_IN_OOB_ACT_INDEX]);

    /* Save unpacked capabilities for later use */
    memcpy(&pCl->pSessionData->deviceCapab, &recvCapabEvt.capabilities,
           sizeof(meshPrvCapabilities_t));
...
void meshPrvClActSendStart(void *pCcb, void *pMsg)
{
    ...
    memcpy(&meshPrvClCb.pSessionData->selectAuth, &pSelectAuth->selectAuthParams,
    ...
    if (pBuf != NULL) {
        ...
        memcpy(&pCl->pSessionData->authParams.confirmationInputs[MESH_PRV_PDU_INVITE_PARAM_SIZE +
               ...
               &pBuf[MESH_PRV_PDU_PARAM_INDEX], MESH_PRV_PDU_START_PARAM_SIZE);
...
void meshPrvClActSendPublicKey(void *pCcb, void *pMsg)
{
    ...
    if (pBuf != NULL) {
        ...
        memcpy(&pCl->pSessionData->authParams.confirmationInputs[MESH_PRV_PDU_INVITE_PARAM_SIZE +
               ...
               &pBuf[MESH_PRV_PDU_PARAM_INDEX], MESH_PRV_PDU_PUB_KEY_PARAM_SIZE);
...
void meshPrvClActWaitPublicKey(void *pCcb, void *pMsg)
{
    ...
    /* Peer needs to send public key over-the-air */
    {
        ...
        MESH_TRACE_INFO0(
            "MESH PRV CL: Public Key available OOB. Simulating Public Key Received event...");
        ...
        if (pEvtMsg != NULL) {
            ...
            memcpy(&pEvtMsg->pubKeyPdu[MESH_PRV_PDU_PUB_KEY_X_INDEX],
                   pCl->pSessionInfo->pDevicePublicKey->pPubKeyX, MESH_PRV_PDU_PUB_KEY_X_SIZE);
            memcpy(&pEvtMsg->pubKeyPdu[MESH_PRV_PDU_PUB_KEY_Y_INDEX],
                   pCl->pSessionInfo->pDevicePublicKey->pPubKeyY, MESH_PRV_PDU_PUB_KEY_Y_SIZE);
...
void meshPrvClActValidatePublicKey(void *pCcb, void *pMsg)
{
    ...
    memcpy(
        &pCl->pSessionData->authParams
             .confirmationInputs[MESH_PRV_PDU_INVITE_PARAM_SIZE + MESH_PRV_PDU_CAPAB_PARAM_SIZE +
                                 MESH_PRV_PDU_START_PARAM_SIZE + MESH_PRV_PDU_PUB_KEY_PARAM_SIZE],
        /* This is the public key of the Device,
                                                     * so it goes right after Provisioner's */
        &pRecvPubKey->pubKeyPdu[MESH_PRV_PDU_PARAM_INDEX], MESH_PRV_PDU_PUB_KEY_PARAM_SIZE);

    /* Compute ECDH Secret */
    uint8_t *pPeerPubX, *pPeerPubY, *pLocalPriv;
    pPeerPubX =
        &meshPrvClCb.pSessionData->authParams
             .confirmationInputs[MESH_PRV_PDU_INVITE_PARAM_SIZE + MESH_PRV_PDU_CAPAB_PARAM_SIZE +
                                 MESH_PRV_PDU_START_PARAM_SIZE + MESH_PRV_PDU_PUB_KEY_PARAM_SIZE];
    pPeerPubY =
        &meshPrvClCb.pSessionData->authParams
    ...
    (void)MeshSecToolEccCompSharedSecret(pPeerPubX, pPeerPubY, pLocalPriv,
                                         meshPrvClEcdhSecretCback);
}
...
void meshPrvClActGeneratePublicKey(void *pCcb, void *pMsg)
{
    ...
    /* Application has provided ECC keys */
    {
        ...
        memcpy(&pCl->pSessionData->eccKeys, pCl->pSessionInfo->pAppEccKeys,
               sizeof(meshPrvEccKeys_t));

        /* Simulate that the Public Key has been generated */
        MESH_TRACE_INFO0(
            "MESH PRV CL: Public Key provided by the application. Simulating PublicKeyGenerated event...");
...
void meshPrvClActPrepareOobAction(void *pCcb, void *pMsg)
{
    ...
    case MESH_PRV_START_AUTH_METHOD_OUTPUT_OOB:
        ...
        MESH_TRACE_INFO0(
            "MESH PRV CL: Authentication method is Output OOB. Changing to WAIT_INPUT...");
    ...
    case MESH_PRV_START_AUTH_METHOD_INPUT_OOB:
        ...
        MESH_TRACE_INFO0(
            "MESH PRV CL: Authentication method is Input OOB. Changing to WAIT_INPUT_COMPLETE...");
    ...
    case MESH_PRV_START_AUTH_METHOD_NO_OOB:
        ...
        memset(
            ...
            0x00, MESH_PRV_AUTH_VALUE_SIZE);

        /* Go to CALC_CONFIRMATION state. */
        MESH_TRACE_INFO0(
            "MESH PRV CL: Authentication method is No OOB. Changing to CALC_CONFIRMATION...");
    ...
    case MESH_PRV_START_AUTH_METHOD_STATIC_OOB:
        if (pCl->pSessionInfo->pStaticOobData == NULL) {
            ...
            memcpy(&pCl->pSessionData->authParams
                        .tempRandomAndAuthValue[MESH_PRV_PDU_RANDOM_RANDOM_SIZE],
                   pCl->pSessionInfo->pStaticOobData, MESH_PRV_AUTH_VALUE_SIZE);

            /* Go to CALC_CONFIRMATION state. */
            MESH_TRACE_INFO0(
                "MESH PRV CL: Authentication method is Static OOB. Changing to CALC_CONFIRMATION...");
            pEvtMsg->event = PRV_CL_EVT_GOTO_CONFIRMATION;
            WsfMsgSend(pCl->timer.handlerId, pEvtMsg);
        }
        break;
...
void meshPrvClActWaitInputComplete(void *pCcb, void *pMsg)
{
    ...
    if (pCl->pSessionData->selectAuth.oobAction.inputOobAction ==
        ...
        memset(pInput + pCl->pSessionData->selectAuth.oobSize, 0x00,
        ...
        if (pCl->pSessionData->selectAuth.oobSize < MESH_PRV_INOUT_OOB_MAX_SIZE) {
            memset(&evtMsg.inputOobData.alphanumericOob[pCl->pSessionData->selectAuth.oobSize],
                   0x00, MESH_PRV_INOUT_OOB_MAX_SIZE - pCl->pSessionData->selectAuth.oobSize);
        }
    } else {
        ...
        memset(
            &pCl->pSessionData->authParams.tempRandomAndAuthValue[MESH_PRV_PDU_RANDOM_RANDOM_SIZE],
            0x00, MESH_PRV_AUTH_VALUE_SIZE - MESH_PRV_NUMERIC_OOB_SIZE_OCTETS);
...
void meshPrvClActCalcConfirmation(void *pCcb, void *pMsg)
{
    ...
    if (pCl->pSessionData->selectAuth.oobAuthMethod == MESH_PRV_CL_USE_OUTPUT_OOB) {
        ...
        meshPrvPackInOutOobToAuthArray(
            &pCl->pSessionData->authParams.tempRandomAndAuthValue[MESH_PRV_PDU_RANDOM_RANDOM_SIZE],
            pOob->outputOobData, pOob->outputOobSize);
    }
    ...
    (void)MeshSecToolGenerateSalt(pCl->pSessionData->authParams.confirmationInputs,
                                  MESH_PRV_CONFIRMATION_INPUTS_SIZE, meshPrvClSaltCback,
...
void meshPrvClActSendRandom(void *pCcb, void *pMsg)
{
    ...
    memcpy(pCl->pSessionData->authParams.peerConfirmation, pConfirm->confirm,
           MESH_PRV_PDU_CONFIRM_CONFIRM_SIZE);
...
void meshPrvClActCheckConfirmation(void *pCcb, void *pMsg)
{
    ...
    memcpy(pCl->pSessionData->authParams.tempRandomAndAuthValue, pRandom->random,
           MESH_PRV_PDU_RANDOM_RANDOM_SIZE);
    ...
    memcpy(&pCl->pSessionData->authParams
                .confirmationSaltAndFinalRandoms[MESH_PRV_CONFIRMATION_SALT_SIZE +
                                                 MESH_PRV_PDU_RANDOM_RANDOM_SIZE],
           pRandom->random, MESH_PRV_PDU_RANDOM_RANDOM_SIZE);
    ...
    MeshSecToolCmacCalculate(pCl->pSessionData->authParams.confirmationKey,
                             ...
                             MESH_PRV_PDU_RANDOM_RANDOM_SIZE + MESH_PRV_AUTH_VALUE_SIZE,
                             meshPrvClConfirmationCback, CONFIRMATION_CBACK_ID_PEER);
}
...
void meshPrvClActCalcSessionKey(void *pCcb, void *pMsg)
{
    ...
    (void)MeshSecToolGenerateSalt(pCl->pSessionData->authParams.confirmationSaltAndFinalRandoms,
                                  MESH_PRV_CONFIRMATION_SALT_SIZE +
                                      2 * MESH_PRV_PDU_RANDOM_RANDOM_SIZE,
                                  meshPrvClSaltCback, SALT_CBACK_ID_PROVISIONING);
}
...
void meshPrvClActEncryptData(void *pCcb, void *pMsg)
{
    ...
    memcpy(&pCl->pSessionData->provisioningDataAndMic[MESH_PRV_DECRYPTED_DATA_NETKEY_INDEX],
           pCl->pSessionInfo->pData->pNetKey, MESH_KEY_SIZE_128);
    UINT16_TO_BE_BUF(
        &pCl->pSessionData->provisioningDataAndMic[MESH_PRV_DECRYPTED_DATA_NETKEYIDX_INDEX],
        pCl->pSessionInfo->pData->netKeyIndex);
    pCl->pSessionData->provisioningDataAndMic[MESH_PRV_DECRYPTED_DATA_FLAGS_INDEX] =
        pCl->pSessionInfo->pData->flags;
    UINT32_TO_BE_BUF(
        &pCl->pSessionData->provisioningDataAndMic[MESH_PRV_DECRYPTED_DATA_IVIDX_INDEX],
        pCl->pSessionInfo->pData->ivIndex);
    UINT16_TO_BE_BUF(
        &pCl->pSessionData->provisioningDataAndMic[MESH_PRV_DECRYPTED_DATA_ADDRESS_INDEX],
    ...
    (void)MeshSecToolCcmEncryptDecrypt(MESH_SEC_TOOL_CCM_ENCRYPT, &params,
                                       meshPrvClDataEncryptCback, NULL);
}
...
void meshPrvClActSendData(void *pCcb, void *pMsg)
{
    ...
    if (pBuf != NULL) {
        ...
        memcpy(&pBuf[MESH_PRV_PDU_DATA_ENC_DATA_INDEX], &pCl->pSessionData->provisioningDataAndMic,
