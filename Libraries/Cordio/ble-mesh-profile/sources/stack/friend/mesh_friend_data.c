    ...
    for (; idx < GET_MAX_NUM_CTX(); idx++) {
        ...
        if ((LPN_CTX_PTR(idx)->inUse) && (LPN_CTX_PTR(idx)->friendSmState == FRIEND_ST_ESTAB) &&
            ...
            if (MESH_IS_ADDR_UNICAST(dst)) {
                if ((dst >= pCtx->lpnAddr) &&
                    (dst < (pCtx->lpnAddr + pCtx->estabInfo.numElements))) {
...
static bool_t meshFriendAddNwkPdu(const meshNwkPduRxInfo_t *pNwkPduRxInfo)
{
    ...
    /* Validate length. */
    if ((pNwkPduRxInfo->pduLen <= hdrLen) ||
        (pNwkPduRxInfo->pduLen > MESH_FRIEND_QUEUE_MAX_LTR_PDU)) {
        return FALSE;
    }

    /* Find LPN */
    while ((pCtx = meshFriendNextLpnDstForPdu(pNwkPduRxInfo->dst, pNwkPduRxInfo->netKeyIndex,
                                              &indexer)) != NULL) {
        pduAccept = TRUE;

        /* Add to Queue. */
        meshFriendQueueAddPdu(pCtx, pNwkPduRxInfo->ctl, pNwkPduRxInfo->ttl - 1,
                              pNwkPduRxInfo->seqNo, pNwkPduRxInfo->src, pNwkPduRxInfo->dst,
...
static bool_t meshFriendAddLtrAccPdu(const meshLtrAccPduInfo_t *pLtrAccPduInfo)
{
    ...
    /* Find LPN. */
    while ((pCtx = meshFriendNextLpnDstForPdu(pLtrAccPduInfo->dst, pLtrAccPduInfo->netKeyIndex,
                                              &indexer)) != NULL) {
        pduAccept = TRUE;
...
static bool_t meshFriendAddLtrCtlPdu(const meshLtrCtlPduInfo_t *pLtrCtlPduInfo)
{
    ...
    /* Find LPN */
    while ((pCtx = meshFriendNextLpnDstForPdu(pLtrCtlPduInfo->dst, pLtrCtlPduInfo->netKeyIndex,
                                              &indexer)) != NULL) {
        pduAccept = TRUE;
