...
static const uint8_t saltVtad[MESH_KEY_SIZE_128] = {
    0xCE, 0xF7, 0xFA, 0x9D, 0xC4, 0x7B, 0xAF, 0x5D, 0xAA, 0xEE, 0xD1, 0x94, 0x06, 0x09, 0x4F, 0x37
};
                                   ...
                                   uint8_t ttl, uint16_t netKeyIndex)
{
    ...
    /* Check if model exists. */
        ...
        if (!MESH_IS_ADDR_UNASSIGNED(pubAddr)) {
            ...
            retVal = MeshLocalCfgGetPublishPeriod(elemId, (const meshModelId_t *)&mdlId,
                                                  &pubParams.publishPeriodNumSteps,
                                                  &pubParams.publishPeriodStepRes);
            WSF_ASSERT(retVal == MESH_SUCCESS);
    ...
    meshCfgMdlSrSendRsp(MESH_CFG_MDL_SR_MODEL_PUB_STATUS, rspMsgParam,
                        CFG_MDL_MSG_MODEL_PUB_STATUS_NUM_BYTES(mdlId.isSigModel), src, ttl,
                        netKeyIndex);
                                   ...
                                   uint8_t ttl, uint16_t netKeyIndex)
{
    ...
    } else if ((evt.pubParams.publishFriendshipCred) &&
               (MeshLocalCfgGetLowPowerState() >= MESH_LOW_POWER_FEATURE_PROHIBITED_START)) {
                                            ...
                                            meshAddress_t virtAddr)
{
    ...
    /* Set publish address. */
    retVal = MeshLocalCfgSetPublishVirtualAddr(pVSetParams->elemId, (const meshModelId_t *)&mdlId,
                                               pVSetParams->labelUuid, virtAddr);

    if (retVal == MESH_SUCCESS) {
        retVal = MeshLocalCfgSetPublishAppKeyIndex(
            pVSetParams->elemId, (const meshModelId_t *)&mdlId, pEvt->pubParams.publishAppKeyIndex);
        WSF_ASSERT(retVal == MESH_SUCCESS);
        ...
        retVal = MeshLocalCfgSetPublishRetransCount(pVSetParams->elemId,
                                                    (const meshModelId_t *)&mdlId,
...
static void cfgMdlSrModelPubLabelUuidCmacCback(const uint8_t *pCmacResult, void *pParam)
{
    ...
    meshCfgMdlSrSendRsp(MESH_CFG_MDL_SR_MODEL_PUB_STATUS, rspMsgParam,
                        ...
                        pEvt->cfgMdlHdr.peerAddress, pVSetParams->recvTtl,
                        pVSetParams->recvNetKeyIndex);
                                       ...
                                       uint8_t ttl, uint16_t netKeyIndex)
{
    ...
    meshCfgMsgUnpackModelPubParam(ptr, &(pEvt->pubParams), &(pEvt->modelId.sigModelId),
                                  &(pEvt->modelId.vendorModelId), pEvt->isSig);
    ...
    if (MeshLocalCfgGetVirtualAddrFromLabelUuid((const uint8_t *)ptr, &(pEvt->pubAddr)) ==
        MESH_SUCCESS) {
        addrExists = TRUE;
    }
    ...
    /* Check that AppKey exists on device by reading the bound NetKey. */
    else if (MeshLocalCfgGetBoundNetKeyIndex(pEvt->pubParams.publishAppKeyIndex,
                                             &dummyNetKeyIndex) != MESH_SUCCESS) {
        /* AppKeyIndex is invalid. */
        rspMsgParam[0] = MESH_CFG_MDL_ERR_INVALID_APPKEY_INDEX;
    } else if ((pEvt->pubParams.publishFriendshipCred) &&
               (MeshLocalCfgGetLowPowerState() >= MESH_LOW_POWER_FEATURE_PROHIBITED_START)) {
        rspMsgParam[0] = MESH_CFG_MDL_ERR_FEATURE_NOT_SUPPORTED;
    } else {
        ...
        if (addrExists) {
            ...
            if (MeshSecToolCmacCalculate((uint8_t *)saltVtad, pVSetParams->labelUuid,
                                         MESH_LABEL_UUID_SIZE, cfgMdlSrModelPubLabelUuidCmacCback,
                                         pVSetParams) == MESH_SUCCESS) {
...
static void cfgMdlSrSubscrCmacCback(const uint8_t *pCmacResult, void *pParam)
{
    ...
    cfgMdlSrModelSubscrVirtAddOvrParams_t *pVAddOvr =
        (cfgMdlSrModelSubscrVirtAddOvrParams_t *)pParam;
    ...
    if (pCmacResult != NULL) {
        ...
        if (pVAddOvr->overwrite) {
            ...
            (void)MeshLocalCfgRemoveAllFromSubscrList(pVAddOvr->elemId,
                                                      (const meshModelId_t *)&mdlId);
        }
        ...
        switch (MeshLocalCfgAddVirtualAddrToSubscrList(pVAddOvr->elemId,
                                                       (const meshModelId_t *)&mdlId,
                                                       pVAddOvr->labelUuid, pEvt->subscrAddr)) {
    ...
    meshCfgMdlSrSendRsp(MESH_CFG_MDL_SR_MODEL_SUBSCR_STATUS, rspMsgParam,
                        CFG_MDL_MSG_MODEL_SUBSCR_STATUS_NUM_BYTES(pEvt->isSig),
                        pEvt->cfgMdlHdr.peerAddress, pVAddOvr->recvTtl, pVAddOvr->recvNetKeyIndex);
 ...
 */
/*************************************************************************************************/
static void meshCfgMdlSrHandleModelSubscrAddOvr(uint8_t *pMsgParam, uint16_t msgParamLen,
                                                ...
                                                uint16_t netKeyIndex, bool_t overwrite)
{
    ...
    if (MESH_IS_ADDR_UNASSIGNED(evt.subscrAddr) || MESH_IS_ADDR_VIRTUAL(evt.subscrAddr) ||
    ...
    /* Send response. */
    meshCfgMdlSrSendRsp(MESH_CFG_MDL_SR_MODEL_SUBSCR_STATUS, rspMsgParam,
                        CFG_MDL_MSG_MODEL_SUBSCR_STATUS_NUM_BYTES(evt.isSig), src, ttl,
                        netKeyIndex);

    if (rspMsgParam[0] == MESH_CFG_MDL_SR_SUCCESS) {
        /* Set event parameters. */
        evt.cfgMdlHdr.hdr.event = MESH_CFG_MDL_SR_EVENT;
        evt.cfgMdlHdr.hdr.param = overwrite ? MESH_CFG_MDL_SUBSCR_OVR_EVENT :
                                              MESH_CFG_MDL_SUBSCR_ADD_EVENT;
        evt.cfgMdlHdr.hdr.status = MESH_CFG_MDL_SR_SUCCESS;
        evt.cfgMdlHdr.peerAddress = src;
    ...
    /* Check if model allows subscription. */
        ...
        if (MeshLocalCfgGetVirtualAddrFromLabelUuid((const uint8_t *)pTemp, &evt.subscrAddr) ==
            ...
            if ((pVAddOvr = WsfBufAlloc(sizeof(cfgMdlSrModelSubscrVirtAddOvrParams_t))) == NULL) {
                ...
                if (MeshSecToolCmacCalculate((uint8_t *)saltVtad, pVAddOvr->labelUuid,
                                             MESH_LABEL_UUID_SIZE, cfgMdlSrSubscrCmacCback,
                                             (void *)pVAddOvr) != MESH_SUCCESS) {
                    rspMsgParam[0] = MESH_CFG_MDL_ERR_UNSPECIFIED;
                    /* Free memory. */
                    WsfBufFree(pVAddOvr);
                } else {
                                          ...
                                          meshAddress_t src, uint8_t ttl, uint16_t netKeyIndex)
{
    ...
    else if (MeshLocalCfgGetVirtualAddrFromLabelUuid((const uint8_t *)pLabelUuid,
                                                     &evt.subscrAddr) != MESH_SUCCESS) {
        rspMsgParam[0] = MESH_CFG_MDL_ERR_INVALID_ADDR;
        evt.subscrAddr = MESH_ADDR_TYPE_UNASSIGNED;
    } else {
        /* Try to remove from subscription list. */
 ...
 */
/*************************************************************************************************/
static void meshCfgMdlSrHandleModelSubscrGet(uint8_t *pMsgParam, uint16_t msgParamLen,
                                             meshAddress_t src, uint8_t ttl, uint16_t netKeyIndex,
                                             bool_t isSig)
{
    ...
    /* Check if model exists. */
        ...
        if (totalNumAddr == 0) {
            ...
            if ((pRspMsgParam = WsfBufAlloc(CFG_MDL_MSG_MODEL_SUBSCR_LIST_EMPTY_NUM_BYTES(isSig) +
                ...
                while (
                    MeshLocalCfgGetNextAddressFromSubscrList(elemId, (const meshModelId_t *)&mdlId,
                                                             &subscrAddr, &idx) == MESH_SUCCESS &&
                    numAddr != 0) {
    ...
    /* Send response. */
    meshCfgMdlSrSendRsp(isSig ? MESH_CFG_MDL_SR_MODEL_SUBSCR_SIG_LIST :
                                         ...
                                         meshAddress_t src, uint8_t ttl, uint16_t netKeyIndex)
{
    ...
    if ((pMsgParam == NULL) || (msgParamLen != CFG_MDL_MSG_MODEL_SUBSCR_SIG_GET_NUM_BYTES)) {
        return;
    }
...
{
    /* Validate message parameters. */
    if ((pMsgParam == NULL) || (msgParamLen != CFG_MDL_MSG_MODEL_SUBSCR_VENDOR_GET_NUM_BYTES)) {
        return;
    }
