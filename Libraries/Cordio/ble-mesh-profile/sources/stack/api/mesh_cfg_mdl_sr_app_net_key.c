        ...
        while (totalIdx < (pMeshConfig->pElementArray[elemId].numSigModels +
            ...
            if (totalIdx < pMeshConfig->pElementArray[elemId].numSigModels) {
                ...
                mdlId.modelId.sigModelId =
                    pMeshConfig->pElementArray[elemId].pSigModelArray[sigIdx].modelId;
            } else {
            ...
            if (MeshLocalCfgValidateModelToAppKeyBind(elemId, &mdlId, appKeyIndex)) {
                /* Unbind. */
                MeshLocalCfgUnbindAppKeyFromModel(elemId, (const meshModelId_t *)&mdlId,
                                                  appKeyIndex);
            }
            ...
            (void)MeshLocalCfgSetPublishFriendshipCredFlag(elemId, (const meshModelId_t *)&mdlId,
                                                           FALSE);
...
static void netKeyDelCleanup(uint16_t netKeyIndex)
{
    ...
    while (MeshLocalCfgGetNextBoundAppKey(netKeyIndex, &appKeyIndex, &indexer) == MESH_SUCCESS) {
 ...
 */
/*************************************************************************************************/
static void netKeyMatCback(meshSecKeyType_t keyType, uint16_t keyIndex, bool_t isSuccess,
                           bool_t keyUpdated, void *pParam)
{
    keyAddUpdtParams_t *pRspParams = (keyAddUpdtParams_t *)pParam;
    meshCfgMdlNetKeyChgEvt_t evt = { .cfgMdlHdr.hdr.status = MESH_CFG_MDL_SR_SUCCESS };
                                 ...
                                 uint8_t ttl, uint16_t netKeyIndex)
{
    ...
    if (MeshLocalCfgGetNetKey(msgNetKeyIndex, tempKey) == MESH_SUCCESS) {
        ...
        if (MeshLocalCfgSetNetKey(msgNetKeyIndex, (const uint8_t *)pKey) != MESH_SUCCESS) {
            ...
            if ((pRspParams = (keyAddUpdtParams_t *)WsfBufAlloc(sizeof(keyAddUpdtParams_t))) ==
                NULL) {
                                  ...
                                  uint8_t ttl, uint16_t netKeyIndex)
{
    ...
    /* Check if phase allows this message. */
        ...
        if (MeshLocalCfgGetUpdatedNetKey(msgNetKeyIndex, tempKey) == MESH_SUCCESS) {
            ...
            retVal =
                (meshSecRetVal_t)MeshLocalCfgUpdateNetKey(msgNetKeyIndex, (const uint8_t *)pKey);
            /* Try to store it in local config. */
            if (retVal != MESH_SUCCESS) {
                ...
                if ((pRspParams = (keyAddUpdtParams_t *)WsfBufAlloc(sizeof(keyAddUpdtParams_t))) ==
                    ...
                    retVal = MeshSecAddKeyMaterial(MESH_SEC_KEY_TYPE_NWK, msgNetKeyIndex, TRUE,
                                                   netKeyMatCback, (void *)pRspParams);
                    if (retVal != MESH_SUCCESS) {
                        MESH_TRACE_WARN1("CFG SR: NetKey update derivation failed with code %x",
                                         retVal);
...
void meshCfgMdlSrHandleNetKeyDel(uint8_t *pMsgParam, uint16_t msgParamLen, meshAddress_t src,
                                 uint8_t ttl, uint16_t netKeyIndex)
{
    meshCfgMdlNetKeyChgEvt_t evt = { .cfgMdlHdr.hdr.event = MESH_CFG_MDL_SR_EVENT,
                                 ...
                                 uint8_t ttl, uint16_t netKeyIndex)
{
    ...
    if ((pRspMsgParam = (uint8_t *)WsfBufAlloc(rspLen)) != NULL) {
        ...
        /* Send response. */
        meshCfgMdlSrSendRsp(MESH_CFG_MDL_SR_NETKEY_LIST, pRspMsgParam, rspLen, src, ttl,
                            netKeyIndex);
    }
}
 ...
 */
/*************************************************************************************************/
static void appKeyMatCback(meshSecKeyType_t keyType, uint16_t keyIndex, bool_t isSuccess,
                           bool_t keyUpdated, void *pParam)
{
    keyAddUpdtParams_t *pRspParams = (keyAddUpdtParams_t *)pParam;
    meshCfgMdlAppKeyChgEvt_t evt = { .cfgMdlHdr.hdr.status = MESH_CFG_MDL_SR_SUCCESS };
                                 ...
                                 uint8_t ttl, uint16_t netKeyIndex)
{
    ...
    /* Verify NetKeyIndex by reading key refresh state. */
    if (MeshLocalCfgGetKeyRefreshPhaseState(bind.netKeyIndex) >=
        ...
        if (MeshLocalCfgGetAppKey(bind.appKeyIndex, tempKey) == MESH_SUCCESS) {
            ...
            if (MeshLocalCfgSetAppKey(bind.appKeyIndex, (const uint8_t *)pKey) != MESH_SUCCESS) {
                ...
                if ((pRspParams = (keyAddUpdtParams_t *)WsfBufAlloc(sizeof(keyAddUpdtParams_t))) ==
                    ...
                    if (retVal != MESH_SUCCESS) {
                        MESH_TRACE_WARN1("CFG SR: AppKey add derivation failed with code %x",
                                         retVal);
                                  ...
                                  uint8_t ttl, uint16_t netKeyIndex)
{
    ...
    /* Check if Key Refresh State allows Update. */
        ...
        if (MeshLocalCfgGetUpdatedAppKey(bind.appKeyIndex, tempKey) == MESH_SUCCESS) {
            ...
            retVal =
                (meshSecRetVal_t)MeshLocalCfgUpdateAppKey(bind.appKeyIndex, (const uint8_t *)pKey);
            /* Try to store it in local config. */
            if (retVal != MESH_SUCCESS) {
                ...
                if ((pRspParams = (keyAddUpdtParams_t *)WsfBufAlloc(sizeof(keyAddUpdtParams_t))) ==
                    ...
                    retVal = MeshSecAddKeyMaterial(MESH_SEC_KEY_TYPE_APP, bind.appKeyIndex, TRUE,
                                                   appKeyMatCback, (void *)pRspParams);
                    if (retVal != MESH_SUCCESS) {
                        MESH_TRACE_WARN1("CFG SR: AppKey update derivation failed with code %x",
                                         retVal);
...
{
    uint16_t boundNetKeyIndex;
    meshCfgMdlAppKeyChgEvt_t evt = { .cfgMdlHdr.hdr.event = MESH_CFG_MDL_SR_EVENT,
    ...
    /* Verify NetKeyIndex by reading key refresh state. */
    if (MeshLocalCfgGetKeyRefreshPhaseState(evt.bind.netKeyIndex) >=
        MESH_KEY_REFRESH_PROHIBITED_START) {
        rspMsgParam[0] = MESH_CFG_MDL_ERR_INVALID_NETKEY_INDEX;
    }
    /* Verify AppKeyIndex by reading the bound NetKeyIndex. */
    else if (MeshLocalCfgGetBoundNetKeyIndex(evt.bind.appKeyIndex, &boundNetKeyIndex) !=
             MESH_SUCCESS) {
                                      ...
                                      uint8_t ttl, uint16_t netKeyIndex)
{
    ...
    rspMsgParam[0] = phaseState < MESH_KEY_REFRESH_PROHIBITED_START ?
                         MESH_CFG_MDL_SR_SUCCESS :
                         MESH_CFG_MDL_ERR_INVALID_NETKEY_INDEX;
                                      ...
                                      uint8_t ttl, uint16_t netKeyIndex)
{
    ...
    meshCfgMdlKeyRefPhaseEvt_t evt = { .cfgMdlHdr.hdr.event = MESH_CFG_MDL_SR_EVENT,
