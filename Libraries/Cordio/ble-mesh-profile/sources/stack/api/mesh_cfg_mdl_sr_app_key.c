                                    ...
                                    uint8_t ttl, uint16_t netKeyIndex)
{
    ...
    meshCfgMdlModelAppBindEvt_t evt = { .cfgMdlHdr.hdr.event = MESH_CFG_MDL_SR_EVENT,
    ...
    /* Validate AppKey Index by reading the bound NetKey Index. */
        ...
        if (!MeshLocalCfgModelExists(elemId, (const meshModelId_t *)&mdlId)) {
            ...
            switch (MeshLocalCfgBindAppKeyToModel(elemId, (const meshModelId_t *)&mdlId,
                                                  evt.appKeyIndex)) {
            case MESH_SUCCESS:
            /* Fall-through. */
    ...
    /* Pack binding. */
    meshCfgMsgPackModelAppBind(&rspMsgParam[1], evt.elemAddr, evt.appKeyIndex,
                               evt.modelId.sigModelId, evt.modelId.vendorModelId, evt.isSig);
       ...
       */
            ...
            if (!MESH_IS_ADDR_UNASSIGNED(pubAddr)) {
                ...
                if (dummyKeyIndex == evt.appKeyIndex) {
                    ...
                    retVal = MeshLocalCfgSetPublishFriendshipCredFlag(
                        elemId, (const meshModelId_t *)&mdlId, FALSE);
                    WSF_ASSERT(retVal == MESH_SUCCESS);
                    retVal =
                        MeshLocalCfgSetPublishPeriod(elemId, (const meshModelId_t *)&mdlId, 0, 0);
                    WSF_ASSERT(retVal == MESH_SUCCESS);
                    retVal = MeshLocalCfgSetPublishRetransCount(elemId,
                                                                (const meshModelId_t *)&mdlId, 0);
                    WSF_ASSERT(retVal == MESH_SUCCESS);
                    retVal = MeshLocalCfgSetPublishRetransIntvlSteps(
                        elemId, (const meshModelId_t *)&mdlId, 0);
                    WSF_ASSERT(retVal == MESH_SUCCESS);
 ...
 */
/*************************************************************************************************/
static void meshCfgMdlSrHandleModelAppGet(uint8_t *pMsgParam, uint16_t msgParamLen,
                                          ...
                                          bool_t isSig)
{
    ...
    /* Check if model exists. */
        ...
        if (count != 0) {
            ...
            if ((pRspMsgParam = WsfBufAlloc(CFG_MDL_MSG_MODEL_APP_LIST_NUM_BYTES(isSig, count))) ==
                ...
                while (count > 1) {
                    retVal = MeshLocalCfgGetNextModelBoundAppKey(
                        elemId, (const meshModelId_t *)&mdlId, &keyIndex1, &indexer);
                    WSF_ASSERT(retVal == MESH_SUCCESS);
                    retVal = MeshLocalCfgGetNextModelBoundAppKey(
                        elemId, (const meshModelId_t *)&mdlId, &keyIndex2, &indexer);
                    WSF_ASSERT(retVal == MESH_SUCCESS);
    ...
    /* Send response. */
    meshCfgMdlSrSendRsp(isSig ? MESH_CFG_MDL_SR_MODEL_APP_SIG_LIST :
