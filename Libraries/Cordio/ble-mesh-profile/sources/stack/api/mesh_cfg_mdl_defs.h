...
#define CFG_MDL_MSG_NETKEY_ADD_NUM_BYTES (CFG_MDL_MSG_1KEY_PACKED_NUM_BYTES + MESH_KEY_SIZE_128)
...
#define CFG_MDL_MSG_NETKEY_LIST_NUM_BYTES(numNetKey) \
    ((((numNetKey) >> 1) * 3) + (((numNetKey)&0x01) << 1))
...
#define CFG_MDL_MSG_APPKEY_ADD_NUM_BYTES (CFG_MDL_MSG_APPKEY_BIND_NUM_BYTES + MESH_KEY_SIZE_128)
...
#define CFG_MDL_MSG_APPKEY_LIST_TO_NUM_APPKEY(size)                  \
    (((((size)-CFG_MDL_MSG_APPKEY_LIST_EMPTY_NUM_BYTES) / 3) << 1) + \
     ((((size)-CFG_MDL_MSG_APPKEY_LIST_EMPTY_NUM_BYTES) % 3) == 2))
/*! Calculates AppKey List number of bytes based on number of AppKeys. */
#define CFG_MDL_MSG_APPKEY_LIST_NUM_BYTES(numAppKey) \
    ((((numAppKey) >> 1) * 3) + (((numAppKey)&0x01) << 1) + CFG_MDL_MSG_APPKEY_LIST_EMPTY_NUM_BYTES)
/*! Validates AppKey List size constraints. */
#define CFG_MDL_MSG_APPKEY_LIST_SIZE_VALID(size)            \
    (((size) >= CFG_MDL_MSG_APPKEY_LIST_EMPTY_NUM_BYTES) && \
     ((((size)-CFG_MDL_MSG_APPKEY_LIST_EMPTY_NUM_BYTES) % 3) != 1))
...
#define CFG_MDL_MSG_MODEL_APP_STATUS_NUM_BYTES(isSig) \
    (1 + CFG_MDL_MSG_MODEL_APP_STATE_NUM_BYTES(isSig))
...
#define CFG_MDL_MSG_MODEL_APP_LIST_SIZE_VALID(isSig, size)            \
    (((size) >= CFG_MDL_MSG_MODEL_APP_LIST_EMPTY_NUM_BYTES(isSig)) && \
     ((((size)-CFG_MDL_MSG_MODEL_APP_LIST_EMPTY_NUM_BYTES(isSig)) % 3) != 1))
/*! Extract number of AppKey Indexes from a Config Model App List message based on model type and
 *  size.
 */
#define CFG_MDL_MSG_MODEL_APP_LIST_TO_NUM_APPKEY(isSig, size)                  \
    (((((size)-CFG_MDL_MSG_MODEL_APP_LIST_EMPTY_NUM_BYTES(isSig)) / 3) << 1) + \
...
#define CFG_MDL_MSG_MODEL_APP_LIST_NUM_BYTES(isSig, numAppKey) \
    ((((numAppKey) >> 1) * 3) + (((numAppKey)&0x01) << 1) +    \
...
#define CFG_MDL_MSG_LPN_POLLTIMEOUT_VALID(t) (!(((t) >= 1) && (((t) <= 9))) || ((t) >= 0x34BC00))
