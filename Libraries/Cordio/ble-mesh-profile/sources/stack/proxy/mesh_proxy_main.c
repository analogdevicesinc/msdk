                                       ...
                                       uint8_t *pNwkPduNetMic, uint8_t netMicSize, void *pParam)
{
    ...
    if (isSuccess) {
        ...
        if (!MeshBrSendCfgPdu(pProxyPduMeta->rcvdBrIfId, pProxyPduMeta->pdu,
                              pProxyPduMeta->pduLen)) {
 ...
 */
/*************************************************************************************************/
static bool_t meshNwkDecryptRequest(meshProxyPduMeta_t *pRecvPduMeta,
                                    meshSecNwkDeobfDecCback_t secCback)
{
   ...
   */
    return (MESH_SUCCESS ==
            MeshSecNwkDeobfDec(TRUE, &nwkDecParams, secCback, (void *)pRecvPduMeta));
}
...
static void meshProxyCheckAndProcessPdu(meshProxyPduMeta_t *pProxyPduMeta)
{
    ...
    /* Extract CTL. */
    ctl =
        MESH_UTILS_BF_GET(*(pProxyPduMeta->pdu + MESH_CTL_TTL_POS), MESH_CTL_SHIFT, MESH_CTL_SIZE);
    ...
    /* Extract TTL. */
    ttl =
        MESH_UTILS_BF_GET(*(pProxyPduMeta->pdu + MESH_CTL_TTL_POS), MESH_TTL_SHIFT, MESH_TTL_SIZE);
    ...
    /* Process the PDU. */
    proxyCb.decryptedProxyPduCback(
        pProxyPduMeta->rcvdBrIfId, pProxyPduMeta->pdu + MESH_DST_ADDR_POS + sizeof(meshAddress_t),
        pProxyPduMeta->pduLen - MESH_NETMIC_SIZE_PROXY_PDU - MESH_NWK_HEADER_LEN);
}
 ...
 */
/*************************************************************************************************/
static void meshBrToProxyPduRecvCback(meshBrInterfaceId_t brIfId, const uint8_t *pPdu,
                                      uint8_t pduLen)
{
