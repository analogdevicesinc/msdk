...
static void meshProxySrPduRecvCback(meshBrInterfaceId_t brIfId, const uint8_t *pPdu, uint8_t pduLen)
{
    ...
    if (sendStatus == TRUE) {
        if ((pNwkIf = MeshNwkIfGet(brIfId)) != NULL) {
            ...
            /* Send Filter Status message. */
            meshProxySendConfigMessage(brIfId, MESH_PROXY_OPCODE_FILTER_STATUS, pdu,
                                       MESH_PROXY_CFG_FILTER_STATUS_MSG_SIZE);
        }
    }
}
...
static void meshBeaconGenCompleteCback(bool_t isSuccess, uint16_t netKeyIndex, uint8_t *pBeacon)
{
    ...
    if (MeshLocalCfgGetNextNetKeyIndex(&netKeyIndex, &meshProxySrCb.bcnNetKeyIndexer) ==
        MESH_SUCCESS) {
...
static void meshProxyComputeNodeIdentity(uint16_t netKeyIndex)
{
    ...
    /* Set Random */
    SecRand(&meshProxySrCb.svcData[MESH_PROXY_NODE_ID_PT_PADDING_SIZE],
            MESH_PROXY_NODE_ID_PT_RANDOM_SIZE);
    ...
    if ((pIdentityKey != NULL) &&
        (MeshSecToolAesEncrypt(pIdentityKey, meshProxySrCb.svcData,
                               meshProxyNodeIdentityCompleteCback, NULL) == MESH_SUCCESS)) {
...
static void meshProxySrCreateServiceData(uint16_t netKeyIndex, meshProxyIdType_t idType)
{
    ...
    else if ((idType == MESH_PROXY_NODE_IDENTITY_TYPE) &&
             (nodeIdentityState != MESH_NODE_IDENTITY_RUNNING)) {
...
void MeshProxySrGetNextServiceData(meshProxyIdType_t idType)
{
    ...
    /* Get subnet key index */
    if (MESH_SUCCESS !=
        MeshLocalCfgGetNextNetKeyIndex(&netKeyIndex, &meshProxySrCb.svcDataNetKeyIndexer)) {
