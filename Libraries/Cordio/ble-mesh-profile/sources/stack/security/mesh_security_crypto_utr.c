...
static void meshSecUtrAppDecCcmCback(const meshSecToolCcmResult_t *pCcmResult, void *pParam)
{
    ...
    if (resumeDec) {
        ...
        if (MeshSecToolCcmEncryptDecrypt(MESH_SEC_TOOL_CCM_DECRYPT, &(pReq->ccmParams),
                                         meshSecUtrAppDecCcmCback, (void *)pReq) != MESH_SUCCESS) {
    ...
    if (pCcmResult != NULL) {
        ...
        if (pResult->isAuthSuccess) {
            ...
            cback(TRUE, pResult->pPlainText, NULL, pResult->plainTextSize,
                  (pReq->keySearchIdx == 0 ? MESH_APPKEY_INDEX_LOCAL_DEV_KEY :
                                             MESH_APPKEY_INDEX_REMOTE_DEV_KEY),
        ...
        if (pReq->keySearchIdx == 0) {
            ...
            if ((meshSecCb.secRemoteDevKeyReader != NULL) &&
                (meshSecCb.secRemoteDevKeyReader(((pReq->nonce[MESH_SEC_NONCE_SRC_POS] << 8) |
                                                  (pReq->nonce[MESH_SEC_NONCE_SRC_POS + 1])),
                                                 pReq->key))) {
...
static meshSecRetVal_t meshSecUtrDecSetNextAppKey(meshSecUtrDecReq_t *pReq)
{
    ...
    for (; pReq->keySearchIdx < MESH_SEC_KEY_MAT_PER_INDEX * secMatLocals.appKeyInfoListSize;
        ...
        if (pAppKeyInfo->hdr.crtKeyId != entryId) {
            if (MeshLocalCfgGetUpdatedAppKey(pAppKeyInfo->hdr.keyIndex, pReq->key) !=
                MESH_SUCCESS) {
            ...
            if (MeshLocalCfgGetAppKey(pAppKeyInfo->hdr.keyIndex, pReq->key) != MESH_SUCCESS) {
                WSF_ASSERT(MeshLocalCfgGetAppKey(pAppKeyInfo->hdr.keyIndex, pReq->key) ==
                           MESH_SUCCESS);
                continue;
            }
        }
                                                 ...
                                                 meshSecToolCcmCback_t ccmCback)
{
    ...
    if (pReq->aid == MESH_SEC_DEVICE_KEY_AID) {
        ...
        if (retVal == MESH_SUCCESS) {
            ...
            if (MESH_IS_ADDR_VIRTUAL(pReq->vtad)) {
                ...
                retVal = meshSecUtrDecSetNextLabelUuid(pReq) ? MESH_SUCCESS :
                                                               MESH_SEC_INVALID_PARAMS;
            } else {
    ...
    retVal = (meshSecRetVal_t)MeshSecToolCcmEncryptDecrypt(
        MESH_SEC_TOOL_CCM_DECRYPT, &(pReq->ccmParams), ccmCback, (void *)pReq);
                                  ...
                                  meshSecUtrEncryptCback_t utrEncryptCompleteCback, void *pParam)
{
   ...
   */
    if ((pReqParams == NULL) || (utrEncryptCompleteCback == NULL) ||
        (pReqParams->netKeyIndex > MESH_SEC_MAX_KEY_INDEX) ||
        (pReqParams->seqNo > MESH_SEQ_MAX_VAL) || (!MESH_IS_ADDR_UNICAST(pReqParams->srcAddr)) ||
        (pReqParams->pAppPayload == NULL) || (pReqParams->appPayloadSize == 0) ||
        (pReqParams->pEncAppPayload == NULL) || (pReqParams->pTransMic == NULL) ||
    ...
    if ((pReqParams->appKeyIndex == MESH_APPKEY_INDEX_LOCAL_DEV_KEY) ||
        ...
        if (pReqParams->appKeyIndex == MESH_APPKEY_INDEX_LOCAL_DEV_KEY) {
            ...
            if ((meshSecCb.secRemoteDevKeyReader == NULL) ||
                (!meshSecCb.secRemoteDevKeyReader(pReqParams->dstAddr,
                                                  secCryptoReq.utrEncReq.key))) {
                return MESH_SEC_INVALID_PARAMS;
            }
        }
    } else {
        ...
        if (pKeyInfo->hdr.crtKeyId != matEntryIdx) {
            retVal =
                MeshLocalCfgGetUpdatedAppKey(pReqParams->appKeyIndex, secCryptoReq.utrEncReq.key);
        } else {
    ...
    /* Build nonce. */
    meshSecBuildNonce((pReqParams->appKeyIndex > MESH_SEC_MAX_KEY_INDEX ? MESH_SEC_NONCE_DEV :
                                                                          MESH_SEC_NONCE_APP),
                      ((pReqParams->transMicSize == MESH_SEC_MIC_SIZE_64 ? 1 : 0)
                       << MESH_SEC_ASZMIC_SHIFT),
                      (pReqParams->srcAddr), (pReqParams->dstAddr), (pReqParams->seqNo), (ivIndex),
    ...
    ccmParams.pAuthData =
        (MESH_IS_ADDR_VIRTUAL(pReqParams->dstAddr) ? pReqParams->pLabelUuid : NULL);
    ccmParams.authDataLen = (MESH_IS_ADDR_VIRTUAL(pReqParams->dstAddr) ? MESH_LABEL_UUID_SIZE : 0);
                                  ...
                                  meshSecUtrDecryptCback_t utrDecryptCompleteCback, void *pParam)
{
    ...
    /* Validate parameters. */
    if ((pReqParams == NULL) || (utrDecryptCompleteCback == NULL) ||
        (pReqParams->netKeyIndex > MESH_SEC_MAX_KEY_INDEX) ||
        (!MESH_IS_ADDR_UNICAST(pReqParams->srcAddr)) || (pReqParams->seqNo > MESH_SEQ_MAX_VAL) ||
        (pReqParams->pAppPayload == NULL) || (pReqParams->pEncAppPayload == NULL) ||
        (pReqParams->pTransMic == NULL) || (pReqParams->appPayloadSize == 0) ||
   ...
   */
    ...
    /* Build nonce. */
    meshSecBuildNonce(
        (pReqParams->aid == MESH_SEC_DEVICE_KEY_AID ? MESH_SEC_NONCE_DEV : MESH_SEC_NONCE_APP),
        ((pReqParams->transMicSize == MESH_SEC_MIC_SIZE_64 ? 1 : 0) << MESH_SEC_ASZMIC_SHIFT),
        (pReqParams->srcAddr), (pReqParams->dstAddr), (pReqParams->seqNo),
        (pReqParams->recvIvIndex), (secCryptoReq.utrDecReq.nonce));
