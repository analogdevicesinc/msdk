...
#define MESH_SEC_RESET_IN_PROGRESS(flags) \
    ((flags) &= ~(MESH_SEC_KEY_CRT_IN_PROGESS | MESH_SEC_KEY_UPDT_IN_PROGRESS))
...
/*! S1("nkbk") */
static const uint8_t secS1NkbkStr[] = { 0x2c, 0x24, 0x61, 0x9a, 0xb7, 0x93, 0xc1, 0x23,
                                        0x3f, 0x6e, 0x22, 0x67, 0x38, 0x39, 0x3d, 0xec };

/*!< S1("nkik") */
static const uint8_t secS1NkikStr[] = { 0xf8, 0x79, 0x5a, 0x1a, 0xab, 0xf1, 0x82, 0xe4,
                                        0xf1, 0x63, 0xd8, 0x6e, 0x24, 0x5e, 0x19, 0xf4 };

/*! String "id128" concatenated with hex 0x01 */
static const uint8_t secId128Str[] = { 0x69, 0x64, 0x31, 0x32, 0x38, 0x01 };
                                   ...
                                   meshSecToolKeyDerivationCback_t cback)
{
    ...
    if (pReq->friendUpdtIdx == secMatLocals.friendMatListSize) {
        ...
        /* Send next request. */
        if (MeshSecToolK2Derive(k2PBuff, sizeof(k2PBuff), secTempData.nwkKey, cback,
                                (void *)pReq) != MESH_SUCCESS) {
            isSuccess = FALSE;
            procComplete = TRUE;
        }
    }
    ...
    if (procComplete) {
       ...
       */
            if (secMatLocals.pNetKeyInfoArray[pReq->netKeyListIdx].hdr.flags &
                MESH_SEC_KEY_CRT_MAT_DELETE) {
                ...
                /* Clear flag. */
                secMatLocals.pNetKeyInfoArray[pReq->netKeyListIdx].hdr.flags &=
                    ~MESH_SEC_KEY_CRT_MAT_DELETE;
                ...
                secMatLocals.pNetKeyInfoArray[pReq->netKeyListIdx].hdr.flags |=
                    MESH_SEC_KEY_UPDT_MAT_AVAILABLE;
            }
        } else {
       ...
       */
        ...
        pReq->cback(MESH_SEC_KEY_TYPE_NWK,
                    secMatLocals.pNetKeyInfoArray[netKeyEntryIdx].hdr.keyIndex, isSuccess,
 ...
 */
/*************************************************************************************************/
static void meshSecFriendMatUpdateCback(const uint8_t *pResult, uint8_t resultSize, void *pParam)
{
    ...
    if (!isSuccess) {
        ...
        /* Invoke user callback. */
        pReq->cback(MESH_SEC_KEY_TYPE_NWK, netKeyIndex, isSuccess, pReq->isUpdate, pReq->pParam);
    }
    (void)resultSize;
}
...
static void meshSecPduSecDerivCback(const uint8_t *pResult, uint8_t resultSize, void *pParam)
{
    ...
    if ((pResult == NULL) || (pKeyInfo->hdr.flags & MESH_SEC_KEY_ALL_DELETE)) {
        ...
        /* Copy NID. */
        pKeyInfo->keyMaterial[entryId].masterPduSecMat.nid =
            MESH_UTILS_BF_GET(pResult[0], MESH_NID_SHIFT, MESH_NID_SIZE);
        /* Copy Ek. */
...
static void meshSecNwkIdDerivCback(const uint8_t *pResult, uint8_t resultSize, void *pParam)
{
    ...
    if ((pResult == NULL) || (pKeyInfo->hdr.flags & MESH_SEC_KEY_ALL_DELETE)) {
        ...
        if (MeshSecToolK2Derive(k2PBuff, sizeof(k2PBuff), secTempData.nwkKey,
                                meshSecPduSecDerivCback, pParam) == MESH_SUCCESS) {
            isSuccess = TRUE;
        }
    }
...
static void meshSecBkDerivCback(const uint8_t *pResult, uint8_t resultSize, void *pParam)
{
    ...
    if ((pResult == NULL) || (pKeyInfo->hdr.flags & MESH_SEC_KEY_ALL_DELETE)) {
        ...
        /* Send next request. */
        if (MeshSecToolK3Derive(secTempData.nwkKey, meshSecNwkIdDerivCback, pParam) ==
            MESH_SUCCESS) {
            isSuccess = TRUE;
        }
    }
...
static void meshSecIkDerivCback(const uint8_t *pResult, uint8_t resultSize, void *pParam)
{
    ...
    if ((pResult == NULL) || (pKeyInfo->hdr.flags & MESH_SEC_KEY_ALL_DELETE)) {
        ...
        /* Send next request. */
        if (MeshSecToolK1Derive((uint8_t *)secId128Str, sizeof(secId128Str),
                                (uint8_t *)secS1NkbkStr, secTempData.nwkKey, MESH_KEY_SIZE_128,
                                meshSecBkDerivCback, pParam) == MESH_SUCCESS) {
            isSuccess = TRUE;
        }
    }
                                                     ...
                                                     void *pParam)
{
    ...
    for (idx = 0; idx < secMatLocals.netKeyInfoListSize; idx++) {
        ...
        if ((secMatLocals.pNetKeyInfoArray[idx].hdr.keyIndex == netKeyIndex) &&
            ...
            if (isUpdate) {
                ...
                if (secMatLocals.pNetKeyInfoArray[idx].hdr.flags &
                    MESH_SEC_KEY_UPDT_MAT_AVAILABLE) {
        ...
        else if ((secMatLocals.pNetKeyInfoArray[idx].hdr.flags == MESH_SEC_KEY_UNUSED) &&
                 (!isUpdate)) {
    ...
    retVal = MeshSecToolK1Derive((uint8_t *)secId128Str, sizeof(secId128Str),
                                 (uint8_t *)secS1NkikStr, secTempData.nwkKey, MESH_KEY_SIZE_128,
                                 meshSecIkDerivCback, (void *)&(secKeyDerivReq.netKeyDerivReq));
...
static void meshSecFriendCredAddCback(const uint8_t *pResult, uint8_t resultSize, void *pParam)
{
    ...
    if ((pResult == NULL) || ((pKeyInfo->hdr.flags & MESH_SEC_KEY_CRT_MAT_AVAILABLE) == 0) ||
        ...
        if (!pReq->doUpdate) {
            ...
            if (pKeyInfo->hdr.flags &
                (MESH_SEC_KEY_UPDT_IN_PROGRESS | MESH_SEC_KEY_UPDT_MAT_AVAILABLE)) {
                /* Mark second operation as in progress. */
                pReq->doUpdate = TRUE;

                /* Read new key. This should never fail as there is another procedure using it. */
                if (MeshLocalCfgGetUpdatedNetKey(pKeyInfo->hdr.keyIndex,
                                                 secTempData.nwkKeyFriend) == MESH_SUCCESS) {
                    /* Call K2 derivation with friendship credentials. */
                    if (MeshSecToolK2Derive(pReq->pK2PBuff, MESH_SEC_K2_P_FRIEND_SIZE,
                                            secTempData.nwkKeyFriend, meshSecFriendCredAddCback,
                                            (void *)pParam) != MESH_SUCCESS) {
                        procComplete = TRUE;
                    }
                } else {
                    /* Trap if it does fail. */
                    WSF_ASSERT(MeshLocalCfgGetUpdatedNetKey(pKeyInfo->hdr.keyIndex,
...
static void meshSecAppKeyDerivCback(const uint8_t *pResult, uint8_t resultSize, void *pParam)
{
    ...
    /* Invoke user callback. */
    pReq->cback(MESH_SEC_KEY_TYPE_APP, appKeyIndex, isSuccess, pReq->isUpdate, pReq->pParam);
    (void)resultSize;
}
                                                     ...
                                                     void *pParam)
{
    ...
    for (idx = 0; idx < secMatLocals.appKeyInfoListSize; idx++) {
        ...
        if ((secMatLocals.pAppKeyInfoArray[idx].hdr.keyIndex == appKeyIndex) &&
            ...
            if (isUpdate) {
                /* Check if there is already updated key material. */
                if (secMatLocals.pAppKeyInfoArray[idx].hdr.flags &
                    MESH_SEC_KEY_UPDT_MAT_AVAILABLE) {
                    return MESH_SEC_KEY_MATERIAL_EXISTS;
                }
                /* Store entry and finish searching. */
        ...
        else if ((secMatLocals.pAppKeyInfoArray[idx].hdr.flags == MESH_SEC_KEY_UNUSED) &&
                 (!isUpdate)) {
            /* Store slot and configure key header information. */
            pAppKeyInfo = &(secMatLocals.pAppKeyInfoArray[idx]);
    ...
    retVal = MeshSecToolK4Derive(secTempData.appKey, meshSecAppKeyDerivCback,
    ...
    if (keyType == MESH_SEC_KEY_TYPE_NWK) {
        ...
        if (MeshLocalCfgGetNextNetKeyIndex(&keyIndex, (uint16_t *)pParam) != MESH_SUCCESS) {
            ...
            *((uint16_t *)pParam) = 0;
            ...
            else if (meshSecHandleAppKeyDerivation(keyIndex, FALSE, meshSecRestoreKeyDerivCback,
                                                   pParam) != MESH_SUCCESS) {
        ...
        else if (meshSecHandleNetKeyDerivation(keyIndex, FALSE, meshSecRestoreKeyDerivCback,
                                               pParam) != MESH_SUCCESS) {
            /* Finish here with error. */
            meshSecCb.restoreCback(FALSE);
        }
                                      ...
                                      void *pParam)
{
    ...
    if (((keyType != MESH_SEC_KEY_TYPE_NWK) && (keyType != MESH_SEC_KEY_TYPE_APP)) ||
        (keyIndex > MESH_SEC_MAX_KEY_INDEX) || (keyMaterialAddedCback == NULL)) {
                                     ...
                                     meshSecFriendCredDerivCback_t friendCredAdded, void *pParam)
{
    ...
    /* Validate parameters. */
    if ((pFriendshipCred == NULL) || (friendCredAdded == NULL) ||
   ...
   */
    ...
    if (retVal != MESH_SUCCESS) {
        ...
        /* Start K2 derivation. */
        retVal = MeshSecToolK2Derive(k2PBuff, sizeof(k2PBuff), secTempData.nwkKeyFriend,
                                        ...
                                        uint16_t netKeyIndex)
{
    ...
    for (idx = 0; idx < secMatLocals.friendMatListSize; idx++) {
        ...
        if ((pFriendMat->friendAddres == friendAddr) && (pFriendMat->lpnAddress == lpnAddr)) {
            ...
            if ((pFriendMat->netKeyInfoIndex < secMatLocals.netKeyInfoListSize) &&
                (secMatLocals.pNetKeyInfoArray[pFriendMat->netKeyInfoIndex].hdr.keyIndex ==
                 netKeyIndex)) {
    ...
    for (idx = 0; idx < secMatLocals.netKeyInfoListSize; idx++) {
        ...
        if ((secMatLocals.pNetKeyInfoArray[idx].keyMaterial[crtEntry].masterPduSecMat.nid == nid) ||
            ((secMatLocals.pNetKeyInfoArray[idx].hdr.flags & MESH_SEC_KEY_UPDT_MAT_AVAILABLE) &&
             (secMatLocals.pNetKeyInfoArray[idx].keyMaterial[1 - crtEntry].masterPduSecMat.nid ==
              nid))) {
            return TRUE;
        }
    }
