...
static void meshSecNwkObfCback(const uint8_t *pCipherTextBlock, void *pParam)
{
    ...
    cback(pCipherTextBlock != NULL, pReq->nonce[MESH_SEC_NONCE_TYPE_POS] == MESH_SEC_NONCE_PROXY,
          pReq->pEncObfNwkPdu, pReq->encObfNwkPduSize, pReq->pNetMic, pReq->netMicSize,
          pReq->pParam);
}
...
static void meshSecNwkEncCcmCback(const meshSecToolCcmResult_t *pCcmResult, void *pParam)
{
    ...
    if (pCcmResult != NULL) {
        ...
        if (MeshSecToolAesEncrypt(pReq->pK, pReq->obfIn, meshSecNwkObfCback, pParam) ==
            MESH_SUCCESS) {
            isSuccess = TRUE;
        }
    }
    ...
    if (!isSuccess) {
        ...
        /* Invoke user callback. */
        cback(FALSE, pReq->nonce[0] == MESH_SEC_NONCE_PROXY, pReq->pEncObfNwkPdu,
              pReq->encObfNwkPduSize, pReq->pNetMic, pReq->netMicSize, pReq->pParam);
        return;
    }
}
...
static void meshSecNwkDecCcmCback(const meshSecToolCcmResult_t *pCcmResult, void *pParam)
{
        ...
        if (pCcmResult->results.decryptResult.isAuthSuccess) {
            ...
            pReq->encObfNwkPduSize -=
                (MESH_UTILS_BF_GET(pReq->ctlTtl, MESH_CTL_SHIFT, MESH_CTL_SIZE) == 0 ?
                     MESH_SEC_MIC_SIZE_32 :
                     MESH_SEC_MIC_SIZE_64);
            ...
            if (pReq->searchInFriendshipMat) {
                ...
                friendMatIdx =
                    (uint8_t)((pReq->keySearchIndex - 1) >> (MESH_SEC_KEY_MAT_PER_INDEX - 1));
            ...
            cback(TRUE, pReq->nonce[MESH_SEC_NONCE_TYPE_POS] == MESH_SEC_NONCE_PROXY, pReq->pNwkPdu,
                  pReq->encObfNwkPduSize, pReq->netKeyIndex, pReq->ivIndex, friendOrLpnAddr,
                  pReq->pParam);
    ...
    if (!isSuccess) {
        ...
        cback(FALSE, pReq->nonce[MESH_SEC_NONCE_TYPE_POS] == MESH_SEC_NONCE_PROXY, pReq->pNwkPdu,
              pReq->encObfNwkPduSize,
              MESH_SEC_INVALID_KEY_INDEX, /* Undetermined in case of failures. */
              0, /* Not important in case of failures. */
              friendOrLpnAddr, pReq->pParam);
    }
}
...
static void meshSecDeobfCback(const uint8_t *pCipherTextBlock, void *pParam)
{
    ...
    if (pCipherTextBlock != NULL) {
        ...
        if ((!MESH_IS_ADDR_UNICAST(pReq->srcAddr)) ||
            (pReq->encObfNwkPduSize <
             (MESH_DST_ADDR_POS + sizeof(meshAddress_t) + params.cbcMacSize))) {
        ...
        {
            /* Build nonce. */
            meshSecBuildNonce(pReq->nonce[MESH_SEC_NONCE_TYPE_POS], pReq->ctlTtl, pReq->srcAddr,
                              0x0000, pReq->seqNo, pReq->ivIndex, pReq->nonce);
            ...
            if (MeshSecToolCcmEncryptDecrypt(MESH_SEC_TOOL_CCM_DECRYPT, &params,
                                             meshSecNwkDecCcmCback, pParam) == MESH_SUCCESS) {
                isSuccess = TRUE;
            }
        }
    }
    ...
    if (!isSuccess) {
        ...
        cback(FALSE, pReq->nonce[MESH_SEC_NONCE_TYPE_POS] == MESH_SEC_NONCE_PROXY, pReq->pNwkPdu,
              ...
              MESH_SEC_INVALID_KEY_INDEX, /* Undetermined in case of failures. */
              0, /* Not important in case of failures. */
              MESH_ADDR_TYPE_UNASSIGNED, pReq->pParam);
    }
}
...
static meshSecRetVal_t meshSecSetNextFriendMatAndDeobf(meshSecNwkDeobfDecReq_t *pReq)
{
    ...
    for (; pReq->keySearchIndex < MESH_SEC_KEY_MAT_PER_INDEX * secMatLocals.friendMatListSize;
         pReq->keySearchIndex++) {
     ...
     */
        ...
        if (entryId != pNetKeyInfo->hdr.crtKeyId) {
            ...
            if (pFriendMat->hasUpdtMaterial) {
                ...
                if ((state < MESH_KEY_REFRESH_FIRST_PHASE) ||
                    (state > MESH_KEY_REFRESH_THIRD_PHASE)) {
                    continue;
                }
            } else {
                continue;
                                 ...
                                 meshSecNwkEncObfCback_t encObfCompleteCback, void *pParam)
{
    ...
    /* Validate parameters. */
    if ((pReqParams == NULL) || (encObfCompleteCback == NULL) ||
        (pReqParams->pNwkPduNoMic == NULL) || (pReqParams->pObfEncNwkPduNoMic == NULL) ||
    ...
    /* Build Nonce. */
    meshSecBuildNonce(isProxyConfig ? MESH_SEC_NONCE_PROXY : MESH_SEC_NONCE_NWK,
                      isProxyConfig ? 0 : pReqParams->pNwkPduNoMic[MESH_CTL_TTL_POS], src, 0x0000,
                      seqNo, pReqParams->ivIndex, pReq->nonce);
    ...
    /* Call toolbox. */
    retVal = (meshSecRetVal_t)MeshSecToolCcmEncryptDecrypt(MESH_SEC_TOOL_CCM_ENCRYPT, &ccmParams,
                                                           meshSecNwkEncCcmCback, (void *)pReq);
    ...
    if (retVal == MESH_SUCCESS) {
        ...
        MESH_UTILS_BF_SET(pReqParams->pObfEncNwkPduNoMic[MESH_IVI_NID_POS],
                          (uint8_t)(pReqParams->ivIndex & 0x01), MESH_IVI_SHIFT, MESH_IVI_SIZE);
        /* Set NID. */
        MESH_UTILS_BF_SET(pReqParams->pObfEncNwkPduNoMic[MESH_IVI_NID_POS], nid, MESH_NID_SHIFT,
                          MESH_NID_SIZE);
        ...
        memcpy(&(pReqParams->pObfEncNwkPduNoMic[MESH_CTL_TTL_POS]),
               &(pReqParams->pNwkPduNoMic[MESH_CTL_TTL_POS]), MESH_SEC_PRIV_RAND_SIZE - 1);
                                   ...
                                   meshSecNwkDeobfDecCback_t nwkDeobfDecCback, void *pParam)
{
    ...
    /* Validate parameters. */
    if ((pReqParams == NULL) || (nwkDeobfDecCback == NULL) || (pReqParams->pNwkPduNoMic == NULL) ||
    ...
    pReq->nonce[MESH_SEC_NONCE_TYPE_POS] = isProxyConfig ? MESH_SEC_NONCE_PROXY :
                                                           MESH_SEC_NONCE_NWK;
