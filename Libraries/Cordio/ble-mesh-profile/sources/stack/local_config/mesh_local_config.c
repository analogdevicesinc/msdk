    ...
    if (!MESH_IS_ADDR_UNASSIGNED(address)) {
        ...
        if (MESH_IS_ADDR_VIRTUAL(address)) {
            ...
            for (i = 0; i < localCfgVirtualAddrList.virtualAddrListSize; i++) {
                ...
                if (localCfgVirtualAddrList.pVirtualAddrList[i].address ==
                    ...
                    if (isPublishAddr == TRUE) {
                        ...
                        if (localCfgVirtualAddrList.pVirtualAddrList[i].referenceCountSubscr == 0) {
                            ...
                            localCfgCb.friendSubscrEventCback(MESH_LOCAL_CFG_FRIEND_SUBSCR_ADD,
                                                              &subscrEventParam);
                        }
                    ...
                    WsfNvmWriteData((uint64_t)MESH_LOCAL_CFG_NVM_VIRTUAL_ADDR_DATASET_ID,
                                    (uint8_t *)localCfgVirtualAddrList.pVirtualAddrList,
                                    sizeof(meshLocalCfgVirtualAddrListEntry_t) *
                                        localCfgVirtualAddrList.virtualAddrListSize,
                                    NULL);
            ...
            for (i = 0; i < localCfgAddressList.addressListSize; i++) {
                ...
                if (localCfgAddressList.pAddressList[i].address == MESH_ADDR_TYPE_UNASSIGNED) {
                    ...
                    WsfNvmWriteData((uint64_t)MESH_LOCAL_CFG_NVM_ADDRESS_DATASET_ID,
                                    (uint8_t *)localCfgAddressList.pAddressList,
                                    sizeof(meshLocalCfgAddressListEntry_t) *
                                        localCfgAddressList.addressListSize,
                                    NULL);
    ...
    if (isVirtualAddr == TRUE) {
        ...
        if (addrEntryIdx < localCfgVirtualAddrList.virtualAddrListSize) {
            if (isPublishAddr == TRUE) {
                ...
                if (localCfgVirtualAddrList.pVirtualAddrList[addrEntryIdx].referenceCountPublish >
                    0) {
                ...
                if (localCfgVirtualAddrList.pVirtualAddrList[addrEntryIdx].referenceCountSubscr >
                    0) {
                    ...
                    if (localCfgVirtualAddrList.pVirtualAddrList[addrEntryIdx]
                            .referenceCountSubscr == 0) {
                        subscrEventParam.address =
                            localCfgVirtualAddrList.pVirtualAddrList[addrEntryIdx].address;
                        subscrEventParam.idx = addrEntryIdx;

                        localCfgCb.friendSubscrEventCback(MESH_LOCAL_CFG_FRIEND_SUBSCR_RM,
                                                          &subscrEventParam);
                    }
                }
            }
        ...
        if (addrEntryIdx < localCfgAddressList.addressListSize) {
            if (isPublishAddr == TRUE) {
                ...
                if (localCfgAddressList.pAddressList[addrEntryIdx].referenceCountSubscr > 0) {
                    ...
                    /* Invoke callback if address was removed from subscription list. */
                    if ((localCfgAddressList.pAddressList[addrEntryIdx].referenceCountSubscr ==
                         0) &&
                        (MESH_IS_ADDR_GROUP(
                            localCfgAddressList.pAddressList[addrEntryIdx].address))) {
                        subscrEventParam.address =
                            localCfgAddressList.pAddressList[addrEntryIdx].address;
                        subscrEventParam.idx = addrEntryIdx;
        ...
        /* Update Address entry in NVM. */
        WsfNvmWriteData(
            (uint64_t)MESH_LOCAL_CFG_NVM_ADDRESS_DATASET_ID,
            (uint8_t *)localCfgAddressList.pAddressList,
...
/*************************************************************************************************/
static void
meshLocalCfgFriendSubscrEventNotifyCback(meshLocalCfgFriendSubscrEvent_t event,
 ...
 */
/*************************************************************************************************/
static meshLocalCfgModelEntry_t *meshLocalCfgGetMdlCfg(meshElementId_t elementId, modelId_t modelId,
                                                       bool_t isSig)
{
    uint16_t i;
    meshModelId_t model;
 ...
 */
/*************************************************************************************************/
static void meshLocalCfgInitModels(meshElementId_t id, uint16_t *pIdx, uint16_t *pAppKeyBindListIdx,
                                   uint16_t *pSubscrListIdx, bool_t sharedSubscrList)
{
    uint32_t j;
    ...
    for (j = 0; j < pMeshConfig->pElementArray[id].numSigModels; j++) {
        ...
        if (((sharedSubscrList == TRUE) &&
             (pSigModel->subscrListSize == MMDL_SUBSCR_LIST_SHARED)) ||
            ((sharedSubscrList == FALSE) &&
             (pSigModel->subscrListSize != MMDL_SUBSCR_LIST_SHARED))) {
    ...
    for (j = 0; j < pMeshConfig->pElementArray[id].numVendorModels; j++) {
        ...
        /* Filter only models that corresponds to the input parameters */
        if (((sharedSubscrList == TRUE) &&
             (pVendModel->subscrListSize == MMDL_SUBSCR_LIST_SHARED)) ||
            ((sharedSubscrList == FALSE) &&
             (pVendModel->subscrListSize != MMDL_SUBSCR_LIST_SHARED))) {
            pLocalMdlCfg->elementId = id;
...
uint32_t MeshLocalCfgGetRequiredMemory(void)
{
    ...
    reqMem =
        ...
        meshLocalCfgGetRequiredMemoryVirtualAddrList(
            pMeshConfig->pMemoryConfig->virtualAddrListMaxSize) +
...
void MeshLocalCfgInit(void)
{
    ...
    localCfgVirtualAddrList.virtualAddrListSize =
        pMeshConfig->pMemoryConfig->virtualAddrListMaxSize;
    /* Increment the memory buffer pointer. */
    tempVal = meshLocalCfgGetRequiredMemoryVirtualAddrList(
        pMeshConfig->pMemoryConfig->virtualAddrListMaxSize);
    pMemBuff += tempVal;
    ...
    /* Increment the memory buffer pointer. */
    tempVal =
        meshLocalCfgGetRequiredMemoryAppKeyBindList(localCfgAppKeyBindList.appKeyBindListSize);
    pMemBuff += tempVal;
    ...
    memset(localCfgModel.pModelArray, 0,
           sizeof(meshLocalCfgModelEntry_t) * localCfgModel.modelArraySize);
    ...
    for (i = 0; i < pMeshConfig->elementArrayLen; i++) {
        meshLocalCfgInitModels((meshElementId_t)i, &k, &appKeyBindListStartIdx, &subscrListStartIdx,
                               FALSE);
    }
    ...
    retVal = WsfNvmReadData(
        (uint64_t)MESH_LOCAL_CFG_NVM_NET_KEY_DATASET_ID, (uint8_t *)localCfgNetKeyList.pNetKeyList,
        sizeof(meshLocalCfgNetKeyListEntry_t) * localCfgNetKeyList.netKeyListSize, NULL);

    retVal = WsfNvmReadData(
        (uint64_t)MESH_LOCAL_CFG_NVM_APP_KEY_DATASET_ID, (uint8_t *)localCfgAppKeyList.pAppKeyList,
        sizeof(meshLocalCfgAppKeyListEntry_t) * localCfgAppKeyList.appKeyListSize, NULL);

    retVal = WsfNvmReadData((uint64_t)MESH_LOCAL_CFG_NVM_APP_KEY_BIND_DATASET_ID,
                            (uint8_t *)localCfgAppKeyBindList.pAppKeyBindList,
    ...
    retVal = WsfNvmReadData((uint64_t)MESH_LOCAL_CFG_NVM_VIRTUAL_ADDR_DATASET_ID,
                            (uint8_t *)localCfgVirtualAddrList.pVirtualAddrList,
                            sizeof(meshLocalCfgVirtualAddrListEntry_t) *
                                localCfgVirtualAddrList.virtualAddrListSize,
                            NULL);
    ...
    retVal = WsfNvmReadData(
        (uint64_t)MESH_LOCAL_CFG_NVM_SUBSCR_DATASET_ID, (uint8_t *)localCfgSubscrList.pSubscrList,
        sizeof(meshLocalCfgModelSubscrListEntry_t) * localCfgSubscrList.subscrListSize, NULL);

    retVal = WsfNvmReadData((uint64_t)MESH_LOCAL_CFG_NVM_SEQ_NUMBER_DATASET_ID,
                            (uint8_t *)localCfgElement.pSeqNumberArray,
                            sizeof(meshSeqNumber_t) * localCfgElement.elementArrayLen, NULL);

    retVal = WsfNvmReadData((uint64_t)MESH_LOCAL_CFG_NVM_SEQ_NUMBER_THRESH_DATASET_ID,
                            (uint8_t *)localCfgElement.pSeqNumberThreshArray,
                            sizeof(meshSeqNumber_t) * localCfgElement.elementArrayLen, NULL);

    retVal = WsfNvmReadData((uint64_t)MESH_LOCAL_CFG_NVM_MODEL_DATASET_ID,
                            (uint8_t *)localCfgModel.pModelArray,
                            sizeof(meshLocalCfgModelEntry_t) * localCfgModel.modelArraySize, NULL);

    retVal = WsfNvmReadData((uint64_t)MESH_LOCAL_CFG_NVM_HB_DATASET_ID, (uint8_t *)&localCfgHb,
                            sizeof(localCfgHb), NULL);
...
meshLocalCfgRetVal_t MeshLocalCfgSetPrimaryNodeAddress(meshAddress_t address)
{
    if (MESH_IS_ADDR_UNICAST(address)) {
        ...
        WsfNvmWriteData((uint64_t)MESH_LOCAL_CFG_NVM_DATASET_ID, (uint8_t *)&localCfg,
                        sizeof(localCfg), NULL);
    ...
    if (modelIdx != MESH_INVALID_ENTRY_INDEX) {
        ...
        if (MESH_IS_ADDR_UNASSIGNED(publishAddress)) {
            ...
            if (addrIdx != MESH_INVALID_ENTRY_INDEX) {
                ...
                WsfNvmWriteData((uint64_t)MESH_LOCAL_CFG_NVM_MODEL_DATASET_ID,
                                (uint8_t *)localCfgModel.pModelArray,
                                sizeof(meshLocalCfgModelEntry_t) * localCfgModel.modelArraySize,
                                NULL);
            }
            ...
            /* Add Index to module. */
            localCfgModel.pModelArray[modelIdx].publicationState.publishAddressIndex = newAddrIdx;
            localCfgModel.pModelArray[modelIdx].publicationState.publishToLabelUuid = FALSE;
                                        ...
                                        meshPublishRetransIntvlSteps_t retransSteps)
{
    ...
    /* Model found. */
    if (modelIdx != MESH_INVALID_ENTRY_INDEX) {
        localCfgModel.pModelArray[modelIdx].publicationState.publishRetransSteps50Ms = retransSteps;
    ...
    if (modelIdx != MESH_INVALID_ENTRY_INDEX) {
        ...
        if (newAddrIdx == MESH_INVALID_ENTRY_INDEX) {
            ...
            for (subscrIdx = subscrListOffset;
                 ...
                 subscrIdx++) {
                /* Empty slot found. */
                if (localCfgSubscrList.pSubscrList[subscrIdx].subscrAddressIndex ==
                    MESH_INVALID_ENTRY_INDEX) {
                    /* Set Address in Address List. */
                    ...
                    WsfNvmWriteData((uint64_t)MESH_LOCAL_CFG_NVM_SUBSCR_DATASET_ID,
                                    (uint8_t *)localCfgSubscrList.pSubscrList,
                                    sizeof(meshLocalCfgModelSubscrListEntry_t) *
                                        localCfgSubscrList.subscrListSize,
                                    NULL);
            ...
            if ((freeIdx != MESH_INVALID_ENTRY_INDEX) &&
                (subscrIdx ==
                 subscrListOffset + localCfgModel.pModelArray[modelIdx].subscrListSize)) {
                                                              ...
                                                              uint8_t *pInOutStartIndex)
{
    ...
    if (modelIdx != MESH_INVALID_ENTRY_INDEX) {
        ...
        for (; *pInOutStartIndex < localCfgModel.pModelArray[modelIdx].subscrListSize;
            ...
            addrIdx = localCfgSubscrList.pSubscrList[subscrListOffset + *pInOutStartIndex]
                          .subscrAddressIndex;
            /* Check for valid entry. */
            if (addrIdx != MESH_INVALID_ENTRY_INDEX) {
                /* Check if subscription address is Virtual or Non-virtual. */
                if (localCfgSubscrList.pSubscrList[subscrListOffset + *pInOutStartIndex]
                        .subscrToLabelUuid == FALSE) {
        ...
        if (addrIdx != MESH_INVALID_ENTRY_INDEX) {
            ...
            for (subscrIdx = subscrListOffset;
                ...
                if ((addrIdx == localCfgSubscrList.pSubscrList[subscrIdx].subscrAddressIndex) &&
                    ...
                    localCfgSubscrList.pSubscrList[subscrIdx].subscrAddressIndex =
                        MESH_INVALID_ENTRY_INDEX;
                    localCfgSubscrList.pSubscrList[subscrIdx].subscrToLabelUuid = FALSE;
                                                         ...
                                                         const meshModelId_t *pModelId)
{
     ...
     */
        WsfNvmWriteData(
            (uint64_t)MESH_LOCAL_CFG_NVM_SUBSCR_DATASET_ID,
            (uint8_t *)localCfgSubscrList.pSubscrList,
            sizeof(meshLocalCfgModelSubscrListEntry_t) * localCfgSubscrList.subscrListSize, NULL);
            ...
            if (MESH_IS_ADDR_VIRTUAL(subscrAddr)) {
                if (pLabelUuid != NULL) {
                    ...
                    if (localCfgSubscrList.pSubscrList[subscrIdx].subscrToLabelUuid) {
                        /* Check for a match. */
                        if ((localCfgVirtualAddrList.pVirtualAddrList[addrIdx].address ==
                             subscrAddr) &&
                            (memcmp(localCfgVirtualAddrList.pVirtualAddrList[addrIdx].labelUuid,
                                    pLabelUuid, MESH_LABEL_UUID_SIZE) == 0x00)) {
                            WSF_ASSERT(localCfgVirtualAddrList.pVirtualAddrList[addrIdx]
                                           .referenceCountSubscr > 0);
                            return TRUE;
                        }
                    }
                ...
                if (!localCfgSubscrList.pSubscrList[subscrIdx].subscrToLabelUuid) {
                    if (localCfgAddressList.pAddressList[addrIdx].address == subscrAddr) {
                        WSF_ASSERT(localCfgAddressList.pAddressList[addrIdx].referenceCountSubscr >
                                   0);
                        return TRUE;
                    }
                }
                                                      ...
                                                      uint16_t *pInOutStartIndex)
{
    ...
    for (; *pInOutStartIndex < localCfgAddressList.addressListSize; (*pInOutStartIndex)++) {
        /* Check if entry is valid. */
        if ((localCfgAddressList.pAddressList[*pInOutStartIndex].address !=
             MESH_ADDR_TYPE_UNASSIGNED) &&
                                                          ...
                                                          uint16_t *pInOutStartIndex)
{
    ...
    for (; *pInOutStartIndex < localCfgVirtualAddrList.virtualAddrListSize; (*pInOutStartIndex)++) {
        /* Check if entry is valid. */
        if ((localCfgVirtualAddrList.pVirtualAddrList[*pInOutStartIndex].address !=
             MESH_ADDR_TYPE_UNASSIGNED) &&
            (localCfgVirtualAddrList.pVirtualAddrList[*pInOutStartIndex].referenceCountSubscr !=
             0)) {
            /* Store Address Index. */
...
void MeshLocalCfgSetDevKey(const uint8_t *pDevKey)
{
    ...
    WsfNvmWriteData((uint64_t)MESH_LOCAL_CFG_NVM_DATASET_ID, (uint8_t *)&localCfg, sizeof(localCfg),
                    NULL);
}
...
meshLocalCfgRetVal_t MeshLocalCfgSetNetKey(uint16_t netKeyIndex, const uint8_t *pNetKey)
{
    ...
    if (netKeyIdx != MESH_INVALID_ENTRY_INDEX) {
        ...
        WsfNvmWriteData((uint64_t)MESH_LOCAL_CFG_NVM_NET_KEY_DATASET_ID,
                        (uint8_t *)localCfgNetKeyList.pNetKeyList,
                        sizeof(meshLocalCfgNetKeyListEntry_t) * localCfgNetKeyList.netKeyListSize,
                        NULL);
...
meshLocalCfgRetVal_t MeshLocalCfgUpdateNetKey(uint16_t netKeyIndex, const uint8_t *pNetKey)
{
    ...
    if (netKeyIdx != MESH_INVALID_ENTRY_INDEX) {
        ...
        if (localCfgNetKeyList.pNetKeyList[netKeyIdx].newKeyAvailable == FALSE) {
            memcpy(localCfgNetKeyList.pNetKeyList[netKeyIdx].netKeyNew, pNetKey, MESH_KEY_SIZE_128);

            localCfgNetKeyList.pNetKeyList[netKeyIdx].newKeyAvailable = TRUE;

            /* Update NetKey list in NVM. */
            WsfNvmWriteData(
                (uint64_t)MESH_LOCAL_CFG_NVM_NET_KEY_DATASET_ID,
                (uint8_t *)localCfgNetKeyList.pNetKeyList,
                sizeof(meshLocalCfgNetKeyListEntry_t) * localCfgNetKeyList.netKeyListSize, NULL);
        ...
        if (removeOldKeyOnly) {
            ...
            if (localCfgNetKeyList.pNetKeyList[netKeyIdx].newKeyAvailable == TRUE) {
                ...
                memcpy(localCfgNetKeyList.pNetKeyList[netKeyIdx].netKeyOld,
                       localCfgNetKeyList.pNetKeyList[netKeyIdx].netKeyNew, MESH_KEY_SIZE_128);
                localCfgNetKeyList.pNetKeyList[netKeyIdx].newKeyAvailable = FALSE;
            }
        } else {
            ...
            for (appKeyIdx = 0; appKeyIdx < localCfgAppKeyList.appKeyListSize; appKeyIdx++) {
                if (netKeyIdx == localCfgAppKeyList.pAppKeyList[appKeyIdx].netKeyEntryIndex) {
                    localCfgAppKeyList.pAppKeyList[appKeyIdx].netKeyEntryIndex =
                        MESH_INVALID_ENTRY_INDEX;
                }
            }
...
meshLocalCfgRetVal_t MeshLocalCfgSetAppKey(uint16_t appKeyIndex, const uint8_t *pAppKey)
{
    ...
    if (appKeyIdx != MESH_INVALID_ENTRY_INDEX) {
        ...
        WsfNvmWriteData((uint64_t)MESH_LOCAL_CFG_NVM_APP_KEY_DATASET_ID,
                        (uint8_t *)localCfgAppKeyList.pAppKeyList,
                        sizeof(meshLocalCfgAppKeyListEntry_t) * localCfgAppKeyList.appKeyListSize,
                        NULL);
...
meshLocalCfgRetVal_t MeshLocalCfgUpdateAppKey(uint16_t appKeyIndex, const uint8_t *pAppKey)
{
    ...
    if (appKeyIdx != MESH_INVALID_ENTRY_INDEX) {
        ...
        if (localCfgAppKeyList.pAppKeyList[appKeyIdx].newKeyAvailable == FALSE) {
            ...
            /* Update AppKey list in NVM. */
            WsfNvmWriteData(
                (uint64_t)MESH_LOCAL_CFG_NVM_APP_KEY_DATASET_ID,
                (uint8_t *)localCfgAppKeyList.pAppKeyList,
                sizeof(meshLocalCfgAppKeyListEntry_t) * localCfgAppKeyList.appKeyListSize, NULL);
        ...
        if (removeOldKeyOnly == TRUE) {
            if (localCfgAppKeyList.pAppKeyList[appKeyIdx].newKeyAvailable == TRUE) {
                ...
                memcpy(localCfgAppKeyList.pAppKeyList[appKeyIdx].appKeyOld,
                       localCfgAppKeyList.pAppKeyList[appKeyIdx].appKeyNew, MESH_KEY_SIZE_128);
                localCfgAppKeyList.pAppKeyList[appKeyIdx].newKeyAvailable = FALSE;
            }
        } else {
...
meshLocalCfgRetVal_t MeshLocalCfgGetAppKey(uint16_t appKeyIndex, uint8_t *pOutAppKey)
{
    ...
    /* Check if the appKeyIndex is in the list. */
    if (appKeyIdx != MESH_INVALID_ENTRY_INDEX) {
        memcpy(pOutAppKey, localCfgAppKeyList.pAppKeyList[appKeyIdx].appKeyOld, MESH_KEY_SIZE_128);
        return MESH_SUCCESS;
    }
    return MESH_LOCAL_CFG_NOT_FOUND;
}
...
meshLocalCfgRetVal_t MeshLocalCfgGetUpdatedAppKey(uint16_t appKeyIndex, uint8_t *pOutAppKey)
{
    ...
    if ((appKeyIdx != MESH_INVALID_ENTRY_INDEX) &&
        (localCfgAppKeyList.pAppKeyList[appKeyIdx].newKeyAvailable == TRUE)) {
        memcpy(pOutAppKey, localCfgAppKeyList.pAppKeyList[appKeyIdx].appKeyNew, MESH_KEY_SIZE_128);
        return MESH_SUCCESS;
    }
    return MESH_LOCAL_CFG_NOT_FOUND;
}
    ...
    if (modelIdx != MESH_INVALID_ENTRY_INDEX) {
        ...
        if (appKeyIdx != MESH_INVALID_ENTRY_INDEX) {
            ...
            for (keyBindIdx = appKeyListOffset;
                 keyBindIdx <
                 appKeyListOffset + localCfgModel.pModelArray[modelIdx].appKeyBindListSize;
                 keyBindIdx++) {
                /* AppKeyIndex found.*/
                ...
                if ((freeIdx == MESH_INVALID_ENTRY_INDEX) &&
                    (localCfgAppKeyBindList.pAppKeyBindList[keyBindIdx] ==
                     MESH_INVALID_ENTRY_INDEX)) {
                    freeIdx = keyBindIdx;
                }
            }
            ...
            if (freeIdx != MESH_INVALID_ENTRY_INDEX) {
                ...
                WsfNvmWriteData((uint64_t)MESH_LOCAL_CFG_NVM_APP_KEY_BIND_DATASET_ID,
                                (uint8_t *)localCfgAppKeyBindList.pAppKeyBindList,
                                sizeof(uint16_t) * localCfgAppKeyBindList.appKeyBindListSize, NULL);
        ...
        if (appKeyIdx != MESH_INVALID_ENTRY_INDEX) {
            ...
            for (keyBindIdx = appKeyListOffset;
                ...
                if (localCfgAppKeyBindList.pAppKeyBindList[keyBindIdx] == appKeyIdx) {
                    ...
                    WsfNvmWriteData((uint64_t)MESH_LOCAL_CFG_NVM_APP_KEY_BIND_DATASET_ID,
                                    (uint8_t *)localCfgAppKeyBindList.pAppKeyBindList,
                                    sizeof(uint16_t) * localCfgAppKeyBindList.appKeyBindListSize,
                                    NULL);
                }
            }
        }
 ...
 */
/*************************************************************************************************/
uint8_t MeshLocalCfgCountModelBoundAppKeys(meshElementId_t elementId, const meshModelId_t *pModelId)
{
    uint16_t modelIdx;
    uint16_t appKeyListOffset;
    uint16_t keyIdx;
                                                         ...
                                                         uint8_t *pInOutIndex)
{
    ...
    if (modelIdx != MESH_INVALID_ENTRY_INDEX) {
        ...
        /* Resume iteration. */
        for (; *pInOutIndex < localCfgModel.pModelArray[modelIdx].appKeyBindListSize;
             (*pInOutIndex)++) {
    ...
    for (appKeyIdx = 0; appKeyIdx < localCfgAppKeyList.appKeyListSize; appKeyIdx++) {
        ...
        if (netKeyIdx < localCfgNetKeyList.netKeyListSize) {
            ...
            if ((localCfgAppKeyList.pAppKeyList[appKeyIdx].appKeyIndex == appKeyIndex) &&
                ...
                WsfNvmWriteData((uint64_t)MESH_LOCAL_CFG_NVM_APP_KEY_DATASET_ID,
                                (uint8_t *)localCfgAppKeyList.pAppKeyList,
                                sizeof(meshLocalCfgAppKeyListEntry_t) *
                                    localCfgAppKeyList.appKeyListSize,
                                NULL);
...
meshLocalCfgRetVal_t MeshLocalCfgSetHbPubDst(meshAddress_t dstAddress)
{
    ...
    if (MESH_IS_ADDR_UNASSIGNED(dstAddress)) {
        ...
        if (addrIdx != MESH_INVALID_ENTRY_INDEX) {
            ...
            WsfNvmWriteData((uint64_t)MESH_LOCAL_CFG_NVM_HB_DATASET_ID, (uint8_t *)&localCfgHb,
                            sizeof(localCfgHb), NULL);
...
void MeshLocalCfgSetSeqNumberThresh(meshElementId_t elementId, meshSeqNumber_t seqNumber)
{
    ...
    if ((seqNumber % MESH_SEQ_NUMBER_NVM_INC) == 0) {
        ...
        WsfNvmWriteData((uint64_t)MESH_LOCAL_CFG_NVM_SEQ_NUMBER_THRESH_DATASET_ID,
                        (uint8_t *)localCfgElement.pSeqNumberThreshArray,
                        sizeof(meshSeqNumber_t) * localCfgElement.elementArrayLen, NULL);
    }
}
