                                     ...
                                     meshModelId_t *pModelId)
{
    ...
    if ((pEvt = (meshModelMsgRecvEvt_t *)WsfMsgAlloc(sizeof(meshModelMsgRecvEvt_t) +
                                                     pAccToMdlMsgInfo->msgParamLen)) != NULL) {
    ...
    while (pCoreMdl != NULL) {
        ...
        if (pCoreMdl->elemId == elemId) {
            ...
            for (opIdx = 0; opIdx < pCoreMdl->opcodeArrayLen; opIdx++) {
                ...
                if ((pAccToMdlMsgInfo->opcode.opcodeBytes[0] !=
                     pCoreMdl->pOpcodeArray[opIdx].opcodeBytes[0])) {
                    continue;
                }
                if ((MESH_OPCODE_SIZE(pAccToMdlMsgInfo->opcode) > 1) &&
                    ((pAccToMdlMsgInfo->opcode.opcodeBytes[1] !=
                      pCoreMdl->pOpcodeArray[opIdx].opcodeBytes[1]))) {
                    continue;
                }
                if ((MESH_OPCODE_SIZE(pAccToMdlMsgInfo->opcode) > 2) &&
                    ((pAccToMdlMsgInfo->opcode.opcodeBytes[2] !=
                      pCoreMdl->pOpcodeArray[opIdx].opcodeBytes[2]))) {
                    continue;
                }
                /* Invoke callback. */
...
static void meshAccSendMsgToModelInstance(meshAccToMdlMsgInfo_t *pAccToMdlMsgInfo,
                                          meshElementId_t dstElemId, uint8_t modelIdx, bool_t isSig)
{
    ...
    if ((pOpcodeArray != NULL) && (pHandlerId != NULL)) {
        ...
        for (jj = 0; jj < opcodeArrayLen; jj++) {
            ...
            if (MeshLocalCfgValidateModelToAppKeyBind(dstElemId, &mdlId,
                                                      pAccToMdlMsgInfo->appKeyIndex)) {
                /* Send WSF message. */
                           ...
                           meshPublishFriendshipCred_t publishFriendshipCred)
{
   ...
   */
    meshAccCancelPendingRetrans(src, pUtrAccPduTxInfo->pAccPduOpcode,
                                pUtrAccPduTxInfo->accPduOpcodeLen);
    ...
    while (ptr != NULL) {
        ...
        if (ptr->retransTmr.msg.param == tmrUid) {
            if (ptr->publishRetransCount > 0) {
                ...
                if (ptr->publishRetransCount != 0) {
                    /* Restart timer. */
                    WsfTimerStartMs(&(ptr->retransTmr),
                                    RETRANS_STEPS_TO_MS_TIME(ptr->publishRetransSteps50Ms));
                } else {
                    /* Remove from queue. */
...
static void meshAccMsgDelayTmrCback(uint16_t tmrUid)
{
    ...
    while (count > 0) {
        ...
        if (pMsg->delayTmr.msg.param == tmrUid) {
            ...
            meshAccSendMsg(NULL, &pMsg->msgInfo, pMsg->pMsgParam, pMsg->msgParamLen,
                           pMsg->netKeyIndex, MESH_PUBLISH_MASTER_SECURITY);
        ...
        if (pAccPduInfo->pduLen >= idx) {
            ...
            if (MESH_IS_ADDR_UNICAST(accToMdlMsgInfo.dst) ||
                ...
                if (MESH_IS_ADDR_FIXED_GROUP(accToMdlMsgInfo.dst)) {
                    ...
                    if (MeshLocalCfgGetElementIdFromAddr(accToMdlMsgInfo.dst, &dstElemId) !=
                        MESH_SUCCESS) {
                        return;
                    }
                ...
                meshAccSendMsgToModelUnicast(&accToMdlMsgInfo, dstElemId);
            } else if (MESH_IS_ADDR_DYN_GROUP(accToMdlMsgInfo.dst) ||
                       MESH_IS_ADDR_VIRTUAL(accToMdlMsgInfo.dst)) {
                                   ...
                                   uint16_t msgParamLen, uint16_t netKeyIndex)
{
    ...
    if ((pMsg = WsfMsgAlloc(apiMsgLen)) != NULL) {
        ...
        if (pMsgInfo->pDstLabelUuid != NULL) {
            pMsg->msgInfo.pDstLabelUuid =
                (uint8_t *)((uint8_t *)pMsg + sizeof(meshSendMessage_t) + msgParamLen);
                           ...
                           uint16_t msgParamLen)
{
    ...
    if (MeshLocalCfgGetPublishAppKeyIndex(pPubMsgInfo->elementId, &mdlId, &pubAppKeyIndex) !=
        MESH_SUCCESS) {
    ...
    if (MeshLocalCfgGetPublishRetransIntvlSteps(pPubMsgInfo->elementId, &mdlId,
                                                &pubRetransSteps50Ms) != MESH_SUCCESS) {
    ...
    if (pubRetransCount != 0) {
        ...
        if ((pAccPduPubInfo = WsfBufAlloc(sizeof(meshAccPduPubTxInfo_t) +
                                          MESH_OPCODE_SIZE(msgInfo.opcode) + msgParamLen)) ==
            NULL) {
            return;
        }
     ...
     */
        ...
        pAccPduPubInfo->utrAccPduTxInfo.pAccPduOpcode =
            ((uint8_t *)pAccPduPubInfo) + sizeof(meshAccPduPubTxInfo_t);
        pAccPduPubInfo->utrAccPduTxInfo.pAccPduParam =
            pAccPduPubInfo->utrAccPduTxInfo.pAccPduOpcode +
        ...
        /* Start timer. */
        WsfTimerStartMs(&(pAccPduPubInfo->retransTmr),
                        RETRANS_STEPS_TO_MS_TIME(pubRetransSteps50Ms));
    } else {
