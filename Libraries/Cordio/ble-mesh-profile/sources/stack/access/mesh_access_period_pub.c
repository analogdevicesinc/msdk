...
static const uint32_t ppStepResToMsTable[] = { 100, 1000, 10000, 60000 };
...
static void meshAccPpSendWsfPubEvt(meshAccPpQueueElem_t *pQueueElem, uint32_t timeToPublishMs)
{
    ...
    if (pQueueElem->isSig) {
        ...
        WSF_ASSERT(
            VENDOR_MODEL_INSTANCE(pQueueElem->elemId, pQueueElem->modelEntryIdx).pHandlerId !=
            NULL);
        handlerId =
            *(VENDOR_MODEL_INSTANCE(pQueueElem->elemId, pQueueElem->modelEntryIdx).pHandlerId);
    }
    ...
    if ((pEvt = (meshModelPeriodicPubEvt_t *)WsfMsgAlloc(sizeof(meshModelPeriodicPubEvt_t))) !=
        NULL) {
...
static bool_t meshAccPpReloadTime(meshAccPpQueueElem_t *pQueueElem, uint32_t *pOutPubTimeMs)
{
    ...
    /* Read publish period. */
    if (MeshLocalCfgGetPublishPeriod(pQueueElem->elemId, &mdlId, &numSteps, &stepRes) !=
        MESH_SUCCESS) {
        return FALSE;
    }
...
void MeshAccPpChanged(meshElementId_t elemId, meshModelId_t *pModelId)
{
    ...
    if (pModelId->isSigModel) {
        for (mdlInstanceIdx = 0; mdlInstanceIdx < pMeshConfig->pElementArray[elemId].numSigModels;
             mdlInstanceIdx++) {
            if (pModelId->modelId.sigModelId ==
                SIG_MODEL_INSTANCE(elemId, mdlInstanceIdx).modelId) {
        ...
        for (mdlInstanceIdx = 0;
             mdlInstanceIdx < pMeshConfig->pElementArray[elemId].numVendorModels;
             mdlInstanceIdx++) {
            if (pModelId->modelId.vendorModelId ==
                VENDOR_MODEL_INSTANCE(elemId, mdlInstanceIdx).modelId) {
                /* Handle Periodic publishing state changed. */
                meshAccHandlePpChanged(elemId, mdlInstanceIdx, pModelId);
                return;
            }
