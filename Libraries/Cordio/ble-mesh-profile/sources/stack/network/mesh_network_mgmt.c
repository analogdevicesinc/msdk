                                      ...
                                      meshKeyRefreshStates_t newState)
{
    ...
    retVal =
        (meshLocalCfgRetVal_t)MeshSecRemoveKeyMaterial(MESH_SEC_KEY_TYPE_NWK, netKeyIndex, TRUE);
...
void MeshNwkMgmtInit(void)
{
    ...
    MeshSeqRegister(meshNwkSeqThreshCback, MESH_NWK_MGMT_LOW_SEQ_THRESH,
                    MESH_NWK_MGMT_HIGH_SEQ_THRESH);
                                      ...
                                      meshKeyRefreshStates_t newState)
{
    ...
    MESH_TRACE_INFO3("NWK MGMT: Key refresh for %d - transition from %d to %d", netKeyIndex,
                     oldState, newState);
}
