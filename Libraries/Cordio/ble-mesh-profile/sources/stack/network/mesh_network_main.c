...
#define MESH_NWK_PDU_TAG_MASK_ADV_ONLY                                                       \
    (MESH_NWK_TAG_SEND_ON_ADV_IF | MESH_NWK_TAG_RLY_ON_ADV_IF | MESH_NWK_TAG_FWD_ON_ALL_IF | \
     MESH_NWK_TAG_FWD_ON_ALL_IF_EXCEPT)

/*! Network PDU Tag Mask for sending only on GATT bearer */
#define MESH_NWK_PDU_TAG_MASK_GATT_ONLY \
    (MESH_NWK_TAG_SEND_ON_GATT_IF | MESH_NWK_TAG_FWD_ON_ALL_IF | MESH_NWK_TAG_FWD_ON_ALL_IF_EXCEPT)

/*! Network PDU Tag Mask for PDU's sent only once */
#define MESH_NWK_PDU_TAG_MASK_NO_RETRANS \
    (MESH_NWK_TAG_FWD_ON_ALL_IF_EXCEPT | MESH_NWK_TAG_FWD_ON_ALL_IF | MESH_NWK_TAG_SEND_ON_GATT_IF)
...
static void meshNetworkManagePduSend(meshNwkPduMeta_t *pNwkPduMeta)
{
    ...
    for (idx = 0; idx < MESH_BR_MAX_INTERFACES; idx++) {
        ...
        if (MeshBrSendNwkPdu(nwkIfCb.interfaces[idx].brIfId, pNwkPduMeta->nwkPdu,
                             pNwkPduMeta->pduLen)) {
...
static void meshNwkTmrCback(uint16_t uid)
{
    ...
    if ((pNwkPduMeta->nwkPduTag & MESH_NWK_TAG_SEND_ON_ADV_IF) ||
        ...
        WSF_ASSERT(MESH_UTILS_BITMASK_XCL((pNwkPduMeta->nwkPduTag), (MESH_NWK_TAG_SEND_ON_ADV_IF |
                                                                     MESH_NWK_TAG_RLY_ON_ADV_IF)));
        /* Validate mutual exclusion between send and relay. */
        WSF_ASSERT(!MESH_UTILS_BITMASK_CHK((pNwkPduMeta->nwkPduTag), (MESH_NWK_TAG_SEND_ON_ADV_IF |
                                                                      MESH_NWK_TAG_RLY_ON_ADV_IF)));
                                             ...
                                             meshSecNwkEncObfCback_t secCback)
{
    ...
    /* Extract CTL. */
    uint8_t ctl =
        MESH_UTILS_BF_GET(pNwkPduMeta->nwkPdu[MESH_CTL_TTL_POS], MESH_CTL_SHIFT, MESH_CTL_SIZE);
                                       ...
                                       uint8_t *pNwkPduNetMic, uint8_t netMicSize, void *pParam)
{
    ...
    if (isSuccess == FALSE) {
        ...
        if (pNwkPduMeta->nwkPduTag != MESH_NWK_TAG_RLY_ON_ADV_IF) {
            ...
            if ((pNwkPduMeta->pduRefCount == 0) && (pNwkPduMeta->pduRetransCount == 0) &&
                (pNwkPduMeta->pduRetransTime == 0)) {
                /* No reason to keep the PDU, free memory. */
                WsfBufFree(pNwkPduMeta);
            } else {
                               ...
                               bool_t ifPassthr)
{
    ...
    if (ifPassthr) {
        ...
        meshNwkReadTransmissionParams(nwkPduTag, &(pNwkPduMeta->pduRetransCount),
 ...
 */
/*************************************************************************************************/
static void meshNwkGetRxInfoFromPdu(uint8_t *pNwkPdu, uint8_t pduLen,
                                    meshNwkPduRxInfo_t *pNwkPduRxInfo)
{
    uint8_t netMicSize = 0;

    /* Extract CTL. */
    pNwkPduRxInfo->ctl =
        MESH_UTILS_BF_GET(*(pNwkPdu + MESH_CTL_TTL_POS), MESH_CTL_SHIFT, MESH_CTL_SIZE);

    /* Extract TTL. */
    pNwkPduRxInfo->ttl =
        MESH_UTILS_BF_GET(*(pNwkPdu + MESH_CTL_TTL_POS), MESH_TTL_SHIFT, MESH_TTL_SIZE);
    /* Extract Sequence number. */
    pNwkPduRxInfo->seqNo = (*(pNwkPdu + MESH_SEQ_POS) << 16) |
                           (*(pNwkPdu + MESH_SEQ_POS + 1) << 8) | (*(pNwkPdu + MESH_SEQ_POS + 2));
                            ...
                            bool_t *pOutFwdToLtr, meshNwkPduBitfieldTag_t *pOutTag)
{
    ...
    *pOutTag = 0;
    ...
    if (!(*pOutFwdToLtr) &&
        nwkCb.lpnDstCheckCback(pNwkPduRxInfo->dst, pNwkPduRxInfo->netKeyIndex)) {
        *pOutFwdToLtr = TRUE;
    }
   ...
   */
    ...
    case MESH_ADV_BEARER:
        if (relayState == MESH_RELAY_FEATURE_ENABLED) {
            ...
            if ((pNwkPduRxInfo->src < elem0Addr) ||
                (pNwkPduRxInfo->src > elem0Addr + pMeshConfig->elementArrayLen)) {
...
static void meshNwkProcessRxPdu(meshNwkPduMeta_t *pNwkPduMeta, uint32_t ivIndex)
{
    ...
    /* Obtain Rx PDU Info. */
    meshNwkGetRxInfoFromPdu((uint8_t *)(pNwkPduMeta->nwkPdu), pNwkPduMeta->pduLen, &nwkPduRxInfo);
   ...
   */
    ...
    if (nwkPduTag != 0) {
        ...
        MESH_UTILS_BF_SET((pNwkPduMeta->nwkPdu[MESH_CTL_TTL_POS]), (nwkPduRxInfo.ttl),
                          MESH_TTL_SHIFT, MESH_TTL_SIZE);

        /* Set meta information based on tag. */
        meshNwkSetMetaInfo(pNwkPduMeta, pNwkPduMeta->rcvdBrIfId, nwkPduTag, nwkPduRxInfo.dst,
                           FALSE);
                                             ...
                                             meshSecNwkDeobfDecCback_t secCback)
{
   ...
   */
    return (meshNwkRetVal_t)MeshSecNwkDeobfDec(FALSE, &nwkDecParams, secCback,
                                               (void *)pRecvPduMeta);
}
                                         ...
                                         meshAddress_t friendOrLpnAddr, void *pParam)
{
    ...
    if ((isSuccess == FALSE) || (pNwkPduNoMic == NULL) || (pNwkPduMeta == NULL) ||
 ...
 */
/*************************************************************************************************/
static void meshBrToNwkPduRecvCback(meshBrInterfaceId_t brIfId, const uint8_t *pNwkPdu,
                                    uint8_t pduLen)
{
                                         ...
                                         const meshBrEventParams_t *pEventParams)
{
    ...
    if (brIfId == MESH_BR_INVALID_INTERFACE_ID || pEventParams == NULL) {
...
meshNwkRetVal_t MeshNwkSendLtrPdu(const meshNwkPduTxInfo_t *pNwkPduTxInfo)
{
    ...
    if (pNwkPduTxInfo == NULL || pNwkPduTxInfo->pLtrHdr == NULL || pNwkPduTxInfo->pUtrPdu == NULL) {
    ...
    if (!MESH_IS_ADDR_UNICAST(pNwkPduTxInfo->dst) && !MESH_IS_ADDR_GROUP(pNwkPduTxInfo->dst) &&
...
{
    /* Pack NID and IVI. */
    *(pHdr + MESH_IVI_NID_POS) =
        (((ivi << MESH_IVI_SHIFT) & MESH_IVI_MASK) | ((nid << MESH_NID_SHIFT) & MESH_NID_MASK));
