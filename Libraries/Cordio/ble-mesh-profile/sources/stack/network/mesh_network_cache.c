                                                  ...
                                                  const uint8_t *pNwkPdu, uint8_t pduLen)
{
    ...
    case MESH_NWK_CACHE_L1:
        ...
        cacheEntryL1 = ((uint32_t)pNwkPdu[pduLen - 4] << 24) |
                       ...
                       ((uint32_t)pNwkPdu[pduLen - 2] << 8) | ((uint32_t)pNwkPdu[pduLen - 1]);

        /* Signal message cache not empty. */
        isCacheNotEmpty = (meshNwkCacheCb.l1.head != meshNwkCacheCb.l1.tail);
    ...
    case MESH_NWK_CACHE_L2:
        ...
        cacheEntryL2.seqNo = ((uint32_t)pNwkPdu[2] << 16) | ((uint32_t)pNwkPdu[3] << 8) |
                             ((uint32_t)pNwkPdu[4]);
