...
#define BEACON_AUTH_WITH_NEW_KEY(netKeyIndex) \
    (MeshLocalCfgGetKeyRefreshPhaseState(netKeyIndex) == MESH_KEY_REFRESH_SECOND_PHASE)
...
static void meshNwkBeaconResumeAuth(void)
{
    ...
    while ((pBeaconMeta = (meshNwkBeaconMeta_t *)WsfQueueDeq(&meshNwkBeaconCb.rxBeaconQueue)) !=
           NULL) {
        /* Request authentication. */
        if (MeshSecBeaconAuthenticate(pBeaconMeta->pBeacon, secAuthCback, pBeaconMeta) !=
            MESH_SUCCESS) {
            /* On fail, free memory. */
 ...
 */
/*************************************************************************************************/
static void meshNwkBeaconPduRecvCback(meshBrInterfaceId_t brIfId, const uint8_t *pBeaconData,
                                      uint8_t dataLen)
{
    meshNwkBeaconMeta_t *pBeaconMeta;
    ...
    if (WsfQueueCount(&meshNwkBeaconCb.rxBeaconQueue) >= MESH_NWK_BEACON_RX_QUEUE_LIMIT) {
        return;
    }
    ...
    if ((pBeaconMeta = (meshNwkBeaconMeta_t *)WsfBufAlloc(sizeof(meshNwkBeaconMeta_t) +
                                                          MESH_NWK_BEACON_NUM_BYTES)) == NULL) {
        return;
    }
...
static void secGenCback(bool_t isSuccess, uint8_t *pBeacon, uint16_t netKeyIndex, void *pParam)
{
    ...
    while ((pBeaconMeta = WsfQueueDeq(&meshNwkBeaconCb.txBeaconQueue)) != NULL) {
        ...
        if (MeshSecBeaconComputeAuth(pBeaconMeta->pBeacon, pBeaconMeta->netKeyIndex,
                                     BEACON_AUTH_WITH_NEW_KEY(netKeyIndex), secGenCback,
                                     pBeaconMeta) == MESH_SUCCESS) {
            ...
            if (pBeaconMeta->onDemand) {
                /* Invoke callback. */
                pBeaconMeta->cbacks.onDemandCback(FALSE, pBeaconMeta->netKeyIndex,
                                                  pBeaconMeta->pBeacon);
                                            ...
                                            meshNwkBeaconMeta_t *pBeaconMeta)
{
    ...
    /* Try to generate next beacon. */
    if (!meshNwkBeaconGenNext(&meshNwkBeaconCb.bcastNetKeyIndexer,
                              meshNwkBeaconBcastGenComplCback)) {
...
static bool_t meshNwkBeaconGenNext(uint16_t *pIndexer, meshBeaconGenInternalCback_t cback)
{
    ...
    while (MeshLocalCfgGetNextNetKeyIndex(&netKeyIndex, pIndexer) == MESH_SUCCESS) {
        /* Allocate beacon and meta. */
        if ((pBeaconMeta = meshNwkBeaconAllocateBeaconMeta(
                 NULL, netKeyIndex, (meshNwkBeaconGenericCback_t)cback)) == NULL) {
            /* Skip to next. */
            continue;
        }
...
void MeshNwkBeaconTriggerSend(uint16_t netKeyIndex)
{
    ...
    if ((netKeyIndex > MESH_NET_KEY_INDEX_MAX_VAL) &&
        (netKeyIndex != MESH_NWK_BEACON_SEND_ON_ALL_NETKEYS)) {
        return;
    }
                                ...
                                meshBeaconGenOnDemandCback_t cback)
{
    ...
    /* Allocate beacon and meta. */
    if ((pBeaconMeta = meshNwkBeaconAllocateBeaconMeta(
             pBuf, netKeyIndex, (meshNwkBeaconGenericCback_t)cback)) == NULL) {
        return FALSE;
    }
