...
static void meshLpnRxPduNotifyCback(meshNwkPduRxInfo_t *pNwkPduRxInfo)
{
    ...
    if ((ctxIdx = meshLpnCtxIdxByNetKeyIndex(pNwkPduRxInfo->netKeyIndex)) !=
        MESH_LPN_INVALID_CTX_IDX) {
...
static void meshLpnCtlRecvCback(const meshLtrCtlPduInfo_t *pCtlPduInfo)
{
    ...
    if ((ctxIdx = meshLpnCtxIdxByNetKeyIndex(pCtlPduInfo->netKeyIndex)) !=
        MESH_LPN_INVALID_CTX_IDX) {
        ...
        case MESH_UTR_CTL_FRIEND_OFFER_OPCODE:
            if (MESH_FRIEND_RECV_WIN_VALID(
                ...
                if ((pMsg = WsfMsgAlloc(sizeof(meshLpnFriendOffer_t))) != NULL) {
                    ...
                    BYTES_BE_TO_UINT16(
                        ((meshLpnFriendOffer_t *)(pMsg))->friendCounter,
...
/*************************************************************************************************/
static void
meshLpnFriendSubscrEventNotifyCback(meshLocalCfgFriendSubscrEvent_t event,
    ...
    case MESH_LOCAL_CFG_FRIEND_SUBSCR_RM:
        for (i = 0; i < lpnCb.maxNumFriendships; i++) {
            if (lpnCb.pLpnTbl[i].inUse && lpnCb.pLpnTbl[i].established) {
                if ((ptr = (meshLpnFriendSubscrEvent_t *)WsfBufAlloc(
                         sizeof(meshLpnFriendSubscrEvent_t))) != NULL) {
...
uint8_t meshLpnCtxAlloc(uint16_t netKeyIndex)
{
    ...
    for (i = 0; i < lpnCb.maxNumFriendships; i++) {
        ...
        if ((lpnCb.pLpnTbl[i].inUse == FALSE) && (freeIdx == MESH_LPN_INVALID_CTX_IDX)) {
            freeIdx = i;
        }
    ...
    if (freeIdx != MESH_LPN_INVALID_CTX_IDX) {
        ...
        lpnCb.pLpnTbl[freeIdx].subscrReq.nextAddressIdx =
            pMeshConfig->pMemoryConfig->addrListMaxSize;
        lpnCb.pLpnTbl[freeIdx].subscrReq.nextVirtualAddrIdx =
...
void meshLpnHistoryAdd(uint16_t netKeyIndex, meshAddress_t addr)
{
    ...
    WSF_ASSERT(MESH_KEY_REFRESH_PROHIBITED_START !=
               MeshLocalCfgGetKeyRefreshPhaseState(netKeyIndex));
...
uint32_t MeshLpnMemInit(uint8_t *pFreeMem, uint32_t freeMemSize)
{
    ...
    lpnCb.pLpnHistory =
        (meshLpnFriendshipHistory_t *)(pFreeMem + meshLpnGetRequiredMemoryCtxTable());
                                  ...
                                  uint8_t establishRetryCount)
{
    ...
    pollTimeoutMs = sleepDurationMs +
                    (MESH_LPN_TX_NUM_RETRIES + 1) * (recvDelayMs + MESH_FRIEND_RECV_WIN_MS_MAX);

    /* Validate params. */
    if ((pLpnCriteria != NULL) && MESH_FRIEND_RSSI_FACTOR_VALID(pLpnCriteria->rssiFactor) &&
...
{
    /* If a valid event ID */
    if ((pMeshLpnEvt->event == MESH_LPN_EVENT) && (pMeshLpnEvt->param < MESH_LPN_MAX_EVENT)) {
