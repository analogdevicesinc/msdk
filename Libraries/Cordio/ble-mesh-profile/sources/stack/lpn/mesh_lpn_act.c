...
void meshLpnActSendFriendReq(meshLpnCtx_t *pLpnCtx, meshLpnSmMsg_t *pMsg)
{
    ...
    MESH_UTILS_BF_SET(ctlPdu[MESH_FRIEND_REQUEST_CRITERIA_OFFSET],
                      pLpnCtx->criteria.minQueueSizeLog, MESH_FRIEND_REQUEST_MIN_QUEUE_SIZE_SHIFT,
    ...
    pollTimeout =
        (pLpnCtx->sleepDurationMs +
         (pLpnCtx->txRetryCount + 1) * (pLpnCtx->recvDelayMs + MESH_FRIEND_RECV_WIN_MS_MAX)) /
...
void meshLpnActProcessFriendOffer(meshLpnCtx_t *pLpnCtx, meshLpnSmMsg_t *pMsg)
{
    ...
    if (((meshLpnFriendOffer_t *)pMsg)->queueSize >= (1 << pLpnCtx->criteria.minQueueSizeLog)) {
        /* Accept Offer. */
...
void meshLpnActFriendshipEstablished(meshLpnCtx_t *pLpnCtx, meshLpnSmMsg_t *pMsg)
{
    ...
    if (((refPhase == MESH_KEY_REFRESH_FIRST_PHASE) ||
         (refPhase == MESH_KEY_REFRESH_SECOND_PHASE)) &&
        (keyRef == TRUE)) {
        useNewKey = TRUE;
    ...
    if (subscrAddressListNotEmpty || subscrVirtualAddrListNotEmpty) {
        if ((pReqMsg = WsfMsgAlloc(sizeof(wsfMsgHdr_t))) != NULL) {
            ...
            while (MeshLocalCfgGetNextSubscrAddress(&address, &pLpnCtx->subscrReq.nextAddressIdx) ==
                   MESH_SUCCESS) {
                /* Check if address is group. */
                if (MESH_IS_ADDR_GROUP(address)) {
                    pLpnCtx->subscrReq.addrList[pLpnCtx->subscrReq.addrListCount] = address;

                    if (++pLpnCtx->subscrReq.addrListCount ==
                        MESH_LPN_SUBSCR_LIST_REQ_MAX_ENTRIES) {
                        break;
                    }
                }
            }
            ...
            if (pLpnCtx->subscrReq.addrListCount < MESH_LPN_SUBSCR_LIST_REQ_MAX_ENTRIES) {
                while (MeshLocalCfgGetNextSubscrVirtualAddr(
                           &address, &pLpnCtx->subscrReq.nextVirtualAddrIdx) == MESH_SUCCESS) {
                    pLpnCtx->subscrReq.addrList[pLpnCtx->subscrReq.addrListCount] = address;
...
void meshLpnActSendFriendSubscrAddRm(meshLpnCtx_t *pLpnCtx, meshLpnSmMsg_t *pMsg)
{
    ...
    /* Set OpCode. */
    ctlPduInfo.opcode = (pLpnCtx->subscrReq.add == TRUE) ?
                            MESH_UTR_CTL_FRIEND_SUBSCR_LIST_ADD_OPCODE :
    ...
    /* Add control PDU to PDU info. */
    ctlPduInfo.pCtlPdu = ctlPdu;
    ctlPduInfo.pduLen = MESH_FRIEND_SUBSCR_LIST_ADD_RM_NUM_BYTES(pLpnCtx->subscrReq.addrListCount);
...
void meshLpnActProcessFriendSubscrCnf(meshLpnCtx_t *pLpnCtx, meshLpnSmMsg_t *pMsg)
{
    ...
    if (((meshLpnFriendSubscrCnf_t *)pMsg)->tranNumber == pLpnCtx->tranNumber) {
        ...
        if ((pLpnCtx->subscrReq.nextAddressIdx != pMeshConfig->pMemoryConfig->addrListMaxSize) ||
            (pLpnCtx->subscrReq.nextVirtualAddrIdx !=
             pMeshConfig->pMemoryConfig->virtualAddrListMaxSize)) {
            if (pLpnCtx->subscrReq.nextAddressIdx != pMeshConfig->pMemoryConfig->addrListMaxSize) {
                while (MeshLocalCfgGetNextSubscrAddress(
                           &address, &pLpnCtx->subscrReq.nextAddressIdx) == MESH_SUCCESS) {
                    /* Check if address is group. */
                    if (MESH_IS_ADDR_GROUP(address)) {
                        pLpnCtx->subscrReq.addrList[pLpnCtx->subscrReq.addrListCount] = address;
        ...
        } else if ((ptr = (meshLpnFriendSubscrEvent_t *)WsfQueueDeq(&pLpnCtx->subscrListQueue)) !=
                   NULL) {
