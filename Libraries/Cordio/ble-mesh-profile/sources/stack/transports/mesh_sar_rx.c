...
#define SAR_RX_TRAN_LAST_FRIEND_SEG_INFO(tranIndex) \
    sarRxCb.pTranInfoTable[tranIndex]               \
        .pFriendSegInfo[sarRxCb.pTranInfoTable[tranIndex].friendSegInfoIdx]

/*! Creates the SAR Rx Block Mask with all fragments received */
#define SAR_RX_BLOCK_MASK(segN) \
    (((segN) == 31U) ? 0xFFFFFFFFU : (uint32_t)MESH_UTILS_BTMASK_MAKE((segN) + 1))
 ...
 */
/*************************************************************************************************/
static void meshSarRxEmptyFriendPduReassembledCback(
    meshSarRxPduType_t pduType, const meshSarRxReassembledPduInfo_t *pReasPduInfo,
    const meshSarRxSegInfoFriend_t *pSegInfoArray, uint32_t ivIndex, uint16_t seqZero, uint8_t segN)
{
...
static void meshSarRxResetTransaction(uint8_t tranIndex, bool_t obo)
{
    ...
    if (sarRxCb.pTranInfoTable[tranIndex].pduType == MESH_SAR_RX_TYPE_CTL) {
        MeshSarRxHistoryAdd(sarRxCb.pTranInfoTable[tranIndex].srcAddr,
                            ...
                            sarRxCb.pTranInfoTable[tranIndex].segN, obo);
    } else {
        MeshSarRxHistoryAdd(sarRxCb.pTranInfoTable[tranIndex].srcAddr,
...
static void meshSarRxSendAck(uint8_t tranIndex)
{
   ...
   */
    ...
    ackUtrPdu[pduOffset++] |=
        (sarRxCb.pTranInfoTable[tranIndex].seqZero >> (MESH_SEQ_ZERO_L_SIZE)) &
        MESH_SEQ_ZERO_H_MASK;
                                 ...
                                 bool_t obo)
{
    ...
    ackUtrPdu[pduOffset] = (obo << MESH_OBO_SHIFT) & MESH_OBO_MASK;
    ackUtrPdu[pduOffset++] |= pNwkPduInfo->pLtrPdu[MESH_SEQ_ZERO_H_PDU_OFFSET] &
                              MESH_SEQ_ZERO_H_MASK;
    ackUtrPdu[pduOffset++] = pNwkPduInfo->pLtrPdu[MESH_SEQ_ZERO_L_PDU_OFFSET] &
                             MESH_SEQ_ZERO_L_MASK;
    ...
    for (tranIndex = 0; tranIndex < sarRxCb.tranInfoSize; tranIndex++) {
        if (sarRxCb.pTranInfoTable[tranIndex].state == MESH_SAR_RX_TRAN_IN_PROGRESS) {
            addr = (sarRxCb.pTranInfoTable[tranIndex].pduType == MESH_SAR_RX_TYPE_ACCESS) ?
                       SAR_RX_TRAN_ACC_INFO(tranIndex)->dst :
                       SAR_RX_TRAN_CTL_INFO(tranIndex)->dst;
     ...
     */
        ...
        if (pNwkPduInfo->ctl == MESH_SAR_RX_TYPE_CTL) {
            ...
            sarRxCb.pTranInfoTable[tranIndex].pLtrPduInfo =
                WsfBufAlloc(sizeof(meshLtrCtlPduInfo_t) + (MESH_CTL_SEG_MAX_LENGTH * (segN + 1)));
            ...
            /* Set pointer to PDU */
            SAR_RX_TRAN_CTL_INFO(tranIndex)->pUtrCtlPdu =
                ((uint8_t *)SAR_RX_TRAN_CTL_INFO(tranIndex)) + sizeof(meshLtrCtlPduInfo_t);
            ...
            SAR_RX_TRAN_CTL_INFO(tranIndex)->pduLen =
                (pNwkPduInfo->pduLen - MESH_SEG_HEADER_LENGTH);
            ...
            SAR_RX_TRAN_CTL_INFO(tranIndex)->opcode =
                pNwkPduInfo->pLtrPdu[MESH_SEG_OPCODE_PDU_OFFSET] & MESH_CTL_OPCODE_MASK;
        } else {
            ...
            sarRxCb.pTranInfoTable[tranIndex].pLtrPduInfo =
                WsfBufAlloc(sizeof(meshLtrAccPduInfo_t) + (MESH_ACC_SEG_MAX_LENGTH * (segN + 1)));
            ...
            /* Set pointer to PDU */
            SAR_RX_TRAN_ACC_INFO(tranIndex)->pUtrAccPdu =
                ((uint8_t *)SAR_RX_TRAN_ACC_INFO(tranIndex)) + sizeof(meshLtrAccPduInfo_t);
            ...
            memcpy(pData, &pNwkPduInfo->pLtrPdu[MESH_SEG_DATA_PDU_OFFSET],
                   pNwkPduInfo->pduLen - MESH_SEG_HEADER_LENGTH);
            SAR_RX_TRAN_ACC_INFO(tranIndex)->pduLen =
                (pNwkPduInfo->pduLen - MESH_SEG_HEADER_LENGTH);
            ...
            SAR_RX_TRAN_ACC_INFO(tranIndex)->akf =
                MESH_UTILS_BF_GET(pNwkPduInfo->pLtrPdu[0], MESH_AKF_SHIFT, MESH_AKF_SIZE);
        }
        ...
        if (sarRxCb.lpnDstCheckCback(pNwkPduInfo->dst, pNwkPduInfo->netKeyIndex)) {
            ...
            /* Allocate Friend Segment Info. */
                          ...
                          (meshSarRxSegInfoFriend_t *)WsfBufAlloc(
                              (segN + 1) * sizeof(meshSarRxSegInfoFriend_t))) == NULL) {
                /* Check if the LPN was the only destination for the PDU. */
                if (MESH_IS_ADDR_UNICAST(pNwkPduInfo->dst)) {
                    /* No other elements needing this. So abort. */
                    meshSarRxResetTransaction(tranIndex, TRUE);
       ...
       */
            ...
            WsfTimerStartMs(&(sarRxCb.pTranInfoTable[tranIndex].incompTmr),
                            MESH_SAR_RX_INCOMPLETE_TIMEOUT_MS);
...
void MeshSarRxProcessSegment(const meshNwkPduRxInfo_t *pNwkPduInfo)
{
    ...
    if (tranIndex == sarRxCb.tranInfoSize) {
        ...
        if (MESH_IS_ADDR_UNICAST(pNwkPduInfo->dst)) {
            meshSarRxSendFastAck(pNwkPduInfo, 0,
                                 sarRxCb.lpnDstCheckCback(pNwkPduInfo->dst,
                                                          pNwkPduInfo->netKeyIndex));
        }
    ...
    if (sarRxCb.pTranInfoTable[tranIndex].state == MESH_SAR_RX_TRAN_IN_PROGRESS) {
        ...
        if ((sarRxCb.pTranInfoTable[tranIndex].recvIvIndex >
             SAR_RX_IVI_LSB(pNwkPduInfo->ivIndex)) ||
            ((sarRxCb.pTranInfoTable[tranIndex].recvIvIndex ==
              SAR_RX_IVI_LSB(pNwkPduInfo->ivIndex)) &&
             (sarRxCb.pTranInfoTable[tranIndex].seqZero > seqZero))) {
            return;
