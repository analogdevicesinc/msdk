...
#define MESH_IS_ADDR_UNICAST(addr) \
    ((((addr)&MESH_ADDR_TYPE_UNICAST_MASK) == 0) && !MESH_IS_ADDR_UNASSIGNED(addr))
...
#define MESH_IS_ADDR_GROUP(addr) (MESH_IS_ADDR_FIXED_GROUP(addr) || MESH_IS_ADDR_DYN_GROUP(addr))
...
#define MESH_ADDR_EXTRACT_TWO_MSBITS(addr) (((addr)&MESH_ADDR_TYPE_GROUP_VIRTUAL_MASK) >> 14)

/*! \brief TRUE if address type is virtual */
#define MESH_IS_ADDR_VIRTUAL(addr) \
    (MESH_ADDR_EXTRACT_TWO_MSBITS(addr) == MESH_ADDR_TYPE_VIRTUAL_MSBITS_VALUE)
...
#define MESH_OPCODE_SIZE(opcode) \
    (MESH_OPCODE_IS_SIZE_ONE(opcode) ? 1 : (uint8_t)MESH_OPCODE_MSBITS_TO_SIZE(opcode))
...
#define MESH_VENDOR_MODEL_MK(compId, modelId) (((meshVendorModelId_t)(compId) << 16) | (modelId))

/*! \brief Makes a vendor opcode from a 6-bit opcode and a 16-bit company ID */
#define MESH_VENDOR_OPCODE_MK(opcode, compId)                                    \
    {                                                                            \
        {                                                                        \
            (opcode | 0xC0), (uint8_t)((compId)&0xFF), (uint8_t)((compId) >> 16) \
        }                                                                        \
    }
