...
#define MESH_MSG_INFO(modelId, opcode)                                                   \
    {                                                                                    \
        { (meshSigModelId_t)(modelId) }, { { UINT16_TO_BE_BYTES(opcode) } }, 0xFF, NULL, \
            MESH_ADDR_TYPE_UNASSIGNED, 0xFF, 0xFF                                        \
    }

/*! \brief Initializer of a publish message info for the specified model ID and opcode */
#define MESH_PUB_MSG_INFO(modelId, opcode)        \
    {                                             \
        { { UINT16_TO_BE_BYTES(opcode) } }, 0xFF, \
        {                                         \
...
typedef struct meshModelLink_tag {
    ...
    meshElementId_t
        rootElementId; /*!< Identifier of the element which shares the subscription list */
...
typedef union meshEvt_tag {
    ...
    meshProxyFilterStatusEvt_t
        filterStatus; /*!< Proxy filter status event. See ::meshProxyFilterStatusEvt_t*/
