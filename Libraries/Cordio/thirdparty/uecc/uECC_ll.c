...
static int EccPoint_mult(EccPointMultCtx *pCtx, EccPoint *RESTRICT result,
                         const EccPoint *RESTRICT point, const uECC_word_t *RESTRICT scalar,
                         const uECC_word_t *RESTRICT initialZ, bitcount_t numBits)
{
    uECC_word_t nb;
    uECC_word_t z[uECC_WORDS];
    ...
    case ECC_POINT_MULT_STATE_BIT_ADD:
        ...
        pCtx->i--;
        pCtx->state = (pCtx->i > 0) ? ECC_POINT_MULT_STATE_BIT_ADDC :
                                      ECC_POINT_MULT_STATE_EXIT_ADDC;
        return 0;
...
int uECC_make_key_continue(void)
{
    ...
    case ECC_MAKE_KEY_STATE_ECC_POINT_MULT:
        if (EccPoint_mult(&uECC_ctx.makeKey.pointMultCtx, &uECC_ctx.makeKey.public, &curve_G,
                          uECC_ctx.makeKey.p2[!uECC_ctx.makeKey.carry], 0, (uECC_BYTES * 8) + 1)) {
                              ...
                              const uint8_t private_key[uECC_BYTES])
{
    ...
    for (tries = 0; tries < MAX_TRIES; ++tries) {
        if (g_rng_function((uint8_t *)uECC_ctx.sharedSecret.random,
                           sizeof(uECC_ctx.sharedSecret.random)) &&
            !vli_isZero(uECC_ctx.sharedSecret.random)) {
    ...
    uECC_ctx.sharedSecret.carry =
        vli_add(uECC_ctx.sharedSecret.private, uECC_ctx.sharedSecret.private, curve_n);
...
{
    return EccPoint_mult(&uECC_ctx.sharedSecret.pointMultCtx, &uECC_ctx.sharedSecret.product,
                         &uECC_ctx.sharedSecret.public,
                         uECC_ctx.sharedSecret.p2[!uECC_ctx.sharedSecret.carry],
                         uECC_ctx.sharedSecret.initial_Z, (uECC_BYTES * 8) + 1);
}
