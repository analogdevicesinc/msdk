...
bool_t smpScProcPairing(smpCcb_t *pCcb, uint8_t *pOob, uint8_t *pDisplay)
{
    ...
    *pOob = SMP_OOB_DATA_NONE;
    ...
    if ((*pOob == SMP_OOB_DATA_NONE) && ((pCcb->pairReq[SMP_AUTHREQ_POS] & SMP_AUTH_MITM_FLAG) ||
        ...
        if ((pCcb->pairReq[SMP_IO_POS] != SMP_IO_NO_IN_NO_OUT) && /* initiator has i/o and */
            ...
            *pDisplay = ((pCcb->pairReq[SMP_IO_POS] ==
                          SMP_IO_DISP_ONLY) || /* initiator is display only or */
                         (pCcb->pairReq[SMP_IO_POS] ==
                          SMP_IO_DISP_YES_NO) || /* initiator is display y/n  or */
                         ((pCcb->pairReq[SMP_IO_POS] ==
                           SMP_IO_KEY_DISP) && /* initiator is key/display and */
                          (pCcb->pairRsp[SMP_IO_POS] ==
                               SMP_IO_KEY_ONLY || /* responder is key only or key/display */
    ...
    if ((pCcb->pairReq[SMP_AUTHREQ_POS] & SMP_AUTH_SC_FLAG) &&
        ...
        if (SmpScAllocScratchBuffers(pCcb)) {
            ...
            memcpy(pCcb->pScCcb->pLocalPublicKey->pubKeyX, DmSecGetEccKey()->pubKey_x,
                   SMP_PUB_KEY_LEN);
            memcpy(pCcb->pScCcb->pLocalPublicKey->pubKeyY, DmSecGetEccKey()->pubKey_y,
                   SMP_PUB_KEY_LEN);
    ...
    if (!justWorks) {
        ...
        pCcb->auth = (pCcb->pairReq[SMP_AUTHREQ_POS] & pCcb->pairRsp[SMP_AUTHREQ_POS]) |
                     SMP_AUTH_MITM_FLAG;
    } else {
...
void smpScActAuthSelect(smpCcb_t *pCcb, smpMsg_t *pMsg)
{
    ...
    WStrReverseCpy(pCcb->pScCcb->pPeerPublicKey->pubKeyX, &pPacket[SMP_PUB_KEY_X_POS],
                   SMP_PUB_KEY_LEN);
    WStrReverseCpy(pCcb->pScCcb->pPeerPublicKey->pubKeyY, &pPacket[SMP_PUB_KEY_Y_POS],
                   SMP_PUB_KEY_LEN);
...
void smpScActJwncCalcF4(smpCcb_t *pCcb, smpMsg_t *pMsg)
{
    ...
    if (pCcb->initiator) {
        ...
        SmpScCalcF4(pCcb, pMsg, pCcb->pScCcb->pPeerPublicKey->pubKeyX,
                    pCcb->pScCcb->pLocalPublicKey->pubKeyX, 0, pCcb->pScCcb->pScratch->Nb_Eb);
    } else {
        SMP_TRACE_256("F4 PKb", pCcb->pScCcb->pLocalPublicKey->pubKeyX);
        SMP_TRACE_256("F4 PKa", pCcb->pScCcb->pPeerPublicKey->pubKeyX);
        SMP_TRACE_128("F4 Nb", pCcb->pScCcb->pScratch->Nb_Eb);

        SmpScCalcF4(pCcb, pMsg, pCcb->pScCcb->pLocalPublicKey->pubKeyX,
                    pCcb->pScCcb->pPeerPublicKey->pubKeyX, 0, pCcb->pScCcb->pScratch->Nb_Eb);
    }
}
