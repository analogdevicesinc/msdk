...
void smprScActStoreLescPin(smpCcb_t *pCcb, smpMsg_t *pMsg)
{
    if (pCcb->pScCcb->authType == SMP_AUTH_TYPE_PASSKEY) {
        ...
        if (pMsg->dm.authRsp.authDataLen <= 3) {
            WStrReverseCpy(&pCcb->pScCcb->pScratch->Ra[13], pMsg->dm.authRsp.authData,
                           pMsg->dm.authRsp.authDataLen);
            WStrReverseCpy(&pCcb->pScCcb->pScratch->Rb[13], pMsg->dm.authRsp.authData,
                           pMsg->dm.authRsp.authDataLen);
        }
    }
}
...
void smprScActPkCalcCb(smpCcb_t *pCcb, smpMsg_t *pMsg)
{
    ...
    SmpScCalcF4(pCcb, pMsg, pCcb->pScCcb->pLocalPublicKey->pubKeyX,
                pCcb->pScCcb->pPeerPublicKey->pubKeyX, smpGetPkBit(pCcb),
                pCcb->pScCcb->pScratch->Nb_Eb);
}
...
void smprScActPkCalcCa(smpCcb_t *pCcb, smpMsg_t *pMsg)
{
    ...
    SmpScCalcF4(pCcb, pMsg, pCcb->pScCcb->pPeerPublicKey->pubKeyX,
                pCcb->pScCcb->pLocalPublicKey->pubKeyX, smpGetPkBit(pCcb),
                pCcb->pScCcb->pScratch->Na_Ea);
}
...
void smprScActOobCalcCa(smpCcb_t *pCcb, smpMsg_t *pMsg)
{
    ...
    if (pCcb->pairRsp[SMP_OOB_POS] == SMP_OOB_DATA_PRESENT) {
        ...
        SmpScCalcF4(pCcb, pMsg, pCcb->pScCcb->pPeerPublicKey->pubKeyX,
                    pCcb->pScCcb->pPeerPublicKey->pubKeyX, 0, pCcb->pScCcb->pScratch->Ra);
    } else {
...
void smprScActDHKeyCheckSend(smpCcb_t *pCcb, smpMsg_t *pMsg)
{
    ...
    if (memcmp(pCcb->pScCcb->pScratch->PeerCa_Ea, pCcb->pScCcb->pScratch->Na_Ea, SMP_RAND_LEN) ==
        0) {
