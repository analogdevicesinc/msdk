...
bool_t smpSendKey(smpCcb_t *pCcb, uint8_t keyDist)
{
    ...
    if ((keyDist == 0) || (keyDist == SMP_KEY_DIST_ENC && pCcb->lastSentKey == SMP_CMD_MASTER_ID) ||
        (keyDist <= (SMP_KEY_DIST_ENC | SMP_KEY_DIST_ID) &&
         pCcb->lastSentKey == SMP_CMD_ID_ADDR_INFO) ||
    ...
    if ((pPkt = smpMsgAlloc(SMP_ENC_INFO_LEN + L2C_PAYLOAD_START)) != NULL) {
        ...
        if (pCcb->lastSentKey == 0 && (keyDist & SMP_KEY_DIST_ENC)) {
            ...
            WsfMsgFree(pPkt);
            SMP_TRACE_WARN2("smpSendKey unexpected state keyDist:%d lastSentKey:%d", keyDist,
                            pCcb->lastSentKey);
            return TRUE;
        }
...
bool_t smpProcRcvKey(smpCcb_t *pCcb, dmSecKeyIndEvt_t *pKeyInd, uint8_t *pBuf, uint8_t keyDist)
{
    ...
    if (cmdCode == SMP_CMD_ENC_INFO) {
        ...
        pKeyInd->secLevel = (pCcb->auth & SMP_AUTH_MITM_FLAG) ? DM_SEC_LEVEL_ENC_AUTH :
                                                                DM_SEC_LEVEL_ENC;
    ...
    else if ((keyDist & SMP_KEY_DIST_SIGN) && (pCcb->nextCmdCode == SMP_CMD_MASTER_ID ||
                                               pCcb->nextCmdCode == SMP_CMD_ID_ADDR_INFO)) {
        /* wait for SRK */
...
void smpSmExecute(smpCcb_t *pCcb, smpMsg_t *pMsg)
{
    ...
    if (smpCb.lescSupported && pCcb->pScCcb && pCcb->pScCcb->lescEnabled)
        SMP_TRACE_INFO2("SMP Exe: evt=%s st=%s", smpEventStr(pMsg->hdr.event),
                        smpStateStr(pCcb->state));
    else
#endif
