...
static const dmCisAct_t dmCisActSetMain[] = {
    dmCisSmActNone, dmCisSmActClose, dmCisSmActCisEst, dmCisSmActCisEstFailed, dmCisSmActCisClosed,
};

/* CIS component function interface */
static const dmFcnIf_t dmCisFcnIf = { dmCisReset, dmCisHciHandler, dmCisMsgHandler };
        ...
        if (pCcb == NULL) {
            ...
            if ((connId = DmConnIdByHandle(pEvent->leCisReq.aclHandle)) != DM_CONN_ID_NONE) {
                ...
                if (DmConnRole(connId) == DM_ROLE_SLAVE) {
                    pCcb = dmCisCcbAlloc(pEvent->leCisReq.cigId, pEvent->leCisReq.cisId,
                                         DM_ROLE_SLAVE);
                }
            }
        }
