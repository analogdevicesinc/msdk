...
static const dmScanAct_t dmScanAct[] = { dmScanActStart, dmScanActStop, dmScanActTimeout };

/* Component function interface */
static const dmFcnIf_t dmScanFcnIf = { dmScanReset, dmScanHciHandler, dmScanMsgHandler };
    ...
    if (dmScanCb.scanState == DM_SCAN_STATE_SCANNING) {
        ...
        if (dmScanCb.discFilter != 0) {
            ...
            /* else it's an advertising response */
                ...
                p = DmFindAdType(DM_ADV_TYPE_FLAGS, pEvent->leAdvReport.len,
                                 pEvent->leAdvReport.pData);
                if (p == NULL) {
                    /* flags not found */
                    filtered = TRUE;
...
void dmScanHciHandler(hciEvt_t *pEvent)
{
    if (pEvent->hdr.event == HCI_LE_ADV_REPORT_CBACK_EVT) {
        ...
        dmDevPassEvtToDevPriv((pEvent->hdr.event == DM_SCAN_START_IND) ? DM_DEV_PRIV_MSG_RPA_START :
