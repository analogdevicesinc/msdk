...
static const dmScanAct_t dmScanAct[] = { dmExtScanActStart, dmExtScanActStop, dmExtScanActTimeout };

/* extended scan component function interface */
static const dmFcnIf_t dmScanFcnIf = { dmExtScanReset, dmExtScanHciHandler, dmExtScanMsgHandler };
...
#define DM_ADV_RPT_SCAN_RSP(evtType)                            \
    ((((evtType) == HCI_ADV_RPT_LEG_CONN_UNDIRECT_SCAN_RSP) ||  \
      ((evtType) == HCI_ADV_RPT_LEG_SCAN_UNDIRECT_SCAN_RSP)) || \
     ((((evtType)&HCI_ADV_RPT_LEG_ADV_BIT) == 0) && ((evtType)&HCI_ADV_RPT_SCAN_RSP_BIT)))
...
void dmExtScanActStart(dmScanMsg_t *pMsg)
{
    ...
    if (dmScanCb.scanState == DM_SCAN_STATE_IDLE) {
        ...
        HciLeExtScanEnableCmd(TRUE, pMsg->apiStart.filterDup, (pMsg->apiStart.duration / 10),
                              pMsg->apiStart.period);
    }
}
    ...
    if (dmScanCb.scanState == DM_SCAN_STATE_SCANNING) {
        ...
        if ((dmScanCb.discFilter != 0) && firstFrag) {
            ...
            /* else it's an advertising response */
                ...
                p = DmFindAdType(DM_ADV_TYPE_FLAGS, pEvent->leExtAdvReport.len,
                                 pEvent->leExtAdvReport.pData);
                if (p == NULL) {
                    /* flags not found */
                    filtered = TRUE;
        ...
        if ((dmScanCb.discFilter != 0) &&
            (DM_ADV_RPT_DATA_STATUS(pEvent->leExtAdvReport.eventType) !=
             HCI_ADV_RPT_DATA_INCMPL_MORE)) {
            /* reset our flags */
            filtered = FALSE;
            firstFrag = TRUE;
        }
...
{
    DM_TRACE_INFO2("dmExtScanHciHandler: event: %d state: %d", pEvent->hdr.event,
                   dmScanCb.scanState);
    ...
    if (pEvent->hdr.event == HCI_LE_EXT_ADV_REPORT_CBACK_EVT) {
        ...
        case DM_SCAN_STATE_STARTING:
            ...
            dmScanCb.scanState = pEvent->hdr.status == HCI_SUCCESS ? DM_SCAN_STATE_SCANNING :
                                                                     DM_SCAN_STATE_IDLE;
            break;

        case DM_SCAN_STATE_STOPPING:
            pEvent->hdr.event = DM_EXT_SCAN_STOP_IND;
            dmScanCb.scanState = pEvent->hdr.status == HCI_SUCCESS ? DM_SCAN_STATE_IDLE :
