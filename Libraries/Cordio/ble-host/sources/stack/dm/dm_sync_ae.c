...
#define DM_SYNC_HCI_EVT_2_MSG(evt) \
    (DM_SYNC_MSG_HCI_LE_SYNC_LOST - HCI_LE_PER_ADV_SYNC_LOST_CBACK_EVT + (evt))
...
static const dmSyncAct_t dmSyncAct[] = {
    dmSyncSmActNone,        dmSyncSmActStart,       dmSyncSmActStop,
    dmSyncSmActCancelStart, dmSyncSmActSyncEst,     dmSyncSmActSyncEstFailed,
    dmSyncSmActSyncLost,    dmSyncSmActSyncTrsfEst, dmSyncSmActSyncTrsfEstFailed
};

/*! DM Sync component function interface */
static const dmFcnIf_t dmSyncFcnIf = { dmSyncReset, dmSyncHciHandler, dmSyncMsgHandler };
...
static dmSyncCb_t *dmSyncCmplState(void)
{
    ...
    for (i = DM_SYNC_MAX; i > 0; i--, pScb++) {
        ...
        if (pScb->inUse && (pScb->state == DM_SYNC_SM_ST_DESYNCING) &&
...
void dmSyncHciHandler(hciEvt_t *pEvent)
{
    ...
    /* handle special cases for past received event */
        ...
        /* first check if scb exists for this sid and bd addr */
        pScb = dmSyncCbBySidBdAddr(pEvent->lePerAdvSyncTrsfRcvd.advSid,
                                   pEvent->lePerAdvSyncTrsfRcvd.advAddr);

        /* if scb not found */
        if (pScb == NULL) {
            /* allocate scb */
            pScb = dmSyncCbAlloc(pEvent->lePerAdvSyncTrsfRcvd.advSid,
                                 pEvent->lePerAdvSyncTrsfRcvd.advAddr);
        }
