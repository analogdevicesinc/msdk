...
static const dmPrivAct_t dmPrivAct[] = {
    dmPrivActResolveAddr,  dmPrivActAddDevToResList,  dmPrivActRemDevFromResList,
    dmPrivActClearResList, dmPrivActSetAddrResEnable, dmPrivActSetPrivacyMode,
    dmPrivActGenAddr
};

/* Privacy component function interface */
static const dmFcnIf_t dmPrivFcnIf = { dmPrivReset, dmPrivHciHandler, dmPrivMsgHandler };

/* Privacy AES action function table */
static const dmPrivAct_t dmPrivAesAct[] = { dmPrivAesActResAddrAesCmpl,
                                            dmPrivAesActGenAddrAesCmpl };

/* Privacy AES component function interface */
static const dmFcnIf_t dmPrivAesFcnIf = { dmEmptyReset, (dmHciHandler_t)dmEmptyHandler,
                                          dmPrivAesMsgHandler };
...
void dmPrivActResolveAddr(dmPrivMsg_t *pMsg)
{
    ...
    if ((dmPrivCb.inProgress & DM_PRIV_INPROGRESS_RES_ADDR) == 0) {
        ...
        /* run calculation */
        SecAes(pMsg->apiResolveAddr.irk, buf, dmCb.handlerId, pMsg->hdr.param,
...
void dmPrivActGenAddr(dmPrivMsg_t *pMsg)
{
    if ((dmPrivCb.inProgress & DM_PRIV_INPROGRESS_GEN_ADDR) == 0) {
        ...
        /* pad buffer */
        memset(dmPrivCb.genAddrBuf + DM_PRIV_PRAND_LEN, 0,
               (DM_PRIV_PLAINTEXT_LEN - DM_PRIV_PRAND_LEN));

        /* set in progress */
        dmPrivCb.inProgress |= DM_PRIV_INPROGRESS_GEN_ADDR;

        /* run calculation */
        SecAes(pMsg->apiGenerateAddr.irk, dmPrivCb.genAddrBuf, dmCb.handlerId, pMsg->hdr.param,
...
void dmPrivHciHandler(hciEvt_t *pEvent)
{
    ...
    case HCI_LE_SET_ADDR_RES_ENABLE_CMD_CMPL_CBACK_EVT:
        ...
        if (pEvent->hdr.status == HCI_SUCCESS) {
            ...
            dmDevPassEvtToDevPriv(dmCb.llPrivEnabled ? DM_DEV_PRIV_MSG_RPA_STOP :
