...
#define DM_BIS_HCI_EVT_2_MSG(evt) \
    (DM_BIS_MSG_HCI_LE_BIG_SYNC_LOST - HCI_LE_BIG_SYNC_LOST_CBACK_EVT + (evt))
...
static const dmBisAct_t dmBisAct[] = { dmBisSmActNone,        dmBisSmActBigSyncStart,
                                       dmBisSmActBigSyncStop, dmBisSmActBigSyncCancelStart,
                                       dmBisSmActBigSyncEst,  dmBisSmActBigSyncEstFailed,
                                       dmBisSmActBigSyncLost, dmBisSmActBigSyncStopped };
    ...
    if (DmSyncEnabled(pBigSyncStart->syncHandle)) {
        if (!dmBigSyncUsed(pBigSyncStart->syncHandle)) {
            if ((pBigSyncStart->numBis > 0) && (pBigSyncStart->numBis <= DM_BIS_MAX)) {
                ...
                DM_TRACE_WARN2(
                    "dmBisSmActBigSyncStart: invalid value for numBis = %u, validRange = 1..%u",
                    pBigSyncStart->numBis, DM_BIS_MAX);
                status = HCI_ERR_UNSUP_FEAT;
            }
        } else {
            DM_TRACE_WARN0(
                "dmBisSmActBigSyncStart: periodic advertising train is associated with another BIG");
            status = HCI_ERR_UNKNOWN_ADV_ID;
        }
    } else {
        DM_TRACE_WARN1("dmBisSmActBigSyncStart: syncHandle = %u not found",
                       pBigSyncStart->syncHandle);
        status = HCI_ERR_UNKNOWN_ADV_ID;
    }
