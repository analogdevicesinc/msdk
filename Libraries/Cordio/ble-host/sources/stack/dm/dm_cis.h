...
typedef union {
    ...
    hciLeSetCigParamsCmdCmplEvt_t
        hciLeSetCigParamsCmdCmpl; /*!< Set CIS Parameters Command Complete */
...
typedef struct {
    uint8_t cigId; /*!< Used to identify the connected isochronous group. */
    uint32_t
        sduIntervalMToS; /*!< Time interval between the start of consecutive SDUs from the master Host (in usec).  */
    uint32_t
        sduIntervalSToM; /*!< Time interval between the start of consecutive SDUs from the slave Host (in usec).  */
    uint8_t sca; /*!< Sleep clock accuracy. */
    uint8_t packing; /*!< Packing scheme. */
    uint8_t framing; /*!< Indicates the format of CIS Data PDUs.  */
    uint16_t
        transLatMToS; /*!< Maximum time for an SDU to be transported from the master Controller to slave Controller (in msec). */
    uint16_t
        transLatSToM; /*!< Maximum time for an SDU to be transported from the slave Controller to master Controller (in msec). */
