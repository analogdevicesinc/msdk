...
static void attsSignedWriteStart(attsSignCcb_t *pCcb, attsSignBuf_t *pBuf)
{
    if (pCcb->pCsrk != NULL) {
        ...
        if (pCmacTxt) {
            ...
            if (SecCmac(revPeerCsrk, pCmacTxt, cmacTxtLen, attCb.handlerId, pBuf->connId,
...
static void attsProcSignedWrite(attsCcb_t *pCcb, uint16_t len, uint8_t *pPacket)
{
    ...
    if ((pAttr = attsFindByHandle(handle, &pGroup)) != NULL) {
        /* verify permissions */
        if (attsPermissions(pCcb->connId, ATTS_PERMIT_WRITE, handle, pAttr->permissions) !=
            ATT_SUCCESS) {
            return;
        }
...
static void attsSignMsgCback(secCmacMsg_t *pMsg)
{
    ...
    if (pMsg->hdr.event == ATTS_MSG_SIGN_CMAC_CMPL) {
        ...
        /* if signature ok */
        if (memcmp(signature, pBuf->packet + pBuf->writeLen + ATT_WRITE_CMD_LEN + 4,
                   ATT_CMAC_RESULT_LEN) == 0) {
            /* check sign counter */
                ...
                if ((pAttr = attsFindByHandle(pBuf->handle, &pGroup)) != NULL) {
                    ...
                    if ((pAttr->settings & ATTS_SET_WRITE_CBACK) && (pGroup->writeCback != NULL)) {
                        ...
                        (*pGroup->writeCback)(pBuf->connId, pBuf->handle, ATT_PDU_SIGNED_WRITE_CMD,
                                              0, pBuf->writeLen, (pBuf->packet + ATT_WRITE_CMD_LEN),
                                              pAttr);
                    }
                    /* else check if CCC */
