...
static uint8_t attsExecPrepWrite(attsCcb_t *pCcb, attsPrepWrite_t *pPrep)
{
    ...
    else if ((pAttr->settings & ATTS_SET_WRITE_CBACK) && (pGroup->writeCback != NULL)) {
...
void attsProcWrite(attsCcb_t *pCcb, uint16_t len, uint8_t *pPacket)
{
    ...
    if ((pAttr = attsFindByHandle(handle, &pGroup)) != NULL) {
        /* verify permissions */
        if ((err = attsPermissions(pCcb->pMainCcb->connId, ATTS_PERMIT_WRITE, handle,
                                   pAttr->permissions)) != ATT_SUCCESS) {
            /* err has been set; fail */
        }
        /* verify write length, fixed length */
        else if (((pAttr->settings & ATTS_SET_VARIABLE_LEN) == 0) && (writeLen != pAttr->maxLen)) {
            err = ATT_ERR_LENGTH;
        }
...
void attsProcPrepWriteReq(attsCcb_t *pCcb, uint16_t len, uint8_t *pPacket)
{
    ...
    /* verify permissions */
    else if ((err = attsPermissions(pCcb->connId, ATTS_PERMIT_WRITE, handle, pAttr->permissions)) !=
             ATT_SUCCESS) {
        /* err has been set; fail */
    }
...
void attsProcExecWriteReq(attsCcb_t *pCcb, uint16_t len, uint8_t *pPacket)
{
    ...
    /* else writing all prepared writes */
        ...
        for (pPrep = attsCb.prepWriteQueue[pCcb->connId].pHead; pPrep != NULL;
             pPrep = pPrep->pNext) {
            /* find attribute */
            if ((pAttr = attsFindByHandle(pPrep->handle, &pGroup)) != NULL) {
                /* verify offset */
