...
/* Interface to ATT */
static const attFcnIf_t attsFcnIf = { attsDataCback, attsL2cCtrlCback,
                                      (attMsgHandler_t)attsMsgCback, attsConnCback };
...
void attsCheckPendDbHashReadRsp(void)
{
    for (uint8_t i = 0; i < DM_CONN_MAX; i++) {
        ...
        if (pCcb->pPendDbHashRsp) {
            ...
            if ((pBuf = attMsgAlloc(pCcb->sccb[ATT_BEARER_SLOT_ID].mtu + L2C_PAYLOAD_START)) !=
                NULL) {
                uint8_t *p;
                attsAttr_t *pAttr;
                attsGroup_t *pGroup;
                ...
                if ((pAttr = attsFindByHandle(pCcb->pPendDbHashRsp->handle, &pGroup)) != NULL) {
                    ...
                    attsErrRsp(pCcb, ATT_BEARER_SLOT_ID, ATT_PDU_READ_TYPE_REQ,
                               pCcb->pPendDbHashRsp->startHandle, ATT_ERR_NOT_FOUND);
                }
            } else {
                attsErrRsp(pCcb, ATT_BEARER_SLOT_ID, ATT_PDU_READ_TYPE_REQ,
                           pCcb->pPendDbHashRsp->startHandle, ATT_ERR_RESOURCES);
            }
...
void AttsCalculateDbHash(void)
{
    ...
    if ((pMsg = WsfBufAlloc(msgLen)) != NULL) {
        ...
        while (pGroup) {
            ...
            for (attsAttr_t *pAttr = pGroup->pAttr; attHandle <= pGroup->endHandle;
                 attHandle++, pAttr++) {
                uint16_t valLen;
                uint8_t uuidLen = 2;
