...
/* Interface to ATT */
static const attFcnIf_t attcFcnIf = { attcDataCback, attcCtrlCback, (attMsgHandler_t)attcMsgCback,
                                      attcConnCback };
...
static void attcSendWriteCmd(attcCcb_t *pCcb)
{
    ...
    if (!(pCcb->pMainCcb->sccb[pCcb->outReq.slot].control & ATT_CCB_STATUS_FLOW_DISABLED)) {
        /* call callback */
        attcExecCallback(pCcb->pMainCcb->connId, ATTC_WRITE_CMD_RSP, pCcb->outReq.handle,
                         ATT_SUCCESS);
    } else {
...
static void attcSendPrepWriteReq(attcCcb_t *pCcb)
{
    ...
    /* send packet to L2CAP */
    attL2cDataReq(pCcb->pMainCcb, pCcb->outReq.slot, dataLen + ATT_PREP_WRITE_REQ_LEN,
                  (uint8_t *)pPkt);
}
    ...
    if (pMsg->hdr.event <= ATTC_MSG_API_READ_MULT_VAR) {
       ...
       already for this handle */
        if (((pCcb->slot == ATT_BEARER_SLOT_ID) &&
            ...
            ((pMsg->hdr.event == ATTC_MSG_API_WRITE_CMD) && attcPendWriteCmd(pCcb, pMsg->handle))) {
