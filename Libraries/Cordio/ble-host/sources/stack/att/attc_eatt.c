...
/* Interface to ATT */
static const eattFcnIf_t attcFcnIf = { eattcL2cCocDataInd, eattcL2cCocDataCnf,
                                       (attMsgHandler_t)attcMsgCback, eattcConnCback };
                             ...
                             uint8_t *pValue, bool_t continuing)
{
    ...
    if ((pPkt = attMsgAlloc(ATT_FIND_TYPE_REQ_BUF_LEN + valueLen)) != NULL) {
        ...
        /* send message */
        eattcSendMsg(connId, priority, startHandle, ATTC_MSG_API_FIND_BY_TYPE_VALUE, pPkt,
                     continuing);
    }
}
                             ...
                             uint16_t endHandle, uint8_t uuidLen, uint8_t *pUuid, bool_t continuing)
{
    ...
    if ((pPkt = attMsgAlloc(ATT_READ_GROUP_TYPE_REQ_BUF_LEN + uuidLen)) != NULL) {
        ...
        /* send message */
        eattcSendMsg(connId, priority, startHandle, ATTC_MSG_API_READ_BY_GROUP_TYPE, pPkt,
                     continuing);
    }
}
