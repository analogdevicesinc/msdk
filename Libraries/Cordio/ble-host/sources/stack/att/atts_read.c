    ...
    if ((pAttr = attsFindByHandle(handle, &pGroup)) != NULL) {
        ...
        /* verify offset */
            ...
            if ((pAttr->settings & ATTS_SET_READ_CBACK) && (pGroup->readCback != NULL)) {
                err = (*pGroup->readCback)(pCcb->pMainCcb->connId, handle, ATT_PDU_READ_BLOB_REQ,
                                           offset, pAttr);
            }
            /* else check if CCC */
            ...
            if (err == ATT_SUCCESS) {
                ...
                readLen = ((*pAttr->pLen - offset) < (mtu - ATT_READ_BLOB_RSP_LEN)) ?
                              (*pAttr->pLen - offset) :
                              (mtu - ATT_READ_BLOB_RSP_LEN);

                /* Allocate response buffer */
                if ((pBuf = attMsgAlloc(L2C_PAYLOAD_START + ATT_READ_BLOB_RSP_LEN + readLen)) !=
                    NULL) {
                    /* build and send PDU */
                    ...
                    attL2cDataReq(pCcb->pMainCcb, pCcb->slot, (ATT_READ_BLOB_RSP_LEN + readLen),
                                  pBuf);
                }
            }
        }
...
void attsProcFindTypeReq(attsCcb_t *pCcb, uint16_t len, uint8_t *pPacket)
{
    ...
    if (!err) {
        ...
        if ((pBuf = attMsgAlloc(mtu + L2C_PAYLOAD_START)) != NULL) {
            ...
            handle = startHandle;
            while ((handle = attsFindUuidInRange(handle, endHandle, ATT_16_UUID_LEN, pUuid, &pAttr,
                                                 &pGroup)) != ATT_HANDLE_NONE) {
...
void attsProcReadTypeReq(attsCcb_t *pCcb, uint16_t len, uint8_t *pPacket)
{
    ...
    if (!err) {
        ...
        /* check permissions */
        else if ((err = attsPermissions(pCcb->pMainCcb->connId, ATTS_PERMIT_READ, handle,
                                        pAttr->permissions)) != ATT_SUCCESS) {
            /* err is set above */
        }
        /* check if read callback should be called */
        else if ((pAttr->settings & ATTS_SET_READ_CBACK) && (pGroup->readCback != NULL)) {
            err = (*pGroup->readCback)(pCcb->pMainCcb->connId, handle, ATT_PDU_READ_TYPE_REQ, 0,
                                       pAttr);
        }
        /* else check if CCC */
        else if ((pAttr->settings & ATTS_SET_CCC) && (attsCb.cccCback != NULL)) {
            err =
                (*attsCb.cccCback)(pCcb->pMainCcb->connId, ATT_METHOD_READ, handle, pAttr->pValue);
        }
       ...
       * re-calculated
       */
            if ((memcmp(pPacket, attGattDbhChUuid, ATT_16_UUID_LEN) == 0) &&
                ...
                if (pCcb->pMainCcb->pPendDbHashRsp) {
                    ...
                    attsErrRsp(pCcb->pMainCcb, pCcb->slot, ATT_PDU_READ_TYPE_REQ, startHandle,
                               ATT_ERR_RESOURCES);
                }
            ...
            if ((pBuf = attMsgAlloc(mtu + L2C_PAYLOAD_START)) != NULL) {
                ...
                attLen = (*pAttr->pLen < (mtu - ATT_READ_TYPE_RSP_LEN - sizeof(uint16_t))) ?
                             *pAttr->pLen :
                             (mtu - ATT_READ_TYPE_RSP_LEN - sizeof(uint16_t));
                ...
                handle++;
                while ((handle = attsFindUuidInRange(handle, endHandle, uuidLen, pPacket, &pAttr,
                                                     &pGroup)) != ATT_HANDLE_NONE) {
                    /* call read callback if desired */
                    if ((pAttr->settings & ATTS_SET_READ_CBACK) && (pGroup->readCback != NULL)) {
                        cbackErr = (*pGroup->readCback)(pCcb->pMainCcb->connId, handle,
                                                        ATT_PDU_READ_TYPE_REQ, 0, pAttr);
                    }
                    /* else check if CCC */
                    else if ((pAttr->settings & ATTS_SET_CCC) && (attsCb.cccCback != NULL)) {
                        cbackErr = (*attsCb.cccCback)(pCcb->pMainCcb->connId, ATT_METHOD_READ,
                                                      handle, pAttr->pValue);
                    }
           ...
           */
                    if ((cbackErr == ATT_SUCCESS) && (*pAttr->pLen == attLen) &&
                        (attsPermissions(pCcb->pMainCcb->connId, ATTS_PERMIT_READ, handle,
                                         pAttr->permissions) == ATT_SUCCESS)) {
                        /* copy result into response buffer; first check if it fits */
...
void attsProcReadMultReq(attsCcb_t *pCcb, uint16_t len, uint8_t *pPacket)
{
    ...
    if ((pBuf = attMsgAlloc(mtu + L2C_PAYLOAD_START)) != NULL) {
        ...
        while (pPacket < pEnd) {
            ...
            /* call read callback if desired */
            if ((pAttr->settings & ATTS_SET_READ_CBACK) && (pGroup->readCback != NULL)) {
                err = (*pGroup->readCback)(pCcb->pMainCcb->connId, handle, ATT_PDU_READ_MULT_REQ, 0,
                                           pAttr);
                if (err != ATT_SUCCESS) {
                    break;
                }
            ...
            if ((pBuf = attMsgAlloc(mtu + L2C_PAYLOAD_START)) != NULL) {
                ...
                attLen =
                    (*pAttr->pLen < (mtu - ATT_READ_GROUP_TYPE_RSP_LEN - (2 * sizeof(uint16_t)))) ?
                        *pAttr->pLen :
                        (mtu - ATT_READ_GROUP_TYPE_RSP_LEN - (2 * sizeof(uint16_t)));
                ...
                while (TRUE) {
                    ...
                    if ((handle = attsFindUuidInRange(handle, endHandle, uuidLen, pPacket, &pAttr,
                                                      &pGroup)) == ATT_HANDLE_NONE) {
                        break;
                    }
