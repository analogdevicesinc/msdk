...
const meshMsgOpcode_t
    mmdlGenPowerLevelSetupSrRcvdOpcodes[MMDL_GEN_POWER_LEVELSETUP_SR_NUM_RCVD_OPCODES] = {
...
const mmdlGenPowerLevelSetupSrHandleMsg_t
    mmdlGenPowerLevelSetupSrHandleMsg[MMDL_GEN_POWER_LEVELSETUP_SR_NUM_RCVD_OPCODES] = {
...
void MmdlGenPowerLevelSetupSrHandler(wsfMsgHdr_t *pMsg)
{
    ...
    if ((pMsg != NULL) && (pMsg->event == MESH_MODEL_EVT_MSG_RECV)) {
        ...
        if (MESH_OPCODE_SIZE(pModelMsg->opCode) == MMDL_GEN_POWER_LEVEL_OPCODES_SIZE) {
            ...
            for (opcodeIdx = 0; opcodeIdx < MMDL_GEN_POWER_LEVELSETUP_SR_NUM_RCVD_OPCODES;
                 opcodeIdx++) {
                if (!memcmp(&mmdlGenPowerLevelSetupSrRcvdOpcodes[opcodeIdx],
