 ...
 */
/*************************************************************************************************/
static void mmdlSchedulerSendMessage(meshElementId_t elementId, meshAddress_t serverAddr,
                        ...
                        uint16_t appKeyIndex)
{
    if (serverAddr != MMDL_USE_PUBLICATION_ADDR) {
        mmdlSchedulerSendMessage(elementId, serverAddr, ttl, appKeyIndex, NULL, 0,
                                 MMDL_SCHEDULER_GET_OPCODE);
    } else {
                                   ...
                                   const mmdlSchedulerRegisterEntry_t *pParam)
{
    ...
    if (serverAddr != MMDL_USE_PUBLICATION_ADDR) {
        mmdlSchedulerSendMessage(elementId, serverAddr, ttl, appKeyIndex, param,
                                 MMDL_SCHEDULER_ACTION_SET_NO_ACK_LEN,
