...
const mmdlLightHslSrHandleMsg_t mmdlLightHslSrHandleMsg[MMDL_LIGHT_HSL_SR_NUM_RCVD_OPCODES] = {
    mmdlLightHslSrHandleGet,       mmdlLightHslSrHandleSet,        mmdlLightHslSrHandleSetNoAck,
    mmdlLightHslSrHandleTargetGet, mmdlLightHslSrHandleDefaultGet, mmdlLightHslSrHandleRangeGet,
};
...

{
    ...
    if ((pDesc != NULL) && (pDesc->pStoredState != NULL)) {
        ...
        if (dstAddr != MMDL_USE_PUBLICATION_ADDR) {
            mmdlLightHslSrSendMessage(elementId, dstAddr, MESH_USE_DEFAULT_TTL, appKeyIndex,
                                      msgParams, MMDL_LIGHT_HSL_RANGE_STATUS_LEN,
                                      MMDL_LIGHT_HSL_RANGE_STATUS_OPCODE, recvOnUnicast);
        } else {
...

{
    ...
    if ((pDesc != NULL) && (pDesc->pStoredState != NULL)) {
        ...
        MMDL_TRACE_INFO3("LIGHT HSL SR: Send Default Ltness=%d Hue=0x%X, Sat=0x%X", defaultLtness,
                         pDesc->pStoredState->defaultHue, pDesc->pStoredState->defaultSat);

        if (dstAddr != MMDL_USE_PUBLICATION_ADDR) {
            mmdlLightHslSrSendMessage(elementId, dstAddr, MESH_USE_DEFAULT_TTL, appKeyIndex,
                                      msgParams, MMDL_LIGHT_HSL_DEF_STATUS_LEN,
                                      MMDL_LIGHT_HSL_DEFAULT_STATUS_OPCODE, recvOnUnicast);
        } else {
                                   ...
                                   mmdlStateUpdateSrc_t stateUpdateSrc)
{
    ...
    if ((pDesc != NULL) && (pDesc->pStoredState != NULL)) {
        MMDL_TRACE_INFO3("LIGHT HSL SR: Set Target Ltness=0x%X Hue=%d Sat=0x%X", pState->ltness,
                         pState->hue, pState->saturation);
        MMDL_TRACE_INFO2("LIGHT HSL SR: TimeRem=%d ms Delay=0x%X", transitionMs, delay5Ms);
    ...
    if ((pDesc != NULL) && (pDesc->pStoredState != NULL)) {
        ...
        if (dstAddr != MMDL_USE_PUBLICATION_ADDR) {
            ...
            mmdlLightHslSrSendMessage(elementId, dstAddr, MESH_USE_DEFAULT_TTL, appKeyIndex,
                                      ...
                                      MMDL_LIGHT_HSL_STATUS_OPCODE, recvOnUnicast);
        } else {
                                           ...
                                           uint16_t appKeyIndex, bool_t recvOnUnicast)
{
    ...
    if ((pDesc != NULL) && (pDesc->pStoredState != NULL)) {
        ...
        if (dstAddr != MMDL_USE_PUBLICATION_ADDR) {
            ...
            mmdlLightHslSrSendMessage(elementId, dstAddr, MESH_USE_DEFAULT_TTL, appKeyIndex,
                                      msgParams, (uint8_t)(pMsgParams - msgParams),
                                      MMDL_LIGHT_HSL_TARGET_STATUS_OPCODE, recvOnUnicast);
        } else {
...
void mmdlLightHslSrHandleGet(const meshModelMsgRecvEvt_t *pMsg)
{
    ...
    if (pMsg->messageParamsLen == 0) {
        ...
        mmdlLightHslSrSendStatus(pMsg->elementId, pMsg->srcAddr, pMsg->appKeyIndex,
                                 pMsg->recvOnUnicast);
    }
}
...
static bool_t mmdlLightHslSrProcessSet(const meshModelMsgRecvEvt_t *pMsg, bool_t ackRequired)
{
    ...
    if ((pDesc != NULL) && (pDesc->pStoredState != NULL)) {
        ...
        if (pDesc->steps > 0) {
            ...
            pDesc->transitionStep.hue =
                (state.hue - pDesc->pStoredState->present.hue) / pDesc->steps;
            pDesc->transitionStep.ltness =
                (state.ltness - pDesc->pStoredState->present.ltness) / pDesc->steps;
            pDesc->transitionStep.saturation =
                (state.saturation - pDesc->pStoredState->present.saturation) / pDesc->steps;
        }
...
static void mmdlLightHslSrHandleTmrCback(meshElementId_t elementId)
{
    ...
    if ((pDesc != NULL) && (pDesc->pStoredState != NULL)) {
        if (pDesc->delay5Ms != 0) {
            ...
            /* Timeout. Set state. */
            mmdlLightHslSrSetState(elementId, &pDesc->pStoredState->target, pDesc->remainingTimeMs,
            ...
            if (pDesc->steps > 0) {
                ...
                state.saturation =
                    pDesc->pStoredState->present.saturation + pDesc->transitionStep.saturation;
                                      ...
                                      uint32_t transitionMs)
{
    ...
    if ((pDesc != NULL) && (pDesc->pStoredState != NULL)) {
        ...
        /* Recall state */
        mmdlLightHslSrSetState(elementId, &(pDesc->pStoredState->hslScenes[sceneIdx]), transitionMs,
