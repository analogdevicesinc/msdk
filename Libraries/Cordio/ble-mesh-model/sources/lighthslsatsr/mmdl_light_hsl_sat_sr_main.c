...
const mmdlLightHslSatSrHandleMsg_t
    mmdlLightHslSatSrHandleMsg[MMDL_LIGHT_HSL_SAT_SR_NUM_RCVD_OPCODES] = {
        mmdlLightHslSatSrHandleGet, mmdlLightHslSatSrHandleSet, mmdlLightHslSatSrHandleSetNoAck
    };
                                             ...
                                             mmdlStateUpdateSrc_t stateUpdateSrc)
{
    ...
    if ((stateUpdateSrc != MMDL_STATE_UPDATED_BY_BIND) &&
        ...
        satCb.fResolveBind(elementId, MMDL_STATE_LT_HSL_SATURATION,
                           &pDesc->pStoredState->presentSat);
    }
                                      ...
                                      mmdlStateUpdateSrc_t stateUpdateSrc)
{
    ...
    if ((pDesc != NULL) && (pDesc->pStoredState != NULL)) {
        MMDL_TRACE_INFO3("LIGHT HSL SAT SR: Set TargetSat=0x%X TimeRem=%d ms, Delay=0x%X", sat,
                                        ...
                                        uint16_t appKeyIndex, bool_t recvOnUnicast)
{
    ...
    if ((pDesc != NULL) && (pDesc->pStoredState != NULL)) {
        ...
        if (dstAddr != MMDL_USE_PUBLICATION_ADDR) {
            MMDL_TRACE_INFO3(
                "LIGHT HSL SAT SR: Send Sat Status Present=0x%X Target=0x%X remTime=%d",
                pDesc->pStoredState->presentSat, pDesc->pStoredState->targetSat,
                pDesc->remainingTimeMs);

            mmdlLightHslSatSrSendMessage(elementId, dstAddr, MESH_USE_DEFAULT_TTL, appKeyIndex,
                                         msgParams, (uint8_t)(pMsgParams - msgParams),
                                         MMDL_LIGHT_HSL_SAT_STATUS_OPCODE, recvOnUnicast);
        } else {
...
void mmdlLightHslSatSrHandleGet(const meshModelMsgRecvEvt_t *pMsg)
{
    ...
    if (pMsg->messageParamsLen == 0) {
        ...
        mmdlLightHslSatSrSendStatus(pMsg->elementId, pMsg->srcAddr, pMsg->appKeyIndex,
                                    pMsg->recvOnUnicast);
    }
}
...
static void mmdlLightHslSatSrHandleTmrCback(meshElementId_t elementId)
{
    ...
    if ((pDesc != NULL) && (pDesc->pStoredState != NULL)) {
        if (pDesc->delay5Ms != 0) {
            ...
            /* Timeout. Set state. */
            mmdlLightHslSatSrSetState(elementId, pDesc->pStoredState->targetSat,
