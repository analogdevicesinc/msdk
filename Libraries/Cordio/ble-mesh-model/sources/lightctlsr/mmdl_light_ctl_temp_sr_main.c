...
const meshMsgOpcode_t
    mmdlLightCtlTemperatureSrRcvdOpcodes[MMDL_LIGHT_CTL_TEMP_SR_NUM_RCVD_OPCODES] = {
...
const mmdlLightCtlTemperatureSrHandleMsg_t
    mmdlLightCtlTemperatureSrHandleMsg[MMDL_LIGHT_CTL_TEMP_SR_NUM_RCVD_OPCODES] = {
        mmdlLightCtlTemperatureSrHandleGet, mmdlLightCtlTemperatureSrHandleSet,
 ...
 */
/*************************************************************************************************/
static void mmdlLightCtlTempSrGetDesc(meshElementId_t elementId,
                                      mmdlLightCtlTempSrDesc_t **ppOutDesc)
{
    uint8_t modelIdx;
                                              ...
                                              mmdlStateUpdateSrc_t stateUpdateSrc)
{
    ...
    if ((stateUpdateSrc != MMDL_STATE_UPDATED_BY_BIND) &&
        ...
        temperatureCb.fResolveBind(elementId, MMDL_STATE_LT_CTL_TEMP,
                                         ...
                                         uint16_t appKeyIndex, bool_t recvOnUnicast)
{
    ...
    if ((pDesc != NULL) && (pDesc->pStoredState != NULL)) {
        ...
        if (dstAddr != MMDL_USE_PUBLICATION_ADDR) {
            MMDL_TRACE_INFO3(
                "LIGHT CTL TEMP SR: Send Temperature Status Present=0x%X Target=0x%X remTime=%d",
                pDesc->pStoredState->present.temperature, pDesc->pStoredState->target.temperature,
                pDesc->remainingTimeMs);

            mmdlLightCtlTempSrSendMessage(elementId, dstAddr, MESH_USE_DEFAULT_TTL, appKeyIndex,
                                          msgParams, (uint8_t)(pMsgParams - msgParams),
                                          MMDL_LIGHT_CTL_TEMP_STATUS_OPCODE, recvOnUnicast);
        } else {
            MMDL_TRACE_INFO3(
                "LIGHT CTL TEMP SR: Publish Temperature Present=0x%X Target=0x%X remTime=%d",
                pDesc->pStoredState->present.temperature, pDesc->pStoredState->target.temperature,
                pDesc->remainingTimeMs);

            mmdlLightCtlTempSrPublishMessage(elementId, msgParams,
...
void mmdlLightCtlTemperatureSrHandleGet(const meshModelMsgRecvEvt_t *pMsg)
{
    ...
    if (pMsg->messageParamsLen == 0) {
        ...
        mmdlLightCtlTempSrSendStatus(pMsg->elementId, pMsg->srcAddr, pMsg->appKeyIndex,
                                     pMsg->recvOnUnicast);
    }
}
...
static bool_t mmdlLightCtlTempSrProcessSet(const meshModelMsgRecvEvt_t *pMsg, bool_t ackRequired)
{
    ...
    if ((pDesc != NULL) && (pDesc->pStoredState != NULL)) {
        ...
        mmdlLightCtlTempSrSetState(pMsg->elementId, &state, transMs, delayMs,
        ...
        if ((pCtlDesc != NULL) && (pCtlDesc->pStoredState != NULL)) {
            ...
            level = (temperature - pCtlDesc->pStoredState->minTemperature) * 65535 /
                        (pCtlDesc->pStoredState->maxTemperature -
                         pCtlDesc->pStoredState->minTemperature) -
                    32768;
        ...
        case MESH_MODEL_EVT_MSG_RECV:
            ...
            for (opcodeIdx = 0; opcodeIdx < MMDL_LIGHT_CTL_TEMP_SR_NUM_RCVD_OPCODES; opcodeIdx++) {
                ...
                if (!memcmp(&mmdlLightCtlTemperatureSrRcvdOpcodes[opcodeIdx],
                    ...
                    /* Process message */
                    (void)mmdlLightCtlTemperatureSrHandleMsg[opcodeIdx](
