...
const mmdlLightCtlSrHandleMsg_t mmdlLightCtlSrHandleMsg[MMDL_LIGHT_CTL_SR_NUM_RCVD_OPCODES] = {
    mmdlLightCtlSrHandleGet,        mmdlLightCtlSrHandleSet,      mmdlLightCtlSrHandleSetNoAck,
    mmdlLightCtlSrHandleDefaultGet, mmdlLightCtlSrHandleRangeGet,
};
...

{
    ...
    if ((pDesc != NULL) && (pDesc->pStoredState != NULL)) {
        ...
        MMDL_TRACE_INFO3(
            "LIGHT CTL SR: Send Range Status=%d MinTemperature=0x%X, MaxTemperature=0x%X", opStatus,
            pDesc->pStoredState->minTemperature, pDesc->pStoredState->maxTemperature);

        if (dstAddr != MMDL_USE_PUBLICATION_ADDR) {
            mmdlLightCtlSrSendMessage(elementId, dstAddr, MESH_USE_DEFAULT_TTL, appKeyIndex,
                                      msgParams, MMDL_LIGHT_CTL_TEMP_RANGE_STATUS_LEN,
                                      MMDL_LIGHT_CTL_RANGE_STATUS_OPCODE, recvOnUnicast);
        } else {
...

{
    ...
    if ((pDesc != NULL) && (pDesc->pStoredState != NULL)) {
        ...
        if (dstAddr != MMDL_USE_PUBLICATION_ADDR) {
            mmdlLightCtlSrSendMessage(elementId, dstAddr, MESH_USE_DEFAULT_TTL, appKeyIndex,
                                      msgParams, MMDL_LIGHT_CTL_DEFAULT_STATUS_LEN,
 ...
 */
/*************************************************************************************************/
static void mmdlLightCtlSrSetPresentState(meshElementId_t elementId, mmdlLightCtlSrDesc_t *pDesc,
    ...
    if ((pDesc != NULL) && (pDesc->pStoredState != NULL)) {
        ...
        if (dstAddr != MMDL_USE_PUBLICATION_ADDR) {
            MMDL_TRACE_INFO2("LIGHT CTL SR: Send Status Ltness=0x%X Temp=0x%X",
                             pDesc->pStoredState->present.ltness,
            ...
            mmdlLightCtlSrSendMessage(elementId, dstAddr, MESH_USE_DEFAULT_TTL, appKeyIndex,
                                      ...
                                      MMDL_LIGHT_CTL_STATUS_OPCODE, recvOnUnicast);
        } else {
...
void mmdlLightCtlSrHandleGet(const meshModelMsgRecvEvt_t *pMsg)
{
    ...
    if (pMsg->messageParamsLen == 0) {
        ...
        mmdlLightCtlSrSendStatus(pMsg->elementId, pMsg->srcAddr, pMsg->appKeyIndex,
                                 pMsg->recvOnUnicast);
    }
}
...
static bool_t mmdlLightCtlSrProcessSet(const meshModelMsgRecvEvt_t *pMsg, bool_t ackRequired)
{
    ...
    if ((pDesc != NULL) && (pDesc->pStoredState != NULL)) {
        ...
        if (pDesc->steps > 0) {
            ...
            pDesc->transitionStep.ltness =
                ((int32_t)state.ltness - pPresent->ltness) / pDesc->steps;
            pDesc->transitionStep.temperature =
                ((int32_t)state.temperature - pPresent->temperature) / pDesc->steps;
            pDesc->transitionStep.deltaUV =
                ((int32_t)state.deltaUV - pPresent->deltaUV) / pDesc->steps;
        }
...
static void mmdlLightCtlSrHandleTmrCback(meshElementId_t elementId)
{
    ...
    if ((pDesc != NULL) && (pDesc->pStoredState != NULL)) {
        if (pDesc->delay5Ms != 0) {
            ...
            /* Timeout. Set state. */
            mmdlLightCtlSrSetState(elementId, &pDesc->pStoredState->target, pDesc->remainingTimeMs,
            ...
            if (pDesc->steps > 0) {
                ...
                nextState.ltness =
                    pDesc->pStoredState->present.ltness + pDesc->transitionStep.ltness;
                nextState.temperature =
                    pDesc->pStoredState->present.temperature + pDesc->transitionStep.temperature;
                nextState.deltaUV =
                    pDesc->pStoredState->present.deltaUV + pDesc->transitionStep.deltaUV;
                                      ...
                                      uint32_t transitionMs)
{
    ...
    if ((pDesc != NULL) && (pDesc->pStoredState != NULL)) {
        ...
        /* Recall state */
        mmdlLightCtlSrSetState(elementId, &(pDesc->pStoredState->ctlScenes[sceneIdx]), transitionMs,
                               0, MMDL_STATE_UPDATED_BY_SCENE);
    }
}
                                ...
                                mmdlLightCtlTempSrState_t *pTargetState)
{
    ...
    MMDL_TRACE_INFO2("LIGHT CTL SR: Set bound Temp=0x%X Delta=0x%X", pState->temperature,
                     pState->deltaUV);
...
static void mmdlBindResolveOnPowerUp2LightCtl(meshElementId_t tgtElementId, void *pStateValue)
{
    ...
    if ((pDesc != NULL) && (pDesc->pStoredState != NULL)) {
        ...
        case MMDL_GEN_ONPOWERUP_STATE_RESTORE:
            ...
            if (pDesc->pStoredState->target.temperature !=
                pDesc->pStoredState->present.temperature) {
...
static void mmdlBindResolveGenLevel2LightCtl(meshElementId_t tgtElementId, void *pStateValue)
{
    ...
    if ((pDesc != NULL) && (pDesc->pStoredState != NULL)) {
        ...
        temperature =
            pDesc->pStoredState->minTemperature +
            (level + 32768) *
                (pDesc->pStoredState->maxTemperature - pDesc->pStoredState->minTemperature) / 65535;
