...
const mmdlGenLevelSrHandleMsg_t mmdlGenLevelSrHandleMsg[MMDL_GEN_LEVEL_SR_NUM_RCVD_OPCODES] = {
    mmdlGenLevelSrHandleGet,           mmdlGenLevelSrHandleSet,
    mmdlGenLevelSrHandleSetNoAck,      mmdlGenLevelSrHandleDeltaSet,
    mmdlGenLevelSrHandleDeltaSetNoAck, mmdlGenLevelSrHandleMoveSet,
                                   ...
                                   mmdlStateUpdateSrc_t stateUpdateSrc)
{
    ...
    MMDL_TRACE_INFO3("GEN LEVEL SR: Set Target=%d, TimeRem=%d ms, Delay=0x%X", targetState,
...
void mmdlGenLevelSrHandleGet(const meshModelMsgRecvEvt_t *pMsg)
{
    ...
    if (pMsg->messageParamsLen == 0) {
        ...
        mmdlGenLevelSrSendStatus(pMsg->elementId, pMsg->srcAddr, pMsg->appKeyIndex,
                                 pMsg->recvOnUnicast);
    }
}
...
static bool_t mmdlGenLevelSrProcessSet(const meshModelMsgRecvEvt_t *pMsg, bool_t ackRequired)
{
    ...
    if (pMsg->messageParamsLen == MMDL_GEN_LEVEL_SET_MAX_LEN) {
        ...
        /* Get Transition time */
        transMs =
            MmdlGenDefaultTransTimeToMs(pMsg->pMessageParams[MMDL_GEN_LEVEL_SET_TRANSITION_IDX]);
...
static bool_t mmdlGenLevelSrProcessMoveSet(const meshModelMsgRecvEvt_t *pMsg, bool_t ackRequired)
{
     ...
     * Time is set to 0. */
        if ((TRANSITION_TIME_STEPS(pMsg->pMessageParams[MMDL_GEN_LEVEL_SET_TRANSITION_IDX]) ==
             MMDL_GEN_TR_UNKNOWN) ||
            (pMsg->pMessageParams[MMDL_GEN_LEVEL_SET_TRANSITION_IDX] == 0)) {
            return FALSE;
        }
    }
    ...
    if (pDesc != NULL) {
        ...
        if (pMsg->messageParamsLen == MMDL_GEN_LEVEL_SET_MAX_LEN) {
            ...
            transitionTimeMs = MmdlGenDefaultTransTimeToMs(
                pMsg->pMessageParams[MMDL_GEN_LEVEL_SET_TRANSITION_IDX]);
            delayMs = pMsg->pMessageParams[MMDL_GEN_LEVEL_SET_DELAY_IDX];
        } else {
...
static bool_t mmdlGenLevelSrProcessDeltaSet(const meshModelMsgRecvEvt_t *pMsg, bool_t ackRequired)
{
     ...
     */
        ...
        if (pMsg->messageParamsLen == MMDL_GEN_LEVEL_DELTA_SET_MAX_LEN) {
            /* Get Transition time */
            transMs = MmdlGenDefaultTransTimeToMs(
                pMsg->pMessageParams[MMDL_GEN_LEVEL_DELTA_SET_TRANSITION_IDX]);
        ...
        if (pDesc->steps > 0) {
            /* Compute the transition step increment */
            pDesc->transitionStep =
                (targetState - pDesc->pStoredStates[PRESENT_STATE_IDX]) / pDesc->steps;
        }
...
static void mmdlGenLevelSrHandleTmrCback(uint8_t elementId)
{
    ...
    if (pDesc != NULL) {
         ...
         */
                ...
                if ((pDesc->isMoveSet) && (pDesc->pStoredStates[TARGET_STATE_IDX] !=
                                           pDesc->pStoredStates[PRESENT_STATE_IDX])) {
                    mmdlGenLevelSrSetState(elementId, pDesc->pStoredStates[TARGET_STATE_IDX],
                                           remainingTimeMs, 0, pDesc->updateSource);
                }
...
static void mmdlGenLevelSrStoreScene(void *pDesc, uint8_t sceneIdx)
{
    ...
    MMDL_TRACE_INFO1("GEN LEVEL SR: Store Level=%d",
                     pGenLevelDesc->pStoredStates[PRESENT_STATE_IDX]);

    /* Store present state */
    pGenLevelDesc->pStoredStates[SCENE_STATE_IDX + sceneIdx] =
        pGenLevelDesc->pStoredStates[PRESENT_STATE_IDX];
}
 ...
 */
/*************************************************************************************************/
static void mmdlGenLevelSrRecallScene(meshElementId_t elementId, uint8_t sceneIdx,
                                      uint32_t transitionMs)
{
    ...
    if (pDesc != NULL) {
        MMDL_TRACE_INFO3("GEN LEVEL SR: Recall elemid=%d onoff=%d transMs=%d", elementId,
                         pDesc->pStoredStates[SCENE_STATE_IDX + sceneIdx], transitionMs);

        /* Recall state */
        mmdlGenLevelSrSetState(elementId, pDesc->pStoredStates[SCENE_STATE_IDX + sceneIdx],
            ...
            if (MESH_OPCODE_SIZE(pModelMsg->msgRecvEvt.opCode) == MMDL_GEN_LEVEL_OPCODES_SIZE) {
                ...
                for (opcodeIdx = 0; opcodeIdx < MMDL_GEN_LEVEL_SR_NUM_RCVD_OPCODES; opcodeIdx++) {
                    if (!memcmp(&mmdlGenLevelSrRcvdOpcodes[opcodeIdx],
                        ...
                        /* Process message */
                        (void)mmdlGenLevelSrHandleMsg[opcodeIdx](
...
void MmdlGenLevelSrPublish(meshElementId_t elementId)
{
    meshPubMsgInfo_t pubMsgInfo =
        MESH_PUB_MSG_INFO(MMDL_GEN_LEVEL_SR_MDL_ID, MMDL_GEN_LEVEL_STATUS_OPCODE);
    uint8_t msgParams[MMDL_GEN_LEVEL_STATUS_MAX_LEN];
    mmdlGenLevelSrDesc_t *pDesc = NULL;
