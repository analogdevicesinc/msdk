...
const mmdlGenPowerLevelSrHandleMsg_t
    mmdlGenPowerLevelSrHandleMsg[MMDL_GEN_POWER_LEVEL_SR_NUM_RCVD_OPCODES] = {
        mmdlGenPowerLevelSrHandleGet,      mmdlGenPowerLevelSrHandleSet,
        mmdlGenPowerLevelSrHandleSetNoAck, mmdlGenPowerLastSrHandleGet,
        mmdlGenPowerDefaultSrHandleGet,    mmdlGenPowerRangeSrHandleGet,
    };
 ...
 */
/*************************************************************************************************/
static void mmdlGenPowerLevelSrGetDesc(meshElementId_t elementId,
                                       mmdlGenPowerLevelSrDesc_t **ppOutDesc)
{
    uint8_t modelIdx;
 ...
 */
/*************************************************************************************************/
static void mmdlGenPowerLevelSrSetState(meshElementId_t elementId,
                                        mmdlGenPowerLevelState_t targetState, uint32_t transitionMs,
                                        uint8_t delay5Ms, mmdlStateUpdateSrc_t stateUpdateSrc)
{
    mmdlGenPowerLevelSrDesc_t *pDesc = NULL;
    bool_t saveToNVM = FALSE;

    MMDL_TRACE_INFO3("GEN POWER LEVEL SR: Set Target=0x%X, TimeRem=%d ms, Delay=0x%X", targetState,
    ...
    if (pDesc != NULL) {
        ...
        /* Check if state will change after a transition or immediately */
            ...
            mmdlGenPowerLevelSrSetPresentState(elementId, pDesc, targetState, transitionMs,
                                               delay5Ms, stateUpdateSrc);
        }
                                          ...
                                          uint16_t appKeyIndex, bool_t recvOnUnicast)
{
    meshMsgInfo_t msgInfo =
        MESH_MSG_INFO(MMDL_GEN_POWER_LEVEL_SR_MDL_ID, MMDL_GEN_POWER_LEVEL_STATUS_OPCODE);
    ...
    if ((pDesc != NULL) && (pDesc->pStoredStates != NULL)) {
        ...
        if (pDesc->remainingTimeMs != 0) {
            ...
            MMDL_TRACE_INFO3(
                "GEN POWER LEVEL SR: Send Status Present=0x%X, Target=0x%X, TimeRem=0x%X",
...
{
    meshMsgInfo_t msgInfo =
        MESH_MSG_INFO(MMDL_GEN_POWER_LEVEL_SR_MDL_ID, MMDL_GEN_POWERLAST_STATUS_OPCODE);
    mmdlGenPowerLevelSrDesc_t *pDesc = NULL;
...
{
    meshMsgInfo_t msgInfo =
        MESH_MSG_INFO(MMDL_GEN_POWER_LEVEL_SR_MDL_ID, MMDL_GEN_POWERDEFAULT_STATUS_OPCODE);
    mmdlGenPowerLevelSrDesc_t *pDesc = NULL;
...
{
    meshMsgInfo_t msgInfo =
        MESH_MSG_INFO(MMDL_GEN_POWER_LEVEL_SR_MDL_ID, MMDL_GEN_POWERRANGE_STATUS_OPCODE);
...
static bool_t mmdlGenPowerLevelSrProcessSet(const meshModelMsgRecvEvt_t *pMsg, bool_t ackRequired)
{
    ...
    if ((pDesc != NULL) && (pDesc->pStoredStates != NULL)) {
        ...
        if (pDesc->steps > 0) {
            ...
            pDesc->transitionStep =
                (state - pDesc->pStoredStates[PRESENT_STATE_IDX]) / pDesc->steps;
        }

        /* Change state */
        mmdlGenPowerLevelSrSetState(pMsg->elementId, state, transMs, delayMs,
...
void mmdlGenPowerLevelSrHandleGet(const meshModelMsgRecvEvt_t *pMsg)
{
    ...
    if (pMsg->messageParamsLen == 0) {
        ...
        mmdlGenPowerLevelSrSendStatus(pMsg->elementId, pMsg->srcAddr, pMsg->appKeyIndex,
                                      pMsg->recvOnUnicast);
    }
}
...
void mmdlGenPowerLastSrHandleGet(const meshModelMsgRecvEvt_t *pMsg)
{
    ...
    if (pMsg->messageParamsLen == 0) {
        /* Send Status message as a response to the Get message */
        mmdlGenPowerLastSrSendStatus(pMsg->elementId, pMsg->srcAddr, pMsg->appKeyIndex,
                                     pMsg->recvOnUnicast);
    }
}
...
void mmdlGenPowerDefaultSrHandleGet(const meshModelMsgRecvEvt_t *pMsg)
{
    ...
    if (pMsg->messageParamsLen == 0) {
        /* Send Status message as a response to the Get message */
        mmdlGenPowerDefaultSrSendStatus(pMsg->elementId, pMsg->srcAddr, pMsg->appKeyIndex,
                                        pMsg->recvOnUnicast);
    }
}
 ...
 */
/*************************************************************************************************/
static void mmdlGenPowerRangeSrSetState(meshElementId_t elementId,
                                        mmdlGenPowerLevelState_t rangeMin,
                                           ...
                                           uint32_t transitionMs)
{
    ...
    if ((pDesc != NULL) && (pDesc->pStoredStates != NULL)) {
        MMDL_TRACE_INFO3("GEN POWER LEVEL SR: Recall elemid=%d powerlevel=%d transMs=%d", elementId,
    ...
    if (pMsg != NULL) {
        ...
        case MESH_MODEL_EVT_MSG_RECV:
            ...
            if (MESH_OPCODE_SIZE(pModelMsg->msgRecvEvt.opCode) ==
                ...
                for (opcodeIdx = 0; opcodeIdx < MMDL_GEN_POWER_LEVEL_SR_NUM_RCVD_OPCODES;
                     opcodeIdx++) {
                    if (!memcmp(&mmdlGenPowerLevelSrRcvdOpcodes[opcodeIdx],
                                pModelMsg->msgRecvEvt.opCode.opcodeBytes,
                                MMDL_GEN_POWER_LEVEL_OPCODES_SIZE)) {
                        /* Process message */
                        (void)mmdlGenPowerLevelSrHandleMsg[opcodeIdx](
...
void MmdlGenPowerLevelSrPublish(meshElementId_t elementId)
{
    meshPubMsgInfo_t pubMsgInfo =
        MESH_PUB_MSG_INFO(MMDL_GEN_POWER_LEVEL_SR_MDL_ID, MMDL_GEN_POWER_LEVEL_STATUS_OPCODE);
    uint8_t msgParams[MMDL_GEN_POWER_LEVEL_STATUS_MAX_LEN];
    uint8_t *pMsgParams, tranTime;
    mmdlGenPowerLevelSrDesc_t *pDesc = NULL;
