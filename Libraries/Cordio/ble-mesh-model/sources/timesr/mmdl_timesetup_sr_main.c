...
const mmdlTimeSetupSrHandleMsg_t mmdlTimeSetupSrHandleMsg[MMDL_TIME_SETUP_SR_NUM_RCVD_OPCODES] = {
    mmdlTimeSetupSrHandleSet,     mmdlTimeSetupSrHandleZoneSet, mmdlTimeSetupSrHandleDeltaSet,
    mmdlTimeSetupSrHandleRoleGet, mmdlTimeSetupSrHandleRoleSet,
};
                                    ...
                                    mmdlStateUpdateSrc_t stateUpdateSrc)
{
    ...
    MMDL_TRACE_INFO3("TIME SETUP SR: Set taiSeconds=0x%x subsecond=0x%x uncertainty=0x%x",
                     pTargetState->taiSeconds, pTargetState->subSecond, pTargetState->uncertainty);
    MMDL_TRACE_INFO3("TIME SETUP SR: Set timeauthority=%d delta=0x%x timezoneoffset=0x%x",
                     pTargetState->timeAuthority, pTargetState->taiUtcDelta,
                     pTargetState->timeZoneOffset);
                                         ...
                                         mmdlStateUpdateSrc_t stateUpdateSrc)
{
    ...
    MMDL_TRACE_INFO2("TIME SETUP DELTA SR: Set new=0x%X, change0x%X,", pTargetState->deltaNew,
                     pTargetState->deltaChange);
...
void mmdlTimeSetupSrHandleSet(const meshModelMsgRecvEvt_t *pMsg)
{
    ...
    if (mmdlTimeSetupSrProcessSet(pMsg, TRUE)) {
        ...
        mmdlTimeSrSendStatus(pMsg->elementId, pMsg->srcAddr, pMsg->appKeyIndex,
                             pMsg->recvOnUnicast);
    }
}
...
void mmdlTimeSetupSrHandleZoneSet(const meshModelMsgRecvEvt_t *pMsg)
{
    ...
    if (mmdlTimeSetupSrProcessZoneSet(pMsg, TRUE)) {
        /* Send Status message as a response to the Set message */
        mmdlTimeSetupSrSendZoneStatus(pMsg->elementId, pMsg->srcAddr, pMsg->appKeyIndex,
                                      pMsg->recvOnUnicast);
    }
}
...
void mmdlTimeSetupSrHandleDeltaSet(const meshModelMsgRecvEvt_t *pMsg)
{
    ...
    if (mmdlTimeSetupSrProcessDeltaSet(pMsg, TRUE)) {
        /* Send Status message as a response to the Set message */
        mmdlTimeSetupSrSendDeltaStatus(pMsg->elementId, pMsg->srcAddr, pMsg->appKeyIndex,
                                       pMsg->recvOnUnicast);
    }
}
...
void mmdlTimeSetupSrHandleRoleGet(const meshModelMsgRecvEvt_t *pMsg)
{
    ...
    if (pMsg->messageParamsLen == 0) {
        ...
        mmdlTimeSetupSrSendRoleStatus(pMsg->elementId, pMsg->srcAddr, pMsg->appKeyIndex,
                                      pMsg->recvOnUnicast);
    }
}
