...
static bool_t appSlaveConnectableAdv(uint8_t numSets, uint8_t *pAdvHandles)
{
    ...
    for (i = 0; i < numSets; i++) {
        ...
        for (j = 0; j < DM_NUM_ADV_SETS; j++) {
            ...
            if ((pAdvHandles[i] == j) && ((appSlaveCb.advType[j] == DM_ADV_CONN_UNDIRECT) ||
...
static void appSetAdvScanDataFrag(uint8_t advHandle, uint8_t location)
{
    ...
    remainLen =
        appSlaveCb.advDataLen[advHandle][location] - appSlaveCb.advDataOffset[advHandle][location];
...
static void appSetAdvScanData(uint8_t advHandle, uint8_t mode)
{
    ...
    if ((appSlaveCb.advDataOffset[advHandle][advLoc] >= appSlaveCb.advDataLen[advHandle][advLoc]) &&
        (appSlaveCb.advDataOffset[advHandle][scanLoc] >=
         appSlaveCb.advDataLen[advHandle][scanLoc])) {
...
void appSlaveSecConnOpen(dmEvt_t *pMsg, appConnCb_t *pCb)
{
    ...
    if ((pCb->dbHdl == NULL) &&
        DM_RAND_ADDR_RPA(pMsg->connOpen.peerAddr, pMsg->connOpen.addrType)) {
...
static void appSecPairInd(dmEvt_t *pMsg, appConnCb_t *pCb)
{
   ...
   */
    if (appSlaveCb.bondable || ((pMsg->pairInd.auth & DM_AUTH_BOND_FLAG) != DM_AUTH_BOND_FLAG) ||
        ...
        pCb->bondByPairing = (pMsg->pairInd.auth & pAppSecCfg->auth & DM_AUTH_BOND_FLAG) ==
                             DM_AUTH_BOND_FLAG;
        ...
        if (pCb->bondByPairing && pCb->dbHdl == APP_DB_HDL_NONE) {
            ...
            pCb->dbHdl =
                AppDbNewRecord(DmConnPeerAddrType(pCb->connId), DmConnPeerAddr(pCb->connId));
        }
 ...
 */
    ...
    void AppSlaveProcDmMsg(dmEvt_t * pMsg)
    {
        ...
        /* look up app connection control block from DM connection ID */
        if ((pMsg->hdr.event != DM_ADV_STOP_IND) && (pMsg->hdr.event != DM_ADV_SET_STOP_IND)) {
 ...
 */
    /*************************************************************************************************/
    void appAdvSetData(uint8_t advHandle, uint8_t location, uint16_t len, uint8_t * pData,
                       uint16_t bufLen, uint16_t maxLen)
    {
        ...
        if ((appSlaveCb.advState[advHandle] != APP_ADV_STOPPED) &&
            (APP_LOC_2_MODE(location) == appSlaveCb.discMode) && (len <= HCI_EXT_ADV_DATA_LEN) &&
            (len <= maxLen)) {
 ...
 */
    /*************************************************************************************************/
    void appAdvStart(uint8_t numSets, uint8_t * pAdvHandles, uint16_t * pInterval,
                     uint16_t * pDuration, uint8_t * pMaxEaEvents, bool_t cfgAdvParam)
    {
        uint8_t i;
            ...
            for (i = 0; i < numSets; i++) {
                ...
                if (cfgAdvParam) {
                    ...
                    DmAdvConfig(pAdvHandles[i], appSlaveCb.advType[pAdvHandles[i]],
                                appSlaveCb.peerAddrType[pAdvHandles[i]],
                                appSlaveCb.peerAddr[pAdvHandles[i]]);
                }
 ...
 */
    /*************************************************************************************************/
    void appSlaveAdvStart(uint8_t numSets, uint8_t * pAdvHandles, uint16_t * pInterval,
                          uint16_t * pDuration, uint8_t * pMaxEaEvents, bool_t cfgAdvParam,
                          uint8_t mode)
    {
        uint8_t i;
                            ...
                            uint8_t * pValue)
    {
        ...
        if (pAdvData != NULL) {
            ...
            if (adType == DM_ADV_TYPE_LOCAL_NAME) {
                ...
                valueSet =
                    DmAdvSetAdValue(adType, len, pValue, &advDataLen, pAdvData, advDataBufLen);
            }
 ...
 */
    /*************************************************************************************************/
    dmConnId_t appConnAccept(uint8_t advHandle, uint8_t advType, uint16_t interval,
                             uint16_t duration, uint8_t maxEaEvents, uint8_t addrType,
                             uint8_t * pAddr, appDbHdl_t dbHdl, bool_t cfgAdvData)
    {
         ...
         */
                ...
                if (cfgAdvData && !appSlaveCb.advDataSynced[advHandle]) {
                    ...
                    if (appSlaveCb
                            .advDataOffset[advHandle][APP_MODE_2_ADV_LOC(APP_MODE_CONNECTABLE)] >=
                        appSlaveCb.maxAdvDataLen[advHandle]) {
                        appSlaveCb.advDataSynced[advHandle] = TRUE;
                    }
                }
                ...
                /* accept connection */
                return DmConnAccept(DM_CLIENT_ID_APP, advHandle, advType, duration, maxEaEvents,
                                    addrType, pAddr);
            }
        }
