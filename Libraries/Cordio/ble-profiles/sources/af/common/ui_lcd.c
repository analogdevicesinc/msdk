...
UiKeyPress_t uiLcdProcessKeyTbl[] = { uiLcdProcessSplashKey, uiLcdProcessMenuKey,
                                      uiLcdProcessDialogKey };

/* LCD display action functions */
static UiActionTbl_t uiLcdActionTbl = { uiLcdDispSplash, uiLcdDispMenu, uiLcdDispDialog,
                                        uiLcdProcessKey };
...
static void uiLcdScrollCallback(void)
{
    if (UiCb.activeScreenType == UI_SCREEN_MENU) {
        ...
        if (++uiLcdCb.scrollOffset > (int8_t)(strlen(pMenu->pItems[pMenu->highlight]) -
                                              (LCD_LINE_LEN - LCD_SEL_COL_WIDTH))) {
            uiLcdCb.scrollOffset = 0;
        }
        ...
        if (++uiLcdCb.scrollOffset > (int8_t)(strlen(pDialog->pSelectItems[pDialog->highlight]) -
                                              (LCD_LINE_LEN - LCD_SEL_COL_WIDTH))) {
            uiLcdCb.scrollOffset = 0;
        }
...
static void uiLcdDispMenu(const UiMenu_t *pMenu)
{
    ...
    for (i = start; i < end; i++) {
        ...
        strncpy(line + LCD_SEL_COL_WIDTH, pMenu->pItems[i] + offset,
                LCD_LINE_LEN - LCD_SEL_COL_WIDTH);
...
static void uiLcdDispDialogSelect(const UiDialog_t *pDialog)
{
    ...
    /* Calculate first and last item displayed on the screen */
    appCalcFirstAndLastItem(&start, &end, LCD_NUM_LINES - 2, pDialog->highlight,
                            pDialog->numSelectItems);
    ...
    for (i = start; i < end; i++) {
        ...
        strncpy(line + LCD_SEL_COL_WIDTH, pDialog->pSelectItems[i] + offset,
                LCD_LINE_LEN - LCD_SEL_COL_WIDTH);
        UiLcdWriteLine(pos++, line);
    }
