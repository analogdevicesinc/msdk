...
enum {
    APP_CONN_UPDATE_TIMEOUT_IND =
        APP_SLAVE_MSG_START /*! Connection parameter update timer expired */
};
...
typedef struct {
    ...
    uint16_t advDataBufLen
        [DM_NUM_ADV_SETS]
        [APP_NUM_DATA_LOCATIONS]; /*! Length of advertising data buffer maintained by Application */
    uint16_t advDataOffset[DM_NUM_ADV_SETS][APP_NUM_DATA_LOCATIONS]; /*! Advertising data offsets */
    uint16_t
        maxAdvDataLen[DM_NUM_ADV_SETS]; /*! Maximum advertising data length supported by Controller */
...
extern void appAdvStart(uint8_t numSets, uint8_t *pAdvHandles, uint16_t *pInterval,
                        uint16_t *pDuration, uint8_t *pMaxEaEvents, bool_t cfgAdvParam);
extern void appAdvSetData(uint8_t advHandle, uint8_t location, uint16_t len, uint8_t *pData,
                          uint16_t bufLen, uint16_t maxLen);
extern void appSlaveAdvStart(uint8_t numSets, uint8_t *pAdvHandles, uint16_t *pInterval,
                             uint16_t *pDuration, uint8_t *pMaxEaEvents, bool_t cfgAdvParam,
                             uint8_t mode);
...
extern dmConnId_t appConnAccept(uint8_t advHandle, uint8_t advType, uint16_t interval,
