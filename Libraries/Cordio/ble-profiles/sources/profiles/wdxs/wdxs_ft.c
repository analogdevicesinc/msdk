...
static void wdxsFtcProcPutReq(dmConnId_t connId, uint16_t handle, uint16_t len, uint8_t *pValue)
{
    ...
    /* verify permissions */
        ...
        APP_TRACE_INFO3("WDXS: FTC PutReq handle=%d offset=%d, len=%d", handle, wdxsCb.ftOffset,
                        wdxsCb.ftLen);
  ...
  if (wdxsCb.ftLen >= len)
  {
    ...
    if (wdxsCb.ftLen >= len) {
        ...
        if (wdxsCb.ftLen == 0) {
            if (wdxsCb.ftOffset == wdxsCb.ftTotalLen) {
                /* Call the media specific WDXS Put Complete command */
                WsfEfsMediaSpecificCommand(wdxsCb.ftHandle, WSF_EFS_WDXS_PUT_COMPLETE_CMD,
                                           wdxsCb.ftTotalLen);
            }
