...
/*! Default device name */
#define CORE_DEFAULT_DEV_NAME "Analog"
...
static const uint8_t gapValDnCh[] = { ATT_PROP_READ, UINT16_TO_BYTES(GAP_DN_HDL),
...
static const uint8_t gapValApCh[] = { ATT_PROP_READ, UINT16_TO_BYTES(GAP_AP_HDL),
...
static const uint8_t gapValCarCh[] = { ATT_PROP_READ, UINT16_TO_BYTES(GAP_CAR_HDL),
                                       UINT16_TO_BYTES(ATT_UUID_CAR) };
...
static const uint8_t gapValRpaoCh[] = { ATT_PROP_READ, UINT16_TO_BYTES(GAP_RPAO_HDL),
                                        UINT16_TO_BYTES(ATT_UUID_RPAO) };
...
static const attsAttr_t gapList[] = {
    { attPrimSvcUuid, (uint8_t *)gapValSvc, (uint16_t *)&gapLenSvc, sizeof(gapValSvc), 0,
      ATTS_PERMIT_READ },
    { attChUuid, (uint8_t *)gapValDnCh, (uint16_t *)&gapLenDnCh, sizeof(gapValDnCh), 0,
      ATTS_PERMIT_READ },
    { attDnChUuid, gapValDn, &gapLenDn, sizeof(gapValDn),
      (ATTS_SET_VARIABLE_LEN | ATTS_SET_WRITE_CBACK), (ATTS_PERMIT_READ | CORE_SEC_PERMIT_WRITE) },
    { attChUuid, (uint8_t *)gapValApCh, (uint16_t *)&gapLenApCh, sizeof(gapValApCh), 0,
      ATTS_PERMIT_READ },
    { attApChUuid, gapValAp, (uint16_t *)&gapLenAp, sizeof(gapValAp), 0, ATTS_PERMIT_READ },
    { attChUuid, (uint8_t *)gapValCarCh, (uint16_t *)&gapLenCarCh, sizeof(gapValCarCh), 0,
      ATTS_PERMIT_READ },
    { attCarChUuid, gapValCar, (uint16_t *)&gapLenCar, sizeof(gapValCar), 0, ATTS_PERMIT_READ },
    { attChUuid, (uint8_t *)gapValRpaoCh, (uint16_t *)&gapLenRpaoCh, sizeof(gapValRpaoCh), 0,
      ATTS_PERMIT_READ },
    { attRpaoChUuid, gapValRpao, (uint16_t *)&gapLenRpao, sizeof(gapValRpao), 0, ATTS_PERMIT_READ },
};

/* GAP group structure */
static attsGroup_t svcGapGroup = { NULL, (attsAttr_t *)gapList, NULL,
                                   NULL, GAP_START_HDL,         GAP_END_HDL };
...
static const uint8_t gattValScCh[] = { ATT_PROP_INDICATE, UINT16_TO_BYTES(GATT_SC_HDL),
                                       UINT16_TO_BYTES(ATT_UUID_SERVICE_CHANGED) };
...
static const uint8_t gattValCsfCh[] = { ATT_PROP_READ | ATT_PROP_WRITE,
                                        UINT16_TO_BYTES(GATT_CSF_HDL),
                                        UINT16_TO_BYTES(ATT_UUID_CLIENT_SUPPORTED_FEATURES) };
...
static const uint8_t gattValDbhCh[] = { ATT_PROP_READ, UINT16_TO_BYTES(GATT_DBH_HDL),
                                        UINT16_TO_BYTES(ATT_UUID_DATABASE_HASH) };
...
static const uint8_t gattValSsfCh[] = { ATT_PROP_READ, UINT16_TO_BYTES(GATT_SSF_HDL),
                                        UINT16_TO_BYTES(ATT_UUID_SERVER_SUPPORTED_FEATURES) };
...
static const attsAttr_t gattList[] = {
    { attPrimSvcUuid, (uint8_t *)gattValSvc, (uint16_t *)&gattLenSvc, sizeof(gattValSvc), 0,
      ATTS_PERMIT_READ },
    { attChUuid, (uint8_t *)gattValScCh, (uint16_t *)&gattLenScCh, sizeof(gattValScCh), 0,
      ATTS_PERMIT_READ },
    { attScChUuid, (uint8_t *)gattValSc, (uint16_t *)&gattLenSc, sizeof(gattValSc), 0, 0 },
    { attCliChCfgUuid, gattValScChCcc, (uint16_t *)&gattLenScChCcc, sizeof(gattValScChCcc),
      ATTS_SET_CCC, (ATTS_PERMIT_READ | CORE_SEC_PERMIT_WRITE) },
    { attChUuid, (uint8_t *)gattValCsfCh, (uint16_t *)&gattLenCsfCh, sizeof(gattValCsfCh), 0,
      (ATTS_PERMIT_READ | CORE_SEC_PERMIT_READ) },
    { attGattCsfChUuid, (uint8_t *)gattValCsf, (uint16_t *)&gattLenCsf, sizeof(gattValCsf),
      (ATTS_SET_READ_CBACK | ATTS_SET_WRITE_CBACK),
      (ATTS_PERMIT_READ | ATTS_PERMIT_WRITE | CORE_SEC_PERMIT_READ | CORE_SEC_PERMIT_WRITE) },
    { attChUuid, (uint8_t *)gattValDbhCh, (uint16_t *)&gattLenDbhCh, sizeof(gattValDbhCh), 0,
      (ATTS_PERMIT_READ | CORE_SEC_PERMIT_READ) },
    { attGattDbhChUuid, (uint8_t *)gattValDbh, (uint16_t *)&gattLenDbh, sizeof(gattValDbh),
      ATTS_SET_READ_CBACK, (ATTS_PERMIT_READ | CORE_SEC_PERMIT_READ) },
    { attChUuid, (uint8_t *)gattValSsfCh, (uint16_t *)&gattLenSsfCh, sizeof(gattValSsfCh), 0,
      ATTS_PERMIT_READ },
    { attSsfChUuid, gattValSsf, (uint16_t *)&gattLenSsf, sizeof(gattValSsf), 0, ATTS_PERMIT_READ }
};

/* GATT group structure */
static attsGroup_t svcGattGroup = { NULL, (attsAttr_t *)gattList, NULL,
                                    NULL, GATT_START_HDL,         GATT_END_HDL };
