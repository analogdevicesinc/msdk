...
static const uint8_t cpsValFeatureCh[] = { ATT_PROP_READ, UINT16_TO_BYTES(CPS_CPF_HDL),
                                           UINT16_TO_BYTES(ATT_UUID_CYCLING_POWER_FEATURE) };
...
static const uint8_t cpsValMeasurementCh[] = { ATT_PROP_NOTIFY, UINT16_TO_BYTES(CPS_CPM_HDL),
                                               UINT16_TO_BYTES(
...
static const uint8_t cpsValLocationCh[] = { ATT_PROP_READ, UINT16_TO_BYTES(CPS_CPSL_HDL),
...
static const attsAttr_t cpsList[] = {
    /* Cycling Power Service declaration */
    { attPrimSvcUuid, (uint8_t *)cpsValSvc, (uint16_t *)&cpsLenSvc, sizeof(cpsValSvc), 0,
      ATTS_PERMIT_READ },
    /* Cycling Power Feature characteristic */
    { attChUuid, (uint8_t *)cpsValFeatureCh, (uint16_t *)&cpsLenFeatureCh, sizeof(cpsValFeatureCh),
      0, ATTS_PERMIT_READ },
    /* Cycling Power Feature value */
    { attCpfChUuid, (uint8_t *)cpsValFeature, (uint16_t *)&cpsLenFeature, sizeof(cpsValFeature), 0,
      CPS_SEC_PERMIT_READ },
    /* Cycling Power Measurement characteristic */
    { attChUuid, (uint8_t *)cpsValMeasurementCh, (uint16_t *)&cpsLenMeasurementCh,
      sizeof(cpsValMeasurementCh), 0, ATTS_PERMIT_READ },
    /* Cycling Power Measurement value */
    { attCpmChUuid, (uint8_t *)cpsValMeasurement, (uint16_t *)&cpsLenMeasurement,
      sizeof(cpsValMeasurement), 0, 0 },
    /* Characteristic CCC descriptor */
    { attCliChCfgUuid, cpsValMeasurementChCcc, (uint16_t *)&cpsLenMeasurementChCcc,
      sizeof(cpsValMeasurementChCcc), ATTS_SET_CCC, (ATTS_PERMIT_READ | CPS_SEC_PERMIT_WRITE) },
    /* Cycling Power Sensor Location characteristic */
    { attChUuid, (uint8_t *)cpsValLocationCh, (uint16_t *)&cpsLenLocationCh,
      sizeof(cpsValLocationCh), 0, ATTS_PERMIT_READ },
    /* Cycling Power Sensor Location value */
    { attSlChUuid, (uint8_t *)cpsValLocation, (uint16_t *)&cpsLenLocation, sizeof(cpsValLocation),
      0, CPS_SEC_PERMIT_READ },
};

/* CPS group structure */
static attsGroup_t svcCpsGroup = { NULL, (attsAttr_t *)cpsList, NULL,
                                   NULL, CPS_START_HDL,         CPS_END_HDL };
