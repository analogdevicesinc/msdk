...
static const uint8_t plxsValFeatureCh[] = { ATT_PROP_READ, UINT16_TO_BYTES(PLXS_FEATURES_HDL),
                                            UINT16_TO_BYTES(ATT_UUID_PULSE_OX_FEATURES) };
...
static const uint8_t plxsValSpotCheckCh[] = { ATT_PROP_INDICATE,
                                              UINT16_TO_BYTES(PLXS_SPOT_CHECK_HDL),
                                              UINT16_TO_BYTES(ATT_UUID_PULSE_OX_SPOT_CHECK) };
...
static const uint8_t plxsValContinuousCh[] = { ATT_PROP_NOTIFY,
                                               UINT16_TO_BYTES(PLXS_CONTINUOUS_HDL),
                                               UINT16_TO_BYTES(ATT_UUID_PULSE_OX_CONTINUOUS) };
...
static const uint8_t plxsValRecordAccessCh[] = { ATT_PROP_INDICATE | ATT_PROP_WRITE,
                                                 UINT16_TO_BYTES(PLXS_RECORD_ACCESS_HDL),
                                                 UINT16_TO_BYTES(ATT_UUID_RACP) };
...
static const attsAttr_t plxsList[] = {
    /* Pulse Oximeter Service declaration */
    { attPrimSvcUuid, (uint8_t *)plxsValSvc, (uint16_t *)&plxsLenSvc, sizeof(plxsValSvc), 0,
      ATTS_PERMIT_READ },
    /* Pulse Oximeter Feature characteristic */
    { attChUuid, (uint8_t *)plxsValFeatureCh, (uint16_t *)&plxsLenFeatureCh,
      sizeof(plxsValFeatureCh), 0, ATTS_PERMIT_READ },
    /* Pulse Oximeter Feature value */
    { attPlxfChUuid, (uint8_t *)plxsValFeature, (uint16_t *)&plxsLenFeature, sizeof(plxsValFeature),
      ATTS_SET_VARIABLE_LEN, PLXS_SEC_PERMIT_READ },
    /* Pulse Oximeter Spot Check characteristic */
    { attChUuid, (uint8_t *)plxsValSpotCheckCh, (uint16_t *)&plxsLenSpotCheckCh,
      sizeof(plxsValSpotCheckCh), 0, ATTS_PERMIT_READ },
    /* Pulse Oximeter Spot Check value */
    { attPlxscmChUuid, (uint8_t *)plxsValSpotCheck, (uint16_t *)&plxsLenSpotCheck,
      sizeof(plxsValSpotCheck), 0, 0 },
    /* Pulse Oximeter Spot Check Characteristic CCC descriptor */
    { attCliChCfgUuid, plxsValSpotCheckChCcc, (uint16_t *)&plxsLenSpotCheckChCcc,
      sizeof(plxsValSpotCheckChCcc), ATTS_SET_CCC, (ATTS_PERMIT_READ | PLXS_SEC_PERMIT_WRITE) },

    /* Pulse Oximeter Continuous Measurement characteristic */
    { attChUuid, (uint8_t *)plxsValContinuousCh, (uint16_t *)&plxsLenContinuousCh,
      sizeof(plxsValContinuousCh), 0, ATTS_PERMIT_READ },
    /* Pulse Oximeter Continuous Measurement value */
    { attPlxcmChUuid, (uint8_t *)plxsValContinuous, (uint16_t *)&plxsLenContinuous,
      sizeof(plxsValContinuous), 0, 0 },
    /* Pulse Oximeter Continuous Measurement Characteristic CCC descriptor */
    { attCliChCfgUuid, plxsValContinuousChCcc, (uint16_t *)&plxsLenContinuousChCcc,
      sizeof(plxsValContinuousChCcc), ATTS_SET_CCC, (ATTS_PERMIT_READ | PLXS_SEC_PERMIT_WRITE) },

    /* Pulse Oximeter Record Access Control Point characteristic */
    { attChUuid, (uint8_t *)plxsValRecordAccessCh, (uint16_t *)&plxsLenRecordAccessCh,
      sizeof(plxsValRecordAccessCh), 0, ATTS_PERMIT_READ },
    /* Pulse Oximeter Record Access Control Point value */
    { attRacpChUuid, (uint8_t *)plxsValRecordAccess, (uint16_t *)&plxsLenRecordAccess,
      ATT_DEFAULT_PAYLOAD_LEN, (ATTS_SET_VARIABLE_LEN | ATTS_SET_WRITE_CBACK),
      PLXS_SEC_PERMIT_WRITE },
    /* Pulse Oximeter Record Access Control Point Characteristic CCC descriptor */
    { attCliChCfgUuid, plxsValRecordAccessChCcc, (uint16_t *)&plxsLenRecordAccessChCcc,
      sizeof(plxsValRecordAccessChCcc), ATTS_SET_CCC, (ATTS_PERMIT_READ | PLXS_SEC_PERMIT_WRITE) },
};

/* PLXS group structure */
static attsGroup_t svcPlxsGroup = { NULL, (attsAttr_t *)plxsList, NULL,
                                    NULL, PLXS_START_HDL,         PLXS_END_HDL };
