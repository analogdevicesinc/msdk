...
static const attsAttr_t battList[] = {
    /* Service declaration */
    { attPrimSvcUuid, (uint8_t *)battValSvc, (uint16_t *)&battLenSvc, sizeof(battValSvc), 0,
      ATTS_PERMIT_READ },
    /* Characteristic declaration */
    { attChUuid, (uint8_t *)battValLvlCh, (uint16_t *)&battLenLvlCh, sizeof(battValLvlCh), 0,
      ATTS_PERMIT_READ },
    /* Characteristic value */
    { attBlChUuid, battValLvl, (uint16_t *)&battLenLvl, sizeof(battValLvl), ATTS_SET_READ_CBACK,
      BATT_SEC_PERMIT_READ },
    /* Characteristic CCC descriptor */
    { attCliChCfgUuid, battValLvlChCcc, (uint16_t *)&battLenLvlChCcc, sizeof(battValLvlChCcc),
      ATTS_SET_CCC, (ATTS_PERMIT_READ | BATT_SEC_PERMIT_WRITE) }
};

/* Battery group structure */
static attsGroup_t svcBattGroup = { NULL, (attsAttr_t *)battList, NULL,
                                    NULL, BATT_START_HDL,         BATT_END_HDL };
...
void *SvcBattAddGroupDyn()
{
    ...
    if (pSHdl != NULL) {
        /* Primary service */
        AttsDynAddAttrConst(pSHdl, attPrimSvcUuid, battValSvc, sizeof(battValSvc), 0,
                            ATTS_PERMIT_READ);

        /* Battery level characteristic */
        AttsDynAddAttrConst(pSHdl, attChUuid, battValLvlCh, sizeof(battValLvlCh), 0,
                            ATTS_PERMIT_READ);
