...
static const uint8_t hrsValHrmCh[] = { ATT_PROP_NOTIFY, UINT16_TO_BYTES(HRS_HRM_HDL),
                                       UINT16_TO_BYTES(ATT_UUID_HR_MEAS) };
...
static const uint8_t hrsValSlCh[] = { ATT_PROP_READ, UINT16_TO_BYTES(HRS_SL_HDL),
                                      UINT16_TO_BYTES(ATT_UUID_HR_SENSOR_LOC) };
...
static const uint8_t hrsValCpCh[] = { ATT_PROP_WRITE, UINT16_TO_BYTES(HRS_CP_HDL),
                                      UINT16_TO_BYTES(ATT_UUID_HR_CP) };
...
static const attsAttr_t hrsList[] = {
    { attPrimSvcUuid, (uint8_t *)hrsValSvc, (uint16_t *)&hrsLenSvc, sizeof(hrsValSvc), 0,
      ATTS_PERMIT_READ },
    { attChUuid, (uint8_t *)hrsValHrmCh, (uint16_t *)&hrsLenHrmCh, sizeof(hrsValHrmCh), 0,
      ATTS_PERMIT_READ },
    { svcHrmUuid, (uint8_t *)hrsValHrm, (uint16_t *)&hrsLenHrm, sizeof(hrsValHrm), 0, 0 },
    { attCliChCfgUuid, (uint8_t *)hrsValHrmChCcc, (uint16_t *)&hrsLenHrmChCcc,
      sizeof(hrsValHrmChCcc), ATTS_SET_CCC, (ATTS_PERMIT_READ | HRS_SEC_PERMIT_WRITE) },
    { attChUuid, (uint8_t *)hrsValSlCh, (uint16_t *)&hrsLenSlCh, sizeof(hrsValSlCh), 0,
      ATTS_PERMIT_READ },
    { svcSlUuid, hrsValSl, (uint16_t *)&hrsLenSl, sizeof(hrsValSl), 0, HRS_SEC_PERMIT_READ },
    { attChUuid, (uint8_t *)hrsValCpCh, (uint16_t *)&hrsLenCpCh, sizeof(hrsValCpCh), 0,
      ATTS_PERMIT_READ },
    { svcCpUuid, (uint8_t *)hrsValCp, (uint16_t *)&hrsLenCp, sizeof(hrsValCp), ATTS_SET_WRITE_CBACK,
      HRS_SEC_PERMIT_WRITE }
};

/* HRS group structure */
static attsGroup_t svcHrsGroup = { NULL, (attsAttr_t *)hrsList, NULL,
                                   NULL, HRS_START_HDL,         HRS_END_HDL };
