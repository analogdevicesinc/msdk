...
static const uint8_t alertAnsValSnaCh[] = { ATT_PROP_READ, UINT16_TO_BYTES(ALERT_ANS_SNA_HDL),
                                            UINT16_TO_BYTES(ATT_UUID_SUP_NEW_ALERT_CAT) };
...
static const uint8_t alertAnsValNewCh[] = { ATT_PROP_NOTIFY, UINT16_TO_BYTES(ALERT_ANS_NEW_HDL),
                                            UINT16_TO_BYTES(ATT_UUID_NEW_ALERT) };
...
static const uint8_t alertAnsValUnrCh[] = { ATT_PROP_READ, UINT16_TO_BYTES(ALERT_ANS_UNR_HDL),
                                            UINT16_TO_BYTES(ATT_UUID_SUP_UNREAD_ALERT_CAT) };
...
static const uint8_t alertAnsValUasCh[] = { ATT_PROP_NOTIFY, UINT16_TO_BYTES(ALERT_ANS_UAS_HDL),
                                            UINT16_TO_BYTES(ATT_UUID_UNREAD_ALERT_STATUS) };
...
static const uint8_t alertAnsValCpCh[] = { ATT_PROP_WRITE, UINT16_TO_BYTES(ALERT_ANS_CP_HDL),
                                           UINT16_TO_BYTES(ATT_UUID_ALERT_NOTIF_CP) };
...
static const uint8_t alertPassValAsCh[] = { ATT_PROP_READ | ATT_PROP_NOTIFY,
                                            UINT16_TO_BYTES(ALERT_PASS_AS_HDL),
                                            UINT16_TO_BYTES(ATT_UUID_ALERT_STATUS) };
...
/* Ringer setting characteristic */
static const uint8_t alertPassValRsCh[] = { ATT_PROP_READ | ATT_PROP_NOTIFY,
                                            UINT16_TO_BYTES(ALERT_PASS_RS_HDL),
                                            UINT16_TO_BYTES(ATT_UUID_RINGER_SETTING) };
...
/* Ringer control point characteristic */
static const uint8_t alertPassValRcpCh[] = { ATT_PROP_WRITE_NO_RSP,
                                             UINT16_TO_BYTES(ALERT_PASS_RCP_HDL),
                                             UINT16_TO_BYTES(ATT_UUID_RINGER_CP) };
...
static const uint8_t alertNwsValNwaCh[] = { ATT_PROP_READ | ATT_PROP_INDICATE,
                                            UINT16_TO_BYTES(ALERT_NWS_NWA_HDL),
                                            UINT16_TO_BYTES(ATT_UUID_NETWORK_AVAIL) };
...
static const attsAttr_t alertList[] = {
    /* Alert notification service */
    { attPrimSvcUuid, (uint8_t *)alertAnsValSvc, (uint16_t *)&alertAnsLenSvc,
      sizeof(alertAnsValSvc), 0, ATTS_PERMIT_READ },
    { attChUuid, (uint8_t *)alertAnsValSnaCh, (uint16_t *)&alertAnsLenSnaCh,
      sizeof(alertAnsValSnaCh), 0, ATTS_PERMIT_READ },
    { alertAnsUuSna, alertAnsValSna, &alertAnsLenSna, sizeof(alertAnsValSna), 0,
      ALERT_SEC_PERMIT_READ },
    { attChUuid, (uint8_t *)alertAnsValNewCh, (uint16_t *)&alertAnsLenNewCh,
      sizeof(alertAnsValNewCh), 0, ATTS_PERMIT_READ },
    { alertAnsUuNew, alertAnsValNew, &alertAnsLenNew, sizeof(alertAnsValNew), 0, 0 },
    { attCliChCfgUuid, alertAnsValNewChCcc, (uint16_t *)&alertAnsLenNewChCcc,
      sizeof(alertAnsValNewChCcc), 0, (ATTS_PERMIT_READ | ALERT_SEC_PERMIT_WRITE) },
    { attChUuid, (uint8_t *)alertAnsValUnrCh, (uint16_t *)&alertAnsLenUnrCh,
      sizeof(alertAnsValUnrCh), 0, ATTS_PERMIT_READ },
    { alertAnsUuUnr, alertAnsValUnr, &alertAnsLenUnr, sizeof(alertAnsValUnr), 0,
      ALERT_SEC_PERMIT_READ },
    { attChUuid, (uint8_t *)alertAnsValUasCh, (uint16_t *)&alertAnsLenUasCh,
      sizeof(alertAnsValUasCh), 0, ATTS_PERMIT_READ },
    { alertAnsUuUas, alertAnsValUas, &alertAnsLenUas, sizeof(alertAnsValUas), 0, 0 },
    { attCliChCfgUuid, alertAnsValUasChCcc, (uint16_t *)&alertAnsLenUasChCcc,
      sizeof(alertAnsValUasChCcc), 0, (ATTS_PERMIT_READ | ALERT_SEC_PERMIT_WRITE) },
    { attChUuid, (uint8_t *)alertAnsValCpCh, (uint16_t *)&alertAnsLenCpCh, sizeof(alertAnsValCpCh),
      0, ATTS_PERMIT_READ },
    { alertAnsUuCp, alertAnsValCp, (uint16_t *)&alertAnsLenCp, sizeof(alertAnsValCp), 0,
      ALERT_SEC_PERMIT_WRITE },
    /* Phone alert status service */
    { attPrimSvcUuid, (uint8_t *)alertPassValSvc, (uint16_t *)&alertPassLenSvc,
      sizeof(alertPassValSvc), 0, ATTS_PERMIT_READ },
    { attChUuid, (uint8_t *)alertPassValAsCh, (uint16_t *)&alertPassLenAsCh,
      sizeof(alertPassValAsCh), 0, ATTS_PERMIT_READ },
    { alertPassUuAs, alertPassValAs, (uint16_t *)&alertPassLenAs, sizeof(alertPassValAs), 0,
      ALERT_SEC_PERMIT_READ },
    { attCliChCfgUuid, alertPassValAsChCcc, (uint16_t *)&alertPassLenAsChCcc,
      sizeof(alertPassValAsChCcc), 0, (ATTS_PERMIT_READ | ALERT_SEC_PERMIT_WRITE) },
    { attChUuid, (uint8_t *)alertPassValRsCh, (uint16_t *)&alertPassLenRsCh,
      sizeof(alertPassValRsCh), 0, ATTS_PERMIT_READ },
    { alertPassUuRs, alertPassValRs, (uint16_t *)&alertPassLenRs, sizeof(alertPassValRs), 0,
      ALERT_SEC_PERMIT_READ },
    { attCliChCfgUuid, alertPassValRsChCcc, (uint16_t *)&alertPassLenRsChCcc,
      sizeof(alertPassValRsChCcc), 0, (ATTS_PERMIT_READ | ALERT_SEC_PERMIT_WRITE) },
    { attChUuid, (uint8_t *)alertPassValRcpCh, (uint16_t *)&alertPassLenRcpCh,
      sizeof(alertPassValRcpCh), 0, ATTS_PERMIT_READ },
    { alertPassUuRcp, alertPassValRcp, (uint16_t *)&alertPassLenRcp, sizeof(alertPassValRcp), 0,
      ALERT_SEC_PERMIT_WRITE },

    /* Network availability service */
    { attPrimSvcUuid, (uint8_t *)alertNwsValSvc, (uint16_t *)&alertNwsLenSvc,
      sizeof(alertNwsValSvc), 0, ATTS_PERMIT_READ },
    { attChUuid, (uint8_t *)alertNwsValNwaCh, (uint16_t *)&alertNwsLenNwaCh,
      sizeof(alertNwsValNwaCh), 0, ATTS_PERMIT_READ },
    { alertNwsUuNwa, alertNwsValNwa, (uint16_t *)&alertNwsLenNwa, sizeof(alertNwsValNwa), 0,
      ALERT_SEC_PERMIT_READ },
    { attCliChCfgUuid, alertNwsValNwaChCcc, (uint16_t *)&alertNwsLenNwaChCcc,
      sizeof(alertNwsValNwaChCcc), 0, (ATTS_PERMIT_READ | ALERT_SEC_PERMIT_WRITE) },
};

/* Alert group structure */
static attsGroup_t svcAlertGroup = { NULL, (attsAttr_t *)alertList, NULL,
                                     NULL, ALERT_START_HDL,         ALERT_END_HDL };
