...
static void palI2sCompHandler(nrfx_i2s_buffers_t const *pRelBuf, uint32_t status)
{
    ...
    if (status == NRFX_I2S_STATUS_NEXT_BUFFERS_NEEDED) {
        if ((palI2sCb.initBuf[1].p_rx_buffer) || (palI2sCb.initBuf[1].p_tx_buffer)) {
            nrfx_err_t err;
...
void PalI2sReadData(uint8_t *pData, uint16_t len)
{
    ...
    default: {
        nrfx_i2s_buffers_t i2sBuf = { .p_rx_buffer = (uint32_t *)pData, .p_tx_buffer = NULL };
    ...
    case PAL_I2S_STATE_READY: {
        if (!palI2sCb.initBuf[0].p_rx_buffer) {
            ...
            err = nrfx_i2s_start(&palI2sCb.initBuf[0], len / sizeof(uint32_t),
                                 0); /* length in size of words */
            PAL_I2S_PARAM_CHECK(err == NRFX_SUCCESS);
...
void PalI2sWriteData(const uint8_t *pData, uint16_t len)
{
    ...
    case PAL_I2S_STATE_BUSY:
    default: {
        nrfx_i2s_buffers_t i2sBuf = { .p_rx_buffer = NULL, .p_tx_buffer = (uint32_t *)pData };
