...
#define BB_154_DRV_CRC_POLYNOMIAL \
    0x011021 /* Polynomial used for CRC calculation in 802.15.4 frames */

/*! \brief  Frame offsets. */
#define BB_154_DRV_RX_FRAME_CTRL_OFFSET \
    1 /* Offset of byte containing Frame Control (LSB) (+1 for frame length byte) */
#define BB_154_DRV_RX_PAN_ID_OFFSET \
    4 /* Offset of byte containing PAN ID (LSB) (+1 for frame length byte) */
#define BB_154_DRV_RX_DEST_ADDR_OFFSET \
    6 /* Offset of byte containing destination address (LSB) (+1 for frame length byte) */
...
#define PAL_BB_154_FRM_TO_UINT16(n, p) (n = (uint16_t)(p)[0] + ((uint16_t)(p)[1] << 8))

/*! \brief  Convert 64-bit unsigned in frame field to uint64_t. */
#define PAL_BB_154_FRM_TO_UINT64(n, p)                                                    \
    (n = (uint64_t)(p)[0] + ((uint64_t)(p)[1] << 8) + ((uint64_t)(p)[2] << 16) +          \
...
#define BB_154_DRV_BACKOFF_FACTOR_US (PAL_BB_154_aUnitBackoffPeriod * PAL_BB_154_SYMBOL_DURATION)
...
static bb154DrvRadioTiming_t bb154DrvRadioTiming = { .txOnLatency = 74,
                                                     .rxOnLatency = 63,
                                                     .txDataPathLatency = 8,
                                                     .rxDataPathLatency = 8 };
...
static void bb154DisableTimerCompare1(void)
{
    ...
    NVIC_ClearPendingIRQ(
        BB_154_DRV_TIMER_IRQ); /* necessary, if interrupt already "fired" this is the only way to clear it */
}
...
static void bb154DrvTxPowerSet(int8_t dbm)
{
    ...
    const int8_t highestValue =
        allowedValues[(sizeof(allowedValues) / sizeof(allowedValues[0])) - 1];
...
static bool_t bb154DrvStartRadioTask(uint32_t due, bool_t now, nrf_radio_task_t radioTask)
{
    if (now) {
        ...
        nrf_ppi_channel_endpoint_setup(
            BB_154_DRV_PPI_TXRX_DELAY_CHAN,
...
static void bb154DrvOff(void)
{
    ...
    nrf_radio_shorts_disable(
        ...
        NRF_RADIO_SHORT_RXREADY_CCASTART_MASK | NRF_RADIO_SHORT_READY_EDSTART_MASK);
...
static void bb154DrvTxDataNoCca(uint32_t due, bool_t now)
{
   ...
   * EVENTS_END -> TASKS_DISABLE
   */
    nrf_radio_shorts_enable(NRF_RADIO_SHORT_READY_START_MASK | NRF_RADIO_SHORT_END_DISABLE_MASK);
...
static void bb154DrvMacCca(uint32_t due, bool_t now)
{
    ...
    if (bb154DrvCb.tx.be > 0) {
        ...
        backoff =
            (uint32_t)(backoffSymb & pow2[bb154DrvCb.tx.be - 1]) * BB_154_DRV_BACKOFF_FACTOR_US;
    }
...
{
    if (ackReqd || bb154DrvPib.rxEnabled || bb154DrvPib.rxOnWhenIdle ||
        bb154DrvPib.promiscuousMode) {
 ...
 */
/*************************************************************************************************/
static uint8_t *bb154GetAddrsFromFrame(uint8_t *pFrame, uint16_t fctl, bb154Addr_t *pSrcAddr,
                                       bb154Addr_t *pDstAddr)
{
            ...
            case BB_154_DRV_HDR_STATE_PAYLOAD:
                /* At payload. Check addresses. */
                {
                    ...
                    if (!restart) {
                        ...
                        if (((fctl & PAL_BB_154_FC_FRAME_TYPE_FP_TEST) ==
                             PAL_BB_154_FC_FRAME_TYPE_FP_TEST) &&
...
static uint8_t bb154DrvEvHRxFmEnd(void)
{
    ...
    if (nrf_radio_crc_status_check()) {
        ...
        int8_t rssi;
        uint32_t ts =
            nrf_timer_cc_read(BB_154_DRV_TIMER, 2); /* TODO: minus preamble and SFD time */
        uint16_t fctl;
       ...
       */
            uint32_t due =
                PalBbGetCurrentTime() + PAL_BB_154_SYMB_TO_US(PAL_BB_154_aTurnaroundTime);
...
static uint8_t bb154DrvEvHTxFmEnd(void)
{
    ...
    /* Disable shortcuts */
    nrf_radio_shorts_disable(NRF_RADIO_SHORT_READY_START_MASK | NRF_RADIO_SHORT_END_DISABLE_MASK);
    ...
    if (bb154DrvCb.op.flags & PAL_BB_154_FLAG_TX_AUTO_RX_ACK) {
        ...
        ackReqd =
            (((pTxFrame[0] & PAL_BB_154_FC_ACK_REQUEST_MASK) != 0) &&
        ...
        ackReqd = (((pTxFrame[0] & PAL_BB_154_FC_ACK_REQUEST_MASK) != 0) &&
                   (((pTxFrame[0] & PAL_BB_154_FC_FRAME_TYPE_MASK) == PAL_BB_154_FRAME_TYPE_DATA) ||
                    ((pTxFrame[0] & PAL_BB_154_FC_FRAME_TYPE_MASK) ==
                     PAL_BB_154_FRAME_TYPE_MAC_COMMAND)));
#endif
    }
...
void PalBb154Init(void)
{
    ...
    BB_ASSERT(bb154DrvCb.state ==
              BB_154_DRV_STATE_UNINITIALIZED); /* Driver should only be initialized once */
...
void PalBb154Enable(void)
{
    ...
    BB_ASSERT(nrf_clock_hf_is_running(
        NRF_CLOCK_HFCLK_HIGH_ACCURACY)); /* HF clock source must be the crystal */
    ...
    nrf_timer_int_disable(BB_154_DRV_TIMER,
                          0xFFFFFFFF); /* Blanket disable all timer interrupts at source */
...
static void Bb154DrvRadioIRQHandler(void)
{
    ...
    static const nrf_radio_event_t eventsToCheck[BB_154_DRV_MAX_RADIO_EVENTS] = {
        NRF_RADIO_EVENT_READY,  NRF_RADIO_EVENT_FRAMESTART, NRF_RADIO_EVENT_BCMATCH,
        NRF_RADIO_EVENT_END,    NRF_RADIO_EVENT_EDEND,      NRF_RADIO_EVENT_CCAIDLE,
    ...
    for (i = 0; i < BB_154_DRV_MAX_RADIO_EVENTS; i++) {
        ...
        if (nrf_radio_event_check(eventToCheck)) {
            ...
            /* Invoke event handler */
            stat = bb154DrvEvtHandler[bb154DrvStateTable[i + BB_154_DRV_RADIO_EVT_OFFSET]
                                                        [bb154DrvCb.state]]();
