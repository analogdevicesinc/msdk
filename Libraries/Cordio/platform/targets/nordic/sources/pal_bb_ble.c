...
#define MAX_TIFS_DEVIATION_USECS \
    (2 + 1) /* from spec, section 4.2.1 Active Clock Accuracy, 50ppm = 2usec */
/*   +1 usec as a margin of uncertainty */

#if defined(NRF52840_XXAA) || defined(NRF52832_XXAA)
#define NRF5x_tTXEN_BLE_USECS \
    39 /* usec, Time between TXEN task and READY event, fast ramp mode (datasheet value of 40 is wrong). */
#define NRF5x_tRXEN_BLE_USECS \
    42 /* usec, Time between RXEN task and READY event, fast ramp mode (datasheet value of 40 is wrong). */
#else
...
#if defined(NRF52840_XXAA) || defined(NRF52832_XXAA)
#define NRF5x_PROP_DELAY_TX_USECS \
    1 /* usec, (precise value is 0.6 usec) transmit propagation delay, START to first bit at antenna */
#define NRF5x_PROP_DELAY_RX_USECS \
    9 /* usec, (precise value is 9.4 usec), receive propagation delay, last bit at antenna to END */
#else
/* internal propagation delays obtained from Nordic support */
#define NRF5x_PROP_DELAY_TX_USECS \
    3 /* usec, transmit propagation delay, START to first bit at antenna */
#define NRF5x_PROP_DELAY_RX_USECS \
    3 /* usec, (precise value is 3.2 usec), receive propagation delay, last bit at antenna to END */
#endif
...
/* TIFS states */
typedef enum { TIFS_NULL, TIFS_TX_RAMPUP, TIFS_RX_RAMPUP } bbTifsState_t;
...
/////////////////////////////////////////////////////////////////////////////////
#ifdef DIAGNOSTICS
typedef enum { DIAG_NULL, DIAG_TX, DIAG_RX, DIAG_TX_TIFS, DIAG_RX_TIFS } diag_op_t;
...
void PalBbBleInlineEncryptSetPacketCount(uint64_t count)
{
    ...
    bbEncryptCcmData[20] = (count >> 32) &
                           0x7F; /* only 7-bits of MSB are used (packet count is 39 bits) */
}
#endif
...
{
    BB_ASSERT(LL_AA_LEN ==
              (NORDIC_PREFIX_LEN + NORDIC_BASE_ADDR_LEN)); /* internal integrity check */
    ...
    /* CRC configuration */
    NRF_RADIO->CRCCNF = LL_CRC_LEN |
                        RADIO_CRCCNF_SKIP_ADDR_Msk; /* set length, skip address as part of CRC */
...
void PalBbBleEnable(void)
{
    ...
    BB_ASSERT(
        NVIC_GetPriority(RADIO_IRQn) ==
        NVIC_GetPriority(TIMER0_IRQn)); /* BB driver related interrupts must have same priority */
...
#else
    BB_ASSERT(NRF_CLOCK->HFCLKSTAT &
              CLOCK_HFCLKSTAT_SRC_Msk); /* HF clock source must be the crystal */
#endif
    ...
    NRF_TIMER0->PRESCALER =
        TIMER_PRESCALER; /* f = 16MHz / (2 ^ TIMER_PRESCALER),  for TIMER_PRESCALER of 4, clock has 1us resolution
...
static void palBbBleSetChannelParam(PalBbBleChan_t *pChan)
{
    ...
    BB_ASSERT((pChan->txPhy == BB_PHY_BLE_1M) || (pChan->txPhy == BB_PHY_BLE_2M) ||
              (pChan->txPhy == BB_PHY_BLE_CODED));
    BB_ASSERT((pChan->rxPhy == BB_PHY_BLE_1M) || (pChan->rxPhy == BB_PHY_BLE_2M) ||
    ...
    if (bbChanIndex <= 10) {
        rfChan = bbChanIndex + 1;
    } else if (bbChanIndex <= 36) {
        rfChan = bbChanIndex + 2;
    } else if (bbChanIndex == 37) {
        rfChan = 0;
    } else if (bbChanIndex == 38) {
        rfChan = 12;
    } else if (bbChanIndex == 39) {
        rfChan = 39;
    } else {
    ...
    NRF_RADIO->DATAWHITEIV =
        bbChanIndex | 0x40; /* position zero is always '1', it gets "or'ed" in here with 0x40 */
    ...
    if (txPower >= 8) {
        ...
        NRF_RADIO->TXPOWER = RADIO_TXPOWER_TXPOWER_Pos7dBm;
        txPower = 7;
    } else if (txPower >= 6) {
        NRF_RADIO->TXPOWER = RADIO_TXPOWER_TXPOWER_Pos6dBm;
        txPower = 6;
    } else if (txPower >= 5) {
        NRF_RADIO->TXPOWER = RADIO_TXPOWER_TXPOWER_Pos5dBm;
        txPower = 5;
    } else if (txPower >= 4) {
        ...
        NRF_RADIO->TXPOWER = RADIO_TXPOWER_TXPOWER_Pos2dBm;
        txPower = 2;
    } else if (txPower >= 0) {
...
/////////////////////////////////////////////////////////////////////////////////
    ...
    if (bbModifyTxHdrMask && ((hdr & bbModifyTxHdrMask) == bbModifyTxHdrValue)) {
        if (bbTxAccAddrInvalidChanMask & (UINT64_C(1) << bbChanIndex)) {
            if ((bbTxAccAddrInvalidAdjMask & (1 << bbTxAccAddrInvalidStep)) ||
    ...
    uint32_t txStart = BB_US_TO_BB_TICKS(bbAntennaDueTimeUsec -
                                         (NRF5x_tTXEN_BLE_USECS + NRF5x_PROP_DELAY_TX_USECS));

    /* set timer compare CC[0] for time to trigger a transmit */
#if (USE_RTC_BB_CLK)
    uint32_t txStartUsec =
        bbAntennaDueTimeUsec - (NRF5x_tTXEN_BLE_USECS + NRF5x_PROP_DELAY_TX_USECS);

    if (bbAntennaDueTimeUsec < NRF5x_tTXEN_BLE_USECS + NRF5x_PROP_DELAY_TX_USECS) {
        txStartUsec = bbAntennaDueTimeUsec - NRF5x_tTXEN_BLE_USECS - NRF5x_PROP_DELAY_TX_USECS +
                      BB_RTC_MAX_VALUE_US + 1;
    }
...
static void palBbTxHwRadioConfig(uint8_t *pTxBuf)
{
    ...
    uint32_t plen =
        ((bbTxPhy != BB_PHY_BLE_1M) ?
             ((bbTxPhy != BB_PHY_BLE_2M) ? RADIO_PCNF0_PLEN_LongRange : RADIO_PCNF0_PLEN_16bit) :
             RADIO_PCNF0_PLEN_8bit)
        << RADIO_PCNF0_PLEN_Pos;
#elif defined(NRF52832_XXAA)
    uint32_t plen = ((bbTxPhy == BB_PHY_BLE_2M) ? RADIO_PCNF0_PLEN_16bit : RADIO_PCNF0_PLEN_8bit)
                    << RADIO_PCNF0_PLEN_Pos;
#else
    uint32_t plen = 0;
#endif
     ...
     */
        ...
        /* configure PCNF0 */
        NRF_RADIO->PCNF0 =
            (((uint32_t)(1 /* S0 field in bytes      */)) << RADIO_PCNF0_S0LEN_Pos) |
            (((uint32_t)(8 /* length field in bits   */)) << RADIO_PCNF0_LFLEN_Pos) |
            (((uint32_t)(0 /* S1 field in bits       */)) << RADIO_PCNF0_S1LEN_Pos) |
#if defined(NRF52840_XXAA)
            (((uint32_t)((bbTxPhy == BB_PHY_BLE_CODED) ? 3 : 0 /* TERM field in bits*/))
             << RADIO_PCNF0_TERMLEN_Pos) |
            (((uint32_t)((bbTxPhy == BB_PHY_BLE_CODED) ? 2 : 0 /* CI field in bits  */))
        ...
        /* configure PCNF1 */
        NRF_RADIO->PCNF1 =
            (((uint32_t)(0 /* maximum packet length  */)) << RADIO_PCNF1_MAXLEN_Pos) |
     ...
     */
        ...
        BB_ASSERT(
            bbTxLen <=
            ENC_MAX_PAYLOAD_LEN); /* payload exceeds Nordic's maximum possible for encryption */
        ...
        BB_ASSERT(bbEncryptTxBuf[sizeof(bbEncryptTxBuf) - 1] ==
                  0xAA); /* buffer overrun from memcpy */
        BB_ASSERT(bbEncryptTxBuf[sizeof(bbEncryptTxBuf) - 2] ==
                  0xAA); /* buffer overrun from memcpy */
...
#if defined(NRF52840_XXAA)
        case BB_PHY_BLE_CODED:
            ccmModeDatarate = ((bbTxPhyOptions == BB_PHY_OPTIONS_BLE_S2) ?
        ...
        NRF_CCM->MODE = (CCM_MODE_MODE_Encryption << CCM_MODE_MODE_Pos) | (ccmModeDatarate) |
        ...
        NRF_CCM->SHORTS =
            CCM_SHORTS_ENDKSGEN_CRYPT_Msk; // TODO - probably just need to set once at initialization *****
...
{
    if (bbEncryptTxSuppressMic) {
        BB_ASSERT(NRF_CCM->INTENSET ==
                  CCM_INTENSET_ENDKSGEN_Msk); /* only keygen complete should be enabled */
...
///////////////////////////////////////////////////////////////////////////////
        ...
        BB_ASSERT(bbTxLen ==
                  (bbEncryptOutBuf[1] - 4)); /* the populated length should be exactly +4 */
...
void PalBbBleRxData(uint8_t *pBuf, uint16_t len)
{
    ...
    BB_ASSERT(len <= (PDU_HEADER_LEN +
                      MAX_PAYLOAD_LEN)); /* buffer should not exceed header + max payload size */
    BB_ALIGN_ASSERT(pBuf);
...
/////////////////////////////////////////////////////////////////////////////////
   ...
   */
    ...
    /* calculate start time */
    uint32_t rxStart = BB_US_TO_BB_TICKS(bbAntennaDueTimeUsec -
                                         (NRF5x_tRXEN_BLE_USECS + NRF5x_PROP_DELAY_RX_USECS));

    /* set timer compare CC[0] for time to trigger a transmit */
#if (USE_RTC_BB_CLK)
    uint32_t rxStartUsec =
        bbAntennaDueTimeUsec - (NRF5x_tRXEN_BLE_USECS + NRF5x_PROP_DELAY_RX_USECS);

    if (bbAntennaDueTimeUsec < NRF5x_tRXEN_BLE_USECS + NRF5x_PROP_DELAY_RX_USECS) {
        rxStartUsec = bbAntennaDueTimeUsec - NRF5x_tRXEN_BLE_USECS - NRF5x_PROP_DELAY_RX_USECS +
                      BB_RTC_MAX_VALUE_US + 1;
    }
   ...
   * to makeup for rounding error. */
    ...
    NRF_TIMER0->CC[1] =
        NRF_TIMER0->CC[0] +
        TICKS_PER_USEC * (bbRxTimeoutUsec + preambleTime + accessAddrTime +
                          NRF5x_PROP_DELAY_RX_USECS + NRF5x_tRXEN_BLE_USECS + correction);
...
static void palBbRxHwRadioConfig(uint8_t *pRxBuf, uint16_t len)
{
    ...
    uint32_t plen =
        ((bbRxPhy != BB_PHY_BLE_1M) ?
             ((bbRxPhy != BB_PHY_BLE_2M) ? RADIO_PCNF0_PLEN_LongRange : RADIO_PCNF0_PLEN_16bit) :
             RADIO_PCNF0_PLEN_8bit)
        << RADIO_PCNF0_PLEN_Pos;
#elif defined(NRF52832_XXAA)
    uint32_t plen = ((bbRxPhy == BB_PHY_BLE_2M) ? RADIO_PCNF0_PLEN_16bit : RADIO_PCNF0_PLEN_8bit)
                    << RADIO_PCNF0_PLEN_Pos;
#else
    uint32_t plen = 0;
#endif
    ...
    NRF_RADIO->PCNF0 = (((uint32_t)(1 /* S0 field in bytes      */)) << RADIO_PCNF0_S0LEN_Pos) |
                       ...
                       (((uint32_t)(0 /* S1 field in bits       */)) << RADIO_PCNF0_S1LEN_Pos) |
#if defined(NRF52840_XXAA)
                       (((uint32_t)((bbRxPhy == BB_PHY_BLE_CODED) ? 3 : 0 /* TERM field in bits*/))
                        << RADIO_PCNF0_TERMLEN_Pos) |
                       (((uint32_t)((bbRxPhy == BB_PHY_BLE_CODED) ? 2 : 0 /* CI field in bits  */))
                        << RADIO_PCNF0_CILEN_Pos) |
                       (((uint32_t)(0 /* CRCINC field           */)) << RADIO_PCNF0_CRCINC_Pos) |
                       (((uint32_t)(0 /* S1INCL filed           */)) << RADIO_PCNF0_S1INCL_Pos) |
#endif
    ...
    /* configure PCNF1 */
    NRF_RADIO->PCNF1 =
        (((uint32_t)(len - PDU_HEADER_LEN /* max len    */)) << RADIO_PCNF1_MAXLEN_Pos) |
...
{
    BB_ASSERT(NRF_TIMER0->INTENSET ==
              TIMER_INTENSET_COMPARE1_Msk); /* only RX timeout should be enabled */
   ...
   * -----------------------------------------------------------------------------*/
    ...
    BB_ASSERT(NRF_RADIO->STATE ==
              RADIO_STATE_STATE_Rx); /* should only be timeout if receiver is actually on */
...
void BbBleDrvRadioIRQHandler(void)
{
   ...
   * -----------------------------------------------------------------------------*/
        ...
        NVIC_ClearPendingIRQ(
            TIMER0_IRQn); /* necessary, if interrupt already "fired" this is the only way to clear it */
     ...
     *
     */
        BB_ASSERT((NRF_PPI->CHENSET & PPI_CHENSET_CH14_Msk) ==
                  0); /* channel should be disabled by now */
     ...
     * - - - - - - - - - - - - - - - - - - - - - - - - - */
        if ((bbOpParam.ifsMode == PAL_BB_IFS_MODE_TOGGLE_TIFS) && (driverState == TX_STATE)) {
...
#if defined(NRF52840_XXAA)
            case BB_PHY_BLE_CODED:
                crcTime = ((bbTxPhyOptions == BB_PHY_OPTIONS_BLE_S2) ?
                               LL_BLE_US_PER_BYTE_CODED_S2 :
                               LL_BLE_US_PER_BYTE_CODED_S8) *
                          LL_CRC_LEN;
                term2Time = ((bbTxPhyOptions == BB_PHY_OPTIONS_BLE_S2) ?
                                 LL_BLE_US_PER_BIT_CODED_S2 :
                                 LL_BLE_US_PER_BIT_CODED_S8) *
                            LL_TERM2_LEN_BITS;
                break;
#endif
            default:
       ...
       */
            antennaTimeTxPktEnd =
                NRF_TIMER0->CC[2] /* start with timer value when payload just finished */
                ...
                + TICKS_PER_USEC *
                      NRF5x_PROP_DELAY_TX_USECS; /* add propagation delay to get "antenna time" */

            /* set timer trigger for upcoming TIFS receive operation */
            NRF_TIMER0->CC[0] =
                antennaTimeTxPktEnd /* start with "antenna time" at end of packet just transmitted */
                + TICKS_PER_USEC * LL_BLE_TIFS_US /* add the TIFS period */
                - TICKS_PER_USEC *
                      NRF5x_tRXEN_BLE_USECS /* subtract time it takes to ramp-up for receive */
            ...
            NRF_TIMER0->CC[1] =
                ...
                + TICKS_PER_USEC *
                      (2 * MAX_TIFS_DEVIATION_USECS) /* add allowed tolerance  +2/-2 usecs */
                +
                TICKS_PER_USEC *
                    correction; /* empirically derived trim factor (NOTE!! units are ticks, scaler change affects this) */
            /*
     ...
     * - - - - - - - - - - - - - - - - - - - - - - - - - */
        else if ((bbOpParam.ifsMode == PAL_BB_IFS_MODE_TOGGLE_TIFS) && (driverState == RX_STATE)) {
...
#if defined(NRF52840_XXAA)
            case BB_PHY_BLE_CODED:
                crcTime = ((bbRxPhyOptions == BB_PHY_OPTIONS_BLE_S2) ?
                               LL_BLE_US_PER_BYTE_CODED_S2 :
                               LL_BLE_US_PER_BYTE_CODED_S8) *
                          LL_CRC_LEN;
                term2Time = ((bbRxPhyOptions == BB_PHY_OPTIONS_BLE_S2) ?
                                 LL_BLE_US_PER_BIT_CODED_S2 :
                                 LL_BLE_US_PER_BIT_CODED_S8) *
                            LL_TERM2_LEN_BITS;
            ...
            antennaTimeRxPktEnd =
                ...
                -
                TICKS_PER_USEC *
                    NRF5x_PROP_DELAY_RX_USECS; /* subtract propagation delay to get "antenna time" */

            /* set timer trigger for upcoming TIFS transmit operation */
            NRF_TIMER0->CC[0] =
                antennaTimeRxPktEnd /* start with antenna time */
                + TICKS_PER_USEC * LL_BLE_TIFS_US /* add the TIFS period */
                ...
                +
                TICKS_PER_USEC *
                    rxCorrection /* empirically derived trim factor (NOTE!! units are ticks, scaler change affects this) */
                +
                TICKS_PER_USEC *
                    txCorrection /* empirically derived trim factor (NOTE!! units are ticks, scaler change affects this) */
     ...
     * - - - - - - - - - - - - - - - - - - - - - - - - - */
        else if ((bbOpParam.ifsMode == PAL_BB_IFS_MODE_SAME_ABS) && (driverState == RX_STATE)) {
            uint32_t preambleTime;
            uint32_t accessAddrTime;
            int32_t correction;
            ...
            NRF_TIMER0->CC[0] = BB_US_TO_BB_TICKS(
                bbOpParam.ifsTime - (NRF5x_tRXEN_BLE_USECS + NRF5x_PROP_DELAY_RX_USECS));
            ...
            NRF_TIMER0->CC[1] =
                ...
                TICKS_PER_USEC * (30 /* TODO Resolve total deviation allowed */
                                  + preambleTime + accessAddrTime + NRF5x_PROP_DELAY_RX_USECS +
                                  NRF5x_tRXEN_BLE_USECS + correction);
     ...
     * - - - - - - - - - - - - - - - - - - - - - - - - - */
        else if ((bbOpParam.ifsMode == PAL_BB_IFS_MODE_SAME_ABS) && (driverState == TX_STATE)) {
            /* calculate start time */
            NRF_TIMER0->CC[0] = BB_US_TO_BB_TICKS(
                bbOpParam.ifsTime - (NRF5x_tTXEN_BLE_USECS + NRF5x_PROP_DELAY_TX_USECS));
   ...
   * -----------------------------------------------------------------------------*/
     ...
     * - - - - - - - - - - - - - - - - - - - - - - - - - */
       ...
       */
            ...
            switch (bbRxPhy) {
            case BB_PHY_BLE_1M:
                timestamp -= (TICKS_PER_USEC * LL_BLE_US_PER_BYTE_1M *
                              (LL_PREAMBLE_LEN_1M + /* preamble */
                               LL_AA_LEN + /* address */
...
#if defined(NRF52840_XXAA)
                timestamp -=
                    (2 * TICKS_PER_USEC); /* Adjustment based on the sniffer trace observation. */
#endif
                break;
            case BB_PHY_BLE_2M:
                timestamp -= (TICKS_PER_USEC * LL_BLE_US_PER_BYTE_2M *
                              (LL_PREAMBLE_LEN_2M + /* preamble */
                               LL_AA_LEN + /* address */
                               PDU_HEADER_LEN + /* PDU header */
                               pduLen)); /* PDU payload */
...
#if defined(NRF52840_XXAA)
                timestamp +=
                    (3 * TICKS_PER_USEC); /* Adjustment based on the sniffer trace observation. */
#endif
                break;
#if defined(NRF52840_XXAA)
            case BB_PHY_BLE_CODED:
                byteRate = (bbRxPhyOptions == BB_PHY_OPTIONS_BLE_S2) ? LL_BLE_US_PER_BYTE_CODED_S2 :
                                                                       LL_BLE_US_PER_BYTE_CODED_S8;
                correction = (bbRxPhyOptions == BB_PHY_OPTIONS_BLE_S2) ? 20 : 160;
                timestamp -= (TICKS_PER_USEC * LL_BLE_US_PER_BIT_CODED_S8 *
                                  (LL_PREAMBLE_LEN_CODED_BITS + /* preamble */
                                   LL_AA_LEN * 8 + /* address */
                                   LL_CI_LEN_BITS + /* CI */
                                   LL_TERM1_LEN_BITS) + /* TERM1 */
                              TICKS_PER_USEC * byteRate *
                                  (PDU_HEADER_LEN + /* PDU header */
                                   pduLen)); /* PDU payload */
                ...
                timestamp +=
                    ((bbRxPhyOptions == BB_PHY_OPTIONS_BLE_S2) ? -29 : -20) * TICKS_PER_USEC;
                break;
#endif
            default:
                timestamp -= (TICKS_PER_USEC * LL_BLE_US_PER_BYTE_1M *
            ...
            timestampUsesc =
                BB_TICKS_TO_US(bbEventStartTime) + bbDueOffsetUsec + TICKS_TO_USEC(timestamp);
            /* Handle wraparound. */
            ...
            if ((NRF_RADIO->CRCSTATUS & RADIO_CRCSTATUS_CRCSTATUS_Msk) ==
                ...
                bbRxCallback(BB_STATUS_CRC_FAILED, rssi, NRF_RADIO->RXCRC, timestampUsesc,
                             bbRxPhyOptions);
            } else {
                /* Success! - run the callback function */
                bbRxCallback(BB_STATUS_SUCCESS, rssi, NRF_RADIO->RXCRC, timestampUsesc,
                             bbRxPhyOptions);
            }
        }
...
void PalBbBleCancelData(void)
{
    ...
    NVIC_ClearPendingIRQ(
        TIMER0_IRQn); /* just in case we are running in interrupt context above TIMER0 */
...
static void DiagFauxTxCallback(uint8_t status)
{
    ...
    BB_ASSERT((diagOp == DIAG_TX) ||
              (diagOp == DIAG_TX_TIFS)); /* apparent criss-cross of tx/rx, typo somewhere? */
    ...
    if (diagOp == DIAG_TX) {
        if (status == BB_STATUS_SUCCESS) {
            ...
            BB_ASSERT(0);
        } /* unexpected status for the operation */
    ...
    BB_ASSERT(diagTxTotalCalls ==
              (uint16_t)(diagTxSuccess + diagTxFail + diagTxTifsSuccess + diagTxTifsFail));
...
{
    BB_ASSERT(diagOp != DIAG_NULL); /* more than one callback for the operation? */
    BB_ASSERT((diagOp == DIAG_RX) ||
              (diagOp == DIAG_RX_TIFS)); /* apparent criss-cross of tx/rx, typo somewhere? */
    ...
    if (diagOp == DIAG_RX) {
        if (status == BB_STATUS_SUCCESS) {
            ...
            diagRxFail++;
        } else if (status == BB_STATUS_RX_TIMEOUT) {
            diagRxTimeout++;
    ...
    BB_ASSERT(diagRxTotalCalls == (uint16_t)(diagRxSuccess + diagRxFail + diagRxTimeout +
                                             diagRxCrcFail + diagRxTifsSuccess + diagRxTifsFail +
                                             diagRxTifsTimeout + diagRxTifsCrcFail));

    BB_ASSERT(diagRxCalls ==
              (uint16_t)(diagRxSuccess + diagRxFail + diagRxTimeout + diagRxCrcFail));

    BB_ASSERT(diagRxTifsCalls == (uint16_t)(diagRxTifsSuccess + diagRxTifsFail + diagRxTifsTimeout +
                                            diagRxTifsCrcFail));
