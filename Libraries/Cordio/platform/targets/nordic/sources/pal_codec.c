...
static void palCodecSbcOpen(cs47x_Stream_t *pStream)
{
    ...
    case PAL_CODEC_DIR_OUTPUT: {
        OI_CODEC_SBC_DecoderReset(&pStream->sbc.dec, palCodecScratch, sizeof(palCodecScratch), 2, 2,
                                  FALSE);
        break;
    }
...
static uint16_t palCodecSbcEncode(cs47x_Stream_t *pStream, uint8_t *pSbcBuf, uint16_t len)
{
    ...
    if (numBufAvail < 0) {
        AUD_TRACE_WARN2("Input audio stream underrun, id=%u, pktCtr[15:0]=%u", pStream->id,
                        pStream->consCtr);
        ...
        AUD_TRACE_WARN2("Input audio stream overrun, id=%u, pktCtr[15:0]=%u", pStream->id,
                        pStream->consCtr);
 ...
 */
/*************************************************************************************************/
static bool_t palCodecSbcDecode(cs47x_Stream_t *pStream, const uint8_t *pSbcBuf, uint16_t len,
                                uint32_t pktCtr)
{
    ...
    for (size_t i = 0; i < AUDIO_FRM_PER_PKT; i++) {
        ...
        OI_INT16 *pPcmData =
            &pStream->pcm[AUDIO_GET_IDX(pktCtr)][(blockSize / sizeof(int16_t)) * i];
        OI_UINT32 pcmBytes = blockSize;

        OI_STATUS status;

        if ((status = OI_CODEC_SBC_DecodeFrame(&pStream->sbc.dec, &pFrameData, &frameBytes,
...
static void palCodecAifOutFrameComplete(void *pCtx)
{
    ...
    if (numBufAvail > 0) {
        ...
        AUD_TRACE_WARN2("Output audio stream underrun, id=%u, pktCtr[15:0]=%u", pStream->id,
                        pStream->consCtr);
    }
...
static void palCodecAifStart(cs47x_Stream_t *pStream)
{
    ...
    if (pStream->chMask & PAL_CODEC_CH_LEFT_BIT)
        cfg.chan |= PAL_I2S_CH_LEFT_BIT;
    if (pStream->chMask & PAL_CODEC_CH_RIGHT_BIT)
        cfg.chan |= PAL_I2S_CH_RIGHT_BIT;
    ...
    case PAL_CODEC_DIR_INPUT:
        ...
        pStream->prodCtr -= 1;
        PalI2sReadData((uint8_t *)pStream->pcm[AUDIO_GET_IDX(pStream->prodCtr + 1)],
                       AUDIO_BLOCK_LEN);
        PalI2sReadData((uint8_t *)pStream->pcm[AUDIO_GET_IDX(pStream->prodCtr + 2)],
                       AUDIO_BLOCK_LEN);
        break;
...
void PalCodecDataStreamOut(uint16_t id, const uint8_t *pBuf, uint16_t len, uint32_t pktCtr)
{
    ...
    if (pBuf == NULL) {
        AUD_TRACE_WARN2("Missed audio packet; conceal with mute packet, id=%u, pktCtr[15:0]=%u",
                        pStream->id, pktCtr);
    ...
    if (pktCtr == (pStream->prodCtr + 1)) /* Consecutive packet. */
    {
        ...
        if (numBufAvail < 0) {
            ...
            AUD_TRACE_WARN2("Output audio stream overrun, id=%u, pktCtr[15:0]=%u", pStream->id,
                            pStream->consCtr);
        ...
        if (pStream->numProd == (AUDIO_NUM_BLOCK / 2)) {
            AUD_TRACE_WARN2("Output stream synchronized, id=%u pktCtr[15:0]=%u", id,
                            pStream->prodCtr);

            PalI2sWriteData((uint8_t *)pStream->pcm[AUDIO_GET_IDX(pStream->consCtr)],
                            AUDIO_BLOCK_LEN);
            pStream->consCtr++;
            PalI2sWriteData((uint8_t *)pStream->pcm[AUDIO_GET_IDX(pStream->consCtr)],
                            AUDIO_BLOCK_LEN);
        ...
        AUD_TRACE_WARN2("Output stream synchronization initialized/lost, id=%u pktCtr[15:0]=%u", id,
                        pktCtr);
