                      ...
                      uint8_t advChanMap, uint8_t advFiltPolicy)
{
    ...
    if ((LL_API_PARAM_CHECK == 1) &&
        ...
        (((advType != LL_ADV_CONN_DIRECT_HIGH_DUTY) &&
          ((advIntervalMin < rangeMin) || (advIntervalMax < advIntervalMin) ||
           (rangeMax < advIntervalMax))) ||
         (advType > advTypeMax) || (ownAddrType > ownAddrTypeMax) ||
         ...
         (((advType == LL_ADV_CONN_DIRECT_HIGH_DUTY) || (advType == LL_ADV_CONN_DIRECT_LOW_DUTY) ||
           ((ownAddrType & LL_ADDR_IDENTITY_BIT) != 0)) &&
...
uint8_t LlSetScanRespData(uint8_t len, const uint8_t *pData)
{
    ...
    if (len > sizeof(lmgrSlvAdvCb.scanRspData.buf)) {
        LL_TRACE_WARN2("Invalid scan response buffer size, len=%u, maxLen=%u", len,
                       sizeof(lmgrSlvAdvCb.scanRspData.buf));
        return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
    }
...
void LlAdvEnable(uint8_t enable)
{
    ...
    if ((LL_API_PARAM_CHECK == 1) && (enable == TRUE) &&
        ...
        if (lmgrSlvAdvCb.advParam.advType == LL_ADDR_RANDOM_IDENTITY) {
            if (bbBleIsPeerInResList(lmgrSlvAdvCb.advParam.peerAddrType,
                                     lmgrSlvAdvCb.advParam.peerAddr)) {
                if ((pMsg = WsfMsgAlloc(sizeof(*pMsg))) != NULL) {
        ...
        LL_TRACE_WARN1("Address type invalid or not available, ownAddrType=%u",
                       lmgrSlvAdvCb.advParam.ownAddrType);
