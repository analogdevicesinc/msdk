...
static bool_t LlIsBigTestParamsValid(LlCreateBigTest_t *pParam)
{
    ...
    if ((pParam->framing == LL_ISO_PDU_TYPE_UNFRAMED) &&
        ...
        LL_TRACE_WARN0(
            "Unframed PDUs must have ISO_Interval that are multiples of the SDU_Interval");
        return FALSE;
    }
...
{
    LL_TRACE_INFO2("### LlApi ###  LlCreateBig bigHandle=%u advHandle=%u", pCreateBig->bigHandle,
                   pCreateBig->advHandle);

    if ((LL_API_PARAM_CHECK == 1) && (LlIsBigParamsValid(pCreateBig) == FALSE)) {
        return LL_ERROR_CODE_PARAM_OUT_OF_MANDATORY_RANGE;
    }

    if ((LL_API_PARAM_CHECK == 1) && (LctrIsPerAdvEnabled(pCreateBig->advHandle) == FALSE)) {
        LL_TRACE_WARN1("Periodic advertiser not found, advHandle=%u", pCreateBig->advHandle);
        return LL_ERROR_CODE_UNKNOWN_ADV_ID;
    }

    uint8_t supPhyMask = LL_PHYS_LE_1M_BIT | (pLctrRtCfg->phy2mSup ? LL_PHYS_LE_2M_BIT : 0) |
                         (pLctrRtCfg->phyCodedSup ? LL_PHYS_LE_CODED_BIT : 0);

    if ((LL_API_PARAM_CHECK == 1) && (pCreateBig->phys & (~supPhyMask))) {
...
uint8_t LlCreateBigTest(LlCreateBigTest_t *pCreateBigTest)
{
    ...
    LL_TRACE_INFO2("### LlApi ###  LlCreateBigTest bigHandle=%u advHandle=%u",
                   pCreateBigTest->bigHandle, pCreateBigTest->advHandle);

    if ((LL_API_PARAM_CHECK == 1) && (LlIsBigTestParamsValid(pCreateBigTest) == FALSE)) {
        return LL_ERROR_CODE_PARAM_OUT_OF_MANDATORY_RANGE;
    }
    ...
    if ((LL_API_PARAM_CHECK == 1) && (LctrIsPerAdvEnabled(pCreateBigTest->advHandle) == FALSE)) {
    ...
    if ((LL_API_PARAM_CHECK == 1) && (pCreateBigTest->phys & (~supPhyMask))) {
