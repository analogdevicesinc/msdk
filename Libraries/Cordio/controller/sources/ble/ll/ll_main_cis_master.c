...
static bool_t LlIsCigParamsValid(LlCisCigParams_t *pCigParam)
{
    ...
    if ((pCigParam->sduIntervalMToS < MIN_SDU_INTERVAL) ||
        (pCigParam->sduIntervalMToS > MAX_SDU_INTERVAL)) {
        LL_TRACE_INFO0("LlIsCigParamsValid Invalid sduIntervalMToS");
        return FALSE;
    }
    if ((pCigParam->sduIntervalSToM < MIN_SDU_INTERVAL) ||
        (pCigParam->sduIntervalSToM > MAX_SDU_INTERVAL)) {
    ...
    if ((pCigParam->transLatMToS < MIN_TRANS_LATENCY) ||
        (pCigParam->transLatMToS > MAX_TRANS_LATENCY)) {
        LL_TRACE_INFO0("LlIsCigParamsValid Invalid transLatMToS");
        return FALSE;
    }
    if ((pCigParam->transLatSToM < MIN_TRANS_LATENCY) ||
        (pCigParam->transLatSToM > MAX_TRANS_LATENCY)) {
...
static bool_t LlIsCigPhyParamsValid(LlCisCigParams_t *pCigParam)
{
    for (unsigned int i = 0; i < pCigParam->numCis; i++) {
        ...
        if (BB_SYM_PHY_REQ && (pCisParam->phyMToS != pCisParam->phySToM)) {
            return FALSE;
        }
    }
    return TRUE;
...
static bool_t LlIsCigTestParamsValid(LlCisCigParamsTest_t *pSetCigParamTest)
{
    ...
    if ((pSetCigParamTest->sduIntervalMToS < MIN_SDU_INTERVAL) ||
        (pSetCigParamTest->sduIntervalMToS > MAX_SDU_INTERVAL)) {
        return FALSE;
    }
    if ((pSetCigParamTest->sduIntervalSToM < MIN_SDU_INTERVAL) ||
        (pSetCigParamTest->sduIntervalSToM > MAX_SDU_INTERVAL)) {
        return FALSE;
    }
    ...
    if ((pSetCigParamTest->isoInterval < MIN_ISO_INTERVAL) ||
        (pSetCigParamTest->isoInterval > MAX_ISO_INTERVAL)) {
        return FALSE;
    }
...
uint8_t LlSetCigParams(LlCisCigParams_t *pCigParam, uint16_t *pCisHandles)
{
    ...
    LL_TRACE_INFO1("### LlApi ###  LlSetCigParams sduSizeMToS=%d",
                   pCigParam->pCisParam[0].sduSizeMToS);
    LL_TRACE_INFO1("### LlApi ###  LlSetCigParams sduSizeSToM=%d",
                   pCigParam->pCisParam[0].sduSizeSToM);
    ...
    if ((LL_API_PARAM_CHECK == 1) && (LlIsCigParamsValid(pCigParam) == FALSE)) {
        LL_TRACE_INFO0("### LlApi ###  LlSetCigParams Invalid Parameters");
        return LL_ERROR_CODE_PARAM_OUT_OF_MANDATORY_RANGE;
    }

    if ((LL_API_PARAM_CHECK == 1) && (LlIsCigPhyParamsValid(pCigParam) == FALSE)) {
...
uint8_t LlSetCigParamsTest(LlCisCigParamsTest_t *pSetCigParamTest, uint16_t *pCisHandles)
{
    ...
    if ((LL_API_PARAM_CHECK == 1) && (LlIsCigTestParamsValid(pSetCigParamTest) == FALSE)) {
        LL_TRACE_INFO0("### LlApi ###  LlSetCigParamsTest Invalid Parameters");
        return LL_ERROR_CODE_PARAM_OUT_OF_MANDATORY_RANGE;
    }

    if ((LL_API_PARAM_CHECK == 1) && (LlIsCigPhyTestParamsValid(pSetCigParamTest) == FALSE)) {
        LL_TRACE_INFO0("### LlApi ###  LlSetCigParams Invalid PHY: Not supported by controller");
        return LL_ERROR_CODE_UNSUPPORTED_FEATURE_PARAM_VALUE;
    }
