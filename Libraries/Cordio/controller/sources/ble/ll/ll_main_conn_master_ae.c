...
static bool_t llValidateInitiateParams(const LlExtInitParam_t *pParam)
{
    ...
    if ((LL_API_PARAM_CHECK == 1) &&
        ((pParam->ownAddrType > addrTypeMax) || (pParam->peerAddrType > addrTypeMax) ||
                        ...
                        const LlConnSpec_t connSpec[])
{
    ...
    if ((LL_API_PARAM_CHECK == 1) && (!LmgrIsExtCommandAllowed() || lmgrCb.numInitEnabled)) {
        LL_TRACE_WARN0(
    ...
    if ((LL_API_PARAM_CHECK == 1) && (!llValidateInitiateParams(pInitParam))) {
        return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
    }

    if (LL_API_PARAM_CHECK == 1) {
        for (unsigned int i = 0; i < numInitPhyBits; i++) {
            if ((llValidateConnSpecParams(&connSpec[i]) != LL_SUCCESS) ||
                (!llValidateInitiateScanParams(&scanParam[i]))) {
                return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
            }
        }
    }
