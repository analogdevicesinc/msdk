...
uint32_t llCalcPacketTime(uint8_t len, uint8_t phy, uint8_t phyOptions)
{
    ...
    case BB_PHY_BLE_CODED:
        totalTime =
            (LL_PREAMBLE_LEN_CODED_BITS + (LL_AA_LEN * 8) + LL_CI_LEN_BITS + LL_TERM1_LEN_BITS) *
            LL_BLE_US_PER_BIT_CODED_S8;
        switch (phyOptions) {
        case BB_PHY_OPTIONS_BLE_S2:
            totalTime += ((LL_DTM_HDR_LEN + len + LL_CRC_LEN) * LL_BLE_US_PER_BYTE_CODED_S2) +
                         (LL_TERM2_LEN_BITS * LL_BLE_US_PER_BIT_CODED_S2);
            break;
        default:
        case BB_PHY_OPTIONS_BLE_S8:
            totalTime += ((LL_DTM_HDR_LEN + len + LL_CRC_LEN) * LL_BLE_US_PER_BYTE_CODED_S8) +
                         (LL_TERM2_LEN_BITS * LL_BLE_US_PER_BIT_CODED_S8);
            break;
        }
        break;
    case BB_PHY_BLE_2M:
        totalTime = (LL_PREAMBLE_LEN_2M + LL_AA_LEN + LL_DTM_HDR_LEN + len + LL_CRC_LEN) *
                    LL_BLE_US_PER_BYTE_2M;
        break;
    case BB_PHY_BLE_1M:
    default:
        totalTime = (LL_PREAMBLE_LEN_1M + LL_AA_LEN + LL_DTM_HDR_LEN + len + LL_CRC_LEN) *
                    LL_BLE_US_PER_BYTE_1M;
        break;
    }
...
static void llTestTxOpEndCback(BbOpDesc_t *pOp)
{
    ...
    if (llTestCb.state == LL_TEST_STATE_TX) {
        ...
        if ((llTestCb.tx.chanIdx != pBle->chan.chanIdx) ||
            ...
            pTx->pktInterUsec = llCalcPacketInterval(llTestCb.tx.pduLen, pBle->chan.txPhy,
...
uint8_t LlEnhancedTxTest(uint8_t rfChan, uint8_t len, uint8_t pktType, uint8_t phy, uint16_t numPkt)
{
    ...
    if ((LL_API_PARAM_CHECK == 1) &&
        (!((llTestCb.state == LL_TEST_STATE_IDLE) || (llTestCb.state == LL_TEST_STATE_TX)) ||
         (lmgrCb.numConnEnabled > 0) || lmgrCb.advEnabled || lmgrCb.numExtAdvEnabled ||
         lmgrCb.numScanEnabled || lmgrCb.numInitEnabled)) {
        return LL_ERROR_CODE_CMD_DISALLOWED;
    }

    /* Extra parameter checks for dynamic changes. */
    if ((LL_API_PARAM_CHECK == 1) && (llTestCb.state == LL_TEST_STATE_TX)) {
        ...
        if (((pktType == LL_TEST_PKT_TYPE_PRBS15) &&
            ...
            ((pktType != LL_TEST_PKT_TYPE_PRBS15) &&
             (llTestCb.tx.pktType == LL_TEST_PKT_TYPE_PRBS15))) {
            return LL_ERROR_CODE_CMD_DISALLOWED;
        }
    }
    ...
    WSF_CS_ENTER();
    llTestCb.tx.pduLen =
        WSF_MIN(len, WSF_MAX(LL_ADVB_MAX_LEN - LL_ADV_HDR_LEN, LL_DTM_MAX_PLD_LEN));
    llTestCb.tx.pktType = pktType;
                   ...
                   uint8_t cteType, uint8_t swPatLen, uint8_t *antIds, uint16_t numPkt)
{
    ...
    retval = LlEnhancedTxTest(rfChan, len, pktType, phy, numPkt);
    if (retval != LL_SUCCESS) {
        return retval;
    }

    LL_TRACE_INFO2("### LlApi ###  LlTxTestV3, cteLen=%u cteType=%u", cteLen, cteType);
    LL_TRACE_INFO1("    swPatLen=%u", swPatLen);
    LL_TRACE_INFO0("    Antenna IDs:");
    for (i = 0; i < swPatLen; i++) {
        LL_TRACE_INFO1("    0x%x", antIds[i]);
    }
...
static void llTestRxOpEndCback(BbOpDesc_t *pOp)
{
    ...
    if (llTestCb.numPkt > 0) {
        uint32_t attempts =
            llTestCb.rpt.numRxSuccess + llTestCb.rpt.numRxCrcError + llTestCb.rpt.numRxTimeout;
...
uint8_t LlEnhancedRxTest(uint8_t rfChan, uint8_t phy, uint8_t modIdx, uint16_t numPkt)
{
    ...
    if ((LL_API_PARAM_CHECK == 1) &&
        ((llTestCb.state != LL_TEST_STATE_IDLE) || (lmgrCb.numConnEnabled > 0) ||
         lmgrCb.advEnabled || lmgrCb.numExtAdvEnabled || lmgrCb.numScanEnabled ||
         lmgrCb.numInitEnabled)) {
        LL_TRACE_INFO1("Command Disallowed %u", llTestCb.state);
                   ...
                   uint8_t slotDur, uint8_t swPatLen, uint8_t *antIds, uint16_t numPkt)
{
    ...
    retval = LlEnhancedRxTest(rfChan, phy, modIdx, numPkt);
    if (retval != LL_SUCCESS) {
        return retval;
    }

    LL_TRACE_INFO3("### LlApi ###  LlRxTestV3, cteLen=%u cteType=%u slotDur=%u", cteLen, cteType,
                   slotDur);
    LL_TRACE_INFO1("    swPatLen=%u", swPatLen);
    LL_TRACE_INFO0("    Antenna IDs:");
    for (i = 0; i < swPatLen; i++) {
