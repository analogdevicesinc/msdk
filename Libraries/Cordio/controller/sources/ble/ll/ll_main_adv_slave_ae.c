...
uint8_t LlSetExtAdvParam(uint8_t handle, LlExtAdvParam_t *pExtAdvParam)
{
    ...
    const uint16_t highDutyConnDirectAdvProp =
    ...
    LL_TRACE_INFO2("### LlApi ###  LlSetExtAdvParam, handle=%u, advEventProp=0x%04x", handle,
                   pExtAdvParam->advEventProp);
    ...
    if ((LL_API_PARAM_CHECK == 1) && (handle > LL_MAX_ADV_HANDLE)) {
    ...
    if ((LL_API_PARAM_CHECK == 1) &&
        ((pExtAdvParam->advEventProp & ~validAdvEventProp) ||
         ((pExtAdvParam->advEventProp &
           (LL_ADV_EVT_PROP_LEGACY_ADV_BIT | LL_ADV_EVT_PROP_CONN_ADV_BIT |
         ...
         ((pExtAdvParam->advEventProp & LL_ADV_EVT_PROP_LEGACY_ADV_BIT) &&
          (pExtAdvParam->advEventProp &
           (LL_ADV_EVT_PROP_OMIT_AA_BIT | LL_ADV_EVT_PROP_TX_PWR_BIT))))) {
    ...
    if ((LL_API_PARAM_CHECK == 1) &&
        ((pExtAdvParam->advEventProp & LL_ADV_EVT_PROP_OMIT_AA_BIT) &&
         (pExtAdvParam->advEventProp &
          (LL_ADV_EVT_PROP_CONN_ADV_BIT | LL_ADV_EVT_PROP_SCAN_ADV_BIT)))) {
        LL_TRACE_WARN1(
            "Only non-connectable and non-scannable advertising can do anonymous advertising, handle=%u",
            handle);
        return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
    }
    if ((LL_API_PARAM_CHECK == 1) &&
        ((((pExtAdvParam->advEventProp & LL_ADV_EVT_PROP_LEGACY_ADV_BIT) != 0) &&
         ...
         (((pExtAdvParam->advEventProp & LL_ADV_EVT_PROP_LEGACY_ADV_BIT) == 0) &&
          (pExtAdvParam->priAdvPhy != LL_PHY_LE_1M) &&
          (pExtAdvParam->priAdvPhy != LL_PHY_LE_CODED)) ||
         (((pExtAdvParam->advEventProp & LL_ADV_EVT_PROP_LEGACY_ADV_BIT) == 0) &&
          (pExtAdvParam->secAdvPhy != LL_PHY_LE_1M) && (pExtAdvParam->secAdvPhy != LL_PHY_LE_2M) &&
          (pExtAdvParam->secAdvPhy != LL_PHY_LE_CODED)))) {
        LL_TRACE_WARN1("Invalid PHY for legacy advertising, handle=%u", handle);
        return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
    }
    if ((LL_API_PARAM_CHECK == 1) && (((pExtAdvParam->secAdvPhy == LL_PHY_LE_2M) &&
                                       ((lmgrCb.features & LL_FEAT_LE_2M_PHY) == 0)) ||
                                      (((pExtAdvParam->priAdvPhy == LL_PHY_LE_CODED) ||
                                        (pExtAdvParam->secAdvPhy == LL_PHY_LE_CODED)) &&
...
void LlExtAdvEnable(uint8_t enable, uint8_t numAdvSets, LlExtAdvEnableParam_t enaParam[])
{
    ...
    if ((LL_API_PARAM_CHECK == 1) &&
        (((numAdvSets > LL_MAX_ADV_SETS)) || ((numAdvSets == 0) && enable))) {
        lmgrCb.extAdvEnaDelayCnt = 1;
...
{
    LL_TRACE_INFO2("### LlApi ###  LlSetPeriodicAdvParam, handle=%u, perAdvProp=0x%04x", handle,
                   pPerAdvParam->perAdvProp);
...
uint8_t LlSetPeriodicAdvData(uint8_t handle, uint8_t op, uint8_t len, const uint8_t *pData)
{
    ...
    if ((LL_API_PARAM_CHECK == 1) && (op > LL_ADV_DATA_OP_COMP)) {
        return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
    }
...
{
    LL_TRACE_INFO2("### LlApi ###  LlSetAuxOffsetDelay: handle=%u, delayUsec=%u", handle,
                   delayUsec);
...
{
    LL_TRACE_INFO3(
        "### LlApi ###  LlSetExtAdvTxPhyOptions: handle=%u, priPhyOpts=%u, secPhyOpts=%u", handle,
        priPhyOpts, secPhyOpts);
