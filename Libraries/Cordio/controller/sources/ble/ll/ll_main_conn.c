...
uint8_t LlGetTxPowerLevel(uint16_t handle, uint8_t type, int8_t *pLevel)
{
    ...
    if ((LL_API_PARAM_CHECK == 1) &&
        ...
        *pLevel = LL_MIN_TX_PWR_LVL;
        result = LL_ERROR_CODE_UNKNOWN_CONN_ID;
    } else if ((LL_API_PARAM_CHECK == 1) && (type > LL_TX_PWR_LVL_TYPE_MAX)) {
        *pLevel = LL_MIN_TX_PWR_LVL;
    ...
    LL_TRACE_INFO3("### LlApi ###  LlGetTxPowerLevel, handle=%u, type, level=%d", handle, type,
                   *pLevel);
...
uint8_t LlGetEnhTxPowerLevel(uint16_t handle, uint8_t phy, int8_t *pCurPwr, int8_t *pMaxPwr)
{
    ...
    LL_TRACE_INFO3("### LlApi ###  LlGetEnhancedTxPowerLevel, handle=%u, phy=%u, curPower=%d",
                   handle, phy, *pCurPwr);
...
uint8_t LlSetPhyTxPowerLevel(uint16_t handle, int8_t level, uint8_t phy)
{
    ...
    LL_TRACE_INFO3("### LlApi ###  LlSetPhyTxPowerLevel, handle=%u, level=%d, phy=%u", handle,
                   level, phy);
...
{
    const uint32_t flagAll = LL_OP_MODE_FLAG_SLV_REQ_IMMED_ACK | LL_OP_MODE_FLAG_BYPASS_CE_GUARD |
                             LL_OP_MODE_FLAG_MST_RETX_AFTER_RX_NACK |
                             LL_OP_MODE_FLAG_MST_IGNORE_CP_RSP | LL_OP_MODE_FLAG_MST_UNCOND_CP_RSP |
                             LL_OP_MODE_FLAG_REQ_SYM_PHY | LL_OP_MODE_FLAG_ENA_SLV_LATENCY_WAKEUP |
                             LL_OP_MODE_FLAG_ENA_WW | LL_OP_MODE_FLAG_ENA_SLV_LATENCY |
    ...
    if ((LL_API_PARAM_CHECK == 1) && ((~flagAll & flags) != 0)) {
...
uint8_t LlDisconnect(uint16_t handle, uint8_t reason)
{
    ...
    if ((LL_API_PARAM_CHECK == 1) && (handle >= (LCTR_MAX_HANDLE_INDEX))) {
        return LL_ERROR_CODE_UNKNOWN_CONN_ID;
    }

    if ((LL_API_PARAM_CHECK == 1) && (handle < pLctrRtCfg->maxConn)) {
...
uint8_t LlConnUpdate(uint16_t handle, const LlConnSpec_t *pConnSpec)
{
    ...
    if ((LL_API_PARAM_CHECK == 1) && ((status = LctrValidateConnSpec(pConnSpec)) != LL_SUCCESS)) {
        return status;
    }
...
uint8_t LlRemoteConnParamReqNegReply(uint16_t handle, uint8_t reason)
{
    ...
    LL_TRACE_INFO2("### LlApi ###  LlRemoteConnParamReqNegReply, handle=%u, reason=%u", handle,
                   reason);
...
uint8_t LlSetDataLen(uint16_t handle, uint16_t txLen, uint16_t txTime)
{
    ...
    if ((LL_API_PARAM_CHECK == 1) &&
        ((txLen < LL_MAX_DATA_LEN_MIN) || (txLen > LL_MAX_DATA_LEN_ABS_MAX) ||
         (txTime < LL_MAX_DATA_TIME_MIN) ||
         ((pLctrRtCfg->btVer <= LL_VER_BT_CORE_SPEC_4_2) &&
          (txTime > LL_MAX_DATA_TIME_ABS_MAX_1M)) ||
...
uint8_t LlWriteDefaultDataLen(uint16_t maxTxLen, uint16_t maxTxTime)
{
    ...
    if ((LL_API_PARAM_CHECK == 1) &&
        ((maxTxLen < LL_MAX_DATA_LEN_MIN) || (maxTxLen > LL_MAX_DATA_LEN_ABS_MAX) ||
         (maxTxTime < LL_MAX_DATA_TIME_MIN) ||
         ((pLctrRtCfg->btVer <= LL_VER_BT_CORE_SPEC_4_2) &&
          (maxTxTime > LL_MAX_DATA_TIME_ABS_MAX_1M)) ||
         ((pLctrRtCfg->btVer >= LL_VER_BT_CORE_SPEC_5_0) &&
          (maxTxTime > LL_MAX_DATA_TIME_ABS_MAX)))) {
...
uint8_t LlSetLocalMinUsedChan(uint8_t phys, int8_t pwrThres, uint8_t minUsedCh)
{
    ...
    if ((LL_API_PARAM_CHECK == 1) && ((lmgrCb.features & LL_FEAT_MIN_NUM_USED_CHAN) == 0)) {
        return LL_ERROR_CODE_CMD_DISALLOWED;
    } else if ((LL_API_PARAM_CHECK == 1) &&
               ((phys == 0) || (phys >= (1 << LL_MAX_PHYS)) || (minUsedCh > LL_MAX_NUM_CHAN_DATA) ||
    ...
    for (handle = 0; handle < pLctrRtCfg->maxConn; handle++) {
        if ((LctrIsConnHandleEnabled(handle)) && (LctrGetRole(handle) == LL_ROLE_SLAVE)) {
...
uint8_t LlGetPeerMinUsedChan(uint16_t handle, uint8_t *pPeerMinUsedChan)
{
    ...
    if ((LL_API_PARAM_CHECK == 1) &&
        ((handle >= pLctrRtCfg->maxConn) || !LctrIsConnHandleEnabled(handle))) {
        result = LL_ERROR_CODE_UNKNOWN_CONN_ID;
    } else if ((LL_API_PARAM_CHECK == 1) && (((lmgrCb.features & LL_FEAT_MIN_NUM_USED_CHAN) == 0) ||
...
uint8_t LlModifySleepClockAccuracy(uint8_t action)
{
    ...
    if ((LL_API_PARAM_CHECK == 1) && (action > LL_MODIFY_SCA_LESS_ACCURATE)) {
        return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
    }
...
uint8_t LlPowerCtrlReq(uint16_t handle, int8_t delta, uint8_t phy)
{
    ...
    LL_TRACE_INFO3("### LlApi ###  LlPowerCtrlReq, handle=%u, delta=%u, phy=%u", handle, delta,
                   phy);
    ...
    if ((LL_API_PARAM_CHECK == 1) && (phy > LL_PC_PHY_TOTAL)) {
        return LL_ERROR_CODE_UNSUPPORTED_FEATURE_PARAM_VALUE;
    } else if ((LL_API_PARAM_CHECK == 1) &&
...
{
    LL_TRACE_INFO3(
        "### LlApi ###  LlSetTxPowerReporting, handle=%u, localEnable=%u, remoteEnable=%u", handle,
        enableLocal, enableRemote);
