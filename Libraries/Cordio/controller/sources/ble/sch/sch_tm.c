 ...
 */
/*************************************************************************************************/
void SchTmAdd(uint8_t handle, uint32_t interUsec, uint32_t durUsec, bool_t movable,
...
{
    uint32_t refEnd =
        refBegin + durUsec + BbGetSchSetupDelayUs(); /* End time of the link to be compared. */
    ...
    for (i = 0; i < SCH_TM_MAX_LINK; i++) {
        if ((schTmCb.tlink[i].enabled == TRUE) && schTmCb.tlink[i].refTimeCb) {
            ...
            if (((interUsec > schTmCb.tlink[i].interUsec) &&
                 (schRmIntCalculateDepth(interUsec, schTmCb.tlink[i].interUsec) == 0)) ||
                ((interUsec < schTmCb.tlink[i].interUsec) &&
                 (schRmIntCalculateDepth(schTmCb.tlink[i].interUsec, interUsec) == 0))) {
                continue;
            }
            ...
            while (!SCH_TM_IS_IN_FUTURE(linkBegin, refEnd)) {
                ...
                if (!SCH_TM_IS_IN_PAST(linkEnd, refBegin)) {
                    LL_TRACE_WARN1("SchTmCheckConflict, ======>>> Found conflict with TM link %u ",
                                   i);
                    return TRUE;
                }

                /* Use the smaller interval to check conflicts. */
                linkBegin += (interUsec < schTmCb.tlink[i].interUsec) ? interUsec :
                                                                        schTmCb.tlink[i].interUsec;
                linkEnd = linkBegin + schTmCb.tlink[i].durUsec + BbGetSchSetupDelayUs();
            }
        }
    }
                             ...
                             uint32_t durUsec)
{
        ...
        if ((schTmCb.tlink[i].enabled == TRUE) && schTmCb.tlink[i].refTimeCb) {
            ...
            if (((interUsec > schTmCb.tlink[i].interUsec) &&
                ...
                if (numNode == 0) {
                    ...
                    schTmNodeCb.tnode[numNode].tEnd =
                        schTmNodeCb.tnode[numNode].tBegin + schTmCb.tlink[i].durUsec;
    ...
    while (TRUE) {
        ...
        if (curLink != 0xFF) {
            ...
            if (SCH_TM_IS_IN_FUTURE(nextBegin, lastEnd)) {
                ...
                /* Overlap. Merge nodes. */
                schTmNodeCb.tnode[numNode - 1].tEnd =
                    SCH_TM_IS_IN_FUTURE(nextEnd, lastEnd) ? nextEnd : lastEnd;
            }
        } else {
            /* Finished sorting. */
            break;
    ...
    if (numNode > 0) {
        ...
        if (SCH_TM_IS_IN_FUTURE(nextBegin, lastEnd)) {
            ...
            /* Overlap. Merge nodes. */
            schTmNodeCb.tnode[numNode - 1].tEnd = SCH_TM_IS_IN_FUTURE(nextEnd, lastEnd) ? nextEnd :
                                                                                          lastEnd;
        }
    }
