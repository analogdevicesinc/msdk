...
#define SCH_AUX_ADV_NO_CHAIN_HDR_LEN \
    (LL_ADV_HDR_LEN + 11) /* Includes extended header len, Flags, AdvA, Txpower, ADI. */

/*! \brief      Estimated header length for AUX_ADV_IND with chained packets. */
#define SCH_AUX_ADV_W_CHAIN_HDR_LEN \
    (LL_ADV_HDR_LEN + 14) /* Includes extended header len, Flags, AdvA, Txpower, ADI, AuxPtr. */

/*! \brief      Estimated header length for periodic advertisement with no chained packets. */
#define SCH_PER_ADV_NO_CHAIN_HDR_LEN \
    (LL_ADV_HDR_LEN + 3) /* Includes extended header len, Flags, Txpower. */

/*! \brief      Estimated header length for periodic advertisement with chained packets. */
#define SCH_PER_ADV_W_CHAIN_HDR_LEN \
    (LL_ADV_HDR_LEN + 8) /* Includes extended header len, Flags, Txpower, AuxPtr, ADI. */
...
uint32_t SchBleCalcAdvPktDurationUsec(uint8_t phy, uint8_t phyOptions, uint16_t len)
{
    ...
    case BB_PHY_BLE_2M:
        ...
        break;
    case BB_PHY_BLE_CODED:
        usec = (LL_PREAMBLE_LEN_CODED_BITS + (LL_AA_LEN * 8) + LL_CI_LEN_BITS + LL_TERM1_LEN_BITS) *
               LL_BLE_US_PER_BIT_CODED_S8;
        usec += ((phyOptions == BB_PHY_OPTIONS_BLE_S2) ? LL_BLE_US_PER_BIT_CODED_S2 :
                                                         LL_BLE_US_PER_BIT_CODED_S8) *
 ...
 */
/*************************************************************************************************/
uint32_t SchBleCalcPerAdvDurationUsec(uint8_t txPhy, uint8_t fragLen, uint16_t addMafDelay,
                                      uint16_t len, bool_t worstCase, uint8_t phyOptions)
{
    static uint16_t perAdvHdrMaxLen = LL_ADV_HDR_LEN + LL_EXT_ADV_HDR_MAX_LEN;

    uint16_t headerLenNoChain = (worstCase == TRUE) ? perAdvHdrMaxLen :
                                                      SCH_PER_ADV_NO_CHAIN_HDR_LEN;
    ...
    if (len <= maxDataBytesNoChain) {
        ...
        usec = numFragWithAuxPtr * SchBleCalcAdvPktDurationUsec(
                                       txPhy, phyOptions, headerLenChain + maxDataBytesWithChain);
...
void SchBleCalcAdvOpDuration(BbOpDesc_t *pBod, uint8_t fragLen)
{
    ...
    case BB_BLE_OP_MST_ADV_EVENT: {
        ...
        if (pAdv->pTxReqBuf) {
            ...
            minDurUsec += SchBleCalcAdvPktDurationUsec(pBle->chan.txPhy, BB_PHY_OPTIONS_DEFAULT,
                                                       pBle->op.mstAdv.txReqLen);
    ...
    case BB_BLE_OP_SLV_ADV_EVENT: {
        uint32_t pktDuration = SchBleCalcAdvPktDurationUsec(
            pBle->chan.txPhy, pBle->chan.initTxPhyOptions, pBle->op.slvAdv.txAdvLen);
        ...
        minDurUsec =
            (numChan - 1) * SchBleGetAlignedAuxOffsUsec(pktDuration + BbGetSchSetupDelayUs());
        ...
        if (pAdv->pRxReqBuf) {
            ...
            if (pAdv->pTxRspBuf) {
                ...
                minDurUsec +=
                    (SchBleCalcAdvPktDurationUsec(pBle->chan.txPhy, BB_PHY_OPTIONS_DEFAULT,
                                                  pBle->op.mstAdv.txReqLen) *
                     numChan);
            }
        }
    ...
    case BB_BLE_OP_MST_AUX_ADV_EVENT: {
        ...
        if (pAdv->isInit == FALSE) {
            ...
            if (pAdv->pTxAuxReqBuf) {
                ...
                minDurUsec += SchBleCalcAdvPktDurationUsec(pBle->chan.txPhy,
            ...
            if (pAdv->pTxAuxReqBuf) {
                ...
                minDurUsec += SchBleCalcAdvPktDurationUsec(
                    pBle->chan.txPhy,
                    (pBle->chan.tifsTxPhyOptions != BB_PHY_OPTIONS_DEFAULT) ?
                        pBle->chan.tifsTxPhyOptions :
                        BB_PHY_OPTIONS_BLE_S8,
                    LL_ADV_HDR_LEN + LL_CONN_IND_PDU_LEN); /* aux_conn_req */
                minDurUsec += LL_BLE_TIFS_US;
                /* Assume longest time, coded S8. */
                minDurUsec += SchBleCalcAdvPktDurationUsec(
                    pBle->chan.rxPhy, BB_PHY_OPTIONS_BLE_S8,
                    LL_ADV_HDR_LEN + LL_CONN_RSP_PDU_LEN); /* aux_conn_rsp */
            }
        }
        break;
    }
    case BB_BLE_OP_SLV_AUX_ADV_EVENT: {
        ...
        uint16_t maxDataBytesNoChain =
            (fragLen == 0) ? (LL_EXT_ADVB_MAX_LEN - SCH_AUX_ADV_NO_CHAIN_HDR_LEN) : fragLen;
        uint16_t maxDataBytesWithChain =
            (fragLen == 0) ? (LL_EXT_ADVB_MAX_LEN - SCH_AUX_ADV_W_CHAIN_HDR_LEN) : fragLen;
        BbBleSlvAuxAdvEvent_t *const pAuxAdv = &pBle->op.slvAuxAdv;

        if (pAuxAdv->pRxAuxReqBuf) /* Scannable advertising */
        {
            minDurUsec += SchBleCalcAdvPktDurationUsec(
                pBle->chan.txPhy, pBle->chan.initTxPhyOptions, SCH_AUX_ADV_NO_CHAIN_HDR_LEN);
            minDurUsec += LL_BLE_TIFS_US;
        ...
        if (dataLen <= maxDataBytesNoChain) {
            minDurUsec += SchBleCalcAdvPktDurationUsec(
                pBle->chan.txPhy, pBle->chan.initTxPhyOptions, extHeadLen + dataLen);
        } else {
            uint16_t remDataLen;
            ...
            minDurUsec +=
                numFragWithAuxPtr *
                SchBleCalcAdvPktDurationUsec(pBle->chan.txPhy, pBle->chan.initTxPhyOptions,
                                             SCH_AUX_ADV_W_CHAIN_HDR_LEN + maxDataBytesWithChain);
            ...
            if (remDataLen) {
                minDurUsec +=
                    SchBleCalcAdvPktDurationUsec(pBle->chan.txPhy, pBle->chan.initTxPhyOptions,
                                                 SCH_AUX_ADV_NO_CHAIN_HDR_LEN + remDataLen);
            }
        }
    ...
    case BB_BLE_OP_SLV_PER_ADV_EVENT: {
        ...
        minDurUsec = SchBleCalcPerAdvDurationUsec(pBle->chan.txPhy, 0, 0, dataLen, FALSE,
