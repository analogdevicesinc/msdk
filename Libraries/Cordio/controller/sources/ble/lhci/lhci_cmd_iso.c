 ...
 */
/*************************************************************************************************/
static uint8_t lhciPackReadBufSizeV2Evt(uint8_t *pBuf, uint8_t status, uint16_t aclPktLen,
                                        uint8_t aclNumPkts, uint16_t isoBufSize, uint8_t numIsoPkts)
{
 ...
 */
/*************************************************************************************************/
static uint8_t lhciPackReadIsoTxSyncEvt(uint8_t *pBuf, uint8_t status, uint16_t handle,
                                        uint16_t pktSn, uint32_t ts, uint32_t timeOffs)
{
 ...
 */
/*************************************************************************************************/
static uint8_t lhciPackIsoReadTestCounters(uint8_t *pBuf, uint8_t status, uint16_t handle,
 ...
 */
/*************************************************************************************************/
static uint8_t lhciPackReadIsoLinkQuality(uint8_t *pBuf, uint8_t status, uint16_t handle,
                                          LlIsoLinkQual_t LinkQualStats)
{
 ...
 */
/*************************************************************************************************/
static uint8_t lhciPackReadLocalSupportedCodecs(uint8_t *pBuf, uint8_t status, uint8_t numStd,
                                                AudioStdCodecInfo_t stdCodecs[], uint8_t numVs,
                                                AudioVsCodecInfo_t vsCodecs[])
{
 ...
 */
/*************************************************************************************************/
static void lhciIsoSendCmdCmplEvt(LhciHdr_t *pCmdHdr, uint8_t status, uint8_t paramLen,
                                  uint16_t handle, const lhciIsoReturnParam_t *pRet)
{
    uint8_t *pBuf;
    uint8_t *pEvtBuf;
    ...
    case HCI_OPCODE_READ_LOCAL_SUP_CODECS: {
        ...
        PalCodecReadLocalSupportedCodecs(&numStdCodecs, stdCodecs, &numVsCodecs, vsCodecs);

        paramLen = lhciPackReadLocalSupportedCodecs(pBuf, status, numStdCodecs, stdCodecs,
                                                    numVsCodecs, vsCodecs);

        /* Re-pack the event header now that the actual length is known. */
        lhciPackEvtHdr(pEvtBuf - (HCI_EVT_HDR_LEN + HCI_LEN_CMD_CMPL), HCI_CMD_CMPL_EVT,
                       HCI_LEN_CMD_CMPL + paramLen);
        break;
    }
...
bool_t lhciIsoDecodeCmdPkt(LhciHdr_t *pHdr, uint8_t *pBuf)
{
    ...
    case HCI_OPCODE_READ_LOCAL_SUP_CODECS: {
        ...
        paramLen = LHCI_LEN_READ_LOCAL_SUP_CODECS + (LHCI_MAX_CODEC * 2) + /* Max standard codecs */
