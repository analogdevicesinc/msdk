...
#define LHCI_LEN_READ_AUTH_PAYLOAD_TO_EVT \
    5 /*!< Read authenticated payload timeout command complete event length. */
#define LHCI_LEN_WRITE_AUTH_PAYLOAD_TO_EVT \
    3 /*!< Write authenticated payload timeout command complete event length. */

/* --- Informational commands --- */
#define LHCI_LEN_READ_LOCAL_VER_EVT 9 /*!< Read local version command complete event length. */
#define LHCI_LEN_READ_LOCAL_SUP_CMDS_EVT \
    65 /*!< Read local supported commands command complete event length. */
#define LHCI_LEN_READ_LOCAL_SUP_FEAT_EVT \
    9 /*!< Read local supported feature command complete event length. */
...
#define LHCI_LEN_LE_READ_LOCAL_SUP_FEAT_EVT \
    9 /*!< LE read local supported feature command complete event length. */
#define LHCI_LEN_LE_SET_RAND_ADDR_EVT 1 /*!< LE set random address command complete event length. */
#define LHCI_LEN_LE_SET_ADV_PARAM_EVT \
    1 /*!< LE set advertising parameter command complete event length. */
#define LHCI_LEN_LE_READ_ADV_TX_POWER_EVT \
    2 /*!< LE read advertising Tx power command complete event length. */
#define LHCI_LEN_LE_SET_ADV_DATA_EVT \
    1 /*!< LE set advertising data command complete event length. */
#define LHCI_LEN_LE_SET_SCAN_RESP_DATA_EVT \
    1 /*!< LE set scan response data command complete event length. */
#define LHCI_LEN_LE_SET_ADV_ENABLE_EVT \
    1 /*!< LE set advertising enable command complete event length. */
#define LHCI_LEN_LE_SET_SCAN_PARAM_EVT \
    1 /*!< LE set scan parameter command complete event length. */
#define LHCI_LEN_LE_SET_SCAN_ENABLE_EVT 1 /*!< LE set scan enable command complete event length. */
#define LHCI_LEN_LE_CREATE_CONN_CANCEL_EVT \
    1 /*!< LE create connection cancel command complete event length. */
#define LHCI_LEN_LE_READ_WL_SIZE_EVT \
    2 /*!< LE read white list size command complete event length. */
#define LHCI_LEN_LE_CLEAR_WHITE_LIST_EVT \
    1 /*!< LE clear white list command complete event length. */
#define LHCI_LEN_LE_ADD_DEV_WHITE_LIST_EVT \
    1 /*!< LE add device to white list command complete event length. */
#define LHCI_LEN_LE_REMOVE_DEV_WHITE_LIST_EVT \
    1 /*!< LE remove device from white list command complete event length. */
#define LHCI_LEN_LE_SET_HOST_CHAN_CLASS \
    1 /*!< LE set host channel class command complete event length. */
...
#define LHCI_LEN_LE_LTK_REQ_NEG_REPL_EVT \
    3 /*!< LE LTK request negative reply command complete event length. */
#define LHCI_LEN_LE_RAND_EVT 9 /*!< LE random command complete event length. */
#define LHCI_LEN_LE_READ_SUP_STATES_EVT \
    9 /*!< LE read supported states command complete event length. */
#define LHCI_LEN_LE_RECEIVER_TEST_EVT 1 /*!< LE receiver test command complete event length. */
#define LHCI_LEN_LE_TRANSMITTER_TEST_EVT \
    1 /*!< LE transmitter test command complete event length. */
#define LHCI_LEN_LE_TEST_END_EVT 3 /*!< LE test end command complete event length. */
/* New in version 4.1 */
#define LHCI_LEN_LE_REM_CONN_PARAM_REP \
    3 /*!< LE remote connection parameter reply command complete event length. */
#define LHCI_LEN_LE_REM_CONN_PARAM_NEG_REP \
    3 /*!< LE remote connection parameter negative reply command complete event length. */
/* New in version 4.2 */
#define LHCI_LEN_LE_SET_DATA_LEN 3 /*!< LE set data length command complete event length. */
#define LHCI_LEN_LE_READ_DEF_DATA_LEN \
    5 /*!< LE read default data length command complete event length. */
#define LHCI_LEN_LE_WRITE_DEF_DATA_LEN \
    1 /*!< LE write default data length command complete event length. */
#define LHCI_LEN_LE_ADD_DEV_RES_LIST_EVT \
    1 /*!< LE add device to resolving list command complete event length. */
#define LHCI_LEN_LE_REMOVE_DEV_RES_LIST_EVT \
    1 /*!< LE remove device from resolving list command complete event length. */
#define LHCI_LEN_LE_CLEAR_RES_LIST_EVT \
    1 /*!< LE clear resolving list command complete event length. */
#define LHCI_LEN_LE_READ_RES_LIST_SIZE_EVT \
    2 /*!< LE read resolving list size command complete event length. */
#define LHCI_LEN_LE_READ_PEER_RES_ADDR_EVT \
    7 /*!< LE read peer resolving address command complete event length. */
#define LHCI_LEN_LE_READ_LOCAL_RES_ADDR_EVT \
    7 /*!< LE read local resolving address command complete event length. */
#define LHCI_LEN_LE_SET_ADDR_RES_ENABLE_EVT \
    1 /*!< LE set address resolution enable command complete event length. */
#define LHCI_LEN_LE_SET_RES_PRIV_ADDR_TO_EVT \
    1 /*!< LE set resolving private address timeout command complete event length. */
#define LHCI_LEN_LE_READ_MAX_DATA_LEN \
    9 /*!< LE read maximum data length command complete event length. */
/* New in version 5.0 */
#define LHCI_LEN_LE_SET_PRIVACY_MODE 1 /*!< LE set privacy mode command complete event length. */
#define LHCI_LEN_LE_SET_ADV_SET_RAND_ADDR \
    1 /*!< LE set advertising set random address command complete event length. */
#define LHCI_LEN_LE_SET_EXT_ADV_PARAM \
    2 /*!< LE set extended advertising parameters command complete event length. */
#define LHCI_LEN_LE_SET_EXT_ADV_DATA \
    1 /*!< LE set extended advertising data command complete event length. */
#define LHCI_LEN_LE_SET_EXT_SCAN_RESP_DATA \
    1 /*!< LE set extended scan response data command complete event length. */
#define LHCI_LEN_LE_SET_EXT_ADV_ENABLE \
    1 /*!< LE set extended advertising enable command complete event length. */
#define LHCI_LEN_LE_READ_MAX_ADV_DATA_LEN \
    3 /*!< LE read maximum advertising data length command complete event length. */
#define LHCI_LEN_LE_READ_NUM_OF_SUP_ADV_SETS \
    2 /*!< LE read number of supported advertising sets command complete event length. */
#define LHCI_LEN_LE_REMOVE_ADV_SET \
    1 /*!< LE remove advertising set command complete event length. */
#define LHCI_LEN_LE_CLEAR_ADV_SETS \
    1 /*!< LE clear advertising sets command complete event length. */
#define LHCI_LEN_LE_SET_PER_ADV_PARAM \
    1 /*!< LE set periodic advertising parameters command complete event length. */
#define LHCI_LEN_LE_SET_PER_ADV_DATA \
    1 /*!< LE set periodic advertising data command complete event length. */
#define LHCI_LEN_LE_SET_PER_ADV_ENABLE \
    1 /*!< LE set periodic advertising enable command complete event length. */
#define LHCI_LEN_LE_SET_EXT_SCAN_PARAM \
    1 /*!< LE set extended scan parameter command complete event length. */
#define LHCI_LEN_LE_SET_EXT_SCAN_ENABLE \
    1 /*!< LE set extended scan enable command complete event length. */
#define LHCI_LEN_LE_PER_ADV_CREATE_SYNC_CANCEL \
    1 /*!< LE periodic advertising create sync cancel command complete event length. */
#define LHCI_LEN_LE_PER_ADV_TERMINATE_SYNC \
    1 /*!< LE periodic advertising terminate sync command complete event length. */
#define LHCI_LEN_LE_ADD_DEV_PER_ADV_LIST \
    1 /*!< LE add device from periodic advertising list command complete event length. */
#define LHCI_LEN_LE_REM_DEV_PER_ADV_LIST \
    1 /*!< LE remove device from periodic advertising list command complete event length. */
#define LHCI_LEN_LE_CLEAR_PER_ADV_LIST \
    1 /*!< LE clear periodic advertising list command complete event length. */
#define LHCI_LEN_LE_READ_PER_ADV_LIST_SIZE \
    2 /*!< LE read periodic advertising list size command complete event length. */
#define LHCI_LEN_LE_READ_SUP_TX_POWER \
    3 /*!< LE read supported Tx power command complete event length. */
#define LHCI_LEN_LE_WRITE_RF_PATH_COMP \
    1 /*!< LE write RF path compensation command complete event length. */
#define LHCI_LEN_LE_READ_RF_PATH_COMP \
    5 /*!< LE read RF path compensation command complete event length. */
#define LHCI_LEN_LE_SET_EXT_SCAN_ENABLE_EVT \
    1 /*!< LE set extended scan enable command complete event length. */
...
#define LHCI_LEN_LE_ENHANCED_RECEIVER_TEST_EVT \
    1 /*!< LE enhanced receiver test command complete event length. */
#define LHCI_LEN_LE_ENHANCED_TRANSMITTER_TEST_EVT \
    1 /*!< LE enhanced transmitter test command complete event length. */
/* New in version 5.1 */
#define LHCI_LEN_LE_SET_PER_ADV_RCV_ENABLE \
    1 /*!< LE set periodic advertising receive enable command complete event length. */
#define LHCI_LEN_LE_PER_ADV_SYNC_TRANSFER \
    3 /*!< LE periodic advertising sync transfer command complete event length. */
#define LHCI_LEN_LE_PER_ADV_SET_INFO_TRANSFER \
    3 /*!< LE periodic advertising set info transfer command complete event length. */
#define LHCI_LEN_LE_SET_PAST_PARAM \
    3 /*!< LE set periodic advertising sync transfer parameters command complete event length. */
#define LHCI_LEN_LE_SET_DEFAULT_PAST_PARAM \
    1 /*!< LE set default periodic advertising sync transfer parameters command complete event length. */
#define LHCI_LEN_LE_MODIFY_SCA_EVT 1 /*!< LE Modify sleep clock accuracy event length. */
/* New in version 5.2 */
#define LHCI_LEN_LE_READ_BUF_SIZE_V2 \
    7 /*!< LE read ISO buffer size command complete event length. */
...
#define LHCI_LEN_LE_SET_PATH_LOSS_REPORTING_PARAMS \
    3 /*!< LE Set path loss reporting parameters event. */
#define LHCI_LEN_LE_SET_PATH_LOSS_REPORTING_ENABLE \
    3 /*!< LE Set path loss reporting enable event. */
...
#define LHCI_OPCODE_VS_MEMORY_ERASE\
...
#define LHCI_OPCODE_VS_SET_EVENT_MASK \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3E1) /*!< Set Vendor Specific Event Mask opcode. */
#define LHCI_OPCODE_VS_SET_RSRC_MGR_MODE HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3E2) /*!< DEPRECATED. */
#define LHCI_OPCODE_VS_ENA_ACL_SINK \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3E3) /*!< Enable ACL Packet Sink opcode. */
#define LHCI_OPCODE_VS_GENERATE_ACL \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3E4) /*!< Generate ACL Packets opcode. */
#define LHCI_OPCODE_VS_ENA_AUTO_GEN_ACL \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3E5) /*!< Enable Auto Generate ACL Packets opcode. */
#define LHCI_OPCODE_VS_SET_TX_TEST_ERR_PATT \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3E6) /*!< Set Tx Test Error Pattern opcode. */
#define LHCI_OPCODE_VS_SET_CONN_OP_FLAGS \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3E7) /*!< Set Connection Operational Flags opcode. */
#define LHCI_OPCODE_VS_SET_P256_PRIV_KEY \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3E8) /*!< Set P-256 Private Key opcode. */
#define LHCI_OPCODE_VS_GET_PER_CHAN_MAP \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3DE) /*!< Get channel map of periodic scan/adv. */
#define LHCI_OPCODE_VS_SET_HCI_SUP_CMD HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3DF) /*!< DEPRECATED. */
#define LHCI_OPCODE_VS_GET_ACL_TEST_REPORT \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3E9) /*!< Get ACL Test Report opcode. */
#define LHCI_OPCODE_VS_SET_LOCAL_MIN_USED_CHAN \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3EA) /*!< Set local minimum number of used channels. */
#define LHCI_OPCODE_VS_GET_PEER_MIN_USED_CHAN \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3EB) /*!< Get peer minimum number of used channels. */
#define LHCI_OPCODE_VS_VALIDATE_PUB_KEY_MODE \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC,          \
               0x3EC) /*!< Set validate public key mode between ALT1 and ALT2. */

#define LHCI_OPCODE_VS_SET_BD_ADDR \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3F0) /*!< Set BD address opcode. */
#define LHCI_OPCODE_VS_GET_RAND_ADDR \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3F1) /*!< Get Random Address opcode. */
#define LHCI_OPCODE_VS_SET_LOCAL_FEAT \
...
#define LHCI_OPCODE_VS_SET_CONN_TX_PWR \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3F6) /*!< Set Connection Tx Power opcode. */
#define LHCI_OPCODE_VS_SET_ENC_MODE \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3F7) /*!< Set Encryption Mode opcode. */
#define LHCI_OPCODE_VS_SET_CHAN_MAP \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3F8) /*!< Set Channel Map opcode. */

#define LHCI_OPCODE_VS_SET_DIAG_MODE \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3F9) /*!< Set Diagnostic Mode opcode. */
...
#define LHCI_OPCODE_VS_GET_ADV_STATS \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3FB) /*!< Get Advertising Statistics opcode. */
#define LHCI_OPCODE_VS_GET_SCAN_STATS \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3FC) /*!< Get Scan Statistics opcode. */
#define LHCI_OPCODE_VS_GET_CONN_STATS \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3FD) /*!< Get Connection Statistics opcode. */
#define LHCI_OPCODE_VS_GET_TEST_STATS \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3FE) /*!< Get Test Statistics opcode. */
#define LHCI_OPCODE_VS_GET_POOL_STATS \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3FF) /*!< Get Pool Statistics opcode. */

#define LHCI_OPCODE_VS_SET_AUX_DELAY \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3D0) /*!< Set Additional AuxPtr offset. */
#define LHCI_OPCODE_VS_SET_EXT_ADV_FRAG_LEN \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC,         \
               0x3D1) /*!< Set extended advertising data fragmentation length. */
#define LHCI_OPCODE_VS_SET_EXT_ADV_PHY_OPTS \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3D2) /*!< Set extended advertising PHY options. */
#define LHCI_OPCODE_VS_SET_EXT_ADV_DEF_PHY_OPTS \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3D3) /*!< Set extended advertising default PHY options. */
#define LHCI_OPCODE_VS_SET_EXT_SCAN_PHY_OPTS \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3D4) /*!< Set extended scanning default PHY options. */
#define LHCI_OPCODE_VS_GENERATE_ISO \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3D5) /*!< Generate ISO Packets opcode. */
#define LHCI_OPCODE_VS_GET_ISO_TEST_REPORT \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3D6) /*!< Get ISO Test Report opcode. */
#define LHCI_OPCODE_VS_ENA_ISO_SINK \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3D7) /*!< Enable ISO Packet Sink opcode. */
#define LHCI_OPCODE_VS_ENA_AUTO_GEN_ISO \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3D8) /*!< Enable Auto Generate ISO Packets opcode. */
#define LHCI_OPCODE_VS_GET_CIS_STATS \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3D9) /*!< Get ISO Connection Statistics opcode. */

#define LHCI_OPCODE_VS_GET_AUX_ADV_STATS \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3DA) /*!< Get Auxiliary Advertising Statistics opcode. */
#define LHCI_OPCODE_VS_GET_AUX_SCAN_STATS \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3DB) /*!< Get Auxiliary Scanning Statistics opcode. */
#define LHCI_OPCODE_VS_GET_PER_SCAN_STATS \
    HCI_OPCODE(HCI_OGF_VENDOR_SPEC, 0x3DC) /*!< Get Periodic Scanning Statistics opcode. */
...
void lhciConnSendCmdCmplEvt(LhciHdr_t *pCmdHdr, uint8_t status, uint8_t paramLen, uint8_t *pParam,
                            uint16_t handle);
