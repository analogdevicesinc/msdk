 ...
 */
/*************************************************************************************************/
static uint8_t lhciPackReadPhyEvt(uint8_t *pBuf, uint8_t status, uint16_t handle, uint8_t txPhy,
                                  uint8_t rxPhy)
{
 ...
 */
/*************************************************************************************************/
static void lhciPhySendCmdCmplEvt(LhciHdr_t *pCmdHdr, uint8_t status, uint8_t paramLen,
                                  uint16_t handle)
{
    uint8_t *pBuf;
    uint8_t *pEvtBuf;
...
bool_t lhciPhyDecodeCmdPkt(LhciHdr_t *pHdr, uint8_t *pBuf)
{
    ...
    case HCI_OPCODE_LE_RECEIVER_TEST_V3: {
        status =
            LlRxTestV3(pBuf[0], pBuf[1], pBuf[2], pBuf[3], pBuf[4], pBuf[5], pBuf[6], &pBuf[7], 0);
        paramLen = LHCI_LEN_LE_ENHANCED_RECEIVER_TEST_EVT;
        break;
    }

    case HCI_OPCODE_LE_TRANSMITTER_TEST_V3: {
        status =
            LlTxTestV3(pBuf[0], pBuf[1], pBuf[2], pBuf[3], pBuf[4], pBuf[5], pBuf[6], &pBuf[7], 0);
        paramLen = LHCI_LEN_LE_ENHANCED_RECEIVER_TEST_EVT;
        break;
    }
#endif
