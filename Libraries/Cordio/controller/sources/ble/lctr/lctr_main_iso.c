...
static void lctrIsoProcessRxTestData(lctrCisCtx_t *pCisCtx, uint8_t *pDataBuf, uint8_t dataLen)
{
    ...
    if (pCisCtx->rxPendInit) {
        ...
        LL_TRACE_INFO1("lctrCisRxPostProcessing, Rx counter initialized to %d",
                       pCisCtx->expectedPkt);
    }
    ...
    switch (pCisCtx->isoRxPldType) {
    case LL_ISO_PLD_TYPE_MAX_LEN:
        if ((dataLen == pldType) && (pktNum == pCisCtx->expectedPkt)) {
    ...
    case LL_ISO_PLD_TYPE_VAR_LEN:
        if ((dataLen >= LL_ISO_TEST_VAR_MIN_LEN) && (pktNum == pCisCtx->expectedPkt)) {
            pCisCtx->numRxSuccess++;
        } else {
            pCisCtx->expectedPkt = pktNum + 1;
            pCisCtx->numRxFailed++;
...
{
    if (!pCisCtx->rxTestEnabled && !pCisCtx->txTestEnabled) {
    ...
    for (unsigned int i = 0; i < pLctrRtCfg->maxCis; i++) {
        ...
        if (pCisCtx->enabled && !pCisCtx->txTestEnabled) {
            if (pCisCtx->numTxComp && (pCisCtx->framing == LL_ISO_PDU_TYPE_UNFRAMED)) {
                ...
                pCisCtx->numTxComp = 0;
                numHandles++;
            } else if ((pCisCtx->framing == LL_ISO_PDU_TYPE_FRAMED) &&
                       pCisCtx->isoalTxCtx.compSdu) {
                numSdu[numHandles] = pCisCtx->isoalTxCtx.compSdu;
                handle[numHandles] = pCisCtx->cisHandle;
                pCisCtx->numTxComp =
                    0; /* numTxComp is still recorded in framed, so clear it here. */
...
void lctrCisRxPendingHandler(void)
{
    ...
    while ((pRxBuf = lctrCisRxDeq(&cisHandle)) != NULL) {
        ...
        if (lctrPktDecryptHdlr) {
            if (lctrPktDecryptHdlr(&pCisCtx->bleData.chan.enc, pRxBuf) == FALSE) {
                ...
                LL_TRACE_ERR1("!!! MIC verification failed on cisEvtCounter=%u",
        ...
        if (pCisCtx->framing == LL_ISO_PDU_TYPE_UNFRAMED) {
            ...
            case LL_LLID_ISO_UNF_END_PDU:
                ...
                default:
                    lctrCisRxPduFree(pRxBuf);
                    LL_TRACE_ERR2(
                        "!!! Invalid rxState; dropping Rx data PDU, connHandle=%u, rxState=%u",
                        cisHandle, pRxCtx->rxState);
                    pRxCtx->rxState = LL_ISO_SDU_STATE_NEW;
                }
                break;
            ...
            case LL_LLID_ISO_UNF_CONT_PDU:
                isoHdr.pb = (pRxCtx->rxState == LL_ISO_SDU_STATE_NEW) ? LCTR_PB_FIRST :
                                                                        LCTR_PB_CONT;
            ...
            default:
                lctrCisRxPduFree(pRxBuf);
                LL_TRACE_ERR2("!!! Invalid LLID; dropping Rx data PDU, connHandle=%u llid=%u",
                              cisHandle, rxHdr.llid);
                break;
            }
            ...
            if (pRxCtx->pduFlushed) {
                ...
                case LL_ISO_SDU_STATE_NEW:
                    ...
                    pRxCtx->data.unframed.ps =
                        (rxHdr.llid == LL_LLID_ISO_UNF_END_PDU) ? LCTR_PS_INVALID : LCTR_PS_LOST;
                    break;
                }
            ...
            if (pRxBuf) {
                ...
                isoHdr.tsFlag = ((isoHdr.pb == LCTR_PB_COMP) || (isoHdr.pb == LCTR_PB_FIRST)) ? 1 :
                                                                                                0;
                if (isoHdr.tsFlag) {
                    isoHdr.ts = 0xFF;
                }
                ...
                if (!lctrIsoUnframedRxSduPendQueue(pRxCtx, pSduBuf, cisHandle, rxHdr.len,
                                                   rxHdr.llid)) {
                    break;
                }
        ...
        } else /* LL_ISO_PDU_TYPE_FRAMED */
        {
            ...
            case LL_LLID_ISO_FRA_PDU: {
                pCisCtx->dataPathOutCtx.cfg.hci.numRxPend +=
                    lctrAssembleRxFramedSdu(pRxCtx, &pCisCtx->dataPathOutCtx.cfg.hci.rxDataQ,
                                            pCisCtx->cisHandle, pRxBuf, cisDataHdr.len);
                ...
                if (pCisCtx->rxTestEnabled == TRUE) {
                    while (pCisCtx->dataPathOutCtx.cfg.hci.numRxPend) {
                        ...
                        lctrIsoProcessRxTestData(pCisCtx,
                                                 pTestRxBuf + HCI_ISO_HDR_LEN + HCI_ISO_DL_MAX_LEN,
                                                 isoHdr.sduLen);
    ...
    for (unsigned int i = 0; i < pLctrRtCfg->maxCis; i++) {
        ...
        if (pCisCtx->enabled && pCisCtx->dataPathOutCtx.cfg.hci.numRxPend) {
...
uint16_t LctrInitIsoMem(uint8_t *pFreeMem, uint32_t freeMemSize)
{
    ...
    if (memUsed == 0) {
        LL_TRACE_ERR2("LctrInitIsoMem: failed to allocate descriptors, need=%u available=%u",
...
void LctrTxIso(uint8_t *pIsoBuf)
{
    ...
    if (isoHdr.sduLen > pLctrRtCfg->maxIsoSduLen) {
        LL_TRACE_ERR2("Invalid ISO header: invalid packet length, actLen=%u, maxLen=%u",
                      isoHdr.sduLen, pLctrRtCfg->maxIsoSduLen);
        WsfMsgFree(pIsoBuf);
    ...
    if (isoHdr.sduLen != expIsoLen) {
        LL_TRACE_ERR2("Invalid ISO header: packet length mismatch, expSduLen=%u, actSduLen=%u",
                      expIsoLen, isoHdr.sduLen);
        WsfMsgFree(pIsoBuf);
        lmgrPersistCb.sendCompCback(isoHdr.handle, 1);
        return;
    }
    ...
    if ((pCisCtx = lctrFindCisByHandle(isoHdr.handle)) != NULL) {
        if (!lctrCheckIsCisEstCis(pCisCtx->cisHandle)) {
            LL_TRACE_ERR1("Invalid ISO handle: link not established cisHandle=%u; dropping packet",
                          isoHdr.handle);
            lmgrPersistCb.sendCompCback(isoHdr.handle, 1);
            WsfMsgFree(pIsoBuf);
            return;
        ...
        if ((pCisCtx->localDataPdu.maxTxLen == 0) ||
            ...
            ((pCisCtx->bnSToM == 0) && (pCisCtx->role == LL_ROLE_SLAVE))) {
            LL_TRACE_ERR1(
                "Invalid CIS state: handle=%u does not accept transmissions; dropping packet",
                isoHdr.handle);
            WsfMsgFree(pIsoBuf);
            lmgrPersistCb.sendCompCback(isoHdr.handle, 1);
            return;
        ...
        if (isoHdr.sduLen > pBisCtx->pBigCtx->maxSdu) {
            LL_TRACE_ERR2("Invalid ISO header: invalid packet length, actLen=%u, maxSdu=%u",
                          isoHdr.sduLen, pBisCtx->pBigCtx->maxSdu);
            WsfMsgFree(pIsoBuf);
            lmgrPersistCb.sendCompCback(isoHdr.handle, 1);
            return;
        }
        ...
        LL_TRACE_ERR1("Invalid ISO handle: unknown handle handle=%u; dropping packet",
                      isoHdr.handle);
        WsfMsgFree(pIsoBuf);
...
uint8_t *LctrRxIso(void)
{
    ...
    for (unsigned int i = 0; i < pLctrRtCfg->maxCis; i++) {
        ...
        if ((pCisCtx->enabled) && ((pBuf = lctrIsoRxConnDeq(&pCisCtx->dataPathOutCtx)) != NULL)) {
            return pBuf;
        }
    }
    ...
    for (unsigned int i = 0; i < pLctrRtCfg->maxBis; i++) {
        ...
        if ((pBisCtx->enabled) && (pBisCtx->pBigCtx->role == LL_ROLE_MASTER) &&
...
uint8_t LctrSetupIsoDataPath(LlIsoSetupDataPath_t *pSetupDataPath)
{
    ...
    if ((pCisCtx = lctrFindCisByHandle(pSetupDataPath->handle)) != NULL) {
        ...
        case LL_ISO_DATA_DIR_OUTPUT:
            ...
            break;
        default:
            LL_TRACE_WARN2("LctrSetupIsoDataPath: handle=%u invalid direction dpDir=%u",
                           pSetupDataPath->handle, pSetupDataPath->dpDir);
            return LL_ERROR_CODE_CMD_DISALLOWED;
        }
        ...
        case LL_ISO_DATA_DIR_INPUT:
            if (pBisCtx->pBigCtx->role == LL_ROLE_SLAVE) {
                ...
                LL_TRACE_WARN1("LctrSetupIsoDataPath: handle=%u invalid input direction for master",
                               pSetupDataPath->handle);
        ...
        case LL_ISO_DATA_DIR_OUTPUT:
            if (pBisCtx->pBigCtx->role == LL_ROLE_MASTER) {
                ...
                LL_TRACE_WARN1("LctrSetupIsoDataPath: handle=%u invalid output direction for slave",
                               pSetupDataPath->handle);
                status = LL_ERROR_CODE_CMD_DISALLOWED;
            }
            break;
...
void lctrIsoSendCodecSdu(uint16_t handle)
{
    ...
    lctrIsoHdr_t hdr = { .handle = handle,
                         .pb = LCTR_PB_COMP,
                         .tsFlag = TRUE,
                         .len = len,
                         .ts = 0,
