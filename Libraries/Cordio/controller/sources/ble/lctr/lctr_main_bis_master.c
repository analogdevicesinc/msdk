...
static void lctrMstBigDisp(lctrMsgHdr_t *pMsg)
{
    ...
    if ((pMsg->dispId != LCTR_DISP_BCST) && (pMsg->event != LCTR_MST_BIG_ACAD_BIG_INFO)) {
...
uint8_t LctrMstBigCreateSync(LlBigCreateSync_t *pParam)
{
    ...
    if ((pParam->syncHandle > LL_MAX_PER_SCAN) || (pPerScanCtx->enabled == FALSE)) {
    ...
    if (pParam->numBis > WSF_MIN(pBigInfo->numBis, LL_MAX_BIS)) {
        LL_TRACE_WARN2("LctrMstBigCreateSync: invalid value for numBis=%u, validRange=1..%u",
                       pParam->numBis, WSF_MIN(pBigInfo->numBis, LL_MAX_BIS));
        return LL_ERROR_CODE_UNSUPPORTED_FEATURE_PARAM_VALUE;
    }

    for (unsigned int i = 0; i < pParam->numBis; i++) {
        if ((pParam->bis[i] < LL_MIN_BIS) || (pParam->bis[i] > LL_MAX_BIS)) {
...
void lctrMstBigBuildOp(lctrBigCtx_t *pBigCtx, LctrAcadBigInfo_t *pBigInfo)
{
    ...
    LL_TRACE_INFO2("    >>> BIG synced, handle=%u, numBis=%u <<<", pBigCtx->handle,
                   pBigCtx->numBis);
...
void lctrMstSetupBigContext(lctrBigCtx_t *pBigCtx, LctrAcadBigInfo_t *pBigInfo)
{
    ...
    pBigCtx->eventCounter =
        pBigInfo->bisPldCtr / pBigInfo->bn; /* TODO use optimized 64-bit divide */
    ...
    if (pBigCtx->encrypt) {
        ...
        lctrBisCalcGroupSessionKey(pBigCtx->gskd, pBigCtx->roleData.mst.bcstCode,
