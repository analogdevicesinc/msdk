...
void lctrInitActInitiate(void)
{
    ...
    do {
        if ((pCtx = lctrAllocConnCtx()) == NULL) {
            result = FALSE;
            lctrScanNotifyHostInitiateError(LL_ERROR_CODE_CONN_LIMIT_EXCEEDED,
                                            pInitMsg->peerAddrType, pInitMsg->peerAddr);
            break;
        }
        ...
        if (!SchRmAdd(LCTR_GET_CONN_HANDLE(pCtx), SCH_RM_PREF_PERFORMANCE, interMinUsec,
                      interMaxUsec, durUsec, &connInterUsec, lctrGetConnRefTime)) {
            lctrFreeConnCtx(pCtx);
            result = FALSE;
            lctrScanNotifyHostInitiateError(LL_ERROR_CODE_CONN_REJ_LIMITED_RESOURCES,
                                            pInitMsg->peerAddrType, pInitMsg->peerAddr);
            break;
        }
    } while (FALSE);
...
void lctrInitActConnect(void)
{
    if (lmgrCb.numInitEnabled) {
        ...
        if ((pMsg = (lctrConnEstablish_t *)WsfMsgAlloc(sizeof(*pMsg))) != NULL) {
            ...
            BbBlePduFiltResultsGetPeerIdAddr(&pScan->filtResults, &pMsg->peerIdAddr,
                                             &pMsg->peerIdAddrType);
...
void lctrInitActScanTerm(void)
{
    ...
    lctrScanNotifyHostInitiateError(LL_ERROR_CODE_UNKNOWN_CONN_ID,
                                    BB_BLE_PDU_FILT_FLAG_IS_SET(&pBle->pduFilt,
                                                                PEER_ADDR_MATCH_RAND),
...
void lctrInitActDisallowCancel(void)
{
    ...
    LL_TRACE_INFO0(
        "### LlEvent ###  LL_CREATE_CONN_CANCEL_CNF, status=LL_ERROR_CODE_CMD_DISALLOWED");
