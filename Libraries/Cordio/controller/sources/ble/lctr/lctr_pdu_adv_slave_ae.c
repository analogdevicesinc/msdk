 ...
 */
/*************************************************************************************************/
static uint8_t lctrPackExtAdvHeader(lctrAdvSet_t *pAdvSet, uint8_t manExtHdrFlags,
                                    uint8_t optExtHdrFlags, uint8_t advMode,
                                    lctrAdvbPduHdr_t *pPduHdr, lctrAdvDataBuf_t *pDataBuf,
                                    uint8_t *pPduBuf, uint8_t commExtAdvPdu, bool_t isPeriodic)
{
    ...
    if ((pAdvSet->param.advEventProp & LL_ADV_EVT_PROP_OMIT_AA_BIT) == 0) {
        if ((manExtHdrFlags & LL_EXT_HDR_ADV_ADDR_BIT) ||
            ((optExtHdrFlags & LL_EXT_HDR_ADV_ADDR_BIT) &&
             (!(pAdvSet->extHdrFlags & LL_EXT_HDR_ADV_ADDR_BIT)))) {
            /* Pack AdvA */
    ...
    if ((manExtHdrFlags & LL_EXT_HDR_TGT_ADDR_BIT) ||
        ((optExtHdrFlags & LL_EXT_HDR_TGT_ADDR_BIT) &&
         (!(pAdvSet->extHdrFlags & LL_EXT_HDR_TGT_ADDR_BIT)))) {
        /* Pack TargetA */
    ...
    if ((manExtHdrFlags & LL_EXT_HDR_ADI_BIT) ||
        ...
        UINT16_TO_BSTREAM(pBuf,
                          (pAdvSet->param.advSID << 12) | ((pAdvSet->param.advDID & 0x0FFF) << 0));
    ...
    if ((manExtHdrFlags & LL_EXT_HDR_SYNC_INFO_BIT) ||
        ((optExtHdrFlags & LL_EXT_HDR_SYNC_INFO_BIT) &&
         (!(pAdvSet->extHdrFlags & LL_EXT_HDR_SYNC_INFO_BIT)))) {
        syncInfoNeeded = TRUE;
    ...
    if ((optExtHdrFlags & LL_EXT_HDR_TX_PWR_BIT) &&
        (((commExtAdvPdu == LCTR_PDU_AUX_SYNC_IND) &&
          (pAdvSet->perParam.advEventProp & LL_EXT_HDR_TX_PWR_BIT)) ||
         ((commExtAdvPdu != LCTR_PDU_AUX_SYNC_IND) &&
          (pAdvSet->param.advEventProp & LL_EXT_HDR_TX_PWR_BIT)))) {
        txPwrNeeded = TRUE;
        /* Reserve packet space now, pack later. */
    ...
    if (pDataBuf) {
        ...
        if ((optExtHdrFlags &
             LL_EXT_HDR_AUX_PTR_BIT) && /* Allow fragmentation only if AUX_CHAIN_IND is allowed. */
    ...
    case LCTR_PDU_AUX_ADV_IND:
        ...
        if (pAuxPtr) {
            ...
            pAdvSet->auxOffsUsec = SchBleCalcAuxPktDurationUsec(pBle->chan.txPhy,
                                                                pBle->chan.initTxPhyOptions,
                                                                LL_ADV_HDR_LEN + pPduHdr->len) +
    ...
    case LCTR_PDU_AUX_CONNECT_RSP:
        ...
        if (pAuxPtr) {
            ...
            /* Now the PDU length/duration is known, complete AuxPtr fields for next CHAIN_IND. */
            pAdvSet->auxOffsUsec = SchBleCalcAuxPktDurationUsec(
                                       pBle->chan.txPhy,
                                       (pBle->chan.tifsTxPhyOptions != BB_PHY_OPTIONS_DEFAULT) ?
                                           pBle->chan.tifsTxPhyOptions :
                                           pBle->op.slvAuxAdv.auxRxPhyOptions,
                                       LL_ADV_HDR_LEN + pPduHdr->len) +
                                   WSF_MAX(pAdvSet->auxDelayUsec, pLctrRtCfg->auxDelayUsec);
            pAdvSet->auxOffsUsec = WSF_MIN(pAdvSet->auxOffsUsec, LL_AUX_PTR_MAX_USEC);
    ...
    case LCTR_PDU_AUX_SYNC_IND:
        ...
        if (pAuxPtr) {
            ...
            /* Now the PDU length/duration is known, complete AuxPtr fields for next CHAIN_IND. */
            pAdvSet->perParam.perOffsUsec =
                SchBleCalcAuxPktDurationUsec(pBle->chan.txPhy, pBle->chan.initTxPhyOptions,
                                             LL_ADV_HDR_LEN + pPduHdr->len) +
                WSF_MAX(pAdvSet->auxDelayUsec, pLctrRtCfg->auxDelayUsec);
            pAdvSet->perParam.perOffsUsec =
                WSF_MIN(pAdvSet->perParam.perOffsUsec, LL_AUX_PTR_MAX_USEC);

            lctrPackAuxPtr(pAdvSet, pAdvSet->perParam.perOffsUsec, pAdvSet->perParam.perChIdx,
                           pAuxPtr);
        }
        break;
    }
...
uint8_t lctrPackAdvExtIndPdu(lctrAdvSet_t *pAdvSet, uint8_t *pPduBuf, bool_t isPeriodic)
{
    ...
    case (/* ------------------- */ 0 | /* ------------------- */ 0 | /* ------------------ */ 0):
        ...
        optExtHdrFlags =
            ((pAdvSet->param.priAdvPhy == LL_PHY_LE_1M) ? LL_EXT_HDR_ADV_ADDR_BIT : 0) |
    ...
    case (LL_ADV_EVT_PROP_CONN_ADV_BIT | /* ------------------- */ 0 |
          LL_ADV_EVT_PROP_DIRECT_ADV_BIT):
        manExtHdrFlags = LL_EXT_HDR_ADI_BIT | LL_EXT_HDR_AUX_PTR_BIT;
        optExtHdrFlags = ((pAdvSet->param.priAdvPhy == LL_PHY_LE_1M) ? LL_EXT_HDR_TX_PWR_BIT : 0);
        break;

    /* Scannable */
    case (/* ------------------- */ 0 | LL_ADV_EVT_PROP_SCAN_ADV_BIT | /* ------------------ */ 0):
    case (/* ------------------- */ 0 | LL_ADV_EVT_PROP_SCAN_ADV_BIT |
          LL_ADV_EVT_PROP_DIRECT_ADV_BIT):
        manExtHdrFlags = LL_EXT_HDR_ADI_BIT | LL_EXT_HDR_AUX_PTR_BIT;
        optExtHdrFlags = ((pAdvSet->param.priAdvPhy == LL_PHY_LE_1M) ? LL_EXT_HDR_TX_PWR_BIT : 0);
        break;
    ...
    uint8_t advMode =
        ((pAdvSet->param.advEventProp & LL_ADV_EVT_PROP_CONN_ADV_BIT) ? LCTR_ADV_MODE_CONN_BIT :
                                                                        0) |
        ((pAdvSet->param.advEventProp & LL_ADV_EVT_PROP_SCAN_ADV_BIT) ? LCTR_ADV_MODE_SCAN_BIT : 0);

    pAdvSet->extHdrFlags = 0;
    return lctrPackExtAdvHeader(pAdvSet, manExtHdrFlags, optExtHdrFlags, advMode, &pduHdr, NULL,
                             ...
                             bool_t isPeriodic)
{
    ...
    case (/* ------------------- */ 0 | /* ------------------- */ 0 | /* ------------------ */ 0):
        manExtHdrFlags = LL_EXT_HDR_ADI_BIT;
        optExtHdrFlags =
            ((!(pAdvSet->extHdrFlags & LL_EXT_HDR_ADV_ADDR_BIT)) ? LL_EXT_HDR_ADV_ADDR_BIT : 0) |
            LL_EXT_HDR_AUX_PTR_BIT | LL_EXT_HDR_TX_PWR_BIT;
    ...
    case (/* ------------------- */ 0 | /* ------------------- */ 0 |
          LL_ADV_EVT_PROP_DIRECT_ADV_BIT):
        manExtHdrFlags =
            ((!(pAdvSet->extHdrFlags & LL_EXT_HDR_TGT_ADDR_BIT)) ? LL_EXT_HDR_TGT_ADDR_BIT : 0) |
            LL_EXT_HDR_ADI_BIT;
        optExtHdrFlags =
            ((!(pAdvSet->extHdrFlags & LL_EXT_HDR_ADV_ADDR_BIT)) ? LL_EXT_HDR_ADV_ADDR_BIT : 0) |
            LL_EXT_HDR_AUX_PTR_BIT | LL_EXT_HDR_TX_PWR_BIT;
    ...
    /* Connectable */
    case (LL_ADV_EVT_PROP_CONN_ADV_BIT | /* ------------------- */ 0 | /* ------------------ */ 0):
        manExtHdrFlags = LL_EXT_HDR_ADV_ADDR_BIT | LL_EXT_HDR_ADI_BIT;
        optExtHdrFlags = LL_EXT_HDR_TX_PWR_BIT;
        break;
    case (LL_ADV_EVT_PROP_CONN_ADV_BIT | /* ------------------- */ 0 |
          LL_ADV_EVT_PROP_DIRECT_ADV_BIT):
        manExtHdrFlags = LL_EXT_HDR_ADV_ADDR_BIT | LL_EXT_HDR_TGT_ADDR_BIT | LL_EXT_HDR_ADI_BIT;
        optExtHdrFlags = LL_EXT_HDR_TX_PWR_BIT;
        break;
    ...
    case (/* ------------------- */ 0 | LL_ADV_EVT_PROP_SCAN_ADV_BIT |
          LL_ADV_EVT_PROP_DIRECT_ADV_BIT):
        manExtHdrFlags = LL_EXT_HDR_ADV_ADDR_BIT | LL_EXT_HDR_TGT_ADDR_BIT | LL_EXT_HDR_ADI_BIT;
        optExtHdrFlags = LL_EXT_HDR_ADI_BIT | LL_EXT_HDR_TX_PWR_BIT;
        break;
    ...
    return lctrPackExtAdvHeader(pAdvSet, manExtHdrFlags, optExtHdrFlags, advMode, &pduHdr, pAdvData,
...
uint8_t lctrPackAuxScanRspPdu(lctrAdvSet_t *pAdvSet, uint8_t *pPduBuf, bool_t isPeriodic)
{
    ...
    uint8_t optExtHdrFlags = LL_EXT_HDR_AUX_PTR_BIT | LL_EXT_HDR_TX_PWR_BIT;

    lctrChooseSetAdvA(&pduHdr, pBle, pAdvSet);
    lctrChooseSetPeerA(&pduHdr, pBle, pAdvSet);

    return lctrPackExtAdvHeader(pAdvSet, manExtHdrFlags, optExtHdrFlags, 0, &pduHdr,
                                &pAdvSet->scanRspData, pPduBuf, LCTR_PDU_AUX_SCAN_RSP, isPeriodic);
}
                               ...
                               bool_t isPeriodic)
{
    ...
    return lctrPackExtAdvHeader(pAdvSet, manExtHdrFlags, optExtHdrFlags, 0, &pduHdr, pAdvData,
...
uint8_t lctrPackAuxConnRspPdu(lctrAdvSet_t *pAdvSet, uint8_t *pPduBuf, bool_t isPeriodic)
{
    ...
    uint8_t manExtHdrFlags = LL_EXT_HDR_ADV_ADDR_BIT | LL_EXT_HDR_TGT_ADDR_BIT;

    lctrChooseSetAdvA(&pAdvSet->rspPduHdr, pBle, pAdvSet);
    lctrChooseSetPeerA(&pAdvSet->rspPduHdr, pBle, pAdvSet);

    return lctrPackExtAdvHeader(pAdvSet, manExtHdrFlags, 0, 0, &pAdvSet->rspPduHdr, NULL, pPduBuf,
                                LCTR_PDU_AUX_CONNECT_RSP, isPeriodic);
}
...
uint8_t lctrPackLegacyAdvPdu(lctrAdvSet_t *pAdvSet, uint8_t *pPduBuf)
{
    ...
    case LL_PDU_ADV_SCAN_IND:
        if (pAdvSet->advData.alt.legacy.len > 0) {
            ...
            memcpy(pBuf, pAdvSet->advData.alt.legacy.buf,
                   WSF_MIN(pAdvSet->advData.alt.legacy.len, LL_ADVBU_MAX_LEN));
        }
        break;
    case LL_PDU_ADV_DIRECT_IND:
    default:
...
uint8_t lctrPackLegacyScanRspPdu(lctrAdvSet_t *pAdvSet, uint8_t *pPduBuf)
{
    ...
    if (pAdvSet->scanRspData.alt.legacy.len > 0) {
        /* Legacy buffers always stored in cache (exclusive buffer). */
        memcpy(pBuf, pAdvSet->scanRspData.alt.legacy.buf,
               WSF_MIN(pAdvSet->scanRspData.alt.legacy.len, LL_ADVBU_MAX_LEN));
    }
 ...
 */
/*************************************************************************************************/
uint8_t lctrPackSyncIndPdu(lctrAdvSet_t *pAdvSet, uint8_t *pPduBuf, lctrAdvDataBuf_t *pAdvData,
                           bool_t isPeriodic)
{
    lctrAdvbPduHdr_t pduHdr = { 0 };
...
uint8_t lctrPackAcadBigInfo(lctrAdvSet_t *pAdvSet, uint8_t *pBuf)
{
    ...
    UINT32_TO_BSTREAM(pBuf, ((pBigInfo->bigOffs & 0x3FFF) << 0) |
                                ...
                                ((pBigInfo->isoInter) << 15) | ((pBigInfo->numBis) << 27));
    UINT32_TO_BSTREAM(pBuf, ((pBigInfo->nse) << 0) | ((pBigInfo->bn) << 5) |
                                ((pBigInfo->subEvtInterUsec) << 8) | ((pBigInfo->pto) << 28));
    UINT32_TO_BSTREAM(pBuf, ((pBigInfo->bisSpaceUsec) << 0) | ((pBigInfo->irc) << 20) |
                                ((pBigInfo->maxPdu) << 24));
    UINT8_TO_BSTREAM(pBuf, ((0 /* RFU */) << 0));
    UINT32_TO_BSTREAM(pBuf, ((pBigInfo->seedAccAddr) << 0));
    UINT32_TO_BSTREAM(pBuf, ((pBigInfo->sduInterUsec) << 0) | ((pBigInfo->maxSdu) << 20));
    UINT16_TO_BSTREAM(pBuf, ((pBigInfo->baseCrcInit) << 0));
    UINT40_TO_BSTREAM(pBuf, ((pBigInfo->chanMap) << 0) | (((uint64_t)bigPhy) << 37));
