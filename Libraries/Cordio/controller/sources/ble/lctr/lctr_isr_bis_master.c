...
{
    if ((pBuf == NULL) && ((pBuf = lctrBisRxIsoDataPduAlloc()) == NULL)) {
        LL_TRACE_WARN2("BIG event flow controlled, bigHandle=%u, ec[15:0]=%u", pBigCtx->handle,
                       pBigCtx->eventCounter);
    ...
    BbBleBisRxData(pBuf, LL_ISO_DATA_HDR_LEN + BB_DATA_PLD_MAX_LEN,
                   ...
                   lctrMstBisIsr.pNextChan, reAcqSync);
    ...
    if (!pBigCtx->bcp.actMsk) {
        if (opcode < LL_BIG_OPCODE_MAX) {
            ...
            case LL_BIG_OPCODE_CHAN_MAP_IND:
                ...
                if (((uint16_t)(pBigCtx->bcp.chanMapUpd.inst - (uint16_t)pBigCtx->eventCounter)) <
                    0x8000) {
                    pBigCtx->bcp.actMsk |= 1 << opcode;
                    LL_TRACE_INFO2("BIG Channel Map Procedure, bigHandle=%u, instant=%u",
                                   pBigCtx->handle, pBigCtx->bcp.chanMapUpd.inst);
                } else {
                    /* Ignore error attempt to remain connected as long as possible. */
                    LL_TRACE_WARN1("Invalid BIG Channel Map parameters, bigHandle=%u",
                                   pBigCtx->handle);
                }
            ...
            case LL_BIG_OPCODE_BIG_TERM_IND:
                lctrBisUnpackBigTerminateInd(&pBigCtx->bcp.term.reason, &pBigCtx->bcp.term.inst,
                                             lctrMstBisIsr.pCtrlBuf + LL_DATA_HDR_LEN);

                if (((uint16_t)(pBigCtx->bcp.term.inst - (uint16_t)pBigCtx->eventCounter)) <
                    0x8000) {
                    pBigCtx->bcp.actMsk |= 1 << opcode;
                    LL_TRACE_INFO2("BIG Terminate Procedure, bigHandle=%u, instant=%u",
                                   pBigCtx->handle, pBigCtx->bcp.term.inst);
                } else {
                    ...
                    LL_TRACE_WARN1("Invalid BIG Terminate parameters, bigHandle=%u",
                                   pBigCtx->handle);
                }
        ...
        LL_TRACE_WARN2("Unexpected overlapped BIS Control Procedure, bigHandle=%u, opcode=%u",
                       pBigCtx->handle, opcode);
    }
        ...
        while ((pDataPdu = lctrBisDequeueRxDataPdu(pBisCtx, &pldCtrOffs)) != NULL) {
            ...
            if (pBisCtx->pBigCtx->framing == LL_ISO_PDU_TYPE_UNFRAMED) {
                ...
                case LL_ISO_DATA_PATH_HCI: {
                    lctrIsoHdr_t isoHdr = { /* ISO header */
                    ...
                    default:
                        LL_TRACE_WARN2(
                            "Unexpected LLID recieved; dropping Rx ISO PDU, bigHandle=%u, pktCtr[15:0]=%u",
                            pBigCtx->handle, pktCtr);
                        WsfMsgFree(pIsoSdu);
                        continue;
                    }
                    ...
                    if (!lctrIsoUnframedRxSduPendQueue(pRxCtx, pIsoSdu, pBisCtx->handle,
                                                       isoHdr.sduLen, pduHdr.llid)) {
                        break;
                    }
                    ...
                    uint8_t handlerId;
                    uint8_t *pSduFrag;
                    while ((pSduFrag = WsfMsgDeq(&pRxCtx->data.unframed.pendSduQ, &handlerId)) !=
                           NULL) {
                ...
                case LL_ISO_DATA_PATH_VS: {
                    ...
                    lctrCodecHdlr.out(pBisCtx->handle, pDataPdu + LL_ISO_DATA_HDR_LEN,
                                      pDataPdu[LCTR_ISO_DATA_PDU_LEN_OFFSET], pktCtr);
                ...
                default: {
                    LL_TRACE_WARN2(
                        "Data path disabled; dropping Rx ISO PDU, bigHandle=%u, pktCtr[15:0]=%u",
                        pBigCtx->handle, pktCtr);
                    WsfMsgFree(pIsoSdu);
                    break;
                }
            ...
            } else /* LL_ISO_PDU_TYPE_FRAMED */
            {
                switch (pBisCtx->path) {
                case LL_ISO_DATA_PATH_HCI: {
                    numSdu[numHandles] += lctrAssembleRxFramedSdu(
                        &pBisCtx->roleData.mst.isoalRxCtx, &pBisCtx->roleData.mst.rxIsoSduQ,
                ...
                case LL_ISO_DATA_PATH_VS: {
                    ...
                    lctrAssembleRxFramedSdu(&pBisCtx->roleData.mst.isoalRxCtx,
                                            &pBisCtx->roleData.mst.rxIsoSduQ, pBisCtx->handle,
                                            pDataPdu, pDataPdu[LCTR_ISO_DATA_PDU_LEN_OFFSET]);
                ...
                while ((pTestBuf = lctrBisRxIsoSduDeq(pBisCtx)) != NULL) {
                    ...
                    if (pBigCtx->framing == LL_ISO_PDU_TYPE_FRAMED) {
                        if (pBisCtx->test.pendInit) {
                            ...
                            LL_TRACE_INFO1("BIS Framed payload counter initialized to %d",
                                           pBisCtx->test.util.framed.payloadCtr);
                    ...
                    {
                        expPayloadCtr =
                            (rxEvtCtr * pBigCtx->bn) + pBisCtx->test.util.unframed.payloadOffset;
                    }

                    lctrValidateIsoTestData((uint8_t *)isoHdr.pSdu, isoHdr.sduLen,
                                            &pBisCtx->roleData.mst.stats, pBisCtx->test.pldType,
                                            pBigCtx->maxSdu, expPayloadCtr);
                    WsfMsgFree(pTestBuf);
...
void lctrMstBisRxCompletion(BbOpDesc_t *pOp, uint8_t *pBuf, uint8_t status)
{
    ...
    if (!lctrMstBisIsr.firstRxComp) {
        ...
        case BB_STATUS_CRC_FAILED:
            ...
            pBigCtx->roleData.mst.anchorPoint = pBigCtx->roleData.mst.rxSyncTime =
                pBis->startTsUsec;
            break;
    ...
    if ((status == BB_STATUS_SUCCESS) && !(hdr.llid == LL_LLID_BIG_CTRL_PDU) &&
        !((hdr.llid == LL_LLID_ISO_EMPTY_PDU) && (hdr.len == 0)) && /* Ignore Empty PDU */
        (lctrMstBisIsr.pDataBuf[lctrMstBisIsr.se.bisEvtIdx][lctrMstBisIsr.se.burstIdx] ==
    ...
    case BB_STATUS_SUCCESS:
        ...
        case LL_LLID_BIG_CTRL_PDU:
            if (lctrMstBisIsr.pCtrlBuf != NULL) {
                LL_TRACE_WARN1(
                    "Unexpected multiple BIS Control PDU received in a single BIG Event, bigHandle=%u",
                    pBigCtx->handle);
            } else if (hdr.cssn == pBigCtx->bcp.cssn) {
                LL_TRACE_WARN1("Unexpected duplicate BIS Control PDU recieved, bigHandle=%u",
                               pBigCtx->handle);
            } else {
...
void lctrMstBigEndOp(BbOpDesc_t *pOp)
{
    ...
    while (TRUE) {
        ...
        uint32_t wwTotalUsec =
            lctrCalcWindowWideningUsec(unsyncTimeUsec, pBigCtx->roleData.mst.totalAcc) +
        ...
        pBis->rxSyncDelayUsec = wwTotalUsec
                                << 1; /* multiply 2 for before and after BIG Anchor Point */
        ...
        LL_TRACE_WARN2("!!! BIG schedule conflict handle=%u, ec[15:0]=%u", pBigCtx->handle,
                       pBigCtx->eventCounter);
    }
