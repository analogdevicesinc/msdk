...
WSF_CT_ASSERT((BB_FIXED_ADVB_PKT_LEN == 0) || (BB_FIXED_ADVB_PKT_LEN >= LL_ADVB_MAX_LEN));
...
void lctrSlvRxScanReq(void)
{
    ...
    while ((pScanReport = WsfMsgDeq(&lctrSlvAdv.rxScanReqQ, &handlerId)) != NULL) {
        LlScanReportInd_t evt = {
            .hdr = { .param = 0, .event = LL_SCAN_REPORT_IND, .status = LL_SUCCESS },
                    ...
                    uint8_t peerAddrType, uint64_t peerAddr, uint64_t *pAdvA)
{
    ...
    if ((lmgrSlvAdvCb.ownAddrType == LL_ADDR_RANDOM_BIT) && (lmgrCb.bdAddrRndValid)) {
...
void lctrSlvAdvBuildOp(void)
{
    ...
    lctrChooseAdvA(pBle, &pduHdr, lmgrSlvAdvCb.advParam.ownAddrType,
                   lmgrSlvAdvCb.advParam.peerAddrType, lmgrSlvAdvCb.advParam.peerAddr, &advA);

    uint64_t peerRpa = 0;
    lctrChoosePeerAddr(pBle, lmgrSlvAdvCb.advParam.ownAddrType, lmgrSlvAdvCb.advParam.peerAddrType,
    ...
    case LL_ADV_SCAN_UNDIRECT: {
        ...
        static int
            buffer[(sizeof(lctrMsgHdr_t) + BB_REQ_PDU_MAX_LEN + sizeof(int) - 1) / sizeof(int)];
