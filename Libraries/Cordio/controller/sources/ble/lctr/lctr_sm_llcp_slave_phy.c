...
static void lctrUpdateDataTime(lctrConnCtx_t *pCtx)
{
    ...
    case BB_PHY_BLE_1M:
        maxTxTime = WSF_MIN(LL_DATA_LEN_TO_TIME_1M(pCtx->effDataPdu.maxTxLen, TRUE),
                            pCtx->effDataPdu.maxTxTime);
        break;
    case BB_PHY_BLE_2M:
        maxTxTime = WSF_MIN(LL_DATA_LEN_TO_TIME_2M(pCtx->effDataPdu.maxTxLen, TRUE),
                            pCtx->effDataPdu.maxTxTime);
        break;
    case BB_PHY_BLE_CODED:
        maxTxTime = WSF_MIN(LL_DATA_LEN_TO_TIME_CODED_S8(pCtx->effDataPdu.maxTxLen, TRUE),
                            pCtx->effDataPdu.maxTxTime);
        break;
    }
    ...
    case BB_PHY_BLE_1M:
        maxRxTime = WSF_MIN(LL_DATA_LEN_TO_TIME_1M(pCtx->effDataPdu.maxRxLen, TRUE),
                            pCtx->effDataPdu.maxRxTime);
        break;
    case BB_PHY_BLE_2M:
        maxRxTime = WSF_MIN(LL_DATA_LEN_TO_TIME_2M(pCtx->effDataPdu.maxRxLen, TRUE),
                            pCtx->effDataPdu.maxRxTime);
        break;
    case BB_PHY_BLE_CODED:
        maxRxTime = WSF_MIN(LL_DATA_LEN_TO_TIME_CODED_S8(pCtx->effDataPdu.maxRxLen, TRUE),
                            pCtx->effDataPdu.maxRxTime);
        break;
    }
...
void lctrActPeerRejectPhyReq(lctrConnCtx_t *pCtx)
{
    ...
    if (pCtx->llcpNotifyMask & (1 << LCTR_PROC_PHY_UPD)) {
        ...
        case LL_PDU_REJECT_EXT_IND:
            LL_TRACE_WARN2("Peer rejected PHY update, handle=%u, reason=%u",
        ...
        case LL_PDU_UNKNOWN_RSP:
            LL_TRACE_WARN1("Peer rejected PHY update, handle=%u, reason=UNSUPPORTED_REMOTE_FEATURE",
 ...
 */
/*************************************************************************************************/
static void lctrAdjustPhyPreference(lctrConnCtx_t *pCtx, uint8_t allPhys, uint8_t *pTxPhys,
                                    uint8_t *pRxPhys, uint8_t peerPhys)
{
...
static void lctrActStartPhyRsp(lctrConnCtx_t *pCtx)
{
    ...
    lctrAdjustPhyPreference(pCtx, pCtx->allPhys, &txPhys, &rxPhys,
                            pCtx->phyReq.txPhys & pCtx->phyReq.rxPhys);
...
static const uint8_t lctrSlvPhyUpdateNextStateTbl[LCTR_PU_STATE_TOTAL][LCTR_PU_EVENT_TOTAL] = {
    ...
    },
    {
        ...
        LCTR_PU_STATE_FLUSH_BEFORE_PHY_REQ, /* LCTR_PU_EVENT_HOST_PHY_UPD */
        LCTR_PU_STATE_FLUSH_BEFORE_PHY_REQ,
        /* LCTR_PU_EVENT_PEER_PHY_UPDATE_IND */ /* out of sequence; ignore */
        LCTR_PU_STATE_FLUSH_BEFORE_PHY_RSP, /* LCTR_PU_EVENT_PEER_PHY_REQ */
        LCTR_PU_STATE_FLUSH_BEFORE_PHY_REQ,
        /* LCTR_PU_EVENT_PEER_REJECT */ /* should never occur; ignore */
        LCTR_PU_STATE_FLUSH_BEFORE_PHY_REQ,
        /* LCTR_PU_EVENT_INT_PROC_COMP */ /* should never occur */
        LCTR_PU_STATE_FLUSH_BEFORE_PHY_REQ,
    ...
    },
    {
        ...
        /* LCTR_PU_EVENT_PEER_PHY_UPDATE_IND */ /* out of sequence; ignore */
        LCTR_PU_STATE_FLUSH_BEFORE_PHY_RSP,
        /* LCTR_PU_EVENT_PEER_PHY_REQ */ /* out of sequence; ignore */
...
static void lctrResolveCollision(lctrConnCtx_t *pCtx, uint8_t event)
{
    ...
    case LCTR_PU_EVENT_PEER_PHY_REQ:
        LL_TRACE_WARN2(
            "Rejecting PHY_UPD=%u procedure: activeProc=%u, reason=DIFFERENT_TRANSACTION_COLLISION",
            LCTR_PROC_PHY_UPD, pCtx->llcpActiveProc);
    ...
    case LCTR_PU_EVENT_HOST_PHY_UPD:
        ...
        pCtx->llcpNotifyMask |= 1 << LCTR_PROC_PHY_UPD;
        pCtx->llcpPendMask |= 1 << LCTR_PROC_PHY_UPD;
        LL_TRACE_INFO2("Pending PHY_UPD=%u procedure: activeProc=%u", LCTR_PROC_PHY_UPD,
                       pCtx->llcpActiveProc);
        break;
...
bool_t lctrSlvLlcpExecutePhyUpdateSm(lctrConnCtx_t *pCtx, uint8_t event)
{
    ...
    case LCTR_LLCP_STATE_IDLE:
        LL_TRACE_INFO3(
            "lctrSlvLlcpExecutePhyUpdateSm: handle=%u, llcpState=IDLE, phyUpdState=%u, event=%u",
    ...
    case LCTR_LLCP_STATE_BUSY:
        LL_TRACE_INFO3(
            "lctrSlvLlcpExecutePhyUpdateSm: handle=%u, llcpState=BUSY, phyUpdState=%u, event=%u",
            LCTR_GET_CONN_HANDLE(pCtx), pCtx->phyUpdState, event);
