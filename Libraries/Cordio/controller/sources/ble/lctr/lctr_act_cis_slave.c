 ...
 */
/*************************************************************************************************/
static uint8_t LctrCheckCisReq(lctrConnCtx_t *pCtx, lctrCisCtx_t *pCisCtx,
                               const lctrCisReq_t *pCisReq)
{
    if ((pCisReq->isoInterval < LL_MIN_ISO_INTERV) || (pCisReq->isoInterval > LL_MAX_ISO_INTERV) ||
        (pCisReq->nse > LL_MAX_CIS_NSE) || (pCisReq->framing > LL_MAX_ISOAL_PDU_TYPE) ||
        (pCisReq->sduSizeMToS > LL_MAX_SDU_SIZE) || (pCisReq->sduSizeSToM > LL_MAX_SDU_SIZE) ||
        (pCisReq->sduIntervalMToS > LL_MAX_SDU_INTERV) ||
        (pCisReq->sduIntervalSToM > LL_MAX_SDU_INTERV) || (pCisReq->plMToS > LL_MAX_CIS_PL) ||
        (pCisReq->plSToM > LL_MAX_CIS_PL) || (pCisReq->phyMToS > (1 << LL_MAX_CIS_PHY_BIT)) ||
        (pCisReq->phySToM > (1 << LL_MAX_CIS_PHY_BIT)) || (pCisReq->ftMToS < LL_MIN_CIS_FT) ||
        (pCisReq->ftMToS > LL_MAX_CIS_FT) || (pCisReq->ftSToM < LL_MIN_CIS_FT) ||
        (pCisReq->ftSToM > LL_MAX_CIS_FT) || (pCisReq->bnMToS > LL_MAX_CIS_BN) ||
    ...
    if (pCisReq->cisOffMaxUsec < pCisReq->cisOffMinUsec) {
        LL_TRACE_WARN0(
            "LctrCheckCisReq: invalid parameters, cisOffMaxUsec shall be greater than cisOffMinUsec");
        return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
    }

    /* CisOffMaxUsec shall be less than (connInterval – ((NSE – 1) × Sub_Interval + MPTM + T_IFS + MPTS + T_MSS)) */
    if (pCisReq->cisOffMaxUsec >
        (LCTR_CONN_IND_US(pCtx->connInterval) -
         (pCisReq->nse * pCisReq->subIntervUsec - pLctrRtCfg->cisSubEvtSpaceDelay))) {
...
static void lctrSendCisRspPdu(lctrConnCtx_t *pCtx, uint8_t opcode)
{
    ...
    if ((pPdu = lctrTxCtrlPduAlloc(LL_CIS_RSP_LEN)) != NULL) {
        ...
        if (cisOffMinUsec != cisOffMaxUsec) {
            cisOffMinUsec =
                WSF_MAX(cisOffMinUsec, pCtx->connBod.minDurUsec + BbGetSchSetupDelayUs());

            if (cisOffMinUsec > cisOffMaxUsec) {
                /* Cannot schedule the CIS. */
                LL_TRACE_WARN1(
                    "lctrSendCisRspPdu: Proposed offset cannot be scheduled within cisOffMaxUsec=%d",
                    cisOffMaxUsec);
...
{
    LlCisReqInd_t evt = {
        .hdr = { .param = pCisCtx->cisHandle, .event = LL_CIS_REQ_IND, .status = LL_SUCCESS }
    };
    ...
    if (!evtSent) {
        ...
        LL_TRACE_WARN1(
            "Host event masked; reply with reason=UNSUPPORTED_REMOTE_FEATURE, cisHandle=%u",
            pCisCtx->cisHandle);
    }
}
...
void lctrSlvCisLlcpActPeerCisReq(lctrConnCtx_t *pCtx, lctrCisCtx_t *pCisCtx)
{
    ...
    if (LctrCheckCisReq(pCtx, pCisCtx, &lctrDataPdu.pld.cisReq) != LL_SUCCESS) {
        ...
        LL_TRACE_WARN1(
            "Invalid CIS request parameters; reply with reason=INVALID_LMP_PARAMS, cisHandle=%u",
            pCisCtx->cisHandle);
        return;
    }
...
void lctrSlvCisLlcpActPeerCisInd(lctrConnCtx_t *pCtx, lctrCisCtx_t *pCisCtx)
{
    ...
    if (pCigCtx->isBodStarted == FALSE) {
        ...
        aclRefTime = pCtx->data.slv.anchorPointUsec +
                     (pCisCtx->ceRef - pCtx->eventCounter + 1) *
                         LCTR_CONN_IND_US(pCtx->connInterval); /* Reference from the */
        pCisCtx->cisCeRef = 0;
        ...
        while (result == FALSE) {
            ...
            if (pCigCtx->roleData.slv.cigEvtCounter == 0) {
                ...
                cigRefTime = pCigCtx->roleData.slv.anchorPointUsec +
                             pCisCtx->cisCeRef * LCTR_ISO_INT_TO_US(pCigCtx->isoInterval);
            } else {
                cigRefTime = pCigCtx->roleData.slv.anchorPointUsec +
                             (pCisCtx->cisCeRef + 1) * LCTR_ISO_INT_TO_US(pCigCtx->isoInterval);
            }
        ...
        if (cisCount >= 2) {
            ...
            if (cisCount == 2) {
                ...
                if (BbGetTargetTimeDelta(cigRefTime + pHeadCisCtx->subIntervUsec,
                                         pCisCtx->data.slv.anchorOffsetUsec +
                                             pCtx->data.slv.anchorPointUsec +
                                             LCTR_CONN_IND_US(pCtx->connInterval)) > 0) {
            ...
            if (pCigCtx->packing == LL_PACKING_INTERLEAVED) {
                lctrCisCtx_t *pCurCisCtx =
                    lctrCisGetHeadCis(&pCigCtx->list); /* original last CIS */
                lctrCisCtx_t *pNextCisCtx =
                    lctrCisGetNextCis(&pCigCtx->list, pCurCisCtx); /* new last last CIS */
                ...
                while (cisCount > 2) {
                    ...
                    pCurCisCtx =
                        lctrCisGetNextCis(&pCigCtx->list, pCurCisCtx); /* original last CIS */
                    pNextCisCtx =
                        lctrCisGetNextCis(&pCigCtx->list, pNextCisCtx); /* new last last CIS */

                    cisCount--;
                }

                pCurCisCtx->delayUsec =
                    pCisCtx->data.slv.anchorOffsetUsec + pCtx->data.slv.anchorPointUsec +
                    LCTR_CONN_IND_US(pCtx->connInterval) - cigRefTime - prevDelayUsec;
                pNextCisCtx->delayUsec =
                    cigRefTime + pHeadCisCtx->subIntervUsec -
                    (pCisCtx->data.slv.anchorOffsetUsec + pCtx->data.slv.anchorPointUsec +
                     LCTR_CONN_IND_US(pCtx->connInterval));

                LL_TRACE_ERR2("lctrSlvCisLlcpActPeerCisInd, handle=%d pCurCisCtx->delayUsec=%d",
                              pCurCisCtx->cisHandle, pCurCisCtx->delayUsec);
                LL_TRACE_ERR2("lctrSlvCisLlcpActPeerCisInd, handle=%d pNextCisCtx->delayUsec=%d",
                              pNextCisCtx->cisHandle, pNextCisCtx->delayUsec);
            } else {
                ...
                while (cisCount > 2) {
                    pTempCisCtx =
                        lctrCisGetPreCis(&pCigCtx->list, pTempCisCtx); /* new last last CIS */
                    prevDelayUsec += pTempCisCtx->nextCisOffsetUsec;
                    cisCount--;
                }

                pPreCisCtx->nextCisOffsetUsec =
                    pCisCtx->cigSyncDelayUsec - pCisCtx->cisSyncDelayUsec - prevDelayUsec;

                LL_TRACE_ERR1("lctrSlvCisLlcpActPeerCisInd, pPreCisCtx->nextCisOffsetUsec=%d",
                              pPreCisCtx->nextCisOffsetUsec);
            }
        }
...
void lctrCisStorePeerCisReq(lctrConnCtx_t *pCtx, lctrCisCtx_t *pCisCtx)
{
    ...
    pCisCtx->delayUsec =
        pCisCtx
            ->subIntervUsec; /* If LL_PACKING_INTERLEAVED,  delayUsec will be updated in lctrSlvCisLlcpActPeerCisInd */
}
