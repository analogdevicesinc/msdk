...
#define LCTR_DATA_PDU_START_OFFSET \
    2 /*!< Data PDU start offset in a buffer (match ACL header size). */
...
#define LCTR_GET_TXPOWER(pCtx, phy, option) \
    (pCtx->phyTxPower[phy -                 \
                      (((phy == LL_PHY_LE_CODED) && (option == BB_PHY_OPTIONS_BLE_S2)) ? 0 : 1)])
...
typedef struct {
    ...
    union {
        struct {
            ...
            uint8_t
                minTimeSpent; /*!< Minimum time spent in an extreme RSSI zone to trigger a request. */
        ...
        struct {
            ...
            bool_t
                initialPathLossRead; /*!< A power control request is required to start path loss monitoring. */
    ...
    lctrCheckCisEstAclFn_t
        checkCisEstAcl; /*!< Pointer to the check if CIS is established function. */
...
typedef bool_t (*lctrPktEncHdlr_t)(PalCryptoEnc_t *pEnc, uint8_t *pHdr, uint8_t *pBuf,
                                   uint8_t *pMic);
...
typedef void (*lctrPcPowInd_t)(lctrConnCtx_t *pCtx, uint8_t phy, int8_t delta, int8_t txPower,
                               bool_t phyChange);

/*! \brief      Call signature of power report notification handler. */
typedef void (*lctrPcNotifyPwr_t)(lctrConnCtx_t *pCtx, uint8_t reason, uint8_t phy, int8_t txPower,
...
static inline void lctrIncPacketCounterTx(lctrConnCtx_t *pCtx)
{
    if (lctrSetEncryptPktCountHdlr) {
        ...
        if ((pEnc->enaEncrypt) && (pEnc->nonceMode == PAL_BB_NONCE_MODE_PKT_CNTR)) {
            pCtx->txPktCounter++;
        }
    }
}
