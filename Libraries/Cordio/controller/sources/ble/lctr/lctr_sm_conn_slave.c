...
void lctrSlvConnExecuteSm(lctrConnCtx_t *pCtx, uint8_t event)
{
    ...
    case LCTR_CONN_STATE_INITIALIZED:
        ...
        case LCTR_CONN_MSG_RX_CONNECT_IND:
            LL_TRACE_INFO1(
                "lctrSlvConnExecuteSm: handle=%u, state=INITIALIZED, event=RX_CONNECT_IND",
            ...
            lctrNotifyHostConnectInd(
                LCTR_GET_CONN_HANDLE(pCtx), LL_ROLE_SLAVE, &pLctrConnMsg->connEstablish.connInd,
...
void lctrConnStatelessEventHandler(lctrConnCtx_t *pCtx, uint8_t event)
{
    ...
    case LCTR_CONN_TERMINATED:
        LL_TRACE_INFO2("lctrConnStatelessEventHandler: handle=%u, state=%u, event=TERMINATED",
        ...
        break;
    case LCTR_CONN_TERM_SUP_TIMEOUT:
        LL_TRACE_WARN2("lctrConnStatelessEventHandler: handle=%u, state=%u, event=SUP_TIMEOUT",
                       LCTR_GET_CONN_HANDLE(pCtx), pCtx->state);
        ...
        break;
    case LCTR_CONN_TERM_MIC_FAILED:
        LL_TRACE_WARN2("lctrConnStatelessEventHandler: handle=%u, state=%u, event=MIC_FAILED",
                       LCTR_GET_CONN_HANDLE(pCtx), pCtx->state);
        lctrStoreMicFailedTerminateReason(pCtx);
        lctrFlagLinkTerm(pCtx);
        break;
    case LCTR_CONN_MSG_RESET:
        LL_TRACE_INFO2("lctrConnStatelessEventHandler: handle=%u, state=%u, event=RESET",
                       LCTR_GET_CONN_HANDLE(pCtx), pCtx->state);
