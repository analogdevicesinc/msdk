...
struct {
    ...
    uint32_t
        firstRxStartTsUsec; /*!< Timestamp in microseconds of the first received frame regardless of CRC error. */
} lctrMstPerScanIsr;
...
static bool_t lctrIsBigInfoParamsValid(LctrAcadBigInfo_t *pBigInfo)
{
    ...
    if ((pBigInfo->sduInterUsec < MIN_SDU_INTERVAL) ||
        (pBigInfo->sduInterUsec > MAX_SDU_INTERVAL)) {
        ...
        case LL_ACAD_OPCODE_BIG_INFO: {
            ...
            if (pAcadParam->hdr.state == LCTR_ACAD_STATE_DISABLED) {
                ...
                if (lctrIsBigInfoParamsValid(&pAcadParam->bigInfo)) {
                    ...
                    lctrNotifyHostBigInfoAdvReport(LCTR_GET_PER_SCAN_HANDLE(pPerScanCtx),
                                                   &pAcadParam->bigInfo);
                } else {
 ...
 */
/*************************************************************************************************/
static bool_t lctrExtAdvRptPend(lctrExtScanCtx_t *pExtScanCtx, LlExtAdvReportInd_t *pRpt,
                                lctrRptState_t *pState)
{
    ...
    uint64_t hash;
    lctrAdvRptGenerateExtHash(&hash, pRpt->addrType, BstreamToBda64(pRpt->addr), pRpt->eventType,
                              pRpt->advSID, pExtScanCtx->extAdvHdr.did);
 ...
 */
/*************************************************************************************************/
static void lctrExtAdvRptSubmit(lctrExtScanCtx_t *pExtScanCtx, LlExtAdvReportInd_t *pRpt,
                                lctrRptState_t *pState)
{
...
static bool_t lctrExtAdvRptPack(lctrExtScanCtx_t *pExtScanCtx, uint16_t extraEventType,
                                lctrAdvbPduHdr_t *pAdvHdr, uint64_t peerIdAddr,
                                uint8_t peerIdAddrType, int8_t rssi, const uint8_t *pAdvBuf,
                                LlExtAdvReportInd_t *pRpt)
{
    ...
    case LL_PDU_AUX_CONNECT_RSP: {
        ...
        pRpt->len = 0;
        pRpt->eventType |=
            ((pExtScanCtx->extAdvHdr.advMode & LCTR_ADV_MODE_CONN_BIT) ? LL_RPT_EVT_CONN_ADV_BIT :
                                                                         0) |
            ((pExtScanCtx->extAdvHdr.advMode & LCTR_ADV_MODE_SCAN_BIT) ? LL_RPT_EVT_SCAN_ADV_BIT :
                                                                         0) |
        ...
        if (pExtScanCtx->extAdvHdr.extHdrFlags & LL_EXT_HDR_TGT_ADDR_BIT) {
            ...
            lctrGetLocalIdAddr(pExtScanCtx, pExtScanCtx->extAdvHdr.tgtAddr, &loocalIdAddr,
                               &localIdAddrType, peerIdAddr, peerIdAddrType);
    ...
    default: {
        ...
        case LL_PDU_ADV_DIRECT_IND: {
            pRpt->eventType |= LL_RPT_EVT_CONN_ADV_BIT | /* --------------- */ 0 |
                               LL_RPT_EVT_DIRECT_ADV_BIT;
            ...
            lctrGetLocalIdAddr(pExtScanCtx, BstreamToBda64(pTargetA), &loocalIdAddr,
                               &localIdAddrType, peerIdAddr, peerIdAddrType);
            Bda64ToBstream(pRpt->directAddr, loocalIdAddr);
            pRpt->directAddrType = localIdAddrType;
            break;
        ...
        case LL_PDU_ADV_SCAN_IND:
            pRpt->eventType |= /* --------------- */ 0 | LL_RPT_EVT_SCAN_ADV_BIT |
                               /* ----------------- */ 0;
            break;
        ...
        case LL_PDU_SCAN_RSP:
            pRpt->eventType |= /* --------------- */ 0 | LL_RPT_EVT_SCAN_ADV_BIT |
                               /* ----------------- */ 0;
            pRpt->eventType |=
                (lctrMstExtScanIsr.advPduType == LL_PDU_ADV_IND) ? LL_RPT_EVT_CONN_ADV_BIT : 0;
 ...
 */
/*************************************************************************************************/
static bool_t lctrExtAdvDataReassemble(lctrExtScanCtx_t *pExtScanCtx, const uint8_t *pChainBuf,
                                       uint8_t chainLen, int8_t rssi)
{
 ...
 */
/*************************************************************************************************/
static bool_t lctrPerAdvDataReassemble(lctrPerScanCtx_t *pPerScanCtx, const uint8_t *pChainBuf,
                                       uint8_t chainLen, int8_t rssi)
{
    /* Ensure report event is already packed. */
...
static inline bool_t lctrPerAdvRptPack(BbOpDesc_t *pOp, const uint8_t *pAdvBuf,
                                       uint8_t extAdvHdrFlags, LlPerAdvReportInd_t *pRpt)
{
 ...
 */
/*************************************************************************************************/
static inline bool_t lctrPerAdvRptPackTruncate(BbOpDesc_t *pOp, const uint8_t *pAdvBuf,
...
void lctrScanBackoffRspSuccess(lctrExtScanCtx_t *pExtScanCtx)
{
    if (++pExtScanCtx->data.scan.consRspSuccess >= 2) {
        pExtScanCtx->data.scan.upperLimit = pExtScanCtx->data.scan.upperLimit >>
                                            1; /* divide by 2 */
    ...
    pExtScanCtx->data.scan.backoffCount =
        (LlMathRandNum() & (pExtScanCtx->data.scan.upperLimit - 1)) + 1;
}
...
void lctrMstDiscoverRxExtAdvPktPostProcessHandler(BbOpDesc_t *pOp, const uint8_t *pAdvBuf)
{
    ...
    case LL_PDU_ADV_EXT_IND: {
        ...
        if (pExtScanCtx->extAdvHdr.advMode == 0) /* Non-connectable and non-scannable. */
        {
            if ((extAdvHdrFlags & LL_EXT_HDR_AUX_PTR_BIT) == 0) {
                if ((extAdvHdrFlags & LL_EXT_HDR_ADV_ADDR_BIT) == 0) {
                    LL_TRACE_WARN0(
                        "Ignoring LL_PDU_ADV_EXT_IND due to missing mandatory advA when there is no auxiliary packet");
                ...
                if (BbBleExtPduFiltCheck(&params, &pOp->prot.pBle->pduFilt, FALSE,
                                         &pScan->filtResults) == FALSE) {
        ...
        if ((lctrPerCreateSync.state == LCTR_CREATE_SYNC_STATE_DISCOVER) &&
            (extAdvHdrFlags & LL_EXT_HDR_ADI_BIT) && (extAdvHdrFlags & LL_EXT_HDR_ADV_ADDR_BIT)) {
            ...
            (void)BbBleExtPduFiltCheck(&params, &pOp->prot.pBle->pduFilt, FALSE,
                                       &pScan->filtResults);
            ...
            if (lctrPerCreateSync.filtParam.filterPolicy == LL_PER_SCAN_FILTER_PL_BIT) {
                ...
                if ((lctrPerCreateSync.filtParam.advSID != pExtScanCtx->extAdvHdr.sid) ||
                    (lctrPerCreateSync.filtParam.advAddrType !=
                     (peerIdAddrType & LL_ADDR_RANDOM_BIT)) ||
        ...
        if (extAdvHdrFlags & LL_EXT_HDR_AUX_PTR_BIT) {
            pOp->minDurUsec =
                0; /* Update primary scan BOD min duration so that secondary scan can be scheduled. */

            lctrUnpackAuxPtr(&pExtScanCtx->priChAuxPtr, pExtScanCtx->extAdvHdr.pAuxPtr);
            uint32_t endTs =
                pScan->advStartTsUsec +
                SchBleCalcAdvPktDurationUsec(pBle->chan.rxPhy, pScan->advRxPhyOptions,
                                             LL_ADV_HDR_LEN + pScan->filtResults.pduLen);
            lctrMstAuxDiscoverOpCommit(pExtScanCtx, &pExtScanCtx->priChAuxPtr,
                                       pScan->advStartTsUsec, endTs);
            ...
            LlExtAdvReportInd_t *const pRpt = &pExtScanCtx->data.scan.advRpt;
            if (lctrExtAdvRptPack(pExtScanCtx, 0, &advHdr, peerIdAddr, peerIdAddrType,
                                  pScan->advRssi, pAdvBuf, pRpt)) {
    ...
    default: {
        ...
        LlExtAdvReportInd_t *const pRpt = &pExtScanCtx->data.scan.advRpt;
        if (lctrExtAdvRptPack(pExtScanCtx, 0, &advHdr, peerIdAddr, peerIdAddrType, pScan->advRssi,
                              pAdvBuf, pRpt)) {
            pRpt->secPhy = LL_PHY_NONE;
            if (lctrExtAdvRptPend(pExtScanCtx, pRpt, &pExtScanCtx->data.scan.advRptState)) {
                lctrExtAdvRptSubmit(pExtScanCtx, pRpt, &pExtScanCtx->data.scan.advRptState);
...
bool_t lctrMstDiscoverRxAuxAdvPktHandler(BbOpDesc_t *pOp, const uint8_t *pAdvBuf)
{
    ...
    lctrMstExtScanIsr.extAdvHdrLen = lctrUnpackExtAdvHeader(
        &pExtScanCtx->extAdvHdr, &lctrMstExtScanIsr.extAdvHdrFlags, pExtAdvHdr);
    ...
    case LL_PDU_AUX_ADV_IND: {
        ...
        if ((lctrPerCreateSync.state == LCTR_CREATE_SYNC_STATE_DISCOVER) &&
            ...
            uint32_t endTs =
                pAuxScan->auxStartTsUsec + SchBleCalcAdvPktDurationUsec(pBle->chan.rxPhy,
                                                                        pAuxScan->auxRxPhyOptions,
                                                                        pAuxScan->txAuxReqLen);
            lctrMstPerScanOpCommit(pExtScanCtx, &pExtScanCtx->priChAuxPtr,
                                   &pExtScanCtx->secSyncInfo, pAuxScan->auxStartTsUsec, endTs);
    ...
    default:
        LL_TRACE_WARN1("Received advertising PDU with invalid PDU type=%u",
                       lctrMstExtScanIsr.advHdr.pduType);
        lctrMstExtScanIsr.filtResult = TRUE;
    }
                ...
                if (pExtScanCtx->scanParam.ownAddrType & LL_ADDR_IDENTITY_BIT) {
                    ...
                    if (pExtScanCtx->extAdvHdr.extHdrFlags & LL_EXT_HDR_TGT_ADDR_BIT) {
                        ...
                        if (BDA64_ADDR_IS_RPA(localAddr)) {
                            BbBleResListUpdateLocal(pAuxScan->filtResults.peerIdAddrRand,
                                                    pAuxScan->filtResults.peerIdAddr, &localAddr);
                        }
                    } else {
                        ...
                        if (BbBleResListReadLocal(pAuxScan->filtResults.peerIdAddrRand,
                                                  pAuxScan->filtResults.peerIdAddr, &localAddr)) {
                            localAddrRand = TRUE;
                        }
                    }
    ...
    pExtScanCtx->data.scan.auxAdvRpt.eventType =
        (pExtScanCtx->extAdvHdr.extHdrFlags & LL_EXT_HDR_TGT_ADDR_BIT) ? LL_RPT_EVT_DIRECT_ADV_BIT :
                                                                         0;
...
bool_t lctrMstDiscoverRxAuxScanRspHandler(BbOpDesc_t *pOp, const uint8_t *pRspBuf)
{
    ...
    if (BbBleExtPduFiltCheck(&params, &pOp->prot.pBle->pduFilt, FALSE, &pAuxScan->filtResults) ==
        FALSE) {
    ...
    if ((pExtScanCtx->extAdvHdr.advMode & (LCTR_ADV_MODE_SCAN_BIT | LCTR_ADV_MODE_CONN_BIT)) != 0) {
        LL_TRACE_WARN1("Received auxiliary PDU with invalid advMode=0x%02x",
                       pExtScanCtx->extAdvHdr.advMode);
        lctrMstExtScanIsr.filtResult = TRUE;
        return FALSE;
    }
    ...
    if (pExtScanCtx->data.scan.scanReqAdvAddr != pAuxScan->filtResults.peerAddr) {
        LL_TRACE_WARN0(
            "Ignore AUX_SCAN_RSP since advAddr doesn't match the one sent in the AUX_SCAN_REQ");
        lctrMstExtScanIsr.filtResult = TRUE;
        return FALSE;
    }
...
uint32_t lctrMstDiscoverRxAuxChainHandler(BbOpDesc_t *pOp, const uint8_t *pChainBuf)
{
    ...
    if (lctrMstExtScanIsr.isHdrParsed == TRUE) {
        ...
        lctrMstExtScanIsr.advDataLen =
            lctrMstExtScanIsr.advHdr.len - LCTR_EXT_HDR_CMN_LEN - pExtScanCtx->extAdvHdr.extHdrLen;
        lctrMstExtScanIsr.isHdrParsed = FALSE;
    } else {
        pExtAdvHdr = pChainBuf + lctrUnpackAdvbPduHdr(&lctrMstExtScanIsr.advHdr, pChainBuf);
        lctrMstExtScanIsr.pAdvData =
            pExtAdvHdr + lctrUnpackExtAdvHeader(&pExtScanCtx->extAdvHdr,
                                                &lctrMstExtScanIsr.extAdvHdrFlags, pExtAdvHdr);
        lctrMstExtScanIsr.advDataLen =
            lctrMstExtScanIsr.advHdr.len - LCTR_EXT_HDR_CMN_LEN - pExtScanCtx->extAdvHdr.extHdrLen;
    }
...
bool_t lctrMstDiscoverRxAuxChainPostProcessHandler(BbOpDesc_t *pOp, const uint8_t *pChainBuf)
{
    ...
    if (pChainBuf == NULL) {
        ...
        lctrExtAdvRptSubmit(pExtScanCtx, &pExtScanCtx->data.scan.advRpt,
                            &pExtScanCtx->data.scan.advRptState);
        return FALSE;
    }
    ...
    if (pExtScanCtx->data.scan.advRptState == LCTR_RPT_STATE_COMP) {
        ...
        LL_TRACE_WARN1(
            "Cannot generate report due to pending report; do not receive CHAIN_IND, SID=%u",
            pExtScanCtx->extAdvHdr.sid);
        return FALSE;
    }
    ...
    do {
        if (pExtScanCtx->data.scan.advRptState == LCTR_RPT_STATE_IDLE) {
            ...
            if (pExtScanCtx->extAdvHdr.advMode & LCTR_ADV_MODE_SCAN_BIT) {
                ...
                if (!lctrExtAdvRptPack(pExtScanCtx, pExtScanCtx->data.scan.auxAdvRpt.eventType,
                                       &lctrMstExtScanIsr.advHdr, peerIdAddr, peerIdAddrType,
                                       pAuxAdvRpt->rssi, NULL, pAuxAdvRpt)) {
                    LL_TRACE_WARN1("Invalid report parameters; do not receive CHAIN_IND, SID=%u",
                                   pExtScanCtx->extAdvHdr.sid);
                ...
                if (lctrExtAdvRptPend(pExtScanCtx, pAuxAdvRpt,
                                      &pExtScanCtx->data.scan.auxAdvRptState)) {
                    lctrExtAdvRptSubmit(pExtScanCtx, pAuxAdvRpt,
                                        &pExtScanCtx->data.scan.auxAdvRptState);
                }
            }

            LlExtAdvReportInd_t *const pAdvRpt = &pExtScanCtx->data.scan.advRpt;
            if (!lctrExtAdvRptPack(pExtScanCtx, lctrMstExtScanIsr.chainExtraEventType,
                                   &lctrMstExtScanIsr.advHdr, peerIdAddr, peerIdAddrType,
                                   pAuxScan->auxAdvRssi, NULL, pAdvRpt)) {
        ...
        if (!lctrExtAdvDataReassemble(pExtScanCtx, lctrMstExtScanIsr.pAdvData,
                                      lctrMstExtScanIsr.advDataLen, pAuxScan->auxAdvRssi)) {
            /* Do not finish receiving chain. */
            LL_TRACE_WARN1("Insufficient scan buffer size; do not receive CHAIN_IND, SID=%u",
                           pExtScanCtx->extAdvHdr.sid);
            pExtScanCtx->data.scan.advRpt.eventType |= LL_RPT_DATA_INC_TRUNC
                                                       << LCTR_DATA_STATUS_SHIFT;
            result = FALSE;
            break;
        }
    ...
    if ((result == FALSE) || /* Invalid packet, duplicate found or insufficient buffer. */
        ((lctrMstExtScanIsr.extAdvHdrFlags & LL_EXT_HDR_AUX_PTR_BIT) ==
         0)) /* No more auxiliary packet. */
    {
    ...
    if (pRspBuf) {
        ...
        case LL_PDU_SCAN_RSP: {
            ...
            if (pExtScanCtx->data.scan.scanReqAdvAddr != pScan->filtResults.peerAddr) {
                LL_TRACE_WARN0(
                    "Ignore scan_rsp since advAddr doesn't match the one sent in the scan_req");
                break;
            }
...
static void lctrMstExtDiscoverReschedule(lctrExtScanCtx_t *pExtScanCtx)
{
    ...
    uint8_t scanPhyIndex = (LCTR_GET_EXT_SCAN_HANDLE(pExtScanCtx) == LCTR_SCAN_PHY_CODED) ?
                               LCTR_SCAN_PHY_CODED :
                               LCTR_SCAN_PHY_1M;
    ...
    if ((pExtScanCtx->scanParam.scanInterval != pExtScanCtx->scanParam.scanWindow) &&
        ((pScan->elapsedUsec + pOp->minDurUsec) <
         LCTR_BLE_TO_US(pExtScanCtx->scanParam.scanWindow))) {
...
void lctrMstExtDiscoverEndOp(BbOpDesc_t *pOp)
{
    ...
    if (pExtScanCtx->shutdown || pExtScanCtx->selfTerm) {
        ...
        if ((pExtScanCtx->auxOpPending &&
             (pExtScanCtx->bodTermCnt >= 2)) || /* Wait for both ExtScan and AuxScan operations. */
            (!pExtScanCtx->auxOpPending &&
             (pExtScanCtx->bodTermCnt >= 1))) /* Wait only for ExtScan operation. */
        {
...
void lctrMstPerScanEndOp(BbOpDesc_t *pOp)
{
    ...
    if (pPerScanCtx->firstPerAdvRcv == FALSE) {
        ...
        uint16_t fastTermCnt = pPerScanCtx->bodAborted ? (LCTR_FAST_TERM_CNT + 1) :
                                                         LCTR_FAST_TERM_CNT;

        if ((pPerScanCtx->eventCounter - pPerScanCtx->initEventCounter) == (fastTermCnt - 1)) {
            LL_TRACE_ERR0(
                "!!! lctrMstPerScanEndOp: Failed to receive AUX_SYNC_IND within first 6 intervals");
    ...
    while (TRUE) {
        ...
        pBle->chan.chanIdx =
            lctrPeriodicSelectNextChannel(&pPerScanCtx->chanParam, pPerScanCtx->eventCounter);

        if (SchInsertAtDueTime(pOp, lctrPerScanResolveConflict)) {
            break;
        }
        LL_TRACE_WARN1("!!! Periodic scanning schedule conflict eventCounter=%u",
...
uint32_t lctrMstPerScanRxPerAdvPktHandler(BbOpDesc_t *pOp, const uint8_t *pAdvBuf, uint8_t status)
{
    ...
    lctrMstExtScanIsr.pAdvData =
        pExtAdvHdr + lctrUnpackExtAdvHeader(&pPerScanCtx->extAdvHdr,
                                            &lctrMstExtScanIsr.extAdvHdrFlags, pExtAdvHdr);
    lctrMstExtScanIsr.advDataLen =
        advHdrLen - 1 -
        pPerScanCtx->extAdvHdr.extHdrLen; /* 1 byte for the extended header length + advMode. */
    ...
    /* Store anchor point. */
    if ((!lctrMstPerScanIsr.syncWithSlave) && (pMstPerScan->perIsFirstTs == TRUE) &&
...
bool_t lctrMstPerScanRxPerAdvPktPostHandler(BbOpDesc_t *pOp, const uint8_t *pAdvBuf)
{
    ...
    if (pAdvBuf == NULL) {
        ...
        if (pPerScanCtx->advRptState == LCTR_RPT_STATE_IN_PROGRESS &&
            pPerScanCtx->state == LCTR_PER_SCAN_STATE_SYNC_ESTD) {
    ...
    if (pPerScanCtx->advRptState == LCTR_RPT_STATE_COMP) {
        /* Do not finish receiving chain. */
        LL_TRACE_WARN1(
            "Cannot generate periodic report due to pending report; do not receive CHAIN_IND, SID=%u",
            pPerScanCtx->extAdvHdr.sid);
        return FALSE;
    }
    ...
    do {
        if (pPerScanCtx->advRptState == LCTR_RPT_STATE_IDLE) {
            ...
            if (!lctrPerAdvRptPack(pOp, pAdvBuf, lctrMstExtScanIsr.extAdvHdrFlags,
                                   &pPerScanCtx->advRpt)) {
            ...
            if (!lctrPerAdvRptPend(pPerScanCtx)) {
                /* Stop receiving chain. */
                LL_TRACE_WARN1("Filter report; do not receive CHAIN_IND, SID=%u",
                               pPerScanCtx->extAdvHdr.sid);
                result = FALSE;
                break;
            }
        ...
        if (!lctrPerAdvDataReassemble(pPerScanCtx, lctrMstExtScanIsr.pAdvData,
                                      lctrMstExtScanIsr.advDataLen, pMstPerScan->perAdvRssi)) {
            /* Stop receiving chain. */
            LL_TRACE_WARN1(
                "Insufficient periodic scan buffer size; do not receive CHAIN_IND, SID=%u",
                pPerScanCtx->extAdvHdr.sid);
