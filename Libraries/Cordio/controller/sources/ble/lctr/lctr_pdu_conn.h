...
enum {
    ...
    LCTR_PB_COMPLETE_AUTO_FLUSH =
        3, /*!< A complete L2CAP PDU automatically flushable (not allowed for LE-U). */
...
typedef struct {
    ...
    uint16_t
        offset[LCTR_OFFSET_COUNT]; /*!< Anchor point offset from \a refConnEvtCnt, first preference. */
} lctrConnParam_t;
...
typedef struct {
    ...
    uint32_t
        sduIntervalMToS; /*!< Time interval between the start of consecutive SDUs from the master Host  */
    uint32_t
        sduIntervalSToM; /*!< Time interval between the start of consecutive SDUs from the master Host  */
    uint16_t plMToS; /*!< Master to slave payload. */
    uint16_t plSToM; /*!< Slave to master payload. */
    uint8_t nse; /*!< Number of subevent. */
    uint32_t
        subIntervUsec; /*!< Contain the time between the start of a subevent and the start of the next subevent, 24 significant bits. */
    ...
    uint32_t
        cisOffMinUsec; /*!< Contain the minimum time between the CE and the first CIS anchor point. */
    uint32_t
        cisOffMaxUsec; /*!< Contain the maximum time between the CE and the first CIS anchor point. */
...
typedef struct {
    ...
    uint32_t
        cisOffUsec; /*!< Contain the time from the start of the referenced CE to the first CIS anchor point. */
