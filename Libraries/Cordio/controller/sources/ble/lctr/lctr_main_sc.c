...
static void lctrNotifyReadLocalP256PubKeyInd(const uint8_t *pPubKey)
{
    LlReadLocalP256PubKeyInd_t evt = {
        .hdr = { .param = 0, .event = LL_READ_LOCAL_P256_PUB_KEY_CMPL_IND, .status = LL_SUCCESS }
    };
...
static void lctrNotifyGenerateDhKeyInd(const uint8_t *pDhKey)
{
    LlGenerateDhKeyInd_t evt = {
        .hdr = { .param = 0, .event = LL_GENERATE_DHKEY_CMPL_IND, .status = LL_SUCCESS }
    };
...
{
    const uint8_t privKey[LL_ECC_KEY_LEN] = { 0xbd, 0x1a, 0x3c, 0xcd, 0xa6, 0xb8, 0x99, 0x58,
