...
static const lctrActFn_t
    lctrCisTermActionTbl[LCTR_CIS_TERM_STATE_TOTAL][LCTR_CIS_TERM_EVENT_TOTAL] = {
        {
...
static uint8_t lctrRemapTermEvent(lctrConnCtx_t *pCtx, uint8_t event)
{
    ...
    case LCTR_CONN_MSG_RX_LLCP:
        ...
        case LL_PDU_CIS_TERM_IND: {
            lctrCisCtx_t *pCisCtx =
                lctrFindCisById(lctrDataPdu.pld.cisTerm.cigId, lctrDataPdu.pld.cisTerm.cisId);
            pCtx->llcpCisHandle =
                pCisCtx->cisHandle; /* Update LLCP CIS handle for connection context from peer. */
    ...
    case LCTR_CONN_MSG_API_DISCONNECT: {
        ...
        if ((pCisCtx = lctrFindCisByHandle(pLctrConnMsg->cisDisc.cisHandle)) != NULL) {
            pCtx->llcpCisHandle =
                pLctrConnMsg->cisDisc
                    .cisHandle; /* Update LLCP CIS handle for connection context from host. */
...
static void lctrResolveCollision(lctrConnCtx_t *pCtx, lctrCisCtx_t *pCisCtx, uint8_t event)
{
    ...
    case LCTR_CIS_TERM_EVENT_PEER_DISC:
        if ((pCtx->llcpActiveProc == LCTR_PROC_CMN_CH_MAP_UPD) ||
            ...
            pCtx->llcpPendMask |= 1 << LCTR_PROC_CIS_TERM_PEER;
            LL_TRACE_INFO2("Pending CIS_TERM_IND=%u procedure: activeProc=%u",
                           LCTR_PROC_CIS_TERM_PEER, pCtx->llcpActiveProc);
        }
        break;
    ...
    case LCTR_CIS_TERM_EVENT_HOST_DISC:
        ...
        pCtx->llcpPendMask |= 1 << LCTR_PROC_CIS_TERM;
        LL_TRACE_INFO2("Pending CIS_TERM_IND=%u procedure: activeProc=%u", LCTR_PROC_CIS_TERM,
                       pCtx->llcpActiveProc);
        break;
...
bool_t lctrLlcpExecuteCisTermSm(lctrConnCtx_t *pCtx, uint8_t event)
{
    ...
    case LCTR_LLCP_STATE_IDLE:
        LL_TRACE_INFO3(
            "lctrLlcpExecuteCisTermSm: cis_handle=%u, llcpState=IDLE, termState=%u, event=%u",
            pCisCtx->cisHandle, pCisCtx->termState, event);
    ...
    case LCTR_LLCP_STATE_BUSY:
        LL_TRACE_INFO3(
            "lctrLlcpExecuteCisTermSm: cis_handle=%u, llcpState=BUSY, termState=%u, event=%u",
            pCisCtx->cisHandle, pCisCtx->termState, event);
