...
void lctrSendPerSyncFromScan(lctrConnCtx_t *pCtx)
{
    ...
    WSF_ASSERT((pPerScanCtx->enabled == TRUE) &&
    ...
    if ((pPdu = lctrTxCtrlPduAlloc(LL_PERIODIC_SYNC_PDU_LEN)) != NULL) {
        ...
        if (BbGetTargetTimeDelta(ceRefEnd, paAnchor) > 0) {
            uint16_t numPE =
                BbGetTargetTimeDelta(ceRefEnd, paAnchor) / pPerScanCtx->perInterUsec + 1;
        ...
        LL_TRACE_INFO1("                               PA lastAnchorPoint = %u",
                       pPerScanCtx->lastAnchorPointUsec);
        ...
        UINT16_TO_BSTREAM(pBuf, offs | (offsUnits << 13) | /* Offset units. */
        ...
        uint64_t temp = pPerScanCtx->chanParam.chanMask | /* SyncInfo - ChMap */
                        ((uint64_t)pPerScanCtx->sca
                         << 37); /* SyncInfo - SCA of the device sending AUX_SYNC_IND. */
        ...
        field8 = (pPerScanCtx->advSID << 0) | (pPerScanCtx->trsfAddrType << 4) |
...
void lctrSendPerSyncFromBcst(lctrConnCtx_t *pCtx)
{
    ...
    if ((pPdu = lctrTxCtrlPduAlloc(LL_PERIODIC_SYNC_PDU_LEN)) != NULL) {
        ...
        if (BbGetTargetTimeDelta(ceRefEnd, paAnchor) > 0) {
            uint16_t numPE =
                BbGetTargetTimeDelta(ceRefEnd, paAnchor) / pAdvSet->perParam.perAdvInterUsec + 1;

            paAnchor += numPE * pAdvSet->perParam.perAdvInterUsec;
            paEventCounter += numPE;
        } else {
            uint16_t numPE =
                BbGetTargetTimeDelta(paAnchor, ceRefEnd) / pAdvSet->perParam.perAdvInterUsec;
        ...
        UINT16_TO_BSTREAM(pBuf,
                          LCTR_PER_INTER_TO_MS(pAdvSet->perParam.perAdvInterUsec)); /* Interval */

        uint64_t temp = pAdvSet->perParam.perChanParam.chanMask | /* SyncInfo - ChMap */
                        ((uint64_t)lctrComputeSca()
                         << 37); /* SyncInfo - SCA of the device sending AUX_SYNC_IND. */
        UINT40_TO_BSTREAM(pBuf, temp);
        ...
        if ((pAdvSet->param.ownAddrType & LL_ADDR_RANDOM_BIT) &&
            (pAdvSet->bdAddrRndValid == TRUE)) {
            aType = 1;
        } else if ((pAdvSet->param.ownAddrType == LL_ADDR_PUBLIC_IDENTITY) &&
                   (BDA64_ADDR_IS_RPA(pAdvSet->advA))) {
            aType = 1;
        }

        field8 = (pAdvSet->param.advSID << 0) | (aType << 4) |
                 (lctrComputeSca() << 5); /* SCA of the device sending this PDU. */
        UINT8_TO_BSTREAM(pBuf, field8);
...
void lctrReceivePeriodicSyncInd(lctrConnCtx_t *pCtx)
{
    ...
    if (LctrMstPerIsSync(lctrDataPdu.pld.perSyncInd.sid, lctrDataPdu.pld.perSyncInd.aType,
                         lctrDataPdu.pld.perSyncInd.advA)) {
        LL_TRACE_WARN0("    ---- Ignore LL_PERIODIC_SYNC_IND, already in sync");
        return;
    }

    LL_TRACE_INFO1("### LlEvent ###  Periodic sync transfer received, handle=%u",
