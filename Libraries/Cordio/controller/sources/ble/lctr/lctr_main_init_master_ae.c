...
static void lctrMstExtInitExecuteCommonSm(LctrExtScanMsg_t *pMsg)
{
    ...
    case LCTR_EXT_INIT_MSG_INITIATE: {
        LL_TRACE_INFO1("lctrMstExtInitExecuteCommonSm: scanMode=%u, event=INITIATE",
                       lmgrCb.scanMode);
        ...
        for (i = 0; i < LCTR_SCAN_PHY_TOTAL; i++) {
            if (lctrMstExtInit.enaPhys & (1 << i)) {
                if ((status = lctrExtInitSetupInitiate(
                         &lctrMstExtInitTbl[i], pExtInitMsg->peerAddrType, pExtInitMsg->peerAddr,
    ...
    case LCTR_EXT_INIT_MSG_INITIATE_CANCEL:
        LL_TRACE_INFO1("lctrMstExtInitExecuteCommonSm: scanMode=%u, event=INITIATE_CANCEL",
                       lmgrCb.scanMode);
        break;
    ...
    case LCTR_EXT_INIT_MSG_TERMINATE:
        LL_TRACE_INFO1("lctrMstExtInitExecuteCommonSm: scanMode=%u, event=TERMINATE",
                       lmgrCb.scanMode);
        break;

    default:
        LL_TRACE_ERR2("lctrMstExtInitExecuteCommonSm: scanMode=%u, event=%u -- unknown event",
                      lmgrCb.scanMode, pMsg->hdr.event);
        /* No action required. */
        break;
    }
}
                                  ...
                                  uint64_t peerAddr, uint8_t peerAddrType)
{
    ...
    if (llTesterCb.connIndEnabled) {
        memcpy(&pConnInd->accessAddr, &llTesterCb.connInd.accessAddr,
    ...
    pExtInitCtx->reqPduHdr.txAddrRnd =
                                  ...
                                  uint64_t peerAddr, uint8_t peerAddrType)
{
    ...
    pBle->pduFilt.pduTypeFilt = (1 << LL_PDU_AUX_ADV_IND) | (1 << LL_PDU_AUX_CONNECT_RSP);
    if (pExtInitCtx->data.init.filtPolicy & LL_SCAN_FILTER_WL_BIT) {
        pBle->pduFilt.wlPduTypeFilt = pBle->pduFilt.pduTypeFilt;
    }
