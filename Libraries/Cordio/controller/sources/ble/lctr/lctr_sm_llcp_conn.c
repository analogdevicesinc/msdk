...
static bool_t lctrFeatureAvail(lctrConnCtx_t *pCtx, uint8_t proc, uint8_t event)
{
    ...
    case LCTR_PROC_CMN_FEAT_EXCH:
        if ((pCtx->role == LL_ROLE_SLAVE) && (event == LCTR_PROC_CMN_EVT_HOST_START) &&
            ((pCtx->usedFeatSet & LL_FEAT_SLV_INIT_FEAT_EXCH) == 0)) {
            LL_TRACE_WARN1("Requested LL_FEAT_SLV_INIT_FEAT_EXCH not available, usedFeatSet=0x%08x",
                           pCtx->usedFeatSet);
            result = FALSE;
        }
        break;
    case LCTR_PROC_CMN_DATA_LEN_UPD:
        if ((pCtx->usedFeatSet & LL_FEAT_DATA_LEN_EXT) == 0) {
            LL_TRACE_WARN1("Requested LCTR_PROC_CMN_DATA_LEN_UPD not available, usedFeatSet=0x%08x",
                           pCtx->usedFeatSet);
            result = FALSE;
        }
        break;
    ...
    case LCTR_PROC_CMN_SET_MIN_USED_CHAN:
        if (pCtx->role == LL_ROLE_MASTER) {
            ...
            if (event == LCTR_PROC_CMN_EVT_RECV_REQ) {
                ...
                LL_TRACE_WARN1(
                    "Requested LCTR_PROC_CMN_SET_MIN_USED_CHAN not available, usedFeatSet=0x%08x",
                    pCtx->usedFeatSet);
                result = FALSE;
            }
        }
    ...
    case LCTR_PROC_CMN_PER_ADV_SYNC_TRSF:
        if ((event == LCTR_PROC_CMN_EVT_HOST_START) &&
            ...
            LL_TRACE_WARN1(
                "Requested LCTR_PROC_CMN_PER_ADV_SYNC_TRSF not available, FeatSet=0x%08x",
                lmgrCb.features);
            result = FALSE;
    ...
    case LCTR_PROC_CMN_REQ_PEER_SCA:
        if ((pCtx->usedFeatSet & LL_FEAT_SCA_UPDATE) == 0) {
            LL_TRACE_WARN1("Requested LCTR_PROC_CMN_REQ_PEER_SCA not available, usedFeatSet=0x%08x",
                           pCtx->usedFeatSet);
            result = FALSE;
        }
        break;
...
static void lctrNotifyHostReadRemoteFeatCnf(lctrConnCtx_t *pCtx, uint8_t status)
{
    ...
    LlReadRemoteFeatCnf_t evt = {
        .hdr = { .param = handle, .event = LL_READ_REMOTE_FEAT_CNF, .status = status },
    ...
    LL_TRACE_INFO2("### LlEvent ###  LL_READ_REMOTE_FEAT_CNF, handle=%u, status=%u", handle,
                   status);
...
bool_t lctrLlcpExecuteCommonSm(lctrConnCtx_t *pCtx, uint8_t event)
{
    ...
    case LCTR_LLCP_STATE_IDLE:
        LL_TRACE_INFO3("lctrLlcpExecuteCommonSm: handle=%u, proc=%u, state=IDLE, event=%u",
    ...
    case LCTR_LLCP_STATE_BUSY:
        LL_TRACE_INFO3("lctrLlcpExecuteCommonSm: handle=%u, proc=%u, state=BUSY, event=%u",
                       LCTR_GET_CONN_HANDLE(pCtx), proc, event);
        ...
        case LCTR_PROC_CMN_EVT_INT_START:
            if (lctrFeatureAvail(pCtx, proc, event)) {
                ...
                if (pCtx->llcpActiveProc != proc) {
                    LL_TRACE_INFO1("Pending procedure in progress: activeProc=%u",
                                   pCtx->llcpActiveProc);
                    pCtx->llcpPendMask |= 1 << proc;
                }
                if (event == LCTR_PROC_CMN_EVT_HOST_START) {
        ...
        case LCTR_PROC_CMN_EVT_RECV_IND: /* Completion from message */
            if (proc != pCtx->llcpActiveProc) {
                if (lctrFeatureAvail(pCtx, proc, event)) {
                    LL_TRACE_WARN1("Procedure collision; pending incomplete request, activeProc=%u",
                                   pCtx->llcpActiveProc);
                    lctrExecAction(pCtx, proc, LCTR_PROC_CMN_ACT_RECV_REQ);
                    pCtx->llcpIncompMask |= 1 << proc;
                }
        ...
        case LCTR_PROC_CMN_EVT_RECV_RSP:
            if (proc == pCtx->llcpActiveProc) {
                ...
                LL_TRACE_ERR1("Unexpected response packet, expected llcpActiveProc=%u",
                              pCtx->llcpActiveProc);
                break;
            }
            /* Fallthrough */
    ...
    if (proc == LCTR_PROC_CMN_TERM) {
        if ((pCtx->llcpActiveProc == LCTR_PROC_CMN_TERM) ||
            (pCtx->llcpIncompMask & (1 << LCTR_PROC_CMN_TERM))) {
