...
static void lctrMstExtInitiateScanReschedule(lctrExtScanCtx_t *pExtInitCtx)
{
    ...
    if ((pExtInitCtx->data.init.param.scanInterval != pExtInitCtx->data.init.param.scanWindow) &&
        ((pScan->elapsedUsec + pOp->minDurUsec) <
         LCTR_BLE_TO_US(pExtInitCtx->data.init.param.scanWindow))) {
        ...
        if (SchInsertEarlyAsPossible(pOp, min, max)) {
            ...
            pScan->elapsedUsec =
                BbGetTargetTimeDelta(pOp->dueUsec, pExtInitCtx->data.init.scanWinStartUsec);
...
void lctrMstExtInitiateScanEndOp(BbOpDesc_t *pOp)
{
    ...
    if (pExtInitCtx->shutdown || pExtInitCtx->selfTerm) {
        ...
        if ((pExtInitCtx->auxOpPending &&
             (pExtInitCtx->bodTermCnt >= 2)) || /* Wait for both ExtScan and AuxScan operations. */
            (!pExtInitCtx->auxOpPending &&
             (pExtInitCtx->bodTermCnt >= 1))) /* Wait only for ExtScan operation. */
        {
...
void lctrMstExtInitiateEndOp(BbOpDesc_t *pOp)
{
                                   ...
                                   * (means that the self termination was generated from a legacy advertisement connection). */
    {
        ...
        while (TRUE) {
            ...
            LL_TRACE_WARN2("!!!Establish CE Schedule conflict handle=%u, evtCtr=%u",
...
bool_t lctrMstInitiateExtAdvPktHandler(BbOpDesc_t *pOp, const uint8_t *pAdvBuf)
{
    ...
    if (!pScan->filtResults.peerMatch) {
        ...
        if (pCtx->data.init.ownAddrType & LL_ADDR_IDENTITY_BIT) {
            ...
            if (pScan->filtResults.pduType == LL_PDU_ADV_DIRECT_IND) {
                ...
                if (BDA64_ADDR_IS_RPA(localAddr)) {
                    BbBleResListUpdateLocal(pScan->filtResults.peerIdAddrRand,
        ...
        uint32_t txWinOffsetUsec =
            SchRmGetOffsetUsec(LCTR_CONN_IND_US(pCtx->data.init.connInterval),
                               LCTR_GET_CONN_HANDLE(pConnCtx), refTime);
    ...
    case LL_PDU_ADV_EXT_IND: {
        ...
        if (extAdvHdrFlags & LL_EXT_HDR_AUX_PTR_BIT) {
            ...
            do {
                ...
                if ((auxPhy == BB_PHY_BLE_CODED) &&
                    ((lmgrCb.features & LL_FEAT_LE_CODED_PHY) == 0)) {
...
bool_t lctrMstInitiateRxAuxAdvPktHandler(BbOpDesc_t *pOp, const uint8_t *pAdvBuf)
{
    ...
    if (!pAuxScan->filtResults.peerMatch) {
        ...
        uint32_t connReqEndTs =
            pAuxScan->auxStartTsUsec +
            SchBleCalcAdvPktDurationUsec(pBle->chan.rxPhy, pAuxScan->auxRxPhyOptions,
                                         LL_ADV_HDR_LEN + advHdr.len) +
            LL_BLE_TIFS_US +
            SchBleCalcAdvPktDurationUsec(pBle->chan.txPhy,
        ...
        uint16_t chanDelay = (pBle->chan.rxPhy == BB_PHY_BLE_CODED) ?
                                 LCTR_DATA_CHAN_DLY_AUX_CODED :
                                 LCTR_DATA_CHAN_DLY_AUX_UNCODED;
        refTime += LCTR_CONN_IND_US(chanDelay);

        /* Now reference time is the start of transmitWindowOffset. */
        uint32_t txWinOffsetUsec = SchRmGetOffsetUsec(LCTR_CONN_IND_US(pCtx->connInterval),
                                                      LCTR_GET_CONN_HANDLE(pCtx), refTime);
        uint16_t txWinOffset = LCTR_US_TO_CONN_IND(txWinOffsetUsec);
        ...
        pExtInitCtx->data.init.usedChSel =
...
bool_t lctrMstInitiateRxAuxConnRspHandler(BbOpDesc_t *pOp, const uint8_t *pRspBuf)
{
    ...
    if (BbBleExtPduFiltCheck(&params, &pOp->prot.pBle->pduFilt, TRUE, &pAuxScan->filtResults) ==
        FALSE) {
