...
static void lctrSlvSetupBigContext(lctrBigCtx_t *pBigCtx, LlCreateBig_t *pCreateBig)
{
    ...
    if (pBigCtx->encrypt) {
        ...
        lctrBisCalcGroupSessionKey(pBigCtx->gskd, pCreateBig->bcstCode,
                                   pBigCtx->bleData.chan.enc.sk);
    }
    ...
    case LL_PACKING_INTERLEAVED: {
        pBigCtx->bisSpaceUsec =
            lctrBisCalcMaxPduTimeUsec(pBigCtx->phy, pBigCtx->maxPdu, pCreateBig->encrypt);
    ...
    if (pBigCtx->bn || pBigCtx->sduInterUsec) {
        pBigCtx->transLatUsec =
            pBigCtx->syncDelayUsec +
...
static void lctrSlvSetupBigTestContext(lctrBigCtx_t *pBigCtx, LlCreateBigTest_t *pCreateBigTest)
{
    ...
    if (pBigCtx->encrypt) {
        ...
        /* Setup encryption parameters. */
        lctrBisCalcGroupSessionKey(pBigCtx->gskd, pCreateBigTest->bcstCode,
                                   pBigCtx->bleData.chan.enc.sk);
    }
    ...
    case LL_PACKING_INTERLEAVED: {
        pBigCtx->bisSpaceUsec =
            lctrBisCalcMaxPduTimeUsec(pBigCtx->phy, pBigCtx->maxPdu, pCreateBigTest->encrypt);
        pBigCtx->subInterUsec = pBigCtx->bisSpaceUsec * pBigCtx->nse;
...
static void lctrSlvBisRemoveAdvSet(uint8_t advHandle)
{
    ...
    for (unsigned int i = 0; i < pLctrRtCfg->maxBig; i++) {
        ...
        if ((pBigCtx->role == LL_ROLE_SLAVE) && (pBigCtx->roleData.slv.pAdvSet == pAdvSet)) {
...
uint8_t LctrSlvBisCreateBig(LlCreateBig_t *pCreateBig)
{
    ...
    if ((pBigCtx = lctrFindBigByHandle(pCreateBig->bigHandle)) != NULL) {
        ...
        if (lctrBigIsPerAdvUsed(pCreateBig->advHandle) == TRUE) {
            LL_TRACE_WARN1("LctrSlvBisCreateBig: advHandle=%u not available",
                           pCreateBig->advHandle);
    ...
    for (unsigned int i = 0; i < pCreateBig->numBis; i++) {
        ...
        if (pBisCtx) {
            lctrSetupBisContext(pBisCtx, pBigCtx->seedAccAddr, pBigCtx->baseCrcInit,
                                lmgrCb.chanClass, pBigCtx->phy);
        } else {
...
uint8_t LctrSlvBisCreateBigTest(LlCreateBigTest_t *pCreateBigTest)
{
    ...
    if ((pBigCtx = lctrFindBigByHandle(pCreateBigTest->bigHandle)) != NULL) {
        LL_TRACE_WARN1("LctrSlvBisCreateBigTest: bigHandle=%u already in use",
                       pCreateBigTest->bigHandle);
        return LL_ERROR_CODE_CMD_DISALLOWED;
    } else {
        if (lctrBigIsPerAdvUsed(pCreateBigTest->advHandle) == TRUE) {
            LL_TRACE_WARN1("LctrSlvBisCreateBigTest: advHandle=%u not available",
                           pCreateBigTest->advHandle);
            return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
        }
...
void lctrNotifyHostCreateBigComplete(lctrBigCtx_t *pBigCtx, uint8_t status)
{
    ...
    LL_TRACE_INFO2("### LlEvent ###  LL_CREATE_BIG_CNF, bigHandle=%u, status=%u", pBigCtx->handle,
                   status);
...
void lctrNotifyHostTerminateBigComplete(lctrBigCtx_t *pBigCtx)
{
    ...
    LL_TRACE_INFO2("### LlEvent ###  LL_TERM_BIG_IND, bigHandle=%u, reason=%u", pBigCtx->handle,
                   pBigCtx->bcp.term.reason);
...
uint8_t lctrSlvBigBuildOp(lctrBigCtx_t *pBigCtx)
{
    ...
    if (!SchRmAdd(LCTR_BIG_TO_RM_HANDLE(pBigCtx), SCH_RM_PREF_PERFORMANCE, pBigCtx->isoInterUsec,
                  pBigCtx->isoInterUsec, pOp->minDurUsec, NULL, lctrGetBigRefTime)) {
    ...
    uint32_t offsetUsec =
        SchRmGetOffsetUsec(pOp->minDurUsec, LCTR_BIG_TO_RM_HANDLE(pBigCtx), curTime);
    ...
    LL_TRACE_INFO2("    >>> BIG started, handle=%u, numBis=%u <<<", pBigCtx->handle,
                   pBigCtx->numBis);
    LL_TRACE_INFO3("                     bn=%u, nse=%u, irc=%u", pBigCtx->bn, pBigCtx->nse,
                   pBigCtx->irc);
    LL_TRACE_INFO3("                     pto=%u, framing=%u, packing=%u", pBigCtx->pto,
                   pBigCtx->framing, pBigCtx->packing);
