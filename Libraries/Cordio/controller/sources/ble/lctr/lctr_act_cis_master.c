...
static void lctrCalCeRefFirstCis(lctrConnCtx_t *pCtx, lctrCisCtx_t *pCisCtx)
{
   ...
   * (connInterval – ((NSE – 1) × Sub_Interval + MPTM + T_IFS + MPTS + T_MSS)) */
    while (pCisCtx->offsetUsec >=
           (LCTR_CONN_IND_US(pCtx->connInterval) -
            (pCisCtx->nse * pCisCtx->subIntervUsec - pLctrRtCfg->cisSubEvtSpaceDelay))) {
...
static void lctrCalCeRefNotFirstCis(lctrConnCtx_t *pCtx, lctrCisCtx_t *pCisCtx)
{
    ...
    while (result == FALSE) {
        pCisCtx->cisCeRef++;
        cigRefTime =
            pCigCtx->cigBod.dueUsec + pCisCtx->cisCeRef * LCTR_ISO_INT_TO_US(pCigCtx->isoInterval);
    ...
    if ((pPdu = lctrTxCtrlPduAlloc(LL_CIS_REQ_LEN)) != NULL) {
        ...
        UINT16_TO_BSTREAM(pBuf, (pCisCtx->sduSizeMToS & 0x0FFF) |
                                    ((pCisCtx->framing & 0x01)
                                     << 15)); /* Max_SDU_Size_M_To_S | ISOAL_PDU_Type */
        ...
        UINT8_TO_BSTREAM(pBuf, ((pCisCtx->bnSToM & 0x0F) << 4) | (pCisCtx->bnMToS & 0x0F));
        /* BN_M_To_S */ /* BN_S_To_M */
...
void lctrMstCisLlcpActHostCisReq(lctrConnCtx_t *pCtx, lctrCisCtx_t *pCisCtx)
{
    ...
    if (pCigCtx->numCisEsted == 0) {
        ...
        if ((result == TRUE) && (pCigCtx->isRmAdded == FALSE)) {
            if (!SchRmAdd(LCTR_GET_CIG_RM_HANDLE(pCigCtx), SCH_RM_PREF_PERFORMANCE,
                          LCTR_ISO_INT_TO_US(pCigCtx->isoInterval),
                          LCTR_ISO_INT_TO_US(pCigCtx->isoInterval), pCigCtx->cigSyncDelayUsec, NULL,
                          lctrGetCigRefTime)) {
    ...
    pCisCtx->aclHandle =
        pLctrConnMsg->createCis.hdr.handle; /* Save ACL handle to the CIS context. */
