...
{
    uint64_t hash;
    lctrAdvRptGenerateLegacyHash(&hash, pRpt->addrType, BstreamToBda64(pRpt->addr),
                                 pRpt->eventType);
    ...
    if (lctrMstScan.state == LCTR_SCAN_STATE_DISCOVER) {
        ...
        LL_TRACE_INFO1("### LlEvent ###  LL_ADV_REPORT_IND, status=LL_SUCCESS addressType=%u",
                       pRpt->addrType);
...
void lctrMstDiscoverBuildOp(void)
{
    ...
    case LL_SCAN_ACTIVE: {
        ...
        lctrMstScan.reqPduHdr.txAddrRnd =
...
{
    LlConnInd_t evt = { .hdr = { .event = LL_CONN_IND, .status = reason },
 ...
 */
/*************************************************************************************************/
void lctrAdvRptGenerateLegacyHash(uint64_t *pHash, uint8_t addrType, uint64_t addr,
                                  uint8_t eventType)
{
...
void LctrMstScanInit(void)
{
    ...
    if (pLctrRtCfg->btVer >= LL_VER_BT_CORE_SPEC_4_2) {
        lmgrPersistCb.featuresDefault |= LL_FEAT_EXT_SCAN_FILT_POLICY;
    }
}
