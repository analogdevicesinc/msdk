...
uint8_t lctrCalcPathLossZone(lctrConnCtx_t *pCtx)
{
    ...
    uint8_t highUpperBound = pCtx->pclMonitorParam.pathLoss.highThreshold +
                             pCtx->pclMonitorParam.pathLoss.highHysteresis;
    uint8_t highLowerBound = pCtx->pclMonitorParam.pathLoss.highThreshold -
                             pCtx->pclMonitorParam.pathLoss.highHysteresis;

    uint8_t lowUpperBound =
        pCtx->pclMonitorParam.pathLoss.lowThreshold + pCtx->pclMonitorParam.pathLoss.lowHysteresis;
    uint8_t lowLowerBound =
        pCtx->pclMonitorParam.pathLoss.lowThreshold - pCtx->pclMonitorParam.pathLoss.lowHysteresis;
...
uint8_t lctrSetPathLossReportingEnable(uint16_t handle, uint8_t enable)
{
    ...
    if (pCtx->peerTxPower == LL_PWR_CTRL_TXPOWER_UNAVAILABLE) {
        ...
        if ((pMsg = (lctrMsgPwrCtrlReq_t *)WsfMsgAlloc(sizeof(*pMsg))) != NULL) {
            ...
            LL_TRACE_WARN0(
                "lctrSetPathLossReportingEnable: Failed to alloc power control read request.");
...
void lctrNotifyHostPathLossRpt(lctrConnCtx_t *pCtx)
{
    ...
    LlPathLossThresholdEvt_t evt = {
        .hdr = { .param = handle, .event = LL_PATH_LOSS_REPORTING_IND, .status = LL_SUCCESS },
    ...
    LL_TRACE_INFO3(
        "### LlEvent ### lctrNotifyHostPathLossRpt , handle=%u, zoneEntered=%d, curPathLoss=%d",
        handle, pCtx->pclMonitorParam.pathLoss.curZone, curPathLoss);
...
void lctrPathLossMonitorAct(lctrConnCtx_t *pCtx)
{
    ...
    if (pCtx->pclMonitorParam.pathLoss.curTimeSpent >=
        ...
        LL_TRACE_INFO2("lctrPathLossMonitorAct, New zone entered. newZone=%u, pathLoss=%u", newZone,
                       lctrCalcPathLoss(pCtx));
        pCtx->pclMonitorParam.pathLoss.curTimeSpent = 0;
