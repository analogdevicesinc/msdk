...
PalBbBleTxBufDesc_t lctrSlvBisTxDesc[LL_MAX_BIS][LL_MAX_BN]
                                    [3]; /* Place outside of lctrSlvBisIsr to reduce memclr(). */
...
{
    if (lctrBisTxQueuePeek(pBigCtx->pBisCtx[bisIdx], burstIdx,
                           &lctrSlvBisTxDesc[bisIdx][burstIdx][0]) > 0) {
        lctrSlvBisIsr.numTxFrags[bisIdx]++;

        /* Update Control PDU flags. */
        LCTR_BIS_WR_CSSN(lctrSlvBisTxDesc[bisIdx][burstIdx][0].pBuf[LCTR_ISO_DATA_PDU_FC_OFFSET],
                         pBigCtx->bcp.cssn);
        if (lctrSlvBisIsr.pCtrlBuf) {
            LCTR_BIS_SET_CSTF(
                lctrSlvBisTxDesc[bisIdx][burstIdx][0].pBuf[LCTR_ISO_DATA_PDU_FC_OFFSET]);
        } else {
            LCTR_BIS_CLR_CSTF(
                lctrSlvBisTxDesc[bisIdx][burstIdx][0].pBuf[LCTR_ISO_DATA_PDU_FC_OFFSET]);
        }
...
static void lctrSlvBisTxData(lctrBigCtx_t *pBigCtx)
{
    ...
    BbBleBisTxData(pTxDesc, descCnt, pBigCtx->bod.dueUsec + lctrSlvBisIsr.nextSeOffs,
            ...
            if (!pBisCtx->test.term) {
                ...
                if (pBigCtx->framing == LL_ISO_PDU_TYPE_UNFRAMED) {
                    for (unsigned int j = 0; j < pBigCtx->bn; j++) {
                        ...
                        if ((pIsoBuf = lctrGenerateIsoTestData(pBisCtx->handle,
                                                               pBisCtx->test.pldType,
...
static void lctrSlvCheckPendingSdu(lctrBigCtx_t *pBigCtx)
{
    for (unsigned int i = 0; i < pBigCtx->numBis; i++) {
        ...
        if ((pDataBuf = lctrTxIsoDataPduAlloc()) != NULL) {
            lctrIsoHdr_t isoHdr = { .handle = pBisCtx->handle, .tsFlag = FALSE };

            if ((isoHdr.sduLen = lctrAssembleTxFramedPdu(&pBisCtx->roleData.slv.isoalTxCtx,
                                                         pDataBuf, pBisCtx->pBigCtx->maxPdu)) > 0) {
            ...
            while ((pSduBuf = WsfMsgDeq(&pBisCtx->roleData.slv.isoalTxCtx.pendingSduQ, &temp)) !=
                   NULL) {
                WsfMsgFree(pSduBuf);
...
void lctrSlvBisTxCompletionSequential(BbOpDesc_t *pOp, uint8_t status)
{
    ...
    if (status != BB_STATUS_SUCCESS) {
        LL_TRACE_WARN2("!!! Failed BIG transmission handle=%u ec[15:0]=%u", pBigCtx->handle,
...
void lctrSlvBigBeginOp(BbOpDesc_t *pOp)
{
    ...
    lctrBisDataPduHdr_t hdr = { .llid = LL_LLID_ISO_EMPTY_PDU,
