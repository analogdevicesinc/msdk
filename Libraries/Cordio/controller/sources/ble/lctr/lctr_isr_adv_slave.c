...
bool_t lctrScanReqHandler(BbOpDesc_t *pOp, uint8_t reqLen)
{
    ...
    if (pAdv->pTxRspBuf && (reqLen == LL_SCAN_REQ_PDU_LEN)) {
        return TRUE;
    }
...
{
    if (lctrMsgDispTbl[LCTR_DISP_CONN] && (reqLen == LL_CONN_IND_PDU_LEN)) {
        /* Stop advertising. */
...
bool_t lctrSlvAdvHandler(BbOpDesc_t *pOp, const uint8_t *pReqBuf)
{
    ...
    case LL_PDU_CONNECT_IND:
        ...
        lctrSlvAdv.reqEndTsUsec = pAdv->reqStartTsUsec + LCTR_CONN_IND_PKT_1M_US;
...
void lctrSlvAdvEndOp(BbOpDesc_t *pOp)
{
        ...
        (lmgrSlvAdvCb.ownAddrType == LL_ADDR_RANDOM)) /* Should be comparison; not valid for RPA. */
    {
        ...
        uint64_t localRpa;
        if (BbBleResListReadLocal(lmgrSlvAdvCb.advParam.peerAddrType,
                                  lmgrSlvAdvCb.advParam.peerAddr, &localRpa)) {
            update = TRUE;
            newAddr = localRpa;
        }
    }
    if (update) {
        ...
        memcpy(lctrSlvAdv.scanRspBuf + LL_ADV_HDR_LEN, /* update scan response PDU */
               lctrSlvAdv.advBuf + LL_ADV_HDR_LEN, BDA_ADDR_LEN);
    ...
    if ((lmgrSlvAdvCb.advParam.advType == LL_ADV_CONN_DIRECT_LOW_DUTY) ||
        ...
        if (BB_BLE_PDU_FILT_FLAG_IS_SET(&pBle->pduFilt, PEER_ADDR_MATCH_ENA) &&
            ...
            if (BbBleResListReadPeer(lmgrSlvAdvCb.advParam.peerAddrType,
                                     lmgrSlvAdvCb.advParam.peerAddr, &peerRpa)) {
    ...
    if ((lmgrSlvAdvCb.advParam.advType != LL_ADV_CONN_DIRECT_LOW_DUTY) &&
        ...
        memcpy(lctrSlvAdv.advBuf + LL_ADVB_MIN_LEN, lmgrSlvAdvCb.advData.buf,
    ...
    if (lmgrSlvAdvCb.scanRspData.modified) {
        ...
        lctrSlvAdv.scanRspBuf[LL_ADV_HDR_LEN_OFFS] =
            LL_SCAN_PREFIX_LEN + lmgrSlvAdvCb.scanRspData.len;
        memcpy(lctrSlvAdv.scanRspBuf + LL_ADVB_MIN_LEN, lmgrSlvAdvCb.scanRspData.buf,
    ...
    case LL_ADV_NONCONN_UNDIRECT: {
        ...
        do {
            ...
            if (lmgrSlvAdvCb.advParam.advInterMinUsec == lmgrSlvAdvCb.advParam.advInterMaxUsec) {
                ...
                result = SchInsertEarlyAsPossible(pOp, lmgrSlvAdvCb.advParam.advInterMinUsec,
    ...
    case LL_ADV_CONN_DIRECT_HIGH_DUTY: {
        ...
        while (!result && lmgrSlvAdvCb.advTermCntDownUsec) {
            result = SchInsertLateAsPossible(pOp, lmgrSlvAdvCb.advParam.advInterMinUsec,
                                             lmgrSlvAdvCb.advParam.advInterMaxUsec);
            if (!result) {
                pOp->dueUsec += lmgrSlvAdvCb.advParam.advInterMaxUsec;
            }
