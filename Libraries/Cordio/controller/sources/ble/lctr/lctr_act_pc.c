...
int8_t lctrAttemptTxPowerChange(lctrConnCtx_t *pCtx, uint8_t phy, int8_t delta)
{
    ...
    /* Update txPower. */
    LL_TRACE_INFO3("lctrAttemptTxPowerChange: Power change -> handle=%d phy=%d txPow=%d",
                   LCTR_GET_CONN_HANDLE(pCtx), phy + (option == BB_PHY_OPTIONS_BLE_S2) ? 1 : 0,
                   newTxPower);
    LCTR_SET_TXPOWER(
        pCtx, phy + (((phy == LL_PHY_LE_CODED) && (option == BB_PHY_OPTIONS_BLE_S2)) ? 1 : 0),
        newTxPower);
...
static void lctrSendPowerCtrlReqPdu(lctrConnCtx_t *pCtx, int8_t delta)
{
    ...
    if ((pPdu = lctrTxCtrlPduAlloc(LL_PWR_CTRL_REQ_LEN)) != NULL) {
        ...
        UINT8_TO_BSTREAM(pBuf,
                         LCTR_GET_TXPOWER(pCtx, pCtx->reqPhy, pCtx->bleData.chan.initTxPhyOptions));
...
{
    uint8_t *pPdu;
    uint8_t option = (pCtx->reqPhy == LL_PC_PHY_CODED_S2) ? BB_PHY_OPTIONS_BLE_S2 :
...
void lctrStorePowerControlAction(lctrConnCtx_t *pCtx)
{
    ...
    if (LCTR_GET_TXPOWER(pCtx, pCtx->reqPhy, BB_PHY_OPTIONS_BLE_S2) ==
        LL_PWR_CTRL_TXPOWER_UNMANAGED) {
        LCTR_SET_TXPOWER(pCtx, pCtx->reqPhy, pLctrRtCfg->defTxPwrLvl);

        if (pCtx->reqPhy >= LL_PC_PHY_CODED_S8) {
            LCTR_SET_TXPOWER(pCtx,
                             (pCtx->reqPhy == LL_PC_PHY_CODED_S8) ? LL_PC_PHY_CODED_S2 :
                                                                    LL_PC_PHY_CODED_S8,
                             pLctrRtCfg->defTxPwrLvl);
        }
    }
...
void lctrStorePeerPowerControlReq(lctrConnCtx_t *pCtx)
{
    ...
    if ((pCtx->powerRptRemote) && (pCtx->bleData.chan.rxPhy == phyIdx) &&
        (pCtx->peerTxPower != pld.txPower)) {
        lctrNotifyPowerReportInd(pCtx, LL_POWER_REPORT_REASON_REMOTE, phyIdx, pld.txPower,
                                 lctrGetPowerLimits(pld.txPower), pld.txPower - pCtx->peerTxPower);
    }
    ...
    if (pCtx->bleData.chan.rxPhy == phyIdx) {
        ...
        if ((pCtx->peerTxPower == LL_PWR_CTRL_TXPOWER_UNAVAILABLE) ||
            ...
            LL_TRACE_INFO1("lctrStorePeerPowerControlReq: txPower unmanaged or unavailable. Phy=%d",
                           pld.phy);
        } else {
    ...
    if ((pCtx->delta != 0) && (pCtx->powerRptLocal)) {
        ...
        lctrNotifyPowerReportInd(pCtx, LL_POWER_REPORT_REASON_LOCAL, phyIdx, newTxPower,
                                 lctrGetPowerLimits(newTxPower), pCtx->delta);
    }
}
...
void lctrSendPeerPowerControlRsp(lctrConnCtx_t *pCtx)
{
    if (pCtx->reqErrCode == LL_SUCCESS) {
        ...
        LL_TRACE_WARN0(
            "lctrSendPeerPowerControlRsp: Peer sent invalid parameters for power control request.");
...
void lctrStorePeerPowerControlRsp(lctrConnCtx_t *pCtx)
{
    ...
    if ((pCtx->powerRptRemote) && (pCtx->peerTxPower != pld.txPower)) {
        lctrNotifyPowerReportInd(pCtx, LL_POWER_REPORT_REASON_REMOTE, pCtx->reqPhy, pld.txPower,
                                 pld.limits, pld.txPower - pCtx->peerTxPower);
    }
    ...
    LL_TRACE_INFO3("lctrStorePeerPowerControlRsp: peerTxPower=%d, peerPwrLimits=%d, peerApr=%d",
                   pld.txPower, pld.limits, pld.apr);
}
                              ...
                              uint8_t limits, int8_t delta)
{
    ...
    LlPowerReportInd_t evt = {
        .hdr = { .param = handle, .event = LL_TX_POWER_REPORTING_IND, .status = LL_SUCCESS },
    ...
    LL_TRACE_INFO2(
        "### LlEvent ### lctrNotifyPowerReportInd , handle=%u, reason=%d, status=LL_SUCCESS",
        handle, reason);
 ...
 */
/*************************************************************************************************/
static void lctrSendPowerChangePdu(lctrConnCtx_t *pCtx, uint8_t opcode, uint8_t phyVal,
                                   int8_t delta, int8_t txPower, bool_t phyChange)
{
    ...
    if ((pPdu = lctrTxCtrlPduAlloc(LL_PWR_CHNG_IND_LEN)) != NULL) {
        ...
        if ((phy == LL_PC_CODED_S8_BIT) && (phyChange == TRUE)) {
            if (LCTR_GET_TXPOWER(pCtx, LL_PHY_LE_CODED, BB_PHY_OPTIONS_BLE_S2) ==
                LCTR_GET_TXPOWER(pCtx, LL_PHY_LE_CODED, BB_PHY_OPTIONS_BLE_S8)) {
...
void lctrStorePeerPowerInd(lctrConnCtx_t *pCtx)
{
    ...
    LL_TRACE_INFO3("lctrStorePeerPowerInd: Phy=%d Delta=%d txPower=%d", pPdu->phy, pPdu->delta,
                   pPdu->txPower);

    if (pPdu->phy & (1 << (pCtx->bleData.chan.rxPhy - 1))) {
        if ((pCtx->peerTxPower != pPdu->txPower) && (pCtx->powerRptRemote)) {
            lctrNotifyPowerReportInd(pCtx, LL_POWER_REPORT_REASON_REMOTE, pCtx->bleData.chan.rxPhy,
                                     pPdu->txPower, pPdu->limits,
        ...
        if (pPdu->phy == pCtx->bleData.chan.rxPhy) {
            ...
            if ((pCtx->peerTxPower == LL_PWR_CTRL_TXPOWER_UNAVAILABLE) ||
                (pCtx->peerTxPower == LL_PWR_CTRL_TXPOWER_UNMANAGED)) {
                LL_TRACE_INFO1("lctrStorePeerPowerInd: txPower unmanaged or unavailable. Phy=%d",
                               pPdu->phy);
            } else {
...
void lctrAutoPowerMonitorAct(lctrConnCtx_t *pCtx)
{
    ...
    if ((pCtx->rssi < pCtx->pclMonitorParam.autoMonitor.lowThreshold) ||
        ...
        if (pCtx->pclMonitorParam.autoMonitor.curTimeSpent >=
            pCtx->pclMonitorParam.autoMonitor.minTimeSpent) {
            if (!(pCtx->peerPwrLimits & LL_PWR_CONTROL_LIMIT_MAX_BIT)) {
                LL_TRACE_INFO1("RSSI too low, requesting increase in power. phy=%u",
                               pCtx->bleData.chan.rxPhy);
        ...
        if (pCtx->pclMonitorParam.autoMonitor.curTimeSpent >=
            ...
            if (!(pCtx->peerPwrLimits & LL_PWR_CONTROL_LIMIT_MIN_BIT)) {
                LL_TRACE_INFO1("RSSI too high, requesting decrease in power. phy=%u",
                               pCtx->bleData.chan.rxPhy);
                sendReqDelta = -(pCtx->pclMonitorParam.autoMonitor.requestVal);
            }
            pCtx->pclMonitorParam.autoMonitor.curTimeSpent = 0;
    ...
    if (sendReqDelta != 0) {
        uint8_t reqPhy = pCtx->bleData.chan.rxPhy +
