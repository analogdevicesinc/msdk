...
static void lctrActStartConnUpd(lctrConnCtx_t *pCtx)
{
    if (pCtx->usedFeatSet & LL_FEAT_CONN_PARAM_REQ_PROC) {
        ...
        LL_TRACE_WARN1(
            "Connection parameter request feature not supported by connection, handle=%u",
...
void lctrActPeerRejectConnParam(lctrConnCtx_t *pCtx)
{
    ...
    case LL_PDU_REJECT_EXT_IND:
        LL_TRACE_WARN2("Peer rejected connection parameter request, handle=%u, reason=%u",
                       LCTR_GET_CONN_HANDLE(pCtx), lctrDataPdu.pld.rejInd.reason);
        lctrNotifyHostConnUpdateInd(pCtx, lctrDataPdu.pld.rejInd.reason);
        break;
    case LL_PDU_UNKNOWN_RSP:
        LL_TRACE_WARN1(
            "Peer rejected connection parameter request, handle=%u, reason=UNSUPPORTED_REMOTE_FEATURE",
            LCTR_GET_CONN_HANDLE(pCtx));
        lctrNotifyHostConnUpdateInd(pCtx, LL_ERROR_CODE_UNSUPPORTED_REMOTE_FEATURE);
        break;
    default:
...
{
    LL_TRACE_WARN1("Host requested connection update collided with peer, handle=%u",
                   LCTR_GET_CONN_HANDLE(pCtx));
    lctrNotifyHostConnUpdateInd(pCtx, lctrDataPdu.pld.rejInd.reason);
}
...
static void lctrResolveCollision(lctrConnCtx_t *pCtx, uint8_t event)
{
    ...
    case LCTR_CU_EVENT_PEER_CONN_UPD_IND:
        LL_TRACE_WARN2(
            "Rejecting CONN_UPD=%u procedure: activeProc=%u, reason=DIFFERENT_TRANSACTION_COLLISION",
            LCTR_PROC_CONN_UPD, pCtx->llcpActiveProc);
        lctrSendRejectInd(pCtx, LL_ERROR_CODE_DIFFERENT_TRANSACTION_COLLISION, FALSE);
        break;

    case LCTR_CU_EVENT_PEER_CONN_PARAM_REQ:
        LL_TRACE_WARN2(
            "Rejecting CONN_PARAM=%u procedure: activeProc=%u, reason=DIFFERENT_TRANSACTION_COLLISION",
            LCTR_PROC_CONN_PARAM, pCtx->llcpActiveProc);
...
bool_t lctrSlvLlcpExecuteConnUpdSm(lctrConnCtx_t *pCtx, uint8_t event)
{
    ...
    case LCTR_LLCP_STATE_IDLE:
        LL_TRACE_INFO3(
            "lctrSlvLlcpExecuteConnUpdSm: handle=%u, llcpState=IDLE, connUpdState=%u, event=%u",
    ...
    case LCTR_LLCP_STATE_BUSY:
        LL_TRACE_INFO3(
            "lctrSlvLlcpExecuteConnUpdSm: handle=%u, llcpState=BUSY, connUpdState=%u, event=%u",
            LCTR_GET_CONN_HANDLE(pCtx), pCtx->connUpdState, event);
...
void lctrLlcpStatelessEventHandler(lctrConnCtx_t *pCtx, uint8_t event)
{
    ...
    case LCTR_CONN_MSG_RX_LLCP_UNKNOWN:
        LL_TRACE_WARN2("Received unknown LLCP packet handle=%u, opcode=%u",
                       LCTR_GET_CONN_HANDLE(pCtx), lctrDataPdu.opcode);
        lctrSendUnknownRsp(pCtx);
        break;
    case LCTR_CONN_MSG_RX_LLCP_INVALID_PARAM:
        LL_TRACE_WARN2("Received LLCP packet with invalid parameter handle=%u, opcode=%u",
                       LCTR_GET_CONN_HANDLE(pCtx), lctrDataPdu.opcode);
    ...
    case LCTR_CONN_MSG_RX_LLCP:
        ...
        LL_TRACE_WARN2("Unhandled LLCP packet handle=%u, opcode=%u", LCTR_GET_CONN_HANDLE(pCtx),
                       lctrDataPdu.opcode);
        break;
    case LCTR_CONN_LLCP_START_PENDING:
        /* No action required. */
        break;
    default:
        LL_TRACE_WARN2("Unhandled LLCP SM event, handle=%u, event=%u", LCTR_GET_CONN_HANDLE(pCtx),
                       event);
        break;
    }
}
...
void lctrSlvLlcpExecuteSm(lctrConnCtx_t *pCtx, uint8_t event)
{
    ...
    if (!(lctrSlvLlcpSmTbl[LCTR_LLCP_SM_ENCRYPT] &&
          lctrSlvLlcpSmTbl[LCTR_LLCP_SM_ENCRYPT](pCtx, event)) &&
        !(lctrSlvLlcpSmTbl[LCTR_LLCP_SM_PING] &&
          lctrSlvLlcpSmTbl[LCTR_LLCP_SM_PING](pCtx, event)) &&
        !(lctrSlvLlcpSmTbl[LCTR_LLCP_SM_CONN_UPD] &&
          lctrSlvLlcpSmTbl[LCTR_LLCP_SM_CONN_UPD](pCtx, event)) &&
        !(lctrSlvLlcpSmTbl[LCTR_LLCP_SM_PHY_UPD] &&
          lctrSlvLlcpSmTbl[LCTR_LLCP_SM_PHY_UPD](pCtx, event)) &&
        !(lctrSlvLlcpSmTbl[LCTR_LLCP_SM_CIS_EST] &&
          lctrSlvLlcpSmTbl[LCTR_LLCP_SM_CIS_EST](pCtx, event)) &&
        !(lctrSlvLlcpSmTbl[LCTR_LLCP_SM_CIS_TERM] &&
          lctrSlvLlcpSmTbl[LCTR_LLCP_SM_CIS_TERM](pCtx, event)) &&
