...
static void lctrSlvProcessConnInd(lctrConnMsg_t *pMsg)
{
    ...
    if (pConnEst->sendAdvSetTerm) {
        LmgrSendAdvSetTermInd(pConnEst->hdr.handle, LL_SUCCESS, LCTR_GET_CONN_HANDLE(pCtx),
                              pConnEst->numExtAdvEvents);
    }
}
...
void lctrSlvConnBuildOp(lctrConnCtx_t *pCtx)
{
    ...
    uint32_t fastTermCnt =
        txWinOffsetCnt + pConnInd->txWinSize + (LCTR_FAST_TERM_CNT * pConnInd->interval);
    WsfTimerStartMs(&pCtx->tmrSupTimeout, LCTR_CONN_IND_MS(fastTermCnt));

    lctrBuildRemapTable(pCtx);
    pCtx->chIdentifier = (pConnInd->accessAddr >> 16) ^ (pConnInd->accessAddr >> 0);
    ...
    const uint32_t wwOffsetUsec =
        lctrCalcWindowWideningUsec((txWinOffsetUsec + txWinSizeUsec), pCtx->data.slv.totalAcc);

    pCtx->data.slv.anchorPointUsec = pLctrConnMsg->connEstablish.connIndEndTsUsec +
                                     txWinOffsetUsec; /* estimated initial anchor point */
    ...
    pConn->rxSyncDelayUsec =
        txWinSizeUsec + (wwOffsetUsec << 1) + 1; /* rounding compensation when computing reqEndTs */
    ...
    while (TRUE) {
        ...
        if (SchInsertAtDueTime(pOp, lctrConnResolveConflict)) {
            LL_TRACE_INFO1("    >>> Connection established, handle=%u <<<",
    ...
    SchTmAdd(LCTR_GET_CONN_HANDLE(pCtx), LCTR_CONN_IND_US(pCtx->connInterval), pCtx->effConnDurUsec,
             TRUE, lctrGetSlvConnRefTime);
}
...
void LctrSlvConnInit(void)
{
    ...
    if (pLctrRtCfg->btVer >= LL_VER_BT_CORE_SPEC_4_1) {
        lmgrPersistCb.featuresDefault |= LL_FEAT_CONN_PARAM_REQ_PROC | LL_FEAT_EXT_REJECT_IND |
                                         LL_FEAT_SLV_INIT_FEAT_EXCH;
    }
    if (pLctrRtCfg->btVer >= LL_VER_BT_CORE_SPEC_4_2) {
        lmgrPersistCb.featuresDefault |= LL_FEAT_DATA_LEN_EXT;
    }
    if (pLctrRtCfg->btVer >= LL_VER_BT_CORE_SPEC_5_0) {
        lmgrPersistCb.featuresDefault |= LL_FEAT_MIN_NUM_USED_CHAN;
    }
    if (pLctrRtCfg->btVer >= LL_VER_BT_CORE_SPEC_5_1) {
        lmgrPersistCb.featuresDefault |= (LL_FEAT_PAST_SENDER | LL_FEAT_SCA_UPDATE);
    }
}
