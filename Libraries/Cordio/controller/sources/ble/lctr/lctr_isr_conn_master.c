    ...
    if ((pCtx->phyUpd.masterToSlavePhy != LL_PHYS_NONE) &&
        ...
        if (pBle->chan.txPower == LL_PWR_CTRL_TXPOWER_UNMANAGED) {
            ...
            if ((pCtx->usedFeatSet & LL_FEAT_POWER_CHANGE_IND) && lctrSendPowerChangeIndCback) {
                lctrSendPowerChangeIndCback(pCtx, pBle->chan.txPhy, 0, pLctrRtCfg->defTxPwrLvl,
                                            TRUE);
            }
        }
    }
...
void lctrMstConnBeginOp(BbOpDesc_t *pOp)
{
    ...
    (void)lctrSetupForTx(pCtx, BB_STATUS_SUCCESS /* does not matter */,
...
void lctrMstConnCleanupOp(BbOpDesc_t *pOp)
{
    ...
    if (pCtx->emptyPduPend && pCtx->emptyPduFirstAtt) {
    ...
    if (pCtx->data.mst.sendConnUpdInd) {
        ...
        if ((pPdu = lctrTxCtrlPduAlloc(LL_CONN_UPD_IND_PDU_LEN)) != NULL) {
            ...
            uint32_t txWinOffsetUsec =
                SchRmGetOffsetUsec(0, LCTR_GET_CONN_HANDLE(pCtx), pOp->dueUsec);
...
void lctrMstConnAbortOp(BbOpDesc_t *pOp)
{
    ...
    LL_TRACE_INFO2("!!! Connection Master BOD aborted, handle=%u, eventCounter=%u",
                   LCTR_GET_CONN_HANDLE(pCtx), pCtx->eventCounter);
...
void lctrMstConnRxCompletion(BbOpDesc_t *pOp, uint8_t *pRxBuf, uint8_t status)
{
    ...
    if (lctrCheckForLinkTerm(pCtx) || (status == BB_STATUS_FAILED) ||
        ...
        if (status == BB_STATUS_RX_TIMEOUT) {
            LL_TRACE_WARN3(
                "lctrMstConnRxCompletion: BB failed with status=RX_TIMEOUT, eventCounter=%u, bleChan=%u, handle=%u",
                pCtx->eventCounter, pCtx->bleData.chan.chanIdx, LCTR_GET_CONN_HANDLE(pCtx));
        }

        if (status == BB_STATUS_FAILED) {
            LL_TRACE_ERR3(
                "lctrMstConnRxCompletion: BB failed with status=FAILED, eventCounter=%u, bleChan=%u, handle=%u",
                pCtx->eventCounter, pCtx->bleData.chan.chanIdx, LCTR_GET_CONN_HANDLE(pCtx));
        }
    ...
    if (status == BB_STATUS_SUCCESS) {
        ...
        /* Trace message here causes too much latency between RX Done and cancel TIFS 
     * LL_TRACE_WARN3("lctrMstConnRxCompletion: BB failed with status=CRC_FAILED, eventCounter=%u, bleChan=%u, handle=%u", pCtx->eventCounter, pCtx->bleData.chan.chanIdx, LCTR_GET_CONN_HANDLE(pCtx));
     */
