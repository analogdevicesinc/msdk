...
WSF_CT_ASSERT((BB_FIXED_DATA_PKT_LEN == 0) || (BB_FIXED_DATA_PKT_LEN >= LCTR_DATA_PDU_MAX_LEN));
...
#if (LL_ENABLE_TESTER)
extern void LctrProcessRxTxAck(lctrConnCtx_t *pCtx, uint8_t *pRxBuf, uint8_t **pNextRxBuf,
                               bool_t *pTxPduIsAcked);
#endif
        ...
        if (pCtx->usedChSel == LL_CH_SEL_1) {
            if (pCtx->lastChanIdx >= (count * pCtx->hopInc)) {
                ...
                if (((count * pCtx->hopInc) - pCtx->lastChanIdx) <= LL_MAX_NUM_CHAN_DATA) {
                    ...
                    pCtx->lastChanIdx =
                        LL_MAX_NUM_CHAN_DATA -
                        LL_MATH_MOD_37(((count * pCtx->hopInc) - pCtx->lastChanIdx));
                }
            }
        }
...
static void lctrSlvConnUpdateOp(lctrConnCtx_t *pCtx, bool_t ignoreOffset)
{
    ...
    pCtx->data.slv.lastActiveEvent =
        pCtx->eventCounter + 1; /* guarantee execution of first connection event. */
    ...
    if (ignoreOffset == FALSE) {
        ...
        const uint32_t wwTxWinOffsetUsec =
            lctrCalcWindowWideningUsec((txWinOffsetUsec + txWinSizeUsec), pCtx->data.slv.totalAcc);
   ...
   *     connIntervalOld + supervisionTimeoutNew */
    ...
    if ((connIntervalOld != pCtx->connInterval) || (latencyOld != pCtx->maxLatency) ||
    ...
    LL_TRACE_INFO2("    >>> Connection updated, handle=%u, eventCounter=%u <<<",
                   LCTR_GET_CONN_HANDLE(pCtx), pCtx->eventCounter);
    LL_TRACE_INFO1("                            connIntervalUsec=%u",
...
static void lctrSlvChanMapUpdateOp(lctrConnCtx_t *pCtx)
{
    ...
    LL_TRACE_INFO2("    >>> Channel map updated, handle=%u, eventCounter=%u <<<",
                   LCTR_GET_CONN_HANDLE(pCtx), pCtx->eventCounter);
}
...
static void lctrSlvPhyUpdateOp(lctrConnCtx_t *pCtx)
{
    ...
    if ((pCtx->phyUpd.slaveToMasterPhy != LL_PHYS_NONE) &&
        ...
        pBle->chan.txPower =
            LCTR_GET_TXPOWER(pCtx, pCtx->bleData.chan.txPhy, pBle->chan.initTxPhyOptions);
    ...
    LL_TRACE_INFO2("    >>> PHY updated, handle=%u, eventCounter=%u <<<",
                   LCTR_GET_CONN_HANDLE(pCtx), pCtx->eventCounter);
...
void lctrSlvConnBeginOp(BbOpDesc_t *pOp)
{
    ...
    if ((pBuf = lctrRxPduAlloc(pCtx->localDataPdu.maxRxLen)) != NULL) {
        ...
        LL_TRACE_ERR1("!!! OOM while initializing receive buffer at start of CE, handle=%u",
...
void lctrSlvConnEndOp(BbOpDesc_t *pOp)
{
    ...
    pBle->chan.txPower =
        LCTR_GET_TXPOWER(pCtx, pCtx->bleData.chan.txPhy, pCtx->bleData.chan.initTxPhyOptions);
#endif
   ...
   * This message shall be sent only once. */
    if ((pCtx->isFirstNonCtrlPdu == TRUE) && (pCtx->isSlvReadySent == FALSE)) {
    ...
    if ((pCtx->llcpActiveProc == LCTR_PROC_CONN_UPD) && lctrSlvCheckConnUpdInstant(pCtx) &&
        ...
        LL_TRACE_WARN1("Received connection update at instant, applying immediately at CE=%d",
                       pCtx->eventCounter);
    }
    ...
    if ((pCtx->data.slv.abortSlvLatency == FALSE) &&
        ...
        (pCtx->maxLatency && pCtx->data.slv.initAckRcvd && pCtx->data.slv.rxFromMaster &&
         (WsfQueueEmpty(&pCtx->txArqQ)) && (pCtx->state != LCTR_CONN_STATE_TERMINATING))) {
        if ((pCtx->llcpActiveProc == LCTR_PROC_INVALID)) {
            ...
            if ((pCtx->llcpActiveProc == LCTR_PROC_CONN_UPD) && lctrSlvCheckConnUpdInstant(pCtx)) {
                if ((uint16_t)(pCtx->connUpd.instant - pCtx->eventCounter) >
                    (pCtx->maxLatency + 1)) {
                    numSkipCe = pCtx->maxLatency;
                ...
                if ((uint16_t)(pCtx->chanMapUpd.instant - pCtx->eventCounter) >
                    (pCtx->maxLatency + 1)) {
                    numSkipCe = pCtx->maxLatency;
                ...
                if ((uint16_t)(pCtx->phyUpd.instant - pCtx->eventCounter) >
                    (pCtx->maxLatency + 1)) {
                    numSkipCe = pCtx->maxLatency;
    ...
    if (numSkipCe > 0) {
        ...
        LL_TRACE_INFO2("Applying slave latency, waking up at eventCounter=%u, numSkipCE=%u",
                       pCtx->eventCounter + 1, numSkipCe);
    }
    ...
    while (TRUE) {
        ...
        if (wwTotalUsec >=
            ((connInterUsec >> 1) - WSF_MAX(LL_BLE_TIFS_US, BbGetSchSetupDelayUs()))) {
            LL_TRACE_WARN2(
                "!!! Terminating connection due to excessive WW handle=%u, eventCounter=%u",
                LCTR_GET_CONN_HANDLE(pCtx), pCtx->eventCounter);
            lctrSendConnMsg(pCtx, LCTR_CONN_TERMINATED);
            WsfTimerStop(&pCtx->tmrSupTimeout);
            return;
        ...
        if (pCtx->svtState != LCTR_SVT_STATE_FATAL) {
            uint8_t nthCE = (pCtx->eventCounter + 1) -
                            pCtx->data.slv.lastActiveEvent; /* Nth CE since last active CE. */
            uint8_t numCEforSVT = ((uint32_t)pCtx->supTimeoutMs * 1000) /
                                  (uint32_t)LCTR_CONN_IND_US(pCtx->connInterval);
        ...
        LL_TRACE_WARN2("!!! CE schedule conflict handle=%u, eventCounter=%u",
                       LCTR_GET_CONN_HANDLE(pCtx), pCtx->eventCounter);
    }
}
...
void lctrSlvConnRxCompletion(BbOpDesc_t *pOp, uint8_t *pRxBuf, uint8_t status)
{
    ...
    if (lctrCheckForLinkTerm(pCtx) || (status == BB_STATUS_FAILED) ||
        (status == BB_STATUS_RX_TIMEOUT)) {
        if (status == BB_STATUS_RX_TIMEOUT) {
            LL_TRACE_WARN3(
                "lctrSlvConnRxCompletion: BB failed with status=RX_TIMEOUT, handle=%u, bleChan=%u, eventCounter=%u",
                LCTR_GET_CONN_HANDLE(pCtx), pBle->chan.chanIdx, pCtx->eventCounter);
        }

        if (status == BB_STATUS_FAILED) {
            LL_TRACE_ERR3(
                "lctrSlvConnRxCompletion: BB failed with status=FAILED, handle=%u, bleChan=%u, eventCounter=%u",
                LCTR_GET_CONN_HANDLE(pCtx), pBle->chan.chanIdx, pCtx->eventCounter);
        }
    ...
    if ((!pCtx->data.slv.syncWithMaster) &&
        ((status == BB_STATUS_SUCCESS) ||
         (!lctrGetConnOpFlag(pCtx, LL_OP_MODE_FLAG_IGNORE_CRC_ERR_TS) &&
    ...
    if (status == BB_STATUS_SUCCESS) {
        ...
        if (status == BB_STATUS_CRC_FAILED) {
            LL_TRACE_WARN3(
                "lctrSlvConnRxCompletion: BB failed with status=CRC_FAILED, handle=%u, bleChan=%u, eventCounter=%u",
                LCTR_GET_CONN_HANDLE(pCtx), pBle->chan.chanIdx, pCtx->eventCounter);
            pCtx->data.slv.consCrcFailed++;
        } else {
            /* Decryption failure */
            LL_TRACE_WARN3(
                "lctrSlvConnRxCompletion: BB failed with status=BB_STATUS_FRAME_FAILED, handle=%u, rxPktCounter=%u, eventCounter=%u",
                LCTR_GET_CONN_HANDLE(pCtx), pCtx->rxPktCounter, pCtx->eventCounter);
            if (!decremented) {
    ...
    if ((pCtx->isSlvReadySent == FALSE) && (pCtx->rxHdr.llid != LL_LLID_CTRL_PDU) &&
    ...
    if (!pCtx->data.slv.initAckRcvd && txPduIsAcked) {
...
SetupTx:
    ...
    if ((pCtx->isSlvReadySent == FALSE) && (pCtx->isFirstNonCtrlPdu == TRUE) &&
        (lctrGetConnOpFlag(pCtx, LL_OP_MODE_FLAG_SLV_DELAY_LLCP_STARTUP))) {
    ...
    loadRxBuf = ((status == BB_STATUS_CRC_FAILED) || pCtx->rxHdr.md || pCtx->txHdr.md) &&
                !lctrExceededMaxDur(pCtx, pConn->startTsUsec,
                                    SchBleCalcDataPktDurationUsec(pCtx->bleData.chan.txPhy, txLen) +
                                        pCtx->effConnDurUsec);
