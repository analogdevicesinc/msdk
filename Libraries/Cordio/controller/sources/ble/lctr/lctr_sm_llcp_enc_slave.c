...
static void lctrSlvCheckProcOverride(lctrConnCtx_t *pCtx, uint8_t event)
{
    ...
    case LCTR_ENC_EVENT_PEER_ENC_REQ:
        if (lctrSlvCheckEncOverrideConnParam(pCtx) || lctrSlvCheckEncOverrideCommonParam(pCtx)) {
...
bool_t lctrSlvExecuteEncryptSm(lctrConnCtx_t *pCtx, uint8_t event)
{
    ...
    case LCTR_LLCP_STATE_IDLE:
        LL_TRACE_INFO3("lctrSlvExecuteEncryptSm: handle=%u, llcpState=IDLE, encState=%u, event=%u",
    ...
    case LCTR_LLCP_STATE_BUSY:
        LL_TRACE_INFO3("lctrSlvExecuteEncryptSm: handle=%u, llcpState=BUSY, encState=%u, event=%u",
                       LCTR_GET_CONN_HANDLE(pCtx), pCtx->encState, event);
...
static void lctrPingSm(lctrConnCtx_t *pCtx, uint8_t event)
{
    ...
    case LCTR_PING_STATE_WAIT_FOR_RSP:
        ...
        case LCTR_PING_EVENT_RECV_REQ:
            LL_TRACE_WARN1("LE Ping procedure collision; send LE_PING_RSP anyway, handle=%u",
...
bool_t lctrExecutePingSm(lctrConnCtx_t *pCtx, uint8_t event)
{
    ...
    switch (pCtx->llcpState) {
    case LCTR_LLCP_STATE_IDLE:
        LL_TRACE_INFO3("lctrExecutePingSm: handle=%u, llcpState=IDLE, pingState=%u, event=%u",
                       LCTR_GET_CONN_HANDLE(pCtx), pCtx->pingState, event);
    ...
    case LCTR_LLCP_STATE_BUSY:
        LL_TRACE_INFO3("lctrExecutePingSm: handle=%u, llcpState=BUSY, pingState=%u, event=%u",
                       LCTR_GET_CONN_HANDLE(pCtx), pCtx->pingState, event);
        ...
        if (lctrCheckActiveOrPend(pCtx, LCTR_PROC_LE_PING)) {
            ...
            LL_TRACE_INFO2("Pending LE_PING=%u procedure: activeProc=%u", LCTR_PROC_LE_PING,
                           pCtx->llcpActiveProc);
        }
        break;
