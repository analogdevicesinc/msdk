...
lctrAdvSet_t *lctrFindAdvSet(uint8_t handle)
{
    ...
    for (i = 0; i < pLctrRtCfg->maxAdvSets; i++) {
        ...
        if ((pAdvSet->handle == handle) && (pAdvSet->enabled)) {
            return pAdvSet;
        }
    }
...
void lctrSelectNextPerChannel(lctrAdvSet_t *pAdvSet)
{
    do {
        ...
        pAdvSet->perParam.perChIdx =
            LL_MATH_MOD_37(pAdvSet->perParam.perChIdx + pAdvSet->perParam.perChHopInc);
 ...
 */
/*************************************************************************************************/
static uint8_t lctrSetExtAdvDataSm(lctrAdvSet_t *pAdvSet, lctrAdvDataBuf_t *pDataBuf,
                                   uint8_t fragOp, uint8_t fragPref, uint8_t fragLen,
                                   const uint8_t *pFragBuf)
{
    switch (fragOp) {
    case LL_ADV_DATA_OP_FRAG_INTER:
    case LL_ADV_DATA_OP_FRAG_LAST:
        if (pDataBuf->ready) {
            LL_TRACE_WARN2("Operation=%u missing a preceding LL_ADV_DATA_OP_FRAG_FIRST, handle=%u",
                           fragOp, pAdvSet->handle);
        ...
        if ((pDataBuf->len + fragLen) > pLctrRtCfg->maxExtAdvDataLen) {
            ...
            LL_TRACE_WARN2(
                "Fragments exceeded maximum buffer length -- discarding buffer, maxExtAdvDataLen=%u, handle=%u",
                pLctrRtCfg->maxExtAdvDataLen, pAdvSet->handle);
    ...
    case LL_ADV_DATA_OP_FRAG_FIRST:
        ...
        {
            LL_TRACE_WARN2("Operation=%u not permitted when advertising enabled, handle=%u", fragOp,
                           pAdvSet->handle);
            return LL_ERROR_CODE_CMD_DISALLOWED;
        }
        if (fragLen == 0) {
            LL_TRACE_WARN2("Fragments for operation=%u must have data, handle=%u", fragOp,
                           pAdvSet->handle);
            return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
        }
        break;
    ...
    case LL_ADV_DATA_OP_UNCHANGED:
        if (pAdvSet->state == LCTR_EXT_ADV_STATE_DISABLED) {
            LL_TRACE_WARN1(
                "Operation=LL_ADV_DATA_OP_UNCHANGED not permitted when advertising disabled, handle=%u",
                pAdvSet->handle);
            return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
        }
        if (fragLen > 0) {
            LL_TRACE_WARN1(
                "Operation=LL_ADV_DATA_OP_UNCHANGED not permitted with data fragment, handle=%u",
                pAdvSet->handle);
            return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
        }
        if (pDataBuf->len == 0) {
            LL_TRACE_WARN1(
                "Operation=LL_ADV_DATA_OP_UNCHANGED not permitted when no data buffer present, handle=%u",
                pAdvSet->handle);
            return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
        }
        break;
 ...
 */
/*************************************************************************************************/
static uint8_t lctrSetPerAdvDataSm(lctrAdvSet_t *pAdvSet, lctrAdvDataBuf_t *pDataBuf,
                                   uint8_t fragOp, uint8_t fragLen, const uint8_t *pFragBuf)
{
    switch (fragOp) {
    case LL_ADV_DATA_OP_FRAG_INTER:
    case LL_ADV_DATA_OP_FRAG_LAST:
        ...
        /* Fallthrough */
    case LL_ADV_DATA_OP_FRAG_FIRST:
        if (pAdvSet->perParam.perState !=
            LCTR_PER_ADV_STATE_DISABLED) /* e.g. advertising enabled */
        {
            LL_TRACE_WARN2(
                "Operation=%u not permitted when periodic advertising enabled, handle=%u", fragOp,
                pAdvSet->handle);
            return LL_ERROR_CODE_CMD_DISALLOWED;
        }
        if (fragLen == 0) {
    ...
    case LL_ADV_DATA_OP_UNCHANGED:
        if (pAdvSet->perParam.perState == LCTR_PER_ADV_STATE_DISABLED) {
            LL_TRACE_WARN1(
                "Operation=LL_ADV_DATA_OP_UNCHANGED not permitted when periodic advertising disabled, handle=%u",
                pAdvSet->handle);
            return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
        }
        if (fragLen > 0) {
                                      ...
                                      uint8_t fragOp, uint8_t len, const uint8_t *pData)
{
    if (fragOp != LL_ADV_DATA_OP_COMP) {
        LL_TRACE_WARN1("Legacy advertising does not accept data fragments, handle=%u",
                       pAdvSet->handle);
        return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
    }

    if (len > LL_ADVBU_MAX_LEN) {
        LL_TRACE_WARN2("Exceeded maximum legacy data buffer size=%u, handle=%u", LL_ADVBU_MAX_LEN,
                       pAdvSet->handle);
        return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
    }
...
uint8_t lctrSlvExtAdvBuildOp(lctrAdvSet_t *pAdvSet, uint32_t maxStartMs)
{
    ...
    if (pAdvSet->pExtAdvAuxPtr) {
        switch (pAdvSet->param.advEventProp &
                (LL_ADV_EVT_PROP_CONN_ADV_BIT | LL_ADV_EVT_PROP_SCAN_ADV_BIT)) {
...
void lctrSlvAuxRescheduleOp(lctrAdvSet_t *pAdvSet, BbOpDesc_t *const pOp)
{
    ...
    SchBleCalcAdvOpDuration(
        pOp, (pAdvSet->advData.fragPref == LL_ADV_DATA_FRAG_ALLOW) ? pAdvSet->advDataFragLen : 0);
...
void lctrSlvAuxNonConnNonScanBuildOp(lctrAdvSet_t *pAdvSet)
{
    ...
    WSF_ASSERT((pAdvSet->param.advEventProp &
                (LL_ADV_EVT_PROP_CONN_ADV_BIT | LL_ADV_EVT_PROP_SCAN_ADV_BIT)) == 0);
...
uint16_t LctrInitAdvSetMem(uint8_t *pFreeMem, uint32_t freeMemSize)
{
    ...
    LL_TRACE_INFO2("    RAM: %u x %u bytes -- advertising set context", pLctrRtCfg->maxAdvSets,
                   sizeof(lctrAdvSet_t));
    ...
    if (((uint32_t)(pAvailMem - pFreeMem)) > freeMemSize) {
        LL_TRACE_ERR2(
            "LctrInitAdvSetMem: failed to allocate advertising set context, need=%u available=%u",
...
uint8_t LctrIsExtAdvEnableParamValid(uint8_t enable, LlExtAdvEnableParam_t *pEnaParam)
{
    ...
    /* Advertising interval is determined by LL for high duty cycle directed adv. */
    if ((pAdvSet->param.advEventProp & LEGACY_HIGH_DUTY) == LEGACY_HIGH_DUTY) {
        if ((pEnaParam->duration > HIGH_DUTY_MAX_DURATION) || (pEnaParam->duration == 0)) {
            return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
        }
    }
...
uint8_t LctrIsExtAdvEnableReady(uint8_t handle)
{
    ...
    if (((pAdvSet->param.advEventProp &
          (LL_ADV_EVT_PROP_CONN_ADV_BIT | LL_ADV_EVT_PROP_LEGACY_ADV_BIT)) ==
         (LL_ADV_EVT_PROP_CONN_ADV_BIT |
          LL_ADV_EVT_PROP_LEGACY_ADV_BIT)) && /* Extended advertising with legacy PDU. */
        ...
        return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
               ...
               (pAdvSet->advData.len >
                (LL_EXT_ADVB_MAX_LEN - LL_ADV_HDR_LEN -
                 LL_EXT_ADV_HDR_MAX_LEN))) /* Assume maximum advertising header length.  */
    {
        LL_TRACE_WARN1("AdvData too large with extended connectable advertising, handle=%u",
                       handle);
        return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
    }
...
uint8_t LctrSetExtAdvSetRandAddr(uint8_t handle, const uint8_t *pAddr)
{
    ...
    if ((pAdvSet->param.advEventProp & LL_ADV_EVT_PROP_CONN_ADV_BIT) &&
        ...
        LL_TRACE_WARN1(
            "Cannot modify AdvSet's random address when connectable advertising, handle=%u",
            handle);
        return LL_ERROR_CODE_CMD_DISALLOWED;
    }
...
void lctrChooseSetAdvA(lctrAdvbPduHdr_t *pPduHdr, BbBleData_t *const pBle, lctrAdvSet_t *pAdvSet)
{
    ...
    lmgrSlvAdvCb.ownAddrType = pAdvSet->param.ownAddrType &
                               ~LL_ADDR_IDENTITY_BIT; /* Fallback to non-RPA. */
    lmgrSlvAdvCb.localRpa = 0;

    /* Fallback onto non-RPA if local IRK is zero or peer address is not in resolving list. */
    if ((lmgrSlvAdvCb.ownAddrType == LL_ADDR_RANDOM_BIT) && (pAdvSet->bdAddrRndValid)) {
    ...
    if (lmgrCb.addrResEna) {
        ...
        if (pAdvSet->param.ownAddrType & LL_ADDR_IDENTITY_BIT) {
            ...
            if (BbBleResListReadPeer(pAdvSet->param.peerAddrType, pAdvSet->param.peerAddr,
                                     &peerRpa) == FALSE) {
                BbBleResListGeneratePeer(pAdvSet->param.peerAddrType, pAdvSet->param.peerAddr,
                                         &peerRpa);
            }
...
uint8_t LctrSetExtAdvParam(uint8_t handle, LlExtAdvParam_t *pExtAdvParam)
{
    ...
    if (propUpdate && (pAdvSet->param.advEventProp & LL_ADV_EVT_PROP_LEGACY_ADV_BIT)) {
        ...
        memcpy(pAdvSet->advData.pBuf, pAdvSet->advData.alt.legacy.buf,
               pAdvSet->advData.alt.legacy.len);
        pAdvSet->scanRspData.len = pAdvSet->scanRspData.alt.legacy.len;
        memcpy(pAdvSet->scanRspData.pBuf, pAdvSet->scanRspData.alt.legacy.buf,
    ...
    if ((pExtAdvParam->advEventProp & LL_ADV_EVT_PROP_LEGACY_ADV_BIT) == 0) {
        if (pExtAdvParam->advEventProp & LL_ADV_EVT_PROP_CONN_ADV_BIT) {
            if (pAdvSet->scanRspData.len) {
                LL_TRACE_WARN1(
                    "Invalid scan response data buffer set with connectable advertising, handle=%u",
                    pAdvSet->handle);
                return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
            }
            ...
            if (pAdvSet->advData.len) {
                LL_TRACE_WARN1(
                    "Invalid advertising data buffer set with scannable advertising, handle=%u",
                    pAdvSet->handle);
                return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
            }
        } else /* ((pExtAdvParam->advEventProp & (LL_ADV_EVT_PROP_CONN_ADV_BIT | LL_ADV_EVT_PROP_SCAN_ADV_BIT)) == 0) */
        {
            if (pAdvSet->scanRspData.len) {
                LL_TRACE_WARN1(
                    "Invalid scan response data buffer set with non-scannable and non-connectable advertising, handle=%u",
                    pAdvSet->handle);
                return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
            }
        }
    } else /* (pExtAdvParam->advEventProp & LL_ADV_EVT_PROP_LEGACY_ADV_BIT) */
    {
        if (propUpdate && ((pAdvSet->param.advEventProp & LL_ADV_EVT_PROP_LEGACY_ADV_BIT) == 0)) {
            ...
            if (pAdvSet->advData.len > LL_ADVBU_MAX_LEN) {
                LL_TRACE_WARN2("Invalid advertising data buffer length=%u, handle=%u",
                               pAdvSet->advData.len, pAdvSet->handle);
                return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
            }
            if (pAdvSet->scanRspData.len > LL_ADVBU_MAX_LEN) {
                LL_TRACE_WARN2("Invalid scan response data buffer length=%u, handle=%u",
                               pAdvSet->scanRspData.len, pAdvSet->handle);
                return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
            }
            ...
            memcpy(pAdvSet->scanRspData.alt.legacy.buf, pAdvSet->scanRspData.pBuf,
                   pAdvSet->scanRspData.len);
        } else {
            if (pAdvSet->advData.len > LL_ADVBU_MAX_LEN) {
                LL_TRACE_WARN2("Invalid advertising data buffer length=%u, handle=%u",
                               pAdvSet->advData.len, pAdvSet->handle);
 ...
 */
/*************************************************************************************************/
uint8_t LctrSetExtAdvData(uint8_t handle, uint8_t op, uint8_t fragPref, uint8_t len,
                          const uint8_t *pData)
{
    uint8_t result;
    ...
    if ((pAdvSet->param.advEventProp & LL_ADV_EVT_PROP_LEGACY_ADV_BIT) == 0) {
        if (pAdvSet->param.advEventProp & LL_ADV_EVT_PROP_SCAN_ADV_BIT) {
            LL_TRACE_WARN1(
                "Cannot add advertising data buffer with scannable advertising, handle=%u",
                pAdvSet->handle);
            return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
        }

        if ((len > 0) && (pAdvSet->scanRspData.len > 0)) {
            LL_TRACE_WARN1("Extended advertising data buffer already in use, handle=%u",
                           pAdvSet->handle);
            return LL_ERROR_CODE_CMD_DISALLOWED;
        }
        ...
        if ((result = lctrSetLegacyAdvDataSm(pAdvSet, &pAdvSet->advData, op, len, pData)) ==
            LL_SUCCESS) {
...
uint8_t LctrSetPeriodicAdvParam(uint8_t handle, LlPerAdvParam_t *pPerAdvParam)
{
    ...
    uint32_t worstCaseUsec = SchBleCalcPerAdvDurationUsec(pAdvSet->param.secAdvPhy, 0, mafOffset,
                                                          pAdvSet->perAdvData.len, TRUE,
    ...
    pAdvSet->perParam.advInterMinUsec =
        LCTR_PER_INTER_TO_US(pPerAdvParam->perAdvInterMin); /* Convert parameter to bb ticks */
    pAdvSet->perParam.advInterMaxUsec =
        LCTR_PER_INTER_TO_US(pPerAdvParam->perAdvInterMax); /* Convert parameter to bb ticks */
...
uint8_t LctrSetPeriodicAdvData(uint8_t handle, uint8_t op, uint8_t len, const uint8_t *pData)
{
    ...
    if ((pAdvSet->perParam.advParamReady ==
         FALSE) || /* Adv set is not configured for periodic adv. */
        (pAdvSet->perParam.perAdvEnabled == TRUE &&
         op !=
             LL_ADV_DATA_OP_COMP) || /* When periodic adv is enabled, complete data shall be provided. Never gonna happen. */
        (len == 0 &&
         op != LL_ADV_DATA_OP_COMP)) /* Existing data shall be deleted and no new data provided. */
    {
        return LL_ERROR_CODE_CMD_DISALLOWED;
    }
    ...
    uint16_t mafOffset = WSF_MAX(pAdvSet->auxDelayUsec, pLctrRtCfg->auxDelayUsec);
    uint32_t maxInterval = (pAdvSet->perParam.perAdvEnabled) ? (pAdvSet->perParam.perAdvInterUsec) :
                                                               (pAdvSet->perParam.advInterMaxUsec);

    if (op == LL_ADV_DATA_OP_FRAG_INTER ||
        op == LL_ADV_DATA_OP_FRAG_LAST) /* Intermediate chained data. */
    {
        worstCaseUsec = SchBleCalcPerAdvDurationUsec(pAdvSet->param.secAdvPhy, 0, mafOffset,
                                                     len + pAdvSet->perAdvData.len, TRUE,
                                                     BB_PHY_OPTIONS_BLE_S8);
    } else /* Len is complete data. */
    {
        worstCaseUsec = SchBleCalcPerAdvDurationUsec(pAdvSet->param.secAdvPhy, 0, mafOffset, len,
                                                     TRUE, BB_PHY_OPTIONS_BLE_S8);
    }
    ...
    if ((result = lctrSetPerAdvDataSm(pAdvSet, &pAdvSet->perAdvData, op, len, pData)) ==
        LL_SUCCESS) {
...
void LctrSetPeriodicAdvEnable(uint8_t handle, bool_t enable)
{
    ...
    if ((pAdvSet->perParam.advParamReady ==
         FALSE) || /* Periodic advertising parameters shall be set. */
        ((enable == TRUE) &&
         (pAdvSet->perAdvData.ready == FALSE)) || /* Periodic advertising data shall be complete. */
        (pAdvSet->param.advEventProp &
         (LL_ADV_EVT_PROP_CONN_ADV_BIT |
          LL_ADV_EVT_PROP_SCAN_ADV_BIT | /* Only non-connectable and non-scannable is allowed. */
          LL_ADV_EVT_PROP_HIGH_DUTY_ADV_BIT |
          LL_ADV_EVT_PROP_OMIT_AA_BIT))) /* No high duty cycle, No anonymous advertising. */
    {
 ...
 */
/*************************************************************************************************/
uint8_t LctrSetExtScanRespData(uint8_t handle, uint8_t op, uint8_t fragPref, uint8_t len,
                               const uint8_t *pData)
{
    uint8_t result;
    ...
    if ((pAdvSet->param.advEventProp & LL_ADV_EVT_PROP_LEGACY_ADV_BIT) == 0) {
        if (((pAdvSet->param.advEventProp & LL_ADV_EVT_PROP_SCAN_ADV_BIT) == 0) && (len > 0)) {
            LL_TRACE_WARN1(
                "Cannot add scan response data buffer with non-scannable advertising, handle=%u",
                pAdvSet->handle);
            return LL_ERROR_CODE_INVALID_HCI_CMD_PARAMS;
        } else {
            if ((len == 0) && (op != LL_ADV_DATA_OP_COMP)) {
                LL_TRACE_WARN1(
                    "Zero-length scan response data is not valid with scannable advertising, handle=%u",
                    pAdvSet->handle);
                return LL_ERROR_CODE_CMD_DISALLOWED;
            }
        }
        ...
        if ((result = lctrSetLegacyAdvDataSm(pAdvSet, &pAdvSet->scanRspData, op, len, pData)) ==
            LL_SUCCESS) {
...
static void lctrSlvPeriodicCommitOp(lctrAdvSet_t *pAdvSet, BbOpDesc_t *const pOp)
{
    ...
    uint32_t offsetUsec = SchRmGetOffsetUsec(
        pAdvSet->perParam.perAdvInterUsec, LL_MAX_CONN + LCTR_GET_EXT_ADV_INDEX(pAdvSet),
        curTime); /* RM is shared by connections and periodic advertising. */
    ...
    while (TRUE) {
        ...
        LL_TRACE_WARN1("!!! Establish periodic advertising schedule conflict handle=%u",
                       pAdvSet->handle);
...
uint8_t lctrSlvPeriodicAdvBuildOp(lctrAdvSet_t *pAdvSet)
{
    ...
    if (!SchRmAdd(LCTR_GET_PER_RM_HANDLE(pAdvSet), SCH_RM_PREF_CAPACITY, interMinUsec, interMaxUsec,
                  durUsec, &perInterUsec, lctrGetPerRefTime)) {
