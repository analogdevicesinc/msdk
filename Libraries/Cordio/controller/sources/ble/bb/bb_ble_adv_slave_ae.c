...
static void bbSlvAuxAdvTxCompCback(uint8_t status)
{
    ...
    case BB_EVT_STATE_TX_ADV_IND:
        ...
        if (!pAuxAdv->pRxAuxReqBuf) {
            ...
            PalBbBleRxTifsData(pAuxAdv->pRxAuxReqBuf,
    ...
    case BB_EVT_STATE_TX_SCAN_OR_CONN_RSP:
        bbBleCb.evtState = BB_EVT_STATE_TX_CHAIN_IND;
        bbBleCb.bbParam.dueUsec =
            BbAdjustTime(pAuxAdv->auxReqStartTsUsec +
 ...
 */
/*************************************************************************************************/
static void bbSlvAuxAdvRxCompCback(uint8_t status, int8_t rssi, uint32_t crc, uint32_t timestamp,
