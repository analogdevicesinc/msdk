...
#define BB_EXT_PDU_FILT_MASK                                                              \
    ((1 << LL_PDU_ADV_EXT_IND) | (1 << LL_PDU_AUX_ADV_IND) | (1 << LL_PDU_AUX_SCAN_RSP) | \
     (1 << LL_PDU_AUX_SYNC_IND) | (1 << LL_PDU_AUX_CHAIN_IND) | (1 << LL_PDU_AUX_CONNECT_RSP))

/*! \brief      Tri-state result. */
enum { BB_BLE_PDU_FILT_UNKNOWN, BB_BLE_PDU_FILT_OK, BB_BLE_PDU_FILT_NOT_OK };
 ...
 */
/*************************************************************************************************/
static inline uint8_t bbBlePduFiltMatchLocal(bool_t addrMatchEna, bool_t idAddrRand,
                                             uint64_t idAddr, bool_t addrMatchRand,
                                             uint64_t addrMatch)
{
    if (addrMatchEna) {
        if ((idAddr != addrMatch) || (idAddrRand != addrMatchRand)) {
                         ...
                         bool_t forceRes, bbBlePduFiltResults_t *pFiltResults)
{
    ...
    switch (bbBlePduFiltResolvePeer(BB_BLE_PDU_FILT_FLAG_IS_SET(pFiltParams, PEER_ADDR_RES_ENA),
                                    forceRes, (uint8_t *)&pFiltResults->peerIdAddrRand,
    ...
    uint8_t matchResult = WSF_MAX(
        bbBlePduFiltMatchPeer(BB_BLE_PDU_FILT_FLAG_IS_SET(pFiltParams, PEER_ADDR_MATCH_ENA),
                              pFiltResults->peerIdAddrRand, pFiltResults->peerIdAddr,
   ...
   */
    ...
    case LL_PDU_CONNECT_IND: {
       ...
       */
        ...
        switch (bbBlePduFiltResolveLocal(
            BB_BLE_PDU_FILT_FLAG_IS_SET(pFiltParams, LOCAL_ADDR_RES_ENA), forceRes, localAddrRand,
            localAddr, pFiltResults->peerIdAddrRand, pFiltResults->peerIdAddr)) {
        case BB_BLE_PDU_FILT_OK:
        ...
        switch (bbBlePduFiltMatchLocal(
            BB_BLE_PDU_FILT_FLAG_IS_SET(pFiltParams, LOCAL_ADDR_MATCH_ENA), localAddrRand,
            localAddr, BB_BLE_PDU_FILT_FLAG_IS_SET(pFiltParams, LOCAL_ADDR_MATCH_RAND),
...
bool_t BbBleExtPduFiltCheck(const bbBlePduExtFiltParams_t *pExtFiltParams,
                            const bbBlePduFiltParams_t *pFiltParams, bool_t forceRes,
