...
# Object files
OBJ_FILES       := $(A_FILES:.S=.o)
OBJ_FILES       += $(C_FILES:.c=.o)
OBJ_FILES       := $(subst $(BSP_DIR)/,$(INT_DIR)/BSP/,$(OBJ_FILES))
...
all: $(TGT_TOKEN) $(BIN) show.options
	@echo "+++ Toolchain"
	@$(CC) --version | head -n 1

$(BIN): $(LIBS) $(OBJ_FILES) $(LD_FILE)
	@echo "+++ Linking: $@"
	@mkdir -p $(BIN_DIR)
	@$(LD) -o $(BIN) -T$(LD_FILE) $(OBJ_FILES) $(LIBS) $(LD_FLAGS)
	@$(OBJDUMP) -t $(BIN) > $(BIN:.elf=.sym)
	@$(OBJCOPY) -O binary $(BIN) $(BIN:.elf=.bin)
	@$(OBJCOPY) -O ihex $(BIN) $(BIN:.elf=.hex)
	@$(OBJDUMP) -t $(BIN) > $(BIN:.elf=.sym)
	@$(OBJDUMP) -S $(BIN) > $(BIN:.elf=.dasm)
