...
#define LL_ADVB_MIN_LEN \
    (LL_ADVB_MAX_LEN - LL_ADVBU_MAX_LEN) /*!< Minimum advertising channel packet length. */
#define LL_ADVB_MAX_TIME_1M \
    ((LL_BLE_US_PER_BYTE_1M * (LL_ADVB_MAX_LEN - LL_ADV_HDR_LEN)) + LL_MIN_PKT_TIME_US_1M)
/*!< Maximum time for a 1M advertising channel PDU. */
#define LL_ADVB_MAX_TIME_2M \
    ((LL_BLE_US_PER_BYTE_2M * (LL_ADVB_MAX_LEN - LL_ADV_HDR_LEN)) + LL_MIN_PKT_TIME_US_2M)
/*!< Maximum time for a 2M advertising channel PDU. */
#define LL_ADVB_MAX_TIME_S2                                               \
    ((LL_BLE_US_PER_BYTE_CODED_S2 * (LL_ADVB_MAX_LEN - LL_ADV_HDR_LEN)) + \
     LL_MIN_PKT_TIME_US_CODED_S2)
/*!< Maximum time for a Coded S2 advertising channel PDU. */
#define LL_ADVB_MAX_TIME_S8                                               \
    ((LL_BLE_US_PER_BYTE_CODED_S8 * (LL_ADVB_MAX_LEN - LL_ADV_HDR_LEN)) + \
     LL_MIN_PKT_TIME_US_CODED_S8)
/*!< Maximum time for a Coded S8 advertising channel PDU. */

#define LL_ADV_PKT_MAX_USEC \
    LL_ADVB_MAX_TIME_1M /*!< Maximum time in microseconds for an advertising packet. */
#define LL_SCAN_REQ_MAX_USEC \
    ((8 * (LL_ADV_PREFIX_LEN + LL_SCAN_PREFIX_LEN)) + LL_MIN_PKT_TIME_US_1M)
/*!< Maximum time in microseconds for a scan request packet. */
#define LL_SCAN_RSP_MAX_USEC \
    LL_ADVB_MAX_TIME_1M /*!< Maximum time in microseconds for a scan response packet. */
...
#define LL_DIR_ADV_INTER_TICKS \
    6 /*!< Advertising interval between directed advertising events (3.75 ms). */
#define LL_DIR_ADV_DUR_TICKS \
    2048 /*!< Maximum high duty cycle directed advertising duration (1.28 seconds). */
...
#define LL_EXT_ADV_HDR_MIN_LEN \
    1 /*!< Minimum extended advertising header length (ExtHdrLen and AdvMode fields). */
#define LL_EXT_ADV_HDR_MAX_LEN \
    64 /*!< Maximum extended advertising header length (ExtHdrLen, AdvMode fields and Extended header). */
...
#define LL_EXT_HDR_ACAD_MAX_LEN                       \
    LL_EXT_ADV_HDR_MAX_LEN - LL_EXT_ADV_HDR_MIN_LEN - \
        LL_EXT_HDR_FLAG_LEN /*!< Maximum possible ACAD length (Max extended header minus ExtHdrLen, AdvMode, and extended header flag field. */

#define LL_EXT_ADVB_MAX_TIME_1M \
    ((LL_BLE_US_PER_BYTE_1M * (LL_EXT_ADVB_MAX_LEN - LL_ADV_HDR_LEN)) + LL_MIN_PKT_TIME_US_1M)
/*!< Maximum time for a 1M advertising channel PDU. */
#define LL_EXT_ADVB_MAX_TIME_2M \
    ((LL_BLE_US_PER_BYTE_2M * (LL_EXT_ADVB_MAX_LEN - LL_ADV_HDR_LEN)) + LL_MIN_PKT_TIME_US_2M)
/*!< Maximum time for a 2M advertising channel PDU. */
#define LL_EXT_ADVB_MAX_TIME_S2                                               \
    ((LL_BLE_US_PER_BYTE_CODED_S2 * (LL_EXT_ADVB_MAX_LEN - LL_ADV_HDR_LEN)) + \
     LL_MIN_PKT_TIME_US_CODED_S2)
/*!< Maximum time for a Coded S2 advertising channel PDU. */
#define LL_EXT_ADVB_MAX_TIME_S8                                               \
    ((LL_BLE_US_PER_BYTE_CODED_S8 * (LL_EXT_ADVB_MAX_LEN - LL_ADV_HDR_LEN)) + \
     LL_MIN_PKT_TIME_US_CODED_S8)
/*!< Maximum time for a Coded S8 advertising channel PDU. */
#define LL_EXT_ADVB_NORMAL_TIME_S8                                               \
    ((LL_BLE_US_PER_BYTE_CODED_S8 * (LL_EXT_ADVB_NORMAL_LEN - LL_ADV_HDR_LEN)) + \
     LL_MIN_PKT_TIME_US_CODED_S8)
...
enum {
    ...
    LL_SYNC_TRSF_MODE_REP_DISABLED =
        1, /*!< Periodic sync transfer receive is enabled, report event is disabled. */
    LL_SYNC_TRSF_MODE_REP_ENABLED =
        2, /*!< Periodic sync transfer receive is enabled, report event is enabled. */
...
#define LL_DATA_HDR_LEN_MSK \
    0xFF /*!< Data header length mask. BLE 4.2 data len extension allows 8 bits. */
...
enum {
    ...
    LL_LLID_START_PDU =
        0x02, /*!< Data PDU: start of an L2CAP message or a complete L2CAP message with no fragmentation. */
...
#define LL_DATA_LEN_TO_TIME_1M(len, enc) \
    ((LL_BLE_US_PER_BYTE_1M * ((len) + ((enc) ? LL_DATA_MIC_LEN : 0))) + LL_MIN_PKT_TIME_US_1M)
/*!< Convert data length to time. */
#define LL_DATA_LEN_TO_TIME_2M(len, enc) \
    ((LL_BLE_US_PER_BYTE_2M * ((len) + ((enc) ? LL_DATA_MIC_LEN : 0))) + LL_MIN_PKT_TIME_US_2M)
/*!< Convert data length to time. */
#define LL_DATA_LEN_TO_TIME_CODED_S8(len, enc)                                 \
    ((LL_BLE_US_PER_BYTE_CODED_S8 * ((len) + ((enc) ? LL_DATA_MIC_LEN : 0))) + \
     LL_MIN_PKT_TIME_US_CODED_S8)
/*!< Convert data length to time. */
#define LL_DATA_LEN_TO_TIME_CODED_S2(len, enc)                                 \
    ((LL_BLE_US_PER_BYTE_CODED_S2 * ((len) + ((enc) ? LL_DATA_MIC_LEN : 0))) + \
     LL_MIN_PKT_TIME_US_CODED_S2)
/*!< Convert data length to time. */
...
#define LL_MAX_DATA_TIME_ABS_MAX_1M \
    2128 /*!< Absolute maximum limit for maximum Data PDU time (LE 1M PHY) */
#define LL_MAX_DATA_TIME_ABS_MIN_CODED \
    2704 /*!< Absolute minimum limit for maximum Data PDU time (CODED PHY) */
...
typedef enum {
    ...
    LL_LLID_ISO_UNF_CONT_PDU =
        0x01, /*!< Unframed ISO Data PDU; start or continuation fragment of a SDU. */
...
#define LL_MIN_PKT_TIME_US_CODED_S8 \
    720 /*!< Minimum packet time (i.e. empty PDU) on LE Coded PHY, S=8. */
#define LL_MIN_PKT_TIME_US_CODED_S2 \
    462 /*!< Minimum packet time (i.e. empty PDU) on LE Coded PHY, S=2. */
...
#ifndef LL_WW_RX_DEVIATION_USEC
#define LL_WW_RX_DEVIATION_USEC 16 /*!< RX deviation in microseconds for window widening. */
#endif

#define LL_30_USEC_OFFS_MAX_USEC \
    245730 /*!< Maximum value for 30 microseconds offset unit in microseconds. */
...
#define LL_ACAD_DATA_FIELD_MAX_LEN \
    (LL_EXT_HDR_ACAD_MAX_LEN - LL_ACAD_OPCODE_LEN - LL_ACAD_LEN_FIELD_LEN)
