...
#define BYTES_TO_UINT24(n, p)                                                        \
    {                                                                                \
        n = ((uint16_t)(p)[0] + ((uint16_t)(p)[1] << 8) + ((uint16_t)(p)[2] << 16)); \
    }
/*! \brief convert little endian byte buffer to uint32_t. */
#define BYTES_TO_UINT32(n, p)                                                        \
    {                                                                                \
        n = ((uint32_t)(p)[0] + ((uint32_t)(p)[1] << 8) + ((uint32_t)(p)[2] << 16) + \
             ((uint32_t)(p)[3] << 24));                                              \
    }
...
#define BYTES_BE_TO_UINT24(n, p)                                                     \
    {                                                                                \
        n = ((uint16_t)(p)[2] + ((uint16_t)(p)[1] << 8) + ((uint16_t)(p)[0] << 16)); \
    }
...
#define BSTREAM_BE_TO_UINT16(n, p) \
    {                              \
        BYTES_BE_TO_UINT16(n, p);  \
        p += 2;                    \
    }
/*! \brief convert big endian byte stream to 24-bit uint32_t (MSB 0), incrementing one byte. */
#define BSTREAM_BE_TO_UINT24(n, p) \
    {                              \
        BYTES_BE_TO_UINT24(n, p);  \
        p += 3;                    \
    }
/**@}*/
...
#define UINT16_TO_BE_BSTREAM(p, n)    \
    {                                 \
        *(p)++ = (uint8_t)((n) >> 8); \
        *(p)++ = (uint8_t)(n);        \
    }
...
#define UINT16_TO_SFLT_M(n) \
    ((((n)&0x0FFF) >= 0x0800) ? ((int16_t)(((n) | 0xF000))) : ((int16_t)((n)&0x0FFF)))
/*! \brief Convert uint16_T to sfloat exponent component */
#define UINT16_TO_SFLT_E(n) \
    (((n >> 12) >= 0x0008) ? ((int8_t)(((n >> 12) | 0xF0))) : ((int8_t)(n >> 12)))
/**@}*/
