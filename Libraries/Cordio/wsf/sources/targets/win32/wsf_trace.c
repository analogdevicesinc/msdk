 ...
 */
/*************************************************************************************************/
static wsfPDumpHciAcl(char *pTypeStr, char *pTimeBuf, char *pLineBuf, uint16_t length,
                      uint8_t *pBuffer)
{
    char *pOpStr;
    uint16_t cid;
    ...
    if ((hciHandle & HCI_PB_FLAG_MASK) == HCI_PB_CONTINUE) {
        snprintf(pLineBuf, WSF_TRACE_BUFFER_SIZE,
                 "[%s] %s: HCI continuation packet Handle=%u Length=%u\n", pTimeBuf, pTypeStr,
                 (hciHandle & HCI_HANDLE_MASK), hciLen);
    } else {
        ...
        if ((cid == L2C_CID_ATT) ||
            ...
            switch (op) {
            case ATT_PDU_ERR_RSP:
                pOpStr = "ATT ERROR Response";
                break;
            case ATT_PDU_MTU_REQ:
                pOpStr = "ATT EXCHANGE_MTU Request";
                break;
            case ATT_PDU_MTU_RSP:
                pOpStr = "ATT EXCHANGE_MTU Response";
                break;
            case ATT_PDU_FIND_INFO_REQ:
                pOpStr = "ATT FIND_INFORMATION Request";
                break;
            case ATT_PDU_FIND_INFO_RSP:
                pOpStr = "ATT FIND_INFORMATION Response";
                break;
            case ATT_PDU_FIND_TYPE_REQ:
                pOpStr = "ATT FIND_BY_TYPE_VALUE Request";
                break;
            case ATT_PDU_FIND_TYPE_RSP:
                pOpStr = "ATT FIND_BY_TYPE_VALUE Response";
                break;
            case ATT_PDU_READ_TYPE_REQ:
                pOpStr = "ATT READ_BY_TYPE Request";
                break;
            case ATT_PDU_READ_TYPE_RSP:
                pOpStr = "ATT READ_BY_TYPE Response";
                break;
            case ATT_PDU_READ_REQ:
                pOpStr = "ATT READ Request";
                break;
            case ATT_PDU_READ_RSP:
                pOpStr = "ATT READ Response";
                break;
            case ATT_PDU_READ_BLOB_REQ:
                pOpStr = "ATT READ_BLOB Request";
                break;
            case ATT_PDU_READ_BLOB_RSP:
                pOpStr = "ATT READ_BLOB Response";
                break;
            case ATT_PDU_READ_MULT_REQ:
                pOpStr = "ATT READ_MULTIPLE Request";
                break;
            case ATT_PDU_READ_MULT_RSP:
                pOpStr = "ATT READ_MULTIPLE Response";
                break;
            case ATT_PDU_READ_GROUP_TYPE_REQ:
                pOpStr = "ATT READ_BY_GROUP_TYPE Request";
                break;
            case ATT_PDU_READ_GROUP_TYPE_RSP:
                pOpStr = "ATT READ_BY_GROUP_TYPE Response";
                break;
            case ATT_PDU_WRITE_REQ:
                pOpStr = "ATT WRITE Request";
                break;
            case ATT_PDU_WRITE_RSP:
                pOpStr = "ATT WRITE Response";
                break;
            case ATT_PDU_WRITE_CMD:
                pOpStr = "ATT WRITE Command";
                break;
            case ATT_PDU_SIGNED_WRITE_CMD:
                pOpStr = "ATT SIGNED WRITE Command";
                break;
            case ATT_PDU_PREP_WRITE_REQ:
                pOpStr = "ATT PREPARE_WRITE Request";
                break;
            case ATT_PDU_PREP_WRITE_RSP:
                pOpStr = "ATT PREPARE_WRITE Response";
                break;
            case ATT_PDU_EXEC_WRITE_REQ:
                pOpStr = "ATT EXECUTE_WRITE Request";
                break;
            case ATT_PDU_EXEC_WRITE_RSP:
                pOpStr = "ATT EXECUTE_WRITE Response";
                break;
            case ATT_PDU_VALUE_NTF:
                pOpStr = "ATT HANDLE_VALUE Notification";
                break;
            case ATT_PDU_VALUE_IND:
                pOpStr = "ATT HANDLE_VALUE Indication";
                break;
            case ATT_PDU_VALUE_CNF:
                pOpStr = "ATT HANDLE_VALUE Confirm";
                break;
            case ATT_PDU_READ_MULT_VAR_REQ:
                pOpStr = "ATT READ_MULTIPLE_VARIABLE Request";
                break;
            case ATT_PDU_READ_MULT_VAR_RSP:
                pOpStr = "ATT READ_MULTIPLE_VARIABLE Response";
                break;
            case ATT_PDU_MULT_VALUE_NTF:
                pOpStr = "ATT MULT_VALUE Notification";
                break;
            default:
                pOpStr = "ATT UNKNOWN PDU";
                break;
            }

            snprintf(pLineBuf, WSF_TRACE_BUFFER_SIZE, "[%s] %s: %s CID=0x%04x (0x%02x)\n", pTimeBuf,
                     pTypeStr, pOpStr, cid, op);
        } else if (cid == L2C_CID_LE_SIGNALING) {
            BSTREAM_TO_UINT8(op, pBuffer);
            switch (op) {
            case L2C_SIG_CMD_REJ:
                pOpStr = "L2CAP Command Reject";
                break;
            case L2C_SIG_DISCONNECT_REQ:
                pOpStr = "L2CAP Disconnect Request";
                break;
            case L2C_SIG_DISCONNECT_RSP:
                pOpStr = "L2CAP Disconnect Response";
                break;
            case L2C_SIG_CONN_UPDATE_REQ:
                pOpStr = "L2CAP Connection Param Update Request";
                break;
            case L2C_SIG_CONN_UPDATE_RSP:
                pOpStr = "L2CAP Connection Param Update Response";
                break;
            case L2C_SIG_LE_CONNECT_REQ:
                pOpStr = "L2CAP LE Connection Request";
                break;
            case L2C_SIG_LE_CONNECT_RSP:
                pOpStr = "L2CAP LE Connection Response";
                break;
            case L2C_SIG_FLOW_CTRL_CREDIT:
                pOpStr = "L2CAP LE Flow Control Credit";
                break;
            case L2C_SIG_EN_CONNECT_REQ:
                pOpStr = "L2CAP LE Enhanced Connection Request";
                break;
            case L2C_SIG_EN_CONNECT_RSP:
                pOpStr = "L2CAP LE Enhanced Connection Response";
                break;
            case L2C_SIG_EN_RECONFIG_REQ:
                pOpStr = "L2CAP LE Reconfiguration Request";
                break;
            case L2C_SIG_EN_RECONFIG_RSP:
                pOpStr = "L2CAP LE Reconfiguration Response";
                break;
            default:
                pOpStr = "L2CAP UNKNOWN PDU";
                break;
            }
            ...
            BSTREAM_TO_UINT8(op, pBuffer);
            switch (op) {
            case SMP_CMD_PAIR_REQ:
                pOpStr = "SMP Pairing Request";
                break;
            case SMP_CMD_PAIR_RSP:
                pOpStr = "SMP Pairing Response";
                break;
            case SMP_CMD_PAIR_CNF:
                pOpStr = "SMP Pairing Confirm";
                break;
            case SMP_CMD_PAIR_RAND:
                pOpStr = "SMP Pairing Random";
                break;
            case SMP_CMD_PAIR_FAIL:
                pOpStr = "SMP Pairing Failed";
                break;
            case SMP_CMD_ENC_INFO:
                pOpStr = "SMP Encryption Information";
                break;
            case SMP_CMD_MASTER_ID:
                pOpStr = "SMP Master Identification";
                break;
            case SMP_CMD_ID_INFO:
                pOpStr = "SMP Identity Information";
                break;
            case SMP_CMD_ID_ADDR_INFO:
                pOpStr = "SMP Identity Address Information";
                break;
            case SMP_CMD_SIGN_INFO:
                pOpStr = "SMP Signing Information";
                break;
            case SMP_CMD_SECURITY_REQ:
                pOpStr = "SMP Security Request";
                break;
            case SMP_CMD_PUBLIC_KEY:
                pOpStr = "SMP Public Key";
                break;
            case SMP_CMD_DHKEY_CHECK:
                pOpStr = "SMP DH Key Check";
                break;
            case SMP_CMD_KEYPRESS:
                pOpStr = "SMP User Key Press";
                break;
            default:
                pOpStr = "SMP UNKNOWN PDU";
                break;
            }
            ...
            snprintf(pLineBuf, WSF_TRACE_BUFFER_SIZE,
                     "[%s] %s: L2CAP data packet CID=0x%04x Length=0x%04x\n", pTimeBuf, pTypeStr,
                     cid, len);
        }
    }
 ...
 */
/*************************************************************************************************/
static wsfPDumpHciIso(char *pTypeStr, char *pTimeBuf, char *pLineBuf, uint16_t length,
                      uint8_t *pBuffer)
{
    uint16_t hciHandle;
    uint16_t hciLen;
 ...
 */
/*************************************************************************************************/
static void wsfPDumpHciEvt(char *pTypeStr, char *pTimeBuf, char *pLineBuf, uint16_t length,
                           uint8_t *pBuffer)
{
    char *pEvtStr;
    uint8_t evt = *pBuffer;
    ...
    case HCI_NUM_CMPL_PKTS_EVT:
        pEvtStr = "NUM_CMPL_PKTS";
        break;
    case HCI_DATA_BUF_OVERFLOW_EVT:
        pEvtStr = "DATA_BUF_OVERFLOW";
        break;
    case HCI_ENC_KEY_REFRESH_CMPL_EVT:
        pEvtStr = "ENC_KEY_REFRESH_CMPL";
        break;
    case HCI_AUTH_PAYLOAD_TIMEOUT_EVT:
        pEvtStr = "AUTH_PAYLOAD_TIMEOUT";
        break;
    ...
    case HCI_LE_META_EVT:
        ...
        case HCI_LE_CONN_CMPL_EVT:
            pEvtStr = "LE_CONN_CMPL";
            break;
        case HCI_LE_ADV_REPORT_EVT:
            pEvtStr = "LE_ADV_REPORT";
            break;
        case HCI_LE_CONN_UPDATE_CMPL_EVT:
            pEvtStr = "LE_CONN_UPDATE_CMPL";
            break;
        case HCI_LE_READ_REMOTE_FEAT_CMPL_EVT:
            pEvtStr = "LE_READ_REMOTE_FEAT_CMPL";
            break;
        case HCI_LE_LTK_REQ_EVT:
            pEvtStr = "LE_LTK_REQ";
            break;
        case HCI_LE_REM_CONN_PARAM_REQ_EVT:
            pEvtStr = "LE_REM_CONN_PARAM_REQ";
            break;
        case HCI_LE_DATA_LEN_CHANGE_EVT:
            pEvtStr = "LE_DATA_LEN_CHANGE";
            break;
        case HCI_LE_READ_LOCAL_P256_PUB_KEY_CMPL_EVT:
            pEvtStr = "LE_READ_LOCAL_P256_PUB_KEY_CMPL";
            break;
        case HCI_LE_GENERATE_DHKEY_CMPL_EVT:
            pEvtStr = "LE_GENERATE_DHKEY_CMPL";
            break;
        case HCI_LE_ENHANCED_CONN_CMPL_EVT:
            pEvtStr = "LE_ENHANCED_CONN_CMPL";
            break;
        case HCI_LE_DIRECT_ADV_REPORT_EVT:
            pEvtStr = "LE_DIRECT_ADV_REPORT";
            break;
        case HCI_LE_PHY_UPDATE_CMPL_EVT:
            pEvtStr = "LE_PHY_UPDATE_CMPL";
            break;
        case HCI_LE_EXT_ADV_REPORT_EVT:
            pEvtStr = "LE_EXT_ADV_REPORT_EVT";
            break;
        case HCI_LE_PER_ADV_SYNC_EST_EVT:
            pEvtStr = "LE_PER_ADV_SYNC_EST_EVT";
            break;
        case HCI_LE_PER_ADV_REPORT_EVT:
            pEvtStr = "LE_PER_ADV_REPORT_EVT";
            break;
        case HCI_LE_PER_ADV_SYNC_LOST_EVT:
            pEvtStr = "LE_PER_ADV_SYNC_LOST_EVT";
            break;
        case HCI_LE_SCAN_TIMEOUT_EVT:
            pEvtStr = "LE_SCAN_TIMEOUT_EVT";
            break;
        case HCI_LE_ADV_SET_TERM_EVT:
            pEvtStr = "LE_ADV_SET_TERM_EVT";
            break;
        case HCI_LE_SCAN_REQ_RCVD_EVT:
            pEvtStr = "LE_SCAN_REQ_RCVD_EVT";
            break;
        case HCI_LE_CH_SEL_ALGO_EVT:
            pEvtStr = "LE_CH_SEL_ALGO_EVT";
            break;
        case HCI_LE_CONNLESS_IQ_REPORT_EVT:
            pEvtStr = "LE_CONNLESS_IQ_REPORT_EVT";
            break;
        case HCI_LE_CONN_IQ_REPORT_EVT:
            pEvtStr = "LE_CONN_IQ_REPORT_EVT";
            break;
        case HCI_LE_CTE_REQ_FAILED_EVT:
            pEvtStr = "LE_CTE_REQ_FAILED_EVT";
            break;
        case HCI_LE_PER_SYNC_TRSF_RCVD_EVT:
            pEvtStr = "LE_PER_SYNC_TRSF_RCVD_EVT";
            break;
        case HCI_LE_CIS_EST_EVT:
            pEvtStr = "LE_CIS_EST_EVT";
            break;
        case HCI_LE_CIS_REQ_EVT:
            pEvtStr = "LE_CIS_REQ_EVT";
            break;
        case HCI_LE_CREATE_BIG_CMPL_EVT:
            pEvtStr = "LE_CREATE_BIG_CMPL_EVT";
            break;
        case HCI_LE_TERMINATE_BIG_CMPL_EVT:
            pEvtStr = "LE_TERMINATE_BIG_CMPL_EVT";
            break;
        case HCI_LE_BIG_SYNC_EST_EVT:
            pEvtStr = "LE_BIG_SYNC_EST_EVT";
            break;
        case HCI_LE_BIG_SYNC_LOST_EVT:
            pEvtStr = "LE_BIG_SYNC_LOST_EVT";
            break;
        case HCI_LE_REQ_PEER_SCA_CMPLT_EVT:
            pEvtStr = "LE_REQ_PEER_SCA_CMPLT_EVT";
            break;
        case HCI_LE_PATH_LOSS_REPORT_EVT:
            pEvtStr = "LE_PATH_LOSS_REPORT_EVT";
            break;
        case HCI_LE_POWER_REPORT_EVT:
            pEvtStr = "LE_POWER_REPORT_EVT";
            break;
        case HCI_LE_BIG_INFO_ADV_REPORT_EVT:
            pEvtStr = "LE_BIG_INFO_ADV_REPORT_EVT";
            break;
        default:
            pEvtStr = "UNKNOWN";
            break;
    ...
    if (evt == HCI_CMD_CMPL_EVT || evt == HCI_CMD_STATUS_EVT) {
        snprintf(pLineBuf, WSF_TRACE_BUFFER_SIZE,
                 "[%s] %s: %s Event (0x%02x) Opcode=0x%04x Status=0x%02x\n", pTimeBuf, pTypeStr,
                 pEvtStr, evt, opcode, status);
 ...
 */
/*************************************************************************************************/
static wsfPDumpHciCmd(char *pTypeStr, char *pTimeBuf, char *pLineBuf, uint16_t length,
                      uint8_t *pBuffer)
{
    uint16_t opcode;
    char *pOpStr;
...
void WsfTrace(wsfTraceType_t ttype, wsfTraceSubsys_t subsys, const char *pMsg, ...)
{
    ...
    GetTimeFormat(LANG_USER_DEFAULT, 0, &curTime, "HH:mm:ss", timeBuf, sizeof(timeBuf));
    ...
    snprintf(traceBuf, WSF_TRACE_BUFFER_SIZE, "[%s] %-3s%s: ", timeBuf, wsfTraceSubsys[subsys],
             pTypeStr);
...
void WsfPDump(wsfPDumpType_t pdType, uint16_t length, uint8_t *pBuffer)
{
    ...
    if (vsEvent != WSF_DETOKEN_VS_EVT_TOKEN) {
        while (length-- > 0) {
            ...
            /* output byte */
            snprintf(lineBuf + strlen(lineBuf), WSF_TRACE_BUFFER_SIZE - strlen(lineBuf), "%02x ",
                     *pBuffer++);
