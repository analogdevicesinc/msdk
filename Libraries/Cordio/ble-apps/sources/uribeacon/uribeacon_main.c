...
#define URIBEACON_BEACONPERIOD_CLAMP(bp)                                  \
    (((bp) < URICFG_ATT_BEACONPERIOD_MIN) ? URICFG_ATT_BEACONPERIOD_MIN : \
...
static const attsCccSet_t uriBeaconCccSet[URIBEACON_NUM_CCC_IDX] = {
    ...
    { GATT_SC_CH_CCC_HDL, ATT_CLIENT_CFG_INDICATE,
      DM_SEC_LEVEL_NONE } /* URIBEACON_GATT_SC_CCC_IDX */
};
...
static uint8_t uriBeaconAdvDataDisc[HCI_ADV_DATA_LEN] = {
    ...
    1u + sizeof(uint8_t), DM_ADV_TYPE_FLAGS, DM_FLAG_LE_LIMITED_DISC | DM_FLAG_LE_BREDR_NOT_SUP,

    /* UriBeacon service UUID */
    3u, DM_ADV_TYPE_16_UUID, UINT16_TO_BYTES(URICFG_SERVICE_UUID),

    /* UriBeacon service data */
    5u + sizeof(uriBeaconUriData), /* length (5-23) */
    DM_ADV_TYPE_SERVICE_DATA, UINT16_TO_BYTES(URICFG_SERVICE_UUID), 0u, /* flags */
    0, /* tx power level */
...
static const char *const uriBeaconPrefixes[] = { "http://www.", "https://www.", "http://",
                                                 "https://", "urn:uuid:" };
...
static void uriBeaconAttWriteCback(uint16_t handle, uint16_t valueLen, const uint8_t *pValue)
{
    ...
    if (updatedAdvData) {
        ...
        AppAdvSetData(APP_ADV_DATA_CONNECTABLE, sizeof(uriBeaconAdvDataDisc),
                      (uint8_t *)uriBeaconAdvDataDisc);
    }
}
...
static void uriBeaconLockChangeCback(uint8_t lockState, const uint8_t lock[URICFG_SIZE_LOCK_ATT])
{
    ...
    if (!WsfNvmWriteData((uint64_t)URI_BEACON_PARAM_LOCK_STATE, &lockState, sizeof(lockState),
                         NULL)) {
...
static void uriBeaconSetup(wsfMsgHdr_t *pMsg)
{
    ...
    AppAdvSetData(APP_ADV_DATA_DISCOVERABLE, sizeof(uriBeaconAdvDataDisc),
                  (uint8_t *)uriBeaconAdvDataDisc);
    ...
    AppAdvSetAdValue(APP_SCAN_DATA_DISCOVERABLE, DM_ADV_TYPE_LOCAL_NAME,
                     sizeof(uriBeaconScanDataLocalName), (uint8_t *)uriBeaconScanDataLocalName);
    AppAdvSetAdValue(APP_SCAN_DATA_DISCOVERABLE, DM_ADV_TYPE_128_UUID,
                     sizeof(uriBeaconScanData128Uuid), (uint8_t *)uriBeaconScanData128Uuid);
    ...
    AppAdvSetAdValue(APP_SCAN_DATA_CONNECTABLE, DM_ADV_TYPE_LOCAL_NAME,
                     sizeof(uriBeaconScanDataLocalName), (uint8_t *)uriBeaconScanDataLocalName);
    AppAdvSetAdValue(APP_SCAN_DATA_CONNECTABLE, DM_ADV_TYPE_128_UUID,
                     sizeof(uriBeaconScanData128Uuid), (uint8_t *)uriBeaconScanData128Uuid);
...
static void uriBeaconProcMsg(wsfMsgHdr_t *pMsg)
{
    ...
    case DM_ADV_START_IND:
        if (pMsg->status == HCI_SUCCESS) {
            uiEvent = (uriBeaconAdvType == DM_ADV_NONCONN_UNDIRECT) ? APP_UI_DISCOVERABLE :
                                                                      APP_UI_ADV_START;
        }
        break;
...
void UriBeaconStart(void)
{
    ...
    if (WsfNvmReadData((uint64_t)URI_BEACON_PARAM_LOCK_STATE, (uint8_t *)&lockState,
                       sizeof(lockState), NULL)) {
    ...
    } else if (WsfNvmReadData((uint64_t)URI_BEACON_PARAM_URI_DATA, &uriData[0], sizeof(uriData),
                              NULL)) {
    ...
    if (WsfNvmReadData((uint64_t)URI_BEACON_PARAM_URI_FLAGS, (uint8_t *)&uriFlags, sizeof(uriFlags),
                       NULL)) {
    ...
    if (WsfNvmReadData((uint64_t)URI_BEACON_PARAM_ADVERTISED_TX_POWER_LEVELS,
                       (uint8_t *)&uriBeaconAdvertisedTxPwrLevels,
                       sizeof(uriBeaconAdvertisedTxPwrLevels), NULL)) {
        APP_TRACE_INFO2("URI rd adv tx pwr lvls from param DB {%d, %d,",
                        uriBeaconAdvertisedTxPwrLevels[0], uriBeaconAdvertisedTxPwrLevels[1]);
        APP_TRACE_INFO2("                                      %d, %d}",
                        uriBeaconAdvertisedTxPwrLevels[2], uriBeaconAdvertisedTxPwrLevels[3]);
    } else {
        memcpy(uriBeaconAdvertisedTxPwrLevels, uriAdvertisedTxPwrLevelsDefault,
               sizeof(uriBeaconAdvertisedTxPwrLevels));
    }

    if (WsfNvmReadData((uint64_t)URI_BEACON_PARAM_TX_POWER_MODE, (uint8_t *)&uriBeaconTxPwrMode,
                       sizeof(uriBeaconTxPwrMode), NULL)) {
        APP_TRACE_INFO1("URI rd tx pwr mode from param DB %u", uriBeaconTxPwrMode);
    } else {
        uriBeaconTxPwrMode = URICFG_ATT_TXPWRMODE_DEFAULT;
    }

    if (WsfNvmReadData((uint64_t)URI_BEACON_PARAM_BEACON_PERIOD, (uint8_t *)&beaconPeriod,
                       sizeof(beaconPeriod), NULL)) {
    ...
    SvcCoreAddGroup();
    SvcDisAddGroup();
    UriCfgStart(uriData, uriDataLen, uriFlags, uriBeaconAdvertisedTxPwrLevels, uriBeaconTxPwrMode,
                beaconPeriod);
