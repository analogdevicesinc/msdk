    ...
    if (pDmEvt->hdr.event == DM_SEC_ECC_KEY_IND) {
        ...
        if ((pMsg = WsfMsgAlloc(len + reportLen)) != NULL) {
            ...
            if (pDmEvt->hdr.event == DM_SCAN_REPORT_IND) {
                ...
                memcpy(pMsg->connIQReport.pISample, pDmEvt->connIQReport.pISample,
                       pDmEvt->connIQReport.sampleCnt);

                /* Copy Q samples to space after I samples space */
                pMsg->connIQReport.pQSample = (int8_t *)((uint8_t *)pMsg->connIQReport.pISample +
                                                         pDmEvt->connIQReport.sampleCnt);
                memcpy(pMsg->connIQReport.pQSample, pDmEvt->connIQReport.pQSample,
                       pDmEvt->connIQReport.sampleCnt);
            }
...
static void locatorScanReport(dmEvt_t *pMsg)
{
    ...
    /* find vendor-specific advertising data */
        ...
        if (pData[DM_AD_LEN_IDX] >= 3 && BYTES_UINT16_CMP(&pData[DM_AD_DATA_IDX], HCI_ID_ANALOG)) {
            connect = TRUE;
        }
    }
    ...
    case APP_DISC_CMPL:
        ...
        if (locatorCb.discState[connId - 1] == LOCATOR_DISC_GAP_SVC) {
            ...
            AppDiscConfigure(connId, APP_DISC_CFG_START, LOCATOR_DISC_CFG_LIST_LEN,
                             (attcDiscCfg_t *)locatorDiscCfgList, LOCATOR_DISC_HDL_LIST_LEN,
                             locatorCb.hdlList[connId - 1]);
        }
        break;
