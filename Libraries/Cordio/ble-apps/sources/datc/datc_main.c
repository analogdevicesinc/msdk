...
static void datcWdxcProcessDataRateTimeout(dmConnId_t connId)
{
    ...
    /* Calculate data rate */
    bps = (datcCb.rxCount[connId - 1] - datcCb.lastCount[connId - 1]) * 8 /
          DATC_WDXS_DATA_RATE_TIMEOUT;
    datcCb.lastCount[connId - 1] = datcCb.rxCount[connId - 1];

    datcCb.streamSeconds[connId - 1] += DATC_WDXS_DATA_RATE_TIMEOUT;
    datcCb.streamRate[connId - 1] =
        datcCb.rxCount[connId - 1] * 8 / datcCb.streamSeconds[connId - 1];

    APP_TRACE_INFO2("Streaming data rate %d bps, overall %d bps", bps,
                    datcCb.streamRate[connId - 1]);
        ...
        case APP_UI_BTN_2_SHORT:
            ...
            if (datcCb.phyMode[connId - 1] == DATC_PHY_1M) {
                ...
                DmSetPhy(connId, HCI_ALL_PHY_ALL_PREFERENCES, HCI_PHY_LE_CODED_BIT,
                         HCI_PHY_LE_CODED_BIT, HCI_PHY_OPTIONS_NONE);
            } else {
