 ...
 */
void arm_fir_f16(
    const arm_fir_instance_f16* S, const float16_t* pSrc, float16_t* pDst, uint32_t blockSize);
...
void arm_biquad_cascade_df1_mve_init_f16(arm_biquad_casd_df1_inst_f16* S, uint8_t numStages,
    const float16_t* pCoeffs, arm_biquad_mod_coef_f16* pCoeffsMod, float16_t* pState);
#endif
...
void arm_biquad_cascade_stereo_df2T_init_f16(arm_biquad_cascade_stereo_df2T_instance_f16* S,
    uint8_t numStages, const float16_t* pCoeffs, float16_t* pState);
