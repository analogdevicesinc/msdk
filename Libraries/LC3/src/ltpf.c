...
static const int16_t h_8k_12k8_q15[8 * 10] = {
    214,   417,   -1052, -4529, 26233, -4529, -1052, 417,   214,  0,    180,   0,
    -1522, -2427, 24506, -5289, 0,     763,   156,   -28,   92,   -323, -1361, 0,
    19741, -3885, 1317,  861,   0,     -61,   0,     -457,  -752, 1873, 13068, 0,
    2389,  598,   -213,  -79,   -61,   -398,  0,     2686,  5997, 5997, 2686,  0,
    -398,  -61,   -79,   -213,  598,   2389,  0,     13068, 1873, -752, -457,  0,
    -61,   0,     861,   1317,  -3885, 19741, 0,     -1361, -323, 92,   -28,   156,
    763,   0,     -5289, 24506, -2427, -1522, 0,     180,
};
...
static const int16_t h_32k_12k8_q15[2 * 40] = {
    -30,  -31,   46,    107,  0,    -199,  -162,  209,  430,   0,     -681,  -526, 658,  1343,
    0,    -2264, -1943, 2999, 9871, 13116, 9871,  2999, -1943, -2264, 0,     1343, 658,  -526,
    -681, 0,     430,   209,  -162, -199,  0,     107,  46,    -31,   -30,   0,

    -14,  -39,   0,     90,   78,   -106,  -229,  0,    382,   299,   -376,  -761, 0,    1194,
    937,  -1214, -2644, 0,    6534, 12253, 12253, 6534, 0,     -2644, -1214, 937,  1194, 0,
    -761, -376,  299,   382,  0,    -229,  -106,  78,   90,    0,     -39,   -14,
};
...
static const int16_t h_48k_12k8_q15[4 * 60] = {
    -13,  -25,  -20,  10,   51,    71,    38,    -47,  -133,  -145,  -42,   139,  277,  242,  0,
    -329, -511, -351, 144,  698,   895,   450,   -535, -1510, -1697, -521,  1999, 5138, 7737, 8744,
    7737, 5138, 1999, -521, -1697, -1510, -535,  450,  895,   698,   144,   -351, -511, -329, 0,
    242,  277,  139,  -42,  -145,  -133,  -47,   38,   71,    51,    10,    -20,  -25,  -13,  0,

    -9,   -23,  -24,  0,    41,    71,    52,    -23,  -115,  -152,  -78,   92,   254,  272,  76,
    -251, -493, -427, 0,    576,   900,   624,   -262, -1309, -1763, -954,  1272, 4356, 7203, 8679,
    8169, 5886, 2767, 0,    -1542, -1660, -809,  240,  848,   796,   292,   -252, -507, -398, -82,
    199,  288,  183,  0,    -130,  -145,  -71,   20,   69,    60,    20,    -15,  -26,  -17,  -3,

    -6,   -20,  -26,  -8,   31,    67,    62,    0,    -94,   -152,  -108,  45,   223,  287,  143,
    -167, -454, -480, -134, 439,   866,   758,   0,    -1071, -1748, -1295, 601,  3559, 6580, 8485,
    8485, 6580, 3559, 601,  -1295, -1748, -1071, 0,    758,   866,   439,   -134, -480, -454, -167,
    143,  287,  223,  45,   -108,  -152,  -94,   0,    62,    67,    31,    -8,   -26,  -20,  -6,

    -3,   -17,  -26,  -15,  20,    60,    69,    20,   -71,   -145,  -130,  0,    183,  288,  199,
    -82,  -398, -507, -252, 292,   796,   848,   240,  -809,  -1660, -1542, 0,    2767, 5886, 8169,
    8679, 7203, 4356, 1272, -954,  -1763, -1309, -262, 624,   900,   576,   0,    -427, -493, -251,
    76,   272,  254,  92,   -78,   -152,  -115,  -23,  52,    71,    41,    0,    -24,  -23,  -9,
};
 ...
 */
LC3_HOT static inline int32_t filter_hp50(struct lc3_ltpf_hp50_state *hp50, int32_t xn)
{
    int32_t yn;
 ...
 */
#if !defined(resample_8k_12k8) || !defined(resample_16k_12k8) || !defined(resample_32k_12k8)
LC3_HOT static inline void resample_x64k_12k8(const int p, const int16_t *h,
                                              struct lc3_ltpf_hp50_state *hp50, const int16_t *x,
                                              int16_t *y, int n)
{
    const int w = 2 * (40 / p);
 ...
 */
#ifndef resample_8k_12k8
LC3_HOT static void resample_8k_12k8(struct lc3_ltpf_hp50_state *hp50, const int16_t *x, int16_t *y,
                                     int n)
{
 ...
 */
#ifndef resample_16k_12k8
LC3_HOT static void resample_16k_12k8(struct lc3_ltpf_hp50_state *hp50, const int16_t *x,
                                      int16_t *y, int n)
{
 ...
 */
#ifndef resample_32k_12k8
LC3_HOT static void resample_32k_12k8(struct lc3_ltpf_hp50_state *hp50, const int16_t *x,
                                      int16_t *y, int n)
{
 ...
 */
#ifndef resample_24k_12k8
LC3_HOT static void resample_24k_12k8(struct lc3_ltpf_hp50_state *hp50, const int16_t *x,
                                      int16_t *y, int n)
{
...
*/
#ifndef resample_48k_12k8
LC3_HOT static void resample_48k_12k8(struct lc3_ltpf_hp50_state *hp50, const int16_t *x,
                                      int16_t *y, int n)
{
...
LC3_HOT static void resample_6k4(const int16_t *x, int16_t *y, int n)
{
    ...
    for (x--; y < ye; x += 2)
        *(y++) = (x[0] * h[0] + (x[-1] + x[1]) * h[1] + (x[-2] + x[2]) * h[2]) >> 16;
}
#endif /* resample_6k4 */
...
static void (*const resample_12k8[])(struct lc3_ltpf_hp50_state *, const int16_t *, int16_t *,
                                     int) = {
    [LC3_SRATE_8K] = resample_8k_12k8,   [LC3_SRATE_16K] = resample_16k_12k8,
    [LC3_SRATE_24K] = resample_24k_12k8, [LC3_SRATE_32K] = resample_32k_12k8,
 ...
 */
#ifndef correlate
LC3_HOT static void correlate(const int16_t *a, const int16_t *b, int n, float *y, int nc)
{
    for (const float *ye = y + nc; y < ye;) *(y++) = dot(a, b--, n);
}
#endif /* correlate */
 ...
 */
LC3_HOT static int argmax_weighted(const float *x, int n, float w_incr, float *x_max)
{
    int arg;
...
{
    static const int16_t h4_q15[][4] = { { 6877, 19121, 6877, 0 },
...
LC3_HOT static float interpolate_corr(const float *x, int d)
{
    static const float h4[][8] = {
        ...
        { 2.74547165e-03, 4.59833449e-03, -7.54404636e-02, 8.17488686e-01, 3.30182571e-01,
          -1.05835916e-01, 2.86823405e-02, -2.87456116e-03 },
        { -3.00125103e-03, 2.95038503e-02, -1.30305021e-01, 6.03297008e-01, 6.03297008e-01,
          -1.30305021e-01, 2.95038503e-02, -3.00125103e-03 },
        { -2.87456116e-03, 2.86823405e-02, -1.05835916e-01, 3.30182571e-01, 8.17488686e-01,
          -7.54404636e-02, 4.59833449e-03, 2.74547165e-03 },
    };

    const float *h = h4[(4 + d) % 4];

    float y = d < 0 ? x[-4] * *(h++) : d > 0 ? x[4] * *(h + 7) : 0;

    y += x[-3] * h[0] + x[-2] * h[1] + x[-1] * h[2] + x[0] * h[3] + x[1] * h[4] + x[2] * h[5] +
         x[3] * h[6];
 ...
 * The `x` vector is aligned on 32 bits
 */
static bool detect_pitch(struct lc3_ltpf_analysis *ltpf, const int16_t *x, int n, int *tc)
{
    float rm1, rm2;
    float r[98];
    ...
    float nc1 = rm1 <= 0 ? 0 : rm1 / sqrtf(dot(x, x, n) * dot(x1, x1, n));

    float nc2 = rm2 <= 0 ? 0 : rm2 / sqrtf(dot(x, x, n) * dot(x2, x2, n));
 ...
 * LTPF Analysis
 */
bool lc3_ltpf_analyse(enum lc3_dt dt, enum lc3_srate sr, struct lc3_ltpf_analysis *ltpf,
                      const int16_t *x, struct lc3_ltpf_data *data)
{
    ...
    memmove(ltpf->x_12k8, ltpf->x_12k8 + n_12k8, (z_12k8 - n_12k8) * sizeof(*ltpf->x_12k8));
    ...
    memmove(ltpf->x_6k4, ltpf->x_6k4 + n_6k4, (z_6k4 - n_6k4) * sizeof(*ltpf->x_6k4));
    ...
    if (ltpf->active) {
        int pitch_diff = LC3_MAX(pitch, ltpf->pitch) - LC3_MIN(pitch, ltpf->pitch);
        ...
        data->active = pitch_present && ((dt == LC3_DT_10M || ltpf->nc[1] > 0.94f) &&
 ...
 */
LC3_HOT static inline void synthesize_template(const float *xh, int nh, int lag, const float *x0,
                                               float *x, int n, const float *c, const int w,
                                               int fade)
{
    ...
    for (int j = 0; j < w - 1; j++) {
        ...
        for (int k = 0; k <= j; k++) u[j - k] -= yi * c[k];

        for (int k = 0; k <= j; k++) u[j - k] += xi * c[w + k];
    }
    ...
    for (int i = 0; i < n; i += w)
        for (int j = 0; j < w; j++, g += g_incr) {
            ...
            for (int k = 0; k < w; k++) u[(j + (w - 1) - k) % w] -= yi * c[k];

            for (int k = 0; k < w; k++) u[(j + (w - 1) - k) % w] += xi * c[w + k];
...
LC3_HOT static void synthesize_4(const float *xh, int nh, int lag, const float *x0, float *x, int n,
                                 const float *c, int fade)
{
    synthesize_template(xh, nh, lag, x0, x, n, c, 4, fade);
}

LC3_HOT static void synthesize_6(const float *xh, int nh, int lag, const float *x0, float *x, int n,
                                 const float *c, int fade)
{
    synthesize_template(xh, nh, lag, x0, x, n, c, 6, fade);
}

LC3_HOT static void synthesize_8(const float *xh, int nh, int lag, const float *x0, float *x, int n,
                                 const float *c, int fade)
{
    synthesize_template(xh, nh, lag, x0, x, n, c, 8, fade);
}

LC3_HOT static void synthesize_12(const float *xh, int nh, int lag, const float *x0, float *x,
                                  int n, const float *c, int fade)
{
    synthesize_template(xh, nh, lag, x0, x, n, c, 12, fade);
}

static void (*const synthesize[])(const float *, int, int, const float *, float *, int,
                                  const float *, int) = {
    [LC3_SRATE_8K] = synthesize_4,  [LC3_SRATE_16K] = synthesize_4,  [LC3_SRATE_24K] = synthesize_6,
    [LC3_SRATE_32K] = synthesize_8, [LC3_SRATE_48K] = synthesize_12,
};
...
void lc3_ltpf_synthesize(enum lc3_dt dt, enum lc3_srate sr, int nbytes, lc3_ltpf_synthesis_t *ltpf,
                         const lc3_ltpf_data_t *data, const float *xh, float *x)
{
    int nh = LC3_NH(dt, sr);
    ...
    if (!ltpf->active && active)
        ...
        synthesize[sr](xh, nh, ltpf->pitch / 4, ltpf->x, x, nt, ltpf->c, -1);
        synthesize[sr](xh, nh, pitch / 4, (x <= xh ? x + nh : x) - (w - 1), x, nt, c, 1);
    }
 ...
 * Put bitstream data
 */
void lc3_ltpf_put_data(lc3_bits_t *bits, const struct lc3_ltpf_data *data)
{
