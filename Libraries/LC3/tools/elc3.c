...
{
    static const char *usage = "Usage: %s [options] [wav_file] [out_file]\n"
                               "\n"
                               "wav_file\t"
                               ...
                               "Display help\n"
                               "\t-b\t"
                               "Bitrate in bps (mandatory)\n"
                               "\t-m\t"
                               "Frame duration in ms (default 10)\n"
                               "\t-r\t"
                               "Encoder samplerate (default is input samplerate)\n"
                               "\n";
...
int main(int argc, char *argv[])
{
    ...
    if (wave_read_header(fp_in, &pcm_sbits, &pcm_sbytes, &srate_hz, &nch, &nsamples) < 0)
    ...
    int enc_samples = !p.srate_hz ? nsamples : ((int64_t)nsamples * enc_srate_hz) / srate_hz;

    lc3bin_write_header(fp_out, frame_us, enc_srate_hz, p.bitrate, nch, enc_samples);
    ...
    for (int i = 0; i * frame_samples < encode_samples; i++) {
        ...
        memset(pcm + nread * nch, 0, nch * (frame_samples - nread) * pcm_sbytes);

        if (floorf(i * frame_us * 1e-6) > nsec) {
            float progress = fminf((float)i * frame_samples / encode_samples, 1);
        ...
        for (int ich = 0; ich < nch; ich++)
            lc3_encode(enc[ich], pcm_fmt, pcm + ich * pcm_sbytes, nch, frame_bytes, out[ich]);
    ...
    for (int ich = 0; ich < nch; ich++) free(enc[ich]);
