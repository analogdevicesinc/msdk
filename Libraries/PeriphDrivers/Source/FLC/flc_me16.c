...
//******************************************************************************
void MXC_FLC_ME16_Flash_Operation(void)
{
...
//******************************************************************************
int MXC_FLC_ME16_GetByAddress(mxc_flc_regs_t **flc, uint32_t addr)
{
...
//******************************************************************************
int MXC_FLC_ME16_GetPhysicalAddress(uint32_t addr, uint32_t *result)
{
    if ((addr >= MXC_FLASH_MEM_BASE) && (addr < (MXC_FLASH_MEM_BASE + MXC_FLASH_MEM_SIZE))) {
        *result = addr - MXC_FLASH_MEM_BASE;
    } else if ((addr >= MXC_INFO_MEM_BASE) && (addr < (MXC_INFO_MEM_BASE + MXC_INFO_MEM_SIZE))) {
        /* For ME16, the info block base was located at the next power of 2 address beyond the main flash.
           The ME16 ends at 0x5FFFF, so the info block starts at 0x80000. */
...
int MXC_FLC_PageErase(uint32_t address)
{
    ...
    // Get FLC Instance
    if ((err = MXC_FLC_ME16_GetByAddress(&flc, address)) != E_NO_ERROR) {
        return err;
    }

    if ((err = MXC_FLC_ME16_GetPhysicalAddress(address, &addr)) < E_NO_ERROR) {
        return err;
    }

    err = MXC_FLC_RevA_PageErase((mxc_flc_reva_regs_t *)flc, addr);
    // Flush the cache
    MXC_FLC_ME16_Flash_Operation();
...
int MXC_FLC_Write32(uint32_t address, uint32_t data)
{
    ...
    if ((err = MXC_FLC_ME16_GetPhysicalAddress(aligned, &addr)) < E_NO_ERROR) {
        return err;
    }
