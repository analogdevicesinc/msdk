...
#define TRC_HWTC_COUNT \
    PIT.TIMER[configTICK_PIT_CHANNEL].CVAL.R // must be the PIT channel used for the systick
...
#if !(defined(TRC_HWTC_TYPE) && defined(TRC_HWTC_COUNT) && defined(TRC_HWTC_PERIOD) && \
      defined(TRC_HWTC_FREQ_HZ) && defined(TRC_IRQ_PRIORITY_ORDER))
