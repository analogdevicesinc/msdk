...
#define isQueueReceiveHookActuallyPeek \
    (__LINE__ < 0) /* instead of pdFALSE to fix a warning of "constant condition" */
...
#define StartIndexMessageBuffer \
    (StartIndexStreamBuffer + (TRC_CFG_NSTREAMBUFFER)*PropertyTableSizeStreamBuffer)

/* Number of bytes used by the object table */
#define TRACE_OBJECT_TABLE_SIZE \
    (StartIndexMessageBuffer + (TRC_CFG_NMESSAGEBUFFER)*PropertyTableSizeMessageBuffer)
...
#define TRACE_SET_TASK_NUMBER(pxTCB) \
    prvTraceSetTaskNumberLow16(      \
        pxTCB, prvTraceGetObjectHandle(TRACE_GET_OBJECT_TRACE_CLASS(TASK, pxTCB)));
...
#define TRACE_SET_QUEUE_NUMBER(queue) \
    prvTraceSetQueueNumberLow16(      \
        queue, (uint16_t)prvTraceGetObjectHandle(TRACE_GET_OBJECT_TRACE_CLASS(QUEUE, queue)));
...
#define TRACE_SET_TIMER_NUMBER(tmr) \
    prvTraceSetTimerNumberLow16(    \
        tmr, (uint16_t)prvTraceGetObjectHandle(TRACE_GET_OBJECT_TRACE_CLASS(TIMER, tmr)));
#else /* (TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_10_0_0) */
...
#define TRACE_SET_EVENTGROUP_NUMBER(eg) \
    prvTraceSetEventGroupNumberLow16(   \
        eg, (uint16_t)prvTraceGetObjectHandle(TRACE_GET_OBJECT_TRACE_CLASS(EVENTGROUP, eg)));
#else /* (TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_10_0_0) */
...
#define traceTASK_CREATE(pxNewTCB)                                                                \
    if (pxNewTCB != NULL) {                                                                       \
        trcKERNEL_HOOKS_TASK_CREATE(                                                              \
            TRACE_GET_OBJECT_EVENT_CODE(CREATE_OBJ, TRCSUCCESS, TASK, pxNewTCB), TASK, pxNewTCB); \
    }
...
#define traceTASK_DELETE(pxTaskToDelete)                                                  \
    {                                                                                     \
        ...
        trcKERNEL_HOOKS_TASK_DELETE(                                                      \
            TRACE_GET_OBJECT_EVENT_CODE(DELETE_OBJ, TRCSUCCESS, TASK, pxTaskToDelete),    \
...
#define traceQUEUE_CREATE(pxNewQueue)                                                        \
    trcKERNEL_HOOKS_OBJECT_CREATE(TRACE_GET_OBJECT_EVENT_CODE(CREATE_OBJ, TRCSUCCESS, QUEUE, \
                                                              pxNewQueue),                   \
                                  QUEUE, pxNewQueue);
...
#define traceGIVE_MUTEX_RECURSIVE_FAILED(pxMutex)                                                \
    trcKERNEL_HOOKS_KERNEL_SERVICE(TRACE_GET_OBJECT_EVENT_CODE(SEND, TRCFAILED, QUEUE, pxMutex), \
                                   QUEUE, pxMutex);

/* Called when a message is sent to a queue */ /* CS IS NEW ! */
#undef traceQUEUE_SEND
#define traceQUEUE_SEND(pxQueue)                                                                  \
    trcKERNEL_HOOKS_KERNEL_SERVICE(TRACE_GET_OBJECT_EVENT_CODE(SEND, TRCSUCCESS, QUEUE, pxQueue), \
                                   QUEUE, pxQueue);                                               \
...
#define traceQUEUE_SEND_FAILED(pxQueue)                                                          \
    trcKERNEL_HOOKS_KERNEL_SERVICE(TRACE_GET_OBJECT_EVENT_CODE(SEND, TRCFAILED, QUEUE, pxQueue), \
                                   QUEUE, pxQueue);

/* Called when the task is blocked due to a send operation on a full queue */
#undef traceBLOCKING_ON_QUEUE_SEND
#define traceBLOCKING_ON_QUEUE_SEND(pxQueue)                                                    \
    trcKERNEL_HOOKS_KERNEL_SERVICE(TRACE_GET_OBJECT_EVENT_CODE(SEND, TRCBLOCK, QUEUE, pxQueue), \
                                   QUEUE, pxQueue);
...
#define traceQUEUE_RECEIVE(pxQueue)                                                            \
    if (isQueueReceiveHookActuallyPeek) {                                                      \
        trcKERNEL_HOOKS_KERNEL_SERVICE(                                                        \
            TRACE_GET_OBJECT_EVENT_CODE(PEEK, TRCSUCCESS, QUEUE, pxQueue), QUEUE, pxQueue);    \
    } else {                                                                                   \
        trcKERNEL_HOOKS_KERNEL_SERVICE(                                                        \
            TRACE_GET_OBJECT_EVENT_CODE(RECEIVE, TRCSUCCESS, QUEUE, pxQueue), QUEUE, pxQueue); \
    }                                                                                          \
...
#define traceQUEUE_RECEIVE_FAILED(pxQueue)                                                    \
    if (isQueueReceiveHookActuallyPeek) {                                                     \
        trcKERNEL_HOOKS_KERNEL_SERVICE(                                                       \
            TRACE_GET_OBJECT_EVENT_CODE(PEEK, TRCFAILED, QUEUE, pxQueue), QUEUE, pxQueue);    \
    } else {                                                                                  \
        trcKERNEL_HOOKS_KERNEL_SERVICE(                                                       \
            TRACE_GET_OBJECT_EVENT_CODE(RECEIVE, TRCFAILED, QUEUE, pxQueue), QUEUE, pxQueue); \
    }
...
#define traceBLOCKING_ON_QUEUE_RECEIVE(pxQueue)                                              \
    if (isQueueReceiveHookActuallyPeek) {                                                    \
        trcKERNEL_HOOKS_KERNEL_SERVICE(                                                      \
            TRACE_GET_OBJECT_EVENT_CODE(PEEK, TRCBLOCK, QUEUE, pxQueue), QUEUE, pxQueue);    \
    } else {                                                                                 \
        trcKERNEL_HOOKS_KERNEL_SERVICE(                                                      \
            TRACE_GET_OBJECT_EVENT_CODE(RECEIVE, TRCBLOCK, QUEUE, pxQueue), QUEUE, pxQueue); \
    }                                                                                        \
...
#define traceQUEUE_PEEK(pxQueue)                                                                  \
    trcKERNEL_HOOKS_KERNEL_SERVICE(TRACE_GET_OBJECT_EVENT_CODE(PEEK, TRCSUCCESS, QUEUE, pxQueue), \
                                   QUEUE, pxQueue);

/* Called on xQueuePeek fail/timeout (added in FreeRTOS v9.0.2) */
#undef traceQUEUE_PEEK_FAILED
#define traceQUEUE_PEEK_FAILED(pxQueue)                                                          \
    trcKERNEL_HOOKS_KERNEL_SERVICE(TRACE_GET_OBJECT_EVENT_CODE(PEEK, TRCFAILED, QUEUE, pxQueue), \
                                   QUEUE, pxQueue);

/* Called on xQueuePeek blocking (added in FreeRTOS v9.0.2) */
#undef traceBLOCKING_ON_QUEUE_PEEK
#define traceBLOCKING_ON_QUEUE_PEEK(pxQueue)                                                    \
    trcKERNEL_HOOKS_KERNEL_SERVICE(TRACE_GET_OBJECT_EVENT_CODE(PEEK, TRCBLOCK, QUEUE, pxQueue), \
                                   QUEUE, pxQueue);                                             \
    if (TRACE_GET_OBJECT_TRACE_CLASS(QUEUE, pxQueue) != TRACE_CLASS_MUTEX) {                    \
        trcKERNEL_HOOKS_SET_TASK_INSTANCE_FINISHED();                                           \
    }
...
#define traceQUEUE_SEND_FROM_ISR(pxQueue)                                                        \
    trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(                                                     \
        TRACE_GET_OBJECT_EVENT_CODE(SEND_FROM_ISR, TRCSUCCESS, QUEUE, pxQueue), QUEUE, pxQueue); \
...
#define traceQUEUE_SEND_FROM_ISR_FAILED(pxQueue) \
    trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(     \
        TRACE_GET_OBJECT_EVENT_CODE(SEND_FROM_ISR, TRCFAILED, QUEUE, pxQueue), QUEUE, pxQueue);
...
#define traceQUEUE_RECEIVE_FROM_ISR(pxQueue)                                              \
    trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(                                              \
        TRACE_GET_OBJECT_EVENT_CODE(RECEIVE_FROM_ISR, TRCSUCCESS, QUEUE, pxQueue), QUEUE, \
        pxQueue);                                                                         \
    trcKERNEL_HOOKS_SET_OBJECT_STATE(QUEUE, pxQueue, (uint8_t)(pxQueue->uxMessagesWaiting - 1));
...
#define traceQUEUE_RECEIVE_FROM_ISR_FAILED(pxQueue) \
    trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(        \
        TRACE_GET_OBJECT_EVENT_CODE(RECEIVE_FROM_ISR, TRCFAILED, QUEUE, pxQueue), QUEUE, pxQueue);
...
#define traceTIMER_COMMAND_SEND(tmr, xCommandID, xOptionalValue, xReturn)                       \
    if (xCommandID > tmrCOMMAND_START_DONT_TRACE) {                                             \
        if (xCommandID == tmrCOMMAND_CHANGE_PERIOD) {                                           \
            if (xReturn == pdPASS) {                                                            \
                trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(TIMER_CHANGE_PERIOD, TIMER, tmr,      \
                                                          xOptionalValue);                      \
            } else {                                                                            \
                trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(TIMER_CHANGE_PERIOD_TRCFAILED, TIMER, \
                                                          tmr, xOptionalValue);                 \
            }                                                                                   \
        } else if ((xCommandID == tmrCOMMAND_DELETE) && (xReturn == pdPASS)) {                  \
            trcKERNEL_HOOKS_OBJECT_DELETE(                                                      \
                TIMER_DELETE_OBJ,                                                               \
                EVENTGROUP_OBJCLOSE_NAME_TRCSUCCESS + TRACE_GET_OBJECT_TRACE_CLASS(TIMER, tmr), \
                EVENTGROUP_OBJCLOSE_PROP_TRCSUCCESS + TRACE_GET_OBJECT_TRACE_CLASS(TIMER, tmr), \
                TIMER, tmr);                                                                    \
        } else {                                                                                \
...
#define traceEVENT_GROUP_DELETE(eg)                                                             \
    {                                                                                           \
        ...
        trcKERNEL_HOOKS_OBJECT_DELETE(                                                          \
            ...
            EVENTGROUP_OBJCLOSE_NAME_TRCSUCCESS + TRACE_GET_OBJECT_TRACE_CLASS(EVENTGROUP, eg), \
            EVENTGROUP_OBJCLOSE_NAME_TRCSUCCESS + TRACE_GET_OBJECT_TRACE_CLASS(EVENTGROUP, eg), \
            EVENTGROUP, eg);                                                                    \
        TRACE_EXIT_CRITICAL_SECTION();                                                          \
    }
...
#define traceEVENT_GROUP_SYNC_BLOCK(eg, bitsToSet, bitsToWaitFor)                        \
    trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(EVENT_GROUP_SYNC_TRCBLOCK, EVENTGROUP, eg, \
                                              bitsToWaitFor);

#undef traceEVENT_GROUP_SYNC_END
#define traceEVENT_GROUP_SYNC_END(eg, bitsToSet, bitsToWaitFor, wasTimeout)                       \
    if (wasTimeout) {                                                                             \
        trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(EVENT_GROUP_SYNC_END_TRCFAILED, EVENTGROUP, eg, \
                                                  bitsToWaitFor);                                 \
    } else {                                                                                      \
        trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(EVENT_GROUP_SYNC_END, EVENTGROUP, eg,           \
                                                  bitsToWaitFor);                                 \
    }

#undef traceEVENT_GROUP_WAIT_BITS_BLOCK
#define traceEVENT_GROUP_WAIT_BITS_BLOCK(eg, bitsToWaitFor)                                   \
    trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(EVENT_GROUP_WAIT_BITS_TRCBLOCK, EVENTGROUP, eg, \
                                              bitsToWaitFor);                                 \
    trcKERNEL_HOOKS_SET_TASK_INSTANCE_FINISHED();
...
#define traceEVENT_GROUP_WAIT_BITS_END(eg, bitsToWaitFor, wasTimeout)                              \
    if (wasTimeout) {                                                                              \
        trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(EVENT_GROUP_WAIT_BITS_END_TRCFAILED, EVENTGROUP, \
                                                  eg, bitsToWaitFor);                              \
    } else {                                                                                       \
        trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(EVENT_GROUP_WAIT_BITS_END, EVENTGROUP, eg,       \
                                                  bitsToWaitFor);                                  \
    }
...
#define traceEVENT_GROUP_SET_BITS_FROM_ISR(eg, bitsToSet)                                         \
    trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM_FROM_ISR(EVENT_GROUP_SET_BITS_FROM_ISR, EVENTGROUP, \
                                                       eg, bitsToSet);                            \
...
#define traceTASK_NOTIFY_TAKE()                                                               \
    if (pxCurrentTCB->eNotifyState == eNotified) {                                            \
        trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(TRACE_TASK_NOTIFY_TAKE, TASK, pxCurrentTCB, \
                                                  xTicksToWait);                              \
    } else {                                                                                  \
        trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(TRACE_TASK_NOTIFY_TAKE_TRCFAILED, TASK,     \
...
#define traceTASK_NOTIFY_TAKE_BLOCK()                                                              \
    trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(TRACE_TASK_NOTIFY_TAKE_TRCBLOCK, TASK, pxCurrentTCB, \
                                              xTicksToWait);                                       \
    trcKERNEL_HOOKS_SET_TASK_INSTANCE_FINISHED();
...
#define traceTASK_NOTIFY_WAIT()                                                                  \
    if (TRACE_GET_OBJECT_FILTER(TASK, pxCurrentTCB) & CurrentFilterMask) {                       \
        if (pxCurrentTCB->eNotifyState == eNotified)                                             \
            prvTraceStoreKernelCallWithParam(TRACE_TASK_NOTIFY_WAIT, TRACE_CLASS_TASK,           \
...
#define traceSTREAM_BUFFER_CREATE(pxStreamBuffer, xIsMessageBuffer)                          \
    trcKERNEL_HOOKS_OBJECT_CREATE(TRACE_GET_OBJECT_EVENT_CODE(CREATE_OBJ, TRCSUCCESS,        \
...
#define traceSTREAM_BUFFER_DELETE(xStreamBuffer)                                             \
    trcKERNEL_HOOKS_OBJECT_DELETE(                                                           \
        TRACE_GET_OBJECT_EVENT_CODE(DELETE_OBJ, TRCSUCCESS, STREAMBUFFER, xStreamBuffer),    \
...
#define traceSTREAM_BUFFER_SEND(xStreamBuffer, xReturn)                                        \
    trcKERNEL_HOOKS_KERNEL_SERVICE(TRACE_GET_OBJECT_EVENT_CODE(SEND, TRCSUCCESS, STREAMBUFFER, \
                                                               xStreamBuffer),                 \
                                   STREAMBUFFER, xStreamBuffer);                               \
...
#define traceBLOCKING_ON_STREAM_BUFFER_SEND(xStreamBuffer)                                   \
    trcKERNEL_HOOKS_KERNEL_SERVICE(TRACE_GET_OBJECT_EVENT_CODE(SEND, TRCBLOCK, STREAMBUFFER, \
                                                               xStreamBuffer),               \
                                   STREAMBUFFER, xStreamBuffer);

#undef traceSTREAM_BUFFER_SEND_FAILED
#define traceSTREAM_BUFFER_SEND_FAILED(xStreamBuffer)                                         \
    trcKERNEL_HOOKS_KERNEL_SERVICE(TRACE_GET_OBJECT_EVENT_CODE(SEND, TRCFAILED, STREAMBUFFER, \
                                                               xStreamBuffer),                \
                                   STREAMBUFFER, xStreamBuffer);

#undef traceSTREAM_BUFFER_RECEIVE
#define traceSTREAM_BUFFER_RECEIVE(xStreamBuffer, xReceivedLength)                                \
    trcKERNEL_HOOKS_KERNEL_SERVICE(TRACE_GET_OBJECT_EVENT_CODE(RECEIVE, TRCSUCCESS, STREAMBUFFER, \
                                                               xStreamBuffer),                    \
                                   STREAMBUFFER, xStreamBuffer);                                  \
    trcKERNEL_HOOKS_SET_OBJECT_STATE(STREAMBUFFER, xStreamBuffer, prvBytesInBuffer(xStreamBuffer));
...
#define traceBLOCKING_ON_STREAM_BUFFER_RECEIVE(xStreamBuffer)                                   \
    trcKERNEL_HOOKS_KERNEL_SERVICE(TRACE_GET_OBJECT_EVENT_CODE(RECEIVE, TRCBLOCK, STREAMBUFFER, \
                                                               xStreamBuffer),                  \
                                   STREAMBUFFER, xStreamBuffer);

#undef traceSTREAM_BUFFER_RECEIVE_FAILED
#define traceSTREAM_BUFFER_RECEIVE_FAILED(xStreamBuffer)                                         \
    trcKERNEL_HOOKS_KERNEL_SERVICE(TRACE_GET_OBJECT_EVENT_CODE(RECEIVE, TRCFAILED, STREAMBUFFER, \
                                                               xStreamBuffer),                   \
                                   STREAMBUFFER, xStreamBuffer);
...
#define traceSTREAM_BUFFER_SEND_FROM_ISR(xStreamBuffer, xReturn)                                 \
    if (xReturn > (size_t)0) {                                                                   \
        trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(                                                 \
            TRACE_GET_OBJECT_EVENT_CODE(SEND_FROM_ISR, TRCSUCCESS, STREAMBUFFER, xStreamBuffer), \
            STREAMBUFFER, xStreamBuffer);                                                        \
        trcKERNEL_HOOKS_SET_OBJECT_STATE(STREAMBUFFER, xStreamBuffer,                            \
                                         prvBytesInBuffer(xStreamBuffer));                       \
    } else {                                                                                     \
        trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(                                                 \
            TRACE_GET_OBJECT_EVENT_CODE(SEND_FROM_ISR, TRCFAILED, STREAMBUFFER, xStreamBuffer),  \
            STREAMBUFFER, xStreamBuffer);                                                        \
    }
...
#define traceSTREAM_BUFFER_RECEIVE_FROM_ISR(xStreamBuffer, xReceivedLength)                        \
    if (xReceivedLength > (size_t)0) {                                                             \
        trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(                                                   \
            TRACE_GET_OBJECT_EVENT_CODE(RECEIVE_FROM_ISR, TRCSUCCESS, STREAMBUFFER,                \
                                        xStreamBuffer),                                            \
            STREAMBUFFER, xStreamBuffer);                                                          \
        trcKERNEL_HOOKS_SET_OBJECT_STATE(STREAMBUFFER, xStreamBuffer,                              \
                                         prvBytesInBuffer(xStreamBuffer));                         \
        ...
        trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(                                                   \
            TRACE_GET_OBJECT_EVENT_CODE(RECEIVE_FROM_ISR, TRCFAILED, STREAMBUFFER, xStreamBuffer), \
            STREAMBUFFER, xStreamBuffer);                                                          \
    }
...
#define traceCREATE_COUNTING_SEMAPHORE()                                                 \
    ...
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)     \
        if (TRACE_GET_OBJECT_FILTER(QUEUE, pxHandle) & CurrentFilterMask)                \
            prvTraceStoreEvent2(PSF_EVENT_SEMAPHORE_COUNTING_CREATE, (uint32_t)pxHandle, \
                                uxCountValue);
#endif /* TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_8_X */
...
#define traceQUEUE_SEND_FAILED(pxQueue)                                                 \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)    \
        if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)                \
            ...
            case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                      \
            case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                                    \
                prvTraceStoreEvent2(PSF_EVENT_SEMAPHORE_GIVE_FAILED, (uint32_t)pxQueue, \
...
#define traceBLOCKING_ON_QUEUE_SEND(pxQueue)                                           \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)   \
        if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)               \
            ...
            case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                     \
            case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                                   \
                prvTraceStoreEvent2(PSF_EVENT_SEMAPHORE_GIVE_BLOCK, (uint32_t)pxQueue, \
                                    pxQueue->uxMessagesWaiting);                       \
                break;                                                                 \
            case queueQUEUE_TYPE_MUTEX:                                                \
            case queueQUEUE_TYPE_RECURSIVE_MUTEX:                                      \
...
#define traceQUEUE_SEND_FROM_ISR(pxQueue)                                            \
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)                 \
        ...
        case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                       \
        case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                                     \
            prvTraceStoreEvent2(PSF_EVENT_SEMAPHORE_GIVE_FROMISR, (uint32_t)pxQueue, \
                                pxQueue->uxMessagesWaiting + 1);                     \
            break;                                                                   \
        }
...
#define traceQUEUE_SEND_FROM_ISR_FAILED(pxQueue)                                            \
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)                        \
        ...
        case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                              \
        case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                                            \
            prvTraceStoreEvent2(PSF_EVENT_SEMAPHORE_GIVE_FROMISR_FAILED, (uint32_t)pxQueue, \
                                pxQueue->uxMessagesWaiting);                                \
            break;                                                                          \
        }
...
#define traceQUEUE_PEEK_FAILED(pxQueue)                                                            \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)               \
        if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)                           \
            ...
            case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                                 \
            case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                                               \
                prvTraceStoreEvent3(PSF_EVENT_SEMAPHORE_PEEK_FAILED, (uint32_t)pxQueue,            \
...
#define traceBLOCKING_ON_QUEUE_PEEK(pxQueue)                                                      \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)              \
        if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)                          \
            ...
            case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                                \
            case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                                              \
                prvTraceStoreEvent3(PSF_EVENT_SEMAPHORE_PEEK_BLOCK, (uint32_t)pxQueue,            \
                                    xTicksToWait, pxQueue->uxMessagesWaiting);                    \
                break;                                                                            \
            case queueQUEUE_TYPE_MUTEX:                                                           \
            case queueQUEUE_TYPE_RECURSIVE_MUTEX:                                                 \
...
#define traceQUEUE_RECEIVE_FROM_ISR(pxQueue)                                         \
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)                 \
        switch (pxQueue->ucQueueType) {                                              \
        case queueQUEUE_TYPE_BASE:                                                   \
            prvTraceStoreEvent2(PSF_EVENT_QUEUE_RECEIVE_FROMISR, (uint32_t)pxQueue,  \
                                pxQueue->uxMessagesWaiting - 1);                     \
            break;                                                                   \
        case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                       \
        case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                                     \
            prvTraceStoreEvent2(PSF_EVENT_SEMAPHORE_TAKE_FROMISR, (uint32_t)pxQueue, \
                                pxQueue->uxMessagesWaiting - 1);                     \
            break;                                                                   \
        }
...
#define traceQUEUE_RECEIVE_FROM_ISR_FAILED(pxQueue)                                         \
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)                        \
        switch (pxQueue->ucQueueType) {                                                     \
        case queueQUEUE_TYPE_BASE:                                                          \
            prvTraceStoreEvent2(PSF_EVENT_QUEUE_RECEIVE_FROMISR_FAILED, (uint32_t)pxQueue,  \
                                pxQueue->uxMessagesWaiting);                                \
            break;                                                                          \
        case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                              \
        case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                                            \
            prvTraceStoreEvent2(PSF_EVENT_SEMAPHORE_TAKE_FROMISR_FAILED, (uint32_t)pxQueue, \
                                pxQueue->uxMessagesWaiting);                                \
            break;                                                                          \
        }
...
#define traceTIMER_COMMAND_SEND_8_0_CASES(tmr)                                                 \
    ...
    case tmrCOMMAND_STOP_FROM_ISR:                                                             \
        prvTraceStoreEvent2((xReturn == pdPASS) ? PSF_EVENT_TIMER_STOP_FROMISR :               \
    ...
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)           \
        if (TRACE_GET_OBJECT_FILTER(TIMER, tmr) & CurrentFilterMask)                           \
            ...
            case tmrCOMMAND_START:                                                             \
                prvTraceStoreEvent1((xReturn == pdPASS) ? PSF_EVENT_TIMER_START :              \
                                                          PSF_EVENT_TIMER_START_FAILED,        \
                                    (uint32_t)tmr);                                            \
                break;                                                                         \
            case tmrCOMMAND_STOP:                                                              \
                prvTraceStoreEvent1((xReturn == pdPASS) ? PSF_EVENT_TIMER_STOP :               \
            ...
            case tmrCOMMAND_DELETE:                                                            \
                prvTraceStoreEvent1((xReturn == pdPASS) ? PSF_EVENT_TIMER_DELETE :             \
...
#define traceTIMER_EXPIRED(tmr)                                                             \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)        \
        if (TRACE_GET_OBJECT_FILTER(TIMER, tmr) & CurrentFilterMask)                        \
            prvTraceStoreEvent2(PSF_EVENT_TIMER_EXPIRED, (uint32_t)tmr->pxCallbackFunction, \
...
#define traceEVENT_GROUP_SYNC_END(eg, bitsToSet, bitsToWaitFor, wasTimeout)                \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)       \
        if (TRACE_GET_OBJECT_FILTER(EVENTGROUP, eg) & CurrentFilterMask)                   \
            prvTraceStoreEvent2((wasTimeout != pdTRUE) ? PSF_EVENT_EVENTGROUP_SYNC :       \
...
#define traceTASK_NOTIFY_TAKE()                                                                    \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask) {             \
        if (pxCurrentTCB->ucNotifyState == taskNOTIFICATION_RECEIVED)                              \
            ...
            prvTraceStoreEvent2(PSF_EVENT_TASK_NOTIFY_TAKE_FAILED, (uint32_t)pxCurrentTCB,         \
                                xTicksToWait);                                                     \
    }
#else /* TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_9_0_0 */
#define traceTASK_NOTIFY_TAKE()                                                                    \
...
#define traceTASK_NOTIFY_WAIT()                                                                    \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask) {             \
        if (pxCurrentTCB->ucNotifyState == taskNOTIFICATION_RECEIVED)                              \
            ...
            prvTraceStoreEvent2(PSF_EVENT_TASK_NOTIFY_WAIT_FAILED, (uint32_t)pxCurrentTCB,         \
                                xTicksToWait);                                                     \
    }
#else /* TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_9_0_0 */
#define traceTASK_NOTIFY_WAIT()                                                                    \
...
#define traceSTREAM_BUFFER_CREATE(pxStreamBuffer, xIsMessageBuffer)                      \
    ...
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)     \
        if (TRACE_GET_OBJECT_FILTER(STREAMBUFFER, pxStreamBuffer) & CurrentFilterMask)   \
            prvTraceStoreEvent2(xIsMessageBuffer == 1 ? PSF_EVENT_MESSAGEBUFFER_CREATE : \
