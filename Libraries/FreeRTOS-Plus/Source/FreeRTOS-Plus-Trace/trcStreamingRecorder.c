...
#define SYMBOL_TABLE_SLOT_SIZE                                                     \
    (sizeof(uint32_t) +                                                            \
     (((TRC_CFG_SYMBOL_MAX_LENGTH) + (sizeof(uint32_t) - 1)) / sizeof(uint32_t)) * \
         sizeof(uint32_t))
...
#define PSF_ASSERT(_assert, _err) \
    if (!(_assert)) {             \
        prvTraceError(_err);      \
        return;                   \
    }
...
static void prvTraceStoreStringEventHelper(int nArgs, uint16_t eventID, traceString userEvtChannel,
                                           int len, const char *str, va_list *vl);

/* Not static to avoid warnings from SysGCC/PPC */
void prvTraceStoreSimpleStringEventHelper(traceString userEvtChannel, const char *str);
...
void vTracePrintF(traceString chn, const char *fmt, ...)
{
    ...
    if (chn != NULL) {
        prvTraceStoreStringEventHelper(nArgs, (uint16_t)(PSF_EVENT_USER_EVENT + nArgs + 1), chn, i,
                                       fmt, &vl);
    } else {
        prvTraceStoreStringEventHelper(nArgs, (uint16_t)(PSF_EVENT_USER_EVENT + nArgs), chn, i, fmt,
                                       &vl);
    }
    va_end(vl);
}
...
static void prvSetRecorderEnabled(uint32_t isEnabled)
{
    ...
    if (RecorderEnabled) {
        ...
        prvTraceStoreEvent3(PSF_EVENT_TRACE_START, (uint32_t)TRACE_GET_OS_TICKS(),
        ...
        for (i = 0; i < (sizeof(ObjectDataTable) / sizeof(uint32_t));
            ...
            if (data != NULL) {
                for (j = 0; j < (OBJECT_DATA_SLOT_SIZE / sizeof(uint32_t)); j++) {
                    data[j] =
                        objectDataTable.ObjectDataTableBuffer.pObjectDataTableBufferUINT32[i + j];
                }
...
void prvTraceSaveSymbol(const void *address, const char *name)
{
    ...
    if (foundSlot < SYMBOL_TABLE_BUFFER_SIZE) {
        /* We access the symbol table via the union member pSymbolTableBufferUINT32 to avoid strict-aliasing issues */
        symbolTable.SymbolTableBuffer.pSymbolTableBufferUINT32[foundSlot / sizeof(uint32_t)] =
            (uint32_t)address;

        /* We access the symbol table via the union member pSymbolTableBufferUINT8 to avoid strict-aliasing issues */
        ptrSymbol =
            &symbolTable.SymbolTableBuffer.pSymbolTableBufferUINT8[foundSlot + sizeof(uint32_t)];
        for (i = 0; i < (TRC_CFG_SYMBOL_MAX_LENGTH); i++) {
            ptrSymbol[i] = (uint8_t)
                name[i]; /* We do this first to ensure we also get the 0 termination, if there is one */
            ...
            if (firstFreeSymbolTableIndex > SYMBOL_TABLE_SLOT_SIZE &&
                ...
                lastEntryPtr =
                    &symbolTable.SymbolTableBuffer.pSymbolTableBufferUINT32
                         [(firstFreeSymbolTableIndex - SYMBOL_TABLE_SLOT_SIZE) / sizeof(uint32_t)];
...
void prvTraceSaveObjectData(const void *address, uint32_t data)
{
    ...
    if (foundSlot < OBJECT_DATA_TABLE_BUFFER_SIZE) {
        ...
        objectDataTable.ObjectDataTableBuffer
            .pObjectDataTableBufferUINT32[foundSlot / sizeof(uint32_t)] = (uint32_t)address;
        objectDataTable.ObjectDataTableBuffer
...
{
    uint16_t checksum = (uint16_t)(0xFFFF - (cmd->cmdCode + cmd->param1 + cmd->param2 +
...
static void prvTraceStoreTSConfig(void)
{
    ...
    prvTraceStoreEvent(5, PSF_EVENT_TS_CONFIG, (uint32_t)timestampFrequency,
                       (uint32_t)(TRACE_TICK_RATE_HZ), (uint32_t)(TRC_HWTC_TYPE),
                       (uint32_t)(TRC_CFG_ISR_TAILCHAINING_THRESHOLD), (uint32_t)(TRC_HWTC_PERIOD));

#else

    prvTraceStoreEvent(4, PSF_EVENT_TS_CONFIG, (uint32_t)timestampFrequency,
                       (uint32_t)(TRACE_TICK_RATE_HZ), (uint32_t)(TRC_HWTC_TYPE),
    ...
    if (pageToTransfer > -1) {
        while (1) /* Keep going until we have transferred all that we intended to */
        {
            if (TRC_STREAM_PORT_WRITE_DATA(
                    &EventBuffer[pageToTransfer * (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_SIZE) +
