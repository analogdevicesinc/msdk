...
volatile int32_t tz_host_command_bytes_to_read =
    0; // This is set by the Tracealyzer host application (to the number of bytes written), after having written to tz_host_commands. Set to zero by the read function after the message in tz_host_commands has been read.
...
static void itm_write_32(uint32_t data)
{
    ...
    {
        while (ITM->PORT[TRC_CFG_ITM_PORT].u32 == 0) {}
        // Block until room in ITM FIFO - This stream port is always in "blocking mode", since intended for high-speed ITM!
