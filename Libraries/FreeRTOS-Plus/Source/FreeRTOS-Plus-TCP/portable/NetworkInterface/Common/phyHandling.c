...
#define phyADVERTISE_ALL \
    (phyADVERTISE_10HALF | phyADVERTISE_10FULL | phyADVERTISE_100HALF | phyADVERTISE_100FULL)
...
void vPhyInitialise(EthernetPhy_t *pxPhyObject, xApplicationPhyReadHook_t fnPhyRead,
...
BaseType_t xPhyStartAutoNegotiation(EthernetPhy_t *pxPhyObject, uint32_t ulPhyMask)
{
    ...
    for (xPhyIndex = 0; xPhyIndex < pxPhyObject->xPortCount; xPhyIndex++) {
        if ((ulPhyMask & (1lu << xPhyIndex)) != 0lu) {
            ...
            pxPhyObject->fnPhyWrite(xPhyAddress, phyREG_00_BMCR,
                                    pxPhyObject->ulBCRValue | phyBMCR_AN_RESTART);
        }
    }
...
BaseType_t xPhyCheckLinkStatus(EthernetPhy_t *pxPhyObject, BaseType_t xHadReception)
{
    ...
    if (xHadReception > 0) {
        ...
        pxPhyObject->xLinkStatusRemaining = pdMS_TO_TICKS(ipconfigPHY_LS_HIGH_CHECK_TIME_MS);
                                    ...
                                    &(pxPhyObject->xLinkStatusRemaining)) != pdFALSE) {
        for (xPhyIndex = 0; xPhyIndex < pxPhyObject->xPortCount; xPhyIndex++, ulBitMask <<= 1) {
            BaseType_t xPhyAddress = pxPhyObject->ucPhyIndexes[xPhyIndex];
            ...
            if (pxPhyObject->fnPhyRead(xPhyAddress, phyREG_01_BMSR, &ulStatus) == 0) {
                if (!!(pxPhyObject->ulLinkStatusMask & ulBitMask) !=
                    ...
                    FreeRTOS_printf(
                        ("xPhyCheckLinkStatus: PHY LS now %02lX\n", pxPhyObject->ulLinkStatusMask));
