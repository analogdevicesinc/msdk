...
void setup_isr(xemacpsif_s *xemacpsif)
{
	 ...
	 * Setup callbacks
	 */
    XEmacPs_SetHandler(&xemacpsif->emacps, XEMACPS_HANDLER_DMASEND, (void *)emacps_send_handler,
                       (void *)xemacpsif);

    XEmacPs_SetHandler(&xemacpsif->emacps, XEMACPS_HANDLER_DMARECV, (void *)emacps_recv_handler,
                       (void *)xemacpsif);

    XEmacPs_SetHandler(&xemacpsif->emacps, XEMACPS_HANDLER_ERROR, (void *)emacps_error_handler,
                       (void *)xemacpsif);
}
...
int emacps_check_errors(xemacpsif_s *xemacps)
{
    ...
    if (xErrorHead == xErrorTail) {
        ...
        xResult = 1;
        emacps_handle_error(xErrorList[xErrorTail].arg, xErrorList[xErrorTail].Direction,
