...
{
    int i;
    for (i = 0; i < F25519_SIZE; i++) x[i] = a[i];
}
...
static void xc_diffadd(byte *x5, byte *z5, const byte *x1, const byte *z1, const byte *x2,
                       const byte *z2, const byte *x3, const byte *z3)
{
...
void fe_mul__distinct(byte *r, const byte *a, const byte *b)
{
    ...
    for (i = 0; i < F25519_SIZE; i++) {
        ...
        c >>= 8;
        for (j = 0; j <= i; j++) c += ((word32)a[j]) * ((word32)b[i - j]);

        for (; j < F25519_SIZE; j++) c += ((word32)a[j]) * ((word32)b[i + F25519_SIZE - j]) * 38;
