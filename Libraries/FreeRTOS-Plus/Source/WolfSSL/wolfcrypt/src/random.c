...
int wc_RNG_HealthTest(int reseed, const byte *entropyA, word32 entropyASz, const byte *entropyB,
                      word32 entropyBSz, byte *output, word32 outputSz)
{
    return RNG_HealthTest_fips(reseed, entropyA, entropyASz, entropyB, entropyBSz, output,
                               outputSz);
}
...
#include <wincrypt.h>
#else
#if !defined(NO_DEV_RANDOM) && !defined(CUSTOM_RAND_GENERATE) && !defined(WOLFSSL_MDK_ARM) && \
...
enum { drbgInitC = 0, drbgReseed = 1, drbgGenerateW = 2, drbgGenerateH = 3, drbgInitV };
...
static int Hash_df(DRBG *drbg, byte *out, word32 outSz, byte type, const byte *inA, word32 inASz,
...
static int Hash_DRBG_Reseed(DRBG *drbg, const byte *entropy, word32 entropySz)
{
    ...
    if (Hash_df(drbg, seed, sizeof(seed), drbgReseed, drbg->V, sizeof(drbg->V), entropy,
                entropySz) != DRBG_SUCCESS) {
        return DRBG_FAILURE;
    }
    ...
    if (Hash_df(drbg, drbg->C, sizeof(drbg->C), drbgInitC, drbg->V, sizeof(drbg->V), NULL, 0) !=
        DRBG_SUCCESS) {
        return DRBG_FAILURE;
    }
...
static int Hash_gen(DRBG *drbg, byte *out, word32 outSz, const byte *V)
{
    ...
    for (i = 0; i < len; i++) {
        if (wc_InitSha256(&sha) != 0 || wc_Sha256Update(&sha, data, sizeof(data)) != 0 ||
...
static int Hash_DRBG_Generate(DRBG *drbg, byte *out, word32 outSz)
{
    ...
    if (drbg->reseedCtr != RESEED_INTERVAL) {
        ...
        if (ret == DRBG_SUCCESS) {
            if (wc_InitSha256(&sha) != 0 || wc_Sha256Update(&sha, &type, sizeof(type)) != 0 ||
...
#endif
                array_add(drbg->V, sizeof(drbg->V), (byte *)&reseedCtr, sizeof(reseedCtr));
                ret = DRBG_SUCCESS;
            }
            drbg->reseedCtr++;
        }
...
/* Returns: DRBG_SUCCESS or DRBG_FAILURE */
static int Hash_DRBG_Instantiate(DRBG *drbg, const byte *seed, word32 seedSz, const byte *nonce,
                                 word32 nonceSz)
{
    ...
    if (Hash_df(drbg, drbg->V, sizeof(drbg->V), drbgInitV, seed, seedSz, nonce, nonceSz) ==
            DRBG_SUCCESS &&
        Hash_df(drbg, drbg->C, sizeof(drbg->C), drbgInitC, drbg->V, sizeof(drbg->V), NULL, 0) ==
            DRBG_SUCCESS) {
        drbg->reseedCtr = 1;
        drbg->lastBlock = 0;
        drbg->matchCount = 0;
...
static int Hash_DRBG_Uninstantiate(DRBG *drbg)
{
    ...
    for (i = 0; i < sizeof(DRBG); i++) compareSum |= compareDrbg[i] ^ 0;
    ...
    if (rng != NULL) {
        if (wc_RNG_HealthTestLocal(0) == 0) {
             ...
             * size. */
                     ...
                     Hash_DRBG_Instantiate(rng->drbg, entropy, ENTROPY_NONCE_SZ, NULL, 0) ==
                         DRBG_SUCCESS) {
...
const byte entropyA[] = { 0x63, 0x36, 0x33, 0x77, 0xe4, 0x1e, 0x86, 0x46, 0x8d, 0xeb, 0x0a, 0xb4,
                          0xa8, 0xed, 0x68, 0x3f, 0x6a, 0x13, 0x4e, 0x47, 0xe0, 0x14, 0xc7, 0x00,
                          0x45, 0x4e, 0x81, 0xe9, 0x53, 0x58, 0xa5, 0x69, 0x80, 0x8a, 0xa3, 0x8f,
                          0x2a, 0x72, 0xa6, 0x23, 0x59, 0x91, 0x5a, 0x9f, 0x8a, 0x04, 0xca, 0x68 };

const byte reseedEntropyA[] = { 0xe6, 0x2b, 0x8a, 0x8e, 0xe8, 0xf1, 0x41, 0xb6, 0x98, 0x05, 0x66,
...
const byte entropyB[] = { 0xa6, 0x5a, 0xd0, 0xf3, 0x45, 0xdb, 0x4e, 0x0e, 0xff, 0xe8, 0x75, 0xc3,
...
static int wc_RNG_HealthTestLocal(int reseed)
{
    ...
    if (reseed) {
        ret = wc_RNG_HealthTest(1, entropyA, sizeof(entropyA), reseedEntropyA,
                                sizeof(reseedEntropyA), check, sizeof(check));
        if (ret == 0) {
            if (ConstantCompare(check, outputA, sizeof(check)) != 0)
                ret = -1;
        }
    } else {
        ret = wc_RNG_HealthTest(0, entropyB, sizeof(entropyB), NULL, 0, check, sizeof(check));
        if (ret == 0) {
...
static void CaviumRNG_GenerateBlock(RNG *rng, byte *output, word32 sz)
{
    ...
    while (sz > WOLFSSL_MAX_16BIT) {
        ...
        if (CspRandom(CAVIUM_BLOCKING, slen, output + offset, &requestId, rng->devId) != 0) {
...
static int wc_GenerateSeed_IntelRD(OS_Seed *os, byte *output, word32 sz)
{
    ...
    for (; sz / 4 > 0; sz -= 4, output += 4) {
        if (IS_INTEL_RDSEED)
            ret = IntelRDseed32_r((word32 *)output);
        else
            return 1;
        if (ret)
            return 1;
...
{
    if (!CryptAcquireContext(&os->handle, 0, 0, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT))
        return WINCRYPT_E;
...
#elif defined(WOLFSSL_SAFERTOS) || defined(WOLFSSL_LEANPSK) || defined(WOLFSSL_IAR_ARM) || \
    defined(WOLFSSL_MDK_ARM) || defined(WOLFSSL_uITRON4) || defined(WOLFSSL_uTKERNEL2)
...
int wc_GenerateSeed(OS_Seed *os, byte *output, word32 sz)
{
    ...
    for (i = 0; i < sz; i++) output[i] = CUSTOM_RAND_GENERATE();
