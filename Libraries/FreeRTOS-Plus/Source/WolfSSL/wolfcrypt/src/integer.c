...
int mp_init(mp_int *a)
{
    ...
    a->dp = OPT_CAST(mp_digit) XMALLOC(sizeof(mp_digit) * MP_PREC, 0, DYNAMIC_TYPE_BIGINT);
    if (a->dp == NULL) {
        return MP_MEM;
    }
...
int mp_grow(mp_int *a, int size)
{
     ...
     */
        tmp = OPT_CAST(mp_digit) XREALLOC(a->dp, sizeof(mp_digit) * size, 0, DYNAMIC_TYPE_BIGINT);
        if (tmp == NULL) {
...
#ifdef WOLFSSL_SMALL_STACK
    M = (mp_int *)XMALLOC(sizeof(mp_int) * TAB_SIZE, NULL, DYNAMIC_TYPE_TMP_BUFFER);
    if (M == NULL)
        return MP_MEM;
#endif
...
int mp_init_size(mp_int *a, int size)
{
    ...
    /* alloc mem */
    a->dp = OPT_CAST(mp_digit) XMALLOC(sizeof(mp_digit) * size, 0, DYNAMIC_TYPE_BIGINT);
    if (a->dp == NULL) {
        return MP_MEM;
    }
