...
static const word64 blake2b_IV[8] = { 0x6a09e667f3bcc908ULL, 0xbb67ae8584caa73bULL,
...
static INLINE int blake2b_increment_counter(blake2b_state *S, const word64 inc)
{
    S->t[0] += inc;
    S->t[1] += (S->t[0] < inc);
    return 0;
...
static int blake2b_compress(blake2b_state *S, const byte block[BLAKE2B_BLOCKBYTES])
{
    int i;
...
int main(int argc, char **argv)
{
    ...
    for (word32 i = 0; i < BLAKE2B_KEYBYTES; ++i) key[i] = (byte)i;

    for (word32 i = 0; i < KAT_LENGTH; ++i) buf[i] = (byte)i;
