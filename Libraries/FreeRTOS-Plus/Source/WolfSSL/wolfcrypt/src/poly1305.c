...
{
    return (((word64)(p[0] & 0xff)) | ((word64)(p[1] & 0xff) << 8) | ((word64)(p[2] & 0xff) << 16) |
            ((word64)(p[3] & 0xff) << 24) | ((word64)(p[4] & 0xff) << 32) |
            ((word64)(p[5] & 0xff) << 40) | ((word64)(p[6] & 0xff) << 48) |
            ((word64)(p[7] & 0xff) << 56));
}
...
{
    return (((word32)(p[0] & 0xff)) | ((word32)(p[1] & 0xff) << 8) | ((word32)(p[2] & 0xff) << 16) |
            ((word32)(p[3] & 0xff) << 24));
}
...
static void poly1305_blocks(Poly1305 *ctx, const unsigned char *m, size_t bytes)
{
    ...
    while (bytes >= POLY1305_BLOCK_SIZE) {
        ...
        /* h *= r */
        d0 = ((word64)h0 * r0) + ((word64)h1 * s4) + ((word64)h2 * s3) + ((word64)h3 * s2) +
             ((word64)h4 * s1);
        d1 = ((word64)h0 * r1) + ((word64)h1 * r0) + ((word64)h2 * s4) + ((word64)h3 * s3) +
             ((word64)h4 * s2);
        d2 = ((word64)h0 * r2) + ((word64)h1 * r1) + ((word64)h2 * r0) + ((word64)h3 * s4) +
             ((word64)h4 * s3);
        d3 = ((word64)h0 * r3) + ((word64)h1 * r2) + ((word64)h2 * r1) + ((word64)h3 * r0) +
             ((word64)h4 * s4);
        d4 = ((word64)h0 * r4) + ((word64)h1 * r3) + ((word64)h2 * r2) + ((word64)h3 * r1) +
             ((word64)h4 * r0);
...
int wc_Poly1305Final(Poly1305 *ctx, byte *mac)
{
    ...
    if (ctx->leftover) {
        ...
        ctx->buffer[i] = 1;
        for (i = i + 1; i < POLY1305_BLOCK_SIZE; i++) ctx->buffer[i] = 0;
        ctx->final = 1;
    ...
    if (ctx->leftover) {
        size_t i = ctx->leftover;
        ctx->buffer[i++] = 1;
        for (; i < POLY1305_BLOCK_SIZE; i++) ctx->buffer[i] = 0;
        ctx->final = 1;
        poly1305_blocks(ctx, ctx->buffer, POLY1305_BLOCK_SIZE);
    }
...
int wc_Poly1305Update(Poly1305 *ctx, const byte *m, word32 bytes)
{
    ...
    if (ctx->leftover) {
        ...
        if (want > bytes)
            want = bytes;
        for (i = 0; i < want; i++) ctx->buffer[ctx->leftover + i] = m[i];
        bytes -= want;
        m += want;
        ctx->leftover += want;
    ...
    /* store leftover */
    if (bytes) {
        for (i = 0; i < bytes; i++) ctx->buffer[ctx->leftover + i] = m[i];
        ctx->leftover += bytes;
    }
    return 0;
}
