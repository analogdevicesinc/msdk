...
static void Hc128_SetIV(HC128 *ctx, const byte *inIv)
{
    ...
    for (i = 0; i < (128 >> 5); i++) ctx->iv[i] = LITTLE32(iv[i]);
...
static INLINE int DoKey(HC128 *ctx, const byte *key, const byte *iv)
{
    ...
    for (i = 0; i < (128 >> 5); i++) ctx->key[i] = LITTLE32(((word32 *)key)[i]);
...
static INLINE int DoProcess(HC128 *ctx, byte *output, const byte *input, word32 msglen)
{
    ...
    if (msglen > 0) {
        ...
        for (i = 0; i < msglen; i++) output[i] = input[i] ^ ((byte *)keystream)[i];
    }
