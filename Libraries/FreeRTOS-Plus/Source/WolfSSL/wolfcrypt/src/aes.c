...
int wc_AesCbcDecryptWithKey(byte *out, const byte *in, word32 inSz, const byte *key, word32 keySz,
                            const byte *iv)
{
...
int wc_AesGcmEncrypt(Aes *aes, byte *out, const byte *in, word32 sz, const byte *iv, word32 ivSz,
                     byte *authTag, word32 authTagSz, const byte *authIn, word32 authInSz)
{
    return AesGcmEncrypt_fips(aes, out, in, sz, iv, ivSz, authTag, authTagSz, authIn, authInSz);
}

int wc_AesGcmDecrypt(Aes *aes, byte *out, const byte *in, word32 sz, const byte *iv, word32 ivSz,
                     const byte *authTag, word32 authTagSz, const byte *authIn, word32 authInSz)
{
    return AesGcmDecrypt_fips(aes, out, in, sz, iv, ivSz, authTag, authTagSz, authIn, authInSz);
}
...
int wc_GmacUpdate(Gmac *gmac, const byte *iv, word32 ivSz, const byte *authIn, word32 authInSz,
                  byte *authTag, word32 authTagSz)
{
    return GmacUpdate(gmac, iv, ivSz, authIn, authInSz, authTag, authTagSz);
}
...
void wc_AesCcmEncrypt(Aes *aes, byte *out, const byte *in, word32 inSz, const byte *nonce,
                      word32 nonceSz, byte *authTag, word32 authTagSz, const byte *authIn,
                      word32 authInSz)
{
    AesCcmEncrypt(aes, out, in, inSz, nonce, nonceSz, authTag, authTagSz, authIn, authInSz);
}

int wc_AesCcmDecrypt(Aes *aes, byte *out, const byte *in, word32 inSz, const byte *nonce,
                     word32 nonceSz, const byte *authTag, word32 authTagSz, const byte *authIn,
                     word32 authInSz)
{
    return AesCcmDecrypt(aes, out, in, inSz, nonce, nonceSz, authTag, authTagSz, authIn, authInSz);
}
#endif /* HAVE_AESCCM */
...
static int wc_AesCaviumSetKey(Aes *aes, const byte *key, word32 length, const byte *iv);
static int wc_AesCaviumCbcEncrypt(Aes *aes, byte *out, const byte *in, word32 length);
static int wc_AesCaviumCbcDecrypt(Aes *aes, byte *out, const byte *in, word32 length);
#else
...
void AES_ECB_encrypt(const unsigned char *in, unsigned char *out, unsigned long length,
                     const unsigned char *KS, int nr) XASM_LINK("AES_ECB_encrypt");

void AES_ECB_decrypt(const unsigned char *in, unsigned char *out, unsigned long length,
                     const unsigned char *KS, int nr) XASM_LINK("AES_ECB_decrypt");

void AES_128_Key_Expansion(const unsigned char *userkey, unsigned char *key_schedule)
    XASM_LINK("AES_128_Key_Expansion");

void AES_192_Key_Expansion(const unsigned char *userkey, unsigned char *key_schedule)
    XASM_LINK("AES_192_Key_Expansion");

void AES_256_Key_Expansion(const unsigned char *userkey, unsigned char *key_schedule)
    XASM_LINK("AES_256_Key_Expansion");

static int AES_set_encrypt_key(const unsigned char *userKey, const int bits, Aes *aes)
{
    if (!userKey || !aes)
        return BAD_FUNC_ARG;
...
static int AES_set_decrypt_key(const unsigned char *userKey, const int bits, Aes *aes)
{
    int nr;
    Aes temp_key;
...
static void wc_AesEncrypt(Aes *aes, const byte *inBlock, byte *outBlock)
{
    ...
    if (haveAESNI && aes->use_aesni) {
        ...
        if ((wolfssl_word)inBlock % 16) {
            ...
            byte *tmp = (byte *)XMALLOC(AES_BLOCK_SIZE, NULL, DYNAMIC_TYPE_TMP_BUFFER);
            if (tmp == NULL)
                return;

            XMEMCPY(tmp, inBlock, AES_BLOCK_SIZE);
            AES_ECB_encrypt(tmp, tmp, AES_BLOCK_SIZE, (byte *)aes->key, aes->rounds);
        ...
        AES_ECB_encrypt(inBlock, outBlock, AES_BLOCK_SIZE, (byte *)aes->key, aes->rounds);
    ...
    for (;;) {
        t0 = Te[0][GETBYTE(s0, 3)] ^ Te[1][GETBYTE(s1, 2)] ^ Te[2][GETBYTE(s2, 1)] ^
             Te[3][GETBYTE(s3, 0)] ^ rk[4];
        t1 = Te[0][GETBYTE(s1, 3)] ^ Te[1][GETBYTE(s2, 2)] ^ Te[2][GETBYTE(s3, 1)] ^
             Te[3][GETBYTE(s0, 0)] ^ rk[5];
        t2 = Te[0][GETBYTE(s2, 3)] ^ Te[1][GETBYTE(s3, 2)] ^ Te[2][GETBYTE(s0, 1)] ^
             Te[3][GETBYTE(s1, 0)] ^ rk[6];
        t3 = Te[0][GETBYTE(s3, 3)] ^ Te[1][GETBYTE(s0, 2)] ^ Te[2][GETBYTE(s1, 1)] ^
             Te[3][GETBYTE(s2, 0)] ^ rk[7];
        ...
        s0 = Te[0][GETBYTE(t0, 3)] ^ Te[1][GETBYTE(t1, 2)] ^ Te[2][GETBYTE(t2, 1)] ^
             Te[3][GETBYTE(t3, 0)] ^ rk[0];
        s1 = Te[0][GETBYTE(t1, 3)] ^ Te[1][GETBYTE(t2, 2)] ^ Te[2][GETBYTE(t3, 1)] ^
             Te[3][GETBYTE(t0, 0)] ^ rk[1];
        s2 = Te[0][GETBYTE(t2, 3)] ^ Te[1][GETBYTE(t3, 2)] ^ Te[2][GETBYTE(t0, 1)] ^
             Te[3][GETBYTE(t1, 0)] ^ rk[2];
        s3 = Te[0][GETBYTE(t3, 3)] ^ Te[1][GETBYTE(t0, 2)] ^ Te[2][GETBYTE(t1, 1)] ^
             Te[3][GETBYTE(t2, 0)] ^ rk[3];
    }
    ...
    s0 = (Te[4][GETBYTE(t0, 3)] & 0xff000000) ^ (Te[4][GETBYTE(t1, 2)] & 0x00ff0000) ^
         (Te[4][GETBYTE(t2, 1)] & 0x0000ff00) ^ (Te[4][GETBYTE(t3, 0)] & 0x000000ff) ^ rk[0];
    s1 = (Te[4][GETBYTE(t1, 3)] & 0xff000000) ^ (Te[4][GETBYTE(t2, 2)] & 0x00ff0000) ^
         (Te[4][GETBYTE(t3, 1)] & 0x0000ff00) ^ (Te[4][GETBYTE(t0, 0)] & 0x000000ff) ^ rk[1];
    s2 = (Te[4][GETBYTE(t2, 3)] & 0xff000000) ^ (Te[4][GETBYTE(t3, 2)] & 0x00ff0000) ^
         (Te[4][GETBYTE(t0, 1)] & 0x0000ff00) ^ (Te[4][GETBYTE(t1, 0)] & 0x000000ff) ^ rk[2];
    s3 = (Te[4][GETBYTE(t3, 3)] & 0xff000000) ^ (Te[4][GETBYTE(t0, 2)] & 0x00ff0000) ^
         (Te[4][GETBYTE(t1, 1)] & 0x0000ff00) ^ (Te[4][GETBYTE(t2, 0)] & 0x000000ff) ^ rk[3];
...
static void wc_AesDecrypt(Aes *aes, const byte *inBlock, byte *outBlock)
{
    ...
    if (haveAESNI && aes->use_aesni) {
        ...
        AES_ECB_decrypt(inBlock, outBlock, AES_BLOCK_SIZE, (byte *)aes->key, aes->rounds);
        return;
    } else {
#ifdef DEBUG_AESNI
        printf("Skipping AES-NI\n");
    ...
    for (;;) {
        t0 = Td[0][GETBYTE(s0, 3)] ^ Td[1][GETBYTE(s3, 2)] ^ Td[2][GETBYTE(s2, 1)] ^
             Td[3][GETBYTE(s1, 0)] ^ rk[4];
        t1 = Td[0][GETBYTE(s1, 3)] ^ Td[1][GETBYTE(s0, 2)] ^ Td[2][GETBYTE(s3, 1)] ^
             Td[3][GETBYTE(s2, 0)] ^ rk[5];
        t2 = Td[0][GETBYTE(s2, 3)] ^ Td[1][GETBYTE(s1, 2)] ^ Td[2][GETBYTE(s0, 1)] ^
             Td[3][GETBYTE(s3, 0)] ^ rk[6];
        t3 = Td[0][GETBYTE(s3, 3)] ^ Td[1][GETBYTE(s2, 2)] ^ Td[2][GETBYTE(s1, 1)] ^
             Td[3][GETBYTE(s0, 0)] ^ rk[7];
        ...
        s0 = Td[0][GETBYTE(t0, 3)] ^ Td[1][GETBYTE(t3, 2)] ^ Td[2][GETBYTE(t2, 1)] ^
             Td[3][GETBYTE(t1, 0)] ^ rk[0];
        s1 = Td[0][GETBYTE(t1, 3)] ^ Td[1][GETBYTE(t0, 2)] ^ Td[2][GETBYTE(t3, 1)] ^
             Td[3][GETBYTE(t2, 0)] ^ rk[1];
        s2 = Td[0][GETBYTE(t2, 3)] ^ Td[1][GETBYTE(t1, 2)] ^ Td[2][GETBYTE(t0, 1)] ^
             Td[3][GETBYTE(t3, 0)] ^ rk[2];
        s3 = Td[0][GETBYTE(t3, 3)] ^ Td[1][GETBYTE(t2, 2)] ^ Td[2][GETBYTE(t1, 1)] ^
             Td[3][GETBYTE(t0, 0)] ^ rk[3];
    }
    /*
     * apply last round and
     * map cipher state to byte array block:
     */
    s0 = (Td[4][GETBYTE(t0, 3)] & 0xff000000) ^ (Td[4][GETBYTE(t3, 2)] & 0x00ff0000) ^
         (Td[4][GETBYTE(t2, 1)] & 0x0000ff00) ^ (Td[4][GETBYTE(t1, 0)] & 0x000000ff) ^ rk[0];
    s1 = (Td[4][GETBYTE(t1, 3)] & 0xff000000) ^ (Td[4][GETBYTE(t0, 2)] & 0x00ff0000) ^
         (Td[4][GETBYTE(t3, 1)] & 0x0000ff00) ^ (Td[4][GETBYTE(t2, 0)] & 0x000000ff) ^ rk[1];
    s2 = (Td[4][GETBYTE(t2, 3)] & 0xff000000) ^ (Td[4][GETBYTE(t1, 2)] & 0x00ff0000) ^
         (Td[4][GETBYTE(t0, 1)] & 0x0000ff00) ^ (Td[4][GETBYTE(t3, 0)] & 0x000000ff) ^ rk[2];
    s3 = (Td[4][GETBYTE(t3, 3)] & 0xff000000) ^ (Td[4][GETBYTE(t2, 2)] & 0x00ff0000) ^
         (Td[4][GETBYTE(t1, 1)] & 0x0000ff00) ^ (Td[4][GETBYTE(t0, 0)] & 0x000000ff) ^ rk[3];
...
static int wc_AesSetKeyLocal(Aes *aes, const byte *userKey, word32 keylen, const byte *iv, int dir)
{
    ...
    case 16:
        while (1) {
            temp = rk[3];
            rk[4] = rk[0] ^ (Te[4][GETBYTE(temp, 2)] & 0xff000000) ^
                    (Te[4][GETBYTE(temp, 1)] & 0x00ff0000) ^
                    (Te[4][GETBYTE(temp, 0)] & 0x0000ff00) ^
                    (Te[4][GETBYTE(temp, 3)] & 0x000000ff) ^ rcon[i];
            rk[5] = rk[1] ^ rk[4];
            rk[6] = rk[2] ^ rk[5];
            rk[7] = rk[3] ^ rk[6];
            if (++i == 10)
    ...
    case 24:
        ...
        while (1) {
            temp = rk[5];
            rk[6] = rk[0] ^ (Te[4][GETBYTE(temp, 2)] & 0xff000000) ^
                    (Te[4][GETBYTE(temp, 1)] & 0x00ff0000) ^
                    (Te[4][GETBYTE(temp, 0)] & 0x0000ff00) ^
                    (Te[4][GETBYTE(temp, 3)] & 0x000000ff) ^ rcon[i];
            rk[7] = rk[1] ^ rk[6];
    ...
    case 32:
        while (1) {
            temp = rk[7];
            rk[8] = rk[0] ^ (Te[4][GETBYTE(temp, 2)] & 0xff000000) ^
                    (Te[4][GETBYTE(temp, 1)] & 0x00ff0000) ^
                    (Te[4][GETBYTE(temp, 0)] & 0x0000ff00) ^
                    (Te[4][GETBYTE(temp, 3)] & 0x000000ff) ^ rcon[i];
            rk[9] = rk[1] ^ rk[8];
            ...
            if (++i == 7)
                break;
            temp = rk[11];
            rk[12] = rk[4] ^ (Te[4][GETBYTE(temp, 3)] & 0xff000000) ^
    ...
    if (dir == AES_DECRYPTION) {
        ...
        for (i = 1; i < aes->rounds; i++) {
            rk += 4;
            rk[0] = Td[0][Te[4][GETBYTE(rk[0], 3)] & 0xff] ^
                    Td[1][Te[4][GETBYTE(rk[0], 2)] & 0xff] ^
                    Td[2][Te[4][GETBYTE(rk[0], 1)] & 0xff] ^ Td[3][Te[4][GETBYTE(rk[0], 0)] & 0xff];
            rk[1] = Td[0][Te[4][GETBYTE(rk[1], 3)] & 0xff] ^
                    Td[1][Te[4][GETBYTE(rk[1], 2)] & 0xff] ^
                    Td[2][Te[4][GETBYTE(rk[1], 1)] & 0xff] ^ Td[3][Te[4][GETBYTE(rk[1], 0)] & 0xff];
            rk[2] = Td[0][Te[4][GETBYTE(rk[2], 3)] & 0xff] ^
                    Td[1][Te[4][GETBYTE(rk[2], 2)] & 0xff] ^
                    Td[2][Te[4][GETBYTE(rk[2], 1)] & 0xff] ^ Td[3][Te[4][GETBYTE(rk[2], 0)] & 0xff];
            rk[3] = Td[0][Te[4][GETBYTE(rk[3], 3)] & 0xff] ^
                    Td[1][Te[4][GETBYTE(rk[3], 2)] & 0xff] ^
                    Td[2][Te[4][GETBYTE(rk[3], 1)] & 0xff] ^ Td[3][Te[4][GETBYTE(rk[3], 0)] & 0xff];
        }
    }
...
#elif defined(HAVE_COLDFIRE_SEC)
static int wc_AesCbcCrypt(Aes *aes, byte *po, const byte *pi, word32 sz, word32 descHeader)
{
#ifdef DEBUG_WOLFSSL
    int i;
    int stat1, stat2;
...
int wc_AesCbcEncrypt(Aes *aes, byte *out, const byte *in, word32 sz)
{
    ...
    while (len > 0) {
        ...
        for (i = 0; i < AES_BLOCK_SIZE; i++) temp_block[i] ^= iv[i];
...
int wc_AesCbcDecrypt(Aes *aes, byte *out, const byte *in, word32 sz)
{
    ...
    while (len > 0) {
        ...
        /* XOR block with IV for CBC */
        for (i = 0; i < AES_BLOCK_SIZE; i++) (out + offset)[i] ^= iv[i];
...
static void wc_AesCrypt(Aes *aes, byte *out, const byte *in, word32 sz, int dir, int algo,
                        int cryptoalgo)
{
    ...
    ByteReverseWords((word32 *)KVA0_TO_KVA1(sa.SA_ENCKEY + 8 - aes->keylen / sizeof(word32)),
                     (word32 *)aes->key_ce, aes->keylen);
    ByteReverseWords((word32 *)KVA0_TO_KVA1(sa.SA_ENCIV), (word32 *)aes->iv_ce, 16);
    ...
    if ((cryptoalgo == PIC32_CRYPTOALGO_CBC) || (cryptoalgo == PIC32_CRYPTOALGO_TCBC) ||
        (cryptoalgo == PIC32_CRYPTOALGO_RCBC)) {
        /* set iv for the next call */
        if (dir == PIC32_ENCRYPTION) {
            XMEMCPY((void *)aes->iv_ce, (void *)KVA0_TO_KVA1(out + sz - AES_BLOCK_SIZE),
                    AES_BLOCK_SIZE);
        } else {
            ByteReverseWords((word32 *)aes->iv_ce, (word32 *)KVA0_TO_KVA1(in + sz - AES_BLOCK_SIZE),
                             AES_BLOCK_SIZE);
        }
    }
...
int wc_AesCbcEncrypt(Aes *aes, byte *out, const byte *in, word32 sz)
{
    wc_AesCrypt(aes, out, in, sz, PIC32_ENCRYPTION, PIC32_ALGO_AES, PIC32_CRYPTOALGO_RCBC);
    return 0;
}

int wc_AesCbcDecrypt(Aes *aes, byte *out, const byte *in, word32 sz)
{
    wc_AesCrypt(aes, out, in, sz, PIC32_DECRYPTION, PIC32_ALGO_AES, PIC32_CRYPTOALGO_RCBC);
    return 0;
}
...
int wc_AesCbcEncrypt(Aes *aes, byte *out, const byte *in, word32 sz)
{
    ...
    if (haveAESNI) {
        ...
        if ((wolfssl_word)in % 16) {
            ...
            XMEMCPY(tmp, in, sz);
            AES_CBC_encrypt(tmp, tmp, (byte *)aes->reg, sz, (byte *)aes->key, aes->rounds);
            /* store iv for next call */
        ...
        AES_CBC_encrypt(in, out, (byte *)aes->reg, sz, (byte *)aes->key, aes->rounds);
        /* store iv for next call */
        XMEMCPY(aes->reg, out + sz - AES_BLOCK_SIZE, AES_BLOCK_SIZE);
...
int wc_AesCbcDecrypt(Aes *aes, byte *out, const byte *in, word32 sz)
{
    ...
    if (haveAESNI) {
        ...
        /* if input and output same will overwrite input iv */
        XMEMCPY(aes->tmp, in + sz - AES_BLOCK_SIZE, AES_BLOCK_SIZE);
        AES_CBC_decrypt(in, out, (byte *)aes->reg, sz, (byte *)aes->key, aes->rounds);
        /* store iv for next call */
        XMEMCPY(aes->reg, aes->tmp, AES_BLOCK_SIZE);
        return 0;
    }
...
void wc_AesCtrEncrypt(Aes *aes, byte *out, const byte *in, word32 sz)
{
    ...
    if (aes->left) {
        ...
        if ((odd + aes->left) == AES_BLOCK_SIZE) {
            wc_AesCrypt(aes, out_block, tmp, AES_BLOCK_SIZE, PIC32_ENCRYPTION, PIC32_ALGO_AES,
    ...
    if (sz / AES_BLOCK_SIZE) {
        ...
        wc_AesCrypt(aes, out, in, even, PIC32_ENCRYPTION, PIC32_ALGO_AES, PIC32_CRYPTOALGO_RCTR);
        out += even;
        in += even;
        do { /* Increment IV */
            for (i = AES_BLOCK_SIZE - 1; i >= 0; i--) {
...
static void GHASH(Aes *aes, const byte *a, word32 aSz, const byte *c, word32 cSz, byte *s,
                  word32 sSz)
{
...
#ifdef WOLFSSL_PIC32MZ_CRYPT
    if (blocks)
        wc_AesCrypt(aes, out, in, blocks * AES_BLOCK_SIZE, PIC32_ENCRYPTION, PIC32_ALGO_AES,
                    PIC32_CRYPTOALGO_AES_GCM);
#endif
    while (blocks--) {
...
#ifdef WOLFSSL_PIC32MZ_CRYPT
    if (blocks)
        wc_AesCrypt(aes, out, in, blocks * AES_BLOCK_SIZE, PIC32_DECRYPTION, PIC32_ALGO_AES,
                    PIC32_CRYPTOALGO_AES_GCM);
#endif
...
WOLFSSL_API int wc_GmacUpdate(Gmac *gmac, const byte *iv, word32 ivSz, const byte *authIn,
                              word32 authInSz, byte *authTag, word32 authTagSz)
{
    return wc_AesGcmEncrypt(&gmac->aes, NULL, NULL, 0, iv, ivSz, authTag, authTagSz, authIn,
                            authInSz);
}
                      ...
                      word32 authInSz)
{
    ...
    B[0] = (authInSz > 0 ? 64 : 0) + (8 * (((byte)authTagSz - 2) / 2)) + (lenSz - 1);
    for (i = 0; i < lenSz; i++) B[AES_BLOCK_SIZE - 1 - i] = (inSz >> (8 * i)) & 0xFF;
    ...
    B[0] = lenSz - 1;
    for (i = 0; i < lenSz; i++) B[AES_BLOCK_SIZE - 1 - i] = 0;
#ifdef FREESCALE_MMCAU
    cau_aes_encrypt(B, key, aes->rounds, A);
#else
    wc_AesEncrypt(aes, B, A);
...
static int AesCaviumCbcEncrypt(Aes *aes, byte *out, const byte *in, word32 length)
{
    ...
    while (length > WOLFSSL_MAX_16BIT) {
        ...
        if (CspEncryptAes(CAVIUM_BLOCKING, aes->contextHandle, CAVIUM_NO_UPDATE, aes->type, slen,
                          (byte *)in + offset, out + offset, (byte *)aes->reg, (byte *)aes->key,
                          &requestId, aes->devId) != 0) {
...
static int AesCaviumCbcDecrypt(Aes *aes, byte *out, const byte *in, word32 length)
{
    word32 requestId;
    wolfssl_word offset = 0;
    ...
    while (length > WOLFSSL_MAX_16BIT) {
        ...
        if (CspDecryptAes(CAVIUM_BLOCKING, aes->contextHandle, CAVIUM_NO_UPDATE, aes->type, slen,
                          (byte *)in + offset, out + offset, (byte *)aes->reg, (byte *)aes->key,
                          &requestId, aes->devId) != 0) {
