...
int main(int argc, char **argv)
{
    ...
    if (argc == 1) {
        ...
        ret =
            ssl_SetPrivateKey(server, port, "../../certs/server-key.pem", FILETYPE_PEM, NULL, err);
        if (ret != 0) {
...
#ifdef HAVE_SNI
        {
            ...
            if (strnlen(altName, 128) > 0) {
                ret = ssl_SetNamedPrivateKey(altName, server, port, "../../certs/server-key.pem",
        ...
        if (pcap == NULL) {
            ...
            ret = ssl_SetPrivateKey(server, port, argv[2], FILETYPE_PEM, passwd, err);
        }
    } else {
        /* usage error */
