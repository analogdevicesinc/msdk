 ...
 * not turned off. */
    ...
    ((!defined(HAVE_ECC) && (defined(NO_DH) || defined(NO_RSA))) || \
     ...
     (defined(NO_SHA256) && !defined(WOLFSSL_SHA384)) || !defined(NO_OLD_TLS))
...
#if !defined(NO_DH) && !defined(NO_AES) && !defined(NO_TLS) && !defined(NO_RSA)
...
#if defined(HAVE_ANON) && !defined(NO_TLS) && !defined(NO_DH) && !defined(NO_AES) && \
    !defined(NO_SHA)
...
#if defined(BUILD_TLS_RSA_WITH_HC_128_SHA) || defined(BUILD_TLS_RSA_WITH_HC_128_MD5) || \
...
#if defined(WOLFSSL_MAX_STRENGTH) || defined(HAVE_AESGCM) || defined(HAVE_AESCCM) || \
...
enum BIO_TYPE { BIO_BUFFER = 1, BIO_SOCKET = 2, BIO_SSL = 3, BIO_MEMORY = 4 };
...
/* for sniffer */
WOLFSSL_LOCAL int DoFinished(WOLFSSL *ssl, const byte *input, word32 *inOutIdx, word32 size,
...
enum { FORCED_FREE = 1, NO_FORCED_FREE = 0 };
...
WOLFSSL_LOCAL
void InitSuites(Suites *, ProtocolVersion, word16, word16, word16, word16, word16, word16, int);
WOLFSSL_LOCAL
...
WOLFSSL_LOCAL int TLSX_Parse(WOLFSSL *ssl, byte *input, word16 length, byte isRequest,
                             Suites *suites);

#elif defined(HAVE_SNI) || defined(HAVE_MAX_FRAGMENT) || defined(HAVE_TRUNCATED_HMAC) || \
    defined(HAVE_SUPPORTED_CURVES) || defined(HAVE_SECURE_RENEGOTIATION) ||              \
...
WOLFSSL_LOCAL int TLSX_SNI_GetFromBuffer(const byte *buffer, word32 bufferSz, byte type, byte *sni,
                                         word32 *inOutSz);
#endif
...
WOLFSSL_LOCAL
int ProcessOldClientHello(WOLFSSL *ssl, const byte *input, word32 *inOutIdx, word32 inSz,
                          word16 sz);
#ifndef NO_CERTS
WOLFSSL_LOCAL
...
enum SignatureAlgorithm { anonymous_sa_algo, rsa_sa_algo, dsa_sa_algo, ecc_dsa_sa_algo };

/* Supprted ECC Curve Types */
enum EccCurves { named_curve = 3 };
...
#ifndef NO_CERTS
WOLFSSL_LOCAL int PemToDer(const unsigned char *buff, long sz, int type, buffer *der, void *heap,
                           EncryptedInfo *info, int *eccKey);

WOLFSSL_LOCAL int ProcessFile(WOLFSSL_CTX *ctx, const char *fname, int format, int type,
...
WOLFSSL_LOCAL int TLS_hmac(WOLFSSL *ssl, byte *digest, const byte *in, word32 sz, int content,
                           int verify);
#endif
...
WOLFSSL_LOCAL void DtlsMsgSet(DtlsMsg *, word32, const byte *, byte, word32, word32);
WOLFSSL_LOCAL DtlsMsg *DtlsMsgFind(DtlsMsg *, word32);
WOLFSSL_LOCAL DtlsMsg *DtlsMsgStore(DtlsMsg *, word32, const byte *, word32, byte, word32, word32,
                                    void *);
...
enum encrypt_side { ENCRYPT_SIDE_ONLY = 1, DECRYPT_SIDE_ONLY, ENCRYPT_AND_DECRYPT_SIDE };
