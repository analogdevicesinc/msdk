...
static INLINE void ShowX509(WOLFSSL_X509 *x509, const char *hdr)
{
    ...
    if (ret == SSL_SUCCESS) {
        ...
        for (i = 0; i < sz; i++) sprintf(serialMsg + strLen + (i * 3), ":%02x ", serial[i]);
        printf("%s\n", serialMsg);
    }
...
static INLINE void build_addr(SOCKADDR_IN_T *addr, const char *peer, word16 port, int udp)
{
    int useLookup = 0;
    (void)useLookup;
    (void)udp;
...
static INLINE void tcp_connect(SOCKET_T *sockfd, const char *ip, word16 port, int udp)
{
    SOCKADDR_IN_T addr;
...
enum { TEST_SELECT_FAIL, TEST_TIMEOUT, TEST_RECV_READY, TEST_ERROR_READY };
...
static INLINE void tcp_listen(SOCKET_T *sockfd, word16 *port, int useAnyAddr, int udp)
{
    SOCKADDR_IN_T addr;
...
static INLINE void udp_accept(SOCKET_T *sockfd, SOCKET_T *clientfd, int useAnyAddr, word16 port,
                              func_args *args)
{
    SOCKADDR_IN_T addr;
...
static INLINE void tcp_accept(SOCKET_T *sockfd, SOCKET_T *clientfd, func_args *args, word16 port,
                              int useAnyAddr, int udp, int ready_file)
{
...
static INLINE unsigned int my_psk_client_cb(WOLFSSL *ssl, const char *hint, char *identity,
...
static INLINE unsigned int my_psk_server_cb(WOLFSSL *ssl, const char *identity, unsigned char *key,
                                            unsigned int key_max_len)
{
    (void)ssl;
    (void)key_max_len;
...
enum { WOLFSSL_CA = 1, WOLFSSL_CERT = 2, WOLFSSL_KEY = 3 };
...
static INLINE void load_buffer(WOLFSSL_CTX *ctx, const char *fname, int type)
{
    ...
    if (type == WOLFSSL_CA) {
        if (wolfSSL_CTX_load_verify_buffer(ctx, buff, sz, SSL_FILETYPE_PEM) != SSL_SUCCESS)
            err_sys("can't load buffer ca file");
    } else if (type == WOLFSSL_CERT) {
        if (wolfSSL_CTX_use_certificate_buffer(ctx, buff, sz, SSL_FILETYPE_PEM) != SSL_SUCCESS)
            err_sys("can't load buffer cert file");
    } else if (type == WOLFSSL_KEY) {
        if (wolfSSL_CTX_use_PrivateKey_buffer(ctx, buff, sz, SSL_FILETYPE_PEM) != SSL_SUCCESS)
...
static INLINE void SetDH(WOLFSSL *ssl)
{
    ...
    static unsigned char p[] = {
        0xE6, 0x96, 0x9D, 0x3D, 0x49, 0x5B, 0xE3, 0x2C, 0x7C, 0xF1, 0x80, 0xC3, 0xBD, 0xD4, 0x79,
        0x8E, 0x91, 0xB7, 0x81, 0x82, 0x51, 0xBB, 0x05, 0x5E, 0x2A, 0x20, 0x64, 0x90, 0x4A, 0x79,
        0xA7, 0x70, 0xFA, 0x15, 0xA2, 0x59, 0xCB, 0xD5, 0x23, 0xA6, 0xA6, 0xEF, 0x09, 0xC4, 0x30,
        0x48, 0xD5, 0xA2, 0x2F, 0x97, 0x1F, 0x3C, 0x20, 0x12, 0x9B, 0x48, 0x00, 0x0E, 0x6E, 0xDD,
        0x06, 0x1C, 0xBC, 0x05, 0x3E, 0x37, 0x1D, 0x79, 0x4E, 0x53, 0x27, 0xDF, 0x61, 0x1E, 0xBB,
        0xBE, 0x1B, 0xAC, 0x9B, 0x5C, 0x60, 0x44, 0xCF, 0x02, 0x3D, 0x76, 0xE0, 0x5E, 0xEA, 0x9B,
        0xAD, 0x99, 0x1B, 0x13, 0xA6, 0x3C, 0x97, 0x4E, 0x9E, 0xF1, 0x83, 0x9E, 0xB5, 0xDB, 0x12,
        0x51, 0x36, 0xF7, 0x26, 0x2E, 0x56, 0xA8, 0x87, 0x15, 0x38, 0xDF, 0xD8, 0x23, 0xC6, 0x50,
...
static INLINE int myMacEncryptCb(WOLFSSL *ssl, unsigned char *macOut, const unsigned char *macIn,
                                 ...
                                 unsigned int encSz, void *ctx)
{
    ...
    ret = wc_HmacSetKey(&hmac, wolfSSL_GetHmacType(ssl), wolfSSL_GetMacSecret(ssl, macVerify),
...
static INLINE int myDecryptVerifyCb(WOLFSSL *ssl, unsigned char *decOut, const unsigned char *decIn,
                                    ...
                                    unsigned int *padSz, void *ctx)
{
    ...
    *padSz = wolfSSL_GetHmacSize(ssl) + pad + padByte;
    ...
    if (memcmp(verify, decOut + decSz - digestSz - pad - padByte, digestSz) != 0) {
...
static INLINE int myEccSign(WOLFSSL *ssl, const byte *in, word32 inSz, byte *out, word32 *outSz,
...
static INLINE int myEccVerify(WOLFSSL *ssl, const byte *sig, word32 sigSz, const byte *hash,
...
static INLINE int myRsaSign(WOLFSSL *ssl, const byte *in, word32 inSz, byte *out, word32 *outSz,
                            const byte *key, word32 keySz, void *ctx)
{
    RNG rng;
    int ret;
...
static INLINE int myRsaVerify(WOLFSSL *ssl, byte *sig, word32 sigSz, byte **out, const byte *key,
...
static INLINE int myRsaEnc(WOLFSSL *ssl, const byte *in, word32 inSz, byte *out, word32 *outSz,
                           const byte *key, word32 keySz, void *ctx)
{
    int ret;
    word32 idx = 0;
...
static INLINE int myRsaDec(WOLFSSL *ssl, byte *in, word32 inSz, byte **out, const byte *key,
                           word32 keySz, void *ctx)
{
    int ret;
    word32 idx = 0;
...
#if defined(__hpux__) || defined(__MINGW32__) || defined(WOLFSSL_TIRTOS) || defined(_MSC_VER)
...
static INLINE int myTicketEncCb(WOLFSSL *ssl, byte key_name[WOLFSSL_TICKET_NAME_SZ],
                                ...
                                int enc, byte *ticket, int inLen, int *outLen, void *userCtx)
{
    ...
    if (enc) {
        ...
        ret =
            wc_ChaCha20Poly1305_Encrypt(myKey_ctx.key, iv, aad, aadSz, ticket, inLen, ticket, mac);
        if (ret != 0)
            return WOLFSSL_TICKET_RET_REJECT;
        *outLen = inLen; /* no padding in this mode */
        ...
        ret =
            wc_ChaCha20Poly1305_Decrypt(myKey_ctx.key, iv, aad, aadSz, ticket, inLen, mac, ticket);
        if (ret != 0)
            return WOLFSSL_TICKET_RET_REJECT;
        *outLen = inLen; /* no padding in this mode */
    }
