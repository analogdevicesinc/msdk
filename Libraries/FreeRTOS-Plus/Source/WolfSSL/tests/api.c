...
#define TEST_VALID_METHOD_ALLOCATOR(a) TEST_METHOD_ALLOCATOR(a, AssertNotNull)

#define TEST_INVALID_METHOD_ALLOCATOR(a) TEST_METHOD_ALLOCATOR(a, AssertNull)
...
static void test_wolfSSL_CTX_use_PrivateKey_file(void)
{
    ...
    /* invalid context */
    AssertFalse(wolfSSL_CTX_use_PrivateKey_file(NULL, svrKey, SSL_FILETYPE_PEM));
    /* invalid key file */
...
/*----------------------------------------------------------------------------*
 | IO
 *----------------------------------------------------------------------------*/
#if !defined(NO_FILESYSTEM) && !defined(NO_CERTS) && !defined(NO_RSA) && !defined(SINGLE_THREADED)
...
#if defined(NO_MAIN_DRIVER) && !defined(USE_WINDOWS_API) && !defined(WOLFSSL_SNIFFER) && \
...
static void test_client_nofail(void *args)
{
    ...
    if (wolfSSL_CTX_use_certificate_file(ctx, cliCert, SSL_FILETYPE_PEM) != SSL_SUCCESS) {
        /*err_sys("can't load client cert file, "
                "Please run from wolfSSL home dir");*/
        goto done2;
    }
    if (wolfSSL_CTX_use_PrivateKey_file(ctx, cliKey, SSL_FILETYPE_PEM) != SSL_SUCCESS) {
...
static THREAD_RETURN WOLFSSL_THREAD run_wolfssl_server(void *args)
{
    ...
    AssertIntEQ(SSL_SUCCESS, wolfSSL_CTX_use_certificate_file(ctx, svrCert, SSL_FILETYPE_PEM));

    AssertIntEQ(SSL_SUCCESS, wolfSSL_CTX_use_PrivateKey_file(ctx, svrKey, SSL_FILETYPE_PEM));
...
static void run_wolfssl_client(void *args)
{
    ...
    AssertIntEQ(SSL_SUCCESS, wolfSSL_CTX_use_certificate_file(ctx, cliCert, SSL_FILETYPE_PEM));

    AssertIntEQ(SSL_SUCCESS, wolfSSL_CTX_use_PrivateKey_file(ctx, cliKey, SSL_FILETYPE_PEM));
...
static void test_wolfSSL_SNI_GetFromBuffer(void)
{
    byte buffer[] = {
        /* www.paypal.com */
        0x00, 0x00, 0x00, 0x00, 0xff, 0x01, 0x00, 0x00, 0x60, 0x03, 0x03, 0x5c, 0xc4, 0xb3, 0x8c,
        0x87, 0xef, 0xa4, 0x09, 0xe0, 0x02, 0xab, 0x86, 0xca, 0x76, 0xf0, 0x9e, 0x01, 0x65, 0xf6,
        0xa6, 0x06, 0x13, 0x1d, 0x0f, 0xa5, 0x79, 0xb0, 0xd4, 0x77, 0x22, 0xeb, 0x1a, 0x00, 0x00,
        0x16, 0x00, 0x6b, 0x00, 0x67, 0x00, 0x39, 0x00, 0x33, 0x00, 0x3d, 0x00, 0x3c, 0x00, 0x35,
        0x00, 0x2f, 0x00, 0x05, 0x00, 0x04, 0x00, 0x0a, 0x01, 0x00, 0x00, 0x21, 0x00, 0x00, 0x00,
        0x13, 0x00, 0x11, 0x00, 0x00, 0x0e, 0x77, 0x77, 0x77, 0x2e, 0x70, 0x61, 0x79, 0x70, 0x61,
        0x6c, 0x2e, 0x63, 0x6f, 0x6d, 0x00, 0x0d, 0x00, 0x06, 0x00, 0x04, 0x04, 0x01, 0x02, 0x01
    };

    byte buffer2[] = {
        /* api.textmate.org */
        
    };
    ...
    byte buffer4[] = {
        /* last extension has zero size */
        
    };
    ...
    AssertIntEQ(BUFFER_ERROR,
                wolfSSL_SNI_GetFromBuffer(buffer, sizeof(buffer), 0, result, &length));
    buffer[0] = 0x16;
    ...
    AssertIntEQ(SNI_UNSUPPORTED,
                wolfSSL_SNI_GetFromBuffer(buffer, sizeof(buffer), 0, result, &length));
    buffer[2] = 0x03;

    AssertIntEQ(INCOMPLETE_DATA,
                wolfSSL_SNI_GetFromBuffer(buffer, sizeof(buffer), 0, result, &length));
    buffer[4] = 0x64;
    ...
    AssertIntEQ(SSL_SUCCESS,
                wolfSSL_SNI_GetFromBuffer(buffer2, sizeof(buffer2), 0, result, &length));
    result[length] = 0;
...
static void test_wolfSSL_UseSNI(void)
{
    ...
    /* error cases */
    AssertIntNE(SSL_SUCCESS, wolfSSL_CTX_UseSNI(NULL, 0, (void *)"ctx", XSTRLEN("ctx")));
    AssertIntNE(SSL_SUCCESS, wolfSSL_UseSNI(NULL, 0, (void *)"ssl", XSTRLEN("ssl")));
    AssertIntNE(SSL_SUCCESS, wolfSSL_CTX_UseSNI(ctx, -1, (void *)"ctx", XSTRLEN("ctx")));
    AssertIntNE(SSL_SUCCESS, wolfSSL_UseSNI(ssl, -1, (void *)"ssl", XSTRLEN("ssl")));
    AssertIntNE(SSL_SUCCESS, wolfSSL_CTX_UseSNI(ctx, 0, (void *)NULL, XSTRLEN("ctx")));
    AssertIntNE(SSL_SUCCESS, wolfSSL_UseSNI(ssl, 0, (void *)NULL, XSTRLEN("ssl")));

    /* success case */
    AssertIntEQ(SSL_SUCCESS, wolfSSL_CTX_UseSNI(ctx, 0, (void *)"ctx", XSTRLEN("ctx")));
    AssertIntEQ(SSL_SUCCESS, wolfSSL_UseSNI(ssl, 0, (void *)"ssl", XSTRLEN("ssl")));
...
static void test_wolfSSL_UseSupportedCurve(void)
{
    ...
    /* error cases */
    AssertIntNE(SSL_SUCCESS, wolfSSL_CTX_UseSupportedCurve(NULL, WOLFSSL_ECC_SECP256R1));
    AssertIntNE(SSL_SUCCESS, wolfSSL_CTX_UseSupportedCurve(ctx, 0));

    AssertIntNE(SSL_SUCCESS, wolfSSL_UseSupportedCurve(NULL, WOLFSSL_ECC_SECP256R1));
    AssertIntNE(SSL_SUCCESS, wolfSSL_UseSupportedCurve(ssl, 0));

    /* success case */
    AssertIntEQ(SSL_SUCCESS, wolfSSL_CTX_UseSupportedCurve(ctx, WOLFSSL_ECC_SECP256R1));
    AssertIntEQ(SSL_SUCCESS, wolfSSL_UseSupportedCurve(ssl, WOLFSSL_ECC_SECP256R1));
#endif
