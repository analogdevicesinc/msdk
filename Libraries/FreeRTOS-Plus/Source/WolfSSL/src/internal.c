...
#error WOLFSSL_CALLBACKS needs LARGE_STATIC_BUFFERS, please add LARGE_STATIC_BUFFERS
#endif
...
static int BuildMessage(WOLFSSL *ssl, byte *output, int outSz, const byte *input, int inSz,
                        int type);

#ifndef NO_WOLFSSL_CLIENT
static int DoHelloVerifyRequest(WOLFSSL *ssl, const byte *input, word32 *, word32);
static int DoServerHello(WOLFSSL *ssl, const byte *input, word32 *, word32);
static int DoServerKeyExchange(WOLFSSL *ssl, const byte *input, word32 *, word32);
#ifndef NO_CERTS
static int DoCertificateRequest(WOLFSSL *ssl, const byte *input, word32 *, word32);
#endif
#ifdef HAVE_SESSION_TICKET
static int DoSessionTicket(WOLFSSL *ssl, const byte *input, word32 *, word32);
#endif
#endif
...
#ifndef NO_OLD_TLS
static int SSL_hmac(WOLFSSL *ssl, byte *digest, const byte *in, word32 sz, int content, int verify);
...
static void PickHashSigAlgo(WOLFSSL *ssl, const byte *hashSigAlgo, word32 hashSigAlgoSz);
...
#if !defined(NO_OLD_TLS) || defined(HAVE_CHACHA) || defined(HAVE_AESCCM) || defined(HAVE_AESGCM)
...
static void InitSuitesHashSigAlgo(Suites *suites, int haveECDSAsig, int haveRSAsig, int haveAnon)
{
    int idx = 0;
...
void InitSuites(Suites *suites, ProtocolVersion pv, word16 haveRSA, word16 havePSK, word16 haveDH,
                word16 haveNTRU, word16 haveECDSAsig, word16 haveStaticECC, int side)
{
    word16 idx = 0;
...
int InitSSL(WOLFSSL *ssl, WOLFSSL_CTX *ctx)
{
    ...
    if (ssl->options.side == WOLFSSL_SERVER_END)
        InitSuites(ssl->suites, ssl->version, haveRSA, havePSK, ssl->options.haveDH,
                   ssl->options.haveNTRU, ssl->options.haveECDSAsig, ssl->options.haveStaticECC,
                   ssl->options.side);
    else
        InitSuites(ssl->suites, ssl->version, haveRSA, havePSK, TRUE, ssl->options.haveNTRU,
                   ssl->options.haveECDSAsig, ssl->options.haveStaticECC, ssl->options.side);
...
DtlsMsg *DtlsMsgNew(word32 sz, void *heap)
{
    ...
    if (msg != NULL) {
        msg->buf = (byte *)XMALLOC(sz + DTLS_HANDSHAKE_HEADER_SZ, heap, DYNAMIC_TYPE_NONE);
        if (msg->buf != NULL) {
            msg->next = NULL;
            msg->seq = 0;
            msg->sz = sz;
...
void DtlsMsgSet(DtlsMsg *msg, word32 seq, const byte *data, byte type, word32 fragOffset,
                word32 fragSz)
{
    if (msg != NULL && data != NULL && msg->fragSz <= msg->sz && fragOffset <= msg->sz &&
        ...
        if (fragOffset == 0)
            XMEMCPY(msg->buf, data - DTLS_HANDSHAKE_HEADER_SZ, fragSz + DTLS_HANDSHAKE_HEADER_SZ);
        else {
...
DtlsMsg *DtlsMsgStore(DtlsMsg *head, word32 seq, const byte *data, word32 dataSz, byte type,
...
static void AddHandShakeHeader(byte *output, word32 length, byte type, WOLFSSL *ssl)
{
    HandShakeHeader *hs;
    (void)ssl;
...
void ShrinkOutputBuffer(WOLFSSL *ssl)
{
    ...
    XFREE(ssl->buffers.outputBuffer.buffer - ssl->buffers.outputBuffer.offset, ssl->heap,
...
void ShrinkInputBuffer(WOLFSSL *ssl, int forcedFree)
{
    ...
    if (!forcedFree && usedLength)
        XMEMCPY(ssl->buffers.inputBuffer.staticBuffer,
                ssl->buffers.inputBuffer.buffer + ssl->buffers.inputBuffer.idx, usedLength);

    XFREE(ssl->buffers.inputBuffer.buffer - ssl->buffers.inputBuffer.offset, ssl->heap,
...
int SendBuffered(WOLFSSL *ssl)
{
    ...
    while (ssl->buffers.outputBuffer.length > 0) {
        int sent = ssl->ctx->CBIOSend(
            ssl, (char *)ssl->buffers.outputBuffer.buffer + ssl->buffers.outputBuffer.idx,
...
/* see if we got our timeout */
                ...
                if (ssl->toInfoOn) {
                    ...
                    getitimer(ITIMER_REAL, &timeout);
                    if (timeout.it_value.tv_sec == 0 && timeout.it_value.tv_usec == 0) {
                        XSTRNCPY(ssl->timeoutInfo.timeoutName, "send() timeout",
                                 MAX_TIMEOUT_NAME_SZ);
                        WOLFSSL_MSG("Got our timeout");
                        return WANT_WRITE;
                    }
...
static INLINE int GrowOutputBuffer(WOLFSSL *ssl, int size)
{
    ...
    if (align) {
        while (align < hdrSz) align *= 2;
    }

    tmp = (byte *)XMALLOC(size + ssl->buffers.outputBuffer.length + align, ssl->heap,
                          DYNAMIC_TYPE_OUT_BUFFER);
...
static int GetRecordHeader(WOLFSSL *ssl, const byte *input, word32 *inOutIdx, RecordLayerHeader *rh,
                           word16 *size)
{
    if (!ssl->options.dtls) {
#ifdef HAVE_FUZZER
...
#ifdef HAVE_FUZZER
        if (ssl->fuzzerCb)
            ssl->fuzzerCb(ssl, input + *inOutIdx - LENGTH_SZ - 8 - ENUM_LEN - VERSION_SZ,
                          ENUM_LEN + VERSION_SZ + 8 + LENGTH_SZ, FUZZ_HEAD, ssl->fuzzerCtx);
#endif
#endif
    }
    ...
    if (rh->pvMajor != ssl->version.major || rh->pvMinor != ssl->version.minor) {
        if (ssl->options.side == WOLFSSL_SERVER_END && ssl->options.acceptState == ACCEPT_BEGIN)
            WOLFSSL_MSG("Client attempting to connect with different version");
        else if (ssl->options.side == WOLFSSL_CLIENT_END && ssl->options.downgrade &&
...
static int GetHandShakeHeader(WOLFSSL *ssl, const byte *input, word32 *inOutIdx, byte *type,
...
#ifdef WOLFSSL_DTLS
static int GetDtlsHandShakeHeader(WOLFSSL *ssl, const byte *input, word32 *inOutIdx, byte *type,
                                  word32 *size, word32 *fragOffset, word32 *fragSz, word32 totalSz)
{
    word32 idx = *inOutIdx;
...
static const byte PAD1[PAD_MD5] = { 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36,
                                    0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36,
                                    0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36,
                                    0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36,
                                    0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36 };
static const byte PAD2[PAD_MD5] = { 0x5c, 0x5c, 0x5c, 0x5c, 0x5c, 0x5c, 0x5c, 0x5c, 0x5c, 0x5c,
...
int CopyDecodedToX509(WOLFSSL_X509 *x509, DecodedCert *dCert)
{
    ...
    if (dCert->issuerName.fullName != NULL) {
        XMEMCPY(&x509->issuer.fullName, &dCert->issuerName, sizeof(DecodedName));
        x509->issuer.fullName.fullName =
            (char *)XMALLOC(dCert->issuerName.fullNameLen, NULL, DYNAMIC_TYPE_X509);
        if (x509->issuer.fullName.fullName != NULL)
            XMEMCPY(x509->issuer.fullName.fullName, dCert->issuerName.fullName,
                    dCert->issuerName.fullNameLen);
    }
#endif /* OPENSSL_EXTRA */
    ...
    if (dCert->subjectName.fullName != NULL) {
        XMEMCPY(&x509->subject.fullName, &dCert->subjectName, sizeof(DecodedName));
        x509->subject.fullName.fullName =
            (char *)XMALLOC(dCert->subjectName.fullNameLen, NULL, DYNAMIC_TYPE_X509);
        if (x509->subject.fullName.fullName != NULL)
            XMEMCPY(x509->subject.fullName.fullName, dCert->subjectName.fullName,
                    dCert->subjectName.fullNameLen);
    }
#endif /* OPENSSL_EXTRA */
    ...
    if (dCert->publicKey != NULL && dCert->pubKeySize != 0) {
        x509->pubKey.buffer = (byte *)XMALLOC(dCert->pubKeySize, NULL, DYNAMIC_TYPE_PUBLIC_KEY);
    ...
    if (dCert->signature != NULL && dCert->sigLength != 0) {
        x509->sig.buffer = (byte *)XMALLOC(dCert->sigLength, NULL, DYNAMIC_TYPE_SIGNATURE);
    ...
    x509->derCert.buffer = (byte *)XMALLOC(dCert->maxIdx, NULL, DYNAMIC_TYPE_CERT);
...
static int DoCertificate(WOLFSSL *ssl, byte *input, word32 *inOutIdx, word32 size)
{
    ...
    *inOutIdx += OPAQUE24_LEN;
    ...
    while (listSz) {
        ...
        *inOutIdx += OPAQUE24_LEN;
        ...
        if (ssl->session.chain.count < MAX_CHAIN_DEPTH && certSz < MAX_X509_SIZE) {
            ...
            XMEMCPY(ssl->session.chain.certs[ssl->session.chain.count].buffer, input + *inOutIdx,
                    certSz);
    ...
    while (count > 1) {
        ...
        ret = ParseCertRelative(dCert, CERT_TYPE, !ssl->options.verifyNone, ssl->ctx->cm);
#ifndef NO_SKID
        ...
        if (ret == 0 && dCert->isCA == 0) {
            ...
            add.buffer = (byte *)XMALLOC(myCert.length, ssl->heap, DYNAMIC_TYPE_CA);
            ...
            ret = AddCA(ssl->ctx->cm, add, WOLFSSL_CHAIN_CA, ssl->ctx->verifyPeer);
            if (ret == 1)
...
#ifdef HAVE_CRL
            if (doCrlLookup && ssl->ctx->cm->crlEnabled && ssl->ctx->cm->crlCheckAll) {
    ...
    if (count) {
...
#ifdef HAVE_SECURE_RENEGOTIATION
        if (fatal == 0 && ssl->secure_renegotiation && ssl->secure_renegotiation->enabled) {
            if (ssl->keys.encryptionOn) {
                /* compare against previous time */
                if (XMEMCMP(dCert->subjectHash, ssl->secure_renegotiation->subject_hash,
                            SHA_DIGEST_SIZE) != 0) {
            ...
            /* cache peer's hash */
            if (fatal == 0) {
                XMEMCPY(ssl->secure_renegotiation->subject_hash, dCert->subjectHash,
                        SHA_DIGEST_SIZE);
            }
        }
#endif
        ...
        if (dCert->extKeyUsageSet) {
            if ((ssl->specs.kea == rsa_kea) && (dCert->extKeyUsage & KEYUSE_KEY_ENCIPHER) == 0) {
                ret = KEYUSE_ENCIPHER_E;
            }
            if ((ssl->specs.sig_algo == rsa_sa_algo ||
                 (ssl->specs.sig_algo == ecc_dsa_sa_algo && !ssl->specs.static_ecdh)) &&
        ...
        if (dCert->extExtKeyUsageSet) {
            if (ssl->options.side == WOLFSSL_CLIENT_END) {
                if ((dCert->extExtKeyUsage & (EXTKEYUSE_ANY | EXTKEYUSE_SERVER_AUTH)) == 0) {
                    WOLFSSL_MSG("ExtKeyUse Server Auth not set");
                    ret = EXTKEYUSE_AUTH_E;
                }
            } else {
                if ((dCert->extExtKeyUsage & (EXTKEYUSE_ANY | EXTKEYUSE_CLIENT_AUTH)) == 0) {
        ...
        if (!ssl->options.verifyNone && ssl->buffers.domainName.buffer) {
            if (MatchDomainName(dCert->subjectCN, dCert->subjectCNLen,
                ...
                if (CheckAltNames(dCert, (char *)ssl->buffers.domainName.buffer) == 0) {
        ...
        case RSAk: {
            ...
            if (ssl->peerRsaKey == NULL) {
                ssl->peerRsaKey = (RsaKey *)XMALLOC(sizeof(RsaKey), ssl->heap, DYNAMIC_TYPE_RSA);
                if (ssl->peerRsaKey == NULL) {
                    WOLFSSL_MSG("PeerRsaKey Memory error");
                    keyRet = MEMORY_E;
                } else {
                    keyRet = wc_InitRsaKey(ssl->peerRsaKey, ssl->ctx->heap);
                }
            ...
            if (keyRet != 0 || wc_RsaPublicKeyDecode(dCert->publicKey, &idx, ssl->peerRsaKey,
                                                     dCert->pubKeySize) != 0) {
                ret = PEER_KEY_ERROR;
            } else {
                ...
                ssl->buffers.peerRsaKey.buffer =
                    (byte *)XMALLOC(dCert->pubKeySize, ssl->heap, DYNAMIC_TYPE_RSA);
                if (ssl->buffers.peerRsaKey.buffer == NULL)
                    ret = MEMORY_ERROR;
                else {
                    XMEMCPY(ssl->buffers.peerRsaKey.buffer, dCert->publicKey, dCert->pubKeySize);
                    ssl->buffers.peerRsaKey.length = dCert->pubKeySize;
                }
#endif /* NO_RSA */
#endif /*HAVE_PK_CALLBACKS */
            }
        ...
        case ECDSAk: {
            if (ssl->peerEccDsaKey == NULL) {
                ...
                ssl->peerEccDsaKey =
                    (ecc_key *)XMALLOC(sizeof(ecc_key), ssl->ctx->heap, DYNAMIC_TYPE_ECC);
                if (ssl->peerEccDsaKey == NULL) {
            ...
            if (wc_ecc_import_x963(dCert->publicKey, dCert->pubKeySize, ssl->peerEccDsaKey) != 0) {
                ret = PEER_KEY_ERROR;
            } else {
                ...
                ssl->buffers.peerEccDsaKey.buffer =
                    (byte *)XMALLOC(dCert->pubKeySize, ssl->heap, DYNAMIC_TYPE_ECC);
                if (ssl->buffers.peerEccDsaKey.buffer == NULL)
                    ret = MEMORY_ERROR;
                else {
                    XMEMCPY(ssl->buffers.peerEccDsaKey.buffer, dCert->publicKey, dCert->pubKeySize);
                    ssl->buffers.peerEccDsaKey.length = dCert->pubKeySize;
                }
#endif /* HAVE_ECC */
#endif /*HAVE_PK_CALLBACKS */
            }
    ...
    store = (WOLFSSL_X509_STORE_CTX *)XMALLOC(sizeof(WOLFSSL_X509_STORE_CTX), NULL,
    ...
    if (ssl->options.verifyNone && (ret == CRL_MISSING || ret == CRL_CERT_REVOKED)) {
...
static int DoHelloRequest(WOLFSSL *ssl, const byte *input, word32 *inOutIdx, word32 size,
                          word32 totalSz)
{
    (void)input;
...
int DoFinished(WOLFSSL *ssl, const byte *input, word32 *inOutIdx, word32 size, word32 totalSz,
               int sniff)
{
    ...
    if (ssl->secure_renegotiation) {
        /* save peer's state */
        if (ssl->options.side == WOLFSSL_CLIENT_END)
            XMEMCPY(ssl->secure_renegotiation->server_verify_data, input + *inOutIdx,
                    TLS_FINISHED_SZ);
        else
            XMEMCPY(ssl->secure_renegotiation->client_verify_data, input + *inOutIdx,
                    TLS_FINISHED_SZ);
    }
#endif
...
static int SanityCheckMsgReceived(WOLFSSL *ssl, byte type)
{
    ...
    case server_hello_done:
        ...
        if (ssl->msgsReceived.got_certificate == 0) {
            if (ssl->specs.kea == psk_kea || ssl->specs.kea == dhe_psk_kea ||
        ...
        if (ssl->msgsReceived.got_server_key_exchange == 0) {
            if (ssl->specs.static_ecdh == 1 || ssl->specs.kea == rsa_kea ||
...
#ifndef NO_WOLFSSL_CLIENT
        if (ssl->options.side == WOLFSSL_CLIENT_END) {
            if (!ssl->options.resuming && ssl->msgsReceived.got_server_hello_done == 0) {
...
static int DoHandShakeMsgType(WOLFSSL *ssl, byte *input, word32 *inOutIdx, byte type, word32 size,
                              word32 totalSz)
{
    int ret = 0;
    (void)totalSz;
    ...
    if (ssl->options.side == WOLFSSL_CLIENT_END && ssl->options.dtls && type == server_hello_done &&
...
static int DtlsMsgDrain(WOLFSSL *ssl)
{
     ...
     * last messge... */
    while (item != NULL && ssl->keys.dtls_expected_peer_handshake_number == item->seq &&
           item->fragSz == item->sz && ret == 0) {
        word32 idx = 0;
...
static int DoDtlsHandShakeMsg(WOLFSSL *ssl, byte *input, word32 *inOutIdx, word32 totalSz)
{
    ...
    if (GetDtlsHandShakeHeader(ssl, input, inOutIdx, &type, &size, &fragOffset, &fragSz, totalSz) !=
        0)
        return PARSE_ERROR;
     ...
     */
    if (ssl->keys.dtls_peer_handshake_number > ssl->keys.dtls_expected_peer_handshake_number) {
        /* Current message is out of order. It will get stored in the list.
         * Storing also takes care of defragmentation. */
        ssl->dtls_msg_list = DtlsMsgStore(ssl->dtls_msg_list, ssl->keys.dtls_peer_handshake_number,
                                          input + *inOutIdx, size, type, fragOffset, fragSz,
                                          ssl->heap);
        *inOutIdx += fragSz;
        ret = 0;
        ...
        *inOutIdx += fragSz;
        ret = 0;
        if (ssl->dtls_msg_list != NULL && ssl->dtls_msg_list->fragSz >= ssl->dtls_msg_list->sz)
...
static int Poly1305Tag(WOLFSSL *ssl, byte *additional, const byte *out, byte *cipher, word16 sz,
                       byte *tag)
{
    int ret = 0;
    int paddingSz = 0;
    ...
    if (msglen % 16 != 0) {
        ...
        if ((ret = wc_Poly1305Update(ssl->auth.poly1305, padding, paddingSz)) != 0)
            return ret;
    }
...
static int Poly1305TagOld(WOLFSSL *ssl, byte *additional, const byte *out, byte *cipher, word16 sz,
                          byte *tag)
{
    int ret = 0;
    int msglen = (sz - ssl->specs.aead_mac_size);
...
static int ChachaAEADEncrypt(WOLFSSL *ssl, byte *out, const byte *input, word16 sz)
{
    ...
    /* get nonce */
    c32toa(ssl->keys.sequence_number, nonce + AEAD_IMP_IV_SZ + AEAD_SEQ_OFFSET);
...
/* Store the type, version. Unfortunately, they are in
    ...
    if ((ret = wc_Chacha_Process(ssl->encrypt.chacha, cipher, cipher, sizeof(cipher))) != 0)
        return ret;

    /* encrypt the plain text */
    if ((ret = wc_Chacha_Process(ssl->encrypt.chacha, out, input, sz - ssl->specs.aead_mac_size)) !=
        0)
        return ret;
...
static int ChachaAEADDecrypt(WOLFSSL *ssl, byte *plain, const byte *input, word16 sz)
{
    ...
    /* get nonce */
    c32toa(ssl->keys.peer_sequence_number, nonce + AEAD_IMP_IV_SZ + AEAD_SEQ_OFFSET);
...
/* Store the type, version. */
    ...
    if ((ret = wc_Chacha_Process(ssl->decrypt.chacha, cipher, cipher, sizeof(cipher))) != 0)
        return ret;
    ...
    if (ConstantCompare(input + sz - ssl->specs.aead_mac_size, tag, ssl->specs.aead_mac_size) !=
        0) {
...
/* Store the type, version. Unfortunately, they are in
        ...
        ret = wc_AesGcmEncrypt(ssl->encrypt.aes, out + AEAD_EXP_IV_SZ, input + AEAD_EXP_IV_SZ,
                               ...
                               out + sz - ssl->specs.aead_mac_size, ssl->specs.aead_mac_size,
...
/* Store the type, version. Unfortunately, they are in
        ...
        wc_AesCcmEncrypt(ssl->encrypt.aes, out + AEAD_EXP_IV_SZ, input + AEAD_EXP_IV_SZ,
                         sz - AEAD_EXP_IV_SZ - ssl->specs.aead_mac_size, nonce, AEAD_NONCE_SZ,
                         out + sz - ssl->specs.aead_mac_size, ssl->specs.aead_mac_size, additional,
                         AEAD_AUTH_DATA_SZ);
        AeadIncrementExpIV(ssl);
        ForceZero(nonce, AEAD_NONCE_SZ);
    } break;
...
static INLINE int Decrypt(WOLFSSL *ssl, byte *plain, const byte *input, word16 sz)
{
    int ret = 0;
    ...
    case wolfssl_aes_gcm: {
        ...
        if (wc_AesGcmDecrypt(ssl->decrypt.aes, plain + AEAD_EXP_IV_SZ, input + AEAD_EXP_IV_SZ,
                             sz - AEAD_EXP_IV_SZ - ssl->specs.aead_mac_size, nonce, AEAD_NONCE_SZ,
                             input + sz - ssl->specs.aead_mac_size, ssl->specs.aead_mac_size,
...
static int SanityCheckCipherText(WOLFSSL *ssl, word32 encryptSz)
{
    ...
    word32 minLength = ssl->truncated_hmac ? TRUNCATED_HMAC_SZ : ssl->specs.hash_size;
#else
...
static INLINE void Md5Rounds(int rounds, const byte *data, int sz)
{
    ...
    for (i = 0; i < rounds; i++) wc_Md5Update(&md5, data, sz);
}
...
static INLINE void ShaRounds(int rounds, const byte *data, int sz)
{
    ...
    for (i = 0; i < rounds; i++) wc_ShaUpdate(&sha, data, sz);
}
#endif
...
static INLINE void RmdRounds(int rounds, const byte *data, int sz)
{
    ...
    for (i = 0; i < rounds; i++) wc_RipeMdUpdate(&ripemd, data, sz);
}
...
static int TimingPadVerify(WOLFSSL *ssl, const byte *input, int padLen, int t, int pLen,
                           int content)
{
...
static int DoAlert(WOLFSSL *ssl, byte *input, word32 *inOutIdx, int *type, word32 totalSz)
{
    byte level;
    byte code;
    ...
    if (ssl->toInfoOn)
        ...
        AddPacketInfo("Alert", &ssl->timeoutInfo, input + *inOutIdx - RECORD_HEADER_SZ,
                      2 + RECORD_HEADER_SZ, ssl->heap);
#endif
...
static int GetInputData(WOLFSSL *ssl, word32 size)
{
    ...
    do {
        in = Receive(ssl, ssl->buffers.inputBuffer.buffer + ssl->buffers.inputBuffer.length, inSz);
        if (in == -1)
            return SOCKET_ERROR_E;
...
static INLINE int VerifyMac(WOLFSSL *ssl, const byte *input, word32 msgSz, int content,
                            word32 *padSz)
{
    int ivExtra = 0;
    int ret;
    ...
    word32 padByte = 0;
#ifdef HAVE_TRUNCATED_HMAC
    word32 digestSz = ssl->truncated_hmac ? TRUNCATED_HMAC_SZ : ssl->specs.hash_size;
#else
    ...
    if (ssl->specs.cipher_type == block) {
        ...
        if (ssl->options.tls) {
            ...
            ret = ssl->hmac(ssl, verify, input, msgSz - digestSz - pad - 1, content, 1);
    ...
    for (;;) {
        ...
        case doProcessInit:
            ...
            if (ssl->options.side == WOLFSSL_SERVER_END && ssl->options.clientState == NULL_STATE &&
                ...
                if (ssl->buffers.inputBuffer.buffer[ssl->buffers.inputBuffer.idx + 2] !=
                    OLD_HELLO_ID) {
                    WOLFSSL_MSG("Not a valid old client hello");
                    return PARSE_ERROR;
                }

                if (ssl->buffers.inputBuffer.buffer[ssl->buffers.inputBuffer.idx + 3] !=
                        SSLv3_MAJOR &&
        ...
        case runProcessOldClientHello:
            ...
            ret = ProcessOldClientHello(
                ssl, ssl->buffers.inputBuffer.buffer, &ssl->buffers.inputBuffer.idx,
        ...
        case getRecordLayerHeader:
            ...
            ret = GetRecordHeader(ssl, ssl->buffers.inputBuffer.buffer,
                                  &ssl->buffers.inputBuffer.idx, &ssl->curRL, &ssl->curSize);
#ifdef WOLFSSL_DTLS
        ...
        case runProcessingOneMessage:
            ...
            case change_cipher_spec:
...
#ifdef HAVE_SESSION_TICKET
                if (ssl->options.side == WOLFSSL_CLIENT_END && ssl->expect_session_ticket) {
...
#ifndef NO_CERTS
                if (ssl->options.side == WOLFSSL_SERVER_END && ssl->options.verifyPeer &&
                ...
                ret = BuildFinished(ssl, &ssl->hsHashes->verifyHashes,
                                    ssl->options.side == WOLFSSL_CLIENT_END ? server : client);
                if (ret != 0)
                    return ret;
                break;
            ...
            case alert:
                WOLFSSL_MSG("got ALERT!");
                ret = DoAlert(ssl, ssl->buffers.inputBuffer.buffer, &ssl->buffers.inputBuffer.idx,
                              &type, ssl->buffers.inputBuffer.length);
                if (ret == alert_fatal)
                    return FATAL_ERROR;
                else if (ret < 0)
                    return ret;
...
int SendChangeCipher(WOLFSSL *ssl)
{
    ...
    if (ssl->keys.encryptionOn && ssl->options.handShakeDone) {
        ...
        sendSz = BuildMessage(ssl, output, sendSz, input, inputSz, change_cipher_spec);
        if (sendSz < 0)
            return sendSz;
    }
    ...
    if (ssl->hsInfoOn)
        AddPacketName("ChangeCipher", &ssl->handShakeInfo);
    if (ssl->toInfoOn)
        AddPacketInfo("ChangeCipher", &ssl->timeoutInfo, output, sendSz, ssl->heap);
#endif
                        ...
                        int type)
{
#ifdef HAVE_TRUNCATED_HMAC
    word32 digestSz =
        min(ssl->specs.hash_size, ssl->truncated_hmac ? TRUNCATED_HMAC_SZ : ssl->specs.hash_size);
#else
    word32 digestSz = ssl->specs.hash_size;
#endif
    ...
    if (ssl->specs.cipher_type == block) {
        ...
        for (i = 0; i <= pad; i++) output[tmpIdx++] = (byte)pad; /* pad byte gets pad value too */
    }

    if (atomicUser) { /* User Record Layer Callback handling */
#ifdef ATOMIC_USER
        if ((ret = ssl->ctx->MacEncryptCb(ssl, output + idx, output + headerSz + ivSz, inSz, type,
                                          0, output + headerSz, output + headerSz, size,
                                          ssl->MacEncryptCtx)) != 0)
            return ret;
#endif
    } else {
...
{
    int sendSz, finishedSz = ssl->options.tls ? TLS_FINISHED_SZ : FINISHED_SZ;
    ...
    ret = BuildFinished(ssl, hashes, ssl->options.side == WOLFSSL_CLIENT_END ? client : server);
    if (ret != 0)
        return ret;
    ...
    if (ssl->secure_renegotiation) {
        if (ssl->options.side == WOLFSSL_CLIENT_END)
            ...
            XMEMCPY(ssl->secure_renegotiation->server_verify_data, hashes, TLS_FINISHED_SZ);
    }
#endif

    sendSz = BuildMessage(ssl, output, outputSz, input, headerSz + finishedSz, handshake);
    if (sendSz < 0)
    ...
    if (ssl->hsInfoOn)
        AddPacketName("Finished", &ssl->handShakeInfo);
    if (ssl->toInfoOn)
        AddPacketInfo("Finished", &ssl->timeoutInfo, output, sendSz, ssl->heap);
#endif
...
int SendCertificate(WOLFSSL *ssl)
{
    ...
    if (certSz) {
        ...
        /* send rest of chain? */
        if (ssl->buffers.certChain.buffer) {
            XMEMCPY(output + i, ssl->buffers.certChain.buffer, ssl->buffers.certChain.length);
    ...
    if (ssl->hsInfoOn)
        AddPacketName("Certificate", &ssl->handShakeInfo);
    if (ssl->toInfoOn)
        AddPacketInfo("Certificate", &ssl->timeoutInfo, output, sendSz, ssl->heap);
#endif
...
int SendCertificateRequest(WOLFSSL *ssl)
{
...
#ifdef HAVE_ECC
    if (ssl->options.cipherSuite0 == ECC_BYTE && ssl->specs.sig_algo == ecc_dsa_sa_algo) {
        output[i++] = ecdsa_sign;
    } else
#endif /* HAVE_ECC */
    {
    ...
    if (ssl->toInfoOn)
        AddPacketInfo("CertificateRequest", &ssl->timeoutInfo, output, sendSz, ssl->heap);
#endif
    ssl->buffers.outputBuffer.length += sendSz;
    if (ssl->options.groupMessages)
        return 0;
...
void InitHandShakeInfo(HandShakeInfo *info)
{
    ...
    info->cipherName[0] = 0;
    for (i = 0; i < MAX_PACKETS_HANDSHAKE; i++) info->packetNames[i][0] = 0;
...
int SendClientHello(WOLFSSL *ssl)
{
    ...
    length = VERSION_SZ + RAN_LEN + idSz + ENUM_LEN + ssl->suites->suiteSz + SUITE_LEN + COMP_LEN +
             ENUM_LEN;
...
static int DoHelloVerifyRequest(WOLFSSL *ssl, const byte *input, word32 *inOutIdx, word32 size)
{
    ProtocolVersion pv;
    byte cookieSz;
    word32 begin = *inOutIdx;
...
static INLINE int DSH_CheckSessionId(WOLFSSL *ssl)
{
    ...
    ret = ret || (ssl->options.haveSessionId &&
                  XMEMCMP(ssl->arrays->sessionID, ssl->session.sessionID, ID_LEN) == 0);

    return ret;
}

static int DoServerHello(WOLFSSL *ssl, const byte *input, word32 *inOutIdx, word32 helloSz)
{
    ...
    *inOutIdx = i;
    ...
    if ((i - begin) < helloSz) {
        ...
        if (TLSX_SupportExtensions(ssl)) {
            ...
            if ((ret = TLSX_Parse(ssl, (byte *)input + i, totalExtSz, 0, NULL)))
                return ret;
    ...
    if (ssl->sessionSecretCb != NULL) {
        ...
        ret =
            ssl->sessionSecretCb(ssl, ssl->session.masterSecret, &secretSz, ssl->sessionSecretCtx);
...
#define ERROR_OUT(err, eLabel) \
    ...
    if (ssl->specs.kea == psk_kea) {
        ...
        *inOutIdx += OPAQUE16_LEN;
        ...
        XMEMCPY(ssl->arrays->server_hint, input + *inOutIdx, min(length, MAX_PSK_ID_LEN));
    ...
    if (ssl->specs.kea == diffie_hellman_kea) {
        ...
        *inOutIdx += OPAQUE16_LEN;
        ...
        ssl->buffers.serverDH_P.buffer = (byte *)XMALLOC(length, ssl->heap, DYNAMIC_TYPE_DH);
        ...
        *inOutIdx += OPAQUE16_LEN;
        ...
        ssl->buffers.serverDH_G.buffer = (byte *)XMALLOC(length, ssl->heap, DYNAMIC_TYPE_DH);
        ...
        *inOutIdx += OPAQUE16_LEN;
        ...
        ssl->buffers.serverDH_Pub.buffer = (byte *)XMALLOC(length, ssl->heap, DYNAMIC_TYPE_DH);
    ...
    if (ssl->specs.kea == ecc_diffie_hellman_kea) {
        ...
        *inOutIdx += 1; /* curve type, eat leading 0 */
        ...
        if (ssl->peerEccKey == NULL) {
            /* alloc/init on demand */
            ssl->peerEccKey = (ecc_key *)XMALLOC(sizeof(ecc_key), ssl->ctx->heap, DYNAMIC_TYPE_ECC);
            if (ssl->peerEccKey == NULL) {
    ...
    if (!ssl->options.usingAnon_cipher &&
        (ssl->specs.kea == ecc_diffie_hellman_kea || ssl->specs.kea == diffie_hellman_kea)) {
#ifndef NO_OLD_TLS
#ifdef WOLFSSL_SMALL_STACK
        Md5 *md5 = NULL;
        Sha *sha = NULL;
...
#ifdef WOLFSSL_SMALL_STACK
        messageVerify = (byte *)XMALLOC(MAX_DH_SZ, NULL, DYNAMIC_TYPE_TMP_BUFFER);
...
/* only using sha and md5 for rsa */
        ...
        *inOutIdx += OPAQUE16_LEN;
        ...
        if (doSha256) {
            ...
            hash256 = (byte *)XMALLOC(SHA256_DIGEST_SIZE, NULL, DYNAMIC_TYPE_TMP_BUFFER);
        ...
        if (sigAlgo == rsa_sa_algo) {
            ...
            if (doUserRsa) {
#ifdef HAVE_PK_CALLBACKS
                verifiedSz = ssl->ctx->RsaVerifyCb(ssl, (byte *)input + *inOutIdx, length, &out,
                                                   ssl->buffers.peerRsaKey.buffer,
                                                   ssl->buffers.peerRsaKey.length,
                                                   ssl->RsaVerifyCtx);
#endif /*HAVE_PK_CALLBACKS */
            } else
                verifiedSz = wc_RsaSSL_VerifyInline((byte *)input + *inOutIdx, length, &out,
                                                    ssl->peerRsaKey);
            ...
            if (IsAtLeastTLSv1_2(ssl)) {
                ...
                if (encSigSz != verifiedSz || !out ||
                    XMEMCMP(out, encodedSig, min(encSigSz, MAX_ENCODED_SIG_SZ)) != 0)
                ...
                if (ret != 0)
                    goto done;
            } else if (verifiedSz != FINISHED_SZ || !out || XMEMCMP(out, hash, FINISHED_SZ) != 0)
            ...
            if (sigAlgo == ecc_dsa_sa_algo) {
                ...
                if (doUserEcc) {
#ifdef HAVE_PK_CALLBACKS
                    ret = ssl->ctx->EccVerifyCb(ssl, input + *inOutIdx, length, digest, digestSz,
                                                ssl->buffers.peerEccDsaKey.buffer,
                                                ssl->buffers.peerEccDsaKey.length, &verify,
                                                ssl->EccVerifyCtx);
#endif
                } else {
                    ret = wc_ecc_verify_hash(input + *inOutIdx, length, digest, digestSz, &verify,
                                             ssl->peerEccDsaKey);
                }
                if (ret != 0 || verify == 0)
                    ERROR_OUT(VERIFY_SIGN_ERROR, done);
...
int SendClientKeyExchange(WOLFSSL *ssl)
{
...
#ifdef WOLFSSL_SMALL_STACK
    encSecret = (byte *)XMALLOC(MAX_ENCRYPT_SZ, NULL, DYNAMIC_TYPE_TMP_BUFFER);
    if (encSecret == NULL)
        return MEMORY_E;
#endif
    ...
    case rsa_kea:
        ...
        if (doUserRsa) {
            ...
            ret = ssl->ctx->RsaEncCb(ssl, ssl->arrays->preMasterSecret, SECRET_LEN, encSecret,
                                     &encSz, ssl->buffers.peerRsaKey.buffer,
                                     ssl->buffers.peerRsaKey.length, ssl->RsaEncCtx);
#endif /* NO_RSA */
#endif /*HAVE_PK_CALLBACKS */
        } else {
            ret = wc_RsaPublicEncrypt(ssl->arrays->preMasterSecret, SECRET_LEN, encSecret,
                                      MAX_ENCRYPT_SZ, ssl->peerRsaKey, ssl->rng);
            if (ret > 0) {
                encSz = ret;
    ...
    case diffie_hellman_kea: {
        ...
        if (serverP.buffer == 0 || serverG.buffer == 0 || serverPub.buffer == 0) {
#ifdef WOLFSSL_SMALL_STACK
            XFREE(encSecret, NULL, DYNAMIC_TYPE_TMP_BUFFER);
#endif
            return NO_PEER_KEY;
...
#ifdef WOLFSSL_SMALL_STACK
        priv = (byte *)XMALLOC(ENCRYPT_LEN, NULL, DYNAMIC_TYPE_TMP_BUFFER);
        if (priv == NULL) {
            XFREE(encSecret, NULL, DYNAMIC_TYPE_TMP_BUFFER);
            return MEMORY_E;
        }
        ...
        wc_InitDhKey(&key);
        ret = wc_DhSetKey(&key, serverP.buffer, serverP.length, serverG.buffer, serverG.length);
        if (ret == 0)
            /* for DH, encSecret is Yc, agree is pre-master */
            ret = wc_DhGenerateKeyPair(&key, ssl->rng, priv, &privSz, encSecret, &encSz);
        if (ret == 0)
            ret = wc_DhAgree(&key, ssl->arrays->preMasterSecret, &ssl->arrays->preMasterSz, priv,
                             privSz, serverPub.buffer, serverPub.length);
#ifdef WOLFSSL_SMALL_STACK
    ...
    case psk_kea: {
        ...
        ssl->arrays->psk_keySz =
            ssl->options.client_psk_cb(ssl, ssl->arrays->server_hint, ssl->arrays->client_identity,
    ...
    case dhe_psk_kea: {
        ...
        if (ret == 0)
            /* for DH, encSecret is Yc, agree is pre-master */
            ret = wc_DhGenerateKeyPair(&key, ssl->rng, priv, &privSz, es + OPAQUE16_LEN, &pubSz);
        if (ret == 0)
            ret = wc_DhAgree(&key, pms + OPAQUE16_LEN, &ssl->arrays->preMasterSz, priv, privSz,
    ...
    case ntru_kea: {
        ...
        rc = ntru_crypto_drbg_instantiate(MAX_NTRU_BITS, wolfsslStr, sizeof(wolfsslStr), GetEntropy,
                                          &drbg);
        if (rc != DRBG_OK) {
#ifdef WOLFSSL_SMALL_STACK
            XFREE(encSecret, NULL, DYNAMIC_TYPE_TMP_BUFFER);
        ...
        rc = ntru_crypto_ntru_encrypt(drbg, ssl->peerNtruKeyLen, ssl->peerNtruKey,
    ...
    case ecc_diffie_hellman_kea: {
        ...
        if (ssl->specs.static_ecdh) {
            ...
            if (!ssl->peerEccDsaKey || !ssl->peerEccDsaKeyPresent || !ssl->peerEccDsaKey->dp) {
#ifdef WOLFSSL_SMALL_STACK
                XFREE(encSecret, NULL, DYNAMIC_TYPE_TMP_BUFFER);
#endif
                return NO_PEER_KEY;
        ...
        if (ret != 0)
            ...
            ret = wc_ecc_shared_secret(&myKey, peerKey, ssl->arrays->preMasterSecret, &size);
            if (ret != 0)
    ...
    if (ret == 0) {
        ...
        if (ssl->toInfoOn)
            AddPacketInfo("ClientKeyExchange", &ssl->timeoutInfo, output, sendSz, ssl->heap);
#endif
...
int SendCertificateVerify(WOLFSSL *ssl)
{
    ...
    if (ret == 0)
        initRsaKey = 1;
    if (ret == 0)
        ret = wc_RsaPrivateKeyDecode(ssl->buffers.key.buffer, &idx, &key, ssl->buffers.key.length);
    if (ret == 0)
...
#endif
    {
        ...
        idx = 0;
        ret =
            wc_EccPrivateKeyDecode(ssl->buffers.key.buffer, &idx, &eccKey, ssl->buffers.key.length);
        if (ret == 0) {
...
#ifdef HAVE_PK_CALLBACKS
#ifdef HAVE_ECC
                ret = ssl->ctx->EccSignCb(ssl, digest, digestSz, encodedSig, &localSz,
                                          ssl->buffers.key.buffer, ssl->buffers.key.length,
                                          ssl->EccSignCtx);
#endif /* HAVE_ECC */
#endif /*HAVE_PK_CALLBACKS */
            } else {
                ret = wc_ecc_sign_hash(digest, digestSz, encodedSig, &localSz, ssl->rng, &eccKey);
            }
            if (ret == 0) {
                length = localSz;
...
/* defaults */
            ...
            if (doUserRsa) {
                ...
                ret = ssl->ctx->RsaSignCb(ssl, signBuffer, signSz, verify + extraSz + VERIFY_HEADER,
                                          &ioLen, ssl->buffers.key.buffer, ssl->buffers.key.length,
                                          ssl->RsaSignCtx);
#endif /* NO_RSA */
#endif /*HAVE_PK_CALLBACKS */
            } else {
                ret = wc_RsaSSL_Sign(signBuffer, signSz, verify + extraSz + VERIFY_HEADER,
                                     ENCRYPT_LEN, &key, ssl->rng);
            }
        ...
        if (ret == 0) {
            ...
            if (ssl->keys.encryptionOn) {
                ...
                if (input == NULL)
                    ...
                    XMEMCPY(input, output + RECORD_HEADER_SZ, inputSz);
                    sendSz = BuildMessage(ssl, output, MAX_CERT_VERIFY_SZ + MAX_MSG_EXTRA, input,
                                          inputSz, handshake);
                    XFREE(input, ssl->heap, DYNAMIC_TYPE_TMP_BUFFER);
...
#ifdef HAVE_SESSION_TICKET
int DoSessionTicket(WOLFSSL *ssl, const byte *input, word32 *inOutIdx, word32 size)
{
    word32 begin = *inOutIdx;
    word32 lifetime;
    word16 length;
    ...
    if (length > 0) {
        ...
        *inOutIdx += length;
        ...
        if (ssl->session_ticket_cb != NULL) {
            ssl->session_ticket_cb(ssl, ssl->session.ticket, ssl->session.ticketLen,
...
int SendServerHello(WOLFSSL *ssl)
{
...
#ifdef SHOW_SECRETS
    {
        ...
        for (j = 0; j < RAN_LEN; j++) printf("%02x", ssl->arrays->serverRandom[j]);
        printf("\n");
    }
#endif
    /* then session id */
    ...
    if (sessIdSz) {
        if (!ssl->options.resuming) {
            ret = wc_RNG_GenerateBlock(ssl->rng, ssl->arrays->sessionID, sessIdSz);
            if (ret != 0)
                return ret;
        }
    ...
    if (ssl->hsInfoOn)
        AddPacketName("ServerHello", &ssl->handShakeInfo);
    if (ssl->toInfoOn)
        AddPacketInfo("ServerHello", &ssl->timeoutInfo, output, sendSz, ssl->heap);
#endif
...
#define ERROR_OUT(err, eLabel) \
    ...
    if (ssl->specs.kea == psk_kea) {
        ...
        if (ssl->hsInfoOn)
            AddPacketName("ServerKeyExchange", &ssl->handShakeInfo);
        if (ssl->toInfoOn)
            AddPacketInfo("ServerKeyExchange", &ssl->timeoutInfo, output, sendSz, ssl->heap);
#endif
    ...
    if (ssl->specs.kea == dhe_psk_kea) {
        ...
        if (ssl->buffers.serverDH_Pub.buffer == NULL) {
            ssl->buffers.serverDH_Pub.buffer = (byte *)XMALLOC(ssl->buffers.serverDH_P.length + 2,
        ...
        if (ssl->buffers.serverDH_Priv.buffer == NULL) {
            ssl->buffers.serverDH_Priv.buffer = (byte *)XMALLOC(ssl->buffers.serverDH_P.length + 2,
                                                                ssl->ctx->heap, DYNAMIC_TYPE_DH);
            if (ssl->buffers.serverDH_Priv.buffer == NULL)
                return MEMORY_E;
        }
        ...
        wc_InitDhKey(&dhKey);
        ret = wc_DhSetKey(&dhKey, ssl->buffers.serverDH_P.buffer, ssl->buffers.serverDH_P.length,
                          ssl->buffers.serverDH_G.buffer, ssl->buffers.serverDH_G.length);
        if (ret == 0)
            ret = wc_DhGenerateKeyPair(&dhKey, ssl->rng, ssl->buffers.serverDH_Priv.buffer,
        ...
        length = LENGTH_SZ * 3 + /* p, g, pub */
                 ssl->buffers.serverDH_P.length + ssl->buffers.serverDH_G.length +
                 ssl->buffers.serverDH_Pub.length;
        ...
        idx += LENGTH_SZ;
        XMEMCPY(output + idx, ssl->buffers.serverDH_P.buffer, ssl->buffers.serverDH_P.length);
        idx += ssl->buffers.serverDH_P.length;

        /*  g */
        c16toa((word16)ssl->buffers.serverDH_G.length, output + idx);
        idx += LENGTH_SZ;
        XMEMCPY(output + idx, ssl->buffers.serverDH_G.buffer, ssl->buffers.serverDH_G.length);
        idx += ssl->buffers.serverDH_G.length;

        /*  pub */
        c16toa((word16)ssl->buffers.serverDH_Pub.length, output + idx);
        idx += LENGTH_SZ;
        XMEMCPY(output + idx, ssl->buffers.serverDH_Pub.buffer, ssl->buffers.serverDH_Pub.length);
    ...
    if (ssl->specs.kea == ecc_diffie_hellman_kea) {
        ...
        if (ssl->eccTempKey == NULL) {
            /* alloc/init on demand */
            ssl->eccTempKey = (ecc_key *)XMALLOC(sizeof(ecc_key), ssl->ctx->heap, DYNAMIC_TYPE_ECC);
            if (ssl->eccTempKey == NULL) {
        ...
        if (ssl->eccTempKeyPresent == 0) {
            if (wc_ecc_make_key(ssl->rng, ssl->eccTempKeySz, ssl->eccTempKey) != 0) {
                return ECC_MAKEKEY_ERROR;
            }
...
#ifdef WOLFSSL_SMALL_STACK
        exportBuf = (byte *)XMALLOC(MAX_EXPORT_ECC_SZ, NULL, DYNAMIC_TYPE_TMP_BUFFER);
        if (exportBuf == NULL)
            return MEMORY_E;
#endif
        ...
        if (ssl->specs.sig_algo == rsa_sa_algo) {
            ...
            /* ecdsa sig size */
            word32 i = 0;
            ret = wc_EccPrivateKeyDecode(ssl->buffers.key.buffer, &i, &dsaKey,
                                         ssl->buffers.key.length);
            if (ret != 0)
                goto done_a;
...
/* only using sha and md5 for rsa */
...
#ifdef HAVE_FUZZER
        if (ssl->fuzzerCb)
            ssl->fuzzerCb(ssl, output + preSigIdx, preSigSz, FUZZ_SIGNATURE, ssl->fuzzerCtx);
#endif
        ...
        /* do signature */
        {
            ...
            if (ssl->suites->sigAlgo == rsa_sa_algo) {
...
#ifdef HAVE_PK_CALLBACKS
                    word32 ioLen = sigSz;
                    ret = ssl->ctx->RsaSignCb(ssl, signBuffer, signSz, output + idx, &ioLen,
                                              ssl->buffers.key.buffer, ssl->buffers.key.length,
                                              ssl->RsaSignCtx);
#endif /*HAVE_PK_CALLBACKS */
                } else
                    ret =
                        wc_RsaSSL_Sign(signBuffer, signSz, output + idx, sigSz, &rsaKey, ssl->rng);
                ...
                if (doUserEcc) {
#if defined(HAVE_PK_CALLBACKS) && defined(HAVE_ECC)
                    ret = ssl->ctx->EccSignCb(ssl, digest, digestSz, output + LENGTH_SZ + idx, &sz,
                                              ssl->buffers.key.buffer, ssl->buffers.key.length,
                                              ssl->EccSignCtx);
#endif
                } else {
                    ret = wc_ecc_sign_hash(digest, digestSz, output + LENGTH_SZ + idx, &sz,
                                           ssl->rng, &dsaKey);
                }
#ifndef NO_RSA
                wc_FreeRsaKey(&rsaKey);
...
done_a:
    ...
    if (ssl->specs.kea == diffie_hellman_kea) {
        ...
        length = LENGTH_SZ * 3; /* p, g, pub */
        length += ssl->buffers.serverDH_P.length + ssl->buffers.serverDH_G.length +
                  ssl->buffers.serverDH_Pub.length;
...
static int DoClientHello(WOLFSSL *ssl, const byte *input, word32 *inOutIdx, word32 helloSz)
{
    byte b;
    ProtocolVersion pv;
    Suites clSuites;
...
#ifdef SHOW_SECRETS
    {
        ...
        for (j = 0; j < RAN_LEN; j++) printf("%02x", ssl->arrays->clientRandom[j]);
        printf("\n");
    }
#endif
    ...
    if (ssl->options.dtls) {
        ...
        if (b) {
            ...
            if ((ssl->ctx->CBIOCookie(ssl, cookie, COOKIE_SZ, ssl->IOCB_CookieCtx) != COOKIE_SZ) ||
                (b != COOKIE_SZ) || (XMEMCMP(cookie, input + i, b) != 0)) {
                return COOKIE_ERROR;
            }
    ...
    *inOutIdx = i;
...
#ifdef HAVE_TLS_EXTENSIONS
            if ((ret = TLSX_Parse(ssl, (byte *)input + i, totalExtSz, 1, &clSuites)))
                return ret;
    ...
    if (ssl->options.resuming &&
        (!ssl->options.dtls || ssl->options.acceptState == HELLO_VERIFY_SENT)) { /* let's try */
        int ret = -1;
        WOLFSSL_SESSION *session = GetSession(ssl, ssl->arrays->masterSecret);
#ifdef HAVE_SESSION_TICKET
        if (ssl->options.useTicket == 1) {
...
static int DoCertificateVerify(WOLFSSL *ssl, byte *input, word32 *inOutIdx, word32 size)
{
    ...
    *inOutIdx += OPAQUE16_LEN;
    ...
    if (ssl->peerRsaKey != NULL && ssl->peerRsaKeyPresent != 0) {
        ...
        if (doUserRsa) {
#ifdef HAVE_PK_CALLBACKS
            outLen = ssl->ctx->RsaVerifyCb(ssl, input + *inOutIdx, sz, &out,
                                           ssl->buffers.peerRsaKey.buffer,
                                           ssl->buffers.peerRsaKey.length, ssl->RsaVerifyCtx);
#endif /*HAVE_PK_CALLBACKS */
        } else {
            outLen = wc_RsaSSL_VerifyInline(input + *inOutIdx, sz, &out, ssl->peerRsaKey);
        }
        ...
        if (IsAtLeastTLSv1_2(ssl)) {
            ...
            if (outLen == (int)sigSz && out &&
                XMEMCMP(out, encodedSig, min(sigSz, MAX_ENCODED_SIG_SZ)) == 0)
                ret = 0; /* verified */
            ...
            if (outLen == FINISHED_SZ && out &&
                XMEMCMP(out, &ssl->hsHashes->certHashes, FINISHED_SZ) == 0) {
                ret = 0; /* verified */
            }
        }
    }
    ...
    if (ssl->peerEccDsaKeyPresent) {
        ...
        if (doUserEcc) {
#ifdef HAVE_PK_CALLBACKS
            ret = ssl->ctx->EccVerifyCb(ssl, input + *inOutIdx, sz, digest, digestSz,
                                        ssl->buffers.peerEccDsaKey.buffer,
                                        ssl->buffers.peerEccDsaKey.length, &verify,
                                        ssl->EccVerifyCtx);
#endif
        } else {
            err = wc_ecc_verify_hash(input + *inOutIdx, sz, digest, digestSz, &verify,
                                     ssl->peerEccDsaKey);
        }
...
int SendServerHelloDone(WOLFSSL *ssl)
{
    ...
    if (ssl->hsInfoOn)
        AddPacketName("ServerHelloDone", &ssl->handShakeInfo);
    if (ssl->toInfoOn)
        AddPacketInfo("ServerHelloDone", &ssl->timeoutInfo, output, sendSz, ssl->heap);
#endif
    ssl->options.serverState = SERVER_HELLODONE_COMPLETE;
...
#define WOLFSSL_TICKET_FIXED_SZ \
    (WOLFSSL_TICKET_NAME_SZ + WOLFSSL_TICKET_IV_SZ + WOLFSSL_TICKET_MAC_SZ + LENGTH_SZ)
...
static int CreateTicket(WOLFSSL *ssl)
{
    ...
    ret = ssl->ctx->ticketEncCb(ssl, et->key_name, et->iv, et->mac, 1, et->enc_ticket,
                                sizeof(InternalTicket), &encLen, ssl->ctx->ticketEncCtx);
    if (ret == WOLFSSL_TICKET_RET_OK) {
        if (encLen < (int)sizeof(InternalTicket) || encLen > WOLFSSL_TICKET_ENC_SZ) {
...
int DoClientTicket(WOLFSSL *ssl, const byte *input, word32 len)
{
    ...
    ret = ssl->ctx->ticketEncCb(ssl, et->key_name, et->iv, et->enc_ticket + inLen, 0,
                                et->enc_ticket, inLen, &outLen, ssl->ctx->ticketEncCtx);
...
int SendHelloVerifyRequest(WOLFSSL *ssl)
{
    ...
    if ((ret = ssl->ctx->CBIOCookie(ssl, output + idx, cookieSz, ssl->IOCB_CookieCtx)) < 0)
        return ret;
    ...
    if (ssl->hsInfoOn)
        AddPacketName("HelloVerifyRequest", &ssl->handShakeInfo);
    if (ssl->toInfoOn)
        AddPacketInfo("HelloVerifyRequest", &ssl->timeoutInfo, output, sendSz, ssl->heap);
#endif
    ssl->options.serverState = SERVER_HELLOVERIFYREQUEST_COMPLETE;
...
static int DoClientKeyExchange(WOLFSSL *ssl, byte *input, word32 *inOutIdx, word32 size)
{
    ...
    case rsa_kea: {
        ...
        if (ret == 0) {
            ...
            if (doUserRsa) {
                ...
                ret = wc_RsaPrivateDecryptInline(input + *inOutIdx, length, &out, &key);
            }
    ...
    case psk_kea: {
        ...
        *inOutIdx += ci_sz;
        ...
        ssl->arrays->psk_keySz = ssl->options.server_psk_cb(ssl, ssl->arrays->client_identity,
    ...
    case ntru_kea: {
        ...
        *inOutIdx += OPAQUE16_LEN;
        ...
        if (NTRU_OK != ntru_crypto_ntru_decrypt(
                           (word16)ssl->buffers.key.length, ssl->buffers.key.buffer, cipherLen,
                           input + *inOutIdx, &plainLen, ssl->arrays->preMasterSecret))
    ...
    case ecc_diffie_hellman_kea: {
        ...
        *inOutIdx += length;
        ...
        if (ssl->specs.static_ecdh) {
            ...
            ret = wc_EccPrivateKeyDecode(ssl->buffers.key.buffer, &i, &staticKey,
                                         ssl->buffers.key.length);
    ...
    case diffie_hellman_kea: {
        ...
        *inOutIdx += OPAQUE16_LEN;
        ...
        ret = wc_DhSetKey(&dhKey, ssl->buffers.serverDH_P.buffer, ssl->buffers.serverDH_P.length,
                          ssl->buffers.serverDH_G.buffer, ssl->buffers.serverDH_G.length);
        if (ret == 0)
            ret = wc_DhAgree(&dhKey, ssl->arrays->preMasterSecret, &ssl->arrays->preMasterSz,
                             ssl->buffers.serverDH_Priv.buffer, ssl->buffers.serverDH_Priv.length,
    ...
    case dhe_psk_kea: {
        ...
        *inOutIdx += OPAQUE16_LEN;
        ...
        XMEMCPY(ssl->arrays->client_identity, input + *inOutIdx, clientSz);
        *inOutIdx += clientSz;
        ssl->arrays->client_identity[min(clientSz, MAX_PSK_ID_LEN - 1)] = 0;
        ...
        *inOutIdx += OPAQUE16_LEN;
        ...
        ret = wc_DhSetKey(&dhKey, ssl->buffers.serverDH_P.buffer, ssl->buffers.serverDH_P.length,
                          ssl->buffers.serverDH_G.buffer, ssl->buffers.serverDH_G.length);
        if (ret == 0)
            ret = wc_DhAgree(&dhKey, pms + OPAQUE16_LEN, &ssl->arrays->preMasterSz,
                             ssl->buffers.serverDH_Priv.buffer, ssl->buffers.serverDH_Priv.length,
