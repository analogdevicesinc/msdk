...
#if !defined(USE_WINDOWS_API) && !defined(NO_WOLFSSL_DIR) && !defined(EBSNET)
#include <dirent.h>
#include <sys/stat.h>
#endif
#ifdef EBSNET
...
{
#ifdef WOLFSSL_DTLS
    if (peer != NULL && peerSz != NULL && *peerSz >= ssl->buffers.dtlsCtx.peer.sz) {
...
int wolfSSL_SetTmpDH(WOLFSSL *ssl, const unsigned char *p, int pSz, const unsigned char *g, int gSz)
{
    ...
    ssl->buffers.serverDH_P.buffer = (byte *)XMALLOC(pSz, ssl->ctx->heap, DYNAMIC_TYPE_DH);
    if (ssl->buffers.serverDH_P.buffer == NULL)
        return MEMORY_E;

    ssl->buffers.serverDH_G.buffer = (byte *)XMALLOC(gSz, ssl->ctx->heap, DYNAMIC_TYPE_DH);
...
int wolfSSL_CTX_SetTmpDH(WOLFSSL_CTX *ctx, const unsigned char *p, int pSz, const unsigned char *g,
                         int gSz)
{
...
static int wolfSSL_read_internal(WOLFSSL *ssl, void *data, int sz, int peek)
{
    ...
    ret = ReceiveData(ssl, (byte *)data, min(sz, min(ssl->max_fragment, OUTPUT_RECORD_SIZE)), peek);
#else
...
int wolfSSL_SNI_GetFromBuffer(const byte *clientHello, word32 helloSz, byte type, byte *sni,
                              word32 *inOutSz)
{
...
WOLFSSL_API int wolfSSL_set_SessionTicket_cb(WOLFSSL *ssl, CallbackSessionTicket cb, void *ctx)
{
    if (ssl == NULL)
        return BAD_FUNC_ARG;
...
WOLFSSL_CERT_MANAGER *wolfSSL_CertManagerNew(void)
{
    ...
    cm =
        (WOLFSSL_CERT_MANAGER *)XMALLOC(sizeof(WOLFSSL_CERT_MANAGER), 0, DYNAMIC_TYPE_CERT_MANAGER);
    if (cm) {
...
int wolfSSL_CertPemToDer(const unsigned char *pem, int pemSz, unsigned char *buff, int buffSz,
                         int type)
{
    int eccKey = 0;
    int ret;
...
/* Return bytes written to buff or < 0 for error */
int wolfSSL_KeyPemToDer(const unsigned char *pem, int pemSz, unsigned char *buff, int buffSz,
                        const char *pass)
{
    int eccKey = 0;
    int ret;
...
#endif
#ifndef NO_SKID
            XMEMCPY(signer->subjectKeyIdHash, cert->extSubjKeyId, SIGNER_DIGEST_SIZE);
#endif
            XMEMCPY(signer->subjectNameHash, cert->subjectHash, SIGNER_DIGEST_SIZE);
...
int PemToDer(const unsigned char *buff, long longSz, int type, buffer *der, void *heap,
             EncryptedInfo *info, int *eccKey)
{
...
static int ProcessBuffer(WOLFSSL_CTX *ctx, const unsigned char *buff, long sz, int format, int type,
                         WOLFSSL *ssl, long *used, int userChain)
{
    ...
    if (info->set) {
        ...
        if (password == NULL || key == NULL || iv == NULL) {
            ...
            if (Base16_Decode(info->iv, info->ivSz, info->iv, &info->ivSz) != 0) {
...
#ifndef NO_MD5
            else if ((ret = EVP_BytesToKey(info->name, "MD5", info->iv, (byte *)password,
                ...
                ret = wc_AesCbcDecryptWithKey(der.buffer, der.buffer, der.length, key,
                                              AES_128_KEY_SIZE, info->iv);
            } else if (XSTRNCMP(info->name, "AES-192-CBC", 13) == 0) {
                ret = wc_AesCbcDecryptWithKey(der.buffer, der.buffer, der.length, key,
                                              AES_192_KEY_SIZE, info->iv);
            } else if (XSTRNCMP(info->name, "AES-256-CBC", 13) == 0) {
                ret = wc_AesCbcDecryptWithKey(der.buffer, der.buffer, der.length, key,
                                              AES_256_KEY_SIZE, info->iv);
            }
#endif
            else {
                ret = SSL_BAD_FILE;
...
static int ProcessChainBuffer(WOLFSSL_CTX *ctx, const unsigned char *buff, long sz, int format,
                              int type, WOLFSSL *ssl)
{
    long used = 0;
    int ret = 0;
    int gotOne = 0;
...
int wolfSSL_CertManagerVerifyBuffer(WOLFSSL_CERT_MANAGER *cm, const byte *buff, long sz, int format)
{
    int ret = 0;
    buffer der;
#ifdef WOLFSSL_SMALL_STACK
...
#ifdef HAVE_OCSP
    if (cm->ocsp == NULL) {
        cm->ocsp = (WOLFSSL_OCSP *)XMALLOC(sizeof(WOLFSSL_OCSP), cm->heap, DYNAMIC_TYPE_OCSP);
        if (cm->ocsp == NULL)
            return MEMORY_E;
...
int wolfSSL_CTX_SetOCSP_Cb(WOLFSSL_CTX *ctx, CbOCSPIO ioCb, CbOCSPRespFree respFreeCb,
                           void *ioCbCtx)
{
...
int ProcessFile(WOLFSSL_CTX *ctx, const char *fname, int format, int type, WOLFSSL *ssl,
                int userChain, WOLFSSL_CRL *crl)
{
#ifdef WOLFSSL_SMALL_STACK
    byte staticBuffer[1]; /* force heap usage */
#else
...
/* Verify the ceritficate, SSL_SUCCESS for ok, < 0 for error */
int wolfSSL_CertManagerVerify(WOLFSSL_CERT_MANAGER *cm, const char *fname, int format)
{
    int ret = SSL_FATAL_ERROR;
#ifdef WOLFSSL_SMALL_STACK
    byte staticBuffer[1]; /* force heap usage */
...
#ifdef HAVE_CRL
    if (cm->crl == NULL) {
        cm->crl = (WOLFSSL_CRL *)XMALLOC(sizeof(WOLFSSL_CRL), cm->heap, DYNAMIC_TYPE_CRL);
        if (cm->crl == NULL)
            return MEMORY_E;
...
#ifdef WOLFSSL_SMALL_STACK
                info =
                    (EncryptedInfo *)XMALLOC(sizeof(EncryptedInfo), NULL, DYNAMIC_TYPE_TMP_BUFFER);
                if (info == NULL)
                    ret = MEMORY_E;
                else
#endif
...
static int wolfSSL_SetTmpDH_buffer_wrapper(WOLFSSL_CTX *ctx, WOLFSSL *ssl, const unsigned char *buf,
                                           long sz, int format)
{
    buffer der;
    int ret = 0;
...
/* server Diffie-Hellman parameters, SSL_SUCCESS on ok */
int wolfSSL_SetTmpDH_buffer(WOLFSSL *ssl, const unsigned char *buf, long sz, int format)
{
...
static int wolfSSL_SetTmpDH_file_wrapper(WOLFSSL_CTX *ctx, WOLFSSL *ssl, const char *fname,
                                         int format)
{
#ifdef WOLFSSL_SMALL_STACK
    byte staticBuffer[1]; /* force heap usage */
...
int wolfSSL_CTX_use_NTRUPrivateKey_file(WOLFSSL_CTX *ctx, const char *file)
{
    ...
    if (ProcessFile(ctx, file, SSL_FILETYPE_RAW, PRIVATEKEY_TYPE, NULL, 0, NULL) == SSL_SUCCESS) {
        ctx->haveNTRU = 1;
        return SSL_SUCCESS;
    }
...
int wolfSSL_memsave_session_cache(void *mem, int sz)
{
    ...
    for (i = 0; i < cache_header.rows; ++i) XMEMCPY(row++, SessionCache + i, sizeof(SessionRow));

#ifndef NO_CLIENT_CACHE
    clRow = (ClientRow *)row;
    for (i = 0; i < cache_header.rows; ++i) XMEMCPY(clRow++, ClientCache + i, sizeof(ClientRow));
#endif
...
int wolfSSL_memrestore_session_cache(const void *mem, int sz)
{
    ...
    if (cache_header.version != WOLFSSL_CACHE_VERSION || cache_header.rows != SESSION_ROWS ||
        cache_header.columns != SESSIONS_PER_ROW ||
        cache_header.sessionSz != (int)sizeof(WOLFSSL_SESSION)) {
    ...
    for (i = 0; i < cache_header.rows; ++i) XMEMCPY(SessionCache + i, row++, sizeof(SessionRow));

#ifndef NO_CLIENT_CACHE
    clRow = (ClientRow *)row;
    for (i = 0; i < cache_header.rows; ++i) XMEMCPY(ClientCache + i, clRow++, sizeof(ClientRow));
#endif
...
{
    int sz = sizeof(signer->pubKeySize) + sizeof(signer->keyOID) + sizeof(signer->nameLen) +
             sizeof(signer->subjectNameHash);
...
static INLINE int GetCertCacheMemSize(WOLFSSL_CERT_MANAGER *cm)
{
    ...
    for (i = 0; i < CA_TABLE_SIZE; i++) sz += GetCertCacheRowMemory(cm->caTable[i]);
...
static INLINE int RestoreCertRow(WOLFSSL_CERT_MANAGER *cm, byte *current, int row, int listSz,
                                 const byte *end)
{
    int idx = 0;
    ...
    while (listSz) {
        ...
        int minSz = sizeof(signer->pubKeySize) + sizeof(signer->keyOID) + sizeof(signer->nameLen) +
                    sizeof(signer->subjectNameHash);
#ifndef NO_SKID
        ...
        signer->publicKey = (byte *)XMALLOC(signer->pubKeySize, cm->heap, DYNAMIC_TYPE_KEY);
        ...
        signer->name = (char *)XMALLOC(signer->nameLen, cm->heap, DYNAMIC_TYPE_SUBJECT_CN);
        if (signer->name == NULL) {
            FreeSigner(signer, cm->heap);
            return MEMORY_E;
        }
...
static INLINE int DoMemSaveCertCache(WOLFSSL_CERT_MANAGER *cm, void *mem, int sz)
{
    ...
    if (realSz > sz) {
        ...
        for (i = 0; i < CA_TABLE_SIZE; ++i) current += StoreCertRow(cm, current, i);
    }
...
int CM_MemRestoreCertCache(WOLFSSL_CERT_MANAGER *cm, const void *mem, int sz)
{
    ...
    if (hdr->version != WOLFSSL_CACHE_CERT_VERSION || hdr->rows != CA_TABLE_SIZE ||
        hdr->signerSz != (int)sizeof(Signer)) {
...
WOLFSSL_METHOD *wolfSSLv3_client_method(void)
{
    WOLFSSL_METHOD *method =
        (WOLFSSL_METHOD *)XMALLOC(sizeof(WOLFSSL_METHOD), 0, DYNAMIC_TYPE_METHOD);
...
int wolfSSL_accept(WOLFSSL *ssl)
{
    ...
    if (!havePSK && !haveAnon &&
        (ssl->buffers.certificate.buffer == NULL || ssl->buffers.key.buffer == NULL)) {
...
int AddSession(WOLFSSL *ssl)
{
    ...
    XMEMCPY(SessionCache[row].Sessions[idx].masterSecret, ssl->arrays->masterSecret, SECRET_LEN);
    XMEMCPY(SessionCache[row].Sessions[idx].sessionID, ssl->arrays->sessionID, ID_LEN);
    ...
    XMEMCPY(SessionCache[row].Sessions[idx].ticket, ssl->session.ticket, ssl->session.ticketLen);
#endif

#ifdef SESSION_CERTS
    SessionCache[row].Sessions[idx].chain.count = ssl->session.chain.count;
    XMEMCPY(SessionCache[row].Sessions[idx].chain.certs, ssl->session.chain.certs,
    ...
    if (ssl->options.side == WOLFSSL_CLIENT_END && ssl->session.idLen) {
        ...
        clientRow = HashSession(ssl->session.serverID, ssl->session.idLen, &error) % SESSION_ROWS;
        if (error != 0) {
            WOLFSSL_MSG("Hash session failed");
        } else {
...
int wolfSSL_GetSessionAtIndex(int idx, WOLFSSL_SESSION *session)
{
    ...
    if (row < SESSION_ROWS && col < (int)min(SessionCache[row].totalCount, SESSIONS_PER_ROW)) {
        XMEMCPY(session, &SessionCache[row].Sessions[col], sizeof(WOLFSSL_SESSION));
        result = SSL_SUCCESS;
    }
...
static int get_locked_session_stats(word32 *active, word32 *total, word32 *peak)
{
    ...
    for (i = 0; i < SESSION_ROWS; i++) {
        ...
        for (; count > 0; --count, idx = idx ? idx - 1 : SESSIONS_PER_ROW - 1) {
            ...
            if (ticks <
                (SessionCache[i].Sessions[idx].bornOn + SessionCache[i].Sessions[idx].timeout)) {
                now++;
            }
        }
    }
...
int wolfSSL_get_session_stats(word32 *active, word32 *total, word32 *peak, word32 *maxSessions)
{
    int result = SSL_SUCCESS;
...
int wolfSSL_PrintSessionStats(void)
{
    ...
    ret = wolfSSL_get_session_stats(&totalSessionsNow, &totalSessionsSeen, &peak, &maxSessions);
    if (ret != SSL_SUCCESS)
        return ret;
...
int wolfSSL_check_domain_name(WOLFSSL *ssl, const char *dn)
{
    ...
    ssl->buffers.domainName.buffer =
        (byte *)XMALLOC(ssl->buffers.domainName.length, ssl->heap, DYNAMIC_TYPE_DOMAIN);
...
int wolfSSL_writev(WOLFSSL *ssl, const struct iovec *iov, int iovcnt)
{
    ...
    for (i = 0; i < iovcnt; i++) sending += (int)iov[i].iov_len;
...
static int wolfSSL_ex_wrapper(WOLFSSL *ssl, HandShakeCallBack hsCb, TimeoutCallBack toCb,
                              Timeval timeout)
{
    int ret = SSL_FATAL_ERROR;
    ...
    if (toCb) {
        if (oldTimerOn) {
            ...
            if (CmpTimes(totalTime, oldTimeout.it_value, <))
                SubtractTimes(oldTimeout.it_value, totalTime, oldTimeout.it_value);
            else {
...
void wolfSSL_CTX_set_psk_client_callback(WOLFSSL_CTX *ctx, psk_client_callback cb)
{
...
void wolfSSL_set_psk_client_callback(WOLFSSL *ssl, psk_client_callback cb)
{
    ...
    haveRSA = 0;
#endif
    InitSuites(ssl->suites, ssl->version, haveRSA, TRUE, ssl->options.haveDH, ssl->options.haveNTRU,
...
int wolfSSL_CTX_use_certificate_buffer(WOLFSSL_CTX *ctx, const unsigned char *in, long sz,
                                       int format)
{
...
int wolfSSL_CTX_use_PrivateKey_buffer(WOLFSSL_CTX *ctx, const unsigned char *in, long sz,
                                      int format)
{
...
int wolfSSL_use_PrivateKey_buffer(WOLFSSL *ssl, const unsigned char *in, long sz, int format)
{
    ...
    return ProcessBuffer(ssl->ctx, in, sz, format, PRIVATEKEY_TYPE, ssl, NULL, 0);
}
...
int wolfSSL_use_certificate_chain_buffer(WOLFSSL *ssl, const unsigned char *in, long sz)
{
    ...
    return ProcessBuffer(ssl->ctx, in, sz, SSL_FILETYPE_PEM, CERT_TYPE, ssl, NULL, 1);
}
...
void wolfSSL_CTX_set_client_CA_list(WOLFSSL_CTX *ctx, STACK_OF(WOLFSSL_X509_NAME) * names)
{
    (void)ctx;
    (void)names;
}
...
int wolfSSL_get_keyblock_size(WOLFSSL *ssl)
{
    ...
    return 2 * (ssl->specs.key_size + ssl->specs.iv_size + ssl->specs.hash_size);
}

/* store keys returns SSL_SUCCESS or -1 on error */
int wolfSSL_get_keys(WOLFSSL *ssl, unsigned char **ms, unsigned int *msLen, unsigned char **sr,
                     unsigned int *srLen, unsigned char **cr, unsigned int *crLen)
{
    if (ssl == NULL || ssl->arrays == NULL)
        return SSL_FATAL_ERROR;
...
WOLFSSL_RSA *wolfSSL_RSA_generate_key(int len, unsigned long bits, void (*f)(int, int, void *),
                                      void *data)
{
...
WOLFSSL_X509 *wolfSSL_X509_STORE_CTX_get_current_cert(WOLFSSL_X509_STORE_CTX *ctx)
{
    (void)ctx;
    return 0;
}
...
int wolfSSL_BIO_read(WOLFSSL_BIO *bio, void *buf, int len)
{
    ...
    while (bio && ((ssl = bio->ssl) == 0)) bio = bio->next;
...
void wolfSSL_CTX_set_default_passwd_cb_userdata(WOLFSSL_CTX *ctx, void *userdata)
{
...
int wolfSSL_EVP_BytesToKey(const WOLFSSL_EVP_CIPHER *type, const WOLFSSL_EVP_MD *md,
                           const byte *salt, const byte *data, int sz, int count, byte *key,
                           byte *iv)
{
    int keyLen = 0;
    int ivLen = 0;
    ...
    while (keyOutput < (keyLen + ivLen)) {
        ...
        if (ivLeft && digestLeft) {
            ...
            XMEMCPY(&iv[ivLen - ivLeft], &digest[MD5_DIGEST_SIZE - digestLeft], store);
            keyOutput += store;
            ivLeft -= store;
        }
    }
...
void wolfSSL_MD5_Update(WOLFSSL_MD5_CTX *md5, const void *input, unsigned long sz)
{
...
void wolfSSL_SHA_Update(WOLFSSL_SHA_CTX *sha, const void *input, unsigned long sz)
{
...
void wolfSSL_SHA1_Update(WOLFSSL_SHA_CTX *sha, const void *input, unsigned long sz)
{
...
void wolfSSL_SHA256_Update(WOLFSSL_SHA256_CTX *sha, const void *input, unsigned long sz)
{
...
void wolfSSL_SHA384_Update(WOLFSSL_SHA384_CTX *sha, const void *input, unsigned long sz)
{
...
void wolfSSL_SHA512_Update(WOLFSSL_SHA512_CTX *sha, const void *input, unsigned long sz)
{
...
/* SSL_SUCCESS on ok */
int wolfSSL_EVP_CipherInit(WOLFSSL_EVP_CIPHER_CTX *ctx, const WOLFSSL_EVP_CIPHER *type, byte *key,
                           byte *iv, int enc)
{
...
#ifndef NO_AES
    if (ctx->cipherType == AES_128_CBC_TYPE || (type && XSTRNCMP(type, "AES128-CBC", 10) == 0)) {
        ...
        if (iv && key == NULL) {
            ...
            if (ret != 0)
               ...
               (type && XSTRNCMP(type, "AES192-CBC", 10) == 0)) {
        ...
        if (iv && key == NULL) {
            ...
            if (ret != 0)
               ...
               (type && XSTRNCMP(type, "AES256-CBC", 10) == 0)) {
        ...
        if (iv && key == NULL) {
            ...
            if (ret != 0)
             ...
             (type && XSTRNCMP(type, "AES128-CTR", 10) == 0)) {
        ...
        if (iv && key == NULL) {
            ...
            if (ret != 0)
               ...
               (type && XSTRNCMP(type, "AES192-CTR", 10) == 0)) {
        ...
        if (iv && key == NULL) {
            ...
            if (ret != 0)
               ...
               (type && XSTRNCMP(type, "AES256-CTR", 10) == 0)) {
...
#ifndef NO_DES3
    else if (ctx->cipherType == DES_CBC_TYPE || (type && XSTRNCMP(type, "DES-CBC", 7) == 0)) {
        WOLFSSL_MSG("DES-CBC");
...
#endif /* NO_DES3 */
#ifndef NO_RC4
    else if (ctx->cipherType == ARC4_TYPE || (type && XSTRNCMP(type, "ARC4", 4) == 0)) {
        WOLFSSL_MSG("ARC4");
...
#endif /* NO_RC4 */
    else if (ctx->cipherType == NULL_CIPHER_TYPE || (type && XSTRNCMP(type, "NULL", 4) == 0)) {
...
/* SSL_SUCCESS on ok */
int wolfSSL_EVP_Cipher(WOLFSSL_EVP_CIPHER_CTX *ctx, byte *dst, byte *src, word32 len)
{
    int ret = 0;
...
/* SSL_SUCCESS on ok */
int wolfSSL_EVP_DigestUpdate(WOLFSSL_EVP_MD_CTX *ctx, const void *data, unsigned long sz)
{
...
#ifndef NO_SHA256
    case SHA256:
        wolfSSL_SHA256_Update((SHA256_CTX *)&ctx->hash, data, (unsigned long)sz);
        break;
#endif
#ifdef WOLFSSL_SHA384
    case SHA384:
        wolfSSL_SHA384_Update((SHA384_CTX *)&ctx->hash, data, (unsigned long)sz);
        break;
#endif
#ifdef WOLFSSL_SHA512
    case SHA512:
        wolfSSL_SHA512_Update((SHA512_CTX *)&ctx->hash, data, (unsigned long)sz);
        break;
#endif
    default:
        return BAD_FUNC_ARG;
...
/* SSL_SUCCESS on ok */
int wolfSSL_EVP_DigestFinal(WOLFSSL_EVP_MD_CTX *ctx, unsigned char *md, unsigned int *s)
{
...
/* SSL_SUCCESS on ok */
int wolfSSL_EVP_DigestFinal_ex(WOLFSSL_EVP_MD_CTX *ctx, unsigned char *md, unsigned int *s)
{
    WOLFSSL_ENTER("EVP_DigestFinal_ex");
    return EVP_DigestFinal(ctx, md, s);
}

unsigned char *wolfSSL_HMAC(const WOLFSSL_EVP_MD *evp_md, const void *key, int key_len,
    ...
    if (wc_HmacSetKey(hmac, type, (const byte *)key, key_len) == 0)
        if (wc_HmacUpdate(hmac, d, n) == 0)
            if (wc_HmacFinal(hmac, md) == 0) {
                if (md_len)
                    *md_len = (type == MD5) ? (int)MD5_DIGEST_SIZE : (int)SHA_DIGEST_SIZE;
                ret = md;
            }
...
#ifndef NO_DES3
/* SSL_SUCCESS on ok */
int wolfSSL_DES_key_sched(WOLFSSL_const_DES_cblock *key, WOLFSSL_DES_key_schedule *schedule)
{
...
void wolfSSL_DES_cbc_encrypt(const unsigned char *input, unsigned char *output, long length,
                             WOLFSSL_DES_key_schedule *schedule, WOLFSSL_DES_cblock *ivec, int enc)
{
    Des myDes;
...
void wolfSSL_DES_ncbc_encrypt(const unsigned char *input, unsigned char *output, long length,
                              WOLFSSL_DES_key_schedule *schedule, WOLFSSL_DES_cblock *ivec, int enc)
{
    Des myDes;
...
int wolfSSL_CTX_set_session_id_context(WOLFSSL_CTX *ctx, const unsigned char *sid_ctx,
...
unsigned long wolfSSL_ERR_get_error_line_data(const char **file, int *line, const char **data,
                                              int *flags)
{
    /* Not implemented */
    (void)file;
...
WOLFSSL_X509 *wolfSSL_X509_load_certificate_file(const char *fname, int format)
{
    ...
    if (der.buffer != NULL) {
...
#endif
        {
            ...
            if (ParseCertRelative(cert, CERT_TYPE, 0, NULL) == 0) {
                x509 = (WOLFSSL_X509 *)XMALLOC(sizeof(WOLFSSL_X509), NULL, DYNAMIC_TYPE_X509);
                if (x509 != NULL) {
                    InitX509(x509, 1);
...
int wolfSSL_set_session_id_context(WOLFSSL *ssl, const unsigned char *id, unsigned int len)
{
    (void)ssl;
    (void)id;
    (void)len;
...
{
    return (ssl->options.isClosed || ssl->options.connReset || ssl->options.sentNotify);
}
...
int wolfSSL_OCSP_parse_url(char *url, char **host, char **port, char **path, int *ssl)
{
    (void)url;
    (void)host;
    (void)port;
...
void wolfSSL_MD4_Update(WOLFSSL_MD4_CTX *md4, const void *data, unsigned long len)
{
...
int wolfSSL_get_ex_new_index(long idx, void *data, void *cb1, void *cb2, void *cb3)
{
    (void)idx;
    (void)data;
    (void)cb1;
...
void wolfSSL_set_dynlock_create_callback(WOLFSSL_dynlock_value *(*f)(const char *, int))
{
    (void)f;
}

void wolfSSL_set_dynlock_lock_callback(void (*f)(int, WOLFSSL_dynlock_value *, const char *, int))
{
    (void)f;
}

void wolfSSL_set_dynlock_destroy_callback(void (*f)(WOLFSSL_dynlock_value *, const char *, int))
{
    (void)f;
}
...
int wolfSSL_X509_LOOKUP_add_dir(WOLFSSL_X509_LOOKUP *lookup, const char *dir, long len)
{
    (void)lookup;
    (void)dir;
    (void)len;
...
int wolfSSL_X509_LOOKUP_load_file(WOLFSSL_X509_LOOKUP *lookup, const char *file, long len)
{
    (void)lookup;
    (void)file;
    (void)len;
...
int wolfSSL_X509_STORE_add_cert(WOLFSSL_X509_STORE *store, WOLFSSL_X509 *x509)
{
    ...
    if (store != NULL && store->cm != NULL && x509 != NULL) {
        buffer derCert;
        derCert.buffer = (byte *)XMALLOC(x509->derCert.length, NULL, DYNAMIC_TYPE_CERT);
        if (derCert.buffer != NULL) {
            derCert.length = x509->derCert.length;
            /* AddCA() frees the buffer. */
            XMEMCPY(derCert.buffer, x509->derCert.buffer, x509->derCert.length);
...
int wolfSSL_X509_STORE_get_by_subject(WOLFSSL_X509_STORE_CTX *ctx, int idx, WOLFSSL_X509_NAME *name,
...
WOLFSSL_X509_STORE_CTX *wolfSSL_X509_STORE_CTX_new(void)
{
    WOLFSSL_X509_STORE_CTX *ctx =
        (WOLFSSL_X509_STORE_CTX *)XMALLOC(sizeof(WOLFSSL_X509_STORE_CTX), NULL, 0);
...
int wolfSSL_X509_STORE_CTX_init(WOLFSSL_X509_STORE_CTX *ctx, WOLFSSL_X509_STORE *store,
                                WOLFSSL_X509 *x509, STACK_OF(WOLFSSL_X509) * sk)
{
    (void)sk;
    if (ctx != NULL) {
        ctx->store = store;
...
{
    if (ctx != NULL && ctx->store != NULL && ctx->store->cm != NULL && ctx->current_cert != NULL) {
        return wolfSSL_CertManagerVerifyBuffer(ctx->store->cm, ctx->current_cert->derCert.buffer,
...
WOLFSSL_EVP_PKEY *wolfSSL_X509_get_pubkey(WOLFSSL_X509 *x509)
{
    ...
    if (x509 != NULL) {
        key = (WOLFSSL_EVP_PKEY *)XMALLOC(sizeof(WOLFSSL_EVP_PKEY), NULL, DYNAMIC_TYPE_PUBLIC_KEY);
        if (key != NULL) {
            key->type = x509->pubKeyOID;
            key->save_type = 0;
            key->pkey.ptr = (char *)XMALLOC(x509->pubKey.length, NULL, DYNAMIC_TYPE_PUBLIC_KEY);
...
WOLFSSL_X509_REVOKED *wolfSSL_sk_X509_REVOKED_value(WOLFSSL_X509_REVOKED *revoked, int value)
{
    (void)revoked;
    (void)value;
    return 0;
...
int wolfSSL_ASN1_INTEGER_cmp(const WOLFSSL_ASN1_INTEGER *a, const WOLFSSL_ASN1_INTEGER *b)
{
    (void)a;
    (void)b;
    return 0;
...
void wolfSSL_DES_set_key_unchecked(WOLFSSL_const_DES_cblock *myDes, WOLFSSL_DES_key_schedule *key)
{
    (void)myDes;
    (void)key;
}
...
void wolfSSL_DES_ecb_encrypt(WOLFSSL_DES_cblock *desa, WOLFSSL_DES_cblock *desb,
                             WOLFSSL_DES_key_schedule *key, int len)
{
    (void)desa;
    (void)desb;
    (void)key;
...
void wolfSSL_CTX_sess_set_new_cb(WOLFSSL_CTX *ctx, int (*f)(WOLFSSL *, WOLFSSL_SESSION *))
{
    (void)ctx;
    (void)f;
}

void wolfSSL_CTX_sess_set_remove_cb(WOLFSSL_CTX *ctx, void (*f)(WOLFSSL_CTX *, WOLFSSL_SESSION *))
{
    (void)ctx;
    (void)f;
}
...
WOLFSSL_SESSION *wolfSSL_d2i_SSL_SESSION(WOLFSSL_SESSION **sess, const unsigned char **p, long i)
{
    (void)p;
    (void)i;
    if (sess)
...
int wolfSSL_CTX_get_ex_new_index(long idx, void *arg, void *a, void *b, void *c)
{
    (void)idx;
    (void)arg;
    (void)a;
...
int wolfSSL_cmp_peer_cert_to_file(WOLFSSL *ssl, const char *fname)
{
    ...
    if (ssl != NULL && fname != NULL) {
...
#endif
        {
            ...
            if ((myBuffer != NULL) && (sz > 0) && (XFREAD(myBuffer, sz, 1, file) > 0) &&
                (PemToDer(myBuffer, sz, CERT_TYPE, &fileDer, ctx->heap, info, &eccKey) == 0) &&
                (fileDer.length != 0) && (fileDer.length == peer_cert->derCert.length) &&
                (XMEMCMP(peer_cert->derCert.buffer, fileDer.buffer, fileDer.length) == 0)) {
                ret = 0;
            }
...
WOLFSSL_BIGNUM *wolfSSL_BN_new(void)
{
    ...
    external = (WOLFSSL_BIGNUM *)XMALLOC(sizeof(WOLFSSL_BIGNUM), NULL, DYNAMIC_TYPE_BIGINT);
    if (external == NULL) {
...
/* SSL_SUCCESS on ok */
int wolfSSL_BN_sub(WOLFSSL_BIGNUM *r, const WOLFSSL_BIGNUM *a, const WOLFSSL_BIGNUM *b)
{
    ...
    if (mp_sub((mp_int *)a->internal, (mp_int *)b->internal, (mp_int *)r->internal) == MP_OKAY)
        return SSL_SUCCESS;
...
/* SSL_SUCCESS on ok */
int wolfSSL_BN_mod(WOLFSSL_BIGNUM *r, const WOLFSSL_BIGNUM *a, const WOLFSSL_BIGNUM *b,
                   const WOLFSSL_BN_CTX *c)
{
    ...
    if (mp_mod((mp_int *)a->internal, (mp_int *)b->internal, (mp_int *)r->internal) == MP_OKAY)
        return SSL_SUCCESS;
...
WOLFSSL_BIGNUM *wolfSSL_BN_bin2bn(const unsigned char *str, int len, WOLFSSL_BIGNUM *ret)
{
...
WOLFSSL_DH *wolfSSL_DH_new(void)
{
    ...
    external = (WOLFSSL_DH *)XMALLOC(sizeof(WOLFSSL_DH), NULL, DYNAMIC_TYPE_DH);
    if (external == NULL) {
...
int wolfSSL_DH_generate_key(WOLFSSL_DH *dh)
{
    ...
    if (rng) {
        if (wc_DhGenerateKeyPair((DhKey *)dh->internal, rng, priv, &privSz, pub, &pubSz) < 0)
...
int wolfSSL_DH_compute_key(unsigned char *key, WOLFSSL_BIGNUM *otherPub, WOLFSSL_DH *dh)
{
    int ret = 0;
    word32 keySz = 0;
    word32 pubSz = 1024;
    ...
    if (dh == NULL || dh->priv_key == NULL || otherPub == NULL)
        ...
        if (privSz <= 0 || pubSz <= 0)
            WOLFSSL_MSG("Bad BN2bin set");
        else if (wc_DhAgree((DhKey *)dh->internal, key, &keySz, priv, privSz, pub, pubSz) < 0)
...
WOLFSSL_DSA *wolfSSL_DSA_new(void)
{
    ...
    external = (WOLFSSL_DSA *)XMALLOC(sizeof(WOLFSSL_DSA), NULL, DYNAMIC_TYPE_DSA);
    if (external == NULL) {
...
int wolfSSL_DSA_generate_parameters_ex(WOLFSSL_DSA *dsa, int bits, unsigned char *seed, int seedLen,
                                       int *counterRet, unsigned long *hRet, void *cb)
{
    (void)dsa;
    (void)bits;
    (void)seed;
...
WOLFSSL_RSA *wolfSSL_RSA_new(void)
{
    ...
    external = (WOLFSSL_RSA *)XMALLOC(sizeof(WOLFSSL_RSA), NULL, DYNAMIC_TYPE_RSA);
    if (external == NULL) {
...
/* SSL_SUCCESS on ok */
int wolfSSL_RSA_generate_key_ex(WOLFSSL_RSA *rsa, int bits, WOLFSSL_BIGNUM *bn, void *cb)
{
    int ret = SSL_FATAL_ERROR;
...
int wolfSSL_RSA_public_encrypt(int len, unsigned char *fr, unsigned char *to, WOLFSSL_RSA *rsa,
                               int padding)
{
    (void)len;
    (void)fr;
...
int wolfSSL_RSA_private_decrypt(int len, unsigned char *fr, unsigned char *to, WOLFSSL_RSA *rsa,
                                int padding)
{
    (void)len;
    (void)fr;
...
int wolfSSL_DSA_do_sign(const unsigned char *d, unsigned char *sigRet, WOLFSSL_DSA *dsa)
{
    int ret = SSL_FATAL_ERROR;
    int initTmpRng = 0;
    RNG *rng = NULL;
...
int wolfSSL_RSA_sign(int type, const unsigned char *m, unsigned int mLen, unsigned char *sigRet,
                     unsigned int *sigLen, WOLFSSL_RSA *rsa)
{
    ...
    if (rng) {
        ...
        if (signSz == 0) {
            ...
            *sigLen =
                wc_RsaSSL_Sign(encodedSig, signSz, sigRet, outLen, (RsaKey *)rsa->internal, rng);
            if (*sigLen <= 0)
...
int wolfSSL_RSA_public_decrypt(int flen, unsigned char *from, unsigned char *to, WOLFSSL_RSA *rsa,
                               int padding)
{
    (void)flen;
    (void)from;
...
int wolfSSL_RSA_GenAdd(WOLFSSL_RSA *rsa)
{
    ...
    if (rsa == NULL || rsa->p == NULL || rsa->q == NULL || rsa->d == NULL || rsa->dmp1 == NULL ||
        rsa->dmq1 == NULL) {
    ...
    if (err != MP_OKAY) {
        ...
        err = mp_mod((mp_int *)rsa->d->internal, &tmp, (mp_int *)rsa->dmp1->internal);
    ...
    if (err != MP_OKAY) {
        ...
        err = mp_mod((mp_int *)rsa->d->internal, &tmp, (mp_int *)rsa->dmq1->internal);
...
void wolfSSL_HMAC_Init(WOLFSSL_HMAC_CTX *ctx, const void *key, int keylen, const EVP_MD *type)
{
...
void wolfSSL_HMAC_Update(WOLFSSL_HMAC_CTX *ctx, const unsigned char *data, int len)
{
...
void wolfSSL_3des_iv(WOLFSSL_EVP_CIPHER_CTX *ctx, int doset, unsigned char *iv, int len)
{
    (void)len;
...
void wolfSSL_aes_ctr_iv(WOLFSSL_EVP_CIPHER_CTX *ctx, int doset, unsigned char *iv, int len)
{
    (void)len;
...
int wolfSSL_PEM_write_bio_RSAPrivateKey(WOLFSSL_BIO *bio, RSA *rsa, const EVP_CIPHER *cipher,
                                        unsigned char *passwd, int len, pem_password_cb cb,
                                        void *arg)
{
    (void)bio;
    (void)rsa;
...
int wolfSSL_PEM_write_bio_DSAPrivateKey(WOLFSSL_BIO *bio, DSA *rsa, const EVP_CIPHER *cipher,
                                        unsigned char *passwd, int len, pem_password_cb cb,
                                        void *arg)
{
    (void)bio;
...
WOLFSSL_EVP_PKEY *wolfSSL_PEM_read_bio_PrivateKey(WOLFSSL_BIO *bio, WOLFSSL_EVP_PKEY **key,
...
#endif
        {
            InitDecodedCert(cert, chain->certs[idx].buffer, chain->certs[idx].length, NULL);
...
/* Get peer's PEM ceritifcate at index (idx), output to buffer if inLen big
   ...
   SSL_SUCCESS on ok */
int wolfSSL_get_chain_cert_pem(WOLFSSL_X509_CHAIN *chain, int idx, unsigned char *buf, int inLen,
                               int *outLen)
{
    ...
    *outLen = inLen; /* input to Base64_Encode */
    if ((err = Base64_Encode(chain->certs[idx].buffer, chain->certs[idx].length, buf + i,
                             (word32 *)outLen)) < 0)
        return err;
    i += *outLen;
