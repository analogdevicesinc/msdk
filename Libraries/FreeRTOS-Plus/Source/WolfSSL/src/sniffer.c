...
enum {
    ...
    MAX_INPUT_SZ =
        MAX_RECORD_SIZE + COMP_EXTRA + MAX_MSG_EXTRA + MTU_EXTRA, /* Max input sz of reassembly */
...
static const char *const msgTable[] = {
    /* 1 */
    "Out of Memory", "New SSL Sniffer Server Registered", "Checking IP Header",
    "SSL Sniffer Server Not Registered", "Checking TCP Header",

    /* 6 */
    "SSL Sniffer Server Port Not Registered", "RSA Private Decrypt Error",
    "RSA Private Decode Error", "Set Cipher Spec Error", "Server Hello Input Malformed",

    /* 11 */
    "Couldn't Resume Session Error", "Server Did Resumption", "Client Hello Input Malformed",
    "Client Trying to Resume", "Handshake Input Malformed",

    /* 16 */
    "Got Hello Verify msg", "Got Server Hello msg", "Got Cert Request msg",
    "Got Server Key Exchange msg", "Got Cert msg",

    /* 21 */
    "Got Server Hello Done msg", "Got Finished msg", "Got Client Hello msg",
    "Got Client Key Exchange msg", "Got Cert Verify msg",

    /* 26 */
    "Got Unknown Handshake msg", "New SSL Sniffer Session created", "Couldn't create new SSL",
    "Got a Packet to decode", "No data present",

    /* 31 */
    "Session Not Found", "Got an Old Client Hello msg", "Old Client Hello Input Malformed",
    "Old Client Hello OK", "Bad Old Client Hello",

    /* 36 */
    "Bad Record Header", "Record Header Input Malformed", "Got a HandShake msg",
    "Bad HandShake msg", "Got a Change Cipher Spec msg",

    /* 41 */
    "Got Application Data msg", "Bad Application Data", "Got an Alert msg",
    "Another msg to Process", "Removing Session From Table",

    /* 46 */
    "Bad Key File", "Wrong IP Version", "Wrong Protocol type", "Packet Short for header processing",
    "Got Unknown Record Type",

    /* 51 */
    "Can't Open Trace File", "Session in Fatal Error State", "Partial SSL record received",
    "Buffer Error, malformed input", "Added to Partial Input",

    /* 56 */
    "Received a Duplicate Packet", "Received an Out of Order Packet",
    "Received an Overlap Duplicate Packet", "Received an Overlap Reassembly Begin Duplicate Packet",
    "Received an Overlap Reassembly End Duplicate Packet",

    /* 61 */
    "Missed the Client Hello Entirely", "Got Hello Request msg", "Got Session Ticket msg",
    "Bad Input", "Bad Decrypt Type",

    /* 66 */
    "Bad Finished Message Processing", "Bad Compression Type", "Bad DeriveKeys Error",
    "Saw ACK for Missing Packet Error", "Bad Decrypt Operation",

    /* 71 */
    "Decrypt Keys Not Set Up", "Late Key Load Error", "Got Certificate Status msg",
...
static void TraceSetNamedServer(const char *name, const char *srv, int port, const char *keyFile)
{
    if (TraceOn) {
        fprintf(TraceFile, "\tTrying to install a new Sniffer Server with\n");
...
static void TraceIP(IpHdr *iphdr)
{
    if (TraceOn) {
        ...
        fprintf(TraceFile, "\tdst:%s src:%s\n", IpToS(iphdr->dst, dst), IpToS(iphdr->src, src));
    }
}
...
{
    if (TraceOn) {
        fprintf(TraceFile, "\t%d bytes added to %d exisiting bytes in User Buffer\n", newBytes,
                existingBytes);
    }
}
...
static SnifferSession *GetSnifferSession(IpInfo *ipInfo, TcpInfo *tcpInfo)
{
    ...
    while (session) {
        if (session->server == ipInfo->src && session->client == ipInfo->dst &&
            session->srvPort == tcpInfo->srcPort && session->cliPort == tcpInfo->dstPort)
            break;
        if (session->client == ipInfo->src && session->server == ipInfo->dst &&
            session->cliPort == tcpInfo->srcPort && session->srvPort == tcpInfo->dstPort)
            break;
    ...
    /* determine side */
    if (session) {
        if (ipInfo->dst == session->context->server && tcpInfo->dstPort == session->context->port)
...
static int LoadKeyFile(byte **keyBuf, word32 *keyBufSz, const char *keyFile, int typeKey,
                       const char *password)
{
    byte *loadBuf;
    byte *saveBuf;
    ...
    if (typeKey == SSL_FILETYPE_PEM) {
        ...
        saveBufSz = wolfSSL_KeyPemToDer(loadBuf, (int)fileSz, saveBuf, (int)fileSz, password);
        free(loadBuf);
...
static int SetNamedPrivateKey(const char *name, const char *address, int port, const char *keyFile,
                              int typeKey, const char *password, char *error)
{
    SnifferServer *sniffer;
    int ret;
    int type = (typeKey == FILETYPE_PEM) ? SSL_FILETYPE_PEM : SSL_FILETYPE_ASN1;
    int isNew = 0;
    word32 serverIp;
    ...
    if (name != NULL) {
        ...
        ret = LoadKeyFile(&namedKey->key, &namedKey->keySz, keyFile, type, password);
        if (ret < 0) {
    ...
    sniffer = ServerList;
    while (sniffer != NULL && (sniffer->server != serverIp || sniffer->port != port)) {
        sniffer = sniffer->next;
    }
...
int ssl_SetNamedPrivateKey(const char *name, const char *address, int port, const char *keyFile,
                           int typeKey, const char *password, char *error)
{
    int ret;
    ...
    LockMutex(&ServerListMutex);
    ret = SetNamedPrivateKey(name, address, port, keyFile, typeKey, password, error);
    UnLockMutex(&ServerListMutex);
...
/* returns 0 on success, -1 on error */
int ssl_SetPrivateKey(const char *address, int port, const char *keyFile, int typeKey,
                      const char *password, char *error)
{
    ...
    LockMutex(&ServerListMutex);
    ret = SetNamedPrivateKey(NULL, address, port, keyFile, typeKey, password, error);
    UnLockMutex(&ServerListMutex);
...
static int ProcessClientKeyExchange(const byte *input, int *sslBytes, SnifferSession *session,
                                    char *error)
{
    word32 idx = 0;
    RsaKey key;
    ...
    if (ret == 0)
        ret = wc_RsaPrivateKeyDecode(session->sslServer->buffers.key.buffer, &idx, &key,
                                     session->sslServer->buffers.key.length);
    if (ret == 0) {
        ...
        ret = wc_RsaPrivateDecrypt(input, length, session->sslServer->arrays->preMasterSecret,
                                   SECRET_LEN, &key);
...
static int ProcessSessionTicket(const byte *input, int *sslBytes, SnifferSession *session,
                                char *error)
{
    word16 len;
...
static int ProcessServerHello(const byte *input, int *sslBytes, SnifferSession *session,
                              char *error)
{
    ProtocolVersion pv;
    byte b;
    ...
    *sslBytes -= ENUM_LEN;
    ...
    if (session->sslServer->options.haveSessionId &&
        XMEMCMP(session->sslServer->arrays->sessionID, session->sslClient->arrays->sessionID,
                ID_LEN) == 0)
        doResume = 1;
    else if (session->sslClient->options.haveSessionId == 0 &&
             session->sslServer->options.haveSessionId == 0 && session->ticketID)
        doResume = 1;
    ...
    if (doResume) {
        int ret = 0;
        SSL_SESSION *resume =
            GetSession(session->sslServer, session->sslServer->arrays->masterSecret);
        if (resume == NULL) {
            SetError(BAD_SESSION_RESUME_STR, error, session, FATAL_ERROR_STATE);
            return -1;
        }
        /* make sure client has master secret too */
        XMEMCPY(session->sslClient->arrays->masterSecret, session->sslServer->arrays->masterSecret,
                SECRET_LEN);
...
#ifdef SHOW_SECRETS
    {
        ...
        for (i = 0; i < RAN_LEN; i++) printf("%02x", session->sslServer->arrays->serverRandom[i]);
        printf("\n");
    }
#endif
    return 0;
...
static int ProcessClientHello(const byte *input, int *sslBytes, SnifferSession *session,
                              char *error)
{
    byte bLen;
    word16 len;
...
#ifdef HAVE_SNI
    {
        ...
        ret = wolfSSL_SNI_GetFromBuffer(input - HANDSHAKE_HEADER_SZ - RECORD_HEADER_SZ,
        ...
        if (ret == SSL_SUCCESS) {
            ...
            while (namedKey != NULL) {
                if (nameSz == namedKey->nameSz &&
                    ...
                    if (wolfSSL_use_PrivateKey_buffer(session->sslServer, namedKey->key,
                                                      namedKey->keySz,
                                                      SSL_FILETYPE_ASN1) != SSL_SUCCESS) {
                        UnLockMutex(&session->context->namedKeysMutex);
                        SetError(CLIENT_HELLO_LATE_KEY_STR, error, session, FATAL_ERROR_STATE);
                        return -1;
                    }
                    break;
                } else
    ...
    *sslBytes -= ENUM_LEN;
...
#ifdef SHOW_SECRETS
    {
        ...
        for (i = 0; i < RAN_LEN; i++) printf("%02x", session->sslServer->arrays->clientRandom[i]);
        printf("\n");
    }
#endif
...
/* Process Finished */
static int ProcessFinished(const byte *input, int size, int *sslBytes, SnifferSession *session,
                           char *error)
{
    SSL *ssl;
    word32 inOutIdx = 0;
    ...
    ret = DoFinished(ssl, input, &inOutIdx, (word32)size, (word32)*sslBytes, SNIFF);
...
static int DoHandShake(const byte *input, int *sslBytes, SnifferSession *session, char *error)
{
    byte type;
    int size;
    int ret = 0;
...
static const byte *DecryptMessage(SSL *ssl, const byte *input, word32 sz, byte *output, int *error,
                                  int *advance)
{
    int ivExtra = 0;
...
static void RemoveSession(SnifferSession *session, IpInfo *ipInfo, TcpInfo *tcpInfo, word32 rowHint)
{
...
static SnifferSession *CreateSession(IpInfo *ipInfo, TcpInfo *tcpInfo, char *error)
{
...
static int DoOldHello(SnifferSession *session, const byte *sslFrame, int *rhSize, int *sslBytes,
                      char *error)
{
    ...
    *rhSize = ((b0 & 0x7f) << 8) | b1;
    ...
    ret = ProcessOldClientHello(session->sslServer, input, &idx, *sslBytes, (word16)*rhSize);
    ...
    XMEMCPY(session->sslClient->arrays->clientRandom, session->sslServer->arrays->clientRandom,
            RAN_LEN);
...
/* returns 0 on success, -1 on error */
static int CheckHeaders(IpInfo *ipInfo, TcpInfo *tcpInfo, const byte *packet, int length,
                        const byte **sslFrame, int *sslBytes, char *error)
{
    TraceHeader();
    TracePacket();
...
static int CheckSession(IpInfo *ipInfo, TcpInfo *tcpInfo, int sslBytes, SnifferSession **session,
                        char *error)
{
...
static PacketBuffer *CreateBuffer(word32 *begin, word32 end, const byte *data, int *bytesLeft)
{
    PacketBuffer *pb;
...
static int AddToReassembly(byte from, word32 seq, const byte *sslFrame, int sslBytes,
                           SnifferSession *session, char *error)
{
    PacketBuffer *add;
    PacketBuffer **front = (from == WOLFSSL_SERVER_END) ? &session->cliReassemblyList :
    ...
    while (bytesLeft > 0) {
        ...
        add = CreateBuffer(&seq, seq + added - 1, &sslFrame[seq - startSeq], &bytesLeft);
        if (add == NULL) {
            SetError(MEMORY_STR, error, session, FATAL_ERROR_STATE);
            return -1;
        }
...
/* returns 0 on success (continue), -1 on error, 1 on success (end) */
static int AdjustSequence(TcpInfo *tcpInfo, SnifferSession *session, int *sslBytes,
                          const byte **sslFrame, char *error)
{
    word32 seqStart = (session->flags.side == WOLFSSL_SERVER_END) ? session->cliSeqStart :
                                                                    session->srvSeqStart;
    word32 real = tcpInfo->sequence - seqStart;
    word32 *expected = (session->flags.side == WOLFSSL_SERVER_END) ? &session->cliExpected :
                                                                     &session->srvExpected;
    ...
    if (real < *expected) {
        ...
        if (*sslBytes > 0)
            return AddToReassembly(session->flags.side, real, *sslFrame, *sslBytes, session, error);
        else if (tcpInfo->fin)
...
{
    if (tcpInfo->ack) {
        word32 seqStart = (session->flags.side == WOLFSSL_SERVER_END) ? session->srvSeqStart :
                                                                        session->cliSeqStart;
        word32 real = tcpInfo->ackNumber - seqStart;
        word32 expected = (session->flags.side == WOLFSSL_SERVER_END) ? session->srvExpected :
                                                                        session->cliExpected;
...
/* returns 0 on success (continue), -1 on error, 1 on success (end) */
static int CheckSequence(IpInfo *ipInfo, TcpInfo *tcpInfo, SnifferSession *session, int *sslBytes,
                         const byte **sslFrame, char *error)
{
    int actualLen;
...
static int CheckPreRecord(IpInfo *ipInfo, TcpInfo *tcpInfo, const byte **sslFrame,
                          SnifferSession **session, int *sslBytes, const byte **end, char *error)
{
    word32 length;
    SSL *ssl = ((*session)->flags.side == WOLFSSL_SERVER_END) ? (*session)->sslServer :
                                                                (*session)->sslClient;
    ...
    if ((*session)->flags.clientHello == 0 && **sslFrame != handshake) {
        ...
        if ((rhSize <= (*sslBytes - 2)) && (*sslFrame)[2] == OLD_HELLO_ID &&
...
static int HaveMoreInput(SnifferSession *session, const byte **sslFrame, int *sslBytes,
...
static int ProcessMessage(const byte *sslFrame, SnifferSession *session, int sslBytes, byte *data,
                          const byte *end, char *error)
{
...
doMessage:
    ...
    /* decrypt if needed */
    if ((session->flags.side == WOLFSSL_SERVER_END && session->flags.serverCipherOn) ||
        (session->flags.side == WOLFSSL_CLIENT_END && session->flags.clientCipherOn)) {
        ...
        sslFrame = DecryptMessage(ssl, sslFrame, rhSize, ssl->buffers.outputBuffer.buffer, &errCode,
                                  &ivAdvance);
        ...
        Trace(GOT_APP_DATA_STR);
        {
            ...
            if (ret == 0) {
                ...
                if (ret) { /* may be blank message */
                    XMEMCPY(&data[decoded], ssl->buffers.clearOutputBuffer.buffer, ret);
...
static void CheckFinCapture(IpInfo *ipInfo, TcpInfo *tcpInfo, SnifferSession *session)
{
    if (session->finCaputre.cliFinSeq && session->finCaputre.cliFinSeq <= session->cliExpected) {
        if (session->finCaputre.cliCounted == 0) {
            session->flags.finCount += 1;
    ...
    if (session->finCaputre.srvFinSeq && session->finCaputre.srvFinSeq <= session->srvExpected) {
        if (session->finCaputre.srvCounted == 0) {
            session->flags.finCount += 1;
...
static int RemoveFatalSession(IpInfo *ipInfo, TcpInfo *tcpInfo, SnifferSession *session,
                              char *error)
{
...
int ssl_DecodePacket(const byte *packet, int length, byte *data, char *error)
{
    ...
    if (CheckHeaders(&ipInfo, &tcpInfo, packet, length, &sslFrame, &sslBytes, error) != 0)
        return -1;
    ...
    ret = CheckPreRecord(&ipInfo, &tcpInfo, &sslFrame, &session, &sslBytes, &end, error);
    if (RemoveFatalSession(&ipInfo, &tcpInfo, session, error))
        return -1;
    else if (ret == -1)
        return -1;
