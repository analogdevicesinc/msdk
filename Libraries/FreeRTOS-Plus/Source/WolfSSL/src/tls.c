...
static int p_hash(byte *result, word32 resLen, const byte *secret, word32 secLen, const byte *seed,
                  word32 seedLen, int hash)
{
            ...
            if ((ret = wc_HmacFinal(hmac, previous)) == 0) { /* A1 */
                for (i = 0; i < times; i++) {
                    ...
                    if ((i == lastTime) && lastLen)
                        XMEMCPY(&result[idx], current, min(lastLen, P_HASH_MAX_SIZE));
                    else {
...
static INLINE void get_xor(byte *digest, word32 digLen, byte *md5, byte *sha)
{
    ...
    for (i = 0; i < digLen; i++) digest[i] = md5[i] ^ sha[i];
}

/* compute TLSv1 PRF (pseudo random function using HMAC) */
static int doPRF(byte *digest, word32 digLen, const byte *secret, word32 secLen, const byte *label,
                 word32 labLen, const byte *seed, word32 seedLen)
{
    int ret = 0;
    ...
    if (md5_half == NULL || sha_half == NULL || labelSeed == NULL || md5_result == NULL ||
        sha_result == NULL) {
        if (md5_half)
    ...
    if ((ret = p_hash(md5_result, digLen, md5_half, half, labelSeed, labLen + seedLen, md5_mac)) ==
        0) {
        if ((ret = p_hash(sha_result, digLen, sha_half, half, labelSeed, labLen + seedLen,
                          sha_mac)) == 0) {
...
/* Wrapper to call straight thru to p_hash in TSL 1.2 cases to remove stack
   use */
static int PRF(byte *digest, word32 digLen, const byte *secret, word32 secLen, const byte *label,
               word32 labLen, const byte *seed, word32 seedLen, int useAtLeastSha256, int hash_type)
{
    int ret = 0;
    ...
    if (useAtLeastSha256) {
        ...
        ret = p_hash(digest, digLen, secret, secLen, labelSeed, labLen + seedLen, hash_type);
        ...
        ret = doPRF(digest, digLen, secret, secLen, label, labLen, seed, seedLen);
    }
#endif
...
int BuildTlsFinished(WOLFSSL *ssl, Hashes *hashes, const byte *sender)
{
    ...
    return PRF((byte *)hashes, TLS_FINISHED_SZ, ssl->arrays->masterSecret, SECRET_LEN, side,
               FINISHED_LABEL_SZ, handshake_hash, hashSz, IsAtLeastTLSv1_2(ssl),
...
int wolfSSL_DeriveTlsKeys(byte *key_data, word32 keyLen, const byte *ms, word32 msLen,
                          const byte *sr, const byte *cr, int tls1_2, int hash_type)
{
    byte seed[SEED_LEN];
    ...
    return PRF(key_data, keyLen, ms, msLen, key_label, KEY_LABEL_SZ, seed, SEED_LEN, tls1_2,
               hash_type);
}
...
int DeriveTlsKeys(WOLFSSL *ssl)
{
    ...
    ret = wolfSSL_DeriveTlsKeys(key_data, length, ssl->arrays->masterSecret, SECRET_LEN,
...
/* External facing wrapper so user can call as well, 0 on success */
int wolfSSL_MakeTlsMasterSecret(byte *ms, word32 msLen, const byte *pms, word32 pmsLen,
                                const byte *cr, const byte *sr, int tls1_2, int hash_type)
{
    byte seed[SEED_LEN];
    ...
    return PRF(ms, msLen, pms, pmsLen, master_label, MASTER_LABEL_SZ, seed, SEED_LEN, tls1_2,
               hash_type);
}
...
int MakeTlsMasterSecret(WOLFSSL *ssl)
{
    ...
    if (ret == 0) {
        ...
        for (i = 0; i < SECRET_LEN; i++) printf("%02x", ssl->arrays->masterSecret[i]);
        printf("\n");
#endif
...
int wolfSSL_make_eap_keys(WOLFSSL *ssl, void *msk, unsigned int len, const char *label)
{
    ...
    ret = PRF((byte *)msk, len, ssl->arrays->masterSecret, SECRET_LEN, (const byte *)label,
              (word32)strlen(label), seed, SEED_LEN, IsAtLeastTLSv1_2(ssl),
              ssl->specs.mac_algorithm);
...
/* TLS type HMAC */
int TLS_hmac(WOLFSSL *ssl, byte *digest, const byte *in, word32 sz, int content, int verify)
{
    Hmac hmac;
    int ret;
    ...
    ret = wc_HmacSetKey(&hmac, wolfSSL_GetHmacType(ssl), wolfSSL_GetMacSecret(ssl, verify),
                        ssl->specs.hash_size);
    if (ret != 0)
        return ret;
...
#define IS_OFF(semaphore, light) ((semaphore)[(light) / 8] ^ (byte)(0x01 << ((light) % 8)))

#define TURN_ON(semaphore, light) ((semaphore)[(light) / 8] |= (byte)(0x01 << ((light) % 8)))
...
static int TLSX_SNI_Parse(WOLFSSL *ssl, byte *input, word16 length, byte isRequest)
{
#ifndef NO_WOLFSSL_SERVER
    word16 size = 0;
    word16 offset = 0;
...
int TLSX_UseSNI(TLSX **extensions, byte type, const void *data, word16 size)
{
    ...
    if (!extension) {
        if ((ret = TLSX_Push(extensions, SERVER_NAME_INDICATION, (void *)sni)) != 0) {
            TLSX_SNI_Free(sni);
            return ret;
        }
    } else {
...
int TLSX_SNI_GetFromBuffer(const byte *clientHello, word32 helloSz, byte type, byte *sni,
                           word32 *inOutSz)
{
    word32 offset = 0;
    word32 len32 = 0;
...
static int TLSX_MFL_Parse(WOLFSSL *ssl, byte *input, word16 length, byte isRequest)
{
    if (length != ENUM_LEN)
        return BUFFER_ERROR;
...
static int TLSX_THM_Parse(WOLFSSL *ssl, byte *input, word16 length, byte isRequest)
{
...
static int TLSX_EllipticCurve_Parse(WOLFSSL *ssl, byte *input, word16 length, byte isRequest)
{
    word16 offset;
    word16 name;
    int r;
...
{
    TLSX *extension = (first == ECC_BYTE) ? TLSX_Find(ssl->extensions, ELLIPTIC_CURVES) : NULL;
...
static int TLSX_SecureRenegotiation_Parse(WOLFSSL *ssl, byte *input, word16 length, byte isRequest)
{
...
int TLSX_UseSecureRenegotiation(TLSX **extensions)
{
    ...
    data = (SecureRenegotiation *)XMALLOC(sizeof(SecureRenegotiation), NULL, DYNAMIC_TYPE_TLSX);
    if (data == NULL)
        return MEMORY_E;
...
static int TLSX_SessionTicket_Parse(WOLFSSL *ssl, byte *input, word16 length, byte isRequest)
{
    int ret = 0;
...
WOLFSSL_LOCAL SessionTicket *TLSX_SessionTicket_Create(word32 lifetime, byte *data, word16 size)
{
    SessionTicket *ticket =
        (SessionTicket *)XMALLOC(sizeof(SessionTicket), NULL, DYNAMIC_TYPE_TLSX);
    if (ticket) {
...
TLSX *TLSX_Find(TLSX *list, TLSX_Type type)
{
    ...
    while (extension && extension->type != type) extension = extension->next;
...
static word16 TLSX_Write(TLSX *list, byte *output, byte *semaphore, byte isRequest)
{
    TLSX *extension;
    word16 offset = 0;
    ...
    while ((extension = list)) {
        ...
        case SECURE_RENEGOTIATION:
            offset += SCR_WRITE(extension->data, output + offset, isRequest);
            break;

        case SESSION_TICKET:
            offset += STK_WRITE(extension->data, output + offset, isRequest);
            break;
        }
...
word16 TLSX_WriteRequest(WOLFSSL *ssl, byte *output)
{
    ...
    if (TLSX_SupportExtensions(ssl) && output) {
        ...
        if (ssl->extensions)
            offset += TLSX_Write(ssl->extensions, output + offset, semaphore, 1);

        if (ssl->ctx && ssl->ctx->extensions)
            offset += TLSX_Write(ssl->ctx->extensions, output + offset, semaphore, 1);
...
int TLSX_Parse(WOLFSSL *ssl, byte *input, word16 length, byte isRequest, Suites *suites)
{
    int ret = 0;
    word16 offset = 0;
