...
#endif
#include <fcntl.h>
#if !(defined(DEVKITPRO) || defined(HAVE_RTP_SYS) || defined(EBSNET)) && !(defined(WOLFSSL_PICOTCP))
#include <sys/socket.h>
#include <arpa/inet.h>
...
int EmbedSendTo(WOLFSSL *ssl, char *buf, int sz, void *ctx)
{
    ...
    sent = (int)SENDTO_FUNCTION(sd, &buf[sz - len], len, ssl->wflags,
                                (const struct sockaddr *)dtlsCtx->peer.sa, dtlsCtx->peer.sz);
    if (sent < 0) {
        err = LastError();
...
static int build_http_request(const char *domainName, const char *path, int ocspReqSz, byte *buf,
                              int bufSize)
{
...
static int decode_url(const char *url, int urlSz, char *outName, char *outPath, word16 *outPort)
{
    int result = -1;
        ...
        if (url == NULL || urlSz == 0) {
            ...
            *outPort = 0;
            ...
            if (url[cur] == '[') {
                ...
                while (url[cur] != 0 && url[cur] != ':' && url[cur] != '/' && cur < urlSz) {
                    outName[i++] = url[cur++];
                }
            }
            outName[i] = 0;
            ...
            if (cur < urlSz && url[cur] == ':') {
                ...
                i = 0;
                cur++;
                while (cur < urlSz && url[cur] != 0 && url[cur] != '/' && i < 6) {
                    port[i++] = url[cur++];
                }
 ...
 *         -1 error */
static int process_http_response(int sfd, byte **respBuf, byte *httpBuf, int httpBufSz)
{
    int result;
    int len = 0;
    char *start, *end;
        ...
        if (end == NULL) {
            ...
            *end = 0;
            ...
            if (XSTRNCASECMP(start, "HTTP/1", 6) == 0) {
                start += 9;
                if (XSTRNCASECMP(start, "200 OK", 6) != 0 || state != phr_init) {
                ...
                if (state == phr_http_start)
                    state = phr_have_type;
                else if (state == phr_have_length)
                    state = phr_wait_end;
                else {
                ...
                if (state == phr_http_start)
                    state = phr_have_length;
                else if (state == phr_have_type)
                    state = phr_wait_end;
                else {
...
int EmbedOcspLookup(void *ctx, const char *url, int urlSz, byte *ocspReqBuf, int ocspReqSz,
                    byte **ocspRespBuf)
{
    SOCKET_T sfd = 0;
    word16 port;
    ...
    if (ocspReqBuf == NULL || ocspReqSz == 0) {
        ...
        byte *httpBuf = (byte *)XMALLOC(httpBufSz, NULL, DYNAMIC_TYPE_IN_BUFFER);

        if (httpBuf == NULL) {
            WOLFSSL_MSG("Unable to create OCSP response buffer");
        } else {
            httpBufSz = build_http_request(domainName, path, ocspReqSz, httpBuf, httpBufSz);

            if ((tcp_connect(&sfd, domainName, port) != 0) || (sfd <= 0)) {
                WOLFSSL_MSG("OCSP Responder connection failed");
            } else if ((int)send(sfd, (char *)httpBuf, httpBufSz, 0) != httpBufSz) {
                WOLFSSL_MSG("OCSP http request failed");
            } else if ((int)send(sfd, (char *)ocspReqBuf, ocspReqSz, 0) != ocspReqSz) {
                WOLFSSL_MSG("OCSP ocsp request failed");
            } else {
                ret = process_http_response(sfd, ocspRespBuf, httpBuf, SCRATCH_BUFFER_SIZE);
            }
...
int NetX_Receive(WOLFSSL *ssl, char *buf, int sz, void *ctx)
{
    ...
    if (nxCtx->nxPacket == NULL) {
        status = nx_tcp_socket_receive(nxCtx->nxSocket, &nxCtx->nxPacket, nxCtx->nxWait);
    ...
    if (nxCtx->nxPacket) {
        ...
        status = nx_packet_data_extract_offset(nxCtx->nxPacket, nxCtx->nxOffset, buf, sz, &copied);
        if (status != NX_SUCCESS) {
