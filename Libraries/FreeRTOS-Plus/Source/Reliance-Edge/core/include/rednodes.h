...
#define METAROOT_HEADER_SIZE \
    (NODEHEADER_SIZE + 16U) /* Size in bytes of the metaroot header fields. */
#else
#define METAROOT_HEADER_SIZE \
    (NODEHEADER_SIZE + 12U) /* Size in bytes of the metaroot header fields. */
#endif
#define METAROOT_ENTRY_BYTES \
    (REDCONF_BLOCK_SIZE -    \
     METAROOT_HEADER_SIZE) /* Number of bytes remaining in the metaroot block for entries. */
...
#define IMAPNODE_ENTRY_BYTES \
    (REDCONF_BLOCK_SIZE -    \
     IMAPNODE_HEADER_SIZE) /* Number of bytes remaining in the imap node for entries. */
...
#define INODE_ENTRIES ((REDCONF_BLOCK_SIZE - INODE_HEADER_SIZE) / 4U)
    ...
    (REDCONF_DIRECT_POINTERS > (INODE_ENTRIES - REDCONF_INDIRECT_POINTERS))
#error "Configuration error: invalid value of REDCONF_DIRECT_POINTERS"
#endif
#if (REDCONF_INDIRECT_POINTERS < 0) || \
    (REDCONF_INDIRECT_POINTERS > (INODE_ENTRIES - REDCONF_DIRECT_POINTERS))
