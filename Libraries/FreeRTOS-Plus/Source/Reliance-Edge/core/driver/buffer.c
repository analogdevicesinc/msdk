...
#define BFLAG_META_MASK                                                           \
    (uint16_t)((uint32_t)BFLAG_META_MASTER | BFLAG_META_IMAP | BFLAG_META_INODE | \
               BFLAG_META_INDIR | BFLAG_META_DINDIR)
...
REDSTATUS RedBufferFlush(uint32_t ulBlockStart, uint32_t ulBlockCount)
{
    ...
    if ((ulBlockStart >= gpRedVolume->ulBlockCount) ||
        ((gpRedVolume->ulBlockCount - ulBlockStart) < ulBlockCount) || (ulBlockCount == 0U)) {
        REDERROR();
        ret = -RED_EINVAL;
    } else {
        uint8_t bIdx;
...
static bool BufferToIdx(const void *pBuffer, uint8_t *pbIdx)
{
    ...
    if ((pBuffer != NULL) && (pbIdx != NULL)) {
        ...
        */
        ...
        if ((bIdx < REDCONF_BUFFER_COUNT) && (gBufCtx.aHead[bIdx].ulBlock != BBLK_INVALID) &&
