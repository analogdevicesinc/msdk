 ...
 * Elliptic Curve Integrated Encryption Scheme, or ECIES, is a hybrid encryption system. ECIES has
 been standardized in ANSI X9.63, IEEE 1363a, ISO/IEC 18033-2, and SECG SEC-1.
 * ECIES combines a Key Encapsulation Mechanism (KEM) with a Data Encapsulation Mechanism (DEM). The
 system independently derives a bulk encryption key and a MAC key from a common secret. Data is
 first encrypted under a symmetric cipher, and then the cipher text is MAC'd under an authentication
 scheme. Finally, the common secret is encrypted under the public part of a public/private key pair.
 The output of the encryption function is the tuple {K,C,T}, where K is the encrypted common secret,
 C is the ciphertext, and T is the authentication tag. There is some hand waiving around the "common
 secret" since its actually the result of applying a Key Agreement function, and it uses the static
 public key and an ephemeral key pair (Wikipedia)
 *
 * @ingroup UCL_ECC
 */
...
int ucl_ecies_encrypt_p192r1_aes_hmac_sha256(unsigned char* rx, unsigned char* ry,
    unsigned char* crypto, unsigned char* t, unsigned int keylength, unsigned char* xG,
    unsigned char* yG, unsigned char* xQ, unsigned char* yQ, unsigned char* a, unsigned char* n,
...
int ucl_ecies_decrypt_p192r1_aes_hmac_sha256(unsigned char* m, unsigned int keylength,
    unsigned char* xG, unsigned char* yG, unsigned char* a, unsigned char* n, unsigned char* p,
    unsigned char* d, unsigned char* rx, unsigned char* ry, unsigned char* crypto, int crypto_len,
    unsigned char* t);
/** <b>ECIES encryption</b>.
...
int ucl_ecies_encrypt_aes_hmac_sha256(ucl_type_ecc_u8_affine_point Q, u8* crypto, u8* t,
    ucl_type_ecc_u8_affine_point pubkey, u8* input, u32 inputlength, ucl_type_curve* curve_params);
...
int ucl_ecies_decrypt_aes_hmac_sha256(u8* message, u8* d, ucl_type_ecc_u8_affine_point pubkey,
    u8* crypto, int cryptolength, u8* t, ucl_type_curve* curve_params);
