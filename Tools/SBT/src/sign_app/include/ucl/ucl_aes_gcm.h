 ...
 * @retval #UCL_INVALID_INPUT  one of the input is the pointer NULL or key length does not match or
 * mode does not match
 *
 * @ingroup UCL_GCM_AES
 */
 ...
 * @param[out]  c: the result u8 array representing the output (crypto in encryption, plain in
decryption); its size is the same than the plaintext one
 * @param[in] p: the input u8 array representing the input (plain in encryption, crypto in
decryption);
 * @param[in] input_bit_length_high, input_bit_len_low: two words representing the bit size of the
input;the  maximum value is 2^39-256
 *  @param[in] aad: the input u8 array representing the add.authenticated data
 *  @param[in] aad_bit_length_high, aad_bit_len_low: two words representing the bit size of the AAD;
...
the maximum value is 2^64
 *  @param[in] mode: UCL_CIPHER_ENCRYPT or UCL_CIPHER_DECRYPT; determines if p is encrypted or
decrypted
 *  @return Error code
 *
 * @retval #UCL_OK             no error occurred
...
int ucl_aes_gcm_auth(u8* auth_tag, int auth_tag_bit_len, u8* c, u8* p, int input_bit_len_high,
    int input_bit_len_low, u8* aad, int aad_bit_len_high, int aad_bit_len_low, u8* key, u8* iv,
    int iv_bit_len_high, int iv_bit_len_low, int mode);
/** <b>AES-GMAC</b>.
...
int ucl_aes_gmac_auth(u8* auth_tag, int auth_tag_bit_len, u8* aad, int aad_bit_len_high,
    int aad_bit_len_low, u8* key, u8* iv, int iv_bit_len_high, int iv_bit_len_low);
...
int ucl_aes_gcm(u8* auth_tag, int auth_tag_bit_len, u8* c, u8* p, int input_bit_len_high,
    int input_bit_len_low, u8* aad, int aad_bit_len_high, int aad_bit_len_low, u8* key, u32 keylen,
    u8* iv, int iv_bit_len_high, int iv_bit_len_low, int mode);
/** <b>AES-GMAC</b>.
 * computes the GMAC data (tag) from authenticated add. data
 * @param[out]  auth_tag: result, u8 array representing the tag
 * @param[in]   auth_tag_bit_len: number of bits of the result tag; values between 64 and 128
...
int ucl_aes_gmac(u8* auth_tag, int auth_tag_bit_len, u8* aad, int aad_bit_len_high,
    int aad_bit_len_low, u8* key, u32 keylen, u8* iv, int iv_bit_len_high, int iv_bit_len_low);

void ucl_gcm_ghash_core(u8* x, u8* h, u8* c, int n, int u, int m, int v);
int ucl_gcm_ghash_finish(
    u8* x, u8* h, u8* c, int n, int u, int m, int v, int input_bit_len_high, int input_bit_len_low);

/** <b>AES-GCM multi-block initialization</b>.
 * the AES-GCM multi-block processing defines three steps: initialization, blocks processing, final
processing
 ...
 * the AES-GCM multi-block processing defines three steps: initialization, blocks core processing,
 final processing
 * this function is the blocks core processing
 * this function handles only full AES blocks, i.e. a multiple of 128 bits
 * if the input length is a multiple of 128 bits, the last block shall not be processed in this core
 processing but in the final processing
 * @param[out]  res: ghash intermediate value
 * @param[in] auth_tag_bit_len: the length of the tag, in bits (usually 128 bits)
 * @param[out]  c: output of the AES processing (either encryption or decryption, determined by
 mode)
 * @param[in]  p: input of the AES processing
 *  @param[in] input_bit_length_high, input_bit_len_low: two words representing the bit size of the
 input (p); the maximum value is 2^64; it has to correspond to a multiple of 128 bits (i.e. full AES
 blocks)
 *  @param[in] aad_bit_length_high, aad_bit_len_low: two words representing the bit size of the AAD;
 the maximum value is 2^64
...
int ucl_aes_gcm_core(u8* res, int auth_tag_bit_len, u8* c, u8* p, int input_bit_len_high,
    int input_bit_len_low, int aad_bit_len_high, int aad_bit_len_low, u8* h, u8* y, u8* key,
    u32 keylen, int mode);
 ...
 * @param[in]  p: input of the AES processing
 *  @param[in] input_bit_length_high, input_bit_len_low: two words representing the bit size of the
 input (p); the maximum value is 2^64; this can not be a zero length so even the input size is a
 multiple of 128 bits, the last block shall be processed within this final processing and not in the
 core processing
 *  @param[in] inputtot_bit_length_high, inputtot_bit_len_low: two words representing the bit size
 of all the input data, so including the blocks transferred in the core processing and the last bits
 transferred in the final processing; the maximum value is 2^64
 *  @param[in] aad_bit_length_high, aad_bit_len_low: two words representing the bit size of the AAD;
 the maximum value is 2^64
 * @param[in]  h: encryption of the null vector
...
int ucl_aes_gcm_finish(u8* auth_tag, int auth_tag_bit_len, u8* res, u8* c, u8* p,
    ...
    int inputtot_bit_len_low, int aad_bit_len_high, int aad_bit_len_low, u8* h, u8* eky0, u8* y,
