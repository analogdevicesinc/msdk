...
if __name__ == "__main__":
    ...
    if result.returncode != 0:
        print(
            f"Failed to run {command} with error {result.returncode}:\n{result.stderr.decode(encoding='utf-8')}"
        )
    ...
    version_year = int(
        version.split("_")[0][1:]
    )  # Split the year and month based on the '_', and strip off the leading 'v'
    version_minor = version.split("_")[1].split("-")[0]
    version_month = (
        int(version_minor[0]) if isinstance(version_minor, list) else int(version_minor)
    )
