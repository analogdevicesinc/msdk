...
if [[ -f "me13_register_name.txt" ]]; then
	rm me13_register_name.txt
fi
...
mkdir chip_test
cp empty_template.svd chip_test/max32570.svd
...
python3 ../../svd_add_peripheral_modified.py chip_periph.txt chip_test/max32570.svd ../../../../../../
error=$?
if [[ $error -ne 0 ]]; then
...
xmlformat -i chip_test/max32570.svd
...
python3 ../../svd_reg3.py chip_test/max32570.svd chip_test me13_register_name.txt -r
error=$?
if [[ $error -ne 0 ]]; then
...
if [[ $1 == "windows" || -z $1 ]]; then
	../../SVDConv.exe chip_test/max32570.svd -b svdconv_error_log.txt
fi

# The ME13 examples uses the Proprietary Copyright Notice. Update register files to proprietary notice too.
python3 ../../../Scripts/Copyright/update_copyright.py -r chip_test/
error=$?
if [[ $error -ne 0 ]]; then
	echo "[ERROR] update_copyright.py: Could update copyright notices."
	exit 1
fi
