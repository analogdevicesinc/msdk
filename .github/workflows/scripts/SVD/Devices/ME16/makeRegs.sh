...
# Note: This part references some of the ME15's peripheral SVD files.

if [[ -d "chip_test" ]]; then
	rm -rf chip_test
fi

if [[ -f "me16_register_name.txt" ]]; then
	rm me16_register_name.txt
fi
...
mkdir chip_test
cp empty_template.svd chip_test/max32675.svd
...
python3 ../../svd_add_peripheral_modified.py chip_periph.txt chip_test/max32675.svd ../../../../../../
error=$?
if [[ $error -ne 0 ]]; then
...
xmlformat -i chip_test/max32675.svd
...
python3 ../../svd_reg3.py chip_test/max32675.svd chip_test me16_register_name.txt -r
error=$?
if [[ $error -ne 0 ]]; then
...
do
	...
	python3 ../../svd_removal.py chip_test/max32675.svd -sp ${afe_file}
	error=$?
	if [[ $error -ne 0 ]]; then
...
if [[ $1 == "windows" || -z $1 ]]; then
	../../SVDConv.exe chip_test/max32675.svd -b svdconv_error_log.txt
fi
