...
if [[ -f "me18_register_name.txt" ]]; then
	rm me18_register_name.txt
fi
...
mkdir chip_test
cp empty_template.svd chip_test/max32690.svd
...
python3 ../../svd_add_peripheral_modified.py chip_periph.txt chip_test/max32690.svd ../../../../../../
error=$?
if [[ $error -ne 0 ]]; then
...
xmlformat -i chip_test/max32690.svd
...
python3 ../../svd_reg3.py chip_test/max32690.svd chip_test me18_register_name.txt -r
error=$?
if [[ $error -ne 0 ]]; then
...
if [[ $1 == "windows" || -z $1 ]]; then
	../../SVDConv.exe chip_test/max32690.svd -b svdconv_error_log.txt
fi
