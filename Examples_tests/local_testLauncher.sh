...
# Target under test
export CMSIS_DAP_ID_1=0409170246dfc09500000000000000000000000097969906
export TARGET_1_LC=max32655
export TARGET_1_UC=MAX32655
export devSerial_1=/dev/"$(ls -la /dev/serial/by-id | grep -n 'D309ZDFO' | rev | cut -b 1-7 | rev)"
export TARGET_1_CFG=${TARGET_1_LC}.cfg
# Helper device for connected tests
export CMSIS_DAP_ID_2=04090000bdf10c0400000000000000000000000097969906
export devSerial_2=/dev/"$(ls -la /dev/serial/by-id | grep -n 'D309ZDEM' | rev | cut -b 1-7 | rev)"
